...
def _ensure_python_exe(python_exe_path: Path):
    
    
    
    
    if not python_exe_path.exists():
        python3_exe_path = python_exe_path.with_name("python3")

        if _is_windows:
            python3_exe_path = python3_exe_path.with_suffix(".exe")

        if not python3_exe_path.exists():
            raise Exception(
...
def _assert_pip_check(pthfile_lines, virtualenv_name, requirements):
    ...
    with tempfile.TemporaryDirectory() as check_env_path:
        ...
        _ensure_python_exe(Path(check_env.python_path))
...
def _create_venv_with_pthfile(
    ...
    _ensure_python_exe(Path(target_venv.python_path))
