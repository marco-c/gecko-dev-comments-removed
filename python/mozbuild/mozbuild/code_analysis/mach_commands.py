...
def check_java(
    ...
    if command_context.substs["MOZ_BUILD_APP"] != "mobile/android":
        ...
        return 1
    rc = _check_for_java(command_context)
    if rc != 0:
        return 1
    if output is not None:
    ...
    if outgoing:
        repo = get_repository_object(command_context.topsrcdir)
        java_sources = _get_java_files(command_context, repo.get_outgoing_files())
        if not java_sources:
            command_context.log(
                logging.WARNING,
                "static-analysis",
        ...
        java_sources = _get_java_files(command_context, source)
        if not java_sources:
            return 0
    if not skip_export:
        ...
        if rc != 0:
            return rc
    rc, infer_path = _get_infer(command_context, verbose=verbose)
    if rc != 0:
        command_context.log(
            logging.WARNING,
            "static-analysis",
    ...
    all_checkers, third_party_path, generated_path = _get_infer_config(command_context)
    checkers, excludes = _get_infer_args(
        checks or all_checkers, third_party_path, generated_path
    )
    rc = rc or _gradle(command_context, ["clean"])  
    
    capture_cmd = [infer_path, "capture"] + excludes + ["--"]
    rc = rc or _gradle(command_context, [task], infer_args=capture_cmd, verbose=verbose)
    tmp_file, args = _get_infer_source_args(java_sources)
    ...
    report_path = os.path.join(command_context.topsrcdir, "infer-out", "report.json")
...
def _get_java_files(command_context, sources):
    java_sources = []
    for i in sources:
...
def _get_infer_source_args(sources):
...
def _get_infer_config(command_context):
...
def _get_infer_args(checks, *input_paths):
...
def _check_for_java(command_context):
...
def _autotest_infer(
    ...
    if command_context.platform[0] == "linux64":
        ...
        rc = _gradle(command_context, ["autotest:clean"], autotest=True)
        if rc != 0:
            return rc
        import yaml
...
def _verify_infer_checker(
    ...
    rc = _gradle(command_context, gradle_args, infer_args=infer_args, autotest=True)
    if rc != 0:
        command_context.log(
            logging.ERROR,
            "static-analysis",
    ...
    if not issues:
        command_context.log(
            ...
            {},
            "ERROR: infer check "
            "{0} did not find any issues in its associated test suite.".format(check),
        )
...
def install(
    ...
    if rc == 0 and not minimal_install:
        ...
        _get_infer(command_context, force=force, skip_cache=skip_cache, verbose=verbose)
    return rc
...
def print_checks(command_context, verbose=False):
    ...
    checkers, _, _ = _get_infer_config(command_context)
...
def check_syntax(command_context, source, verbose=False):
    command_context._set_log_level(verbose)
    command_context.log_manager.enable_unstructured()
    ...
    if rc != 0:
        ...
        return rc
    rc = _build_export(command_context, jobs=2, verbose=verbose)
    ...
    path_list = _generate_path_list(command_context, source)
    ...
    for file in path_list:
        ...
        if ext.lower() not in _check_syntax_include_extensions:
            command_context.log(
