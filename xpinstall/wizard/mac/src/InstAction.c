...
{	
  ...
  ourHZ = GetZone();
	...
	if (!gControls->opt->saveBits  && !bXPIsExisted)
		DeleteXPIs(srcVRefNum, srcDirID);  
...
DownloadFile(Handle destFolder, long destFolderLen, Handle archive, int resPos, int urlIndex, CONN *myConn)
{
    ...
    URL = (char *) malloc(globalURLLen + archiveLen + 1); 
    ...
    HLock(destFolder);
    destFolderCopy = (char *) malloc(destFolderLen + 1);  
    if (! destFolderCopy)
    {    
        HUnlock(destFolder);
        return eMem;
    ...
    *(destFolderCopy + destFolderLen) = 0;
    ...
    if (gControls->opt->proxyHost && gControls->opt->proxyPort)
    {
        ...
        isNewConn = CheckConn( proxyServerURL, TYPE_PROXY, myConn, false ); 
    ...
    else if (strncmp(URL, kHTTP, strlen(kHTTP)) == 0)
    {
        ...
        isNewConn = CheckConn( URL, TYPE_HTTP, myConn, false ); 
    ...
    else if (strncmp(URL, kFTP, strlen(kFTP)) == 0)
    {
        ...
        else
        {
        	...
        	isNewConn = CheckConn( ftpHost, TYPE_FTP, myConn, false ); 
...
{   
    ...
    int len;
    char compressedStr[kProgMsgLen + 1];  
    char *fullPathCopy = 0; 
    float rate = 0;
    time_t now;
    Rect teRect;
...
AddKeyToIDI(short key, Handle val, char *ostream)
{
	...
	strncat(ostream, keybuf, strlen(keybuf));	
	ch = '=';
	strncat(ostream, &ch, 1);					
	strncat(ostream, cval, strlen(cval));		
	ch = '\r';
	strncat(ostream, &ch, 1);					
...
InitProgressBar(void)
{
	...
	if (gWPtr)
	{
		...
		if (gControls->tw->allProgressBar && gControls->tw->xpiProgressBar)
		{
			...
			gControls->tw->xpiProgressMsg = NULL;	
