                        ...
                        const int16_t *band_counts, const int16_t *qmul)
{
    ...
    do {
        ...
        val = vpx_rac_get_prob_branchy(c, tp[0]); 
        eob[band][nnz][val]++;
        if (!val)
            break;

skip_eob:
        if (!vpx_rac_get_prob_branchy(c, tp[1])) { 
            cnt[band][nnz][0]++;
            if (!--band_left)
        ...
        rc = scan[i];
        if (!vpx_rac_get_prob_branchy(c, tp[2])) { 
            cnt[band][nnz][1]++;
            val       = 1;
            cache[rc] = 1;
        } else {
            cnt[band][nnz][2]++;
            if (!vpx_rac_get_prob_branchy(c, tp[3])) { 
            ...
            } else if (!vpx_rac_get_prob_branchy(c, tp[6])) { 
                cache[rc] = 4;
...
static av_always_inline int decode_coeffs(VP9TileData *td, int is8bitsperpixel)
{
    ...
    const uint8_t (*p)[6][11] = s->prob.coef[b->tx][0 ][!b->intra];
