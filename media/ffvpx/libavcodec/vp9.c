                               ...
                               const uint8_t *data, int size, int *ref)
{
    ...
    if (s->s.h.keyframe) {
        if (get_bits(&s->gb, 24) != VP9_SYNCCODE) { 
    ...
    if (vpx_rac_get_prob_branchy(&s->c, 128)) { 
                        ...
                        const uint8_t *data, int size)
{
    ...
    for (tile_row = 0; tile_row < s->s.h.tiling.tile_rows; tile_row++) {
        ...
        for (tile_col = 0; tile_col < s->s.h.tiling.tile_cols; tile_col++) {
            ...
            if (ret < 0)
                return ret;
            if (vpx_rac_get_prob_branchy(&td->c_b[tile_col], 128)) 
                return AVERROR_INVALIDDATA;
            data += tile_size;
            size -= tile_size;
        }
        ...
        if (avctx->active_thread_type == FF_THREAD_SLICE) {
            ...
            for (tile_row = 0; tile_row < s->s.h.tiling.tile_rows; tile_row++) {
                for (tile_col = 0; tile_col < s->s.h.tiling.tile_cols; tile_col++) {
                    ...
                    if (ret < 0)
                        return ret;
                    if (vpx_rac_get_prob_branchy(&s->td[tile_col].c_b[tile_row], 128)) 
                        return AVERROR_INVALIDDATA;
                    data += tile_size;
                    size -= tile_size;
                }
