...
static int read_colorspace_details(AVCodecContext *ctx)
{
    ...
    VP9Context *s = ctx->priv_data;
    int bits = ctx->profile <= 1 ? 0 : 1 + get_bits1(&s->gb); 

    s->bpp_index = bits;
    s->bpp = 8 + bits * 2;
    s->bytesperpixel = (7 + s->bpp) >> 3;
    ctx->colorspace = colorspaces[get_bits(&s->gb, 3)];
    if (ctx->colorspace == AVCOL_SPC_RGB) { 
                               ...
                               const uint8_t *data, int size, int *ref)
{
    ...
    if (s->s.h.keyframe) {
        if (get_bits_long(&s->gb, 24) != VP9_SYNCCODE) { 
    ...
    if (vp56_rac_get_prob_branchy(&s->c, 128)) { 
...
static av_always_inline int decode_coeffs(AVCodecContext *ctx, int is8bitsperpixel)
{
    ...
    uint8_t (*p)[6][11] = s->prob.coef[b->tx][0 ][!b->intra];
    unsigned (*c)[6][3] = s->counts.coef[b->tx][0 ][!b->intra];
    unsigned (*e)[6][2] = s->counts.eob[b->tx][0 ][!b->intra];
...
#define DECODE_UV_COEF_LOOP(step, v) \
    ...
    p = s->prob.coef[b->uvtx][1 ][!b->intra];
    c = s->counts.coef[b->uvtx][1 ][!b->intra];
    e = s->counts.eob[b->uvtx][1 ][!b->intra];
    w4 >>= s->ss_h;
    end_x >>= s->ss_h;
    h4 >>= s->ss_v;
    end_y >>= s->ss_v;
                                             ...
                                             int p, int ss_h, int ss_v, int bytesperpixel)
{
    ...
    int bpp = s->bpp;
    static const uint8_t mode_conv[10][2 ][2 ] = {
        ...
        for (tile_row = 0; tile_row < s->s.h.tiling.tile_rows; tile_row++) {
            ...
            if (s->pass != 2) {
                for (tile_col = 0; tile_col < s->s.h.tiling.tile_cols; tile_col++) {
                    ...
                    if (vp56_rac_get_prob_branchy(&s->c_b[tile_col], 128)) { 
                        ff_thread_report_progress(&s->s.frames[CUR_FRAME].tf, INT_MAX, 0);
                        return AVERROR_INVALIDDATA;
                    }
                    data += tile_size;
