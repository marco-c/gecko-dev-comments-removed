...
static inline void NVToARGBRow_SVE2(const uint8_t* src_y,
  ...
  asm volatile (
      ...
      : [vl] "r"(vl),                                       
        [kUVCoeff] "r"(&yuvconstants->kUVCoeff),            
        [kRGBCoeffBias] "r"(&yuvconstants->kRGBCoeffBias),  
        [nv_u_start] "r"(nv_u_start),                       
        [nv_u_step] "r"(nv_u_step),                         
        [nv_v_start] "r"(nv_v_start),                       
        [nv_v_step] "r"(nv_v_step),                         
        [width_last_y] "r"(width_last_y),                   
        [width_last_uv] "r"(width_last_uv)                  
      : "cc", "memory", YUVTORGB_SVE_REGS);
}
...
#define AYUVTOUV_SVE(zU0, zV0, zU1, zV1)          \
