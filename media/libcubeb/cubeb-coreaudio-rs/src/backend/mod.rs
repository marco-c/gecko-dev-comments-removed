...
fn audiounit_create_device_from_hwdev(
    ...
    *dev_info = ffi::cubeb_device_info::default();
    ...
    if ret == NO_ERR {
        ...
        dev_info.latency_lo = 10 * dev_info.default_rate / 1000; 
        dev_info.latency_hi = 100 * dev_info.default_rate / 1000; 
    }
...
impl AudioUnitContext {
    ...
    fn remove_devices_changed_listener(&mut self, devtype: DeviceType) -> OSStatus {
        ...
        if devtype.contains(DeviceType::INPUT) {
            ...
            self.input_device_array.clear();
        }

        if devtype.contains(DeviceType::OUTPUT) {
            self.output_collection_changed_callback = None;
            self.output_collection_changed_user_ptr = ptr::null_mut();
...
impl<'ctx> AudioUnitStream<'ctx> {
    ...
    fn setup(&mut self) -> Result<()> {
        ...
        if self.has_input() {
            input_unconverted_params = unsafe { (*(self.input_stream_params.as_ptr())) }; 
