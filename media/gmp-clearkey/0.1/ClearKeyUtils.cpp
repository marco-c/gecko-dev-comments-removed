...
static bool ParseKeys(ParserContext& aCtx, vector<KeyIdPair>& aOutKeys) {
 ...
 bool ClearKeyUtils::ParseJWK(const uint8_t* aKeyData,
...
static bool ParseKeyIds(ParserContext& aCtx, vector<KeyId>& aOutKeyIds) {
 ...
 bool ClearKeyUtils::ParseKeyIdsInitData(
 ...
 const char* ClearKeyUtils::SessionTypeToString(
    SessionType aSessionType) {
  switch (aSessionType) {
 ...
 bool ClearKeyUtils::IsValidSessionId(const char* aBuff,
                                                  uint32_t aLength) {
  if (aLength > 10) {
