...
static void SubtractGreenFromBlueAndRed_AVX2(uint32_t* argb_data,
  ...
  for (i = 0; i + 8 <= num_pixels; i += 8) {
    const __m256i in = _mm256_loadu_si256((__m256i*)&argb_data[i]);  
...
static void TransformColor_AVX2(const VP8LMultipliers* WEBP_RESTRICT const m,
  ...
  const __m256i mask_rb = _mm256_set1_epi32(0x00ff00ff);  
  const __m256i kCstShuffle = _mm256_set_epi8(
  ...
  for (i = 0; i + 8 <= num_pixels; i += 8) {
    const __m256i in = _mm256_loadu_si256((__m256i*)&argb_data[i]);  
    const __m256i A = _mm256_shuffle_epi8(in, kCstShuffle);          
    const __m256i B = _mm256_mulhi_epi16(A, mults_rb);  
    const __m256i C = _mm256_slli_epi16(in, 8);         
    const __m256i D = _mm256_mulhi_epi16(C, mults_b2);  
    const __m256i E = _mm256_srli_epi32(D, 16);         
    const __m256i F = _mm256_add_epi8(E, B);            
    const __m256i G = _mm256_and_si256(F, mask_rb);     
  ...
  switch (xbits) {
    ...
    case 2: {
      ...
      for (x = 0; x + 32 <= width; x += 32, dst += 8) {
        ...
        const __m256i in = _mm256_loadu_si256((const __m256i*)&row[x]);
        const __m256i mul =
            _mm256_mullo_epi16(in, mul_cst);  
        const __m256i tmp =
            _mm256_and_si256(mul, mask_mul);               
        const __m256i shift = _mm256_srli_epi32(tmp, 12);  
        const __m256i pack = _mm256_or_si256(shift, tmp);  
...
static void PredictorSub11_AVX2(const uint32_t* in, const uint32_t* upper,
  ...
  for (i = 0; i + 8 <= num_pixels; i += 8) {
    ...
    const __m256i src = _mm256_loadu_si256((const __m256i*)&in[i]);
    __m256i pa, pb;
    GetSumAbsDiff32_AVX2(&T, &TL, &pa);  
    GetSumAbsDiff32_AVX2(&L, &TL, &pb);  
    {
...
WEBP_TSAN_IGNORE_FUNCTION void VP8LEncDspInitAVX2(void) {
  ...
  VP8LPredictorsSub[14] = PredictorSub0_AVX2;  
