...
static void FTransform_NEON(const uint8_t* WEBP_RESTRICT src,
  ...
  {    
    ...
    const int16x8_t a0a1 = vaddq_s16(d0d1, d3d2);   
    const int16x8_t a3a2 = vsubq_s16(d0d1, d3d2);   
...
static void FTransformWHT_NEON(const int16_t* WEBP_RESTRICT src,
  ...
  }
  {
    ...
    const int32x4_t b0 = vhaddq_s32(a0, a1);  
    const int32x4_t b1 = vhaddq_s32(a3, a2);  
    const int32x4_t b2 = vhsubq_s32(a3, a2);  
    const int32x4_t b3 = vhsubq_s32(a0, a1);  
...
static int16x8_t Quantize_NEON(int16_t* WEBP_RESTRICT const in,
  ...
  const uint16x8_t c = vaddq_u16(b, sharp);                  
  ...
  const int16x8_t c3 = vsubq_s16(c2, sign);                  
