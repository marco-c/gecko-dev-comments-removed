...
static uint64_t SharpYuvUpdateY_NEON(const uint16_t* ref, const uint16_t* src,
  ...
  for (i = 0; i + 8 <= len; i += 8) {
    ...
    const int16x8_t D = vsubq_s16(A, B);       
    const int16x8_t F = vaddq_s16(C, D);       
    const uint16x8_t H =
        vreinterpretq_u16_s16(vmaxq_s16(vminq_s16(F, max), zero));
    const int16x8_t I = vabsq_s16(D);          
    vst1q_u16(dst + i, H);
...
static void SharpYuvUpdateRGB_NEON(const int16_t* ref, const int16_t* src,
  ...
  for (i = 0; i + 8 <= len; i += 8) {
    ...
    const int16x8_t D = vsubq_s16(A, B);   
    const int16x8_t E = vaddq_s16(C, D);   
    vst1q_s16(dst + i, E);
  }
  for (; i < len; ++i) {
...
static void SharpYuvFilterRow16_NEON(const int16_t* A, const int16_t* B,
  ...
  for (i = 0; i + 8 <= len; i += 8) {
    ...
    const int16x8_t a0a1b0b1 = vaddq_s16(a0b1, a1b0);  
    const int16x8_t a0b1_2 = vaddq_s16(a0b1, a0b1);    
    const int16x8_t a1b0_2 = vaddq_s16(a1b0, a1b0);    
...
static void SharpYuvFilterRow32_NEON(const int16_t* A, const int16_t* B,
  ...
  for (i = 0; i + 4 <= len; i += 4) {
    ...
    const int32x4_t a0a1b0b1 = vaddq_s32(a0b1, a1b0);  
    const int32x4_t a0b1_2 = vaddq_s32(a0b1, a0b1);    
    const int32x4_t a1b0_2 = vaddq_s32(a1b0, a1b0);    
