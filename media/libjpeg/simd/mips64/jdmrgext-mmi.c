                                    ...
                                    JSAMPARRAY output_buf)
{
  ...
  for (num_cols = output_width >> 1; num_cols > 0; num_cols -= 8,
    ...
    cbh = _mm_unpackhi_pi8(cb, zero);         
    crl = _mm_unpacklo_pi8(cr, zero);         
    crh = _mm_unpackhi_pi8(cr, zero);         
    ...
    cbl2 = _mm_add_pi16(cbl, cbl);            
    cbh2 = _mm_add_pi16(cbh, cbh);            
    crl2 = _mm_add_pi16(crl, crl);            
    crh2 = _mm_add_pi16(crh, crh);            

    bl = _mm_mulhi_pi16(cbl2, PW_MF0228);     
    bh = _mm_mulhi_pi16(cbh2, PW_MF0228);     
    rl = _mm_mulhi_pi16(crl2, PW_F0402);      
    rh = _mm_mulhi_pi16(crh2, PW_F0402);      

    bl = _mm_add_pi16(bl, PW_ONE);
    bh = _mm_add_pi16(bh, PW_ONE);
    bl = _mm_srai_pi16(bl, 1);                
    bh = _mm_srai_pi16(bh, 1);                
    rl = _mm_add_pi16(rl, PW_ONE);
    rh = _mm_add_pi16(rh, PW_ONE);
    rl = _mm_srai_pi16(rl, 1);                
    rh = _mm_srai_pi16(rh, 1);                
    ...
    ynexto = _mm_srli_pi16(ynext, BYTE_BIT);  

    rle = _mm_add_pi16(rl, ythise);           
    rlo = _mm_add_pi16(rl, ythiso);           
    rhe = _mm_add_pi16(rh, ynexte);           
    rho = _mm_add_pi16(rh, ynexto);           
    re = _mm_packs_pu16(rle, rhe);            
    ro = _mm_packs_pu16(rlo, rho);            

    gle = _mm_add_pi16(gl, ythise);           
    glo = _mm_add_pi16(gl, ythiso);           
    ghe = _mm_add_pi16(gh, ynexte);           
    gho = _mm_add_pi16(gh, ynexto);           
    ge = _mm_packs_pu16(gle, ghe);            
    go = _mm_packs_pu16(glo, gho);            

    ble = _mm_add_pi16(bl, ythise);           
    blo = _mm_add_pi16(bl, ythiso);           
    bhe = _mm_add_pi16(bh, ynexte);           
    bho = _mm_add_pi16(bh, ynexto);           
    be = _mm_packs_pu16(ble, bhe);            
    bo = _mm_packs_pu16(blo, bho);            
    ...
    mmE = _mm_unpackhi_pi8(mmE, mmB);         
    mmC = _mm_unpacklo_pi8(mmD, mmF);         
    mmD = _mm_unpackhi_pi8(mmD, mmF);         

    mmB = _mm_unpacklo_pi16(mmG, mmA);        
    mmA = _mm_unpackhi_pi16(mmG, mmA);        
    mmF = _mm_unpacklo_pi16(mmH, mmE);        
    mmE = _mm_unpackhi_pi16(mmH, mmE);        
    mmH = _mm_unpacklo_pi16(mmC, mmD);        
    mmG = _mm_unpackhi_pi16(mmC, mmD);        

    mmC = _mm_unpacklo_pi16(mmB, mmF);        
    mmB = _mm_srli_si64(mmB, 4 * BYTE_BIT);
    mmB = _mm_unpacklo_pi16(mmH, mmB);        
    mmD = _mm_unpackhi_pi16(mmF, mmH);        
    mmF = _mm_unpacklo_pi16(mmA, mmE);        
    mmA = _mm_srli_si64(mmA, 4 * BYTE_BIT);
    mmH = _mm_unpacklo_pi16(mmG, mmA);        
    mmG = _mm_unpackhi_pi16(mmE, mmG);        

    mmA = _mm_unpacklo_pi32(mmC, mmB);        
    mmE = _mm_unpackhi_pi32(mmC, mmB);        
    mmB = _mm_unpacklo_pi32(mmD, mmF);        
    mmF = _mm_unpackhi_pi32(mmD, mmF);        
    mmC = _mm_unpacklo_pi32(mmH, mmG);        
    mmG = _mm_unpackhi_pi32(mmH, mmG);        
    ...
    mmA = _mm_unpacklo_pi8(mmE, mmG);         
    mmE = _mm_unpackhi_pi8(mmE, mmG);         

    mmG = _mm_unpacklo_pi8(mmB, mmD);         
    mmB = _mm_unpackhi_pi8(mmB, mmD);         
    mmD = _mm_unpacklo_pi8(mmF, mmH);         
    mmF = _mm_unpackhi_pi8(mmF, mmH);         

    mmH = _mm_unpacklo_pi16(mm8, mmA);        
    mm8 = _mm_unpackhi_pi16(mm8, mmA);        
    mmA = _mm_unpacklo_pi16(mmG, mmD);        
    mmD = _mm_unpackhi_pi16(mmG, mmD);        

    mmG = _mm_unpackhi_pi16(mm9, mmE);        
    mm9 = _mm_unpacklo_pi16(mm9, mmE);        
    mmE = _mm_unpacklo_pi16(mmB, mmF);        
    mmF = _mm_unpackhi_pi16(mmB, mmF);        

    mmB = _mm_unpackhi_pi32(mmH, mmA);        
    mmA = _mm_unpacklo_pi32(mmH, mmA);        
    mmC = _mm_unpacklo_pi32(mm8, mmD);        
    mmD = _mm_unpackhi_pi32(mm8, mmD);        

    mmH = _mm_unpackhi_pi32(mmG, mmF);        
    mmG = _mm_unpacklo_pi32(mmG, mmF);        
    mmF = _mm_unpackhi_pi32(mm9, mmE);        
    mmE = _mm_unpacklo_pi32(mm9, mmE);        
  ...
  if (!((output_width >> 1) & 7)) {
    if (output_width & 1) {
      ...
      rl = _mm_add_pi16(rl, yl);              
      gl = _mm_add_pi16(gl, yl);              
      bl = _mm_add_pi16(bl, yl);              
