                                   ...
                                   JSAMPARRAY *output_data_ptr)
{
  ...
  for (inrow = 0, outrow = 0; outrow < max_v_samp_factor; inrow++) {
    ...
    this0 = _mm_load_si64((__m64 *)inptr0);    
    this_1 = _mm_load_si64((__m64 *)inptr_1);  
    this1 = _mm_load_si64((__m64 *)inptr1);    

    this0l = _mm_unpacklo_pi8(this0, zero);    
    this0h = _mm_unpackhi_pi8(this0, zero);    
    this_1l = _mm_unpacklo_pi8(this_1, zero);  
    this_1h = _mm_unpackhi_pi8(this_1, zero);  
    this1l = _mm_unpacklo_pi8(this1, zero);    
    this1h = _mm_unpackhi_pi8(this1, zero);    

    this0l = _mm_mullo_pi16(this0l, PW_THREE);
    this0h = _mm_mullo_pi16(this0h, PW_THREE);

    thiscolsum_1l = _mm_add_pi16(this_1l, this0l);  
    thiscolsum_1h = _mm_add_pi16(this_1h, this0h);  
    thiscolsum1l = _mm_add_pi16(this0l, this1l);    
    thiscolsum1h = _mm_add_pi16(this0h, this1h);    
    ...
    wk[0] = _mm_and_si64(thiscolsum_1l, mask0);  
    wk[1] = _mm_and_si64(thiscolsum1l, mask0);   
    ...
    for (incol = downsampled_width; incol > 0;
      ...
      if (incol > 8) {
        ...
        next0 = _mm_load_si64((__m64 *)inptr0 + 1);    
        next_1 = _mm_load_si64((__m64 *)inptr_1 + 1);  
        next1 = _mm_load_si64((__m64 *)inptr1 + 1);    

        next0l = _mm_unpacklo_pi8(next0, zero);    
        next0h = _mm_unpackhi_pi8(next0, zero);    
        next_1l = _mm_unpacklo_pi8(next_1, zero);  
        next_1h = _mm_unpackhi_pi8(next_1, zero);  
        next1l = _mm_unpacklo_pi8(next1, zero);    
        next1h = _mm_unpackhi_pi8(next1, zero);    

        next0l = _mm_mullo_pi16(next0l, PW_THREE);
        next0h = _mm_mullo_pi16(next0h, PW_THREE);

        nextcolsum_1l = _mm_add_pi16(next_1l, next0l);  
        nextcolsum_1h = _mm_add_pi16(next_1h, next0h);  
        nextcolsum1l = _mm_add_pi16(next0l, next1l);    
        nextcolsum1h = _mm_add_pi16(next0h, next1h);    
      ...
      samp0123 = _mm_load_si64((__m64 *)outptr0);       \
      samp4567 = _mm_load_si64((__m64 *)outptr0 + 1);   \
      PROCESS_ROW(0, 2, PW_EIGHT, PW_SEVEN, 4)

      
      samp0123 = _mm_load_si64((__m64 *)outptr1);       \
      samp4567 = _mm_load_si64((__m64 *)outptr1 + 1);   \
                                   ...
                                   JSAMPARRAY *output_data_ptr)
{
  ...
  for (inrow = 0; inrow < max_v_samp_factor; inrow++) {
    ...
    this = _mm_load_si64((__m64 *)inptr0);    
    thisl = _mm_unpacklo_pi8(this, zero);     
    wk[0] = _mm_and_si64(thisl, mask0);       
    ...
    for (incol = downsampled_width; incol > 0;
      ...
      if (incol > 8) {
        ...
        next = _mm_load_si64((__m64 *)inptr0 + 1);  
        nextl = _mm_unpacklo_pi8(next, zero);       
        wk[1] = _mm_slli_si64(nextl, (SIZEOF_MMWORD - 2) * BYTE_BIT);  
      } else {
        __m64 thish;

        
        this = _mm_load_si64((__m64 *)inptr0);  
        thish = _mm_unpackhi_pi8(this, zero);   
        wk[1] = _mm_and_si64(masklast, thish);  
      }

      
      this = _mm_load_si64((__m64 *)inptr0);    
      samp0123 = _mm_unpacklo_pi8(this, zero);  
      samp4567 = _mm_unpackhi_pi8(this, zero);  
