...
int32_t WebrtcMediaCodecVP8VideoEncoder::Encode(
  ...
  if (!mMediaCodecEncoder->isStarted) {
    ...
    nsresult res = mMediaCodecEncoder->Configure(mFrameWidth, mFrameHeight, nullptr, MediaCodec::CONFIGURE_FLAG_ENCODE, MEDIACODEC_VIDEO_MIME_VP8, true );
  ...
  if (inputIndex >= 0) {
    ...
    mMediaCodecEncoder->QueueInputBuffer(inputIndex, 0, size, inputImage.render_time_ms() * PR_USEC_PER_MSEC , 0);
#ifdef WEBRTC_MEDIACODEC_DEBUG
...
int32_t WebrtcMediaCodecVP8VideoDecoder::Decode(
  ...
  if (!mMediaCodecDecoder->isStarted) {
    ...
    nsresult res = mMediaCodecDecoder->Configure(mFrameWidth, mFrameHeight, nullptr, 0, MEDIACODEC_VIDEO_MIME_VP8, false );
