...
public:
    CTelephonyEvent(VoiceEngine* voiceEngine, int channel, CDialog* pParentDialog, CWnd* pParent = NULL);   
...
CWinTestDlg::CWinTestDlg(CWnd* pParent )
...
BOOL CWinTestDlg::OnInitDialog()
{
    ...
    if (_veVolumeControlPtr)
    {
        ...
        leftVol = (int)(lVol*10.0f);    
        rightVol = (int)(rVol*10.0f);    

        slider = (CSliderCtrl*)GetDlgItem(IDC_SLIDER_PAN_LEFT);
        slider->SetRange(0,10);
        slider->SetPos(10-leftVol);        
