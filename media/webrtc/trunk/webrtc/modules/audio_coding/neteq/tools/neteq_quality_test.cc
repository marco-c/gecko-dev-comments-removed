...
static bool ValidatePacketLossRate(const char* , int32_t value) {
...
static bool ValidateRandomLossMode(const char* , int32_t value) {
  if (value >= 0 && value <= 2)
    return true;
...
static bool ValidateBurstLength(const char* , int32_t value) {
...
static bool ValidateDriftFactor(const char* , double value) {
  if (value > -0.1)
    return true;
