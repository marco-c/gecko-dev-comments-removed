    ...
    switch (coder) {
      ...
      case webrtc::kDecoderG729_1:
        if (sampfreq == 16000) {
          ...
          WebRtcG7291_EncoderInit(G729_1_inst[k], bitrate, 0 ,
                                  0 );
        } else {
      ...
      case webrtc::kDecoderSPEEX_8:
        if (sampfreq == 8000) {
          ...
          ok = WebRtcSpeex_EncoderInit(SPEEX8enc_inst[k], 0 ,
                                       3 , vad);
          if (ok != 0)
            exit(0);
        } else {
      ...
      case webrtc::kDecoderSPEEX_16:
        if (sampfreq == 16000) {
          ...
          ok = WebRtcSpeex_EncoderInit(SPEEX16enc_inst[k], 0 ,
                                       3 , vad);
          if (ok != 0)
            exit(0);
        } else {
...
size_t NetEQTest_encode(webrtc::NetEqDecoder coder,
  ...
  *vad = 1;
  ...
  for (size_t k = 0; k < numChannels; k++) {
    ...
    if (coder == webrtc::NetEqDecoder::kDecoderPCMu) { 
#ifdef CODEC_G711
      cdlen = WebRtcG711_EncodeU(indata, frameLen, encoded);
#endif
    } else if (coder == webrtc::NetEqDecoder::kDecoderPCMa) { 
#ifdef CODEC_G711
...
#endif
#ifdef CODEC_G722
    else if (coder == webrtc::NetEqDecoder::kDecoderG722) { 
      cdlen = WebRtcG722_Encode(g722EncState[k], indata, frameLen, encoded);
      assert(cdlen == frameLen >> 1);
    }
#endif
#ifdef CODEC_ILBC
    else if (coder == webrtc::NetEqDecoder::kDecoderILBC) { 
    ...
    else if (coder == webrtc::NetEqDecoder::kDecoderISAC) { 
      int noOfCalls = 0;
      int res = 0;
      while (res <= 0) {
#ifdef CODEC_ISAC 
...
#endif
#ifdef CODEC_ISAC_SWB
    else if (coder == webrtc::NetEqDecoder::kDecoderISACswb) { 
      int noOfCalls = 0;
      int res = 0;
      while (res <= 0) {
