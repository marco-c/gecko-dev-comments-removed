		...
		if (enc_len > 0 && (sendtime <= STOPSENDTIME || sendtime > RESTARTSENDTIME)) {
            if(useRed) {
                if(red_len[0] > 0) {
                    ...
                    enc_len += red_len[0] + RTPheaderLen - 4 - 12; 
                }
            }
            else {
...
int NetEQTest_init_coders(webrtc::NetEqDecoder coder, int enc_frameSize, int bitrate, int sampfreq , int vad, int numChannels){
    ...
    case webrtc::kDecoderG729_1:
        if (sampfreq==16000) {
            if ((enc_frameSize==320)||(enc_frameSize==640)||(enc_frameSize==960)
                ...
                WebRtcG7291_EncoderInit(G729_1_inst[k], bitrate, 0 , 0 );
        } else {
    ...
    case webrtc::kDecoderSPEEX_8 :
        if (sampfreq==8000) {
            ...
            ok=WebRtcSpeex_EncoderInit(SPEEX8enc_inst[k], 0, 3 , vad);
            if (ok!=0) exit(0);
        } else {
    ...
    case webrtc::kDecoderSPEEX_16 :
        if (sampfreq==16000) {
            ...
            ok=WebRtcSpeex_EncoderInit(SPEEX16enc_inst[k], 0, 3 , vad);
            if (ok!=0) exit(0);
        } else {
...
int NetEQTest_encode(int coder, int16_t *indata, int frameLen, unsigned char * encoded,int sampleRate , 
	...
	*vad =1;
        ...
        *vad = 0;
        ...
        for (int k = 0; k < numChannels; k++)
        {
            ...
            while (tempLen >= 10*sampleRate/1000) {
                if ((tempLen % 30*sampleRate/1000) == 0) { 
        ...
        if(!*vad){
            ...
            for (int k = 0; k < numChannels; k++)
            {
                WebRtcCng_Encode(CNGenc_inst[k],&indata[k*frameLen], (frameLen <= 640 ? frameLen : 640) ,
    ...
    for (int k = 0; k < numChannels; k++)
    {
        ...
        if (coder==webrtc::kDecoderPCMu) { 
#ifdef CODEC_G711
            cdlen = WebRtcG711_EncodeU(indata, frameLen, encoded);
#endif
        }  
        else if (coder==webrtc::kDecoderPCMa) { 
#ifdef CODEC_G711
...
#endif
#ifdef CODEC_G722
        else if (coder==webrtc::kDecoderG722) { 
            cdlen=WebRtcG722_Encode(g722EncState[k], indata, frameLen, encoded);
            assert(cdlen == frameLen>>1);
        }
#endif
#ifdef CODEC_ILBC
        else if (coder==webrtc::kDecoderILBC) { 
            cdlen = WebRtcIlbcfix_Encode(iLBCenc_inst[k], indata,
                                         frameLen, encoded);
        }
#endif
#if (defined(CODEC_ISAC) || defined(NETEQ_ISACFIX_CODEC)) 
        else if (coder==webrtc::kDecoderISAC) { 
            int noOfCalls=0;
            cdlen=0;
            while (cdlen<=0) {
#ifdef CODEC_ISAC 
...
#endif
#ifdef CODEC_ISAC_SWB
        else if (coder==webrtc::kDecoderISACswb) { 
            int noOfCalls=0;
            cdlen=0;
            while (cdlen<=0) {
