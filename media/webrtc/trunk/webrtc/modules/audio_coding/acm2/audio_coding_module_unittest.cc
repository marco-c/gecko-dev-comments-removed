...
class AudioCodingModuleTestOldApi : public ::testing::Test {
 protected:
  ...
  void SetUp() {
    ...
    input_frame_.samples_per_channel_ = kSampleRateHz * 10 / 1000;  
...
class AudioCodingModuleMtTestOldApi : public AudioCodingModuleTestOldApi {
 protected:
  ...
  EventTypeWrapper RunTest() {
    return test_complete_->Wait(10 * 60 * 1000);  
  }
 ...
 protected:
  ...
  bool CbReceiveImpl() {
    ...
    for (int i = 0; i < rtc::CheckedDivExact(kPacketSizeMs, 10); ++i) {
      AudioFrame audio_frame;
      bool muted;
      EXPECT_EQ(0, acm_->PlayoutData10Ms(-1 ,
                                         &audio_frame, &muted));
      if (muted) {
        ADD_FAILURE();
        return false;
...
class AcmReceiverBitExactnessOldApi : public ::testing::Test {
 ...
 protected:
  ...
  void Run(int output_freq_hz,
    ...
    packet_source->FilterOutPayloadType(102);  
    packet_source->FilterOutPayloadType(104);  
#endif
...
TEST_F(AcmReceiverBitExactnessOldApi, 48kHzOutputExternalDecoder) {
  class ADFactory : public AudioDecoderFactory {
   ...
   private:
    ...
    rtc::scoped_refptr<AudioDecoderFactory> fact_;  
  };
...
class AcmSwitchingOutputFrequencyOldApi : public ::testing::Test,
 ...
 protected:
  ...
  bool WriteArray(const int16_t* audio, size_t num_samples) override {
    ...
    if (num_samples ==
        static_cast<size_t>(output_freq_2_ / 100))  
      has_toggled_ = true;
