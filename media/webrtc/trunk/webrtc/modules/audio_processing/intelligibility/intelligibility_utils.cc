...
void VarianceArray::DecayStep(const complex<float>* data, bool ) {
  array_mean_ = 0.0f;
  ++count_;
  for (size_t i = 0; i < num_freqs_; ++i) {
    complex<float> sample = data[i];
...
void VarianceArray::WindowedStep(const complex<float>* data, bool ) {
...
void VarianceArray::BlockedStep(const complex<float>* data, bool ) {
