...
void RateCounterFilter::RunFor(int64_t , Packets* in_out) {
  assert(in_out);
...
void LossFilter::RunFor(int64_t , Packets* in_out) {
  assert(in_out);
...
void DelayFilter::RunFor(int64_t , Packets* in_out) {
  assert(in_out);
...
void JitterFilter::RunFor(int64_t , Packets* in_out) {
  assert(in_out);
  for (Packet* packet : *in_out) {
    int64_t jitter_us =
...
void ReorderFilter::RunFor(int64_t , Packets* in_out) {
  assert(in_out);
  if (in_out->size() >= 2) {
...
void ChokeFilter::RunFor(int64_t , Packets* in_out) {
  assert(in_out);
  for (PacketsIt it = in_out->begin(); it != in_out->end(); ) {
