...
rtc::Buffer GenerateRtcpPacket(Random* prng) {
  ...
  report_block.SetMediaSsrc(prng->Rand<uint32_t>());  
...
TEST(RtcEventLogTest, LogSessionAndReadBack) {
  ...
  extensions = (1u << kNumExtensions) - 1;  
  ...
  for (extensions = 0; extensions < (1u << kNumExtensions); extensions++) {
    for (uint32_t csrcs_count = 0; csrcs_count < 3; csrcs_count++) {
      LogSessionAndReadBack(5 + extensions,   
                            2 + csrcs_count,  
                            3 + csrcs_count,  
                            1 + csrcs_count,  
                            extensions,       
                            csrcs_count,      
                            extensions * 3 + csrcs_count + 1);  
    }
  }
}
