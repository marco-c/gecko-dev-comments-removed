...
TEST(IPAddressTest, TestComparison) {
  ...
  IPAddress addr0;  
  IPAddress addr1(INADDR_ANY);  
  IPAddress addr2(kIPv4PublicAddr);  
  IPAddress addr3(INADDR_LOOPBACK);  
  IPAddress addr4(kIPv4RFC1918Addr);  
  IPAddress addr5(in6addr_any);  
  IPAddress addr6(in6addr_loopback);  
  IPAddress addr7(kIPv6PublicAddr);  
  IPAddress addr8(kIPv6LinkLocalAddr);  
