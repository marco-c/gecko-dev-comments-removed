...
const uint32 JINGLE_HEADER_SIZE = 64; 
...
const uint32 MIN_RTO   =   250; 
const uint32 DEF_RTO   =  3000; 
const uint32 MAX_RTO   = 60000; 
const uint32 DEF_ACK_DELAY = 100; 
...
const uint8 TCP_OPT_EOL = 0;  
const uint8 TCP_OPT_NOOP = 1;  
const uint8 TCP_OPT_MSS = 2;  
const uint8 TCP_OPT_WND_SCALE = 3;  
...
const uint32 IDLE_PING = 20 * 1000; 
const uint32 IDLE_TIMEOUT = 90 * 1000; 
#endif 
  ...
  if ((seg.ack > m_snd_una) && (seg.ack <= m_snd_nxt)) {
    ...
    if (m_dup_acks >= 3) {
      if (m_snd_una >= m_recover) { 
        uint32 nInFlight = m_snd_nxt - m_snd_una;
        m_cwnd = std::min(m_ssthresh, nInFlight + m_mss);  
#if _DEBUGMSG >= _DBG_NORMAL
