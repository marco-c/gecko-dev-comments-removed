...
class FlagValue {
 ...
 private:
  ...
  friend class GOOGLE_NAMESPACE::FlagSaverImpl;  
...
bool FlagValue::ParseFrom(const char* value) {
  ...
  switch (type_) {
    case FV_INT32: {
      ...
      if (errno || end != value + strlen(value))  return false;  
...
class CommandLineFlag {
 public:
  ...
  const char* CleanFileName() const;  
  ...
  while ( clean_name > filename() ) {
    if (*clean_name == PATH_SEPARATOR) {
      if (strncmp(clean_name, kRootDir, sizeof(kRootDir)-1) == 0) {
        clean_name += sizeof(kRootDir)-1;    
        break;
      }
    }
    --clean_name;
  }
  while ( *clean_name == PATH_SEPARATOR ) ++clean_name;  
  return clean_name;
}
...
class FlagRegistry {
 ...
 private:
  friend class GOOGLE_NAMESPACE::FlagSaverImpl;  
  friend class CommandLineFlagParser;    
...
class CommandLineFlagParser {
 ...
 private:
  ...
  map<string, string> error_flags_;      
  
  map<string, string> undefined_names_;  
};
...
uint32 CommandLineFlagParser::ParseNewCommandLineFlags(int* argc, char*** argv,
                                                       bool remove_flags) {
  const char *program_name = strrchr((*argv)[0], PATH_SEPARATOR);   
...
string CommandLineFlagParser::ProcessOptionsFromStringLocked(
  ...
  for (; line_end; flagfile_contents = line_end + 1) {
    ...
    if (line.empty() || line[0] == '#') {
      ...
      const char* name_and_val = line.c_str() + 1;    
      if (*name_and_val == '-')
        name_and_val++;                               
      string key;
      const char* value;
      ...
      if (!in_filename_section) {   
        in_filename_section = true;
        flags_are_relevant = false;
      }

      
      const char* space = line.c_str();   
...
class FlagSaverImpl {
 ...
 private:
  ...
  FlagSaverImpl(const FlagSaverImpl&);  
