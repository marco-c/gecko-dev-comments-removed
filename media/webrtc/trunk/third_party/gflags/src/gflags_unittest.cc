...
TEST(DeprecatedFunctionsTest, AppendFlagsIntoFile) {
  ...
  EXPECT_TRUE(fp != NULL);
  char line[8192];
  EXPECT_TRUE(fgets(line, sizeof(line)-1, fp) != NULL);  
...
TEST(FlagsValidator, FlagSaver) {
  ...
  EXPECT_NE("", SetCommandLineOption("test_flag", "50"));  
