...
fn read_box_header_short() {
    ...
    assert_eq!(header.name, BoxType::UnknownBox(0x7465_7374)); 
...
fn read_ftyp() {
    ...
    assert_eq!(parsed.compatible_brands[0], FourCC::from("isom")); 
    assert_eq!(parsed.compatible_brands[1], FourCC::from("mp42")); 
}
...
fn read_hdlr_short_name() {
    let mut stream = make_fullbox(BoxSize::Short(33), b"hdlr", 0, |s| {
        s.B32(0).append_bytes(b"vide").B32(0).B32(0).B32(0).B8(0) 
    });
    let mut iter = super::BoxIter::new(&mut stream);
    let mut stream = iter.next_box().unwrap().unwrap();
    assert_eq!(stream.head.name, BoxType::HandlerBox);
