...
TEST(EncodeAPI, InvalidUVStrides) {
  ...
  for (const auto *iface : kCodecIfaces) {
    ...
    for (const auto img_fmt :
      ...
      ASSERT_EQ(vpx_img_wrap(&img, img_fmt, kWidth, kHeight, 1,
                             buf.get()),
                &img)
      ...
      for (const auto uv_stride : kUVStrides) {
        ...
        EXPECT_EQ(vpx_codec_encode(&enc, &img, 0, 1,
                                   0, VPX_DL_REALTIME),
      ...
      EXPECT_EQ(
          vpx_codec_encode(&enc, nullptr, 0, 0,
                           0, VPX_DL_REALTIME),
          VPX_CODEC_OK);
...
TEST(EncodeAPI, InvalidImageFormats) {
  EXPECT_EQ(vpx_img_alloc(nullptr, VPX_IMG_FMT_NONE, 32,
                          32, 1),
            nullptr);
  EXPECT_EQ(vpx_img_alloc(nullptr,
...
void VP8Encoder::Configure(unsigned int threads, unsigned int width,
  ...
  if (!initialized_) {
    vpx_codec_iface_t *const iface = vpx_codec_vp8_cx();
    ASSERT_EQ(vpx_codec_enc_config_default(iface, &cfg_, 0),
              VPX_CODEC_OK);
    cfg_.g_threads = threads;
    cfg_.g_w = width;
    cfg_.g_h = height;
    cfg_.g_timebase.num = 1;
    cfg_.g_timebase.den = 1000 * 1000;  
...
TEST(EncodeAPI, Vp8ChromiumIssue431520320) {
  ...
  ASSERT_EQ(
      vpx_codec_encode(&enc, image, 0, 800000, 0, VPX_DL_REALTIME),
      VPX_CODEC_OK);

  
  ASSERT_EQ(vpx_codec_encode(&enc, image, 40000, 40000, 0,
                             VPX_DL_REALTIME),
            VPX_CODEC_OK);

  
  ASSERT_EQ(vpx_codec_encode(&enc, image, 80000, 40000, 0,
                             VPX_DL_REALTIME),
            VPX_CODEC_OK);
...
TEST(EncodeAPI, AomediaIssue3509VbrMinSection2PercentVP8) {
  ...
  ASSERT_EQ(
      vpx_codec_encode(&enc, image, 0, 300, 0, VPX_DL_REALTIME),
      VPX_CODEC_OK);
...
void EncodeOssFuzz69906(int cpu_used, vpx_enc_deadline_t deadline) {
  ...
  libvpx_test::Y4mVideoSource video("repro-oss-fuzz-69906.y4m", 0,
                                    3);
  video.Begin();
  do {
    ASSERT_EQ(vpx_codec_encode(&enc, video.img(), video.pts(), video.duration(),
...
TEST(EncodeAPI, OssFuzz471723682) {
  ...
  video.Begin();
  do {
    ASSERT_EQ(vpx_codec_encode(&enc, video.img(), 0, 46640,
                               0, VPX_DL_GOOD_QUALITY),
              VPX_CODEC_OK);
    video.Next();
  } while (video.img() != nullptr);
...
TEST(EncodeAPI, MultiResEncode) {
  ...
  for (const auto *iface : kCodecIfaces) {
    ...
    cfg[0].rc_target_bitrate = target_bitrate; 
...
TEST(EncodeAPI, MultiResEncodeFirstIterFailOOB) {
  ...
  EXPECT_EQ(vpx_codec_encode(enc.data(), imgs, 0, 1,
                             0, VPX_DL_REALTIME),
            VPX_CODEC_INVALID_PARAM);
...
TEST(EncodeAPI, PerFramePsnr) {
  for (const auto *iface : kCodecIfaces) {
    ...
    ASSERT_EQ(vpx_codec_encode(&enc, image, 0, 1,
    ...
    ASSERT_EQ(vpx_codec_encode(&enc, image, 1, 1,
...
TEST(EncodeAPI, Buganizer319964497) {
  VP9Encoder encoder(7);
  encoder.Configure(1, 320, 240, VPX_VBR,
                    VPX_DL_REALTIME);
  encoder.Encode(true);
  encoder.Encode(true);
  encoder.Encode(false);
  encoder.Configure(1, 1, 1, VPX_VBR,
                    VPX_DL_REALTIME);
  encoder.Encode(false);
  encoder.Configure(1, 2, 2, VPX_CBR,
                    VPX_DL_REALTIME);
  encoder.Encode(false);
}
...
TEST(EncodeAPI, Buganizer329088759RowMT0) {
  ...
  encoder.Configure(8, 1686, 398, VPX_VBR,
                    VPX_DL_REALTIME);
  encoder.Encode(true);
  encoder.Encode(false);
  encoder.Configure(0, 1686, 1, VPX_VBR,
                    VPX_DL_REALTIME);
  encoder.Encode(true);
  encoder.Configure(0, 1482, 113, VPX_CBR,
                    VPX_DL_REALTIME);
  encoder.Encode(true);
  encoder.Configure(0, 881, 59, VPX_CBR,
                    VPX_DL_REALTIME);
  encoder.Configure(13, 1271, 385, VPX_CBR,
                    VPX_DL_REALTIME);
  encoder.Encode(false);
  encoder.Configure(2, 1, 62, VPX_VBR,
                    VPX_DL_REALTIME);
}
...
TEST(EncodeAPI, Buganizer329088759RowMT1) {
  ...
  encoder.Configure(2, 1686, 1, VPX_VBR,
                    VPX_DL_REALTIME);
  encoder.Encode(true);
  encoder.Configure(2, 1482, 113, VPX_CBR,
                    VPX_DL_REALTIME);
  encoder.Encode(true);
  encoder.Configure(2, 881, 59, VPX_CBR,
                    VPX_DL_REALTIME);
  encoder.Configure(13, 1271, 385, VPX_CBR,
                    VPX_DL_REALTIME);
  encoder.Encode(false);
...
TEST(EncodeAPI, Buganizer331108922BitDepth8) {
  ...
  encoder.Configure(1, 1, 1080, VPX_VBR,
                    VPX_DL_REALTIME);
  encoder.Encode(false);
  encoder.Configure(0, 1, 1080, VPX_CBR,
                    VPX_DL_GOOD_QUALITY);
  encoder.Configure(16, 1, 394, VPX_CBR,
                    VPX_DL_REALTIME);
  encoder.Encode(false);
  encoder.Encode(true);
  encoder.Configure(16, 1, 798, VPX_CBR,
                    VPX_DL_REALTIME);
  encoder.Encode(false);
}
...
TEST(EncodeAPI, Buganizer442105459_2RowTiles) {
  ...
  for (int frame = 0; frame < num_frames; frame++) {
    ...
    ASSERT_EQ(vpx_codec_encode(&ctx, img, pts, duration,  0, deadline),
              VPX_CODEC_OK);
  }
...
TEST(EncodeAPI, Buganizer329674887RowMT0BitDepth12) {
  ...
  encoder.Configure(2, 1030, 583, VPX_VBR,
                    VPX_DL_REALTIME);
  encoder.Encode(true);
  encoder.Configure(0, 1030, 1, VPX_CBR,
                    VPX_DL_REALTIME);
  encoder.Encode(true);
  encoder.Configure(0, 548, 322, VPX_VBR,
                    VPX_DL_REALTIME);
  encoder.Encode(false);
  encoder.Configure(16, 24, 583, VPX_CBR,
                    VPX_DL_GOOD_QUALITY);
}

TEST(EncodeAPI, Buganizer329179808RowMT0BitDepth10) {
  VP9Encoder encoder(4, 0, VPX_BITS_10, VPX_IMG_FMT_I444);
  encoder.Configure(16, 1488, 5, VPX_VBR,
                    VPX_DL_REALTIME);
  encoder.Encode(true);
  encoder.Configure(16, 839, 1, VPX_CBR,
                    VPX_DL_REALTIME);
  encoder.Encode(false);
  encoder.Configure(11, 657, 5, VPX_CBR,
                    VPX_DL_REALTIME);
  encoder.Encode(false);
}
...
TEST(EncodeAPI, InvalidInputRange) {
  ...
  for (const auto bitdepth : { VPX_BITS_8, VPX_BITS_10, VPX_BITS_12 }) {
    ...
    EXPECT_EQ(vpx_codec_encode(&enc, nullptr, 0, 0,
                               0, VPX_DL_REALTIME),
              VPX_CODEC_OK);
    EXPECT_EQ(vpx_codec_destroy(&enc), VPX_CODEC_OK);
  }
...
TEST(EncodeAPI, Chromium352414650) {
  ...
  ASSERT_EQ(vpx_codec_encode(&enc, image, 0, 500000,
...
TEST(EncodeAPI, PerFramePsnrNotSupportedWithLagInFrames) {
  ...
  vpx_enc_frame_flags_t psnr_flags = VPX_EFLAG_CALCULATE_PSNR;
  ASSERT_EQ(vpx_codec_encode(&enc, image, 0, 1, psnr_flags,
                             VPX_DL_REALTIME),
            VPX_CODEC_INCAPABLE);
...
TEST(EncodeAPI, Buganizer487259772ScaledRefs) {
  ...
  encoder.Configure(1, 478, 755, VPX_VBR,
                    VPX_DL_REALTIME);
  encoder.Configure(1, 387, 438, VPX_VBR,
                    VPX_DL_REALTIME);
  encoder.Encode(false, &rng);
  encoder.Encode(true, &rng);
  encoder.Encode(false, &rng);
  encoder.Encode(false, &rng);

  encoder.Configure(1, 341, 655, VPX_VBR,
                    VPX_DL_REALTIME);
  encoder.Encode(false, &rng);
  encoder.Encode(false, &rng);
}
...
TEST(EncodeAPI, Buganizer488585490CostTableOverflow) {
  ...
  libvpx_test::YUVVideoSource video(
      ...
      1, 30, 0, 1);
  video.Begin();
  ASSERT_EQ(vpx_codec_encode(&enc, video.img(), video.pts(), 66666,
                             0, VPX_DL_REALTIME),
            VPX_CODEC_INVALID_PARAM);
