...
static INLINE void idct8x8_12_add_kernel_sse2(__m128i *const io ) {
  ...
  
  {
    ...
    step1[4] = idct_calc_wraplow_sse2(cp_28_n4, cp_4_28, lo_1);    
    step1[5] = idct_calc_wraplow_sse2(cp_n20_12, cp_12_20, lo_3);  
  }
  ...
  
  {
    ...
    step2[0] = _mm_packs_epi32(t, t);                            
    step2[2] = idct_calc_wraplow_sse2(cp_8_24, cp_24_n8, lo_2);  
    step2[4] = _mm_add_epi16(step1[4], step1[5]);                
    step2[5] = _mm_sub_epi16(step1[4], step1[5]);                
    step2[6] = _mm_unpackhi_epi64(step2[5], zero);               
  }

  
  {
    const __m128i lo_65 = _mm_unpacklo_epi16(step2[6], step2[5]);
    tmp[0] = _mm_add_epi16(step2[0], step2[2]);                     
    tmp[1] = _mm_sub_epi16(step2[0], step2[2]);                     
    step1[2] = _mm_unpackhi_epi64(tmp[1], tmp[0]);                  
    step1[3] = _mm_unpacklo_epi64(tmp[1], tmp[0]);                  
    step1[5] = idct_calc_wraplow_sse2(cp_16_n16, cp_16_16, lo_65);  
  }

  
  tmp[0] = _mm_add_epi16(step1[3], step2[4]);  
  tmp[1] = _mm_add_epi16(step1[2], step1[5]);  
  tmp[2] = _mm_sub_epi16(step1[3], step2[4]);  
  tmp[3] = _mm_sub_epi16(step1[2], step1[5]);  
