...
static INLINE void extend_64bit(const __m128i in,
                                __m128i *const out ) {
  out[0] = _mm_unpacklo_epi32(in, in);  
  out[1] = _mm_unpackhi_epi32(in, in);  
}
...
static INLINE __m128i pack_4(const __m128i in0, const __m128i in1) {
  const __m128i t0 = _mm_unpacklo_epi32(in0, in1);  
  const __m128i t1 = _mm_unpackhi_epi32(in0, in1);  
...
static INLINE void abs_extend_64bit_sse2(const __m128i in,
  ...
  sign[1] = _mm_unpackhi_epi32(sign[0], sign[0]);  
  sign[0] = _mm_unpacklo_epi32(sign[0], sign[0]);  
  out[1] = _mm_unpackhi_epi32(out[0], out[0]);     
  out[0] = _mm_unpacklo_epi32(out[0], out[0]);     
}
