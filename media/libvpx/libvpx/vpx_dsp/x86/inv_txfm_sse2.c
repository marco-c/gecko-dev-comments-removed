...
void iadst4_sse2(__m128i *const in) {
  ...
  v[0] = _mm_madd_epi16(in[0], k__sinpi_1_3);    
  v[1] = _mm_madd_epi16(in[1], k__sinpi_4_2);    
  v[2] = _mm_madd_epi16(in[0], k__sinpi_2_3);    
  v[3] = _mm_madd_epi16(in[1], k__sinpi_1_4);    
  v[4] = _mm_madd_epi16(in[0], k__sinpi_12_n3);  
  in[0] = _mm_sub_epi16(in[0], in[1]);           
  in[1] = _mm_srli_epi32(in[1], 16);
  in[0] = _mm_add_epi16(in[0], in[1]);
  in[0] = _mm_slli_epi32(in[0], 16);  
