...
static INLINE void dc_4x4(uint8_t *dst, ptrdiff_t stride,
  ...
  if (do_left) {
    ...
    const uint16x4_t p0 = vpaddl_u8(L);  
    const uint16x4_t p1 = vpadd_u16(p0, p0);
...
static INLINE void dc_16x16(uint8_t *dst, ptrdiff_t stride,
  ...
  if (do_above) {
    const uint8x16_t A = vld1q_u8(above);  
  ...
  if (do_left) {
    const uint8x16_t L = vld1q_u8(left);  
    const uint16x8_t p0 = vpaddlq_u8(L);  
    const uint16x4_t p1 = vadd_u16(vget_low_u16(p0), vget_high_u16(p0));
    const uint16x4_t p2 = vpadd_u16(p1, p1);
    const uint16x4_t p3 = vpadd_u16(p2, p2);
...
static INLINE void dc_32x32(uint8_t *dst, ptrdiff_t stride,
  ...
  if (do_left) {
    const uint8x16_t L0 = vld1q_u8(left);  
    const uint8x16_t L1 = vld1q_u8(left + 16);
    const uint16x8_t p0 = vpaddlq_u8(L0);  
...
void vpx_d45_predictor_4x4_neon(uint8_t *dst, ptrdiff_t stride,
                                const uint8_t *above, const uint8_t *left) {
  const uint64x1_t A0 = vreinterpret_u64_u8(vld1_u8(above));  
