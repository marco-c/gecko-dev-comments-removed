...
static INLINE uint16x8_t filter_flat_hev_mask(
  ...
  *flat = vcleq_u16(*flat, vdupq_n_u16(1 << (bd - 8))); 
...
static INLINE void calc_7_tap_filter(const uint16x8_t p3, const uint16x8_t p2,
  ...
  uint16x8_t sum;
  sum = vaddq_u16(p3, p3);   
  sum = vaddq_u16(sum, p3);  
  sum = vaddq_u16(sum, p2);  
  sum = vaddq_u16(sum, p2);  
  sum = vaddq_u16(sum, p1);  
  sum = vaddq_u16(sum, p0);  
  sum = vaddq_u16(sum, q0);  
...
static INLINE void apply_15_tap_filter(
  ...
  uint16x8_t sum;
  sum = vshlq_n_u16(p7, 3);  
  sum = vsubq_u16(sum, p7);  
  sum = vaddq_u16(sum, p6);  
  sum = vaddq_u16(sum, p6);  
  sum = vaddq_u16(sum, p5);  
  sum = vaddq_u16(sum, p4);  
  sum = vaddq_u16(sum, p3);  
  sum = vaddq_u16(sum, p2);  
  sum = vaddq_u16(sum, p1);  
  sum = vaddq_u16(sum, p0);  
