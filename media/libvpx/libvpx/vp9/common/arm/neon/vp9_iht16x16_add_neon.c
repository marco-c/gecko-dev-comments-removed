...
void vp9_iht16x16_256_add_neon(const tran_low_t *input, uint8_t *dest,
  ...
  ht.rows(input, row_output, dest, stride, 0);               
  ht.rows(input + 8 * 16, row_output + 8, dest, stride, 0);  

  
  ht.cols(row_output, NULL, dest, stride, 0);               
  ht.cols(row_output + 16 * 8, NULL, dest + 8, stride, 0);  
}
