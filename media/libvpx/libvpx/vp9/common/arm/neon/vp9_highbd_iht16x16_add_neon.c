...
void vp9_highbd_iht16x16_256_add_neon(const tran_low_t *input, uint16_t *dest,
  ...
  if (bd == 8) {
    ...
    ht.rows(input, row_output, dest, stride, 1);               
    ht.rows(input + 8 * 16, row_output + 8, dest, stride, 1);  

    
    ht.cols(row_output, NULL, dest, stride, 1);               
    ht.cols(row_output + 16 * 8, NULL, dest + 8, stride, 1);  
  } else {
    static const highbd_iht_2d IHT_16[] = {
      ...
      { vpx_highbd_idct16x16_256_add_half1d,
        highbd_iadst16_neon },                      
      { highbd_iadst16_neon, highbd_iadst16_neon }  
    };
    const highbd_iht_2d ht = IHT_16[tx_type];
    int32_t row_output[16 * 16];

    
    ht.rows(input, row_output, dest, stride, bd);               
    ht.rows(input + 8 * 16, row_output + 8, dest, stride, bd);  

    
    ht.cols(row_output, NULL, dest, stride, bd);  
