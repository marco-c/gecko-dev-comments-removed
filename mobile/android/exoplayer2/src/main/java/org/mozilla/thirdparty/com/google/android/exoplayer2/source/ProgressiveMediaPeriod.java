...
import org.checkerframework.checker.nullness.compatqual.NullableType;
 ...
 final class ProgressiveMediaPeriod
    ...
    if (retryDelayMs == C.TIME_UNSET) {
      ...
      loadErrorAction =
          configureRetry(loadable, extractedSamplesCount)
              ? Loader.createRetryAction( madeProgress, retryDelayMs)
              : Loader.DONT_RETRY;
    }
  ...
  public void seekMap(SeekMap seekMap) {
    this.seekMap = icyHeaders == null ? seekMap : new Unseekable( C.TIME_UNSET);
    handler.post(maybeFinishPrepareRunnable);
  }
  ...
  private void startLoading() {
    ExtractingLoadable loadable =
        new ExtractingLoadable(
            uri, dataSource, extractorHolder,  this, loadCondition);
    if (prepared) {
      SeekMap seekMap = getPreparedState().seekMap;
   ...
   final class ExtractingLoadable implements Loadable, IcyDataSource.Listener {
    ...
    public void onIcyMetadata(ParsableByteArray metadata) {
      ...
      icyTrackOutput.sampleMetadata(
          timeUs, C.BUFFER_FLAG_KEY_FRAME, length,  0,  null);
