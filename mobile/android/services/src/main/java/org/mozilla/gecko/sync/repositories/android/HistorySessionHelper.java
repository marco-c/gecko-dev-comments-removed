...
import java.util.ArrayList;
 ...
 class HistorySessionHelper extends SessionHelper {
    ...
    private ArrayList<HistoryRecord> recordsBuffer = new ArrayList<HistoryRecord>();
     ...
     HistorySessionHelper(StoreTrackingRepositorySession session, DataAccessor dbHelper) {
        super(session, dbHelper);
    }

    @Override
     Record retrieveDuringFetch(Cursor cur) {
        return RepoUtils.historyFromMirrorCursor(cur);
    }

    @Override
     Runnable getWipeRunnable(RepositorySessionWipeDelegate delegate) {
    ...
    @Override
     void storeRecordDeletion(RepositorySessionStoreDelegate storeDelegate, Record record, Record existingRecord) {
    ...
    @Override
     Record processBeforeInsertion(Record toProcess) {
        return toProcess;
    }
    ...
    private void flushNewRecords(RepositorySessionStoreDelegate delegate) throws NullCursorException {
        ...
        delegate.onRecordStoreSucceeded(outgoing.size()); 
    }
}
