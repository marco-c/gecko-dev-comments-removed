...
import org.mozilla.gecko.sync.repositories.domain.Record;
 ...
 abstract class SessionHelper {
    ...
    abstract Runnable getStoreDoneRunnable(RepositorySessionStoreDelegate delegate);
     ...
     void updateBookkeeping(Record record)
            throws NoGuidForIdException, NullCursorException, ParentNotFoundException {
    ...
    private void putRecordToGuidMap(String recordString, String guid)
     ...
     Runnable getStoreRunnable(final Record record, final RepositorySessionStoreDelegate storeDelegate) {
        return new Runnable() {
            @Override
            public void run() {
                if (!session.isActive()) {
    ...
    private void createRecordToGuidMap() throws NoGuidForIdException, NullCursorException, ParentNotFoundException {
     ...
     Runnable getFetchSinceRunnable(long since, long end, RecordFilter filter, RepositorySessionFetchRecordsDelegate delegate) {
        return new FetchSinceRunnable(since, end, filter, delegate);
    }

     Runnable getFetchRunnable(String[] guids,
                                           long end,
                                           RecordFilter filter,
    ...
    private class FetchRunnable extends FetchingRunnable {
     ...
     abstract class FetchingRunnable implements Runnable {
        protected final RepositorySessionFetchRecordsDelegate delegate;
         ...
         FetchingRunnable(RepositorySessionFetchRecordsDelegate delegate) {
            this.delegate = delegate;
        }

         void fetchFromCursor(Cursor cursor, RecordFilter filter, long end) {
     ...
     class WipeRunnable implements Runnable {
        protected RepositorySessionWipeDelegate delegate;
         ...
         WipeRunnable(RepositorySessionWipeDelegate delegate) {
            this.delegate = delegate;
        }
