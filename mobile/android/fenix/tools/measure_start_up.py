...
FOCUS_CHANNEL_TO_PKG = {
    'nightly': 'org.mozilla.focus',  
    'beta': 'org.mozilla.focus.beta',  
...
def measure(test_name, product, pkg_id, start_cmd_args, iter_count):
    ...
    force_stop(pkg_id)
    subprocess.run(start_cmd_args, check=True, capture_output=True)  
    time.sleep(5)  
    ...
    for _ in range(0, iter_count):
        ...
        proc = subprocess.run(start_cmd_args, check=True, capture_output=True)  
...
def get_measurement_from_nav_start_logcat(product, pkg_id, logcat_bytes):
    ...
    def get_page_start_datetime():
        ...
        if is_old_version_of_focus:
            assert page_start_line_count == 3, page_start_assert_msg  
        else:
            assert page_start_line_count == 2, page_start_assert_msg  
        return line_to_datetime(page_start_lines[1])  

    logcat = logcat_bytes.decode('UTF-8')  
    lines = logcat.split('\n')
