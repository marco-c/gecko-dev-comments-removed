...
public final class FlacExtractor implements Extractor {
  ...
  public FlacExtractor(int flags) {
    ...
    buffer = new ParsableByteArray(new byte[BUFFER_LENGTH],  0);
  ...
  private void readId3Metadata(ExtractorInput input) throws IOException, InterruptedException {
    id3Metadata = FlacMetadataReader.readId3Metadata(input,  !id3MetadataDisabled);
  ...
  private @ReadResult int readFrames(ExtractorInput input, PositionHolder seekPosition)
    ...
    if (currentLimit < BUFFER_LENGTH) {
      int bytesRead =
          input.read(
              buffer.data,  currentLimit,  BUFFER_LENGTH - currentLimit);
    ...
    if (buffer.bytesLeft() < FlacConstants.MAX_FRAME_HEADER_SIZE) {
      ...
      System.arraycopy(
          buffer.data, buffer.getPosition(), buffer.data,  0, buffer.bytesLeft());
