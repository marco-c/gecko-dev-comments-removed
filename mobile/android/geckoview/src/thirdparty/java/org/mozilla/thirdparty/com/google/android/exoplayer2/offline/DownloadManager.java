...
public final class DownloadManager {
  ...
  public DownloadManager(
    ...
    pendingMessages = 1;
    internalHandler
        .obtainMessage(MSG_INITIALIZE, notMetRequirements,  0)
        .sendToTarget();
  }
  ...
  private void setDownloadsPaused(boolean downloadsPaused) {
    ...
    pendingMessages++;
    internalHandler
        .obtainMessage(MSG_SET_DOWNLOADS_PAUSED, downloadsPaused ? 1 : 0,  0)
        .sendToTarget();
  ...
  private void onRequirementsStateChanged(
    ...
    if (this.notMetRequirements != notMetRequirements) {
      ...
      pendingMessages++;
      internalHandler
          .obtainMessage(MSG_SET_NOT_MET_REQUIREMENTS, notMetRequirements,  0)
          .sendToTarget();
    }
    boolean waitingForRequirementsChanged = updateWaitingForRequirements();
    for (Listener listener : listeners) {
  ...
  private static final class InternalHandler extends Handler {
    ...
    private void setStopReason(@Nullable String id, int stopReason) {
      if (id == null) {
        ...
        @Nullable Download download = getDownload(id,  false);
        if (download != null) {
    ...
    private void addDownload(DownloadRequest request, int stopReason) {
      @Nullable Download download = getDownload(request.id,  true);
    ...
    private void removeDownload(String id) {
      @Nullable Download download = getDownload(id,  true);
      if (download == null) {
    ...
    private void removeAllDownloads() {
      ...
      for (int i = 0; i < downloads.size(); i++) {
        DownloadUpdate update =
            new DownloadUpdate(downloads.get(i),  false, updateList);
    ...
    private void onContentLengthChanged(Task task) {
      ...
      Download download =
          Assertions.checkNotNull(getDownload(downloadId,  false));
    ...
    private void onDownloadTaskStopped(Download download, @Nullable Throwable finalError) {
      ...
      DownloadUpdate update =
          new DownloadUpdate(download,  false, new ArrayList<>(downloads));
      mainHandler.obtainMessage(MSG_DOWNLOAD_UPDATE, update).sendToTarget();
    }
    ...
    private void onRemoveTaskStopped(Download download) {
      if (download.state == STATE_RESTARTING) {
        ...
        DownloadUpdate update =
            new DownloadUpdate(download,  true, new ArrayList<>(downloads));
        mainHandler.obtainMessage(MSG_DOWNLOAD_UPDATE, update).sendToTarget();
      }
    }
