...
public final class PlaybackStatsListener
  ...
  public void finishAllSessions() {
    ...
    for (String session : trackerCopy.keySet()) {
      onSessionFinished(dummyEventTime, session,  false);
    }
  }
  ...
  public void onSessionCreated(EventTime eventTime, String session) {
    ...
    tracker.onPlayerStateChanged(
        eventTime, playWhenReady, playbackState,  true);
    tracker.onIsSuppressedChanged(eventTime, isSuppressed,  true);
  ...
  public void onSessionFinished(EventTime eventTime, String session, boolean automaticTransition) {
    ...
    if (automaticTransition) {
      ...
      tracker.onPlayerStateChanged(
          eventTime,  true, Player.STATE_ENDED,  false);
    }
    ...
    public PlaybackStats build(boolean isFinal) {
      ...
      return new PlaybackStats(
          ...
          firstReportedTimeMs,
           ...
           hasBeenReady ? 0 : 1,
           hasEnded ? 1 : 0,
           hasBackgroundJoin ? 1 : 0,
          validJoinTimeMs,
           isJoinTimeInvalid ? 0 : 1,
          pauseCount,
          pauseBufferCount,
          seekCount,
          rebufferCount,
          ...
          audioFormatBitrateTimeProduct,
           initialVideoFormatHeight == C.LENGTH_UNSET ? 0 : 1,
           initialVideoFormatBitrate == C.LENGTH_UNSET ? 0 : 1,
          initialVideoFormatHeight,
          initialVideoFormatBitrate,
           initialAudioFormatBitrate == C.LENGTH_UNSET ? 0 : 1,
          initialAudioFormatBitrate,
          bandwidthTimeMs,
          bandwidthBytes,
          droppedFrames,
          audioUnderruns,
           fatalErrorCount > 0 ? 1 : 0,
          fatalErrorCount,
          nonFatalErrorCount,
          fatalErrorHistory,
