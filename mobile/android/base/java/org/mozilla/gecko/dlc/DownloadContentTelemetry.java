...
public class DownloadContentTelemetry {
    ...
    @StringDef({ERROR_NO_NETWORK, ERROR_NETWORK_METERED, ERROR_DISK_SPACE, ERROR_CHECKSUM, ERROR_DISK_IO,
     ...
     static void eventDownloadSuccess(DownloadContent content) {
        try {
     ...
     static void eventDownloadFailure(DownloadContent content, RecoverableDownloadContentException e) {
        eventDownloadFailure(content, translateErrorType(e.getErrorType()));
    }

     static void eventDownloadFailure(DownloadContent content, @Error String errorType) {
        try {
            final JSONObject extras = new JSONObject();
            extras.put(EXTRA_ACTION, ACTION_DOWNLOAD);
     ...
     static void eventSyncSuccess(boolean updated, boolean actionRequired) {
        try {
            final JSONObject extras = new JSONObject();
     ...
     static void eventSyncFailure(RecoverableDownloadContentException e) {
        eventSyncFailure(translateErrorType(e.getErrorType()));
    }

     static void eventSyncFailure(@Error String errorType) {
        try {
            final JSONObject extras = new JSONObject();
            extras.put(EXTRA_ACTION, ACTION_SYNC);
            extras.put(EXTRA_RESULT, RESULT_FAILURE);
