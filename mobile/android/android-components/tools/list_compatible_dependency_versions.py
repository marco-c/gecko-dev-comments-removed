...
Please send feedback to @mcomella.""".format(
...
PATH_AC_VERSION = os.path.join(
    "buildSrc", "src", "main", "java", "AndroidComponents.kt"
)
PATH_GV_VERSION = os.path.join(
...
def validate_args():
    if len(sys.argv) != 2:  
        ...
        raise Exception(
            "expected one argument: path-to-fenix-repository or --no-fenix. See usage above."
        )
...
def get_hash_from_pom(pom_str, debug_pom_url=None):
    ...
    if not mc_hash:
        raise Exception(
            "mc hash could not be found in pom.xml from...\n"
            + INDENT
            + str(debug_pom_url)
        )
    return mc_hash
...
def validate_ac_version(ac_version):
    ...
    is_nightly_url = (
        len(bugfix_version) > 4
    )  
    if not is_nightly_url:
        raise NotImplementedError(
            "AC release versions not yet implemented: try using a nightly ac. Attempted to fetch version {}.".format(
                ac_version
            )
        )
    ...
    if int(bugfix_version) < int(VERSION_MIN_AC_NIGHTLY_BUGFIX):
        raise Exception(
            "Found AC nightly version {} is too old: use version {} or newer (e.g. use a newer version of fenix).".format(
                ac_version, VERSION_MIN_AC_NIGHTLY
            )
        )
...
def fenix_checkout_to_ac_version(fenix_path):
    ...
    raise Exception(
        'Unable to find ac version from fenix repository file "{}". This is likely a bug where the file path or format has changed.'.format(
            ac_version_path
        )
    )
...
def validate_gv_nightly_version(version):
    ...
    if int(major) < int(VERSION_MIN_GV_NIGHTLY_MAJOR):
        raise Exception(
            "Found GV nightly version {} is too old: use major version {} or newer (e.g. use a newer version of ac).".format(
                version, VERSION_MIN_GV_NIGHTLY_MAJOR
            )
        )
...
def ac_checkout_to_gv_version(ac_root):
    ...
    with open(gv_version_path) as f:
        ...
        if not version:
            raise Exception(
                'Unable to find gv versions from ac repository file "{}". This is likely a bug where the file path or format has changed.'.format(
                    gv_version_path
                )
            )
    return version
...
def main_repo_to_hash(fenix_path, is_no_fenix_passed):
    ...
    (
        mc_hash,
        nightlyv,
    ) = ac_checkout_to_mc_hash(PATH_AC_ROOT)
    print("Building ac with a local GeckoView?")
    print(
        INDENT
        + "The last known GV nightly version that cleanly builds with your ac checkout is..."
    )
