...
class LeanplumManifestParser {
  ...
  static String decompressXml(byte[] xml) {
    ...
    while (off < xml.length) {
      ...
      if (tag0 == START_TAG) {
        int numbAttrs = littleEndianValue(xml, off + 7 * 4);  
        off += 9 * 4;  
        String name = compXmlString(xml, sitOff, stOff, nameSi);
        
        StringBuilder sb = new StringBuilder();
        for (int ii = 0; ii < numbAttrs; ii++) {
          int attrNameSi = littleEndianValue(xml, off + 4);  
          int attrValueSi = littleEndianValue(xml, off + 2 * 4); 
          int attrResId = littleEndianValue(xml, off + 4 * 4);  
  ...
  private static int scanForFirstStartTag(byte[] xml) {
    ...
    int xmlTagOff = littleEndianValue(xml, 3 * 4);  
