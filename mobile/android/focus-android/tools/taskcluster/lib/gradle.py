...
def get_variant(build_type, product):
    print("Fetching variant information for build_type='{}', product='{}'".format(build_type, product))
    output = _run_gradle_process('printVariant', variantBuildType=build_type, variantProduct=product)
    content = _extract_content_from_command_output(output, prefix='variant: ')
    raw_variant = json.loads(content)
    return Variant(
        raw_variant['name'],
        build_type,
        [VariantApk(build_type, raw_apk['abi'], product, raw_apk['fileName']) for raw_apk in raw_variant['apks']]
    )
