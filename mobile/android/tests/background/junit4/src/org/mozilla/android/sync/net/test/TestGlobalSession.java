...
public class TestGlobalSession {
  ...
  public void testBackoffCalledByHandleHTTPError() {
    try {
      ...
      response.setHeader("X-Weave-Backoff", Long.toString(TEST_BACKOFF_IN_SECONDS)); 
      ...
      assertEquals(TEST_BACKOFF_IN_SECONDS * 1000, callback.weaveBackoff); 
    } catch (Exception e) {
      e.printStackTrace();
      fail("Got exception.");
    }
    ...
    try {
      ...
      final GlobalSyncStage stage = new MockAbstractNonRepositorySyncStage() {
        ...
        public void execute() {
          ...
          response.addHeader("X-Weave-Backoff", Long.toString(TEST_BACKOFF_IN_SECONDS)); 
