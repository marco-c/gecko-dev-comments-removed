...
function NtesMailInfo(){
	...
	this.render = function (){
		...
		if(this.hasnew){
			...
			this.$("dvNewMsg").style.display = "";		
			
			this.$("imgNewMsg").title = "ÄúÓÐ"+ this.newCount +"·âÎ´¶ÁÓÊ¼þ";
			this.$("lnkNewMsg").innerHTML = this.newCount > 999 ? "999+" : this.newCount; 
			if(this.newCount == 0){ 
			...
			this.$("lnkNewMsg").href = this.$("lnkMsgImg").href = this.getLoginUrl(); 
		}else{
	...
	this.P_INFO = this.getCookie("P_INFO");			
	this.cm_newmsg = this.getCookie("cm_newmsg");	
	this.isLogin = this.getCookie("S_INFO") ? true : false; 
	this.username = "";								
	this.hasnew = false;							
	this.domain = "";						
	this.newCount = 0;								
