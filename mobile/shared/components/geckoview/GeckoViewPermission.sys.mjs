...
export class GeckoViewPermission {
  ...
  async promptPermission(aRequest) {
    ...
    const windowEventDispatcher = lazy.EventDispatcher.for(window);

    let allowOrDeny;
    try {
      allowOrDeny = await windowEventDispatcher.sendRequestForResult(
        "GeckoView:ContentPermission",
        {
          ...
          contextId:
            principal.originAttributes.geckoViewSessionContextId ?? null,
