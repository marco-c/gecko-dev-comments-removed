...
add_setup(async () => {
  ...
  Services.prefs.setBoolPref("services.settings.startup_bundle_enabled", true);
  ...
  registerCleanupFunction(() => {
    ...
    Services.prefs.clearUserPref("services.settings.startup_bundle_enabled");
  });
});
...
add_task(async function test_startup_bundle_can_be_disabled_via_pref() {
  Services.prefs.setBoolPref("services.settings.startup_bundle_enabled", false);
  const client = startupClients[0];

  let error;
  try {
    await client.get({ emptyListFallback: false });
...
add_task(
  async function test_signature_of_extracted_data_from_bundle_is_verified() {
    const c = RemoteSettings("tracking-protection-lists"); 
