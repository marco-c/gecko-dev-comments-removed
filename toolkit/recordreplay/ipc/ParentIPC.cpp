...
SpawnRecordingChild()
{
  ...
  gRecordingChild =
    new ChildProcessInfo(MakeUnique<ChildRoleActive>(),  true);
}
...
SpawnSingleReplayingChild()
{
  ...
  gFirstReplayingChild =
    new ChildProcessInfo(MakeUnique<ChildRoleActive>(),  false);
}
...
SpawnReplayingChildren()
{
  ...
  gFirstReplayingChild =
    new ChildProcessInfo(std::move(firstRole),  false);
  gSecondReplayingChild =
    new ChildProcessInfo(MakeUnique<ChildRoleStandby>(),  false);
...
SaveRecordingInternal(char* aFilename)
{
  ...
  int readfd = DirectOpenFile(gRecordingFilename,  false);
  int writefd = DirectOpenFile(aFilename,  true);
  char buf[4096];
  while (true) {
