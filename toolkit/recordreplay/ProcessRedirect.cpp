...
Redirect(size_t aCallId, Redirection& aRedirection, Assembler& aAssembler, bool aFirstPass)
{
  ...
  if (!MaybeInternalJumpTarget(ro, extent)) {
    ...
    AddJumpPatch(functionStart, newFunction,  false);
  ...
  uint8_t* newFunction = GenerateRedirectStub(aAssembler, aCallId);
  AddJumpPatch(ro + JumpBytesClobberRax, newFunction,  false);
...
InitializeRedirections()
{
...

  {
    ...
    for (size_t i = 0;; i++) {
      ...
      Redirect(i, redirection, assembler,  true);
    }
    ...
    for (size_t i = 0;; i++) {
      ...
      Redirect(i, redirection, assembler,  false);
    }
  }
