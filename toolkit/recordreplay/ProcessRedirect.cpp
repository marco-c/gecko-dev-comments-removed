...
Redirect(Redirection& aRedirection, Assembler& aAssembler, bool aFirstPass)
{
  ...
  if (!MaybeInternalJumpTarget(ro, extent)) {
    ...
    AddJumpPatch(functionStart, aRedirection.mNewFunction,  false);
  ...
  AddJumpPatch(ro, firstJumpTarget,  false);

  
  AddJumpPatch(ro + JumpBytesClobberRax, aRedirection.mNewFunction,  false);
  AddClobberPatch(ro + 2 * JumpBytesClobberRax, afterip);

  
  AddJumpPatch(functionStart, ro + JumpBytesClobberRax,  true);
...
InitializeRedirections()
{
...

  {
    ...
    for (size_t i = 0;; i++) {
      ...
      Redirect(redirection, assembler,  true);
    }
    ...
    for (size_t i = 0;; i++) {
      ...
      Redirect(redirection, assembler,  false);
    }
  }
