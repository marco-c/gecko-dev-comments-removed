...
private:
  ...
  typename ScaffoldingObjectConverter<&kRemoteSettingsRemoteSettingsServicePointerType>::IntermediateType mRemoteSettings;
...
public:
  void PrepareRustArgs(const dom::Sequence<dom::UniFFIScaffoldingValue>& aArgs, ErrorResult& aError) override {
    ...
    ScaffoldingObjectConverter<&kRemoteSettingsRemoteSettingsServicePointerType>::FromJs(aArgs[1], &mRemoteSettings, aError);
    if (aError.Failed()) {
      return;
    }
  }
  ...
  void MakeRustCall(RustCallStatus* aOutStatus) override {
    mUniffiReturnValue = ScaffoldingObjectConverter<&kRelevancyRelevancyStorePointerType>::FromRust(
      uniffi_relevancy_fn_constructor_relevancystore_new(
        ...
        ScaffoldingObjectConverter<&kRemoteSettingsRemoteSettingsServicePointerType>::IntoRust(std::move(mRemoteSettings)),
        aOutStatus
      )
    );
  }
