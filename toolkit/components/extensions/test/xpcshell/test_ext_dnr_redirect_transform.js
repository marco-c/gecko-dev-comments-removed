...
server.identity.add("http", "dest", 443); 
server.identity.add("http", "dest", 700); 
server.identity.add("http", "dest", 777); 
...
add_task(async function test_redirect_transform_host() {
  await runAsDNRExtension({
    background: async dnrTestUtils => {
      ...
      await setRedirectTransform({ host: "%44%65%73%54" }); 
      await testFetch(
        "http://from:777/",
        "http://dest:777/",
