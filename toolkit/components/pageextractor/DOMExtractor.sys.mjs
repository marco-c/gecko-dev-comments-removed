...
class ExtractionContext {
  ...
  constructor(document, options) {
    ...
    if (options.justViewport) {
      const { width, height } = document.defaultView.visualViewport;
      this.#viewportRect = {
        top: 0,
        left: 0,
        right: width,
  ...
  maybeAppendTextContent(node) {
    ...
    if (element) {
      if (this.#options.includeLinks && this.#hasInlineAnchors(element)) {
...
export function extractTextFromDOM(document, options) {
  ...
  return {
    ...
    links: options.includeLinks ? context.links : undefined,
  };
}
