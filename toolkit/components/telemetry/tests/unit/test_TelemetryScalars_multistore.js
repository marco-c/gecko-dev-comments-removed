...
add_task(async function test_multistore_basics() {
  ...
  const mainKeyedScalars = getParentSnapshot("main", true );
  const syncKeyedScalars = getParentSnapshot("sync", true );
...
add_task(async function test_multistore_uint() {
  ...
  let syncScalars = getParentSnapshot("sync", false , true );
...
add_task(async function test_empty_absence() {
  ...
  let snapshot = getParentSnapshot("main", false , true );
  ...
  snapshot = getParentSnapshot("main", false , true );
  Assert.equal(snapshot, undefined,
               `Parent snapshot must be empty if no data.`);


  snapshot = getParentSnapshot("sync", false , true );
  Assert.ok(MULTIPLE_STORES in snapshot,
...
add_task(async function test_empty_absence_keyed() {
  ...
  let snapshot = getParentSnapshot("main", true , true );
  ...
  snapshot = getParentSnapshot("main", true );
  Assert.equal(snapshot, undefined,
               `Parent snapshot should be empty if no data.`);
  snapshot = getParentSnapshot("sync", true );
