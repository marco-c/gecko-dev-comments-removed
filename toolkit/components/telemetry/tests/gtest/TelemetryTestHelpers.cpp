                   ...
                   ProcessID aProcessType)
{
  ...
  if (aKeyed) {
    rv = telemetry->GetSnapshotForKeyedScalars(NS_LITERAL_CSTRING("main"), false, false , aCx, &scalarsSnapshot);
  } else {
    rv = telemetry->GetSnapshotForScalars(NS_LITERAL_CSTRING("main"), false, false , aCx, &scalarsSnapshot);
  }
             ...
             const char* name, JS::MutableHandleValue valueOut, bool is_keyed)
{
  ...
  nsresult rv = is_keyed ? mTelemetry->GetSnapshotForKeyedHistograms(NS_LITERAL_CSTRING("main"), false, false , cx, &snapshots)
                         : mTelemetry->GetSnapshotForHistograms(NS_LITERAL_CSTRING("main"), false, false , cx, &snapshots);
