...
TEST_F(TelemetryTestFixture, ScalarEventSummary) {
  ...
  TelemetryScalar::SummarizeEvent(nsCString(kLongestEvent), ProcessID::Parent, false );
  ...
  TelemetryScalar::SummarizeEvent(nsCString(kTooLongEvent), ProcessID::Parent, false );
  ...
  for (int i = 1; i < 500; i++) {
    ...
    TelemetryScalar::SummarizeEvent(nsCString(eventName.str().c_str()), ProcessID::Parent, false );
  }
...
#ifndef DEBUG
  TelemetryScalar::SummarizeEvent(nsCString("whoops#too#many"), ProcessID::Parent, false );
...
TEST_F(TelemetryTestFixture, ScalarEventSummary_Dynamic) {
  ...
  const char* kLongestEvent = "oohwowlookthiscategoryissolong#thismethodislongtooo#thisobjectisnoslouch";
  TelemetryScalar::SummarizeEvent(nsCString(kLongestEvent), ProcessID::Parent, true );
  TelemetryScalar::SummarizeEvent(nsCString(kLongestEvent), ProcessID::Content, true );
