...
// Please also update the expected domain count in
// test_convert_provider_infos_to_match_patterns.js
...
const staticProviderList = [
  {
    name: "google",
    domains: gGoogleDomainsSource.split(" ").map(d => "www" + d),
  },
];

/**
 * Provides static data about search engines that is consistent across all
 * locales, such as the list of known alternate domains for a given provider.
 */
class StaticData {
  #providerNameToDomains = new Map(
    staticProviderList.map(p => [p.name, p.domains])
  );
  ...
  getAlternateDomains(aDomain) {
    for (let domains of this.#providerNameToDomains.values()) {
      if (domains.includes(aDomain)) {
        return [...domains];
      }
    }
    return [];
  ...
  /**
   * Returns a list of domains for a given provider name.
   *
   * @param {string} name
   *   The provider name to look up, e.g. "google".
   *
   * @returns {string[]}
   *   Containing one entry for each host name, or empty array if none is known.
   */
  getAlternateDomainsForProvider(name) {
    return [...(this.#providerNameToDomains.get(name) ?? [])];
  }
}

export const SearchStaticData = new StaticData();
