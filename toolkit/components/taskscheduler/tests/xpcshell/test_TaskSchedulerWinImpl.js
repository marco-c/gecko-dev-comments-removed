...
add_task(async function test_create() {
  ...
  const intervalSecsIn = 2 * 60 * 60; 
  const expectedIntervalOutWin10 = "PT2H"; 
  const expectedIntervalOutWin7 = `PT${intervalSecsIn}S`; 
  ...
  const updatedIntervalSecsIn = 3 * 60 * 60; 
  ...
  Assert.equal(WinSvc.validateTaskDefinition(enabledXML), 0 );
  ...
  Assert.equal(WinSvc.validateTaskDefinition(basicXML), 0 );
});
...
add_task(async function test_migrate() {
  ...
  const description = "Entities: < &. Non-ASCII: abc😀def.";
  const intervalSecsIn = 2 * 60 * 60; 
  const expectedIntervalOut = "PT2H"; 
  ...
  Assert.equal(WinSvc.validateTaskDefinition(newTaskXML), 0 );
