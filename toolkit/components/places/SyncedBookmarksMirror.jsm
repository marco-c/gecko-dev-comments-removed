...
class BookmarkObserverRecorder {
  ...
  noteGuidChanged(info) {
    ...
    this.bookmarkObserverNotifications.push({
      name: "onItemChanged",
      isTagging: false,
      args: [info.id, "guid",  false, info.newGuid,
  ...
  noteAnnoChanged(info) {
    ...
    this.bookmarkObserverNotifications.push({
      name: "onItemChanged",
      isTagging: false,
      args: [info.id, info.name,  true,
              "", info.lastModified, info.type, info.parentId,
             info.guid, info.parentGuid,  "",
             PlacesUtils.bookmarks.SOURCES.SYNC],
    });
  }
