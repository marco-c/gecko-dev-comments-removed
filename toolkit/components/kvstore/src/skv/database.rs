...
impl<'a> Database<'a> {
    ...
    pub fn enumerate(
        ...
        reader.read(|conn| {
            ...
            let params: &[_] = match (fragment.start_param(), fragment.end_param()) {
                
                
                (Some(p), Some(q)) => &[(":name", &self.name as &dyn ToSql), p, q],
                
                
                (Some(p), None) | (None, Some(p)) => &[(":name", &self.name as &dyn ToSql), p],
                
                (None, None) => &[(":name", &self.name as &dyn ToSql)],
            };
