...
#define ACCOUNT_STATE_TABLE_NAME "identity"_ns
...
IdentityCredentialStorageService::~IdentityCredentialStorageService() {
  ...
  Unused << mDatabase->Close();
  Unused << mPrivateBrowsingDatabase->Close();
  mDatabase = nullptr;
  mPrivateBrowsingDatabase = nullptr;
}
...
nsresult IdentityCredentialStorageService::Init() {
  ...
  rv = storage->OpenSpecialDatabase(kMozStorageMemoryStorageKey, VoidCString(),
                                    mozIStorageService::CONNECTION_DEFAULT,
                                    getter_AddRefs(mPrivateBrowsingDatabase));
  NS_ENSURE_SUCCESS(rv, rv);
  ...
  if (rv == NS_ERROR_FILE_CORRUPTED) {
    ...
    NS_ENSURE_SUCCESS(rv, rv);
    rv = storage->OpenDatabase(profileDir,
                               mozIStorageService::CONNECTION_DEFAULT,
                               getter_AddRefs(mDatabase));
  }
  NS_ENSURE_SUCCESS(rv, rv);
  ...
  bool ready = false;
  mPrivateBrowsingDatabase->GetConnectionReady(&ready);
  NS_ENSURE_TRUE(ready, NS_ERROR_UNEXPECTED);
  ready = false;
  mDatabase->GetConnectionReady(&ready);
  NS_ENSURE_TRUE(ready, NS_ERROR_UNEXPECTED);

  
  rv = createTable(mPrivateBrowsingDatabase);
  NS_ENSURE_SUCCESS(rv, rv);
  bool tableExists = false;
  mDatabase->TableExists(ACCOUNT_STATE_TABLE_NAME, &tableExists);
  if (!tableExists) {
    rv = createTable(mDatabase);
    NS_ENSURE_SUCCESS(rv, rv);
  }
...
NS_IMETHODIMP IdentityCredentialStorageService::SetState(
  ...
  if (OriginAttributes::IsPrivateBrowsing(rpOrigin)) {
    rv = mPrivateBrowsingDatabase->CreateStatement(upsert_query,
                                                   getter_AddRefs(stmt));
    NS_ENSURE_SUCCESS(rv, rv);
  } else {
    rv = mDatabase->CreateStatement(upsert_query, getter_AddRefs(stmt));
    NS_ENSURE_SUCCESS(rv, rv);
  }
...
NS_IMETHODIMP IdentityCredentialStorageService::GetState(
  ...
  nsCOMPtr<mozIStorageStatement> stmt;
  if (OriginAttributes::IsPrivateBrowsing(rpOrigin)) {
    rv = mPrivateBrowsingDatabase->CreateStatement(selectQuery,
                                                   getter_AddRefs(stmt));
    NS_ENSURE_SUCCESS(rv, rv);
  } else {
    rv = mDatabase->CreateStatement(selectQuery, getter_AddRefs(stmt));
    NS_ENSURE_SUCCESS(rv, rv);
  }
...
NS_IMETHODIMP IdentityCredentialStorageService::Delete(
  ...
  NS_ENSURE_SUCCESS(rv, rv);
  if (OriginAttributes::IsPrivateBrowsing(rpOrigin)) {
    rv = mPrivateBrowsingDatabase->CreateStatement(deleteQuery,
                                                   getter_AddRefs(stmt));
    NS_ENSURE_SUCCESS(rv, rv);
  } else {
    rv = mDatabase->CreateStatement(deleteQuery, getter_AddRefs(stmt));
    NS_ENSURE_SUCCESS(rv, rv);
  }
  nsCString idpOrigin;
  rv = aIDPPrincipal->GetOrigin(idpOrigin);
...
NS_IMETHODIMP IdentityCredentialStorageService::Clear() {
  ...
  nsresult rv = mPrivateBrowsingDatabase->ExecuteSimpleSQL(
      nsLiteralCString("DELETE FROM identity;"));
  NS_ENSURE_SUCCESS(rv, rv);
  rv = mDatabase->ExecuteSimpleSQL(nsLiteralCString("DELETE FROM identity;"));
  NS_ENSURE_SUCCESS(rv, rv);
  return NS_OK;
}
...
IdentityCredentialStorageService::DeleteFromOriginAttributesPattern(
  ...
  if (!oaPattern.mPrivateBrowsingId.WasPassed() ||
    ...
    database = mPrivateBrowsingDatabase;
  }

  database->BeginTransaction();
  Vector<int64_t> rowIdsToDelete;
  nsCOMPtr<mozIStorageStatement> stmt;
  rv = database->CreateStatement(
...
NS_IMETHODIMP IdentityCredentialStorageService::DeleteFromTimeRange(
  ...
  rv = mPrivateBrowsingDatabase->CreateStatement(deleteTimeQuery,
                                                 getter_AddRefs(stmt));
  NS_ENSURE_SUCCESS(rv, rv);
  ...
  rv = mDatabase->CreateStatement(deleteTimeQuery, getter_AddRefs(stmt));
  NS_ENSURE_SUCCESS(rv, rv);
  rv = stmt->BindInt64ByIndex(0, aStart);
  NS_ENSURE_SUCCESS(rv, rv);
  rv = stmt->BindInt64ByIndex(1, aEnd);
...
NS_IMETHODIMP IdentityCredentialStorageService::
  ...
  NS_ENSURE_SUCCESS(rv, rv);
  if (OriginAttributes::IsPrivateBrowsing(rpOrigin)) {
    rv = mPrivateBrowsingDatabase->CreateStatement(deletePrincipalQuery,
                                                   getter_AddRefs(stmt));
    NS_ENSURE_SUCCESS(rv, rv);
  } else {
    rv = mDatabase->CreateStatement(deletePrincipalQuery, getter_AddRefs(stmt));
    NS_ENSURE_SUCCESS(rv, rv);
  }
  rv = stmt->BindUTF8StringByIndex(0, rpOrigin);
  NS_ENSURE_SUCCESS(rv, rv);
...
NS_IMETHODIMP IdentityCredentialStorageService::DeleteFromBaseDomain(
  ...
  rv = mPrivateBrowsingDatabase->CreateStatement(deleteBaseDomainQuery,
                                                 getter_AddRefs(stmt));
  NS_ENSURE_SUCCESS(rv, rv);
  ...
  rv = mDatabase->CreateStatement(deleteBaseDomainQuery, getter_AddRefs(stmt));
  NS_ENSURE_SUCCESS(rv, rv);
  rv = stmt->BindUTF8StringByIndex(0, aBaseDomain);
  NS_ENSURE_SUCCESS(rv, rv);
  rv = stmt->Execute();
