...
add_task(async function test() {
  ...
  await BrowserTestUtils.withNewTab(url, async function(browser) {
    await ContentTask.spawn(browser, adjustedCerts, async function(
      adjustedCerts
    ) {
      let certificateSectionDiv;
      await ContentTaskUtils.waitForCondition(() => {
        certificateSectionDiv = content.document.querySelector(
          "certificate-section"
        );
        return certificateSectionDiv != null;
      }, "Certificate section found");

      let infoGroups = certificateSectionDiv.shadowRoot.querySelectorAll(
        "info-group"
      );
