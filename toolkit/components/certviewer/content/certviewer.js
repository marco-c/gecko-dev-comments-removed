...
const render = certs => {
  customElements.whenDefined("certificate-section").then(() => {
    let CertificateSection = customElements.get("certificate-section");
    document
      .querySelector("body")
      .append(new CertificateSection([certs.certItems], certs.tabName));
  });
};

const buildChain = async certBase64 => {
  let certDER = pemToDER(certBase64);
  let cert = await parse(certDER);
  let adjustedCerts = adjustCertInformation(cert);
  render(adjustedCerts);
};
