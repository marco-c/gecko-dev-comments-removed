...
const getPublicKeyInfo = x509 => {
  ...
  if (spki.kty === "RSA") {
    spki.e = b64urltodec(spki.e); 
    spki.keysize = b64urltohex(spki.n).length * 8; 
    spki.n = hashify(b64urltohex(spki.n)); 
  } else if (spki.kty === "EC") {
    spki.kty = "Elliptic Curve";
    spki.keysize = parseInt(spki.crv.split("-")[1]); 
    spki.x = hashify(b64urltohex(spki.x)); 
    spki.y = hashify(b64urltohex(spki.y)); 
    spki.xy = `04:${spki.x}:${spki.y}`; 
  }
  return spki;
};
