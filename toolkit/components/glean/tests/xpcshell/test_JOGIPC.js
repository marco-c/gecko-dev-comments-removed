...
add_task({ skip_if: () => runningInParent }, async function run_child_stuff() {
  ...
  Glean.jogIpc.jogTimingDist.stopAndAccumulate(t2); 
  Glean.jogIpc.jogTimingDist.stopAndAccumulate(t3); 
  ...
  Glean.jogIpc.jogLabeledTimingDist.label1.stopAndAccumulate(l2); 
  Glean.jogIpc.jogLabeledTimingDist.label1.stopAndAccumulate(l3); 
});
