...
add_task(async function test_jog_timing_distribution_works() {
  ...
  Glean.jogCat.jogTimingDist.stopAndAccumulate(t2); 
  Glean.jogCat.jogTimingDist.stopAndAccumulate(t3); 
  
  Glean.jogCat.jogTimingDist.accumulateSingleSample(5000); 
  Glean.jogCat.jogTimingDist.accumulateSamples([2000, 8000]); 
...
add_task(function test_jog_name_collision() {
  ...
  Services.fog.testRegisterRuntimeMetric(
    ...
    `"ping"`,
    false,
    JSON.stringify({ allowed_extra_keys: ["extra1", "extra2", "extra3"] }) 
  );
  const extra123 = {
...
add_task(async function test_jog_labeled_timing_distribution_works() {
  ...
  Glean.jogCat.jogLabeledTimingDist.label1.stopAndAccumulate(t2); 
  Glean.jogCat.jogLabeledTimingDist.label1.stopAndAccumulate(t3); 
