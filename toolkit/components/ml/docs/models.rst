...
`convert_to_external_data.py <https://searchfox.org/firefox-main/source/toolkit/components/ml/tools/convert_to_external_data.py>`_.
...
If you are adding in Firefox a new inference call, create a new unique `featureId` in `FEATURES <https://searchfox.org/firefox-main/source/toolkit/components/ml/content/EngineProcess.sys.mjs>`_ and add a record in `ml-inference-options` with the task settings.
