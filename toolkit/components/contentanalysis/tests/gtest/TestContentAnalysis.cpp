...
class ContentAnalysisTest : public testing::Test {
 protected:
  ...
  nsresult SendRequestsCancelAndExpectResponse(
      ...
      bool aDelayCancel, bool aExpectFailure);
  ...
  auto* GetCompoundUserActions() { return &mContentAnalysis->mCompoundUserActions; }
  auto CancelAllRequestsAssociatedWithUserAction(const nsACString& aUserActionId) {
    return mContentAnalysis->CancelAllRequestsAssociatedWithUserAction(aUserActionId);
  };
};
...
TEST_F(ContentAnalysisTest,
  ...
  nsresult rv = SendRequestsCancelAndExpectResponse(
      mContentAnalysis, requests, true ,
      false );
  EXPECT_EQ(rv, NS_OK);
...
TEST_F(ContentAnalysisTest, CheckAssignedUserActionIdCanCancel) {
  ...
  nsresult rv = SendRequestsCancelAndExpectResponse(
      mContentAnalysis, requests, false ,
      false );
  EXPECT_EQ(rv, NS_OK);
}
...
TEST_F(ContentAnalysisTest, CheckBrowserReportsTimeout) {
  ...
  SendRequestAndExpectResponse(
      mContentAnalysis, request, Some(false) ,
...
TEST_F(ContentAnalysisTest,
  ...
  if (!compoundActions->empty()) {
    ...
    if (!compoundActionIds->empty()) {
      ...
      MOZ_ALWAYS_SUCCEEDS(CancelAllRequestsAssociatedWithUserAction(userActionId));
    }
  }

  mozilla::SpinEventLoopUntil(
  "Waiting for ContentAnalysis cancel"_ns, [&, timedOut]() {
