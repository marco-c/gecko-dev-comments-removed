...
ExtensionSet::~ExtensionSet() {
  ...
  ForEach([](int , Extension& ext) { ext.Free(); }, PrefetchNta{});
...
int ExtensionSet::NumExtensions() const {
  int result = 0;
  ForEachNoPrefetch([&result](int , const Extension& ext) {
    if (!ext.is_cleared) {
      ++result;
    }
  });
...
void ExtensionSet::Clear() {
  ForEach([](int , Extension& ext) { ext.Clear(); }, Prefetch{});
}
...
void ExtensionSet::Swap(Arena* arena, const MessageLite* extendee,
  ...
  if (internal::CanUseInternalSwap(arena, other_arena)) {
    ...
    extension_set.MergeFrom(nullptr, extendee, *other, other_arena);
    other->Clear();
    other->MergeFrom(other_arena, extendee, *this, arena);
    Clear();
    MergeFrom(arena, extendee, extension_set, nullptr);
  }
}
...
void ExtensionSet::SwapExtension(Arena* arena, const MessageLite* extendee,
  ...
  if (this_ext != nullptr && other_ext != nullptr) {
    ...
    ExtensionSet temp;
    temp.InternalExtensionMergeFrom(nullptr, extendee, number,
