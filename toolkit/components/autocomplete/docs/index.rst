...
The autocomplete components implement the `nsIAutoCompleteSearch <https://searchfox.org/firefox-main/source/toolkit/components/autocomplete/nsIAutoCompleteSearch.idl>`_ interface which contains a ``startSearch`` function to start a new search given a search term.
...
In most circumstances, an autocomplete action begins when typing into an input field, or, by pressing the cursor or tab keys. For browser UI, this happens in event listeners within the ``autocomplete-input`` implementation. For web pages, this happens in the event listeners added within the `nsFormFillController <https://searchfox.org/firefox-main/source/toolkit/components/satchel/nsFormFillController.cpp>`_. An autocomplete popup can also trigger when clicking the mouse in a field.

A special case occurs for password fields as the autocomplete popup can be opened when the input field is focused without having to type into the field. This is done in the handling for the focus event in `nsFormFillController.cpp <https://searchfox.org/firefox-main/source/toolkit/components/satchel/nsFormFillController.cpp>`_. This is where changes would be made to determine whether a popup should be opened on different events. However, at this point, it has not been determined which type of autocomplete search is to be used, so any changes would be general to all autocomplete types. This is probably a good thing, so that the autocomplete behaviour is consistent across all types.
...
.. container::
  ...
  `nsIAutoCompleteInput <https://searchfox.org/firefox-main/source/toolkit/components/autocomplete/nsIAutoCompleteInput.idl>`_
  ...
  `nsIAutoCompletePopup <https://searchfox.org/firefox-main/source/toolkit/components/autocomplete/nsIAutoCompletePopup.idl>`_
    This is an abstraction around the popup that appears for autocomplete. It is implemented by `autocomplete-popup.js <https://searchfox.org/firefox-main/source/toolkit/content/widgets/autocomplete-popup.js>`_ for browser UI and supports displaying a number of different types of autocomplete rows. For form fill in web pages, it is implemented by the ``AutoCompleteChild`` actor which redirects through the process boundary to the actual popup also implemented by `autocomplete-popup.js <https://searchfox.org/firefox-main/source/toolkit/content/widgets/autocomplete-popup.js>`_.

  `nsIAutoCompleteSearch <https://searchfox.org/firefox-main/source/toolkit/components/autocomplete/nsIAutoCompleteSearch.idl>`_
  ...
  `nsIAutoCompleteResult <https://searchfox.org/firefox-main/source/toolkit/components/autocomplete/nsIAutoCompleteResult.idl>`_
...
5. The search results are supplied to the listener passed to `nsIAutoCompleteSearch::StartSearch <https://searchfox.org/firefox-main/source/toolkit/components/autocomplete/nsIAutoCompleteSearch.idl>`_. This listener is the autocomplete controller ``nsAutocompleteController`` which takes the results and uses them to display an autocomplete popup if needed.

To summarize, if you wish to modify the events which cause an autocomplete popup to show, you would need to modify the `autocomplete-input.js <https://searchfox.org/firefox-main/source/toolkit/content/widgets/autocomplete-input.js>`_ for browser UI and the `nsFormController <https://searchfox.org/firefox-main/source/toolkit/components/satchel/nsFormFillController.cpp>`_ for web page fields. The `nsAutocompleteController <https://searchfox.org/firefox-main/source/toolkit/components/autocomplete/nsAutoCompleteController.cpp>`_ contains code which is shared between both.
...
There is an actor pair for each of these three types:
  ...
  ``Login Manager``:
    * `toolkit/components/passwordmgr/LoginManagerChild.sys.mjs <https://searchfox.org/firefox-main/source/toolkit/components/passwordmgr/LoginManagerChild.sys.mjs>`_
    * `toolkit/components/passwordmgr/LoginManagerParent.sys.mjs <https://searchfox.org/firefox-main/source/toolkit/components/passwordmgr/LoginManagerParent.sys.mjs>`_

  ``Form Autofill``:
    * `toolkit/components/formautofill/FormAutofillChild.sys.mjs <https://searchfox.org/firefox-main/source/toolkit/components/formautofill/FormAutofillChild.sys.mjs>`_
    * `toolkit/components/formautofill/FormAutofillParent.sys.mjs <https://searchfox.org/firefox-main/source/toolkit/components/formautofill/FormAutofillParent.sys.mjs>`_

  ``Form History``:
    * `toolkit/components/satchel/FormHistoryChild.sys.mjs <https://searchfox.org/firefox-main/source/toolkit/components/satchel/FormHistoryChild.sys.mjs>`_
    * `toolkit/components/satchel/FormHistoryParent.sys.mjs <https://searchfox.org/firefox-main/source/toolkit/components/satchel/FormHistoryParent.sys.mjs>`_

There is also a fourth more general actor pair used for the autocomplete system itself:

  ``AutoComplete``:
    * `toolkit/actors/AutoCompleteChild.sys.mjs <https://searchfox.org/firefox-main/source/toolkit/actors/AutoCompleteChild.sys.mjs>`_
    * `toolkit/actors/AutoCompleteParent.sys.mjs <https://searchfox.org/firefox-main/source/toolkit/actors/AutoCompleteParent.sys.mjs>`_
...
hard-coded in the ``AUTOCOMPLETE_PROVIDERS`` constant within `toolkit/actors/AutoCompleteParent.sys.mjs <https://searchfox.org/firefox-main/source/toolkit/actors/AutoCompleteParent.sys.mjs>`_.
...
`autocomplete-richlistitem.js <https://searchfox.org/firefox-main/source/toolkit/content/widgets/autocomplete-richlistitem.js>`_ and then adding a corresponding reference within `autocomplete-popup.js <https://searchfox.org/firefox-main/source/toolkit/content/widgets/autocomplete-popup.js>`_ to match.
