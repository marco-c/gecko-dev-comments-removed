const PAGE =
  "https://example.com/browser/toolkit/content/tests/browser/file_multiplePlayingAudio.html";
...
function check_all_audio_suspended(suspendedType) {
  ...
  is(
    audio1.computedSuspended,
    suspendedType,
    "The suspeded state of audio1 is correct."
  );
  is(
    audio2.computedSuspended,
    suspendedType,
...
async function audio_competing_for_active_agent(url, browser) {
  ...
  await ContentTask.spawn(browser, null, play_audio1_from_page);
  await ContentTask.spawn(
    browser,
    SuspendedType.NONE_SUSPENDED,
  ...
  await ContentTask.spawn(browser, null, stop_audio1_from_page);
  await ContentTask.spawn(
    browser,
    SuspendedType.NONE_SUSPENDED,
    check_audio1_suspended
