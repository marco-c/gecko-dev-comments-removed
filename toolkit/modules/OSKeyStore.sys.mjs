...
export var OSKeyStore = {
  ...
  /**
   * Check if a test value is remembered by the OS key store
   */
  async retentionTest() {
    // This is where we store an encrypted version of our dummy:
    const ENCRYPTED_DUMMY_PREF = "keystore.dummy";
    // The following stores the name of the value in the keystore.
    // This is not hardcoded because we want to distinguish different
    // Firefox versions running on the same machine!
    const IDENTIFIER_PREF = "keystore.test_identifier";
    // This is the dummy value which will be encrypted:
    const DUMMY = "Retention Test";

    let identifier = Services.prefs.getStringPref(IDENTIFIER_PREF, "");
    if (identifier === "") {
      identifier = "Firefox_Consistency_Check_" + crypto.randomUUID();
      Services.prefs.setStringPref(IDENTIFIER_PREF, identifier);
    }

    // Does the keystore have a value with the given identifier?
    let available;
    try {
      available = await lazy.nativeOSKeyStore.asyncSecretAvailable(identifier);
      Glean.oskeystore.dummyStorage.asyncSecretAvailableSuccess.add(1);
    } catch (e) {
      available = false;
      Glean.oskeystore.dummyStorage.asyncSecretAvailableError.add(1);
      lazy.log.debug("RetentionTest: asyncSecretAvailable failed");
    }
    if (available) {
      Glean.oskeystore.dummyStorage.available.add(1);
    }

    // Check if we think the value *should* be available
    let storedCiphertext = Services.prefs.getStringPref(
      ENCRYPTED_DUMMY_PREF,
      ""
    );

    if (storedCiphertext !== "" && !available) {
      // There should be a value but there is not.
      Glean.oskeystore.dummyStorage.lost.add(1);
    } else if (storedCiphertext !== "" && available) {
      // The keystore has a value. Is it the right one?
      try {
        await lazy.nativeOSKeyStore.asyncDecryptBytes(
          identifier,
          storedCiphertext
        );
        Glean.oskeystore.dummyStorage.asyncDecryptBytesSuccess.add(1);
        lazy.log.debug("RetentionTest: found matching value");
      } catch (e) {
        available = false;
        Glean.oskeystore.dummyStorage.asyncDecryptBytesError.add(1);
        lazy.log.debug("RetentionTest: found non-matching value");
      }
    } else {
      // Not initalized yet!
      Glean.oskeystore.dummyStorage.first.add(1);
    }

    // No secret or wrong secret. Generate new and store matching ciphertext.
    if (!available) {
      lazy.log.debug("RetentionTest: writing dummy value");
      try {
        lazy.nativeOSKeyStore.asyncGenerateSecret(identifier);
        Glean.oskeystore.dummyStorage.asyncGenerateSecretSuccess.add(1);
      } catch (e) {
        Glean.oskeystore.dummyStorage.asyncGenerateSecretError.add(1);
      }

      let dummyBytes = new TextEncoder().encode(DUMMY);
      let encryptedBase64Dummy;
      try {
        encryptedBase64Dummy = await lazy.nativeOSKeyStore.asyncEncryptBytes(
          identifier,
          dummyBytes
        );
        Glean.oskeystore.dummyStorage.asyncEncryptBytesSuccess.add(1);
      } catch (e) {
        encryptedBase64Dummy = "";
        Glean.oskeystore.dummyStorage.asyncEncryptBytesError.add(1);
      }
      if (encryptedBase64Dummy !== "") {
