...
use nix::{
    ...
    sys::socket::{
        getsockopt, recv, send, sockopt::LocalPeerPid, MsgFlags, SockFlag, SockType, UnixAddr,
    },
    Result,
};
use std::{
...
pub(crate) const SOCKET_TYPE: SockType = SockType::Stream;

pub(crate) const SOCKET_FLAGS: SockFlag = SockFlag::empty();

pub(crate) fn set_socket_flags(socket: &OwnedFd) -> Result<()> {
    fcntl(socket.as_raw_fd(), F_SETFL(OFlag::O_CLOEXEC))?;
    fcntl(socket.as_raw_fd(), F_SETFL(OFlag::O_NONBLOCK))?;
