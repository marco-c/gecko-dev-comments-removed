...
extern "C" void* CrashGenerationServer_init(breakpad_init_type aBreakpadData,
  ...
  CrashGenerationServer* server = new CrashGenerationServer(
      breakpadData,
      [](void* aDumpContext, const ClientInfo& aClientInfo,
         const breakpad_string& aFilePath) {
        onClientDumpRequestCallback(aDumpContext, aClientInfo, aFilePath);
      },
      reinterpret_cast<void*>(aDumpCallback), &minidumpPath);
