  ...
  if (pid == -1)
    ...
    if (androidUserSerial) {
      Unused << execlp("/system/bin/am", "/system/bin/am",
                       ...
                       "true", "--es", "processVisibility", "MAIN", (char*)0);
    } else {
      Unused << execlp("/system/bin/am", "/system/bin/am",
                       androidStartServiceCommand, "-a",
                       "org.mozilla.gecko.ACTION_CRASHED", "-n", aProgramPath,
                       "--es", "minidumpPath", aMinidumpPath, "--es",
                       "extrasPath", extrasPath, "--ez", "fatal", "true",
                       "--es", "processVisibility", "MAIN", (char*)0);
    }
    _exit(1);
...
#else
  setenv(aEnvVarName, directoryPath->c_str(),  1);
#endif
...
#else
  setenv(eventsDirectoryEnv, path->c_str(),  1);
#endif
}
