...
add_task(async function test_keyboard_lock_change_warning_change_oop_iframe() {
  await BrowserTestUtils.withNewTab(
    "http://mochi.test:8888/browser/browser/base/content/test/fullscreen/fullscreen_frame.html",
    async browser => {
      info("Start fullscreen without keyboard lock");
      let warningShownPromise = DOMFullscreenTestUtils.waitForWarningState(
        browser,
      ...
      info("Request fullscreen in OOP iframe with keyboard lock");
      warningShownPromise = DOMFullscreenTestUtils.waitForWarningState(
        browser,
        "onscreen",
        true
      ...
      info("Exit fullscreen by long press esc");
      EventUtils.synthesizeKey("KEY_Escape", { repeat: 2 });

      info("Wait a bit before next tests");
      ...
      info("Request fullscreen in OOP iframe without keyboard lock");
      warningShownPromise = DOMFullscreenTestUtils.waitForWarningState(
        browser,
        "onscreen"
      );
