"use strict";

const VALID_TESTS = [
  ["John Doe", true],
  ["John O'Brian'", true],
  ["John O-Brian'", true],
  ["John Doe", true],
];


const COMPARE_TESTS = [
  
  ["John", "John", SAME],                             
  ["John Doe", "John Doe", SAME],                     
  ["John Middle Doe", "John Middle Doe", SAME],       
  ["John Mid1 Mid2 Doe", "John Mid1 Mid2 Doe", SAME],

  
  ["John Doe", "john doe", SAME],

  
  ["John Doe", "John  Doe", SIMILAR],

  
  ["John Doe", "John Döe", SIMILAR], 

  
  ["John Doe", "Doe", A_CONTAINS_B],                 
  ["John Doe", "John", A_CONTAINS_B],                
  ["John Middle Doe", "Doe", A_CONTAINS_B],          
  ["John Middle Doe", "John", A_CONTAINS_B],         
  ["John Middle Doe", "Middle", A_CONTAINS_B],       
  ["John Middle Doe", "Middle Doe", A_CONTAINS_B],   
  ["John Middle Doe", "John Middle", A_CONTAINS_B],  
  ["John Middle Doe", "John Doe", A_CONTAINS_B],        
  ["John Mary Jane Doe", "John Doe", A_CONTAINS_B],  

  
  ["John Doe", "Jane Roe", DIFFERENT],
  ["John Doe", "Doe John", DIFFERENT],                
  ["John Middle Doe", "Middle John", DIFFERENT],
  ["John Middle Doe", "Doe Middle", DIFFERENT],
  ["John Doe", "John Roe.", DIFFERENT],               
  ["John Doe", "Jane Doe", DIFFERENT],                
  ["John Middle Doe", "Jane Michael Doe", DIFFERENT], 

  
  ["John O'Brian", "John OBrian", SIMILAR],
  ["John O'Brian", "John O-Brian", SIMILAR],
  ["John O'Brian", "John O Brian", SIMILAR],
  ["John-Mary Doe", "JohnMary Doe", SIMILAR],
  ["John-Mary Doe", "John'Mary Doe", SIMILAR],
  ["John-Mary Doe", "John Mary Doe", SIMILAR],
  ["John-Mary Doe", "John Mary", A_CONTAINS_B],

  
  ["John Doe", "J. Doe", A_CONTAINS_B],               
  ["John Doe", "J. doe", A_CONTAINS_B],
  ["John Doe", "J. Doe", A_CONTAINS_B],               

  ["John Middle Doe", "J. Middle Doe", A_CONTAINS_B], 
  ["John Middle Doe", "J. Doe", A_CONTAINS_B],        

  ["John Middle Doe", "John M. Doe", A_CONTAINS_B],   
  ["John Middle Doe", "J. M. Doe", A_CONTAINS_B],     
  ["John Middle Doe", "J M Doe", A_CONTAINS_B],       
  ["John Middle Doe", "John M. Doe", A_CONTAINS_B],   

  
  ["John Mary Jane Doe", "J. MARY JANE Doe", A_CONTAINS_B],   
  ["John Mary Jane Doe", "john. M. J. doe", A_CONTAINS_B],    
  ["John Mary Jane Doe", "J. M. J. Doe", A_CONTAINS_B],       
  ["John Mary Jane Doe", "J. M. Doe", A_CONTAINS_B],          
  ["John Mary Jane Doe", "John M. Doe", A_CONTAINS_B],
  ["John Mary Jane Doe", "J. Doe", A_CONTAINS_B],

  
  ["John Middle Doe", "JM Doe", A_CONTAINS_B],
  ["John Mary Jane Doe", "JMJ. doe", A_CONTAINS_B],

  
  ["John Middle Doe", "JMD", DIFFERENT],
  ["John Middle Doe", "John Middle D.", DIFFERENT],
  ["John Middle Doe", "J. M. D.", DIFFERENT],
];

const TEST_FIELD_NAME = "name";

add_setup(async () => {});

add_task(async function test_isValid() {
  runIsValidTest(VALID_TESTS, TEST_FIELD_NAME, value => {
    return { name: value };
  });
});

add_task(async function test_compare() {
  runCompareTest(COMPARE_TESTS, TEST_FIELD_NAME, value => {
    return { name: value };
  });
});
