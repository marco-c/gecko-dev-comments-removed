...
add_task(async function test_bayesHist() {
  ...
  Assert.ok(
    result.every((v, i) => Math.abs(v - vec[i] / 3) < 1e-6), 
...
add_task(async function test_updateWeights_batch() {
  ...
  updated = await Ranker.updateWeights(
    {
      data: { guid_B: { clicks: 1, impressions: 0 } }, 
      scores: scores2,
      features,
      weights: { ...initial2 },
      eta,
...
add_task(async function test_buildFrecencyFeatures_recency_monotonic() {
  ...
  const visitsByGuid = {
    ...
    recent: [{ visit_date_us: us(nowMs), visit_type: 2  }],
...
add_task(
  async function test_buildFrecencyFeatures_log_scaling_and_interaction() {
    ...
    const visitsByGuid = {
      A: [{ visit_date_us: us(nowMs), visit_type: 2  }],
      B: [{ visit_date_us: us(nowMs), visit_type: 2 }],
    };
    const visitCounts = { A: 9, B: 99 }; 
...
add_task(
  async function test_weightedSampleTopSites_new_features_scores_and_norms() {
    ...
    const input = {
      ...
      bmark_scores: { g1: 1, g2: 0 }, 
      rece_scores: { g1: 2.0, g2: 1.0 }, 
      freq_scores: { g1: 0.5, g2: 0.25 }, 
      refre_scores: { g1: 10, g2: 5 }, 
    ...
    const vecBmark = guids.map(g => input.bmark_scores[g]); 
    const vecRece = guids.map(g => input.rece_scores[g]); 
    const vecFreq = guids.map(g => input.freq_scores[g]); 
    const vecRefre = guids.map(g => input.refre_scores[g]); 
  ...
  const execStub = sandbox
    ...
    .callsFake(async sql => {
      ...
      if (s.includes("LIMIT 10") && s.includes("ORDER BY vv.visit_date DESC")) {
        ...
        return [
          ["g1", nowUs, 2], 
          ["g1", nowUs - 1 * Number(dayUs), 1], 
          ["g2", nowUs - 10 * Number(dayUs), 1], 
        ];
      }
...
add_task(async function test_weightedSampleTopSites_open_with_existing_norms() {
  ...
  const maxClosed = Math.max(finals[0], finals[2]); 
  const minOpen = Math.min(finals[1], finals[3]); 
...
add_task(async function test_buildFrecencyFeatures_unid_counts_unique_days() {
  ...
  const nowMs = Date.UTC(2025, 0, 10); 
  const clock = sandbox.useFakeTimers({ now: nowMs });
  ...
  const visitsByGuid = {
    g1: [
      { visit_date_us: us(nowMs), visit_type: 1 }, 
      { visit_date_us: us(nowMs - 2 * dayMs), visit_type: 1 }, 
      { visit_date_us: us(nowMs - 2 * dayMs), visit_type: 1 }, 
    ],
    g2: [
      { visit_date_us: us(nowMs - 7 * dayMs), visit_type: 1 }, 
    ],
  };
...
add_task(async function test_weightedSampleTopSites_ctr_with_prior_norm() {
  ...
  const prior = { beta: 1e-3, mean: 0.3, var: 1.0 }; 
  const input = {
    features: ["ctr"],
    guid: ["x", "y"],
    clicks: [3, 0],
...
add_task(
  async function test_applyStickyClicks_preserves_null_and_clamps_negative() {
    ...
    const guids = ["g1", "g2", "g3"];
    const positions = [3, null, 1]; 
    const numSponsored = 2; 
