...
describe("<CardSections />", () => {
  ...
  describe("Keyboard navigation", () => {
    ...
    it("should update first tab target when active column layout changes on resize", () => {
      Object.defineProperty(window, "innerWidth", {
        writable: true,
        configurable: true,
        value: 500,
    ...
    it("should fall back to the first available focus position after resize", () => {
      Object.defineProperty(window, "innerWidth", {
        writable: true,
        configurable: true,
        value: 500,
      ...
      assert.equal(wrapper.find(DSCard).at(0).prop("tabIndex"), 0);
      assert.equal(wrapper.find(DSCard).at(1).prop("tabIndex"), -1);
      assert.equal(wrapper.find(DSCard).at(2).prop("tabIndex"), -1);
    });
    ...
    it("should reset focused index to first card when focus leaves section grid", () => {
      wrapper.find(DSCard).at(1).prop("onFocus")();
      wrapper.update();

      assert.equal(wrapper.find(DSCard).at(1).prop("tabIndex"), 0);

      const grid = wrapper.find(".ds-section-grid.ds-card-grid").first();
      grid.prop("onBlur")({
        currentTarget: { contains: sandbox.stub().returns(false) },
        relatedTarget: null,
      });
      wrapper.update();
