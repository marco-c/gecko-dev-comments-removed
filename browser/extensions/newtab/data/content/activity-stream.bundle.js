...
class DiscoveryStreamAdminUI extends (external_React_default()).PureComponent {
  ...
  refreshTopicSelectionCache() {
    this.props.dispatch(actionCreators.SetPref(PREFS.TOPIC_SELECTION_ONBOARDING_DISPLAY_COUNT, 0));
    this.props.dispatch(actionCreators.SetPref(PREFS.TOPIC_SELECTION_ONBOARDING_MAYBE_DISPLAY, true));
  }
  ...
  handleWeatherSubmit(e) {
    ...
    this.props.dispatch(actionCreators.SetPref(PREFS.WEATHER_QUERY, weatherQuery));
  }
  toggleIABBanners(e) {
    const {
      pressed,
    ...
    switch (id) {
      case "newtab_billboard":
        ...
        this.props.dispatch(actionCreators.SetPref(PREFS.BILLBOARD_ENABLED, pressed));
        break;
      case "newtab_leaderboard":
        
        this.props.dispatch(actionCreators.SetPref(PREFS.LEADERBOARD_ENABLED, pressed));
        break;
      case "newtab_rectangle":
        
        this.props.dispatch(actionCreators.SetPref(PREFS.AD_SIZE_MEDIUM_RECTANGLE, pressed));
        break;
    }
    ...
    const generateSpocPrefValues = () => {
      const placements = this.props.otherPrefs[PREFS.SPOC_PLACEMENTS]?.split(",").map(item => item.trim()).filter(item => item) || [];
      const counts = this.props.otherPrefs[PREFS.SPOC_COUNTS]?.split(",").map(item => item.trim()).filter(item => item) || [];
    ...
    this.props.dispatch(actionCreators.SetPref(PREFS.SPOC_PLACEMENTS, placements));
    this.props.dispatch(actionCreators.SetPref(PREFS.SPOC_COUNTS, counts));
    ...
    if (PREFS.CONTEXTUAL_ADS_ENABLED && PREFS.SECTIONS_ENABLED) {
      if (PREFS.BILLBOARD_ENABLED && placements.includes("newtab_billboard")) {
        this.props.dispatch(actionCreators.SetPref(PREFS.CONTEXTUAL_BANNER_PLACEMENTS, "newtab_billboard"));
        this.props.dispatch(actionCreators.SetPref(PREFS.CONTEXTUAL_BANNER_COUNTS, "1"));
      } else if (PREFS.LEADERBOARD_ENABLED && placements.includes("newtab_leaderboard")) {
        this.props.dispatch(actionCreators.SetPref(PREFS.CONTEXTUAL_BANNER_PLACEMENTS, "newtab_leaderboard"));
        this.props.dispatch(actionCreators.SetPref(PREFS.CONTEXTUAL_BANNER_COUNTS, "1"));
      } else {
        this.props.dispatch(actionCreators.SetPref(PREFS.CONTEXTUAL_BANNER_PLACEMENTS, ""));
        this.props.dispatch(actionCreators.SetPref(PREFS.CONTEXTUAL_BANNER_COUNTS, ""));
      }
    }
  }
  handleSectionsToggle(e) {
    ...
    this.props.dispatch(actionCreators.SetPref(PREFS.SECTIONS_ENABLED, pressed));
    this.props.dispatch(actionCreators.SetPref(PREFS.SECTIONS_CARDS_ENABLED, pressed));
  }
  sendConversionEvent() {
    const detail = {
  ...
  renderPersonalizationData() {
    ...
    const inferredPersonalizationEnabled = Boolean(this.props.otherPrefs?.[PREFS.INFERRED_PERSONALIZATION_SYSTEM]);
  ...
  handleAllizomToggle(e) {
    ...
    if (pressed) {
      ...
      setPref(PREFS.ALLOWED_ENDPOINTS, `${allowedEndpoints},https://ads.allizom.org/`);
      setPref(PREFS.OHTTP_CONFIG, "https://stage.ohttp-gateway.nonprod.webservices.mozgcp.net/ohttp-configs");
      setPref(PREFS.OHTTP_RELAY, "https://mozilla-ohttp-relay-test.edgecompute.app/");
    } else {
  ...
  renderSpocs() {
    ...
    const unifiedAdsSpocsEnabled = this.props.otherPrefs[PREFS.UNIFIED_ADS_ENABLED];

    
    const adsFeedEnabled = this.props.otherPrefs[PREFS.UNIFIED_ADS_ADSFEED_ENABLED];
    const unifiedAdsEndpoint = this.props.otherPrefs[PREFS.UNIFIED_ADS_ENDPOINT];
  ...
  render() {
    ...
    const mediumRectangleEnabled = this.props.otherPrefs[PREFS.AD_SIZE_MEDIUM_RECTANGLE];
...
class _DSCard extends (external_React_default()).PureComponent {
  ...
  renderSectionCardImages() {
    ...
    return external_React_default().createElement((external_React_default()).Fragment, null, images);
  }
  render() {
    const {
      isRecentSave,
...
const PromoCard = () => {
  ...
  const onDismissClick = (0,external_React_namespaceObject.useCallback)(() => {
    ...
    dispatch(actionCreators.SetPref(PREFS.PROMOCARD_VISIBLE, false));
  }, [dispatch]);
...
const AdBanner = ({
  ...
  const promoCardEnabled = spoc.format === "billboard" && prefs[PREFS.PROMOCARD_ENABLED] && prefs[PREFS.PROMOCARD_VISIBLE];
  ...
  renderCards() {
    ...
    if ((billboardEnabled || leaderboardEnabled) && spocs?.data?.newtab_spocs?.items) {
      ...
      if (spocToRender && !spocs.blocked.includes(spocToRender.url)) {
        const row = spocToRender.format === "leaderboard" ? prefs[PREFS.LEADERBOARD_POSITION] : prefs[PREFS.BILLBOARD_POSITION];
...
class _CollapsibleSection extends (external_React_default()).PureComponent {
  ...
  handleTopicSelectionButtonClick() {
    const maybeDisplay = this.props.Prefs.values[PREFS.TOPIC_SELECTION_ONBOARDING_MAYBE_DISPLAY];
    this.props.dispatch(actionCreators.OnlyToMain({
      type: actionTypes.TOPIC_SELECTION_USER_OPEN
    }));
    if (maybeDisplay) {
      
      this.props.dispatch(actionCreators.SetPref(PREFS.TOPIC_SELECTION_ONBOARDING_MAYBE_DISPLAY, false));
    }
  ...
  render() {
    ...
    const hasBeenUpdatedPreviously = this.props.Prefs.values[PREFS.TOPIC_SELECTION_HAS_BEEN_UPDATED_PREVIOUSLY];
...
class TopSiteFormInput extends (external_React_default()).PureComponent {
  ...
  render() {
    ...
    }), this.renderLoadingOrCloseButton(), validationError && external_React_default().createElement("aside", {
  ...
  render() {
    ...
    for (let i = 0, l = topSites.length; i < l; i++) {
      ...
      if (!link || props.App.isForStartupCache.TopSites && isSponsored(link)) {
        ...
        topSiteLink = external_React_default().createElement(TopSiteAddButton, TopSite_extends({}, slotProps, commonProps, {
...
function InterestPicker({
  ...
  const visibleSections = prefs[PREFS.VISIBLE_SECTIONS]?.split(",").map(item => item.trim()).filter(item => item);
  const handleIntersection = (0,external_React_namespaceObject.useCallback)(() => {
    dispatch(actionCreators.AlsoToMain({
  ...
  function handleChange(e, index) {
    ...
    if (checked) {
      ...
      if (!visibleSections.includes(topic)) {
        ...
        dispatch(actionCreators.SetPref(PREFS.VISIBLE_SECTIONS, visibleSections.join(", ")));
      }
    } else {
...
function CardSection({
  ...
  const availableTopics = prefs[PREFS.TOPICS_AVAILABLE];
  const spocsStartupCacheEnabled = prefs[PREFS.SPOCS_STARTUPCACHE_ENABLED];
  const dailyBriefEnabled = prefs.trainhopConfig?.dailyBriefing?.enabled || prefs[PREFS.DAILY_BRIEF_ENABLED];
  const dailyBriefSectionId = prefs.trainhopConfig?.dailyBriefing?.sectionId || prefs[PREFS.DAILY_BRIEF_SECTIONID];
  const mayHaveSectionsPersonalization = prefs[PREFS.SECTIONS_PERSONALIZATION_ENABLED];
  const {
    sectionKey,
    title,
    subtitle
...
function CardSections({
  ...
  if (interestPickerEnabled && personalizationEnabled && interestPicker?.sections) {
    ...
    Math.min(sectionsToRender.length - 1, index), 0, external_React_default().createElement(InterestPicker, {
  ...
  function displayP13nCard() {
    if (messageData && Object.keys(messageData).length >= 1) {
      if (shouldShowOMCHighlight(messageData, "PersonalizedCard") && prefs[PREFS.INFERRED_PERSONALIZATION_USER]) {
...
function Lists({
  ...
  return external_React_default().createElement("article", {
    ...
    ref: selectRef,
    value: selected
  }, Object.entries(lists).map(([key, list]) => external_React_default().createElement("moz-option", Lists_extends({
    key: key,
    value: key
...
const FocusTimer = ({
  ...
  return timerData ? external_React_default().createElement("article", {
    ...
    onClick: () => {
      handlePrefUpdate(PREFS.WIDGETS_TIMER_SHOW_SYSTEM_NOTIFICATIONS, !showSystemNotifications);
    }
  }), external_React_default().createElement("panel-item", {
    "data-l10n-id": "newtab-widget-menu-hide",
    onClick: () => {
...
function LocationSearch({
  ...
  (0,external_React_namespaceObject.useEffect)(() => {
    if (selectedLocation) {
      ...
      dispatch(actionCreators.SetPref(PREFS.WEATHER_QUERY, selectedLocation.key));
...
function Widgets() {
  ...
  const feedbackEnabled = prefs.trainhopConfig?.widgets?.feedbackEnabled || prefs[PREFS.WIDGETS_FEEDBACK_ENABLED];
  const hideAllToastEnabled = prefs.trainhopConfig?.widgets?.hideAllToastEnabled || prefs[PREFS.WIDGETS_HIDE_ALL_TOAST_ENABLED];
  const feedbackUrl = prefs.trainhopConfig?.widgets?.feedbackUrl ?? WIDGETS_FEEDBACK_URL;
  const listsEnabled = (nimbusListsTrainhopEnabled || nimbusListsEnabled || prefs[PREFS.WIDGETS_SYSTEM_LISTS_ENABLED]) && prefs[PREFS.WIDGETS_LISTS_ENABLED];
  const timerEnabled = (nimbusTimerTrainhopEnabled || nimbusTimerEnabled || prefs[PREFS.WIDGETS_SYSTEM_TIMER_ENABLED]) && prefs[PREFS.WIDGETS_TIMER_ENABLED];
  ...
  const weatherForecastSystemEnabled = nimbusWeatherForecastTrainhopEnabled || prefs[PREFS.WIDGETS_SYSTEM_WEATHER_FORECAST_ENABLED];
  ...
  function hideAllWidgets() {
    (0,external_ReactRedux_namespaceObject.batch)(() => {
      dispatch(actionCreators.SetPref(PREFS.WIDGETS_LISTS_ENABLED, false));
      dispatch(actionCreators.SetPref(PREFS.WIDGETS_TIMER_ENABLED, false));
      
      if (weatherForecastEnabled) {
        dispatch(actionCreators.SetPref(PREFS.SHOW_WEATHER, false));
      }
      const telemetryData = {
  ...
  function toggleMaximize() {
    ...
    (0,external_ReactRedux_namespaceObject.batch)(() => {
      dispatch(actionCreators.SetPref(PREFS.WIDGETS_MAXIMIZED, newMaximizedState));
      const telemetryData = {
  ...
  return external_React_default().createElement("div", {
    ...
    dispatch: dispatch
  }))), (nimbusMaximizedTrainhopEnabled || prefs[PREFS.WIDGETS_SYSTEM_MAXIMIZED]) && external_React_default().createElement("moz-button", {
...
class _DiscoveryStreamBase extends (external_React_default()).PureComponent {
  ...
  render() {
    ...
    const sectionsEnabled = this.props.Prefs.values[PREFS.SECTIONS_ENABLED];
    const topicSelectionEnabled = this.props.Prefs.values[PREFS.TOPIC_SELECTION_ENABLED];
    ...
    const widgetsSystemPrefsEnabled = this.props.Prefs.values[PREFS.WIDGETS_SYSTEM_ENABLED];
...
class ContentSection extends (external_React_default()).PureComponent {
  ...
  render() {
    ...
    return external_React_default().createElement("div", {
      ...
      className: "divider",
      role: "separator"
    })), mayHaveWidgets && external_React_default().createElement("div", {
      ...
      type: "checkbox",
      onChange: this.onPreferenceSelect,
      "data-preference": PREFS.INFERRED_PERSONALIZATION_USER,
...
class _CustomizeMenu extends (external_React_default()).PureComponent {
  ...
  render() {
    const activationWindowVariant = this.props.Prefs.values[PREFS.ACTIVATION_WINDOW_VARIANT];
...
class _Weather extends (external_React_default()).PureComponent {
  ...
  handleChangeDisplay = value => {
    const weatherForecastEnabled = this.props.Prefs.values[PREFS.WIDGETS_SYSTEM_WEATHER_FORECAST_ENABLED];
    if (this.panelElement) {
      this.panelElement.hide();
    }
    (0,external_ReactRedux_namespaceObject.batch)(() => {
  ...
  handleRejectOptIn = () => {
    (0,external_ReactRedux_namespaceObject.batch)(() => {
      this.props.dispatch(actionCreators.SetPref(PREFS.WEATHER_OPT_IN_ACCEPTED, false));
      this.props.dispatch(actionCreators.SetPref(PREFS.WEATHER_OPT_IN_DISPLAYED, false));
  ...
  isEnabled() {
    ...
    const systemValue = values[PREFS.SYSTEM_SHOW_WEATHER] && values[PREFS.FEEDS_WEATHERFEED];
  ...
  render() {
    ...
    const weatherForecastWidgetEnabled = nimbusWeatherForecastTrainhopEnabled || Prefs.values[PREFS.WIDGETS_SYSTEM_WEATHER_FORECAST_ENABLED];
    ...
    const isLocationSearchEnabled = Prefs.values[PREFS.WEATHER_LOCATION_SEARCH_ENABLED];
    ...
    if (Weather.searchActive) {
      ...
      return external_React_default().createElement("div", {
      ...
      }, "22\xB0", Prefs.values[PREFS.WEATHER_TEMPERATURE_UNITS])), external_React_default().createElement("div", {
        ...
        className: "weatherTemperature"
      }, WEATHER_SUGGESTION.current_conditions.temperature[Prefs.values[PREFS.WEATHER_TEMPERATURE_UNITS]], "\xB0", Prefs.values[PREFS.WEATHER_TEMPERATURE_UNITS])), external_React_default().createElement("div", {
        className: "weatherCityRow"
      }, external_React_default().createElement("span", {
        className: "weatherCity"
      ...
      }, external_React_default().createElement("span", null, WEATHER_SUGGESTION.forecast.high[Prefs.values[PREFS.WEATHER_TEMPERATURE_UNITS]], "\xB0", Prefs.values[PREFS.WEATHER_TEMPERATURE_UNITS]), external_React_default().createElement("span", null, "\u2022"), external_React_default().createElement("span", null, WEATHER_SUGGESTION.forecast.low[Prefs.values[PREFS.WEATHER_TEMPERATURE_UNITS]], "\xB0", Prefs.values[PREFS.WEATHER_TEMPERATURE_UNITS])), external_React_default().createElement("span", {
...
function TopicSelection({
  ...
  const selectedTopics = prefs[PREFS.TOPICS_SELECTED];
  const suggestedTopics = prefs[PREFS.TOPIC_SELECTION_SUGGESTED_TOPICS]?.split(", ");
  const displayCount = prefs[PREFS.TOPIC_SELECTION_ONBOARDING_DISPLAY_COUNT];
  const topicsHaveBeenPreviouslySet = prefs[PREFS.TOPIC_SELECTION_HAS_BEEN_UPDATED_PREVIOUSLY];
  ...
  function handleUserClose(e) {
    ...
    if (id === "first-run") {
      ...
      dispatch(actionCreators.SetPref(PREFS.TOPIC_SELECTION_ONBOARDING_MAYBE_DISPLAY, true));
    } else {
      dispatch(actionCreators.SetPref(PREFS.TOPIC_SELECTION_ONBOARDING_MAYBE_DISPLAY, false));
    }
    handleModalClose();
  }
  ...
  function handleSubmit() {
    ...
    dispatch(actionCreators.SetPref(PREFS.TOPICS_SELECTED, topicsString));
    dispatch(actionCreators.SetPref(PREFS.TOPIC_SELECTION_ONBOARDING_MAYBE_DISPLAY, false));
    if (!topicsHaveBeenPreviouslySet) {
      dispatch(actionCreators.SetPref(PREFS.TOPIC_SELECTION_HAS_BEEN_UPDATED_PREVIOUSLY, true));
    }
    dispatch(actionCreators.OnlyToMain({
...
function DownloadMobilePromoHighlight({
  ...
  const prefs = (0,external_ReactRedux_namespaceObject.useSelector)(state => state.Prefs.values);
  const mobileDownloadPromoVarA = prefs[PREFS.MOBILE_DOWNLOAD_HIGHLIGHT_VARIANT_A];
  const mobileDownloadPromoVarB = prefs[PREFS.MOBILE_DOWNLOAD_HIGHLIGHT_VARIANT_B];
  const mobileDownloadPromoVarC = prefs[PREFS.MOBILE_DOWNLOAD_HIGHLIGHT_VARIANT_C];
