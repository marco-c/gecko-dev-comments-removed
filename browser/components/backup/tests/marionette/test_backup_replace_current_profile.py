...
class BackupReplaceCurrentProfileTest(BackupTestBase):
    ...
    def test_replace_current_profile(self):
        ...
        self.logger.info("Step 5: Verifying old profile was deleted from database")
        profiles_after = self.get_all_profiles()
        self.logger.info(f"Profiles after recovery: {profiles_after}")

        old_profile = next(
            (p for p in profiles_after if p["id"] == original_profile_id), None
        )
        self.assertIsNone(
            old_profile,
            f"Old profile (id={original_profile_id}) should be deleted from database",
        )
        self.logger.info(f"Verified old profile (id={original_profile_id}) was deleted")

        self.logger.info("Step 6: Launching recovered profile to verify it works")
