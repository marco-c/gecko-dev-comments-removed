...
async function ensureTabOrder(order, win = window) {
  ...
  for (const match of order) {
    ...
    const matches = target.matches(match);
    ok(matches, `Expected ${match} next, got ${target}`);
    if (!matches) {
      return false;
    }
  }
...
async function ensureExpectedTabOrder(
  ...
  order.push("#report-broken-site-popup-cancel-button");
  order.push("#report-broken-site-popup-send-button");
  order.push(order[0]); 
