...
export class AIWindowTabStatesManager {
  ...
  /**
   * Adds necessary event listeners to a tab.
   *
   * @param {MozTabbrowserTab} tab The tab that needs listeners
   *
   * @private
   */
  #addEventListeners(tab) {
    ...
    this.#tabStates.set(tab, { browser, listener, state: null });
  }
  ...
  #removeEventListeners(tab) {
    const entry = this.#tabStates.get(tab);
    if (!entry) {
      return;
    }

    const { browser, listener } = entry;
  ...
  #getTabState(tab, newState = null) {
    ...
    if (newState) {
      const { input, mode, pageUrl, conversationId } = tabState;
      delete newState.tab;
  ...
  /**
   * Gets event listener object for a tab's browser to listen for
   * location changes. Updates the state of the sidebar on navigation.
   */
  #getListener() {
    return {
