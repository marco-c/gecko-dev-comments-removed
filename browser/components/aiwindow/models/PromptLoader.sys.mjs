...
async function selectFeatureConfig(feature, opts = {}) {
  ...
  if (!featureConfigs.length) {
    throw new Error(`No Remote Settings records found for feature: ${feature}`);
  }
 ...
 */
export async function loadPrompt(feature) {
  ...
  const mainConfig = await selectFeatureConfig(feature);
  if (!mainConfig.prompts) {
    throw new Error(`No prompts field in record for feature: ${feature}`);
  }
