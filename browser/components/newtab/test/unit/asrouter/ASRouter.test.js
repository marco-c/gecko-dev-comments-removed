  ...
  describe("onMessage", () => {
    ...
    describe("#isUnblockedMessage", () => {
      it("should block a message if the group is blocked", async () => {
        ...
        await Router.setGroupState({ id: "foo", value: true });
    ...
    describe("#onMessage: SET_GROUP_STATE", () => {
      ...
      it("should persist state to disk", async () => {
        ...
        await Router.setState({ groups: [{ id: "foo", enabled: false }] });
        ...
        assert.isTrue(
          Router.state.groups.find(({ id }) => id === "foo").enabled
        );
      });
    });
  ...
  describe("#setGroupState", () => {
    ...
    it("should enable group", async () => {
      await Router.setState({
        ...
        groupImpressions: { foo: [1] },
      });

      await Router.setGroupState({ id: "bar", value: true });

      assert.lengthOf(
        Router.state.groups.filter(({ enabled }) => enabled),
        2
      );
      assert.property(Router.state.groupImpressions, "foo");
      assert.property(Router.state.groupImpressions, "bar");
    });
    it("should cleanup storage groupImpressions", async () => {
      await Router.setState({
        groups: [
          { id: "foo", enabled: true },
          { id: "bar", enabled: true },
      ...
      await Router.setGroupState({ id: "bar", value: false });

      assert.calledOnce(Router._storage.set);
      const [field, value] = Router._storage.set.firstCall.args;
      assert.equal(field, "groupImpressions");
      assert.notProperty(value, "bar");
      assert.deepEqual(value.foo, [1]);
    });
    it("should add storage groupImpressions on enable", async () => {
      await Router.setState({
        groups: [
          { id: "foo", enabled: false },
          { id: "bar", enabled: true },
        ],
        groupImpressions: { bar: [2] },
      });
