...
async function testClearData(clearSiteData, clearCache) {
  ...
  await Promise.all([
    TestUtils.waitForCondition(
      () => clearSiteDataLabel.value.includes(convertedTotalUsage), "Should show the quota usage", 500, 200),
    TestUtils.waitForCondition(
      () => clearCacheLabel.value.includes(convertedCacheUsage), "Should show the cache usage", 500, 200)
  ]);
