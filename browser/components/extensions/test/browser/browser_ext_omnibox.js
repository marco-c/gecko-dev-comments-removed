...
add_task(async function() {
  ...
  async function testSuggestions(info) {
    ...
    function expectSuggestion({content, description}, index) {
      let item = gURLBar.popup.richlistbox.itemChildren[index + 1]; 
  ...
  await testHeuristicResult("Generated extension", false );
  await testHeuristicResult("hello world", true );
  await testHeuristicResult("foo bar", true );
