...
add_task(async function test_CreateWindowsShortcut() {
  ...
  const sh = Cc["@mozilla.org/toolkit/shell-service;1"].createInstance();
  sh.QueryInterface(Ci.nsIWindowsShellService);
  ...
  ok(
    removeFromShortcutLog(logFiles, DEST, "STARTMENU"),
    "Programs shortcut was logged"
  );
  ...
  ok(
    removeFromShortcutLog(logFiles, "ShortcutTest\\" + DEST, "STARTMENU"),
  ...
  ok(
    removeFromShortcutLog(logFiles, DEST, "DESKTOP"),
    "Desktop shortcut was logged"
  );
