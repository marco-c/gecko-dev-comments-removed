...
add_task(async function test_selector_window() {
  ...
  let windowOpening = BrowserTestUtils.domWindowOpenedAndLoaded();

  await gCUITestUtils.hidePanelMultiView(gCUITestUtils.PanelUI.panel, () =>
    manageButton.click()
  );

  let dialog = await windowOpening;
  ...
  let activated = BrowserTestUtils.waitForEvent(dialog, "activate");
  await gCUITestUtils.hidePanelMultiView(gCUITestUtils.PanelUI.panel, () =>
    manageButton.click()
  );
  await activated;
