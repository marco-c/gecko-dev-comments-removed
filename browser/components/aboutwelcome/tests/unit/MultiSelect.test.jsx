...
describe("MultiSelect component", () => {
  ...
  it("should filter out checkboxes when targeting evaluates to false", async () => {
    const evaluateTargetingStub = sandbox.stub();
    sandbox
      .stub(window, "AWEvaluateAttributeTargeting")
      .value(evaluateTargetingStub);

    const TARGETING_PROPS = { ...MULTISELECT_SCREEN_PROPS };
    TARGETING_PROPS.content.tiles.data = [
      {
        id: "checkbox-1",
        defaultValue: true,
        label: "Checkbox 1",
    ...
    evaluateTargetingStub.withArgs("doesAppNeedPin").resolves(true);
    evaluateTargetingStub.withArgs("!isDefaultBrowser").resolves(false);

    const wrapper = mount(<MultiSelect {...TARGETING_PROPS} />);
    ...
    const checkBoxes = wrapper.find(".checkbox-container input");
    assert.strictEqual(checkBoxes.length, 2);
    assert.strictEqual(checkBoxes.at(0).prop("value"), "checkbox-1");
    assert.strictEqual(checkBoxes.at(1).prop("value"), "checkbox-2");
  });

  it("should show checkboxes by default if no targeting prop", async () => {
    const wrapper = mount(<MultiSelect {...MULTISELECT_SCREEN_PROPS} />);
    ...
    const checkBoxes = wrapper.find(".checkbox-container input");
    assert.strictEqual(checkBoxes.length, 3);
    // Checkboxes have no targeting prop
    assert.strictEqual(checkBoxes.at(0).prop("value"), "checkbox-1");
    assert.strictEqual(checkBoxes.at(1).prop("value"), "checkbox-2");
    assert.strictEqual(checkBoxes.at(2).prop("value"), "checkbox-3");
  });

  it("should not call AWEvaluateAttributeTargeting when checkboxes have no targeting", async () => {
    const evaluateTargetingStub = sandbox.stub();
    sandbox
      .stub(window, "AWEvaluateAttributeTargeting")
      .value(evaluateTargetingStub);
    ...
    await new Promise(resolve => setTimeout(resolve, 0));
    wrapper.update();
    assert.notCalled(evaluateTargetingStub);

    const checkBoxes = wrapper.find(".checkbox-container input");
    assert.strictEqual(checkBoxes.length, 3);
  });

  it("should render nothing when all checkboxes evaluate to false", async () => {
    const evaluateTargetingStub = sandbox.stub();
    sandbox
      .stub(window, "AWEvaluateAttributeTargeting")
      .value(evaluateTargetingStub);
    ...
    evaluateTargetingStub.withArgs("false").resolves(false);
    ...
    assert.strictEqual(wrapper.isEmptyRender(), true);
  });
});
