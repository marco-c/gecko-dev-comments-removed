...
export class SearchModeSwitcher {
  ...
  async #buildSearchModeList() {
    ...
    for (let item of this.#panelList.querySelectorAll(
    ...
    if (this.#panelList.wasOpenedByKeyboard) {
      // Focus may not be on first item anymore
      // because new items were added before.
  ...
  search({ engine = null, restrict = null, whereToOpenSerp = null } = {}) {
    ...
    prefsbutton.addEventListener("click", this);
    prefsbutton.addEventListener("keydown", this);
  }
  ...
  #disableObservers() {
    ...
    prefsbutton.removeEventListener("click", this);
    prefsbutton.removeEventListener("keydown", this);
  }
