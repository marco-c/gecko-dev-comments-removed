...
const tests = [
  ...
  async function (win) {
    info("Drop something.");
    let promise = BrowserTestUtils.browserLoaded(win.gBrowser.selectedBrowser);
    let trimmedProtocol = UrlbarTestUtils.getTrimmedProtocolWithSlashes();
    let url = "http://www.example.com";
    let trimmedUrlLength =
      url.length -
      (url.startsWith(trimmedProtocol) ? trimmedProtocol.length : 0);
    EventUtils.synthesizeDrop(
      win.document.getElementById("back-button"),
      win.gURLBar.inputField,
      [[{ type: "text/plain", data: url }]],
      "copy",
      win
    );
    await promise;
