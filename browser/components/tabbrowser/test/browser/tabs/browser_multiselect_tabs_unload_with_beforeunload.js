...
add_task(async function test_unload_selected_and_allow() {
  ...
  injectBeforeUnload(tab1.linkedBrowser);
  ...
  await awaitAndCloseBeforeUnloadDialog(tab1.linkedBrowser, false);
...
add_task(async function test_unload_multiple_and_allow() {
  ...
  injectBeforeUnload(tab2.linkedBrowser);
  injectBeforeUnload(tab3.linkedBrowser);
...
add_task(async function test_unload_multiple_and_block_first() {
  await SpecialPowers.pushPrefEnv({
    set: [
      ["browser.tabs.unloadTabInContextMenu", true],
      ["dom.require_user_interaction_for_beforeunload", false],
  ...
  await BrowserTestUtils.switchTab(gBrowser, tab2);
  await triggerClickOn(tab3, { ctrlKey: true });
  let tab2Promise = awaitAndCloseBeforeUnloadDialog(tab2.linkedBrowser, true);
  {
    let menu = await openTabMenuFor(tab2);
    menu.activateItem(menuItemUnload);
  }
  ...
  await BrowserTestUtils.removeTab(tab3);
  await tab3Promise;
  tab2Promise = awaitAndCloseBeforeUnloadDialog(tab2.linkedBrowser, false);
  await BrowserTestUtils.removeTab(tab2);
  await tab2Promise;
  await BrowserTestUtils.removeTab(tab1);
});

add_task(async function test_unload_multiple_and_block_second() {
  await SpecialPowers.pushPrefEnv({
    set: [
      ["browser.tabs.unloadTabInContextMenu", true],
      ["dom.require_user_interaction_for_beforeunload", false],
