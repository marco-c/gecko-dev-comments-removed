...
add_task(async function panelAppearsOnTabGroupHover() {
  ...
  const tab2 = await addTabTo(gBrowser, "about:welcome");
  ...
  const controlTab = BrowserTestUtils.addTab(gBrowser, "https://example.com");
  await BrowserTestUtils.browserLoaded(controlTab.linkedBrowser);
...
add_task(async function panelClickElementSwitchesTabs() {
  const tabs = [
    BrowserTestUtils.addTab(gBrowser, "about:robots"),
    BrowserTestUtils.addTab(gBrowser, "about:welcome"),
  ];
