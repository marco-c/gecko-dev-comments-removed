...
add_task(async function feature_callout_is_repositioned_if_it_does_not_fit() {
  ...
  await BrowserTestUtils.withNewTab(
    ...
    async browser => {
      ...
      browser.contentWindow.resizeTo(400, 800);
      await waitForCalloutScreen(document, ".FEATURE_CALLOUT_3");
      
      let callout = document.querySelector(calloutSelector);
      ...
      ok(
        ...
        "On third screen with very narrow window, the callout is positioned above the parent element"
      );
    }
  );
});
