...
async function testScreenElementsRendered(win, message) {
  ...
  const selectors = [
    `main.${message.content.screens[0].id}`, 
    `img[src="${screenContent.logo.imageURL}"]`, 
    `h1[data-l10n-id="${screenContent.title.string_id}"]`, 
    `h2[data-l10n-id="${screenContent.subtitle.string_id}"]`, 
    `button.primary[data-l10n-id="${screenContent.primary_button.label.string_id}"]`, 
    `button.secondary[data-l10n-id="${screenContent.secondary_button.label.string_id}"]`, 
  ];
