...
add_UITour_task(async function test_highlight_size_attributes() {
  ...
  await elementVisiblePromise(highlight, "Highlight should be moved to the urlbar");
...
add_UITour_task(async function test_info_size_attributes() {
  ...
  await elementVisiblePromise(tooltip, "Tooltip should be shown after showInfo() for the appMenu");
