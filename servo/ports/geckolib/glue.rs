...
pub extern "C" fn Servo_ResolvePseudoStyle(
    ...
    let style = get_pseudo_style(
        ...
        &doc_data.stylist,
        is_probe,
         if pseudo_element.is_highlight() {Some(&matching_fn)} else {None},
    );
...
pub extern "C" fn Servo_GetComputedKeyframeValues(
    ...
    let mut raw_custom_properties_block; 
...
pub extern "C" fn Servo_GetAnimationValues(
    ...
    let container_size_query = ContainerSizeQuery::for_element(element, None,  false);
    let mut conditions = Default::default();
    let mut context = create_context_for_animation(
        &data,
        &style,
...
pub extern "C" fn Servo_RegisterCustomProperty(
    ...
    let Ok(syntax) = Descriptor::from_str(syntax,  false) else { return InvalidSyntax };
