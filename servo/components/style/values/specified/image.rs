...
impl Parse for Image {
    fn parse<'i, 't>(
        ...
        Image::parse_with_cors_mode(context, input, CorsMode::None,  true,  false)
    }
}
...
impl CrossFadeImage {
    fn parse<'i, 't>(
        ...
        if let Ok(image) = input.try_parse(|input| {
            Image::parse_with_cors_mode(context, input, cors_mode,  false,  false)
        }) {
    ...
    fn parse<'i, 't>(
        ...
        let image = match input.try_parse(|i| i.expect_url_or_string()) {
            ...
            Err(..) => Image::parse_with_cors_mode(
                context, input, cors_mode,  false,   only_url
            )?,
        };
