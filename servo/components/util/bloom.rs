...
impl BloomFilter {
    ...
    fn mask(&self) -> uint {
        (self.buf.len()*BUCKETS_PER_WORD) - 1 
    }
    ...
    fn bucket_set(&mut self, a_idx: uint, shift_amount: uint, new_val: uint) {
        ...
        let mask = (1 << BITS_PER_BUCKET) - 1;                
        let select_in_bucket = mask << shift_amount;          
        let select_out_of_bucket = !select_in_bucket;         
        let new_array_val = (new_val << shift_amount)         
                          | (*old_val & select_out_of_bucket); 
