...
SideBits nsLayoutUtils::GetSideBitsForFixedPositionContent(
  ...
  if (aFixedPosFrame) {
    ...
    if (!position
             ->GetAnchorResolvedInset(eSideRight, params)
             ->IsAuto()) {
      sides |= SideBits::eRight;
    }
    if (!position
             ->GetAnchorResolvedInset(eSideLeft, params)
             ->IsAuto()) {
      sides |= SideBits::eLeft;
    }
    if (!position
             ->GetAnchorResolvedInset(eSideBottom, params)
             ->IsAuto()) {
      sides |= SideBits::eBottom;
    }
    if (!position
             ->GetAnchorResolvedInset(eSideTop, params)
             ->IsAuto()) {
      sides |= SideBits::eTop;
    }
  }
