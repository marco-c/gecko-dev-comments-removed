...
void CSSKeyframesRule::GetCssText(nsACString& aCssText) const {
 ...
 dom::CSSRuleList* CSSKeyframesRule::CssRules() {
  return EnsureRules();
}

 dom::CSSKeyframeRule* CSSKeyframesRule::IndexedGetter(
    uint32_t aIndex, bool& aFound) {
  return EnsureRules()->IndexedGetter(aIndex, aFound);
}

 uint32_t CSSKeyframesRule::Length() {
  return EnsureRules()->Length();
}

 dom::CSSKeyframeRule* CSSKeyframesRule::FindRule(
