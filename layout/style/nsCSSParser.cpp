...
enum class GridTrackListFlags {
  eDefaultTrackList  = 0x0, 
  eExplicitTrackList = 0x1, 
};
...
protected:
  ...
  bool ParseValueList(nsCSSPropertyID aPropID); 
                             ...
                             char16_t aPrevCombinator)
{
  ...
  while (parsingStatus == eSelectorParsingStatus_Continue) {
    if (mToken.IsSymbol(':')) {    
      ...
      UngetToken();
      break;
    } else if (eCSSToken_ID == mToken.mType) { 
...
CSSParserImpl::ParseWebkitGradient(nsCSSValue& aValue)
{
  ...
  RefPtr<nsCSSValueGradient> cssGradient =
    new nsCSSValueGradient(isRadial, false );
