  ...
  public:
                                           ...
                                           ValType result, MDefinition* oldv, MDefinition* newv)
    {
        ...
        if (isSmallerAccessForI64(result, access)) {
            auto* cvtOldv = MWrapInt64ToInt32::New(alloc(), oldv,  true);
            curBlock_->add(cvtOldv);
            oldv = cvtOldv;

            auto* cvtNewv = MWrapInt64ToInt32::New(alloc(), newv,  true);
                                    ...
                                    MDefinition* value)
    {
        ...
        if (isSmallerAccessForI64(result, access)) {
            auto* cvtValue = MWrapInt64ToInt32::New(alloc(), value,  true);
    ...
    bool passArg(MDefinition* argDef, ValType type, CallCompileState* call)
    {
        ...
        switch (arg.kind()) {
          ...
          case ABIArg::GPR_PAIR: {
            auto mirLow = MWrapInt64ToInt32::New(alloc(), argDef,  true);
            curBlock_->add(mirLow);
            auto mirHigh = MWrapInt64ToInt32::New(alloc(), argDef,  false);
...
EmitOldAtomicsLoad(FunctionCompiler& f)
{
    ...
    MemoryAccessDesc access(viewType, addr.align, addr.offset, Some(f.bytecodeOffset()),
                             0, MembarBeforeLoad, MembarAfterLoad);
...
EmitOldAtomicsStore(FunctionCompiler& f)
{
    ...
    MemoryAccessDesc access(viewType, addr.align, addr.offset, Some(f.bytecodeOffset()),
                             0, MembarBeforeStore, MembarAfterStore);
...
SimdToLaneType(ValType type)
{
    switch (type) {
      ...
      case ValType::B8x16:
      case ValType::B16x8:
      case ValType::B32x4:  return ValType::I32; 
      case ValType::I32:
      case ValType::I64:
      case ValType::F32:
      case ValType::F64:
    ...
    while (true) {
        ...
        switch (op.b0) {
          ...
          case uint16_t(Op::F32Div):
            CHECK(EmitDiv(f, ValType::F32, MIRType::Float32,  false));
          ...
          case uint16_t(Op::F64Div):
            CHECK(EmitDiv(f, ValType::F64, MIRType::Double,  false));
          ...
          case uint16_t(Op::MozPrefix): {
            ...
            switch (op.b1) {
              ...
              case uint16_t(MozOp::F64Mod):
                CHECK(EmitRem(f, ValType::F64, MIRType::Double,  false));
