...
#  endif
  ...
  template <>
  BaseCompiler& selectCompiler<BaseCompiler>() {
    return *this;
  }
  template <>
  MacroAssembler& selectCompiler<MacroAssembler>() {
    return masm;
  }
...
#  endif  
          ...
          case uint32_t(SimdOp::I64x2ShrS):
            CHECK_NEXT(emitVectorShiftRightI64x2( false));
          case uint32_t(SimdOp::I64x2ShrU):
            CHECK_NEXT(emitVectorShiftRightI64x2( true));
      ...
      case uint16_t(Op::MiscPrefix): {
        switch (op.b1) {
          ...
          case uint32_t(MiscOp::MemInit):
            CHECK_NEXT(emitMemOrTableInit(true));
