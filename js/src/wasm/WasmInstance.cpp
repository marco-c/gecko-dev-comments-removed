...
void* Instance::checkedCallEntry(const Tier tier,
                                 const uint32_t functionIndex) const {
...
void* Instance::checkedCallEntryForWasmImportedFunction(JSFunction* fun) const {
  MOZ_ASSERT(IsWasmExportedFunction(fun));
  const Tier tier = code().bestTier();
  return checkedCallEntry(tier, ExportedFunctionToFuncIndex(fun));
}
...
inline int32_t WasmMemoryCopy32(T memBase, size_t memLen,
 ...
 int32_t Instance::memCopy32(Instance* instance,
 ...
 int32_t Instance::memCopyShared32(Instance* instance,
                                               uint32_t dstByteOffset,
...
inline int32_t WasmMemoryFill32(T memBase, size_t memLen, uint32_t byteOffset,
 ...
 int32_t Instance::memFill32(Instance* instance,
 ...
 int32_t Instance::memFillShared32(Instance* instance,
                                               uint32_t byteOffset,
 ...
 int32_t Instance::memInit32(Instance* instance, uint32_t dstOffset,
...
void* Instance::createIndirectStub(uint32_t funcIndex, TlsData* targetTlsData) {
  ...
  void* checkedCallEntry =
      targetTlsData->instance->checkedCallEntry(calleeTier, funcIndex);
  if (!targets.append(
...
void* Instance::getIndirectStub(uint32_t funcIndex,
                                TlsData* targetTlsData) const {
  MOZ_ASSERT(funcIndex != NullFuncIndex);

  if (code().hasTier(Tier::Baseline)) {
    auto stubs = code(Tier::Baseline).lazyStubs().lock();
    void* result = stubs->lookupIndirectStub(funcIndex, targetTlsData);
    if (result) {
      return result;
    }
  }

  if (code().hasTier(Tier::Optimized)) {
    auto stubs = code(Tier::Optimized).lazyStubs().lock();
    return stubs->lookupIndirectStub(funcIndex, targetTlsData);
  }
...
static bool AddIndirectStubTarget(
    VectorOfIndirectStubTarget* indirectStubTargets, uint32_t functionIndex,
    Instance* callerInstance, TlsData* calleeTls,
    void* calleeCheckedCallEntry) {
  if (callerInstance->getIndirectStub(functionIndex, calleeTls)) {
    return true;
  }

  auto target =
      IndirectStubTarget{functionIndex, calleeCheckedCallEntry, calleeTls};
  return indirectStubTargets->append(std::move(target));
}
...
bool Instance::ensureIndirectStubs(const Uint32Vector& elemFuncIndices,
  ...
  for (uint32_t i = 0; i < len; i++) {
    ...
    if (IsImportedFunction(funcIndex, metadataTier)) {
      ...
      if (!AddIndirectStubTarget(
              &targets, funcIndex, this, import.tls->instance->tlsData(),
              import.tls->instance->checkedCallEntryForWasmImportedFunction(
                  fun))) {
        return false;
      }
      continue;
    ...
    if (!AddIndirectStubTarget(&targets, funcIndex, this, tlsData(),
                               checkedCallEntry(tier, funcIndex))) {
      return false;
    }
  }
...
bool Instance::initElems(uint32_t tableIndex, const ElemSegment& seg,
  ...
  for (uint32_t i = 0; i < len; i++) {
    ...
    if (IsImportedFunction(funcIndex, metadataTier)) {
      ...
      JSFunction* fun = import.fun;
      if (IsJSExportedFunction(fun)) {
        code = checkedCallEntry(tier, funcIndex);
      } else {
        code = getIndirectStub(funcIndex, import.tls);
      }
    } else {
      ...
      if (table.isImportedOrExported()) {
        code = getIndirectStub(funcIndex, tlsData());
      } else {
        code = checkedCallEntry(tier, funcIndex);
      }
    }
 ...
 void* Instance::exceptionNew(Instance* instance, uint32_t exnIndex,
                                          uint32_t nbytes) {
 ...
 uint32_t Instance::getLocalExceptionIndex(Instance* instance,
                                                       JSObject* exn) {
 ...
 int32_t Instance::refTest(Instance* instance, void* refPtr,
                                       void* rttPtr) {
 ...
 void* Instance::rttSub(Instance* instance, void* rttParentPtr,
                                    void* rttSubCanonPtr) {
 ...
 int32_t Instance::intrI8VecMul(Instance* instance, uint32_t dest,
