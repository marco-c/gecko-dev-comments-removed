...
{
    switch (type) {
      case ExprType::Void: return true; 
...
RenderBlock(WasmRenderContext& c, AstBlock& block, bool isInline = false)
{
        ...
        block.exprs()[0]->as<AstBlock>().op() == Op::Block)
    {
        ...
        if (!RenderBlock(c, block.exprs()[0]->as<AstBlock>(),  true))
            return false;
...
static bool
RenderExpr(WasmRenderContext& c, AstExpr& expr, bool newLine )
{
    switch (expr.kind()) {
