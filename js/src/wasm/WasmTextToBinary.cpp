...
static AstExpr* ParseExprBody(WasmParseContext& c, WasmToken token,
  ...
  switch (token.kind()) {
    ...
    case WasmToken::MemCopy:
      return ParseMemOrTableCopy(c, inParens, true);
    case WasmToken::DataDrop:
      return ParseDataOrElemDrop(c, true);
    case WasmToken::MemFill:
      return ParseMemFill(c, inParens);
    case WasmToken::MemInit:
      return ParseMemOrTableInit(c, inParens, true);
    case WasmToken::TableCopy:
      return ParseMemOrTableCopy(c, inParens, false);
    case WasmToken::ElemDrop:
      return ParseDataOrElemDrop(c, false);
    case WasmToken::TableInit:
      return ParseMemOrTableInit(c, inParens, false);
#endif
#ifdef ENABLE_WASM_REFTYPES
