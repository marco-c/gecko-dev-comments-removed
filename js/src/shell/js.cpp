...
bool OffThreadJob::dispatch() {
 ...
 void OffThreadJob::OffThreadMain(OffThreadJob* self) {
  self->run();
}
...
static bool WasmCompileInSeparateProcess(JSContext* cx, unsigned argc,
  ...
  size_t numBytes;
  if (!args[0].isObject() ||
      !IsBufferSource(cx, &args[0].toObject(),  false,
                       false, &bytecode, &numBytes)) {
    RootedObject callee(cx, &args.callee());
...
class StreamCacheEntryObject : public NativeObject {
 ...
 public:
  ...
  static bool construct(JSContext* cx, unsigned argc, Value* vp) {
    ...
    size_t numBytes;
    if (!args[0].isObject() ||
        !IsBufferSource(cx, &args[0].toObject(),  true,
                         true, &ptr, &numBytes)) {
      RootedObject callee(cx, &args.callee());
...
static bool ConsumeBufferSource(JSContext* cx, JS::HandleObject obj,
  ...
  size_t byteLength;
  if (IsBufferSource(cx, obj,  true,  true,
