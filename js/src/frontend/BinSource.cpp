...
BinASTParser::parseAux(const uint8_t* start, const size_t length)
{
    ...
    BinParseContext globalpc(cx_, this, &globalsc,  nullptr);
    if (!globalpc.init())
        return cx_->alreadyReportedError();
...
BinASTParser::parseStringList(MutableHandle<Maybe<Names>> out)
{
    ...
    for (uint32_t i = 0; i < length; ++i) {
        ...
        result.infallibleAppend(Move(string)); 
    }
...
BinASTParser::parseStatementAux(const BinKind kind, const BinFields& fields)
{
    ...
    switch (kind) {
      ...
      case BinKind::ForStatement: {
        ...
        TRY_VAR(result, factory_.newForStatement(start, forHead, body,  0));
        ...
        switch (field) {
          case BinField::Label:
            ...
            if (label && !label->isKind(PNK_NAME))
                return raiseError("ContinueStatement | BreakStatement - Label MUST be an identifier"); 
...
BinASTParser::parseObjectPropertyName()
{
    ...
    switch (kind) {
      ...
      case BinKind::Identifier:
        MOZ_TRY_VAR(result, parseIdentifierAux(kind, fields,  true));
        break;
        ...
        for (auto field : fields) {
            switch (field) {
              ...
              case BinField::Declarations: {
                ...
                TRY(tokenizer_->enterList(length, guard));
                TRY_VAR(result, factory_.newDeclarationList(PNK_CONST , tokenizer_->pos(start)));
...
BinASTParser::parseExpressionAux(const BinKind kind, const BinFields& fields)
{
    ...
    switch (kind) {
      ...
      case BinKind::UpdateExpression: {
        ...
        if (kind == BinKind::UnaryExpression) {
            ...
            if (!expr->isKind(PNK_NAME) && !factory_.isPropertyAccess(expr))
                return raiseError("Invalid increment/decrement operand"); 
      ...
      case BinKind::LogicalExpression: {
        ...
        if (left->isKind(pnk) && pnk != PNK_POW ) {
        ...
        for (auto field : fields) {
            switch (field) {
              ...
              case BinField::Arguments:
                MOZ_TRY_VAR(result, parseExpressionList( false));
                break;
              default:
    ...
    for (auto field : fields) {
        switch (field) {
          case BinField::Elements: {
            MOZ_TRY_VAR(result, parseExpressionList( true));
            break;
          }
          default:
...
BinASTParser::parseCatchClause()
{
    ...
    switch (kind) {
      ...
      case BinKind::CatchClause: {
        ...
        TRY_DECL(catchClause, factory_.newCatchBlock(param,  nullptr, body));
        TRY_DECL(bindings, NewLexicalScopeData(cx_, scope, alloc_, parseContext_));

        TRY_DECL(catchClauseScope, factory_.newLexicalScope(*bindings, catchClause));

        TRY_VAR(result, factory_.newCatchList(tokenizer_->pos(start)));

        result->appendWithoutOrderAssumption(catchClauseScope);
      }
    }
...
JS::Result<ParseNode*>
BinASTParser::parseIdentifierAux(const BinKind, const BinFields& fields, const bool expectObjectPropertyName )
{
    const size_t start = tokenizer_->offset();
