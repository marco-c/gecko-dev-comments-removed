...
template <>
bool
Parser<FullParseHandler, char16_t>::finishFunction(bool isStandaloneFunction )
{
...
template <>
bool
Parser<SyntaxParseHandler, char16_t>::finishFunction(bool isStandaloneFunction )
{
                                                       ...
                                                       Directives* newDirectives)
{
    ...
    FunctionBox* funbox = newFunctionBox(fn, fun,  0, inheritedDirectives,
                                         generatorKind, asyncKind,  false);
    if (!funbox)
        return null();
                                                          ...
                                                          FunctionSyntaxKind kind, bool tryAnnexB)
{
    ...
    FunctionBox* funbox = newFunctionBox(pn, fun, toStringStart, Directives( false),
                                                           ...
                                                           FunctionAsyncKind asyncKind)
{
    ...
    FunctionBox* funbox = newFunctionBox(pn, fun,  0, directives,
                                         generatorKind, asyncKind,  false);
    if (!funbox)
        return null();
                                       ...
                                       InvokedPrediction invoked )
{
    ...
    switch (tt) {
      ...
      default: {
        Node expr = memberExpr(yieldHandling, tripledotHandling, tt,  true,
                               possibleError, invoked);
        if (!expr)
            return null();
...
Parser<ParseHandler, CharT>::generatorComprehensionLambda(unsigned begin)
{
    ...
    RootedFunction fun(context, newFunction( nullptr, Expression,
    ...
    FunctionBox* genFunbox = newFunctionBox(genfn, fun,  0, directives,
                                            StarGenerator, SyncFunction,  false);
    if (!genFunbox)
        return null();
