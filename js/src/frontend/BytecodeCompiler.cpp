...
BytecodeCompiler::createParser()
{
    ...
    parser.emplace(cx, alloc, options, sourceBuffer.get(), sourceBuffer.length(),
                    true, *usedNames, syntaxParser.ptrOr(nullptr), nullptr);
...
bool
BytecodeCompiler::createSourceAndParser(const Maybe<uint32_t>& parameterListEnd )
{
...
bool
BytecodeCompiler::createScript(uint32_t preludeStart , uint32_t postludeEnd )
{
    script = JSScript::Create(cx, options,
                              sourceObject,  0, sourceBuffer.length(),
...
BytecodeCompiler::emplaceEmitter(Maybe<BytecodeEmitter>& emitter, SharedContext* sharedContext)
{
    ...
    emitter.emplace( nullptr, parser.ptr(), sharedContext, script,
                     nullptr, options.lineno, emitterMode);
    return emitter->init();
}
                              ...
                              SourceCompressionTask** sourceCompressionTaskOut)
{
    ...
    BytecodeCompiler compiler(cx, alloc, options, srcBuf,  nullptr);
