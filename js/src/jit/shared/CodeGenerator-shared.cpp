...
void CodeGeneratorShared::encode(LRecoverInfo* recover) {
  ...
  ResumeMode mode = recover->mir()->mode();
  MOZ_ASSERT(mode == ResumeMode::ResumeAt || mode == ResumeMode::ResumeAfter);
  bool resumeAfter = (mode == ResumeMode::ResumeAfter);
...
OutOfLineCode* CodeGeneratorShared::oolTruncateDouble(
  ...
  OutOfLineTruncateSlow* ool = new (alloc()) OutOfLineTruncateSlow(
      src, dest,  false, bytecodeOffset, preserveTls);
