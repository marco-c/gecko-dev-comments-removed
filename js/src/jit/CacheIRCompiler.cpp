...
bool CacheIRCompiler::emitMathClz32Result(Int32OperandId inputId) {
  ...
  masm.clz32(input, scratch,  false);
  EmitStoreResult(masm, scratch, JSVAL_TYPE_INT32, output);
  return true;
}
...
bool CacheIRCompiler::emitNumberMinMax(bool isMax, NumberOperandId firstId,
  ...
  if (isMax) {
    masm.maxDouble(scratch2, scratch1,  true);
  } else {
    masm.minDouble(scratch2, scratch1,  true);
  }
...
bool CacheIRCompiler::emitComparePointerResultShared(JSOp op,
  ...
  Label ifTrue, done;
  masm.branchPtr(JSOpToCondition(op,  true), left, right,
                 &ifTrue);
...
bool CacheIRCompiler::emitCompareInt32Result(JSOp op, Int32OperandId lhsId,
  ...
  Label ifTrue, done;
  masm.branch32(JSOpToCondition(op,  true), left, right, &ifTrue);
...
bool CacheIRCompiler::emitCompareBigIntInt32ResultShared(
  ...
  
  {
    ...
    if (reversed != op) {
      masm.branchPtr(JSOpToCondition(reversed,  false), scratch1,
                     scratch2, &ifTrue);
      masm.jump(&ifFalse);
    }

    masm.bind(&doCompare);
    masm.branchPtr(JSOpToCondition(op,  false), scratch1,
                   scratch2, &ifTrue);
  }
