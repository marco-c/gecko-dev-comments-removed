...
IonBuilder::inlineConstantStringSplitString(CallInfo& callInfo)
{
    ...
    for (uint32_t i = 0; i < initLength; i++) {
        ...
        MOZ_TRY(initializeArrayElement(array, i, value, unboxedType,  false));
    }
...
IonBuilder::inlineDefineDataProperty(CallInfo& callInfo)
{
    ...
    bool emitted = false;
    MOZ_TRY(setElemTryDense(&emitted, obj, id, value,  true));
    if (!emitted)
        return InliningStatus_NotInlined;
...
IonBuilder::inlineSimd(CallInfo& callInfo, JSFunction* target, SimdType type)
{
    ...
    switch(simdOp) {
      ...
      case SimdOperation::Fn_allTrue:
        return inlineSimdAnyAllTrue(callInfo, true, native, type);
      case SimdOperation::Fn_anyTrue:
        return inlineSimdAnyAllTrue(callInfo, false, native, type);
