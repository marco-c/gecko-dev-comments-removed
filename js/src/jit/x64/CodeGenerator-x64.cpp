                                    ...
                                    const LDefinition* output, bool isInt64)
{
    ...
    verifyLoadDisassembly(before, after, isInt64, accessType,  0, srcAddr,
                          *output->output());
...
{
    visitWasmLoadBase(ins->mir(), ins->ptr(), ins->output(),  false);
}

void
CodeGeneratorX64::visitWasmLoadI64(LWasmLoadI64* ins)
{
    visitWasmLoadBase(ins->mir(), ins->ptr(), ins->output(),  true);
}
...
CodeGeneratorX64::emitSimdLoad(LAsmJSLoadHeap* ins)
{
    ...
    if (numElems == 3) {
        ...
        uint32_t after = masm.size();
        verifyLoadDisassembly(before, after,  false, type, 2, srcAddr,
                              *ins->output()->output());
        ...
        after = masm.size();
        verifyLoadDisassembly(before, after,  false, type, 1, srcAddrZ,
        ...
        uint32_t after = masm.size();
        verifyLoadDisassembly(before, after,  true, type, numElems, srcAddr,
                              *ins->output()->output());
        masm.append(AsmJSMemoryAccess(before, wasm::MemoryAccess::Throw));
    }
...
CodeGeneratorX64::visitAsmJSLoadHeap(LAsmJSLoadHeap* ins)
{
    ...
    verifyLoadDisassembly(before, after,  false, accessType, 0, srcAddr,
                          *out->output());
...
CodeGeneratorX64::emitSimdStore(LAsmJSStoreHeap* ins)
{
    ...
    if (numElems == 3) {
        ...
        uint32_t after = masm.size();
        verifyStoreDisassembly(before, after,  false, type, 1, dstAddrZ,
                               LFloatReg(ScratchSimd128Reg));
        masm.append(AsmJSMemoryAccess(before, wasm::MemoryAccess::Throw, 8));
        ...
        after = masm.size();
        verifyStoreDisassembly(before, after,  false, type, 2, dstAddr,
                               *ins->value());
        masm.append(AsmJSMemoryAccess(before, wasm::MemoryAccess::Throw));
    } else {
        uint32_t before = masm.size();
        ...
        uint32_t after = masm.size();
        verifyStoreDisassembly(before, after,  false, type, numElems, dstAddr,
                               *ins->value());
        masm.append(AsmJSMemoryAccess(before, wasm::MemoryAccess::Throw));
    }
...
CodeGeneratorX64::visitAsmJSStoreHeap(LAsmJSStoreHeap* ins)
{
    ...
    verifyStoreDisassembly(before, after,  false, accessType, 0, dstAddr, *value);
