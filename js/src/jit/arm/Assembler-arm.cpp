...
BufferOffset Assembler::as_mov(Register dest, Operand2 op2, SBit s,
 ...
 void Assembler::as_alu_patch(Register dest, Register src1,
                                          Operand2 op2, ALUOp op, SBit s,
                                          Condition c, uint32_t* pos) {
  WriteInstStatic(EncodeAlu(dest, src1, op2, op, s, c), pos);
}

 void Assembler::as_mov_patch(Register dest, Operand2 op2, SBit s,
                                          Condition c, uint32_t* pos) {
...
BufferOffset Assembler::as_movw(Register dest, Imm16 imm, Condition c) {
 ...
 void Assembler::as_movw_patch(Register dest, Imm16 imm,
...
BufferOffset Assembler::as_movt(Register dest, Imm16 imm, Condition c) {
 ...
 void Assembler::as_movt_patch(Register dest, Imm16 imm,
                                           Condition c, Instruction* pos) {
...
BufferOffset Assembler::as_dtr(LoadStore ls, int size, Index mode, Register rt,
 ...
 void Assembler::as_dtr_patch(LoadStore ls, int size, Index mode,
...
BufferOffset Assembler::as_Imm32Pool(Register dest, uint32_t value,
 ...
 void Assembler::WritePoolEntry(Instruction* addr, Condition c,
                                            uint32_t data) {
...
BufferOffset Assembler::writeVFPInst(vfp_size sz, uint32_t blob) {
 ...
 void Assembler::WriteVFPInstStatic(vfp_size sz, uint32_t blob,
                                                uint32_t* dest) {
  MOZ_ASSERT((sz & blob) == 0);
  MOZ_ASSERT((VfpTag & blob) == 0);
...
BufferOffset Assembler::as_vdtr(
 ...
 void Assembler::as_vdtr_patch(LoadStore ls, VFPRegister vd,
