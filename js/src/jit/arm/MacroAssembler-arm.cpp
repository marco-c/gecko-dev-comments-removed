                                ...
                                Register ptrScratch, AnyRegister output, Register64 out64)
{
    ...
    if (out64 != Register64::Invalid()) {
        if (type == Scalar::Int64) {
            ...
            load = ma_dataTransferN(IsLoad, 32,  false, HeapReg, ptr, out64.low);
                                 ...
                                 Register64 val64, Register ptr, Register ptrScratch)
{
    ...
    if (type == Scalar::Int64) {
        ...
        store = ma_dataTransferN(IsStore, 32 ,  false, HeapReg, ptr, val64.low);
        append(access, store.getOffset(), framePushed);

        as_add(ptr, ptr, Imm8(INT64HIGH_OFFSET));

        store = ma_dataTransferN(IsStore, 32 ,  true, HeapReg, ptr, val64.high);
        append(access, store.getOffset(), framePushed);
    } else {
        if (value.isFloat()) {
            ScratchRegisterScope scratch(asMasm());
            ...
            store = ma_dataTransferN(IsStore, 8 * byteSize , isSigned, HeapReg, ptr, val);
            append(access, store.getOffset(), framePushed);
        }
    }
