                           ...
                           Index mode, Condition cc)
{
    MOZ_ASSERT((rt.code() & 1) == 0);
    MOZ_ASSERT(rt2.value.code() == rt.code() + 1);
    MOZ_ASSERT(addr.maybeOffsetRegister() != rt); 
...
{
    wasmTruncateToInt32(input, output, MIRType::Double,  true, isSaturating,
                        oolEntry);
}
...
MacroAssembler::wasmTruncateDoubleToInt32(FloatRegister input, Register output,
                                          bool isSaturating, Label* oolEntry)
{
    wasmTruncateToInt32(input, output, MIRType::Double,  false,isSaturating,
                         oolEntry);
}
...
MacroAssembler::wasmTruncateFloat32ToUInt32(FloatRegister input, Register output,
                                            bool isSaturating, Label* oolEntry)
{
    wasmTruncateToInt32(input, output, MIRType::Float32,  true,isSaturating,
                         oolEntry);
}
...
MacroAssembler::wasmTruncateFloat32ToInt32(FloatRegister input, Register output,
                                           bool isSaturating, Label* oolEntry)
{
    wasmTruncateToInt32(input, output, MIRType::Float32,  false,isSaturating,
                         oolEntry);
}
                                ...
                                Register64 out64)
{
    ...
    if (out64 != Register64::Invalid()) {
        if (type == Scalar::Int64) {
            ...
            load = ma_dataTransferN(IsLoad, 32,  false, memoryBase, ptr, out64.low);
                                 ...
                                 Register ptrScratch)
{
    ...
    if (type == Scalar::Int64) {
        ...
        store = ma_dataTransferN(IsStore, 32 ,  false, memoryBase, ptr,
                                 val64.low);
        append(access, store.getOffset());

        as_add(ptr, ptr, Imm8(INT64HIGH_OFFSET));

        store = ma_dataTransferN(IsStore, 32 ,  true, memoryBase, ptr,
                                 val64.high);
        append(access, store.getOffset());
    } else {
        if (value.isFloat()) {
            ...
            store = ma_dataTransferN(IsStore, 8 * byteSize , isSigned, memoryBase, ptr,
                                     val);
            append(access, store.getOffset());
        }
    }
                                         ...
                                         Register tmp3)
{
    ...
    switch (access.type()) {
      case Scalar::Float32: {
        ...
        emitUnalignedLoad(&access, false, 4, ptr, tmp, tmp2);
      ...
      case Scalar::Float64: {
        ...
        emitUnalignedLoad(&access, false, 4, ptr, tmp, tmp3, 4);
        emitUnalignedLoad(nullptr, false, 4, ptr, tmp, tmp2);
      ...
      case Scalar::Int64: {
        ...
        emitUnalignedLoad(&access, false, 4, ptr, tmp, out64.high, 4);
        emitUnalignedLoad(nullptr, false, 4, ptr, tmp, out64.low);
        break;
      }
      case Scalar::Int16:
      case Scalar::Uint16:
                                          ...
                                          Register ptrScratch, Register valOrTmp)
{
    ...
    if (val64 != Register64::Invalid()) {
        MOZ_ASSERT(byteSize == 8);
        emitUnalignedStore(&access, 4, ptr, val64.high, 4);
        emitUnalignedStore(nullptr, 4, ptr, val64.low);
    } else if (!floatValue.isInvalid()) {
        if (floatValue.isDouble()) {
            MOZ_ASSERT(byteSize == 8);
            ScratchRegisterScope scratch(asMasm());
            ma_vxfer(floatValue, scratch, valOrTmp);
            emitUnalignedStore(&access, 4, ptr, valOrTmp, 4);
            emitUnalignedStore(nullptr, 4, ptr, scratch);
        } else {
            MOZ_ASSERT(byteSize == 4);
            ma_vxfer(floatValue, valOrTmp);
            emitUnalignedStore(&access, 4, ptr, valOrTmp);
        }
    } else {
                                     ...
                                     Register tmp, Register dest, unsigned offset)
{
    ...
    while (i >= 0) {
        ma_dataTransferN(IsLoad, 8, false, ptr, Imm32(offset + i), tmp, scratch);
