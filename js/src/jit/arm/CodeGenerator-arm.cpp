...
CodeGeneratorARM::emitWasmLoad(T* lir)
{
    ...
    if (mir->type() == MIRType::Int64) {
        ...
        if (type == Scalar::Int64) {
            ...
            load = masm.ma_dataTransferN(IsLoad, 32,  false, HeapReg, ptr, output.low);
...
CodeGeneratorARM::emitWasmStore(T* lir)
{
    ...
    if (type == Scalar::Int64) {
        ...
        store = masm.ma_dataTransferN(IsStore, 32 ,  false, HeapReg, ptr, value.low);
        masm.append(mir->access(), store.getOffset(), masm.framePushed());

        masm.as_add(ptr, ptr, Imm8(INT64HIGH_OFFSET));

        store = masm.ma_dataTransferN(IsStore, 32 ,  true, HeapReg, ptr, value.high);
        masm.append(mir->access(), store.getOffset(), masm.framePushed());
    } else {
        ...
        if (value.isFloat()) {
            ...
            store = masm.ma_dataTransferN(IsStore, 8 * byteSize , isSigned, HeapReg, ptr, val);
            masm.append(mir->access(), store.getOffset(), masm.framePushed());
        }
    }
