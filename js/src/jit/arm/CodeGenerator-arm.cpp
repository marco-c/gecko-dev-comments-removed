    ...
    if (ptr->isConstant()) {
        ...
        if (isFloat)
            masm.ma_loadHeapAsmJS(ptrReg, size, mir->needsBoundsCheck(),
                                  false, ToFloatRegister(ins->output()));
        else
...
CodeGeneratorARM::emitWasmLoad(T* lir)
{
    ...
    if (mir->type() == MIRType::Int64) {
        ...
        if (type == Scalar::Int64) {
            ...
            masm.ma_dataTransferN(IsLoad, 32,  false, HeapReg, ptr, output.low);
...
CodeGeneratorARM::emitWasmUnalignedLoad(T* lir)
{
    ...
    if (IsFloatingPointType(mirType)) {
        ...
        if (byteSize == 4) {
            ...
            masm.emitUnalignedLoad( false, 4, ptr, tmp, high,  4);
        ...
        if (type == Scalar::Int64) {
            MOZ_ASSERT(byteSize == 8);
            masm.emitUnalignedLoad(isSigned, 4, ptr, tmp, output.high,  4);
        } else {
...
CodeGeneratorARM::emitWasmStore(T* lir)
{
    ...
    if (type == Scalar::Int64) {
        ...
        masm.ma_dataTransferN(IsStore, 32 ,  false, HeapReg, ptr, value.low);
        masm.ma_add(Imm32(INT64HIGH_OFFSET), ptr);
        masm.ma_dataTransferN(IsStore, 32 ,  true, HeapReg, ptr, value.high);
    } else {
        ...
        if (value.isFloat()) {
            ...
            masm.ma_dataTransferN(IsStore, 8 * byteSize , isSigned, HeapReg, ptr, val);
        }
    }
}
...
CodeGeneratorARM::emitWasmUnalignedStore(T* lir)
{
    ...
    if (byteSize > 4) {
        ...
        masm.emitUnalignedStore(4, ptr, val,  4);
     }
}
...
CodeGeneratorARM::visitAsmJSStoreHeap(LAsmJSStoreHeap* ins)
{
    ...
    if (ptr->isConstant()) {
        ...
        Register ptrReg = ToRegister(ptr);
        if (isFloat)
            masm.ma_storeHeapAsmJS(ptrReg, size, mir->needsBoundsCheck(), false,
