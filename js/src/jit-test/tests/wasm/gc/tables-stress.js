...
var mod = new WebAssembly.Module(wasmTextToBinary(
    `(module
       ...
       (func (export "run") (param $numiters i32)
         ...
         (local $iters i32)
         (local $tmp anyref)
         (set_local $last (table.grow (i32.const 1) (ref.null anyref)))
         (table.set (get_local $last) (call $item))
         (loop $iter_continue
           ...
           (block $l_break
             (loop $l_continue
               ...
               (set_local $tmp (table.get (get_local $i)))
               (if (i32.eqz (i32.rem_s (get_local $i) (i32.const 3)))
                   (set_local $tmp (call $item)))
               (table.set (get_local $i) (table.get (get_local $j)))
               (table.set (get_local $j) (get_local $tmp))
