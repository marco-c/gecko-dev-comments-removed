...
for (let toBuffer of toBufferOpts) {
    ...
    assertErrorMessage(() => wasmEval(toBuffer(moduleHeaderThen(0))), CompileError, sectionError("custom"));  
    assertErrorMessage(() => wasmEval(toBuffer(moduleHeaderThen(0, 0))), CompileError, sectionError("custom"));  
    assertErrorMessage(() => wasmEval(toBuffer(moduleHeaderThen(0, 0, 0))), CompileError, sectionError("custom"));  
    assertErrorMessage(() => wasmEval(toBuffer(moduleHeaderThen(0, 1, 1))), CompileError, sectionError("custom"));  
    assertErrorMessage(() => wasmEval(toBuffer(moduleHeaderThen(0, 1, 1, 65))), CompileError, sectionError("custom"));  
    assertErrorMessage(() => wasmEval(toBuffer(moduleHeaderThen(0, 1, 0, 0))), CompileError, sectionError("custom"));  
    wasmEval(toBuffer(moduleHeaderThen(0, 1, 0)));  
    wasmEval(toBuffer(moduleHeaderThen(0, 1, 0,  0, 1, 0)));  
    wasmEval(toBuffer(moduleHeaderThen(0, 2, 1, 65)));  
    ...
    assertEq(arr.length, 1);
    assertEq(arr[0].byteLength, nameSec.body.length - 5 );
    ...
    for (let prefix of [ThreadPrefix, MiscPrefix, SimdPrefix, MozPrefix]) {
        ...
        let binary = moduleWithSections([v2vSigSection, declSection([0]), bodySection([funcBody({locals:[], body:[prefix]}, false)])], toBuffer);
