...
assert.sameValue(DBL_MAX, Number.MAX_VALUE); 
...
assert.sameValue(Math.fround(FLT_MAX + Math.pow(2, Math.pow(2, 8 - 1) - 1 - 23 - 2)), FLT_MAX); 
assert.sameValue(Math.fround(FLT_MAX + Math.pow(2, Math.pow(2, 8 - 1) - 1 - 23 - 1)), Infinity); 
...
assert.sameValue(DBL_MIN, Number.MIN_VALUE); 
...
assert.sameValue(Math.fround(FLT_MIN / 2), 0); 
assert.sameValue(Math.fround(FLT_MIN / 2 + Math.pow(2, -202)), FLT_MIN); 
