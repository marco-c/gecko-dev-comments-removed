












const calendar = "ethioaa";
const options = { overflow: "reject" };




const common1230 = Temporal.ZonedDateTime.from({ year: 7514, monthCode: "M12", day: 30, hour: 12, minute: 34, timeZone: "UTC", calendar }, options);
const leap0130 = Temporal.ZonedDateTime.from({ year: 7515, monthCode: "M01", day: 30, hour: 12, minute: 34, timeZone: "UTC", calendar }, options);
const leap1230 = Temporal.ZonedDateTime.from({ year: 7515, monthCode: "M12", day: 30, hour: 12, minute: 34, timeZone: "UTC", calendar }, options);
const common0130 = Temporal.ZonedDateTime.from({ year: 7516, monthCode: "M01", day: 30, hour: 12, minute: 34, timeZone: "UTC", calendar }, options);

const months1 = new Temporal.Duration(0, 1);
const months1n = new Temporal.Duration(0, -1);

TemporalHelpers.assertPlainDateTime(
  common1230.add(months1).toPlainDateTime(),
  7514, 13, "M13", 5, 12, 34, 0, 0, 0, 0, "Adding 1 month to last day of Nahase constrains to day 5 of common-year epagomenal month",
  "aa", 7514);
assert.throws(RangeError, function () {
  common1230.add(months1, options);
}, "Adding 1 month to last day of Nahase rejects in common year");

TemporalHelpers.assertPlainDateTime(
  leap1230.add(months1).toPlainDateTime(),
  7515, 13, "M13", 6, 12, 34, 0, 0, 0, 0, "Adding 1 month to last day of Nahase constrains to day 6 of leap-year epagomenal month",
  "aa", 7515);
assert.throws(RangeError, function () {
  leap1230.add(months1, options);
}, "Adding 1 month to last day of Nahase rejects in leap year");

TemporalHelpers.assertPlainDateTime(
  leap0130.add(months1n).toPlainDateTime(),
  7514, 13, "M13", 5, 12, 34, 0, 0, 0, 0, "Subtracting 1 month from last day of Maskaram constrains to day 5 of common-year epagomenal month",
  "aa", 7514);
assert.throws(RangeError, function () {
  leap0130.add(months1n, options);
}, "Subtracting 1 month from last day of Maskaram rejects in common year");

TemporalHelpers.assertPlainDateTime(
  common0130.add(months1n).toPlainDateTime(),
  7515, 13, "M13", 6, 12, 34, 0, 0, 0, 0, "Subtracting 1 month from last day of Maskaram constrains to day 6 of leap-year epagomenal month",
  "aa", 7515);
assert.throws(RangeError, function () {
  common0130.add(months1n, options);
}, "Subtracting 1 month from last day of Maskaram rejects in leap year");

reportCompare(0, 0);
