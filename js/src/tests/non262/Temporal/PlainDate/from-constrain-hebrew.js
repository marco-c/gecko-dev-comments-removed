...
for (let {month: [month], monthCode, days} of months) {
  ...
  assertEq(startOfMonth.year, year);
  assertEq(startOfMonth.month, month);
  assertEq(startOfMonth.monthCode, monthCode);
  assertEq(startOfMonth.day, 1);
  ...
  assertEq(endOfMonth.year, year);
  assertEq(endOfMonth.month, month);
  assertEq(endOfMonth.monthCode, monthCode);
  assertEq(endOfMonth.day, days[0]);
}
...

{
  ...
  for (let year = 0; year < daysPerMonth.Cheshvan.length; ++year) {
    ...
    assertEq(endOfCheshvan.day, daysPerMonth.Cheshvan[year]);
    ...
    assertEq(endOfKislev.day, daysPerMonth.Kislev[year]);
  }
}
