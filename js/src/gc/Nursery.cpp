...
void* js::Nursery::allocateZeroedBuffer(
    Zone* zone, size_t nbytes, arena_id_t arena ) {
  MOZ_ASSERT(nbytes > 0);
...
void* js::Nursery::allocateZeroedBuffer(
    JSObject* obj, size_t nbytes, arena_id_t arena ) {
  MOZ_ASSERT(obj);
  MOZ_ASSERT(nbytes > 0);
...
void js::Nursery::printCollectionProfile(JS::GCReason reason,
  ...
  fprintf(
      stderr, "MinorGC: %6zu %12p %10.6f %-20.20s %4.1f%% %5zu %5zu %6" PRIu32,
...
void js::Nursery::printTotalProfileTimes() {
  if (enableProfiling_) {
    fprintf(stderr,
            "MinorGC: %6zu %12p TOTALS: %7" PRIu64
