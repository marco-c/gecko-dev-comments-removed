        ...
        #define INVALID(report, arg, n2)                                \
                ...
                (v == 0xF4 && ((uint8_t)src[i + 1] & 0xF0) != 0x80))    
            {
                if (std::is_same<InputCharsT, UTF8Chars>::value) {
                    ...
                    if (v == 0xED && ((uint8_t)src[i + 1] & 0xE0) != 0xA0) { 
                        INVALID(ReportInvalidCharacter, i, 1);
                    }
                }
            }
...
UTF8EqualsChars(const JS::UTF8Chars utfChars, const CharT* chars)
{
    ...
    InflateUTF8ToUTF16<OnUTF8Error::Crash>( nullptr, utfChars, checkEqual);
...
{
    CopyAndInflateUTF8IntoBuffer<OnUTF8Error::Crash>( nullptr, src, dst, dstLen,
                                                     encoding == JS::SmallestEncoding::ASCII);
}
...
JS::ConstUTF8CharsZ::validate(size_t aLength)
{
    ...
    InflateUTF8ToUTF16<OnUTF8Error::Crash>( nullptr, chars, nop);
}
#endif
