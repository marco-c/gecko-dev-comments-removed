...
EqualStringsPure(JSString* s1, JSString* s2)
{
    ...
    if (s1->isLinear()) {
        ...
        if (!s1->asRope().copyChars<Char1>( nullptr, ownedChars1))
            MOZ_CRASH("oom");
        c1 = ownedChars1;
    }
    ...
    if (s2->isLinear()) {
        ...
        if (!s2->asRope().copyChars<Char2>( nullptr, ownedChars2))
            MOZ_CRASH("oom");
        c2 = ownedChars2;
    }
...
StoreStringChars(char* buffer, size_t bufferSize, JSString* str)
{
    ...
    if (str->isLinear()) {
        ...
        if (!str->asRope().copyChars<CharT>( nullptr, ownedChars))
            MOZ_CRASH("oom");
        chars = ownedChars;
    }
               ...
               ServoSizes* sizes)
{
    ...
    if (!CollectRuntimeStatsHelper(cx, &rtStats, opv,  false,
                                   StatsCellCallback<CoarseGrained>))
        return false;
