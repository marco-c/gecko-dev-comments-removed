...
#undef COMMON_NAME_INFO
  ...
  permanentAtoms_ =
      js_new<FrozenAtomSet>(atomSet.release());  
  if (!permanentAtoms_) {
    return false;
  }
...
template <typename CharT>
static MOZ_ALWAYS_INLINE JSAtom* AtomizeAndCopyCharsNonStaticFromLookup(
...
static MOZ_ALWAYS_INLINE JSAtom* AtomizeAndCopyChars(
  ...
  return AtomizeAndCopyCharsNonStaticFromLookup(cx, chars, length, lookup,
                                                indexValue);
}
