    ...
    while (true) {
        ...
        for (size_t i = 0; i < finished.length(); i++) {
            ...
            if (task->runtimeMatches(rt)) {
                ...
                HelperThreadState().finishParseTask( nullptr, rt, task->kind,
                                                    task);
            }
        }
        if (!found)
...
void
GlobalHelperThreadState::wait(CondVar which, TimeDuration timeout )
{
    MOZ_ASSERT(isLocked());
#ifdef DEBUG
    lockOwner = nullptr;
...
{
    CancelOffThreadIonCompile(nullptr, nullptr,  false);
