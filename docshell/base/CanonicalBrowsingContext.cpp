...
void CanonicalBrowsingContext::ReactivateDocuments(
  ...
  if (IsInProcess()) {
    BrowsingContext::ReactivateDocuments(
        Some(mActiveEntry->Info()), topNewSHIs,
        ToMaybeRef(aPreviousEntryForActivation).map([](auto& entry) {
          return entry.Info();
        }));
