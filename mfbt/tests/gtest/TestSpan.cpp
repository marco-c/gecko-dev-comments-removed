...
#ifdef CONFIRM_COMPILATION_ERRORS
    Span<char16_t> scca(arr);  

    Span<const char16_t> scccea;
    scccea = arr;  

    Span<const char16_t> scccl(u"literal");  

    Span<const char16_t>* sccel;
    *sccel = u"literal";  

    cs = MakeSpan(u"literal");  
#endif
  }
}
...
SPAN_TEST(as_const_static) {
  {
    ...
    constexpr auto span = Span{constArr};  
