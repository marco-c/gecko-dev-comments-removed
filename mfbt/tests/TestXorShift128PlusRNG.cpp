...
TestDumbSequence()
{
  mozilla::XorShift128PlusRNG rng(1, 4);
  ...
  MOZ_RELEASE_ASSERT(rng.nextDouble() == ldexp(0x1400003105049, -53));
  MOZ_RELEASE_ASSERT(rng.nextDouble() == ldexp(0x2000802e49146, -53));
  MOZ_RELEASE_ASSERT(rng.nextDouble() == ldexp(0x248300468544d, -53));
}
...
TestPopulation()
{
  mozilla::XorShift128PlusRNG rng(698079309544035222ULL, 6012389156611637584ULL);
...
TestSetState()
{
  ...
  mozilla::XorShift128PlusRNG rng(seed[0], seed[1]);
...
{
  mozilla::XorShift128PlusRNG rng(0xa207aaede6859736, 0xaca6ca5060804791);
