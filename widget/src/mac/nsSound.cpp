...
{
  SInt32   nameLen = strlen(inString) & 0xFF;    
...
nsSystemSoundRequest::PlaySound()
{
  ...
  OSErr err = ::SndPlay(mSndChannel, (SndListHandle)mSoundHandle, true );
  if (err != noErr) {
     Cleanup();
     return NS_ERROR_FAILURE;
  }
  ...
  err = ::SndDoCommand(mSndChannel, &theCmd, false);   
  if (err != noErr) {
    Cleanup();
    return NS_ERROR_FAILURE;
  }
  
  const PRInt32   kSoundTimerInterval = 250;      
...
{
  OSType    fileFormat = kQTFileTypeMovie;    
...
nsMovieSoundRequest::PlaySound()
{
  ...
  {  
    ...
    const PRInt32   kMovieTimerInterval = 250;      
...
PRBool
nsMovieSoundRequest::TaskOneMovie(Movie inMovie)    
{
