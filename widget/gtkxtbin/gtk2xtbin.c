...
static gboolean xt_event_polling_timer_callback(gpointer user_data) {
  ...
  while (eventsToProcess-- && XtAppPending(ac)) XtAppProcessEvent(ac, XtIMAll);
  return TRUE;
}

GType gtk_xtbin_get_type(void) {
  ...
  if (!xtbin_type) {
    ...
    xtbin_type =
        g_type_register_static(GTK_TYPE_SOCKET, "GtkXtBin", &xtbin_info, 0);
  }
  return xtbin_type;
}
...
static void gtk_xtbin_realize(GtkWidget *widget) {
  ...
  xt_client_create(&(xtbin->xtclient), gtk_socket_get_id(GTK_SOCKET(xtbin)), h,
                   w);
...
GtkWidget *gtk_xtbin_new(GdkWindow *parent_window, String *f) {
  ...
  xt_client_init(&(xtbin->xtclient), GDK_VISUAL_XVISUAL(gdk_rgb_get_visual()),
  ...
  if (user_data) gtk_container_add(GTK_CONTAINER(user_data), GTK_WIDGET(xtbin));
...
void xt_client_init(XtClient *xtclient, Visual *xtvisual, Colormap xtcolormap,
  ...
  if (!xt_is_initialized) {
    ...
    if (fallback) XtAppSetFallbackResources(app_context, fallback);

    xtdisplay = XtOpenDisplay(app_context, gdk_get_display(), NULL, "Wrapper",
                              NULL, 0, &mArgc, mArgv);
    if (xtdisplay) xt_is_initialized = TRUE;
  }
...
void xt_client_create(XtClient *xtclient, Window embedderid, int height,
...
#endif
  top_widget =
      XtAppCreateShell("drawingArea", "Wrapper", applicationShellWidgetClass,
                       xtclient->xtdisplay, NULL, 0);
  ...
  child_widget =
      XtVaCreateWidget("form", compositeWidgetClass, top_widget, NULL);
  ...
  XtSetArg(args[n], XtNwidth, width);
  n++;
  XtSetArg(args[n], XtNvisual, xtclient->xtvisual);
  n++;
  XtSetArg(args[n], XtNdepth, xtclient->xtdepth);
  n++;
  XtSetArg(args[n], XtNcolormap, xtclient->xtcolormap);
  n++;
  ...
  XtRegisterDrawable(xtclient->xtdisplay, embedderid, top_widget);
#else
  _XtRegisterWindow(embedderid, top_widget);
#endif
  XtRealizeWidget(child_widget);

  
  XSelectInput(xtclient->xtdisplay, embedderid, XtBuildEventMask(top_widget));
  ...
  XtAddEventHandler(child_widget, StructureNotifyMask | KeyPressMask, TRUE,
                    (XtEventHandler)xt_client_event_handler, xtclient);
  XtAddEventHandler(child_widget, SubstructureNotifyMask | ButtonReleaseMask,
                    FALSE, (XtEventHandler)xt_client_focus_listener, xtclient);
  XSync(xtclient->xtdisplay, FALSE);
}
...
#else
  _XtUnregisterWindow(xtclient->top_widget->core.window, xtclient->top_widget);
#endif
...
static void xt_client_handle_xembed_message(Widget w, XtPointer client_data,
  ...
  switch (event->xclient.data.l[1]) {
    ...
    case XEMBED_FOCUS_OUT: {
      ...
      XSendEvent(XtDisplay(xtplug->child_widget), xevent.xfocus.window, False,
                 NoEventMask, &xevent);
  ...
  switch (event->type) {
    case ClientMessage:
      ...
      if (event->xclient.message_type ==
          XInternAtom(XtDisplay(xtplug->child_widget), "_XEMBED", False)) {
...
#endif
      send_xembed_message(xtclient, XEMBED_REQUEST_FOCUS, 0, 0, 0, 0);
      break;
    default:
      break;
  } 
