...
bool Key::GetChildName(uint32_t aIndex, nsAString& aResult) const {
  ...
  LRESULT rv = RegEnumKeyExW(mKey, aIndex, nameBuf, &nameLen, nullptr, nullptr,
                             nullptr, &lastWritten);
...
bool Key::RemoveValue(const nsString& aName) const {
  MOZ_ASSERT(mKey);
  return LSTATUS_SUCCEEDED(RegDeleteValueW(mKey, aName.get()));
}
...
bool Key::GetValueAsString(const nsString& aName, nsString& aResult,
  ...
  {
    auto handleOrError =
        aResult.BulkWrite(resultLen, 0,  false);
...
KeyWatcher::KeyWatcher(Key&& aKey,
  ...
  mEvent = CreateEvent(nullptr,  FALSE,
                        FALSE, nullptr);
