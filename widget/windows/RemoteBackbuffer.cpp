 ...
 public:
  ...
  bool Initialize(int32_t aWidth, int32_t aHeight) {
    ...
    mFileMapping = ::CreateFileMappingW(
        ...
        0 , bufferSize, nullptr );
    if (!mFileMapping) {
      return false;
    }

    void* mappedFilePtr =
        ::MapViewOfFile(mFileMapping, FILE_MAP_ALL_ACCESS, 0 ,
                        0 , 0 );
    if (!mappedFilePtr) {
      return false;
    }
  ...
  HBITMAP CreateDIBSection() {
    ...
    void* dummy = nullptr;
    return ::CreateDIBSection(nullptr , &bitmapInfo,
 ...
 public:
  ...
  bool PresentToWindow(HWND aWindowHandle, nsTransparencyMode aTransparencyMode,
    ...
    for (auto& ipcDirtyRect : aDirtyRects) {
      ...
      if (!::BitBlt(windowDC, bltRect.x , bltRect.y ,
                    bltRect.width, bltRect.height, mDeviceContext,
                    bltRect.x , bltRect.y , SRCCOPY)) {
        result = false;
        break;
      }
    }
...
bool Provider::Initialize(HWND aWindowHandle, DWORD aTargetProcessId,
  ...
  mTargetProcess = ::OpenProcess(PROCESS_DUP_HANDLE, FALSE ,
                                 aTargetProcessId);
  if (!mTargetProcess) {
    return false;
  }
  ...
  mFileMapping = ::CreateFileMappingW(
      ...
      static_cast<DWORD>(sizeof(SharedData)), nullptr );
  if (!mFileMapping) {
    return false;
  }
