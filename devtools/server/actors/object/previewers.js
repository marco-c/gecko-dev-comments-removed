...
const previewers = {
  ...
  Set: [
    function(objectActor, grip) {
      ...
      const items = (grip.preview.items = []);
      for (const item of PropertyIterators.enumSetEntries(objectActor,  true)) {
        items.push(item);
        if (items.length == OBJECT_PREVIEW_MAX_ITEMS) {
          break;
        }
  ...
  WeakSet: [
    function(objectActor, grip) {
      const enumEntries = PropertyIterators.enumWeakSetEntries(objectActor,  true);
  ...
  Map: [
    function(objectActor, grip) {
      ...
      for (const entry of PropertyIterators.enumMapEntries(objectActor,  true)) {
        entries.push(entry);
  ...
  WeakMap: [
    function(objectActor, grip) {
      const enumEntries = PropertyIterators.enumWeakMapEntries(objectActor,  true);
