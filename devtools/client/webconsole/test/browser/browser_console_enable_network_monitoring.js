...
add_task(async function testEnableNetworkMonitoringInBrowserConsole() {
  ...
  const isEnabled = waitFor(() => {
    const el = getConsoleSettingElement(hud, enableNetworkMonitoringSelector);
    return el.getAttribute("aria-checked") === "true";
  });
  await toggleConsoleSetting(hud, enableNetworkMonitoringSelector);
  await isEnabled;
