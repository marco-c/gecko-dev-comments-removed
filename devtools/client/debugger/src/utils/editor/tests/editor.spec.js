...
describe("toEditorLine", () => {
  it("returns an editor line", () => {
    ...
    const line = 30;
    const expectedLine = features.codemirrorNext ? line : line - 1;
    expect(toEditorLine(testId, line)).toEqual(expectedLine);
  });
});
...
describe("toEditorPosition", () => {
  it("returns an editor position", () => {
    const line = 100;
    const column = 25;
    const loc = { source: { id: "source" }, line, column };
    expect(toEditorPosition(loc)).toEqual({
      line: features.codemirrorNext ? line : line - 1,
      column,
    });
  });
});
...
describe("toSourceLine", () => {
  it("returns a source line", () => {
    ...
    const line = 30;
    const expectedLine = features.codemirrorNext ? line : line + 1;
    expect(toSourceLine(testId, line)).toEqual(expectedLine);
  });
});
