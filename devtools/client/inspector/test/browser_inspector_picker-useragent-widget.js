...
const TEST_URI = `data:text/html;charset=utf-8,
  <!DOCTYPE html>
  <input type="date" value="2018-10-25" max="2018-10-26">`;
...
async function runUserAgentWidgetPickerTest({ enableAnonymousContent }) {
  ...
  info("Use the node picker inside the <input> element");
  await startPicker(toolbox);
  const onPickerStopped = toolbox.nodePicker.once("picker-stopped");
  await pickElement(inspector, "input", 5, 5);
  ...
  if (enableAnonymousContent) {
    ...
    ok(
      selectedNode.tagName.toLowerCase() !== "input",
      "The selected node is not the <input>"
    );
    ok(shadowHost, "The selected node is in a shadow root");
    is(shadowHost.tagName.toLowerCase(), "input", "The shadowHost is <input>");
  } else {
    is(
      selectedNode.tagName.toLowerCase(),
      "input",
      "The selected node is the <input>"
    );
  }
}
