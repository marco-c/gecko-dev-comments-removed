...
add_task(async function test_adjustWindowGeometry_floatValues() {
  ...
  try {
    ...
    if (!isWayland) {
      is(resizedRect.x, 400, "Decimal x is floored");
      is(resizedRect.y, 200, "Decimal y is floored");
    }
    is(resizedRect.width, 600, "Decimal width is floored");
    is(resizedRect.height, 300, "Decimal height is floored");
  } finally {
    await BrowserTestUtils.closeWindow(testWin);
  }
});
