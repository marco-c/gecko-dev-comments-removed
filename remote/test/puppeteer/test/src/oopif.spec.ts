  ...
  it('clickablePoint, boundingBox, boxModel should work for elements inside OOPIFs', async () => {
    ...
    for (const quad of [
      ...
      for (const part of quad) {
        expect(part.x).toBeGreaterThan(150); 
        expect(part.y).toBeGreaterThan(150); 
      }
    }
    const resultBoundingBox = (await button.boundingBox())!;
    expect(resultBoundingBox.x).toBeGreaterThan(150); 
    expect(resultBoundingBox.y).toBeGreaterThan(150); 
  });
