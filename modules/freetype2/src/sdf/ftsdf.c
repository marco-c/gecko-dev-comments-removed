...
#define VEC_26D6_DOT( p, q )  ( MUL_26D6( p.x, q.x ) + \
  ...
  FT_DEFINE_OUTLINE_FUNCS(
    ...
    (FT_Outline_MoveTo_Func) sdf_move_to,   
    (FT_Outline_LineTo_Func) sdf_line_to,   
    (FT_Outline_ConicTo_Func)sdf_conic_to,  
    (FT_Outline_CubicTo_Func)sdf_cubic_to,  
                          ...
                          SDF_Signed_Distance*  out )
  {
    ...
    FT_16D16  roots[3] = { 0, 0, 0 };    
    FT_16D16  min_factor;                
    FT_16D16  cross;                     
    FT_16D16  min      = FT_INT_MAX;     
                ...
                const FT_Bitmap*  bitmap )
  {
    ...
    for ( y = 0; y < rows; y++ )
    {
      ...
      for ( x = 0; x < width; x++ )
      {
        ...
        value  = min_dist.distance & 0x0000FFFF; 
        value *= min_dist.sign;
                              ...
                              const FT_Bitmap*  bitmap )
  {
    ...
    for ( j = 0; j < rows; j++ )
    {
      for ( i = 0; i < width; i++ )
      {
        ...
        FT_SDFFormat  val_c  = 0;         
        FT_SDFFormat  val_ac = UCHAR_MAX; 
        ...
        for ( c = 0; c < num_contours; c++ )
        {
          ...
          if ( orientations[c] == SDF_ORIENTATION_CW )
            val_c = FT_MAX( val_c, temp );   
          else
            val_ac = FT_MIN( val_ac, temp ); 
        }
  ...
  FT_DEFINE_RASTER_FUNCS(
    ...
    (FT_Raster_New_Func)     sdf_raster_new,       
    (FT_Raster_Reset_Func)   sdf_raster_reset,     
    (FT_Raster_Set_Mode_Func)sdf_raster_set_mode,  
    (FT_Raster_Render_Func)  sdf_raster_render,    
    (FT_Raster_Done_Func)    sdf_raster_done       
  )
