...
NPError NP_LOADDS
NPP_DestroyStream(NPP instance, NPStream* , NPError )
{
  if (!instance)
    return NPERR_INVALID_INSTANCE_ERROR;
...
void NP_LOADDS
NPP_StreamAsFile(NPP , NPStream *, const char* )
{
}
...
void CPlugin::DetermineURL(int16_t argc, char* argn[], char* argv[])
{
  ...
  if (fType)
    additionalLength += (strlen(fType) + 1); 
  ...
  for (i = 0; i < argc; i++) {
    if ((strcasecmp(argn[i], "PLUGINSPAGE") == 0) || (strcasecmp(argn[i], "CODEBASE") == 0)) {
      url = argv[i];
      fPageURL = (char*)NPN_MemAlloc(strlen(url) + 1 + additionalLength);  
      if (fPageURL) {
...
{
  CFStringRef pluginKey = CFSTR("DefaultPluginSeenTypes"); 
  ...
  if (!prefsList) {
    ...
    if (::CFGetTypeID(prefsList) == ::CFArrayGetTypeID()) {
      ...
      for (CFIndex i = 0; i < count; i ++) {
        CFStringRef item = (CFStringRef)::CFArrayGetValueAtIndex(prefsList, i); 
