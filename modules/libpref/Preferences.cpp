...
nsPrefBranch::Observe(nsISupports* aSubject, const char* aTopic,
 ...
 void nsPrefBranch::NotifyObserver(const char* aNewPref,
                                               void* aData) {
...
static void AssertNotAlreadyCached(const char* aPrefType,
 ...
 void Preferences::AddSizeOfIncludingThis(
...
PreferenceServiceReporter::CollectReports(
  ...
  for (uint32_t i = 0; i < suspectPreferences.Length(); i++) {
    ...
    aHandleReport->Callback(
         EmptyCString(), suspectPath, KIND_OTHER, UNITS_COUNT,
        totalReferentCount,
...
NS_IMPL_ISUPPORTS(Preferences, nsIPrefService, nsIObserver, nsIPrefBranch,
 ...
 void Preferences::DeserializePreferences(char* aStr,
                                                      size_t aPrefsLen) {
  MOZ_ASSERT(!XRE_IsParentProcess());
 ...
 void Preferences::InitSnapshot(const FileDescriptor& aHandle,
                                            size_t aSize) {
  MOZ_ASSERT(!XRE_IsParentProcess());
...
float MOZ_MAYBE_UNUSED GetPref<float>(const char* aName, float aDefaultValue) {
 ...
 Result<Ok, const char*> Preferences::InitInitialObjects(
    bool aIsStartup) {
...
#  endif
 ...
 nsresult Preferences::GetInt(const char* aPrefName,
                                          int32_t* aResult,
                                          PrefValueKind aKind) {
  MOZ_ASSERT(aResult);
  NS_ENSURE_TRUE(InitStaticMembers(), NS_ERROR_NOT_AVAILABLE);
 ...
 nsresult Preferences::GetFloat(const char* aPrefName,
                                            float* aResult,
                                            PrefValueKind aKind) {
  MOZ_ASSERT(aResult);
 ...
 nsresult Preferences::GetCString(const char* aPrefName,
                                              nsACString& aResult,
                                              PrefValueKind aKind) {
  NS_ENSURE_TRUE(InitStaticMembers(), NS_ERROR_NOT_AVAILABLE);
 ...
 nsresult Preferences::GetString(const char* aPrefName,
                                             nsAString& aResult,
                                             PrefValueKind aKind) {
  nsAutoCString result;
  nsresult rv = Preferences::GetCString(aPrefName, result, aKind);
 ...
 nsresult Preferences::GetComplex(const char* aPrefName,
                                              const nsIID& aType,
                                              void** aResult,
                                              PrefValueKind aKind) {
  NS_ENSURE_TRUE(InitStaticMembers(), NS_ERROR_NOT_AVAILABLE);
 ...
 nsresult Preferences::SetComplex(const char* aPrefName,
                                              const nsIID& aType,
                                              nsISupports* aValue,
                                              PrefValueKind aKind) {
  NS_ENSURE_TRUE(InitStaticMembers(), NS_ERROR_NOT_AVAILABLE);
...
template <typename T>
 nsresult Preferences::RegisterCallbackImpl(
 ...
 nsresult Preferences::RegisterCallbackAndCall(
 ...
 nsresult Preferences::RegisterCallbacksAndCall(
...
template <typename T>
 nsresult Preferences::UnregisterCallbackImpl(
 ...
 nsresult Preferences::UnregisterCallback(
 ...
 nsresult Preferences::UnregisterCallbacks(
...
static void BoolVarChanged(const char* aPref, void* aClosure) {
 ...
 nsresult Preferences::AddBoolVarCache(bool* aCache,
                                                   const nsACString& aPref,
                                                   bool aDefault,
...
template <MemoryOrdering Order>
 nsresult Preferences::AddAtomicBoolVarCache(
...
static void IntVarChanged(const char* aPref, void* aClosure) {
 ...
 nsresult Preferences::AddIntVarCache(int32_t* aCache,
                                                  const nsACString& aPref,
                                                  int32_t aDefault,
                                                  bool aSkipAssignment) {
...
template <MemoryOrdering Order>
 nsresult Preferences::AddAtomicIntVarCache(
...
static void UintVarChanged(const char* aPref, void* aClosure) {
 ...
 nsresult Preferences::AddUintVarCache(uint32_t* aCache,
                                                   const nsACString& aPref,
                                                   uint32_t aDefault,
                                                   bool aSkipAssignment) {
...
template <MemoryOrdering Order>
 nsresult Preferences::AddAtomicUintVarCache(
...
static void FloatVarChanged(const char* aPref, void* aClosure) {
 ...
 nsresult Preferences::AddFloatVarCache(float* aCache,
                                                    const nsACString& aPref,
                                                    float aDefault,
                                                    bool aSkipAssignment) {
