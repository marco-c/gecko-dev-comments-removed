        ...
        S: Serializer,
    {
        ...
        match self.att_stmt {
            ...
            AttestationStatement::AndroidKey(ref v) => {
                map.serialize_entry(&"fmt", &"android-key")?; 
                map.serialize_entry(&"attStmt", v)?; 
            }
            AttestationStatement::AndroidSafetyNet(ref v) => {
                map.serialize_entry(&"fmt", &"android-safetynet")?; 
                map.serialize_entry(&"attStmt", v)?; 
            }
            AttestationStatement::Apple(ref v) => {
                map.serialize_entry(&"fmt", &"apple")?; 
                map.serialize_entry(&"attStmt", v)?; 
            }
            AttestationStatement::Tpm(ref v) => {
                map.serialize_entry(&"fmt", &"tpm")?; 
                map.serialize_entry(&"attStmt", v)?; 
            }
        }
        map.serialize_entry(&"authData", &self.auth_data)?; 
        map.end()
    }
}
...
pub mod test {
    ...
    const SAMPLE_ATTESTATION_OBJ_PACKED: [u8; 677] = [
        ...
        0x81, 
        0x59, 0x01, 0x97, 
        0x30, 0x82, 0x01, 0x93, 0x30, 0x82, 0x01, 
        ...
        0xd3, 0x43, 0xcb, 0x2f, 0x11, 0x3d, 0xa2, 0x37, 0x23, 0xf3, 0x68, 
