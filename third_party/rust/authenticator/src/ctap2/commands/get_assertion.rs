    ...
    pub fn calculate(&mut self, secret: &ECDHSecret) -> Result<(), AuthenticatorError> {
        ...
        let salt_enc = match &self.salt2 {
            Some(salt2) => {
                ...
                let salts = [&self.salt1[..32], &salt2[..32]].concat(); 
...
impl RequestCtap1 for GetAssertion {
        ...
        Dev: io::Read + io::Write + fmt::Debug + FidoDevice,
    {
        ...
        let mut auth_data =
            Vec::with_capacity(2 * PARAMETER_SIZE + 1  + key_handle.len());
...
pub mod test {
    ...
    fn test_get_assertion_ctap2() {
        ...
        let mut msg = cid.to_vec();
        msg.extend(&[HIDCmd::Cbor.into(), 0x1, 0x5c]); 
    ...
    fn fill_device_ctap1(device: &mut Device, cid: [u8; 4], flags: u8, answer_status: [u8; 2]) {
        ...
        let mut msg = cid.to_vec();
        msg.extend(&[HIDCmd::Msg.into(), 0x00, 0x8A]); 
        msg.extend(&[0x00, 0x2]); 
        msg.extend(&[flags]);
        ...
        let mut msg = cid.to_vec();
        msg.extend(&[HIDCmd::Msg.into(), 0x0, 0x4D]); 
    ...
    const CLIENT_DATA_VEC: [u8; 140] = [
        ...
        0x3a, 
        0x22, 0x41, 0x41, 0x45, 0x43, 0x41, 0x77, 0x22, 
        0x2c, 0x22, 0x6f, 0x72, 0x69, 0x67, 0x69, 0x6e, 0x22, 0x3a, 
        ...
        0x22, 0x70, 0x72, 0x65, 0x73, 0x65, 0x6e, 0x74, 0x22, 
        0x2c, 0x22, 0x69, 0x64, 0x22, 0x3a, 
        0x22, 0x41, 0x41, 0x45, 0x43, 0x41, 0x77, 0x22, 
        0x7d, 
        0x7d, 
    ];
