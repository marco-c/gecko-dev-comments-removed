...
pub mod tests {
    ...
    fn test_read_large_blob_array() {
        ...
        let mut msg = cid.to_vec();
        msg.extend(vec![HIDCmd::Cbor.into(), 0x00, cmd.len() as u8 + 1]); 
        msg.extend(vec![0x0C]); 
        msg.extend(cmd); 
        device.add_write(&msg, 0);
    ...
    fn test_add_large_blob_element_multi_write() {
        ...
        for (ii, chunk) in LARGE_BLOB_ARRAY_LONGER.chunks(100).enumerate() {
            ...
            cmd.push(0x02); 
            if ii == 6 {
                cmd.extend([0x58, 0x20]); 
            } else {
                cmd.extend([0x58, 0x64]); 
            }
          ...
          0x59, 0x01, 0xdf, 
            0x83,             
              ...
              0xa3,          
                ...
                0x03,       
                0x18, 0x22,    
           0x15, 0xee, 0x84, 0xa0, 0xce, 0x5d, 0xa7, 0xd6, 0x6d, 0x3e, 0xb6, 0xf2, 0xc1, 0x40, 0x28, 0x65 
    ];
    ...
    pub const LARGE_BLOB_ARRAY_LONGER: [u8; 632] = [
            ...
            0x84,             
              ...
              0xa3,          
                ...
                0x03,       
                0x18, 0x22, 
            0xb9, 0xd5, 0x4e, 0x96, 0xcf, 0x6e, 0xd8, 0xf6, 0xb4, 0x4c, 0x2e, 0xdc, 0xec, 0x76, 0x67, 0x0, 
    ];
}
