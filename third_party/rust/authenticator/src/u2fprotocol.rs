...
pub(crate) mod tests {
    ...
    fn test_init_device() {
        ...
        let mut msg = CID_BROADCAST.to_vec();
        msg.extend(vec![HIDCmd::Init.into(), 0x00, 0x11]); 
        msg.extend_from_slice(&nonce);
        msg.extend_from_slice(&cid); 
        msg.extend(vec![0x02, 0x04, 0x01, 0x08, 0x01]); 
    ...
    fn test_get_version() {
        ...
        msg.extend([HIDCmd::Msg.into(), 0x0, 0x7]); 
        msg.extend([0x0, 0x3, 0x0, 0x0, 0x0, 0x0, 0x0]);
        device.add_write(&msg, 0);

        
        let mut msg = cid.to_vec();
        msg.extend([HIDCmd::Msg.into(), 0x0, 0x08]); 
        msg.extend([0x55, 0x32, 0x46, 0x5f, 0x56, 0x32]); 
    ...
    fn test_sendrecv_multiple() {
        ...
        let mut msg = cid.to_vec();
        msg.extend(vec![HIDCmd::Ping.into(), 0x00, 0xe4]); 
    ...
    fn test_sendapdu() {
        ...
        msg.extend(vec![HIDCmd::Msg.into(), 0x00, 0x0e]); 
