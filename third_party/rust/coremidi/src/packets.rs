...
const PACKET_LIST_HEADER_SIZE: usize = 4; 
const PACKET_HEADER_SIZE: usize = 8 +      
                                  2; 

const INLINE_PACKET_BUFFER_SIZE: usize = 28; 
...
impl PacketBufferStorage {
    ...
    unsafe fn assign_packet(&mut self, packet_offset: usize, time: MIDITimeStamp, data: &[u8]) {
        assert!(data.len() <= MAX_PACKET_DATA_LENGTH, "packet data too long"); 
