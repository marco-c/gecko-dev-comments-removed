    ...
    match op {
        ...
        Opcode::Ishl | Opcode::Ushr | Opcode::Sshr | Opcode::Rotl | Opcode::Rotr => {
            ...
            if !dst_ty.is_vector() && dst_ty.bits() <= 64 {
                ...
                match op {
                    ...
                    Opcode::Ushr => {
                        emit_shr_i128(ctx, src, dst, amt_src,  false);
                    }
                    Opcode::Sshr => {
                        emit_shr_i128(ctx, src, dst, amt_src,  true);
                    }
                    Opcode::Rotl => {
                    ...
                    Opcode::Rotr => {
                        ...
                        let tmp = ctx.alloc_tmp(types::I128);
                        emit_shr_i128(ctx, src, tmp, amt_src,  false);
                        let inv_amt = ctx.alloc_tmp(types::I64).only_reg().unwrap();
                        ctx.emit(Inst::imm(OperandSize::Size64, 128, inv_amt));
                        ctx.emit(Inst::alu_rmi_r(
                            OperandSize::Size64,
        ...
        | Opcode::Ireduce => {
            ...
            if src_ty == types::I128 {
                ...
                let ext_mode = ExtMode::new(src_ty.bits(),  64);
