...
fn ech_retry() {
    ...
    let records = client.handshake_raw(now(), None).unwrap(); 
    let records = forward_records(now(), &mut server, records).unwrap(); 
    let records = forward_records(now(), &mut client, records).unwrap(); 
