    ...
    pub fn decode(dec: &mut Decoder) -> Res<Self> {
        ...
        match t {
            ...
            FRAME_TYPE_CRYPTO => {
                ...
                Ok(Self::Crypto {
                    offset: o,
                    data: d!(dec.decode_vvec()).to_vec(), 
                })
            }
            FRAME_TYPE_NEW_TOKEN => {
                Ok(Self::NewToken {
                    token: d!(dec.decode_vvec()).to_vec(), 
                })
            }
            ...
            FRAME_TYPE_NEW_CONNECTION_ID => {
                ...
                let cid = d!(dec.decode_vec(1)).to_vec(); 
            ...
            FRAME_TYPE_CONNECTION_CLOSE_TRANSPORT | FRAME_TYPE_CONNECTION_CLOSE_APPLICATION => {
                Ok(Self::ConnectionClose {
                    ...
                    frame_type: dv!(dec),
                    reason_phrase: d!(dec.decode_vvec()).to_vec(), 
                })
            }
...
mod tests {
    ...
    fn too_large_new_connection_id() {
        let mut enc = Encoder::from_hex("18523400"); 
