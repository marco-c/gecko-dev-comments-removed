...
pub fn decode_packet_hdr(cid_parser: &dyn ConnectionIdDecoder, pd: &[u8]) -> Res<PacketHdr> {
    ...
    if (p.tbyte & 0x80) == 0 {
        ...
        p.dcid = ConnectionId(cid.to_vec()); 
    ...
    if version == 0 {
        ...
        p.tipe = match (p.tbyte >> 4) & 0x3 {
            ...
            PACKET_TYPE_INITIAL => {
                p.epoch = 0;
                PacketType::Initial(d!(d.decode_vvec()).to_vec()) 
            }
            PACKET_TYPE_0RTT => {
                p.epoch = 1;
                PacketType::ZeroRTT
            ...
            PACKET_TYPE_RETRY => {
                let odcid = ConnectionId(d!(d.decode_vec(1)).to_vec()); 
                let token = d.decode_remainder().to_vec(); 
