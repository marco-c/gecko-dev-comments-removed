...
fn reorder_server_initial() {
    ...
    assert_eq!(dec.decode_varint(), Some(0x06)); 
    assert_eq!(dec.decode_varint(), Some(0x00)); 
    dec.skip_vvec(); 
    let end = dec.offset();
...
fn overflow_crypto() {
    ...
    for pn in 0..100_u64 {
        payload.truncate(0);
        payload
            .encode_varint(0x06_u64) 
            .encode_varint(pn * 1000 + 1) 
            .encode_varint(1000_u64); 
        ...
        packet
            ...
            .encode_vvec(&[]) 
            .encode_varint(u64::try_from(2 + payload.len() + aead.expansion()).unwrap()); 
