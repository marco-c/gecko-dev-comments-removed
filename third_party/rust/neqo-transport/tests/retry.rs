...
fn retry_basic() {
    ...
    let dgram = server.process(dgram, now()).dgram().unwrap(); 
    assertions::assert_retry(&dgram);

    let dgram = client.process(Some(dgram), now()).dgram(); 
    assert!(dgram.is_some());
    let dgram = server.process(dgram, now()).dgram(); 
    assert!(dgram.is_some());
    drop(client.process(dgram, now()).dgram()); 
...
fn new_token_expired() {
    ...
    let dgram = Some(Datagram::new(src, d.destination(), d.tos(), &d[..]));
    let dgram = server.process(dgram, the_future).dgram(); 
    assert!(dgram.is_some());
    assertions::assert_retry(dgram.as_ref().unwrap());
}
...
fn retry_after_initial() {
    ...
    let cinit = client.process_output(now()).dgram(); 
    assert!(cinit.is_some());
    let server_flight = server.process(cinit.clone(), now()).dgram(); 
    ...
    let retry = retry_server.process(cinit, now()).dgram(); 
