...
fn retry_basic() {
    ...
    let dgram = server.process(dgram.as_ref(), now()).dgram(); 
    assert!(dgram.is_some());
    mem::drop(client.process(dgram.as_ref(), now()).dgram()); 
...
fn retry_different_ip() {
    ...
    let dgram = client.process(None.as_ref(), now()).dgram(); 
    assert!(dgram.is_some());
    let dgram = server.process(dgram.as_ref(), now()).dgram(); 
    assert!(dgram.is_some());
...
fn new_token_expired() {
    ...
    let dgram = Some(Datagram::new(src, d.destination(), d.tos(), &d[..]));
    let dgram = server.process(dgram.as_ref(), the_future).dgram(); 
    assert!(dgram.is_some());
    assertions::assert_retry(dgram.as_ref().unwrap());
}
...
fn retry_after_initial() {
    ...
    let cinit = client.process(None, now()).dgram(); 
    assert!(cinit.is_some());
    let server_flight = server.process(cinit.as_ref(), now()).dgram(); 
    ...
    let retry = retry_server.process(cinit.as_ref(), now()).dgram(); 
