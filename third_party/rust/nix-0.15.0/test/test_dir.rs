...
fn read() {
    ...
    assert!(&[Some(Type::Directory), None].contains(&entries[0].file_type())); 
    assert!(&[Some(Type::Directory), None].contains(&entries[1].file_type())); 
    assert!(&[Some(Type::Symlink), None].contains(&entries[2].file_type())); 
    assert!(&[Some(Type::File), None].contains(&entries[3].file_type())); 
}
