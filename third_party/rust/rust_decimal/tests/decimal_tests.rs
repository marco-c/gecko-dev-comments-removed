...
fn it_subtracts_decimals() {
    ...
    let tests = &[
        ...
        ("4951760157141521099596496896", "1", "4951760157141521099596496895"), 
...
fn it_can_round_complex_numbers_using_bankers_rounding() {
    ...
    let part = rate / (rate + one); 
    let part = part.round_dp_with_strategy(2, RoundingStrategy::BankersRounding); 
...
fn it_can_round_complex_numbers_using_round_half_up() {
    ...
    let one = Decimal::new(1, 0); 
    let part = rate / (rate + one); 
    let part = part.round_dp_with_strategy(2, RoundingStrategy::RoundHalfUp); 
    assert_eq!("0.16", part.to_string());
}
...
fn it_can_round_complex_numbers_using_round_half_down() {
    ...
    let one = Decimal::new(1, 0); 
    let part = rate / (rate + one); 
    let part = part.round_dp_with_strategy(2, RoundingStrategy::RoundHalfDown); 
    assert_eq!("0.16", part.to_string());
}
