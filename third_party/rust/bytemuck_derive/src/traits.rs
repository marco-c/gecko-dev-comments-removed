...
impl Derivable for NoUninit {
  ...
  fn asserts(input: &DeriveInput, crate_name: &TokenStream) -> Result<TokenStream> {
    ...
    match &input.data {
      ...
      Data::Union(_) => bail!("NoUninit cannot be derived for unions"), 
    }
  }
...
impl Derivable for CheckedBitPattern {
  ...
  fn asserts(input: &DeriveInput, crate_name: &TokenStream) -> Result<TokenStream> {
    ...
    match &input.data {
      ...
      Data::Union(_) => bail!("Internal error in CheckedBitPattern derive"), 
    }
  }
...
fn generate_checked_bit_pattern_enum_without_fields(
  ...
  let repr = get_repr(&input.attrs)?;
  let integer = repr.repr.as_integer().unwrap(); 
  Ok((
    quote!(),
    quote! {
        type Bits = #integer;
