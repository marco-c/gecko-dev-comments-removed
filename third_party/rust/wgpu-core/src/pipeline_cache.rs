...
mod tests {
    ...
    fn written_header() {
        ...
        let cache: [[u8; 8]; HEADER_LENGTH / 8] = [
            *b"WGPUPLCH",                                 
            [0, 0, 0, 1, 0, 0, 0, ABI as u8],             
            [1, 255, 255, 255, 0, 2, 0xFE, 0xED],         
            [0xFE, 0xFE, 0xFE, 0xFE, 255, 255, 255, 255], 
            0xFFFFFFFF_FFFFFFFFu64.to_be_bytes(),         
            0x88888888_88888888u64.to_be_bytes(),         
            0x0u64.to_be_bytes(),                         
            0xFEDCBA9_876543210u64.to_be_bytes(),         
        ];
    ...
    fn wrong_validation() {
        let cache: [[u8; 8]; HEADER_LENGTH / 8] = [
            *b"WGPUPLCH",                                 
            ...
            [0xFE, 0xFE, 0xFE, 0xFE, 255, 255, 255, 255], 
            0xFFFFFFFF_FFFFFFFFu64.to_be_bytes(),         
            0x88888888_00000000u64.to_be_bytes(),         
            0x0u64.to_be_bytes(),                         
            0xFEDCBA9_876543210u64.to_be_bytes(),         
        ];
        let cache = cache.into_iter().flatten().collect::<Vec<u8>>();
    ...
    fn wrong_hash() {
        let cache: [[u8; 8]; HEADER_LENGTH / 8] = [
            *b"WGPUPLCH",                                 
            ...
            0x88888888_88888888u64.to_be_bytes(),         
            0x0u64.to_be_bytes(),                         
            0x00000000_00000000u64.to_be_bytes(),         
        ];
        let cache = cache.into_iter().flatten().collect::<Vec<u8>>();
        let validation_result = super::validate_pipeline_cache(&cache, &ADAPTER, VALIDATION_KEY);
        assert_eq!(validation_result, Err(E::Corrupted));
