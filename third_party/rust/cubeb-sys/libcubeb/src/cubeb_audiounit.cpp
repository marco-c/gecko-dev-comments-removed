...
extern "C" {
int
audiounit_init(cubeb ** context, char const * )
{
#if !TARGET_OS_IPHONE
...
static int
audiounit_get_preferred_sample_rate(cubeb * , uint32_t * rate)
{
#if TARGET_OS_IPHONE
...
int audiounit_stream_device_destroy(cubeb_stream * ,
                                    cubeb_device * device)
{
...
audiounit_create_device_from_hwdev(cubeb_device_info * ret, AudioObjectID devid, cubeb_device_type type)
{
  ...
  ret->max_channels = ch;
  ret->format = (cubeb_device_fmt)CUBEB_DEVICE_FMT_ALL; 
  ...
  if (AudioObjectGetPropertyData(devid, &adr, 0, NULL, &size, &range) == noErr) {
    ...
    ret->latency_lo = 10 * ret->default_rate / 1000;  
    ret->latency_hi = 100 * ret->default_rate / 1000; 
  }
...
static int
audiounit_enumerate_devices(cubeb * , cubeb_device_type type,
...
static int
audiounit_device_collection_destroy(cubeb * ,
                                    cubeb_device_collection * collection)
{
...
static OSStatus
audiounit_collection_changed_callback(AudioObjectID ,
                                      UInt32 ,
