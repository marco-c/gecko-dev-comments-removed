...
struct MixerContext {
  ...
  const cubeb_channel_layout _in_ch_layout;              
  const cubeb_channel_layout _out_ch_layout;             
  const uint32_t _in_ch_count;                           
  const uint32_t _out_ch_count;                          
  const float _surround_mix_level = C_30DB;              
  const float _center_mix_level = C_30DB;                
  const float _lfe_mix_level = 1;                        
  double _matrix[CHANNELS_MAX][CHANNELS_MAX] = {{ 0 }};        
  float _matrix_flt[CHANNELS_MAX][CHANNELS_MAX] = {{ 0 }};     
  int32_t _matrix32[CHANNELS_MAX][CHANNELS_MAX] = {{ 0 }};     
  uint8_t _matrix_ch[CHANNELS_MAX][CHANNELS_MAX+1] = {{ 0 }};  
