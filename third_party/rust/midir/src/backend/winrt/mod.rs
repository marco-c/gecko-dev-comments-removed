...
impl MidiInput {
    ...
    fn handle_input<T>(args: &MidiMessageReceivedEventArgs, handler_data: &mut HandlerData<T>) {
        ...
        message_bytes = unsafe { byte_access.get_buffer().expect("get_buffer failed") }; 
...
impl<T> MidiInputConnection<T> {
    pub fn close(self) -> (MidiInput, T) {
        ...
        let device_selector = MidiInPort::get_device_selector().expect("get_device_selector failed"); 
...
impl MidiOutputConnection {
    pub fn close(self) -> MidiOutput {
        ...
        let _ = closable.close();
        let device_selector = MidiOutPort::get_device_selector().expect("get_device_selector failed"); 
