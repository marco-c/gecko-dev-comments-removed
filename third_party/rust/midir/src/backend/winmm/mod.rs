...
impl MidiInputPort {
    ...
    fn name(port_number: UINT) -> Result<String, PortInfoError> {
        ...
        let pname: &[u16] = unsafe { &device_caps.szPname }; 
    ...
    pub fn connect<F, T: Send>(
        ...
        for i in 0..RT_SYSEX_BUFFER_COUNT {
            handler_data.sysex_buffer.0[i] = Box::into_raw(Box::new(MIDIHDR {
                lpData: unsafe { allocate(RT_SYSEX_BUFFER_SIZE) } as *mut i8,
                dwBufferLength: RT_SYSEX_BUFFER_SIZE as u32,
                dwBytesRecorded: 0,
                dwUser: i as DWORD_PTR, 
                dwFlags: 0,
                lpNext: ptr::null_mut(),
                reserved: 0,
                dwOffset: 0,
    ...
    fn close_internal(&mut self) {
        ...
        for i in 0..RT_SYSEX_BUFFER_COUNT {
            ...
            unsafe {
                ...
                deallocate((*self.handler_data.sysex_buffer.0[i]).lpData as *mut u8, RT_SYSEX_BUFFER_SIZE);
