...
mod tests {
    ...
    impl JxlCmsTransformer for FourToThreeTransformer {
        fn do_transform(&mut self, input: &[f32], output: &mut [f32]) -> Result<()> {
            ...
            for i in 0..num_pixels {
                ...
                output[i * 3] = 1.0 - input[i * 4]; 
                output[i * 3 + 1] = 1.0 - input[i * 4 + 1]; 
                output[i * 3 + 2] = 1.0 - input[i * 4 + 2]; 
            }
            Ok(())
        }
    ...
    fn test_cms_stage_cmyk_to_rgb() {
        ...
        let mut ch0 = vec![0.2, 0.5]; 
        let mut ch1 = vec![0.3, 0.5]; 
        let mut ch2 = vec![0.4, 0.5]; 
        let mut ch3 = vec![0.1, 0.5]; 
    ...
    fn test_cms_stage_uses_channel() {
        ...
        assert!(!stage_cmyk.uses_channel(3)); 
        assert!(!stage_cmyk.uses_channel(4)); 
        assert!(stage_cmyk.uses_channel(5)); 
