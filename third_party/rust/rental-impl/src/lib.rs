...
fn write_rental_struct_and_impls(tokens: &mut proc_macro2::TokenStream, struct_info: &syn::ItemStruct) {
	let def_site: Span = Span::call_site(); 
	let call_site: Span = Span::call_site();
	...
	let struct_span = struct_info.span();
	let suffix_ty_span = suffix_ty.span();
...
fn get_struct_attribs(struct_info: &syn::ItemStruct) -> RentalStructAttribs
{
	...
	if let Some(rental_pos) = rattribs.iter().position(|attr| match attr.parse_meta().expect(&format!("Struct `{}` Attribute `{}` is not properly formatted.", struct_info.ident, attr.path.clone().into_token_stream())) {
