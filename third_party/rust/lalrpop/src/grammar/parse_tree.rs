...
pub struct NonterminalData {
    ...
    pub annotations: Vec<Annotation>,
    pub span: Span,
    pub args: Vec<NonterminalString>, 
...
impl Display for TerminalLiteral {
    fn fmt(&self, fmt: &mut Formatter) -> Result<(), Error> {
        match *self {
            TerminalLiteral::Quoted(ref s) => write!(fmt, "{:?}", s.as_ref()), 
            TerminalLiteral::Regex(ref s) => write!(fmt, "r#{:?}#", s.as_ref()), 
        }
    }
}
