...
fn parse_rfc2822<'a>(parsed: &mut Parsed, mut s: &'a str) -> ParseResult<(&'a str, ())> {
    ...
    s = try!(scan::char(s.trim_left(), b':')).trim_left(); 
...
fn test_parse() {
    ...
    check!("42195080354",  [internal_fix!(Nanosecond9NoDot), num!(Second)]; nanosecond: 421_950_803, second: 54); 
    ...
    check!("???",       [fix!(TimezoneName)]; BAD_FORMAT); 
