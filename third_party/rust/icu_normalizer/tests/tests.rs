...
fn test_nfd_basic() {
    ...
    assert_eq!(normalizer.normalize("\u{2126}"), "Ω"); 
    assert_eq!(normalizer.normalize("ﾍﾞ"), "ﾍﾞ"); 
    assert_eq!(normalizer.normalize("ﾍﾟ"), "ﾍﾟ"); 
    assert_eq!(normalizer.normalize("ﬁ"), "ﬁ"); 
    assert_eq!(normalizer.normalize("\u{FDFA}"), "\u{FDFA}"); 
...
fn test_nfkd_basic() {
    ...
    assert_eq!(normalizer.normalize("𝅗𝅥"), "𝅗\u{1D165}");
    assert_eq!(normalizer.normalize("\u{2126}"), "Ω"); 
    assert_eq!(normalizer.normalize("ﾍﾞ"), "ヘ\u{3099}"); 
    assert_eq!(normalizer.normalize("ﾍﾟ"), "ヘ\u{309A}"); 
...
fn test_nfkc_basic() {
    ...
    assert_eq!(normalizer.normalize("\u{2126}"), "Ω"); 
    assert_eq!(normalizer.normalize("ﾍﾞ"), "ベ"); 
    assert_eq!(normalizer.normalize("ﾍﾟ"), "ペ"); 
    assert_eq!(normalizer.normalize("ﬁ"), "fi"); 
...
fn test_uts46_map_normalize() {
    ...
    assert_eq!(mapper.map_normalize("ﾍﾞ".chars()).collect::<String>(), "ベ"); 
    assert_eq!(mapper.map_normalize("ﾍﾟ".chars()).collect::<String>(), "ペ"); 
    assert_eq!(mapper.map_normalize("ﬁ".chars()).collect::<String>(), "fi"); 
...
fn test_canonical_composition() {
    ...
    assert_eq!(comp.compose('ে', 'া'), Some('ো')); 
    assert_eq!(comp.compose('𑄱', '𑄧'), Some('𑄮')); 

    assert_eq!(comp.compose('ᄀ', 'ᅡ'), Some('가')); 
    assert_eq!(comp.compose('가', 'ᆨ'), Some('각')); 
}
...
fn test_canonical_decomposition() {
    ...
    assert_eq!(decomp.decompose('\u{212B}'), Decomposed::Singleton('Å')); 
    assert_eq!(decomp.decompose('\u{2126}'), Decomposed::Singleton('Ω')); 

    assert_eq!(decomp.decompose('\u{1F71}'), Decomposed::Singleton('ά')); 
    assert_eq!(
