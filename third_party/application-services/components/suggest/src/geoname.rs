...
pub(crate) mod tests {
    ...
    fn geonames_metrics() -> anyhow::Result<()> {
        ...
        store.read(|dao| {
            ...
            assert_eq!(cache.keywords_metrics.max_len, 21); 
        ...
        store.read(|dao| {
            let cache = dao.geoname_cache();
            assert_eq!(cache.keywords_metrics.max_len, 8); 
            assert_eq!(cache.keywords_metrics.max_word_count, 1); 
            Ok(())
        })?;
        ...
        store.read(|dao| {
            let cache = dao.geoname_cache();
            assert_eq!(cache.keywords_metrics.max_len, 14); 
