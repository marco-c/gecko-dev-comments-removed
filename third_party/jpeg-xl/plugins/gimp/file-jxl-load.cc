...
Status FillGimpLayer(const gint32 layer, const CodecInOut& io,
  ...
  switch (layer_type) {
    case GIMP_GRAY_IMAGE:
      FillBuffer<precision, false, 1>(
          io, &pixel_data);
      break;
    case GIMP_GRAYA_IMAGE:
      FillBuffer<precision, true, 1>(
          io, &pixel_data);
      break;
    case GIMP_RGB_IMAGE:
      FillBuffer<precision, false, 3>(
          io, &pixel_data);
      break;
    case GIMP_RGBA_IMAGE:
      FillBuffer<precision, true, 3>(
          io, &pixel_data);
      break;
    default:
      return false;
...
Status LoadJpegXlImage(const gchar* const filename, gint32* const image_id) {
  ...
  *image_id = gimp_image_new_with_precision(io.xsize(), io.ysize(), image_type,
  ...
  const gint32 layer = gimp_layer_new(
      *image_id, "image", io.xsize(), io.ysize(), layer_type, 100,
