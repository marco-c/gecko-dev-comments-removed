...
void ValidateHuffmanTable(j_compress_ptr cinfo, const JHUFF_TBL* table,
  ...
  total_p += 1u << (kJpegHuffmanMaxBitLength - max_depth);  
  if (total_symbols == 0) {
...
void CopyHuffmanCodes(j_compress_ptr cinfo,
  ...
  for (int c = 0; c < cinfo->num_components; ++c) {
    ...
    CopyHuffmanTable(cinfo, comp->dc_tbl_no, true, huffman_codes);
    CopyHuffmanTable(cinfo, comp->ac_tbl_no, false, huffman_codes);
  }
  for (int i = 0; i < cinfo->num_scans; ++i) {
