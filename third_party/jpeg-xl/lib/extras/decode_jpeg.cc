...
std::unique_ptr<RenderPipeline> PreparePipeline(
  ...
  if (!cs.Is444()) {
    for (size_t c = 0; c < 3; c++) {
      if (cs.HShift(c) != 0) {
        builder.AddStage(GetChromaUpsamplingStage(c, true));
      }
      if (cs.VShift(c) != 0) {
        builder.AddStage(GetChromaUpsamplingStage(c, false));
      }
    }
  }
  if (is_ycbcr) {
  ...
  builder.AddStage(GetWriteToOutputStage(
      ...
      false,
      false,
      0, Orientation::kIdentity, extra_output));
...
Status DecodeJpeg(const std::vector<uint8_t>& compressed,
  ...
  frame_dim.Set(xsize, ysize, 1, cs.MaxHShift(),
                cs.MaxVShift(),
                false, 1);
