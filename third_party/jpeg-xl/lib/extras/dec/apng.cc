...
struct Pixels {
  ...
  Status Resize(size_t row_bytes, size_t num_rows) {
    size_t new_size = row_bytes * num_rows;  
...
struct Reader {
  ...
  Bytes ReadChunk() {
    ...
    constexpr size_t kMaxPNGChunkSize = 1u << 30;  
    
    if (size > kMaxPNGChunkSize) {
      JXL_WARNING("APNG chunk size is too big");
      return Bytes();
    }
    size_t full_size = size + 12;  
...
Status DecodeImageAPNG(const Span<const uint8_t> bytes,
  ...
  FrameControl current_frame = {1, 10, image_rect,
    ...
    switch (id) {
      ...
      case MakeTag('f', 'c', 'T', 'L'): {
        ...
        FrameControl next_frame = {
            png_get_uint_16(payload.data() + 20),
            png_get_uint_16(payload.data() + 22), raw_viewport,
      ...
      case MakeTag('I', 'D', 'A', 'T'): {
        ...
        if (!seen_idat) {
          ...
          format = {
              num_channels,
              ppf->info.bits_per_sample > 8 ? JXL_TYPE_UINT16
                                                          : JXL_TYPE_UINT8,
              JXL_BIG_ENDIAN,
              0,
          };
