...
Status DecodeImagePSD(const Span<const uint8_t> bytes,
                      const ColorHints& , ThreadPool* pool,
                      CodecInOut* io) {
  const uint8_t* pos = bytes.data();
  const uint8_t* maxpos = bytes.data() + bytes.size();
  if (safe_strncmp(pos, maxpos, "8BPS", 4)) return false;  
  ...
  while (pos < metaoffset + metalength) {
    ...
    if (id == 1039) {  
      ...
      for (int k = 0; k < spotcolorcount; k++) {
        ...
        color.push_back(get_be_int(2, pos, maxpos) / 65535.f);  
        color.push_back(get_be_int(2, pos, maxpos) / 65535.f);  
        color.push_back(get_be_int(2, pos, maxpos) / 65535.f);  
        color.push_back(get_be_int(2, pos, maxpos) / 65535.f);  
  ...
  if (layerlength) {
    ...
    for (int l = 0; l < layercount; l++) {
      ...
      for (int lc = 0; lc < nb_chs; lc++) {
        ...
        if (id == 65535) {
          chan_ids.push_back(colormodel);  
        } else if (id == 65534) {
          chan_ids.push_back(-1);  
        } else {
      ...
      for (const auto& ec_meta : layer.metadata()->extra_channel_info) {
        ...
        if (ec_meta.type == ExtraChannel::kAlpha) {
          FillPlane(1.0f, &extra, Rect(extra));  
        } else {
          ZeroFillPlane(&extra, Rect(extra));  
        }
