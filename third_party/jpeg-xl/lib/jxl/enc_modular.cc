...
void MergeTrees(const std::vector<Tree>& trees,
  ...
  tree->emplace_back(1 , splitval, 0, 0, Predictor::Zero, 0, 1);
  ...
  for (const Channel& ch : img.channel) {
    ...
    for (size_t y = 0; y < ch.h; y++) {
      ...
      for (size_t x = 0; x < ch.w; x++) {
        ...
        pixel_type guess = wp_state.Predict<true>(
        ...
        [&](const uint32_t chunk, size_t ) {
          ...
          for (size_t i = start; i < stop; i++) {
            JXL_CHECK(ModularGenericCompress(
                stream_images_[i], stream_options_[i], nullptr,
                nullptr, LayerType::Header, i, &tree_samples,
                &total_pixels));
          }
...
Status ModularFrameEncoder::ComputeTokens(ThreadPool* pool) {
  ...
  JXL_RETURN_IF_ERROR(RunOnPool(
      ...
      [&](const uint32_t stream_id, size_t ) {
        AuxOut my_aux_out;
        tokens_[stream_id].clear();
        JXL_CHECK(ModularGenericCompress(
            stream_images_[stream_id], stream_options_[stream_id],
            nullptr, &my_aux_out, LayerType::Header, stream_id,
            nullptr,
            nullptr,
            &tree_, &stream_headers_[stream_id],
            &tokens_[stream_id],
