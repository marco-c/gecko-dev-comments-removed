...
void GaborishRow(const FilterRows& rows, const LoopFilter& ,
...
void Epf0Row(const FilterRows& rows, const LoopFilter& lf,
  ...
  for (size_t x = x0; x < x1; x += Lanes(df)) {
    ...
    for (size_t i = 0; i < 12; i++) {
      AddPixelStep1<false>(sads_off[i][0], rows,
...
void Epf1Row(const FilterRows& rows, const LoopFilter& lf,
  ...
  for (size_t x = x0; x < x1; x += Lanes(df)) {
    ...
    AddPixelStep1<true>(-1, rows, x, sad0, inv_sigma, lf,
                                    &X, &Y, &B, &w);
    
    AddPixelStep1<false>(0, rows, x - 1, sad1, inv_sigma,
                                     lf, &X, &Y, &B, &w);
    AddPixelStep1<false>(0, rows, x + 1, sad2, inv_sigma,
                                     lf, &X, &Y, &B, &w);
    
    AddPixelStep1<true>(1, rows, x, sad3, inv_sigma, lf, &X,
                                    &Y, &B, &w);
#if JXL_HIGH_PRECISION
    auto inv_w = Set(df, 1.0f) / w;
#else
...
void Epf2Row(const FilterRows& rows, const LoopFilter& lf,
  ...
  for (size_t x = x0; x < x1; x += Lanes(df)) {
    ...
    AddPixelStep2<true>(-1, rows, x, x_cc, y_cc, b_cc,
                                    inv_sigma, lf, &X, &Y, &B, &w);
    
    AddPixelStep2<false>(0, rows, x - 1, x_cc, y_cc, b_cc,
                                     inv_sigma, lf, &X, &Y, &B, &w);
    AddPixelStep2<false>(0, rows, x + 1, x_cc, y_cc, b_cc,
                                     inv_sigma, lf, &X, &Y, &B, &w);
