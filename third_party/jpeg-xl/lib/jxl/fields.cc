...
struct InitVisitor : public VisitorBase {
  ...
  Status U32(const U32Enc , const uint32_t default_value,
             uint32_t* JXL_RESTRICT value) override {
    *value = default_value;
    return true;
  }
  ...
  Status Conditional(bool ) override { return true; }

  Status AllDefault(const Fields& ,
  ...
  Status VisitNested(Fields* ) override {
...
class ReadVisitor : public VisitorBase {
 public:
  ...
  Status U32(const U32Enc dist, const uint32_t ,
             uint32_t* JXL_RESTRICT value) override {
...
class MaxBitsVisitor : public VisitorBase {
 public:
  Status Bits(const size_t bits, const uint32_t ,
              uint32_t* JXL_RESTRICT ) override {
  ...
  Status U64(const uint64_t ,
             uint64_t* JXL_RESTRICT ) override {
    max_bits_ += U64Coder::MaxEncodedBits();
    return true;
  }

  Status F16(const float ,
             float* JXL_RESTRICT ) override {
  ...
  Status BeginExtensions(uint64_t* JXL_RESTRICT ) override {
...
bool Bundle::AllDefault(const Fields& fields) {
  AllDefaultVisitor visitor(PrintAllDefault());
  const char* name =
...
Status Bundle::CanEncode(const Fields& fields, size_t* extension_bits,
                         size_t* total_bits) {
  CanEncodeVisitor visitor(PrintSizes());
...
Status Bundle::Read(BitReader* reader, Fields* fields) {
  ReadVisitor visitor(reader, PrintRead());
  JXL_RETURN_IF_ERROR(
