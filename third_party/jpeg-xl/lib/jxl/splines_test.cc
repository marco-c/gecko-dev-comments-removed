...
TEST(SplinesTest, Serialization) {
  std::vector<Spline> spline_data = {
      {{
       ...
       {32.7, 21.5, 44.4, 1.8,  45.8, 90.6, 29.3, 59.2,
      ...
      {{{172, 309},
       ...
       {72.5, 2.6,  41.7, 2.2,  39.7, 79.1, 69.6, 19.9,
      ...
      {{{100, 186},
       ...
       {83.5, 1.7,  25.1, 18.7, 46.5, 75.3, 28,   62.3,
  ...
  ASSERT_TRUE(decoded_splines.Decode(&reader, 1000));
...
TEST(SplinesTest, TooManySplinesTest) {
  ...
  BitReader reader(writer.GetSpan());
  Splines decoded_splines;
  EXPECT_FALSE(decoded_splines.Decode(&reader, 1000));
...
TEST(SplinesTest, Drawing) {
  ...
  (void)OpsinToLinearInplace(&image, nullptr, opsin_params);
