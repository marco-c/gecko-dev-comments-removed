...
static inline const char* EnumName(ColorSpace ) {
  return "ColorSpace";
}
static inline constexpr uint64_t EnumBits(ColorSpace ) {
  using CS = ColorSpace;
  return MakeBit(CS::kRGB) | MakeBit(CS::kGray) | MakeBit(CS::kXYB) |
         MakeBit(CS::kUnknown);
}

static inline const char* EnumName(WhitePoint ) {
  return "WhitePoint";
}
static inline constexpr uint64_t EnumBits(WhitePoint ) {
  return MakeBit(WhitePoint::kD65) | MakeBit(WhitePoint::kCustom) |
         MakeBit(WhitePoint::kE) | MakeBit(WhitePoint::kDCI);
}

static inline const char* EnumName(Primaries ) { return "Primaries"; }
static inline constexpr uint64_t EnumBits(Primaries ) {
  using Pr = Primaries;
  return MakeBit(Pr::kSRGB) | MakeBit(Pr::kCustom) | MakeBit(Pr::k2100) |
         MakeBit(Pr::kP3);
}

static inline const char* EnumName(TransferFunction ) {
  return "TransferFunction";
}

static inline constexpr uint64_t EnumBits(TransferFunction ) {
...
static inline const char* EnumName(RenderingIntent ) {
  return "RenderingIntent";
}
static inline constexpr uint64_t EnumBits(RenderingIntent ) {
