...
void RgbFromSingle(const size_t xsize,
                   const pixel_type* const JXL_RESTRICT row_in,
                   const float factor, Image3F* decoded, size_t ,
                   size_t y) {
  const HWY_FULL(float) df;
  const Rebind<pixel_type, HWY_FULL(float)> di;  
...
Status ModularFrameDecoder::FinalizeDecoding(PassesDecoderState* dec_state,
  ...
  if (output->HasExtraChannels()) {
    for (size_t ec = 0; ec < output->extra_channels().size(); ec++, c++) {
      ...
      const size_t ec_xsize = eci.Size(xsize);  
...
Status ModularFrameDecoder::DecodeQuantTable(
  ...
  if (modular_frame_decoder) {
    JXL_RETURN_IF_ERROR(ModularGenericDecompress(
        ...
        &options, -1, &modular_frame_decoder->tree,
