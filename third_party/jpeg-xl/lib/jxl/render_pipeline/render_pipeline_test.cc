...
TEST(RenderPipelineTest, Build) {
  RenderPipeline::Builder builder(1);
...
TEST(RenderPipelineTest, CallAllGroups) {
  ...
  ASSERT_TRUE(pipeline->PrepareForThreads(1, false));
...
TEST_P(RenderPipelineTestParam, PipelineTest) {
  ...
  FakeParallelRunner fake_pool(123, 8);
