 ...
 public:
  ...
  void ProcessRow(const RowInfo& input_rows, const RowInfo& output_rows,
    ...
    for (ssize_t x = -xextra; x < static_cast<ssize_t>(xsize + xextra);
      ...
      for (size_t i = 0; i < 12; i++) {
        AddPixel<false>(sads_off[i][0], input_rows,
  ...
  void ProcessRow(const RowInfo& input_rows, const RowInfo& output_rows,
    ...
    for (ssize_t x = -xextra; x < static_cast<ssize_t>(xsize + xextra);
      ...
      for (size_t c = 0; c < 3; c++) {
        ...
        sad1c += AbsDiff(p02, p12);  
        sad0c += AbsDiff(p11, p12);  
      ...
      AddPixel<true>(-1, input_rows, x, sad0, inv_sigma, &X,
                                 &Y, &B, &w);
      
      AddPixel<false>(0, input_rows, x - 1, sad1, inv_sigma,
                                  &X, &Y, &B, &w);
      AddPixel<false>(0, input_rows, x + 1, sad2, inv_sigma,
                                  &X, &Y, &B, &w);
      
      AddPixel<true>(1, input_rows, x, sad3, inv_sigma, &X,
                                 &Y, &B, &w);
#if JXL_HIGH_PRECISION
      auto inv_w = Set(df, 1.0f) / w;
#else
 ...
 public:
  ...
  void ProcessRow(const RowInfo& input_rows, const RowInfo& output_rows,
    ...
    for (ssize_t x = -xextra; x < static_cast<ssize_t>(xsize + xextra);
      ...
      AddPixel<true>(-1, input_rows, x, x_cc, y_cc, b_cc,
                                 inv_sigma, &X, &Y, &B, &w);
      
      AddPixel<false>(0, input_rows, x - 1, x_cc, y_cc,
