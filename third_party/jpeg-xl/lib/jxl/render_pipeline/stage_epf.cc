 ...
 public:
  ...
  Status ProcessRow(const RowInfo& input_rows, const RowInfo& output_rows,
    ...
    for (ssize_t x = -xextra; x < static_cast<ssize_t>(xsize + xextra);
      ...
      for (size_t i = 0; i < 12; i++) {
        AddPixel<false>(sads_off[i][0], rows,
  ...
  Status ProcessRow(const RowInfo& input_rows, const RowInfo& output_rows,
    ...
    for (ssize_t x = -xextra; x < static_cast<ssize_t>(xsize + xextra);
      ...
      for (size_t c = 0; c < 3; c++) {
        ...
        sad0c = Add(sad0c, AbsDiff(p31, p32));  
        t = AbsDiff(p22, p32);
        sad1c = Add(sad1c, t);  
        sad2c = Add(sad2c, t);  

        const auto p42 = LoadU(df, rows[c][2 + 0] + x + 2);
        sad2c = Add(sad2c, AbsDiff(p42, p32));  

        const auto p13 = LoadU(df, rows[c][2 + 1] + x - 1);
        sad3c = Add(sad3c, AbsDiff(p13, p12));  

        const auto p23 = Load(df, rows[c][2 + 1] + x);
        t = AbsDiff(p22, p23);
        sad0c = Add(sad0c, t);                  
        sad3c = Add(sad3c, t);                  
        sad1c = Add(sad1c, AbsDiff(p13, p23));  

        const auto p33 = LoadU(df, rows[c][2 + 1] + x + 1);
        sad2c = Add(sad2c, AbsDiff(p33, p23));  
        sad3c = Add(sad3c, AbsDiff(p33, p32));  

        const auto p24 = Load(df, rows[c][2 + 2] + x);
        sad3c = Add(sad3c, AbsDiff(p24, p23));  
      ...
      AddPixel<true>(-1, rows, x, sad0, inv_sigma, &X, &Y,
                                 &B, &w);
      
      AddPixel<false>(0, rows, x - 1, sad1, inv_sigma, &X,
                                  &Y, &B, &w);
      AddPixel<false>(0, rows, x + 1, sad2, inv_sigma, &X,
                                  &Y, &B, &w);
      
      AddPixel<true>(1, rows, x, sad3, inv_sigma, &X, &Y,
                                 &B, &w);
#if JXL_HIGH_PRECISION
      auto inv_w = Div(Set(df, 1.0f), w);
#else
 ...
 public:
  ...
  Status ProcessRow(const RowInfo& input_rows, const RowInfo& output_rows,
    ...
    for (ssize_t x = -xextra; x < static_cast<ssize_t>(xsize + xextra);
      ...
      AddPixel<true>(-1, rows, x, x_cc, y_cc, b_cc,
                                 inv_sigma, &X, &Y, &B, &w);
      
      AddPixel<false>(0, rows, x - 1, x_cc, y_cc, b_cc,
                                  inv_sigma, &X, &Y, &B, &w);
      AddPixel<false>(0, rows, x + 1, x_cc, y_cc, b_cc,
                                  inv_sigma, &X, &Y, &B, &w);
