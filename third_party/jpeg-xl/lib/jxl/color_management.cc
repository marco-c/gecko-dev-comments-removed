...
Status ToneMapPixel(const ColorEncoding& c, const float in[3],
  ...
  if (c.tf.IsPQ()) {
    ...
    HlgOOTF ootf(300, 80, luminances);
    ootf.Apply(&r, &g, &b);
  }
  GamutMap(&r, &g, &b, luminances, 0.3f);
  StoreU(r, d, &linear[0]);
  StoreU(g, d, &linear[1]);
  StoreU(b, d, &linear[2]);
...
Status CreateICCHeader(const ColorEncoding& c,
  ...
  WriteICCUint32(0, 44, header);  
  WriteICCUint32(0, 48, header);  
  WriteICCUint32(0, 52, header);  
  WriteICCUint32(0, 56, header);  
  WriteICCUint32(0, 60, header);  
