...
void VerifySymmetric(const int64_t width, const int64_t peak,
  ...
  for (int64_t i = 1;; ++i) {
    ...
    EXPECT_GT(out[peak + i - 1] + tolerance, out[peak + i]);  
    EXPECT_NEAR(out[peak - i], out[peak + i], tolerance);     
  }
}
...
TEST(GaussBlurTest, ImpulseResponse) {
  TestImpulseResponseForWidth(10);  
  TestImpulseResponseForWidth(15);  
  TestImpulseResponseForWidth(32);  
  TestImpulseResponseForWidth(31);  
  TestImpulseResponseForWidth(33);  
}
...
void Benchmark1D() {
  ...
  const double mps_rg1 =
      Measure(length, 1, 1, [=](size_t , size_t ) {
        ImageF in(length, 1);
