...
struct OutputNone {
  template <class V>
  void operator()(const V& , float* JXL_RESTRICT ,
                  ptrdiff_t ) const {}
};
...
hwy::AlignedUniquePtr<RecursiveGaussian> CreateRecursiveGaussian(double sigma) {
  ...
  const double radius = roundf(3.2795 * sigma + 0.2546);  
  ...
  const double gamma[3] = {1, radius * radius - sigma * sigma,  
                           zeta_15 * rho[0] + zeta_35 * rho[1] + rho[2]};
  double beta[3];
  Mul3x3Vector(A, gamma, beta);  

  
  const double sum = beta[0] * p_1 + beta[1] * p_3 + beta[2] * p_5;  
  ...
  double d1[3];
  for (size_t i = 0; i < 3; ++i) {
    n2[i] = -beta[i] * std::cos(omega[i] * (radius + 1.0));  
    d1[i] = -2.0 * std::cos(omega[i]);                       
