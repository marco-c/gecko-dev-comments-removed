  ...
  *kernel_storage = AllocateArray(stride * sizeof(float) * num_coeffs);
  ...
  for (size_t offset = 0; offset < num_coeffs; ++offset) {
    ...
    for (size_t kernel = 0; kernel < num_kernels; ++kernel) {
      ...
      size_t kx = kx_ % N;
      size_t shift = kx_ / N;
      if ((ix < shift) || (ix - shift >= M)) continue;  
...
void UpsampleRect(size_t upsampling, const float* kernels, const ImageF& src,
  ...
  if (upsampling == 2) {
    if (x_repeat == 1) {
      Upsample<2, 1>(src, src_rect, dst, dst_rect, kernels,
                                        image_y_offset, image_ysize, arena);
    } else if (x_repeat == 2) {
      Upsample<2, 2>(src, src_rect, dst, dst_rect, kernels,
                                        image_y_offset, image_ysize, arena);
    } else if (x_repeat == 4) {
      Upsample<2, 4>(src, src_rect, dst, dst_rect, kernels,
                                        image_y_offset, image_ysize, arena);
    } else {
    ...
    Upsample<4, 1>(src, src_rect, dst, dst_rect, kernels,
                                      image_y_offset, image_ysize, arena);
  } else if (upsampling == 8) {
    JXL_ASSERT(x_repeat == 1);
    Upsample<8, 1>(src, src_rect, dst, dst_rect, kernels,
                                      image_y_offset, image_ysize, arena);
  } else {
    JXL_ABORT("Not implemented");
  }
