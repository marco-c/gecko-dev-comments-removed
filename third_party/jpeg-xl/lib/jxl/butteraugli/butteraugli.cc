...
Status Blur(const ImageF& in, float sigma, const ButteraugliParams& params,
  ...
  if (kernel.size() == 5 && &in != out) {
    ...
    Separable5(in, Rect(in), weights, nullptr, out);
    return true;
  }
...
template <class D>
Vec<D> MaltaUnit(MaltaTag , const D df,
                 const float* BUTTERAUGLI_RESTRICT d, const intptr_t xs) {
  const intptr_t xs3 = 3 * xs;
...
HWY_EXPORT(L2Diff);                    
HWY_EXPORT(SetL2Diff);                 
HWY_EXPORT(CombineChannelsToDiffmap);  
HWY_EXPORT(MaltaDiffMap);              
HWY_EXPORT(MaltaDiffMapLF);            
HWY_EXPORT(OpsinDynamicsImage);        
HWY_EXPORT(ButteraugliDiffmapInPlace);  
...
void ScoreToRgb(double score, double good_threshold, double bad_threshold,
  ...
  ix = std::min(std::max(0, ix), kTableSize - 2);  
  double mix = score - ix;
  for (int i = 0; i < 3; ++i) {
