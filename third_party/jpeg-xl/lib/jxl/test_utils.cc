...
size_t Roundtrip(const extras::PackedPixelFile& ppf_in,
  ...
  SetThreadParallelRunner(dparams, pool);
  std::vector<uint8_t> compressed;
  Check(extras::EncodeImageJXL(cparams, ppf_in, nullptr,
                               &compressed));
...
jxl::CodecInOut SomeTestImageToCodecInOut(const std::vector<uint8_t>& buf,
  ...
  Check(ConvertFromExternal(
      ...
      jxl::ColorEncoding::SRGB(num_channels < 3),
      16, format,
      nullptr,
      &io.Main()));
  return io;
...
Status EncodeFile(const CompressParams& params, CodecInOut* io,
  ...
  JXL_RETURN_IF_ERROR(
      WriteCodestreamHeaders(metadata.get(), &writer,  nullptr));
  ...
  JXL_RETURN_IF_ERROR(
      writer.WithMaxBits(8, LayerType::Header, nullptr, [&] {
        writer.ZeroPadToByte();
        return true;
      }));
...
extras::JXLCompressParams CompressParamsForLossless() {
  ...
  cparams.AddOption(JXL_ENC_FRAME_SETTING_MODULAR_PREDICTOR, 6);  
