...
struct GaussianEllipse {
  ...
  double ridge_loss;              
  double custom_loss;             
  std::array<double, 3> bgColor;  
  size_t neg_pixels;  
  std::array<double, 3> neg_value;  
};
...
void ComputeDotLosses(GaussianEllipse* ellipse, const ConnectedComponent& cc,
  ...
  const double kIntensityR = 0.0;   
  const double kSigmaR = 0.0;       
  const double kZeroEpsilon = 0.1;  
...
GaussianEllipse FitGaussianOpt(const ConnectedComponent& cc,
  ...
  auto l2_loss = [&img, &cc, &background,
    ...
    ellipse.sigma_x = exp(params[2]);  
    ellipse.sigma_y = exp(params[3]);  
