...
template <typename T>
void StoreValue(float val, const T& info, JxlPixelFormat format,
                uint8_t** out) {
  const float mul = (1u << info.bits_per_sample) - 1;
...
template <typename T>
void FillPackedImage(const T& info, uint16_t seed, extras::PackedImage* image) {
...
class TestImage {
  ...
  TestImage& SetChannels(size_t num_channels) {
    ...
    for (size_t i = 1; i < ppf_.info.num_extra_channels; ++i) {
      ...
      if (ec.ec_info.bits_per_sample == 0) {
        ...
        ec.ec_info.exponent_bits_per_sample =
            ppf_.info.exponent_bits_per_sample;
      }
  ...
  class Frame {
   public:
    ...
    void RandomFill(uint16_t seed = 177) {
      FillPackedImage(ppf().info, seed, &frame().color);
 ...
 private:
  ...
  static void CropImage(size_t xsize, size_t ysize,
