...
struct Globals {
 ...
 private:
  ...
  Globals() : pool(kNumThreads) {
    ...
    c_native = ColorEncoding::LinearSRGB(false);
    c_gray = ColorEncoding::LinearSRGB(true);
  }
...
TEST_F(ColorManagementTest, HlgOotf) {
  ...
  ASSERT_TRUE(grayscale_transform.Init(
      grayscale_hlg, ColorEncoding::LinearSRGB(true), 1000, 1, 1));
