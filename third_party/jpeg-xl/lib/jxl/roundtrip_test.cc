...
jxl::CodecInOut ConvertTestImage(const std::vector<uint8_t>& buf,
  ...
  EXPECT_TRUE(ConvertFromExternal(
      ...
      false,
      bitdepth, pixel_format.endianness,
      false, nullptr, &io.Main(), float_in,
      0));
  return io;
}
...
void VerifyRoundtripCompression(
  ...
  if (alpha_in_extra_channels_vector && !has_interleaved_alpha) {
    ...
    EXPECT_EQ(
        jxl::ConvertFromExternal(
            ...
            input_pixel_format.endianness, nullptr, &alpha_channel,
            input_pixel_format.data_type == JXL_TYPE_FLOAT,
            0),
        true);
  ...
  if (input_pixel_format.data_type == JXL_TYPE_FLOAT) {
    JxlColorEncodingSetToLinearSRGB(
        &color_encoding,
        input_pixel_format.num_channels < 3);
  } else {
...
TEST(RoundtripTest, JXL_TRANSCODE_JPEG_TEST(TestJPEGReconstruction)) {
  ...
  ASSERT_TRUE(
      SetFromBytes(jxl::Span<const uint8_t>(orig), &orig_io, nullptr));
