...
TEST(ModularTest, RoundtripExtraProperties) {
  ...
  JXL_ASSIGN_OR_DIE(Image decoded, Image::Create(kSize, kSize, 8,
                                                 image.channel.size()));
  ...
  Status status = true;
  {
    ...
    ASSERT_TRUE(ModularGenericDecompress(&reader, decoded, nullptr,
                                         0, &options));
  }
  ASSERT_TRUE(status);
...
void WriteHeaders(BitWriter* writer, size_t xsize, size_t ysize) {
  ...
  metadata.m.color_encoding = ColorEncoding::LinearSRGB(true);
...
void WriteHistograms(BitWriter* writer) {
  ...
  writer->Write(1, 1);         
  writer->Write(7, 0x63);      
  writer->Write(12, 0xfef);    
  writer->Write(32, 0x10003);  
