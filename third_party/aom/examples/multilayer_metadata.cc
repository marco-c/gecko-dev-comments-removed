...
bool parse_integer_list(std::ifstream &file, int min_indent, int *line_idx,
  ...
  ParsedValue value;
  bool syntax_error;
  while (parse_line(file, min_indent, true, &indent,
...
bool parse_color_properties(std::ifstream &file, int min_indent, int *line_idx,
  ...
  bool syntax_error;
  *color = {};
  while (parse_line(file, min_indent, false, &indent,
                    &has_list_prefix, line_idx, &field_name, &value,
                    &syntax_error)) {
    if (field_name == "color_range") {
      RETURN_IF_FALSE(value.IntegerValueInRange(0, 1, *line_idx,
                                                &color->color_range));
    } else if (field_name == "color_primaries") {
      if (!value.IntegerValueInRange(0, 255, *line_idx,
                                     &color->color_primaries)) {
        return false;
      }
    } else if (field_name == "transfer_characteristics") {
      RETURN_IF_FALSE(value.IntegerValueInRange(
          0, 255, *line_idx, &color->transfer_characteristics));
    } else if (field_name == "matrix_coefficients") {
      RETURN_IF_FALSE(value.IntegerValueInRange(
          0, 255, *line_idx, &color->matrix_coefficients));
    } else {
  ...
  *alpha_info = {};
  while (parse_line(file, min_indent, false, &indent,
    ...
    if (field_name == "alpha_use_idc") {
      RETURN_IF_FALSE(value.IntegerValueInRange(
          0, 7, *line_idx, &alpha_info->alpha_use_idc));
    } else if (field_name == "alpha_bit_depth") {
      RETURN_IF_FALSE(value.IntegerValueInRange(
          8, 15, *line_idx, &alpha_info->alpha_bit_depth));
    } else if (field_name == "alpha_clip_idc") {
      RETURN_IF_FALSE(value.IntegerValueInRange(0, 3, *line_idx,
      ...
      RETURN_IF_FALSE(
          parse_integer_list<uint16_t>(file, indent + 1,
  ...
  *depth_info = {};
  while (parse_line(file, min_indent, false, &indent,
    ...
    if (field_name == "z_near") {
      ...
      RETURN_IF_FALSE(
          value.IntegerValueInRange(0, 15, *line_idx,
                                    &depth_info->depth_representation_type));
    } else if (field_name == "disparity_ref_view_id") {
      RETURN_IF_FALSE(value.IntegerValueInRange(
          0, 3, *line_idx, &depth_info->disparity_ref_view_id));
    } else if (field_name == "depth_nonlinear_precision") {
      RETURN_IF_FALSE(
          value.IntegerValueInRange(8, 23, *line_idx,
...
bool parse_multilayer_metadata(std::ifstream &file,
  ...
  bool syntax_error;
  *multilayer = {};
  while (parse_line(file, 0, false, &indent,
                    &has_list_prefix, &line_idx, &field_name, &value,
                    &syntax_error)) {
    
    if ((field_name == "use_case")) {
      RETURN_IF_FALSE(value.IntegerValueInRange(
          0, 63, line_idx, &multilayer->use_case));
    } else if ((field_name == "layers")) {
      RETURN_IF_FALSE(parse_multilayer_layer_metadata(
          file,
          indent + 1, &line_idx, multilayer->layers));
    } else {
      fprintf(stderr, "Error: Unknown field %s at line %d\n",
              field_name.c_str(), line_idx);
      return false;
