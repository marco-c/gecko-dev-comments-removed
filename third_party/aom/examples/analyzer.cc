...
class AnalyzerFrame : public wxFrame {
 ...
 public:
  AnalyzerFrame(const bool bit_accounting);  

  void onOpen(wxCommandEvent &event);   
  void onClose(wxCommandEvent &event);  
  void onQuit(wxCommandEvent &event);   

  void onTogglePadding(wxCommandEvent &event);  
...
void Analyzer::OnInitCmdLine(wxCmdLineParser &parser) {  
  parser.SetDesc(CMD_LINE_DESC);
  parser.SetSwitchChars(_("-"));
}

bool Analyzer::OnCmdLineParsed(wxCmdLineParser &parser) {  
