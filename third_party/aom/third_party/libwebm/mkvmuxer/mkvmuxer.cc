...
void VideoTrack::set_colour_space(const char* colour_space) {
  if (colour_space) {
    ...
    colour_space_ = new (std::nothrow) char[length];  
    if (colour_space_) {
...
bool Chapters::Write(IMkvWriter* writer) const {
  ...
  const uint64_t payload_size = WriteEdition(NULL);  
...
bool Tag::ExpandSimpleTagsArray() {
  ...
  SimpleTag* const simple_tags = new (std::nothrow) SimpleTag[size];  
  if (simple_tags == NULL)
    return false;

  for (int idx = 0; idx < simple_tags_count_; ++idx) {
    simple_tags[idx] = simple_tags_[idx];  
  }
...
bool Segment::SetChunking(bool chunking, const char* filename) {
  ...
  if (chunking) {
    ...
    char* const temp = new (std::nothrow) char[filename_length + 1];  
    if (!temp)
      return false;
