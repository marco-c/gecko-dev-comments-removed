...
template <typename T, size_t N>
HWY_INLINE Vec128<T, N> Add(hwy::NonFloatTag , Vec128<T, N> a,
                            Vec128<T, N> b) {
  for (size_t i = 0; i < N; ++i) {
...
template <typename T, size_t N>
HWY_INLINE Vec128<T, N> Sub(hwy::NonFloatTag , Vec128<T, N> a,
                            Vec128<T, N> b) {
  for (size_t i = 0; i < N; ++i) {
    const uint64_t a64 = static_cast<uint64_t>(a.raw[i]);
    const uint64_t b64 = static_cast<uint64_t>(b.raw[i]);
...
template <typename T, size_t N>
HWY_INLINE Vec128<T, N> Add(hwy::FloatTag , Vec128<T, N> a,
                            Vec128<T, N> b) {
  for (size_t i = 0; i < N; ++i) {
    a.raw[i] += b.raw[i];
  }
...
template <typename T, size_t N>
HWY_INLINE Vec128<T, N> Sub(hwy::FloatTag , Vec128<T, N> a,
                            Vec128<T, N> b) {
  for (size_t i = 0; i < N; ++i) {
    a.raw[i] -= b.raw[i];
  }
...
template <typename T, size_t N>
HWY_INLINE Vec128<T, N> Min(hwy::NonFloatTag , Vec128<T, N> a,
                            Vec128<T, N> b) {
  for (size_t i = 0; i < N; ++i) {
...
template <typename T, size_t N>
HWY_INLINE Vec128<T, N> Max(hwy::NonFloatTag , Vec128<T, N> a,
                            Vec128<T, N> b) {
  for (size_t i = 0; i < N; ++i) {
...
template <typename T, size_t N>
HWY_INLINE Vec128<T, N> Min(hwy::FloatTag , Vec128<T, N> a,
                            Vec128<T, N> b) {
  for (size_t i = 0; i < N; ++i) {
...
template <typename T, size_t N>
HWY_INLINE Vec128<T, N> Max(hwy::FloatTag , Vec128<T, N> a,
                            Vec128<T, N> b) {
  for (size_t i = 0; i < N; ++i) {
    if (ScalarIsNaN(a.raw[i])) {
      a.raw[i] = b.raw[i];
...
template <typename T, size_t N>
HWY_INLINE Vec128<T, N> Mul(hwy::FloatTag , Vec128<T, N> a,
                            Vec128<T, N> b) {
  for (size_t i = 0; i < N; ++i) {
    a.raw[i] *= b.raw[i];
  }
...
template <typename T, size_t N>
HWY_INLINE Vec128<T, N> Mul(UnsignedTag , Vec128<T, N> a,
                            Vec128<T, N> b) {
  for (size_t i = 0; i < N; ++i) {
    a.raw[i] = static_cast<T>(static_cast<uint64_t>(a.raw[i]) *
                              static_cast<uint64_t>(b.raw[i]));
...
HWY_API VFromD<DI32> DemoteToNearestInt(DI32 ,
...
template <class D>
HWY_API MFromD<D> Lt128Upper(D , Vec128<uint64_t> a,
                             Vec128<uint64_t> b) {
...
template <class D>
HWY_API Mask128<uint64_t> Ne128(D , Vec128<uint64_t> a,
                                Vec128<uint64_t> b) {
...
template <class D>
HWY_API MFromD<D> Eq128Upper(D , Vec128<uint64_t> a,
                             Vec128<uint64_t> b) {
...
template <class D>
HWY_API MFromD<D> Ne128Upper(D , Vec128<uint64_t> a,
                             Vec128<uint64_t> b) {
...
HWY_API VFromD<DTo> ConvertTo(hwy::FloatTag , DTo ,
...
template <typename TFrom, typename DTo>
HWY_API VFromD<DTo> ConvertTo(hwy::NonFloatTag , DTo ,
                              Vec128<TFrom, HWY_MAX_LANES_D(DTo)> from) {
  using ToT = TFromD<DTo>;
  static_assert(sizeof(ToT) == sizeof(TFrom), "Should have same size");
  VFromD<DTo> ret;
...
HWY_API V InterleaveEvenBlocks(D, V a, V ) {
  return a;
}
