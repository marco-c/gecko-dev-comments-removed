...
HWY_INLINE V FastTan(D d, V x) {
  ...
  if constexpr (CanLookup8(d)) {
    ...
    if constexpr (HWY_REGISTERS >= 32) {
      ...
      auto b_low = Set(d, static_cast<T>(0.133808575986231942));  
      ...
      auto b_high = Set(d, static_cast<T>(9.45968454580926554));  
...
HWY_INLINE V FastTanh(D d, V val) {
  ...
  if constexpr (CanLookup8(d)) {
    ...
    if constexpr (HWY_REGISTERS >= 32) {
      ...
      auto a_low = Set(d, static_cast<T>(-0.0600814996891072));  
      ...
      auto a_high = Set(d, static_cast<T>(8.15222218981581e-06));  
...
HWY_INLINE V FastLog10(D d, V x) {
  ...
  const auto kLog10_2 = Set(d, static_cast<T>(0.3010299956639812));  
