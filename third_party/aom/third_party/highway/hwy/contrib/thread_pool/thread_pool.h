...
class Stats {
 ...
 public:
  ...
  void NotifyThreadRun(size_t worker_idx, timer::Ticks d_wait, size_t wait_reps,
    ...
    PerThread(thread_idx, kTBeforeRun) = t_before_run;  
...
class Shared {
 public:
  static HWY_CONTRIB_DLLEXPORT Shared& Get();  
...
struct WaitBlock {
  ...
  template <class Spin>
  size_t UntilWoken(const Worker& worker, const Spin& ) const {
...
class alignas(HWY_ALIGNMENT) ThreadPool {
 ...
 public:
  ...
  void Run(uint64_t begin, uint64_t end, pool::Caller caller,
    ...
    pool::Config next = auto_tuner.NextConfig();  
    if (auto_tuner.Best()) {  
 ...
 private:
  ...
  struct MainWakeAndBarrier {
    ...
    void operator()(const Spin& spin, const Wait& wait, pool::Worker& main,
      ...
      HWY_DASSERT(&main == main.AllWorkers());  
