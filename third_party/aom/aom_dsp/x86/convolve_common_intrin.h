...
static INLINE void prepare_coeffs_12tap(const InterpFilterParams *filter_params,
  ...
  coeffs[0] = _mm_shuffle_epi32(coeffs_y, 0);    
  coeffs[1] = _mm_shuffle_epi32(coeffs_y, 85);   
  coeffs[2] = _mm_shuffle_epi32(coeffs_y, 170);  
  coeffs[3] = _mm_shuffle_epi32(coeffs_y, 255);  

  coeffs_y = _mm_loadl_epi64((__m128i *)(y_filter + 8));

  coeffs[4] = _mm_shuffle_epi32(coeffs_y, 0);  
  coeffs[5] =
      _mm_shuffle_epi32(coeffs_y, 85);  
}
...
static INLINE __m128i convolve_lo_x_12tap(const __m128i *s,
  ...
  ss[1] = _mm_unpacklo_epi8(s[1], zero);  
  ss[2] = _mm_unpacklo_epi8(s[2], zero);  
  ss[3] = _mm_unpacklo_epi8(s[3], zero);  
  ss[4] = _mm_unpackhi_epi8(s[2], zero);  
  ss[5] = _mm_unpackhi_epi8(s[3], zero);  
