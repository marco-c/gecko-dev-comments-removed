...
DECLARE_ALIGNED(16, const uint8_t, kTblIdx0_3[16]) = {
...
static AOM_FORCE_INLINE void warp_affine_horizontal_neon_i8mm(
...
void av1_warp_affine_neon_i8mm(const int32_t *mat, const uint8_t *ref,
  ...
  for (int i = 0; i < p_height; i += 8) {
    for (int j = 0; j < p_width; j += 8) {
      ...
      int16x8_t tmp[15];
      warp_affine_horizontal_neon_i8mm(ref, width, height, stride, p_width,
                                       p_height, alpha, beta, x4, y4, i, tmp);
