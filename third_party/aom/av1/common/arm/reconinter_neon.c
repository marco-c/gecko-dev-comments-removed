  ...
  if (w >= 16) {
    ...
    do {
      ...
      do {
        ...
        uint8x16_t m;
        if (inverse) {
          m = vqsubq_u8(vdupq_n_u8(64 - 38), diff);  
        } else {
    ...
    do {
      ...
      uint8x8_t m;
      if (inverse) {
        m = vqsub_u8(vdup_n_u8(64 - 38), diff_u8);  
      } else {
...
void av1_build_compound_diffwtd_mask_d16_neon(
  ...
  if (mask_type == DIFFWTD_38) {
    diffwtd_mask_d16_neon(mask, false, src0, src0_stride, src1,
                          src1_stride, h, w, conv_params, bd);
  } else {  
    diffwtd_mask_d16_neon(mask, true, src0, src0_stride, src1,
                          src1_stride, h, w, conv_params, bd);
  }
}
...
void av1_build_compound_diffwtd_mask_neon(uint8_t *mask,
  ...
  if (mask_type == DIFFWTD_38) {
    diffwtd_mask_neon(mask, false, src0, src0_stride, src1,
                      src1_stride, h, w);
  } else {  
    diffwtd_mask_neon(mask, true, src0, src0_stride, src1,
                      src1_stride, h, w);
  }
}
