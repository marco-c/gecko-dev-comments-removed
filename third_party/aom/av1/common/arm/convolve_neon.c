...
static INLINE void convolve_x_sr_12tap_neon(const uint8_t *src_ptr,
  ...
  do {
    ...
    do {
      ...
      int16x4_t s1 = vext_s16(s0, s4, 1);    
      int16x4_t s2 = vext_s16(s0, s4, 2);    
      int16x4_t s3 = vext_s16(s0, s4, 3);    
      int16x4_t s5 = vext_s16(s4, s8, 1);    
      int16x4_t s6 = vext_s16(s4, s8, 2);    
      int16x4_t s7 = vext_s16(s4, s8, 3);    
      int16x4_t s9 = vext_s16(s8, s12, 1);   
      int16x4_t s10 = vext_s16(s8, s12, 2);  
      int16x4_t s11 = vext_s16(s8, s12, 3);  
...
void av1_convolve_x_sr_neon(const uint8_t *src, int src_stride, uint8_t *dst,
  ...
  while (h-- != 0) {
    ...
    do {
      uint8x8_t t8 = vld1_u8(s);  
      int16x8_t s8 = vreinterpretq_s16_u16(vmovl_u8(t8));

      int16x8_t s1 = vextq_s16(s0, s8, 1);  
      int16x8_t s2 = vextq_s16(s0, s8, 2);  
      int16x8_t s3 = vextq_s16(s0, s8, 3);  
      int16x8_t s4 = vextq_s16(s0, s8, 4);  
      int16x8_t s5 = vextq_s16(s0, s8, 5);  
      int16x8_t s6 = vextq_s16(s0, s8, 6);  
      int16x8_t s7 = vextq_s16(s0, s8, 7);  
...
static INLINE void convolve_2d_sr_horiz_8tap_neon(
  ...
  do {
    ...
    uint8x8_t t0 = vld1_u8(s);  
    int16x8_t s0 = vreinterpretq_s16_u16(vmovl_u8(t0));

    do {
      uint8x8_t t1 = vld1_u8(s + 8);  
      int16x8_t s8 = vreinterpretq_s16_u16(vmovl_u8(t1));
