...
static void iadst4_sse2(const __m128i *input, __m128i *output) {
  ...
  __m128i x1[16];
  x1[0] = _mm_madd_epi16(u[0], sinpi_p01_p04);  
  x1[1] = _mm_madd_epi16(u[1], sinpi_p01_p04);
  x1[2] = _mm_madd_epi16(u[0], sinpi_p02_m01);  
  x1[3] = _mm_madd_epi16(u[1], sinpi_p02_m01);
  x1[4] = _mm_madd_epi16(u[2], sinpi_p03_p02);  
  x1[5] = _mm_madd_epi16(u[3], sinpi_p03_p02);
  x1[6] = _mm_madd_epi16(u[2], sinpi_p03_m04);  
  x1[7] = _mm_madd_epi16(u[3], sinpi_p03_m04);
  x1[8] = _mm_madd_epi16(u[0], sinpi_p03_m03);  
  x1[9] = _mm_madd_epi16(u[1], sinpi_p03_m03);
  x1[10] = _mm_madd_epi16(u[2], sinpi_0_p03);  
  x1[11] = _mm_madd_epi16(u[3], sinpi_0_p03);
  x1[12] = _mm_madd_epi16(u[0], sinpi_p04_p02);  
  x1[13] = _mm_madd_epi16(u[1], sinpi_p04_p02);
  x1[14] = _mm_madd_epi16(u[2], sinpi_m03_m01);  
  x1[15] = _mm_madd_epi16(u[3], sinpi_m03_m01);

  __m128i x2[8];
  x2[0] = _mm_add_epi32(x1[0], x1[4]);  
  x2[1] = _mm_add_epi32(x1[1], x1[5]);
  x2[2] = _mm_add_epi32(x1[2], x1[6]);  
  x2[3] = _mm_add_epi32(x1[3], x1[7]);
  x2[4] = _mm_add_epi32(x1[8], x1[10]);  
  x2[5] = _mm_add_epi32(x1[9], x1[11]);
  x2[6] = _mm_add_epi32(x1[12], x1[14]);  
...
static void iadst4_w4_sse2(const __m128i *input, __m128i *output) {
  ...
  __m128i x1[8];
  x1[0] = _mm_madd_epi16(u[0], sinpi_p01_p04);  
  x1[1] = _mm_madd_epi16(u[0], sinpi_p02_m01);  
  x1[2] = _mm_madd_epi16(u[1], sinpi_p03_p02);  
  x1[3] = _mm_madd_epi16(u[1], sinpi_p03_m04);  
  x1[4] = _mm_madd_epi16(u[0], sinpi_p03_m03);  
  x1[5] = _mm_madd_epi16(u[1], sinpi_0_p03);    
  x1[6] = _mm_madd_epi16(u[0], sinpi_p04_p02);  
  x1[7] = _mm_madd_epi16(u[1], sinpi_m03_m01);  

  __m128i x2[4];
  x2[0] = _mm_add_epi32(x1[0], x1[2]);  
  x2[1] = _mm_add_epi32(x1[1], x1[3]);  
  x2[2] = _mm_add_epi32(x1[4], x1[5]);  
  x2[3] = _mm_add_epi32(x1[6], x1[7]);  
...
static INLINE void lowbd_inv_txfm2d_add_no_identity_ssse3(
  ...
  for (int i = 0; i < buf_size_nonzero_h_div8; i++) {
    ...
    load_buffer_32bit_to_16bit(input + 8 * i, input_stride, buf0,
                               buf_size_nonzero_w);
    if (rect_type == 1 || rect_type == -1) {
      round_shift_ssse3(buf0, buf0, buf_size_nonzero_w);  
    }
    row_txfm(buf0, buf0);
...
static void lowbd_inv_txfm2d_add_4x8_ssse3(const int32_t *input,
  ...
  round_shift_ssse3(buf, buf, txfm_size_col);  
  row_txfm(buf, buf);
