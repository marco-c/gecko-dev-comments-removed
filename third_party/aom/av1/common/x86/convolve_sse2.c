...
static INLINE void prepare_coeffs(const InterpFilterParams *const filter_params,
  ...
  coeffs[0] = _mm_unpacklo_epi64(tmp_0, tmp_0);  
  coeffs[1] = _mm_unpackhi_epi64(tmp_0, tmp_0);  
  coeffs[2] = _mm_unpacklo_epi64(tmp_1, tmp_1);  
  coeffs[3] = _mm_unpackhi_epi64(tmp_1, tmp_1);  
}
...
static void convolve_y_sr_12tap_sse2(const uint8_t *src, int src_stride,
  ...
  do {
    ...
    do {
      ...
      res_lo = convolve_lo_y_12tap(s, coeffs);  
      res_hi = convolve_hi_y_12tap(s, coeffs);  
      ...
      res_lo = convolve_lo_y_12tap(s + 1, coeffs);  
      res_hi = convolve_hi_y_12tap(s + 1, coeffs);  
    ...
    if (w <= 4) {
      ...
      do {
        ...
        do {
          ...
          res_lo = convolve_lo_y(s, coeffs);  
          res_hi = convolve_hi_y(s, coeffs);  
          ...
          res_lo = convolve_lo_y(s + 1, coeffs);  
          res_hi = convolve_hi_y(s + 1, coeffs);  
