...
#if CONFIG_LV_MAP
typedef struct {
  int base_ctx_table[2 ][2 ][2 ]
                    [BASE_CONTEXT_POSITION_NUM + 1];
} LV_MAP_CTX_TABLE;
typedef int BASE_CTX_TABLE[2 ][2 ]
                          [BASE_CONTEXT_POSITION_NUM + 1];
#endif
...
typedef struct AV1Common {
  ...
  int ref_frame_sign_bias[TOTAL_REFS_PER_FRAME]; 
  ...
  aom_bit_depth_t dequant_bit_depth;  
  ...
  int log2_tile_cols;                        
  int log2_tile_rows;                        
  int tile_col_start_sb[MAX_TILE_COLS + 1];  
  int tile_row_start_sb[MAX_TILE_ROWS + 1];  
#if CONFIG_DEPENDENT_HORZTILES
  int tile_row_independent[MAX_TILE_ROWS];  
#endif
#else
