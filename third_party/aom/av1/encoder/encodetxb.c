...
static const int HBT_EOB = 16;            
static const int HBT_TABLE_SIZE = 65536;  
static const int HBT_ARRAY_LENGTH = 256;  
...
static INLINE int get_br_cost(tran_low_t abs_qc, int ctx,
  ...
  if (abs_qc >= min_level) {
    if (abs_qc >= max_level) {
      return coeff_lps[COEFF_BASE_RANGE];  
    } else {
      return coeff_lps[(abs_qc - min_level)];  
    }
  }
  return 0;
}
...
void hbt_init() {
  ...
  av1_crc32c_calculator_init(&crc_calculator);  
...
int hbt_create_hashes(TxbInfo *txb_info, const LV_MAP_COEFF_COST *txb_costs,
  ...
  for (int i = 0; i < txb_info->eob; i++) {
    ...
    assert((int8_t)HBT_KICKOUT == HBT_KICKOUT);  
