...
TEST(EncodeAPI, InvalidUVStrides) {
  ...
  for (const auto img_fmt : kAv1ImageFormats) {
    ...
    ASSERT_EQ(aom_img_wrap(&img, img_fmt, kWidth, kHeight, 1,
                           buf.get()),
              &img)
    ...
    for (cfg.monochrome = 0; cfg.monochrome <= max_monochrome;
      ...
      for (const auto uv_stride : kUVStrides) {
        ...
        const aom_codec_err_t expected_err =
            (cfg.monochrome || img_fmt == AOM_IMG_FMT_NV12)
                ? AOM_CODEC_OK
                : AOM_CODEC_INVALID_PARAM;
        EXPECT_EQ(aom_codec_encode(&enc, &img, 0, 1,
                                   0),
                  expected_err)
      ...
      EXPECT_EQ(
          aom_codec_encode(&enc, nullptr, 0, 0,
                           0),
          AOM_CODEC_OK);
      EXPECT_EQ(aom_codec_destroy(&enc), AOM_CODEC_OK);
    }
...
TEST(EncodeAPI, AomediaIssue3509VbrMinSection2Percent) {
  ...
  ASSERT_EQ(aom_codec_encode(&enc, image, 0, 300, 0),
            AOM_CODEC_OK);
...
TEST(EncodeAPI, Buganizer392929025) {
  ...
  EXPECT_EQ(
      aom_codec_encode(&enc, image, 0, 1, 0),
      AOM_CODEC_INVALID_PARAM);
...
TEST(EncodeAPI, Buganizer487259772NoThreads) {
  ReproBuganizer487259772(false, 1);
}



TEST(EncodeAPI, DISABLED_Buganizer487259772NoRowMT) {
  ReproBuganizer487259772(false);
}

TEST(EncodeAPI, Buganizer487259772RowMT) {
  ReproBuganizer487259772(true);
}
...
TEST_P(EncodeAPIParameterized, HighBDEncoderHighBDFrames) {
  ...
  ASSERT_EQ(
      ...
      AOM_CODEC_OK);
  ASSERT_EQ(
      aom_codec_encode(&enc, image, 1, 1, 0),
      AOM_CODEC_OK);
...
TEST(EncodeAPI, PerFramePsnr) {
  ...
  ASSERT_EQ(
      aom_codec_encode(&enc, image, 0, 1, psnr_flags),
      AOM_CODEC_OK);
  ...
  ASSERT_EQ(
      aom_codec_encode(&enc, image, 1, 1, no_psnr_flags),
      AOM_CODEC_OK);
...
TEST(EncodeAPI, FreezeInternalState) {
  ...
  ASSERT_EQ(aom_codec_encode(&enc, image, 0, 1,
                             AOM_EFLAG_FORCE_KF),
            AOM_CODEC_OK);
  ...
  ASSERT_EQ(aom_codec_encode(&enc, image, 1, 1,
  ...
  ASSERT_EQ(aom_codec_encode(&enc, image, 2, 1,
                             0),
            AOM_CODEC_OK);
  while (aom_codec_get_cx_data(&enc, &iter) != nullptr) {
...
TEST(EncodeAPI, FreezeInternalStateTemporalLayers) {
  ...
  ref_frame_config.reference[0] = 1;
  ref_frame_config.ref_idx[0] = 0;
  ASSERT_EQ(aom_codec_encode(&enc, image, 3, 1,
                             0),
            AOM_CODEC_OK);
  while (aom_codec_get_cx_data(&enc, &iter) != nullptr) {
...
TEST(EncodeAPI, FreezeInternalStateNotAllowedWithNonZeroLag) {
  ...
  ASSERT_EQ(aom_codec_encode(
                &enc, image, 0, 1,
                AOM_EFLAG_FREEZE_INTERNAL_STATE | AOM_EFLAG_FORCE_KF),
            AOM_CODEC_INCAPABLE);
...
TEST(EncodeAPI, SizeAlignOverflow) {
  ...
  aom_image_t *img =
      aom_img_alloc_with_border(NULL, AOM_IMG_FMT_NV12, 16, 16,
                                32,
                                32,
                                15);
  ASSERT_NE(img, nullptr);
...
TEST(EncodeAPI, DynamicSvcTemporalIssue502735235) {
  ...
  ASSERT_NE(raw, nullptr);
  FillImage(raw, 128);
  ASSERT_EQ(aom_codec_encode(&codec, raw, 0, 1,
                             0),
            AOM_CODEC_OK);
  aom_img_free(raw);
  ...
  ASSERT_NE(raw, nullptr);
  FillImage(raw, 128);
  ASSERT_EQ(aom_codec_encode(&codec, raw, 1, 1,
                             AOM_EFLAG_FORCE_KF),
            AOM_CODEC_OK);
  ASSERT_EQ(aom_codec_encode(&codec, raw, 2, 1,
                             0),
            AOM_CODEC_OK);
  aom_img_free(raw);
  ...
  ASSERT_NE(raw, nullptr);
  FillImage(raw, 128);
  ASSERT_EQ(aom_codec_encode(&codec, raw, 3, 1,
                             0),
            AOM_CODEC_OK);
  aom_img_free(raw);
