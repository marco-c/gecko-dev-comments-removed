...
static struct ggml_dyn_tallocr * ggml_dyn_tallocr_new(size_t alignment, size_t max_buffer_size) {
    ...
    *alloc = (struct ggml_dyn_tallocr) {
         alignment,
         MIN(max_buffer_size, SIZE_MAX/2), 
         {NULL},
         0,
#ifdef GGML_ALLOCATOR_DEBUG
         {{0}},
...
void ggml_gallocr_reserve_n_size(
    ...
    GGML_ASSERT(ggml_gallocr_reserve_n_impl(galloc, graph, node_buffer_ids, leaf_buffer_ids,  true));
    for (int i = 0; i < galloc->n_buffers; i++) {
        sizes[i] = 0;
        for (int c = 0; c < galloc->buf_tallocs[i]->n_chunks; c++) {
...
bool ggml_gallocr_reserve_n(ggml_gallocr_t galloc, struct ggml_cgraph * graph, const int * node_buffer_ids, const int * leaf_buffer_ids) {
    return ggml_gallocr_reserve_n_impl(galloc, graph, node_buffer_ids, leaf_buffer_ids,  false);
}
...
size_t ggml_backend_alloc_ctx_tensors_from_buft_size(struct ggml_context * ctx, ggml_backend_buffer_type_t buft) {
    ...
    ggml_backend_buffer_t buf = ggml_backend_alloc_ctx_tensors_from_buft_impl(ctx, buft, &nbytes_total,  true);
    GGML_ASSERT(!buf);
    return nbytes_total;
}
...
ggml_backend_buffer_t ggml_backend_alloc_ctx_tensors_from_buft(struct ggml_context * ctx, ggml_backend_buffer_type_t buft) {
    ...
    return ggml_backend_alloc_ctx_tensors_from_buft_impl(ctx, buft, &nbytes_total,  false);
}
