...
bool llama_batch_allocr::init(
    ...
    if (debug > 0) {
        ...
        llama_ubatch ubatch {
             false,
             (uint32_t) batch.n_tokens,
             (uint32_t) 1,
             (uint32_t) batch.n_tokens,
             (uint32_t) this->seq_id_unq.size(),
             batch.token,
             batch.embd,
             batch.pos,
             batch.n_seq_id,
             batch.seq_id,
             this->seq_id_unq.data(),
             this->seq_idx.data(),
             batch.logits,
             {},
        };
...
llama_ubatch llama_batch_allocr::ubatch_add(const std::vector<int32_t> & idxs, uint32_t n_seqs, bool equal_seqs) {
    ...
    llama_ubatch res {
         ...
         n_tokens/n_seqs,
         n_seqs,
         (uint32_t) udata->seq_id_unq.size(),

         batch.token ? udata->token.data() : nullptr,
         batch.embd ? udata->embd.data() : nullptr,
         udata->pos.data(),
         udata->n_seq_id.data(),
         udata->seq_id.data(),
         udata->seq_id_unq.data(),
