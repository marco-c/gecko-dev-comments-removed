...
llama_model_gemma3n::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    
    {
        ...
        ggml_tensor * altup_added =
            ggml_mul_mat(ctx0, model.altup_proj, inp_repeated);  
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        {
            first_prediction = ggml_view_2d_slice(ctx0, corrected, i_altup_act);          
    ...
    {
        ggml_tensor * target_magnitude = calc_magnitude(ggml_view_2d_slice(ctx0, cur, i_altup_act));  
        ...
        ggml_tensor * altup_unembd =
            ggml_mul_mat(ctx0, model.altup_unembd_proj, alt_slice);  
        ...
        cur = ggml_view_2d_slice(ctx0, cur, 0);  
...
ggml_tensor * llama_model_gemma3n::graph::altup_predict(ggml_tensor * cur, int il) {
    ggml_tensor * activated  = ggml_view_2d_slice(ctx0, cur, i_altup_act);      
