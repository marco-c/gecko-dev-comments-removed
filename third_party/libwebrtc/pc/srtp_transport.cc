...
bool SrtpTransport::SendRtpPacket(rtc::CopyOnWriteBuffer* packet,
  ...
  packet->SetSize(len);
  return SendPacket(false, packet, updated_options, flags);
}
...
void SrtpTransport::OnWritableState(
  ...
  SignalWritableState(IsWritable(false) && IsWritable(true));
}
...
void SrtpTransport::MaybeUpdateWritableState() {
  bool writable = IsWritable(true) && IsWritable(false);
