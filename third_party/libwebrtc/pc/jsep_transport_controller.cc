...
RTCError JsepTransportController::SetLocalDescription(
  ...
  return ApplyDescription_n(true, type, local_desc, remote_desc);
}
...
RTCError JsepTransportController::SetRemoteDescription(
  ...
  RTC_DCHECK_RUN_ON(network_thread_);
  return ApplyDescription_n(false, type, local_desc, remote_desc);
}
...
RTCError JsepTransportController::MaybeCreateJsepTransport(
  ...
  rtc::scoped_refptr<IceTransportInterface> ice =
      CreateIceTransport(content_info.name, false);
  ...
  if (config_.rtcp_mux_policy !=
          PeerConnectionInterface::kRtcpMuxPolicyRequire &&
      content_info.type == cricket::MediaProtocolType::kRtp) {
    rtcp_ice = CreateIceTransport(content_info.name, true);
    rtcp_dtls_transport =
