...
static const int kTestKeyGcm128Len = 28;  
...
static const int kTestKeyGcm256Len = 44;  
  ...
  void TestRtpAuthParams(SrtpTransport* transport, int crypto_suite) {
    ...
    switch (crypto_suite) {
      case rtc::kSrtpAes128CmSha1_32:
        EXPECT_EQ(32 / 8, overhead);  
        break;
      case rtc::kSrtpAes128CmSha1_80:
        EXPECT_EQ(80 / 8, overhead);  
        break;
      default:
...
TEST_F(SrtpTransportTest, RemoveSrtpReceiveStream) {
  ...
  auto srtp_transport =
      std::make_unique<SrtpTransport>(true, field_trials);
