...
void PassFramesThroughAnalyzerSenderOnly(
  ...
  for (int i = 0; i < frames_count; ++i) {
    for (absl::string_view stream_label : stream_labels) {
      VideoFrame frame = NextFrame(&frame_generator, 1);
      uint16_t frame_id =
...
TEST(DefaultVideoQualityAnalyzerTest,
  ...
  VideoFrame to_be_dropped_frame =
      NextFrame(frame_generator.get(), 1);
  uint16_t frame_id =
...
TEST(DefaultVideoQualityAnalyzerTest, CheckFrameSenderPeerName) {
  ...
  VideoFrame frame_alice = NextFrame(frame_generator.get(), 1);
  VideoFrame frame_bob = NextFrame(frame_generator.get(), 2);
  frame_alice.set_id(
