...
TEST_F(FramesStorageTest, CanGetAllAddedFrames) {
  VideoFrame frame1 = Create2x2Frame(1);
  VideoFrame frame2 = Create2x2Frame(2);
  VideoFrame frame3 = Create2x2Frame(3);
  VideoFrame frame4 = Create2x2Frame(4);
  VideoFrame frame5 = Create2x2Frame(5);

  FramesStorage storage(TimeDelta::Seconds(1), GetClock());

  storage.Add(frame1, NowPlusSeconds(1));
  storage.Add(frame2, NowPlusSeconds(2));
  storage.Add(frame3, NowPlusSeconds(3));
  storage.Add(frame4, NowPlusSeconds(2));
  storage.Add(frame5, NowPlusSeconds(1));

  AssertHasFrame(storage, 1);
  AssertHasFrame(storage, 2);
  AssertHasFrame(storage, 3);
  AssertHasFrame(storage, 4);
  AssertHasFrame(storage, 5);
}
...
TEST_F(FramesStorageTest, AutoCleanupRemovesOnlyOldFrames) {
  ...
  VideoFrame frame4 = Create2x2Frame(4);
  VideoFrame frame5 = Create2x2Frame(5);
  VideoFrame frame6 = Create2x2Frame(6);

  FramesStorage storage(TimeDelta::Seconds(1), GetClock());

  storage.Add(frame1, NowPlusSeconds(0));
  storage.Add(frame2, NowPlusSeconds(1));
  storage.Add(frame3, NowPlusSeconds(2));
  storage.Add(frame4, NowPlusSeconds(1));
  storage.Add(frame5, NowPlusSeconds(0));

  AdvanceTime(TimeDelta::Millis(1001));
  storage.Add(frame6, NowPlusSeconds(3));

  AssertHasFrame(storage, 2);
  AssertHasFrame(storage, 3);
  AssertHasFrame(storage, 4);
  EXPECT_FALSE(storage.Get(1).has_value());
  EXPECT_FALSE(storage.Get(5).has_value());
}
...
TEST_F(FramesStorageTest, AllFramesAutoCleaned) {
  ...
  storage.Add(frame1, NowPlusSeconds(0));
  storage.Add(frame2, NowPlusSeconds(0));
  storage.Add(frame3, NowPlusSeconds(0));
  ...
  EXPECT_FALSE(storage.Get(3).has_value());
}
