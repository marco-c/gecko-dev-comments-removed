...
class SSLStreamAdapterTestDTLSCertChain : public SSLStreamAdapterTestDTLSBase {
 public:
  ...
  void SetUp() override {
    ...
    client_ssl_->SetInitialRetransmissionTimeout(1000);
    server_ssl_->SetInitialRetransmissionTimeout(1000);
...
TEST_F(SSLStreamAdapterTestDTLS, TestDTLSSrtpExporter) {
  ...
  EXPECT_TRUE(GetDtlsSrtpCryptoSuite(false, &selected_crypto_suite));
  int key_len;
  int salt_len;
  ASSERT_TRUE(
