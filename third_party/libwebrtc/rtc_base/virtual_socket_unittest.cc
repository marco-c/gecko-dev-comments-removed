...
class VirtualSocketServerTest : public ::testing::Test {
 public:
  ...
  void ConnectTest(const SocketAddress& initial_addr) {
    ...
    EXPECT_NE(client->GetLocalAddress(), kEmptyAddr);          
    EXPECT_NE(AF_UNSPEC, client->GetLocalAddress().family());  
  ...
  void BandwidthTest(const SocketAddress& initial_addr) {
    ...
    EXPECT_TRUE(receiver.count <= 6 * bandwidth);  
...
TEST_F(VirtualSocketServerTest, CreatesStandardDistribution) {
  ...
  const uint32_t kTestSamples[] = { 1000};
