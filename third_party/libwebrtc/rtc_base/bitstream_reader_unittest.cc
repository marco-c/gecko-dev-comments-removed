...
TEST(BitstreamReaderTest,
  ...
  EXPECT_EQ(reader.ReadNonSymmetric(1 << 4), 0xfu);
  EXPECT_EQ(reader.ReadNonSymmetric(1 << 4), 0x3u);
  EXPECT_EQ(reader.ReadNonSymmetric(1 << 4), 0xau);
  EXPECT_EQ(reader.ReadNonSymmetric(1 << 4), 0x0u);
...
TEST(BitstreamReaderTest, ReadNonSymmetricOnlyValueConsumesZeroBits) {
  ...
  ASSERT_EQ(reader.RemainingBitCount(), 16);
  EXPECT_EQ(reader.ReadNonSymmetric(1), 0u);
