...
TEST(NtpTimeTest, VerifyInt64MsToUQ32x32NearZero) {
  ...
  EXPECT_EQ(Int64MsToUQ32x32(-1), uint64_t{0});  

  
  EXPECT_EQ(Int64MsToUQ32x32(1000), uint64_t{kOneSecQ32x32});

  
  EXPECT_EQ(Int64MsToUQ32x32(-1000), uint64_t{0});  
}
...
TEST(NtpTimeTest, VerifyInt64MsToQ32x32NearMax) {
  ...
  EXPECT_EQ(Int64MsToQ32x32(kBoundaryMs + 1), kMaxQ32x32);  

  
  EXPECT_LE(Int64MsToQ32x32(kBoundaryMs - 1), kMaxQ32x32 - kOneMsQ32x32);

  
  EXPECT_EQ(Int64MsToQ32x32(kBoundaryMs + 1000), kMaxQ32x32);  
...
TEST(NtpTimeTest, VerifyInt64MsToUQ32x32NearMax) {
  ...
  EXPECT_EQ(Int64MsToUQ32x32(kBoundaryMs + 1), kMaxUQ32x32);  

  
  EXPECT_LE(Int64MsToUQ32x32(kBoundaryMs - 1), kMaxUQ32x32 - kOneMsQ32x32);

  
  EXPECT_EQ(Int64MsToUQ32x32(kBoundaryMs + 1000), kMaxUQ32x32);  
...
TEST(NtpTimeTest, VerifyInt64MsToQ32x32NearMin) {
  ...
  EXPECT_EQ(Int64MsToQ32x32(kBoundaryMs - 1), kBoundaryQ32x32);  
  ...
  EXPECT_EQ(Int64MsToQ32x32(kBoundaryMs - 1000), kBoundaryQ32x32);  
}
