...
class MockVideoEncoder : public VideoEncoder {
 public:
  ...
  int32_t InitEncode(const VideoCodec* codecSettings,
                     const VideoEncoder::Settings& ) override {
...
class TestSimulcastEncoderAdapterFake : public ::testing::Test,
 ...
 public:
  ...
  void SetupCodec() { SetupCodec({true, true, true}); }
...
TEST_F(TestSimulcastEncoderAdapterFake,
  ...
  encoders[1]->SendEncodedImage(640, 360, 100, 0);
...
TEST_F(TestSimulcastEncoderAdapterFake,
  ...
  encoders[0]->SendEncodedImage(1280, 720, 100, 0);
  ASSERT_TRUE(GetLastEncodedImageInfo(&width, &height, &last_simulcast_index));
...
TEST_F(TestSimulcastEncoderAdapterFake, CreatesEncoderOnlyIfStreamIsActive) {
  ...
  ReSetUp();
  helper_->factory()->set_supports_simulcast(false);
  SetupCodec({true, false});
  EXPECT_EQ(1u, helper_->factory()->encoders().size());

  
  ReSetUp();
  helper_->factory()->set_supports_simulcast(false);
  SetupCodec({false, true});
  EXPECT_EQ(1u, helper_->factory()->encoders().size());
}
...
TEST_F(TestSimulcastEncoderAdapterFake,
  ...
  allocation.SetBitrate(1, 0, 100000);  
  allocation.SetBitrate(1, 1, 200000);  
