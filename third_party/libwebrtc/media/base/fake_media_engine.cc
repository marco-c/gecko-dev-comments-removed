...
bool FakeVoiceMediaSendChannel::GetStats(VoiceMediaSendInfo* ) {
  return false;
}
...
bool FakeVideoMediaSendChannel::GetStats(VideoMediaSendInfo* ) {
  return false;
}
...
void FakeVideoMediaReceiveChannel::SetRecordableEncodedFrameCallback(
    uint32_t ,
    std::function<void(const webrtc::RecordableEncodedFrame&)> ) {
}

void FakeVideoMediaReceiveChannel::ClearRecordableEncodedFrameCallback(
    uint32_t ) {}

void FakeVideoMediaReceiveChannel::RequestRecvKeyFrame(uint32_t ) {}

bool FakeVideoMediaReceiveChannel::GetStats(VideoMediaReceiveInfo* ) {
  return false;
}
...
bool FakeVoiceEngine::StartAecDump(webrtc::FileWrapper ,
                                   int64_t ) {
  return false;
}
...
std::vector<Codec> FakeVideoEngine::LegacySendCodecs(bool ) const {
  return send_codecs_;
}

std::vector<Codec> FakeVideoEngine::LegacyRecvCodecs(bool ) const {
  return recv_codecs_;
}
