...
TEST(SctpPacketTest, SerializeAndDeserializeSingleChunk) {
  ...
  InitChunk init(VerificationTag(123), 456,
                 65535,
                 65534, TSN(789),
                 Parameters());
...
TEST(SctpPacketTest, ReturnsCorrectSpaceAvailableToStayWithinMTU) {
  ...
  EXPECT_EQ(builder.bytes_remaining(), 976u);  
  ...
  EXPECT_EQ(builder.bytes_remaining(), 0u);  
}
