...
void UDPPort::OnLocalAddressReady(AsyncPacketSocket* ,
                                  const SocketAddress& address) {
...
void UDPPort::PostAddAddress(bool ) {
  MaybeSetPortCompleteOrError();
}
...
void UDPPort::OnSentPacket(AsyncPacketSocket* ,
                           const SentPacketInfo& sent_packet) {
  NotifySentPacket(sent_packet);
}

void UDPPort::OnReadyToSend(AsyncPacketSocket* ) {
  Port::OnReadyToSend();
}
...
void UDPPort::SendStunRequest(const void* data, size_t size, StunRequest* req) {
  ...
  SendTo(data, size, sreq->server_addr(), options, true);
