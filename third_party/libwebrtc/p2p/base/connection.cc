...
Connection::Connection(const Environment& env,
    ...
    : env_(env),
      ...
      recv_rate_tracker_(TimeDelta::Seconds(1)),
      send_rate_tracker_(TimeDelta::Seconds(1)),
...
void Connection::HandleStunBindingOrGoogPingRequest(IceMessage* msg) {
  ...
  if (msg->type() == STUN_BINDING_REQUEST) {
    MaybeHandleDtlsPiggybackingAttributes(msg, nullptr);
