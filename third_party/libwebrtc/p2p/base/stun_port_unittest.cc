...
class StunPortTestBase : public ::testing::Test, public sigslot::has_slots<> {
 ...
 protected:
  ...
  void OnPortComplete(cricket::Port* ) {
    ASSERT_FALSE(done_);
    done_ = true;
    error_ = false;
  }
  void OnPortError(cricket::Port* ) {
    done_ = true;
    error_ = true;
  }
  void OnCandidateError(cricket::Port* ,
  ...
  SetDnsResolverExpectations(
      [](webrtc::MockAsyncDnsResolver* resolver,
        ...
        EXPECT_CALL(*resolver, Start(kValidHostnameAddr, AF_INET, _))
            .WillOnce([](const rtc::SocketAddress& , int ,
  ...
  SetDnsResolverExpectations(
      [](webrtc::MockAsyncDnsResolver* resolver,
        ...
        EXPECT_CALL(*resolver,
                    Start(kValidHostnameAddr, AF_INET6, _))
