...
class DtlsTestClient {
 public:
  ...
  void OnTransportReadPacket(PacketTransportInternal* ,
                             const ReceivedIpPacket& packet) {
  ...
  void OnTransportSentPacket(PacketTransportInternal* ,
  ...
  void OnFakeIceTransportReadPacket(PacketTransportInternal* ,
                                    const ReceivedIpPacket& packet) {
  ...
  void AddPacketLogging() {
    ...
    client1_.fake_ice_transport()->set_packet_send_filter(
        [&](auto data, auto len, auto options, auto flags) {
          return LogSend(client1_.name(), false, data, len);
        });
    client2_.fake_ice_transport()->set_packet_send_filter(
        [&](auto data, auto len, auto options, auto flags) {
          return LogSend(client2_.name(), false, data, len);
        });
  }
...
TEST_F(DtlsTransportInternalImplTest, TestWriteError) {
  ...
  AsyncSocketPacketOptions packet_options;
  EXPECT_EQ(client1_.SendPacket(1000, false, packet_options), -1);
}
...
TEST_F(DtlsTransportInternalImplTest, TestPacketOptionsResetAfterWriteError) {
  ...
  AsyncSocketPacketOptions packet_options;
  packet_options.packet_id = kFakePacketId;
  ASSERT_LT(client1_.SendPacket(1000, false, packet_options), 0);
  ...
  ASSERT_GT(client1_.SendPacket(1000, false, packet_options_2), 0);
...
class DtlsTransportInternalImplVersionTest
 ...
 public:
  ...
  std::pair< int, std::vector<HandshakeTestEvent>>
    ...
    client1_.fake_ice_transport()->set_packet_send_filter(
        [&](auto data, auto len, auto options, auto flags) {
          ...
          if (packet_type == PacketType::kIceConnectivityCheck ||
            ...
            return LogSend("client-stun",  false, data, len);
          }
    ...
    client2_.fake_ice_transport()->set_packet_send_filter(
        [&](auto data, auto len, auto options, auto flags) {
          ...
          if (packet_type == PacketType::kIceConnectivityCheck ||
            ...
            return LogSend("server-stun",  false, data, len);
          }
          bool drop = packets_to_drop.find(packet_num) != packets_to_drop.end();
          packet_num++;
          if (!drop) {
...
TEST_P(DtlsTransportInternalImplVersionTest, HandshakeLoseFirstClientPacket) {
  ...
  Prepare();
  auto [dtls_version_bytes, events] = RunHandshake({ 0});
...
TEST_P(DtlsTransportInternalImplVersionTest,
  ...
  Prepare();
  auto [dtls_version_bytes, events] = RunHandshake({ 1});
...
TEST_P(DtlsTransportInternalImplVersionTest, HandshakeLoseSecondClientPacket) {
  ...
  Prepare();
  auto [dtls_version_bytes, events] = RunHandshake({ 2});
...
TEST_P(DtlsTransportLastHandshakePacketTest,
  ...
  client2_.SendPackets(size, count,  false);
...
TEST_P(DtlsTransportLastHandshakePacketTest,
  ...
  client2_.ExpectPackets(size);
  client1_.SendPackets(size, count,  false);
