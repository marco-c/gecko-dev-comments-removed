...
static const int kDelayMetricsAggregationWindow = 1250;  
...
static const float cnScaleHband = 0.4f;  
...
static const int kDelayCorrectionStart = 1500;  
#endif
  ...
  if (level->framelevel.EndOfBlock()) {
    ...
    if (new_frame_level > 0) {
      if (new_frame_level < level->minlevel) {
        level->minlevel = new_frame_level;  
      } else {
