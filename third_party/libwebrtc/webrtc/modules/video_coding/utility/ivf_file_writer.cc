...
bool IvfFileWriter::WriteHeader() {
  ...
  ivf_header[2] = 'I';
  ivf_header[3] = 'F';
  ByteWriter<uint16_t>::WriteLittleEndian(&ivf_header[4], 0);   
  ByteWriter<uint16_t>::WriteLittleEndian(&ivf_header[6], 32);  
  ...
  ByteWriter<uint32_t>::WriteLittleEndian(&ivf_header[28], 0);  
