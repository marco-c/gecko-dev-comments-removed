...
rtcp::ExtendedReports EventGenerator::NewExtendedReports() {
  ...
  target_bitrate.AddTargetBitrate( prng_.Rand(0, 3),
                                   prng_.Rand(0, 3),
                                   prng_.Rand(0, 50000));
  target_bitrate.AddTargetBitrate( prng_.Rand(4, 7),
                                   prng_.Rand(4, 7),
                                   prng_.Rand(0, 50000));
...
rtcp::Fir EventGenerator::NewFir() {
  ...
  fir.AddRequestTo( prng_.Rand<uint32_t>(),
                    prng_.Rand<uint8_t>());
  fir.AddRequestTo( prng_.Rand<uint32_t>(),
                    prng_.Rand<uint8_t>());
  return fir;
...
std::unique_ptr<RtcEventRtpPacketIncoming> EventGenerator::NewRtpPacketIncoming(
  ...
  const bool padding = prng_.Rand(0, 9) == 0;  
...
void EventVerifier::VerifyLoggedAlrStateEvent(
  ...
  EXPECT_THAT(
      ...
      Near(original_event.timestamp(), TimeDelta::Millis(1)));
...
void EventVerifier::VerifyLoggedSenderReport(
  ...
  EXPECT_THAT(logged_sr.log_time(),
              Near(log_time, TimeDelta::Millis(1)));
