...
void RtcEventLogSession::WriteAudioRecvConfigs(size_t audio_recv_streams,
  ...
  for (size_t i = 0; i < audio_recv_streams; i++) {
    ...
    RtpHeaderExtensionMap extensions = gen_.NewRtpHeaderExtensionMap(
        false,
        {RtpDependencyDescriptorExtension::kId});
...
void RtcEventLogSession::ReadAndVerifyLog() {
  ...
  EXPECT_THAT(parsed_log.first_timestamp(),
              Near(first_timestamp_, TimeDelta::Millis(1)));
  EXPECT_THAT(parsed_log.last_timestamp(),
              Near(last_timestamp_, TimeDelta::Millis(1)));
  ...
  EXPECT_THAT(parsed_log.first_log_segment().stop_time(),
              Near(stop_time_, TimeDelta::Millis(1)));
}
