...
"""
  ...
  def GenStartIds(self):
    ...
    for item in self._item_list:  
      ...
      if trace:
        if trace[-1].old_id >= node.old_id:  
          if num_join:
            raise ValueError('Cannot join on jump: %d' % node.old_id)
          jumps.append(trace[-1])  
          while trace and trace[-1].old_id > node.old_id:  
            trace.pop()
          if not trace or trace[-1].old_id != node.old_id:  
            raise ValueError('Cannot jump to unvisited: %d' % node.old_id)
          node.new_id = trace.pop().new_id  
        else:  
          node.new_id = trace[-1].new_id + trace[-1].weight  
          if num_join:  
            for _ in range(1, num_join):
              t = jumps.pop()
              node.new_id = max(node.new_id, t.new_id + t.weight)  
      else:
...
def GenerateNewIds(item_list, use_naive):
  ...
  for item, start_id in coarse_id_assigner.GenStartIds():  
    cur_id = start_id
    for tag, next_id in quota_assigner.Gen(item, start_id):  
      yield tag, cur_id
      cur_id = next_id
