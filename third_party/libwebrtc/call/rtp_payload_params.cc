  ...
  switch (codec_specific_info->codecType) {
    case VideoCodecType::kVideoCodecGeneric:
      if (simulate_generic_structure_) {
        return MinimalisticStructure(1,
                                     1);
      }
      return std::nullopt;
    case VideoCodecType::kVideoCodecVP8:
...
void RtpPayloadParams::H264ToGeneric(const CodecSpecificInfoH264& h264_info,
  ...
  if (is_keyframe) {
    ...
    last_frame_id_[ 0].fill(-1);
    last_frame_id_[ 0][temporal_index] = frame_id;
    return;
  }

  if (h264_info.base_layer_sync) {
    int64_t tl0_frame_id = last_frame_id_[ 0][0];

    for (int i = 1; i < RtpGenericFrameDescriptor::kMaxTemporalLayers; ++i) {
      if (last_frame_id_[ 0][i] < tl0_frame_id) {
        last_frame_id_[ 0][i] = -1;
      }
    }
    ...
    for (int i = 0; i <= temporal_index; ++i) {
      int64_t last_frame_id = last_frame_id_[ 0][i];
...
void RtpPayloadParams::Vp9ToGeneric(const CodecSpecificInfoVP9& ,
                                    int64_t frame_id,
                                    RTPVideoHeader& rtp_video_header) {
