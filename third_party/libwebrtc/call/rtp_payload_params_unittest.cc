...
TEST(RtpPayloadParamsTest, PictureIdWraps) {
  ...
  EXPECT_EQ(0, params.state().picture_id);  
...
TEST(RtpPayloadParamsTest, CreatesGenericDescriptorForVp8) {
  ...
  RTPVideoHeader key_frame_header = params.GetRtpVideoHeader(
      key_frame_image, &key_frame_info, 123);
  ...
  RTPVideoHeader delta_t1_header = params.GetRtpVideoHeader(
      delta_t1_image, &delta_t1_info, 124);
  ...
  RTPVideoHeader delta_t0_header = params.GetRtpVideoHeader(
      delta_t0_image, &delta_t0_info, 125);
...
TEST(RtpPayloadParamsTest, SetsGenericFromGenericFrameInfo) {
  ...
  RTPVideoHeader key_header =
      params.GetRtpVideoHeader(encoded_image, &codec_info, 1);
  ...
  RTPVideoHeader delta_header =
      params.GetRtpVideoHeader(encoded_image, &codec_info, 3);
...
TEST(RtpPayloadParamsVp9ToGenericTest, NoScalability) {
  RtpPayloadState state;
  RtpPayloadParams params(CreateTestEnvironment(), 123, &state);
...
TEST(RtpPayloadParamsVp9ToGenericTest, TemporalScalabilityWith2Layers) {
  ...
  headers[0] = params.GetRtpVideoHeader(image, &info, 1);
  ...
  headers[1] = params.GetRtpVideoHeader(image, &info, 3);

  info.codecSpecific.VP9.temporal_up_switch = false;
  info.codecSpecific.VP9.temporal_idx = 0;
  info.codecSpecific.VP9.num_ref_pics = 1;
  info.codecSpecific.VP9.p_diff[0] = 2;
  headers[2] = params.GetRtpVideoHeader(image, &info, 5);
  ...
  info.codecSpecific.VP9.num_ref_pics = 1;
  info.codecSpecific.VP9.p_diff[0] = 2;
  headers[4] = params.GetRtpVideoHeader(image, &info, 9);

  info.codecSpecific.VP9.temporal_up_switch = true;
  info.codecSpecific.VP9.temporal_idx = 1;
  info.codecSpecific.VP9.num_ref_pics = 1;
  info.codecSpecific.VP9.p_diff[0] = 1;
  headers[5] = params.GetRtpVideoHeader(image, &info, 11);
  ...
  EXPECT_THAT(headers[0].generic->dependencies, IsEmpty());          
  EXPECT_THAT(headers[1].generic->dependencies, ElementsAre(1));     
  EXPECT_THAT(headers[2].generic->dependencies, ElementsAre(1));     
  EXPECT_THAT(headers[3].generic->dependencies, ElementsAre(5, 3));  
  EXPECT_THAT(headers[4].generic->dependencies, ElementsAre(5));     
  EXPECT_THAT(headers[5].generic->dependencies, ElementsAre(9));     
...
TEST(RtpPayloadParamsVp9ToGenericTest, TemporalScalabilityWith3Layers) {
  ...
  headers[6] = params.GetRtpVideoHeader(image, &info, 13);

  info.codecSpecific.VP9.temporal_up_switch = true;
  info.codecSpecific.VP9.temporal_idx = 2;
  info.codecSpecific.VP9.num_ref_pics = 1;
  info.codecSpecific.VP9.p_diff[0] = 1;
  headers[7] = params.GetRtpVideoHeader(image, &info, 15);

  info.codecSpecific.VP9.temporal_up_switch = true;
  info.codecSpecific.VP9.temporal_idx = 0;
  info.codecSpecific.VP9.num_ref_pics = 1;
  info.codecSpecific.VP9.p_diff[0] = 4;
  headers[8] = params.GetRtpVideoHeader(image, &info, 17);
  ...
  EXPECT_THAT(headers[1].generic->dependencies, ElementsAre(1));     
  EXPECT_THAT(headers[2].generic->dependencies, ElementsAre(1));     
  EXPECT_THAT(headers[3].generic->dependencies, ElementsAre(5));     
  EXPECT_THAT(headers[4].generic->dependencies, ElementsAre(1));     
  EXPECT_THAT(headers[5].generic->dependencies, ElementsAre(9, 5));  
  EXPECT_THAT(headers[6].generic->dependencies, ElementsAre(9, 5));  
  EXPECT_THAT(headers[7].generic->dependencies, ElementsAre(13));    
  EXPECT_THAT(headers[8].generic->dependencies, ElementsAre(9));     
...
TEST(RtpPayloadParamsVp9ToGenericTest, SpatialScalabilityKSvc) {
  ...
  EXPECT_THAT(headers[1].generic->dependencies, ElementsAre(1));  
  EXPECT_THAT(headers[2].generic->dependencies, ElementsAre(1));  
  EXPECT_THAT(headers[3].generic->dependencies, ElementsAre(3));  
...
TEST(RtpPayloadParamsVp9ToGenericTest,
  ...
  EXPECT_THAT(headers[0].generic->dependencies, IsEmpty());       
  EXPECT_THAT(headers[1].generic->dependencies, ElementsAre(1));  
  EXPECT_THAT(headers[2].generic->dependencies, ElementsAre(3));  
...
TEST(RtpPayloadParamsVp9ToGenericTest, ChangeFirstActiveLayer) {
  ...
  info.codecSpecific.VP9.inter_pic_predicted = false;
  info.codecSpecific.VP9.num_ref_pics = 0;
  headers[0] = params.GetRtpVideoHeader(image, &info, 0);
  ...
  info.codecSpecific.VP9.inter_pic_predicted = false;
  info.codecSpecific.VP9.num_ref_pics = 0;
  headers[1] = params.GetRtpVideoHeader(image, &info, 1);
  ...
  info.codecSpecific.VP9.num_ref_pics = 1;
  info.codecSpecific.VP9.p_diff[0] = 1;
  headers[2] = params.GetRtpVideoHeader(image, &info, 2);
  ...
  info.codecSpecific.VP9.num_ref_pics = 1;
  info.codecSpecific.VP9.p_diff[0] = 1;
  headers[3] = params.GetRtpVideoHeader(image, &info, 3);
  ...
  info.codecSpecific.VP9.inter_pic_predicted = false;
  info.codecSpecific.VP9.num_ref_pics = 0;
  headers[4] = params.GetRtpVideoHeader(image, &info, 4);
  ...
  info.codecSpecific.VP9.num_ref_pics = 1;
  info.codecSpecific.VP9.p_diff[0] = 1;
  headers[5] = params.GetRtpVideoHeader(image, &info, 5);
  ...
  info.codecSpecific.VP9.inter_pic_predicted = false;
  info.codecSpecific.VP9.num_ref_pics = 0;
  headers[6] = params.GetRtpVideoHeader(image, &info, 6);
  ...
  info.codecSpecific.VP9.inter_pic_predicted = false;
  info.codecSpecific.VP9.num_ref_pics = 0;
  headers[7] = params.GetRtpVideoHeader(image, &info, 7);
