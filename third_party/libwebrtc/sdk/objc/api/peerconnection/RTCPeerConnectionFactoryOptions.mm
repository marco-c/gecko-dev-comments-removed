...
- (webrtc::PeerConnectionFactoryInterface::Options)nativeOptions {
  ...
  setNetworkBit(&options,
                webrtc::ADAPTER_TYPE_LOOPBACK,
                self.ignoreLoopbackNetworkAdapter);
  setNetworkBit(
      &options, webrtc::ADAPTER_TYPE_VPN, self.ignoreVPNNetworkAdapter);
  setNetworkBit(&options,
                webrtc::ADAPTER_TYPE_CELLULAR,
                self.ignoreCellularNetworkAdapter);
  setNetworkBit(
      &options, webrtc::ADAPTER_TYPE_WIFI, self.ignoreWiFiNetworkAdapter);
  setNetworkBit(&options,
                webrtc::ADAPTER_TYPE_ETHERNET,
                self.ignoreEthernetNetworkAdapter);
