...
ScopedJavaLocalRef<jbyteArray> NativeToJavaByteArray(
  ...
  int8_t* array_ptr =
      env->GetByteArrayElements(jarray.obj(), nullptr);
  memcpy(array_ptr, container.data(), container.size() * sizeof(int8_t));
  env->ReleaseByteArrayElements(jarray.obj(), array_ptr, 0);
  return jarray;
}
...
ScopedJavaLocalRef<jintArray> NativeToJavaIntArray(
  ...
  int32_t* array_ptr =
      env->GetIntArrayElements(jarray.obj(), nullptr);
  memcpy(array_ptr, container.data(), container.size() * sizeof(int32_t));
  env->ReleaseIntArrayElements(jarray.obj(), array_ptr, 0);
  return jarray;
}
...
std::vector<int8_t> JavaToNativeByteArray(JNIEnv* env,
  ...
  env->ReleaseByteArrayElements(jarray.obj(), array_ptr, JNI_ABORT);
  return container;
}
...
std::vector<int32_t> JavaToNativeIntArray(JNIEnv* env,
  ...
  env->ReleaseIntArrayElements(jarray.obj(), array_ptr, JNI_ABORT);
  return container;
}
