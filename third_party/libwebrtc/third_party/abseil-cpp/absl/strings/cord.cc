...
static inline CordRep* VerifyTree(CordRep* node) {
  ...
  assert(node == nullptr || VerifyNode(node, node, true));
#else   
  assert(node == nullptr || VerifyNode(node, node, false));
#endif  
...
void Cord::Prepend(absl::string_view src) {
  ...
  if (!contents_.is_tree() && cur_size + src.size() <= InlineRep::kMaxInline) {
    ...
    data[InlineRep::kMaxInline] = cur_size + src.size();  
...
static CordRep* NewSubRange(CordRep* node, size_t pos, size_t n) {
  ...
  do {
    ...
    if (node == nullptr) {
      ...
      todo.push_back(SubRange(nullptr, 0, 0));  
...
static bool VerifyNode(CordRep* root, CordRep* start_node,
 ...
 size_t Cord::MemoryUsageAux(const CordRep* rep) {
