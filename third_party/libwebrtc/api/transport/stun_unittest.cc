...
TEST_F(StunTest, EqualAttributes) {
  ...
  EXPECT_TRUE(copy->EqualAttributes(&msg, [](int ) { return true; }));

  {
    ...
    msg.AddAttribute(std::move(attr));
    EXPECT_FALSE(
        copy->EqualAttributes(&msg, [](int ) { return true; }));
...
TEST_F(StunTest, ByteStringAsVectorOfUint32) {
  ...
  EXPECT_THAT(*integrity_vector,
              ::testing::ElementsAreArray(expected_integrity_vector));
}
