...
TEST(PclfVideoSubscriptionTest, WhenSpecIsNotSetFieldsAreCompared) {
  VideoResolution test_resolution(1, 2,
                                  3);
  VideoResolution equal_resolution(1, 2,
                                   3);
  VideoResolution different_width(10, 2,
                                  3);
  VideoResolution different_height(1, 20,
                                   3);
  VideoResolution different_fps(1, 20,
                                30);
...
TEST(PclfVideoSubscriptionTest, GetMaxResolutionSelectMaxForEachDimention) {
  VideoConfig max_width(1000, 1, 1);
  VideoConfig max_height(1, 100, 1);
  VideoConfig max_fps(1, 1, 10);
...
struct TestVideoFrameWriter : public test::VideoFrameWriter {
 public:
  ...
  bool WriteFrame(const VideoFrame& ) override { return true; }
...
TEST(VideoDumpOptionsTest, InputVideoWriterHasCorrectFileName) {
  VideoResolution resolution(1280, 720, 30);

  TestVideoFrameWriter* writer = nullptr;
  VideoDumpOptions options("foo", 1,
                           false,
