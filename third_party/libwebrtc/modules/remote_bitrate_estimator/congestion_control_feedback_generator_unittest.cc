...
RtpPacketReceived CreatePacket(
    ...
    uint32_t ssrc = 1234,
    uint16_t seq = 1,
    rtc::EcnMarking  = rtc::EcnMarking::kNotEct) {
...
TEST(CongestionControlFeedbackGeneratorTest,
  ...
  generator.OnReceivedPacket(
      CreatePacket(clock.CurrentTime(), false));
  generator.OnReceivedPacket(
      CreatePacket(clock.CurrentTime(), true));
}
...
TEST(CongestionControlFeedbackGeneratorTest,
  ...
  const std::vector<RtpPacketReceived> kReceivedPackets = {
      ...
      CreatePacket(clock.CurrentTime() + kSmallTimeInterval,  false,
                   123,
                   0xFFFA),
      CreatePacket(clock.CurrentTime(),  false, 123,
                   1),
      ...
      CreatePacket(clock.CurrentTime() + kSmallTimeInterval,
                   ...
                   234,
                   0xFFFC),
      CreatePacket(clock.CurrentTime(),  false, 234,
                   1),
  };
...
TEST(CongestionControlFeedbackGeneratorTest,
  ...
  RtpPacketReceived packet_1 =
      CreatePacket(clock.CurrentTime(), false, 1,
                   2, rtc::EcnMarking::kEct1);
