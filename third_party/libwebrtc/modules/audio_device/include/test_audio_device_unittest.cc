...
TEST(WavFileReaderTest, RepeatedTrueWithSingleFrameFileReadTwice) {
...

  {
    ...
    BufferT<int16_t> buffer =
        BufferT<int16_t>::CreateUninitializedWithSize(kExpectedSamples.size());
...

  {
    std::unique_ptr<TestAudioDeviceModule::Capturer> reader =
        TestAudioDeviceModule::CreateRawFileReader(
            ...
            2, false);
    BufferT<int16_t> buffer =
        BufferT<int16_t>::CreateUninitializedWithSize(expected_samples.size());
...
class TestAudioTransport : public AudioTransport {
 public:
  ...
  int32_t RecordedDataIsAvailable(const void* ,
                                  size_t ,
                                  size_t ,
                                  size_t ,
                                  uint32_t ,
