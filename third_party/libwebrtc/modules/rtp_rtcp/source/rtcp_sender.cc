...
void RTCPSender::SetSendingStatus(const FeedbackState& ,
                                  bool sending) {
...
int32_t RTCPSender::SendLossNotification(const FeedbackState& feedback_state,
  ...
  std::optional<PacketSender> sender;
  {
    ...
    SetFlag(kRtcpLossNotification, true);
...
void RTCPSender::BuildSDES(const RtcpContext& , PacketSender& sender) {
...
void RTCPSender::BuildPLI(const RtcpContext& , PacketSender& sender) {
  rtcp::Pli pli;
...
void RTCPSender::BuildFIR(const RtcpContext& , PacketSender& sender) {
...
void RTCPSender::BuildREMB(const RtcpContext& , PacketSender& sender) {
  rtcp::Remb remb;
...
void RTCPSender::BuildTMMBN(const RtcpContext& ,
                            PacketSender& sender) {
  rtcp::Tmmbn tmmbn;
...
void RTCPSender::BuildAPP(const RtcpContext& , PacketSender& sender) {
  rtcp::App app;
  app.SetSenderSsrc(ssrc_);
  sender.AppendPacket(app);
}

void RTCPSender::BuildLossNotification(const RtcpContext& ,
                                       PacketSender& sender) {
...
void RTCPSender::BuildBYE(const RtcpContext& , PacketSender& sender) {
  rtcp::Bye bye;
