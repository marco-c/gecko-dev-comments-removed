...
constexpr uint8_t kPacketWithTwoByteExtensionIdLast[] = {
    ...
    kTwoByteExtensionId, 0x03, 0x00, 0x30,  
    0x22, 0x00, 0x00, 0x00};                
...
void TestCreateAndParseColorSpaceExtension(bool with_hdr_metadata) {
  ...
  RtpPacket::ExtensionManager extensions(true);
...
struct UncopyableExtension {
  ...
  static size_t ValueSize(const UncopyableValue& ) { return 1; }
  static bool Write(rtc::ArrayView<uint8_t> ,
                    const UncopyableValue& ) {
    return true;
  }
  static bool Parse(rtc::ArrayView<const uint8_t> ,
                    UncopyableValue* ) {
    return true;
  }
};
...
struct ParseByReferenceExtension {
  ...
  static size_t ValueSize(uint8_t , uint8_t ) {
    return 2;
  }
...
TEST(RtpPacketTest, CreateAndParseColorSpaceExtension) {
  TestCreateAndParseColorSpaceExtension(true);
}

TEST(RtpPacketTest, CreateAndParseColorSpaceExtensionWithoutHdrMetadata) {
  TestCreateAndParseColorSpaceExtension(false);
}
