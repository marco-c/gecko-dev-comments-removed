...
TEST(VideoRtpDepacketizerVp9Test, ParseRefIdx) {
  ...
  uint8_t packet[13] = {0};
  packet[0] = 0xD8;  
  packet[1] = 0x80 | ((kPictureId >> 8) & 0x7F);  
  packet[2] = kPictureId;
  packet[3] = (kPdiff1 << 1) | 1;  
  packet[4] = (kPdiff2 << 1) | 1;  
  packet[5] = (kPdiff3 << 1) | 0;  
...
TEST(VideoRtpDepacketizerVp9Test, ParseSsData) {
  ...
  uint8_t packet[23] = {0};
  packet[0] = 0x0A;  
  packet[1] = ((kNs - 1) << 5) | (kYbit << 4) | (1 << 3);  
  packet[2] = kNg;                                         
  packet[3] = (0 << 5) | (1 << 4) | (0 << 2) | 0;          
  packet[4] = (2 << 5) | (0 << 4) | (1 << 2) | 0;          
  packet[5] = 33;
