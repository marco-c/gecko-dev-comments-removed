...
class LoopbackTransportTest : public Transport {
 public:
  ...
  bool SendRtcp(ArrayView<const uint8_t> ,
                const PacketOptions& ) override {
    return false;
  }
...
TEST_F(RtpSenderVideoTest,
  ...
  EXPECT_NEAR(message.std_dev(), 2.0392156862745097, 0.041);  
...
TEST_F(RtpSenderVideoTest,
  ...
  rtp_module_.SetStorePacketsStatus(true, 10);
  ...
  EXPECT_TRUE(rtp_sender_video_->SendVideo(
      kMediaPayloadId, kVideoCodecVP8, 0,
      Timestamp::Seconds(1), kPayload, sizeof(kPayload),
      header,
  ...
  for (size_t frame_size = 800; frame_size < kMaxPacketSize; ++frame_size) {
    ...
    EXPECT_TRUE(rtp_sender_video_->SendVideo(
        kMediaPayloadId, kVideoCodecVP8, 0,
        Timestamp::Seconds(1), payload, frame_size, header,
        TimeDelta::Seconds(1), {}));
