...
TEST(VideoRtpDepacketizerH265Test, EmptyApRejected) {
  uint8_t lone_empty_packet[] = {0x60, 0x02,  
                                 0x00, 0x00};
  uint8_t leading_empty_packet[] = {0x60, 0x02,  
                                    0x00, 0x00, 0x00, 0x05, 0x26,
                                    0x02, 0xFF, 0x00, 0x11};  
  uint8_t middle_empty_packet[] = {0x60, 0x02,  
                                   0x00, 0x04, 0x26, 0x02, 0xFF,
                                   0x00, 0x00, 0x00, 0x00, 0x05,
                                   0x26, 0x02, 0xFF, 0x00, 0x11};  
  uint8_t trailing_empty_packet[] = {0x60, 0x02,  
                                     0x00, 0x04, 0x26,
                                     0x02, 0xFF, 0x00,  
                                     0x00, 0x00};
...
TEST(VideoRtpDepacketizerH265Test, ApDelta) {
  uint8_t packet[20] = {0x60, 0x02,  
                                     
                        0, 0x03, 0x02, 0x02, 0xFF,               
                        0, 0x04, 0x02, 0x02, 0xFF, 0x00,         
                        0, 0x05, 0x02, 0x02, 0xFF, 0x00, 0x11};  
  uint8_t expected_packet[] = {
      0x00, 0x00, 0x00, 0x01, 0x02, 0x02, 0xFF,               
      0x00, 0x00, 0x00, 0x01, 0x02, 0x02, 0xFF, 0x00,         
      0x00, 0x00, 0x00, 0x01, 0x02, 0x02, 0xFF, 0x00, 0x11};  
  CopyOnWriteBuffer rtp_payload(packet);
...
TEST(VideoRtpDepacketizerH265Test, InvalidNaluSizeApNalu) {
  const uint8_t kPayload[] = {0x60, 0x02,  
                                           
                              0, 0xff, 0x02, 0x02, 0xFF,  
...
TEST(VideoRtpDepacketizerH265Test, FuFromNonIdrFrameSecondSlice) {
  ...
  const uint8_t kPayload[] = {0x62, 0x02,  
                              0x80,  
...
TEST(VideoRtpDepacketizerH265Test, EmptyNaluPayload) {
  const uint8_t kPayload[] = {0x48, 0x00};  
  VideoRtpDepacketizerH265 depacketizer;
  std::optional<VideoRtpDepacketizer::ParsedRtpPayload> parsed =
      depacketizer.Parse(CopyOnWriteBuffer(kPayload));
  ASSERT_TRUE(parsed.has_value());
