...
int AudioFormatValidityToErrorCode(AudioFormatValidity validity) {
  switch (validity) {
    ...
    case AudioFormatValidity::kValidButUnsupportedSampleRate:  
...
AudioProcessing::Config AdjustConfig(
  ...
  if (agc1_analog_enabled && gain_controller2_config_override.has_value()) {
    ...
    if (!one_and_only_one_input_volume_controller ||
      ...
      auto& adjusted_adaptive_digital =  
          adjusted_config.gain_controller2.adaptive_digital;
      const auto& adaptive_digital_override =  
...
bool AudioProcessingImpl::SubmoduleStates::CaptureMultiBandProcessingPresent()
  ...
  return CaptureMultiBandProcessingActive(true);
}
...
void AudioProcessingImpl::InitializeLocked() {
  ...
  InitializeGainController2(true);
  InitializeVoiceActivityDetector(true);
