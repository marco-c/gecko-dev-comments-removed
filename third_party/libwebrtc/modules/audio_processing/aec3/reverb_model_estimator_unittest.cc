...
void ReverbModelEstimatorTest::RunEstimator() {
  ...
  estimated_decay_ = estimator.ReverbDecay(false);
  mild_estimated_decay_ = estimator.ReverbDecay(true);
...
TEST(ReverbModelEstimatorTests, NotChangingDecay) {
  ...
  for (size_t num_capture_channels : {1, 2, 4, 8}) {
    ...
    test.RunEstimator();
    EXPECT_EQ(test.GetDecay(false), kDefaultDecay);
    EXPECT_EQ(test.GetDecay(true),
...
TEST(ReverbModelEstimatorTests, ChangingDecay) {
  ...
  for (size_t num_capture_channels : {1, 2, 4, 8}) {
    ReverbModelEstimatorTest test(kDefaultDecay, num_capture_channels);
    test.RunEstimator();
    EXPECT_NEAR(test.GetDecay(false), test.GetTrueDecay(), 0.1f);
    EXPECT_NEAR(test.GetDecay(true), test.GetTrueDecay(), 0.1f);
    EXPECT_NEAR(test.GetPowerTailDb(), test.GetTruePowerTailDb(), 5.f);
  }
}
