...
std::vector<float> RunSubtractorTest(
  ...
  Block y(1, num_capture_channels);
  ...
  for (int k = 0; k < num_blocks_to_process; ++k) {
    for (size_t render_ch = 0; render_ch < num_render_channels; ++render_ch) {
      RandomizeSampleVector(&random_generator, x.View(0, render_ch));
    }
    if (uncorrelated_inputs) {
      ...
      for (size_t capture_ch = 0; capture_ch < num_capture_channels;
           ++capture_ch) {
        rtc::ArrayView<float> y_view = y.View(0, capture_ch);
    ...
    for (size_t ch = 0; ch < num_capture_channels; ++ch) {
      y_hp_filter[ch]->Process(y.View(0, ch));
    }
  ...
  for (size_t ch = 0; ch < num_capture_channels; ++ch) {
    ...
    const float y_power =
        std::inner_product(y.begin(0, ch), y.end(0, ch),
                           y.begin(0, ch), 0.f);
    if (y_power == 0.f) {
      ADD_FAILURE();
      results[ch] = -1.f;
