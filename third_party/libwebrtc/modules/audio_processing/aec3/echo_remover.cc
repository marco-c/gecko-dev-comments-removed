...
void EchoRemoverImpl::ProcessCapture(
  ...
  for (size_t ch = 0; ch < num_capture_channels_; ++ch) {
    ...
    WindowedPaddedFft(fft_, y->View(0, ch), y_old_[ch], &Y[ch]);
  ...
  data_dumper_->DumpRaw("aec3_output", y->View(0, 0));
  data_dumper_->DumpRaw("aec3_narrow_render",
                        render_signal_analyzer_.NarrowPeakBand() ? 1 : 0);
  data_dumper_->DumpRaw("aec3_N2", cng_.NoiseSpectrum()[0]);
  data_dumper_->DumpRaw("aec3_suppressor_gain", G);
  data_dumper_->DumpWav("aec3_output", y->View(0, 0),
                        16000, 1);
  ...
  data_dumper_->DumpRaw(
      "aec3_X2", render_buffer->Spectrum(
                     aec_state_.MinDirectPathFilterDelay())[0]);
