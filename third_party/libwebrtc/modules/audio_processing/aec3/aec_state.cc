...
void ComputeAvgRenderReverb(
  ...
  if (num_render_channels > 1) {
    ...
    X2 = spectrum_buffer.buffer[idx_at_delay][0];
  }
...
void AecState::Update(
  ...
  for (int ch = 0; ch < aligned_render_block.NumChannels(); ++ch) {
    const float render_energy =
        std::inner_product(aligned_render_block.begin(0, ch),
                           aligned_render_block.end(0, ch),
                           aligned_render_block.begin(0, ch), 0.f);
  ...
  ComputeAvgRenderReverb(render_buffer.GetSpectrumBuffer(),
                         ...
                         ReverbDecay(false), &avg_render_reverb_,
  ...
  data_dumper_->DumpRaw("aec3_erle", Erle(false)[0]);
