...
TEST(SuppressionFilter, ComfortNoiseInUnityGain) {
  ...
  std::vector<FftData> E(1);
  fft.PaddedFft(e.View(0, 0), e_old_,
                Aec3Fft::Window::kSqrtHanning, &E[0]);
  std::copy(e.begin(0, 0),
            e.end(0, 0), e_old_.begin());
...
TEST(SuppressionFilter, SignalSuppression) {
  ...
  for (size_t k = 0; k < 100; ++k) {
    ...
    e0_input = std::inner_product(e.begin(0, 0),
                                  e.end(0, 0),
                                  e.begin(0, 0), e0_input);
    ...
    e0_output = std::inner_product(
        e.begin(0, 0), e.end(0, 0),
        e.begin(0, 0), e0_output);
  }
