...
void ApmTest::ReadFrameWithRewind(FILE* ,
                                  Int16FrameData* ,
                                  ChannelBuffer<float>* cb) {
...
void ApmTest::VerifyDebugDumpTest(Format format) {
  ...
  while (ref_size > 0 && out_size > 0) {
    ...
    EXPECT_TRUE(ExpectMessageEq({out_bytes.get(), out_size},
                                {ref_bytes.get(), ref_size}));
    if (limited_size > 0) {
      EXPECT_TRUE(
          ExpectMessageEq({limited_bytes.get(), limited_size},
                          {ref_bytes.get(), ref_size}));
    }
...
#if defined(WEBRTC_AUDIOPROC_FLOAT_PROFILE)
      const int kStatsAggregationFrameNum = 100;  
...
TEST(ApmAnalyzeReverseStreamFormatTest, AnalyzeReverseStream) {
  for (auto&& [input_config, expect_error] :
       {std::tuple(StreamConfig(16000, 2), false),
        std::tuple(StreamConfig(8000, 1), false),
        std::tuple(StreamConfig(384000, 1), false),
        std::tuple(StreamConfig(7900, 1), true),
        std::tuple(StreamConfig(390000, 1), true),
        std::tuple(StreamConfig(16000, 0), true),
        std::tuple(StreamConfig(-16000, 0), true)}) {
