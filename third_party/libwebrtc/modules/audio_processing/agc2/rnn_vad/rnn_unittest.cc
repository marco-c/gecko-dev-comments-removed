...
void WarmUpRnnVad(RnnBasedVad& rnn_vad) {
  for (int i = 0; i < 10; ++i) {
    rnn_vad.ComputeVadProbability(kFeatures, false);
  }
}
...
TEST_P(RnnParametrization, CheckFullyConnectedLayerOutput) {
  FullyConnectedLayer fc(
      ...
      rnnoise::TansigApproximated, GetParam());
...
TEST(RnnVadTest, CheckZeroProbabilityWithSilence) {
  ...
  EXPECT_EQ(rnn_vad.ComputeVadProbability(kFeatures, true), 0.f);
}
...
TEST(RnnVadTest, CheckRnnVadReset) {
  RnnBasedVad rnn_vad(GetAvailableCpuFeatures());
  WarmUpRnnVad(rnn_vad);
  float pre = rnn_vad.ComputeVadProbability(kFeatures, false);
  rnn_vad.Reset();
  WarmUpRnnVad(rnn_vad);
  float post = rnn_vad.ComputeVadProbability(kFeatures, false);
  EXPECT_EQ(pre, post);
}
...
TEST(RnnVadTest, CheckRnnVadSilence) {
  ...
  WarmUpRnnVad(rnn_vad);
  float pre = rnn_vad.ComputeVadProbability(kFeatures, false);
  rnn_vad.ComputeVadProbability(kFeatures, true);
  WarmUpRnnVad(rnn_vad);
  float post = rnn_vad.ComputeVadProbability(kFeatures, false);
  EXPECT_EQ(pre, post);
}
