...
constexpr int kMinQindex = 40;  
constexpr int kUsageProfile = AOM_USAGE_REALTIME;
constexpr int kLowQindex = 145;   
constexpr int kHighQindex = 205;  
...
int LibaomAv1Encoder::InitEncode(const VideoCodec* codec_settings,
  ...
  cfg_.rc_target_bitrate = encoder_settings_.startBitrate;  
  ...
  cfg_.g_lag_in_frames = kLagInFrames;  
  ...
  if (encoder_speed_experiment_.IsDynamicSpeedEnabled()) {
    ...
    if (SvcEnabled()) {
      ...
      speed_controllers_.push_back(EncoderSpeedController::Create(
          speed_config, GetFrameInterval(0)));
    }
  }
...
void LibaomAv1Encoder::SetSvcRefFrameConfig(
  ...
  static constexpr int kPreferedSlotName[] = {0,  
                                              3,  
                                              1, 2, 4, 5, 6};
