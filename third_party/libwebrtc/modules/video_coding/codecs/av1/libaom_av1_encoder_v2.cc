...
bool ValidateEncodeParams(
    ...
    const VideoEncoderInterface::TemporalUnitSettings& ,
...
void PrepareInputImage(const VideoFrameBuffer& input_buffer,
  ...
  if (!out_aom_image || out_aom_image->fmt != input_format ||
    ...
    out_aom_image.reset(
        aom_img_wrap(nullptr, input_format, input_buffer.width(),
                     input_buffer.height(), 1, nullptr));
...
aom_svc_ref_frame_config_t GetSvcRefFrameConfig(
  ...
  static constexpr std::array<int, 7> kPreferedAlias = {0,  
                                                        3,  
                                                        6,  
                                                        1, 2, 4, 5};
  ...
  if (!settings.reference_buffers.empty()) {
    for (size_t i = 0; i < settings.reference_buffers.size(); ++i) {
      ref_idx_view[kPreferedAlias[alias_index]] = settings.reference_buffers[i];
    ...
    for (size_t i = settings.reference_buffers.size(); i < ref_idx_view.size();
         ++i) {
...
aom_svc_params_t GetSvcParams(
  ...
  for (const FrameEncodeSettings& settings : frame_settings) {
    ...
    scaling_factor_den_view[settings.spatial_id] = scaling_factor->denominator;
...
bool LibaomAv1EncoderV2::InitEncode(
  ...
  if (aom_codec_err_t ret =
          aom_codec_enc_init(&ctx_, aom_codec_av1_cx(), &cfg_, 0);
      ret != AOM_CODEC_OK) {
