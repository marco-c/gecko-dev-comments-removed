...
constexpr DecodeTargetIndication kDtis[3][2][4] = {
    ...
    {{kSwitch, kSwitch, kRequired, kRequired},                
     {kNotPresent, kNotPresent, kSwitch, kSwitch}}};          
...
ScalabilityStructureL2T2::NextFrameConfig(bool restart) {
  ...
  if (next_pattern_ == kDeltaT1 &&  
      !DecodeTargetIsActive(0, 1) &&
      !DecodeTargetIsActive(1, 1)) {
  ...
  switch (next_pattern_) {
    ...
    case kDeltaT0: {
      for (int sid = 0; sid < kNumSpatialLayers; ++sid) {
        ...
        if (sid == 1 && DecodeTargetIsActive(0, 0)) {
          config.Reference(BufferIndex(0, 0));
    ...
    case kDeltaT1:
      ...
      if (DecodeTargetIsActive(1, 1)) {
        ...
        config.Id(next_pattern_)
            .S(1)
            .T(1)
            .Reference(BufferIndex(1, 0));
        if (DecodeTargetIsActive(0, 1)) {
          config.Reference(BufferIndex(0, 1));
        }
      }
      next_pattern_ = kDeltaT0;
      break;
