...
void IgnoreResult(PacketBuffer::InsertResult ) {}
...
class PacketBufferTest : public ::testing::Test {
 protected:
  ...
  PacketBufferInsertResult Insert(uint16_t seq_num,  
                                  IsKeyFrame keyframe,  
                                  IsFirst first,  
                                  IsLast last,    
...
TEST_F(PacketBufferTest, PacketTimestamps) {
  ...
  Insert(100, kKeyFrame, kFirst, kLast, {}, 1000);
  packet_ms = packet_buffer_.LastReceivedPacketMs();
  packet_keyframe_ms = packet_buffer_.LastReceivedKeyframePacketMs();
  ...
  Insert(101, kDeltaFrame, kFirst, kLast, {}, 2000);
  packet_ms = packet_buffer_.LastReceivedPacketMs();
  packet_keyframe_ms = packet_buffer_.LastReceivedKeyframePacketMs();
  EXPECT_TRUE(packet_ms);
  EXPECT_TRUE(packet_keyframe_ms);
...
TEST_F(PacketBufferTest,
  ...
  clock_.AdvanceTimeMilliseconds(100);
  Insert(100, kKeyFrame, kFirst, kNotLast, {}, 1000);
  EXPECT_EQ(packet_buffer_.LastReceivedKeyframePacketMs(),
            clock_.TimeInMilliseconds());

  clock_.AdvanceTimeMilliseconds(100);
  Insert(102, kDeltaFrame, kNotFirst, kLast, {},
         1000);
  EXPECT_EQ(packet_buffer_.LastReceivedKeyframePacketMs(),
            clock_.TimeInMilliseconds());

  clock_.AdvanceTimeMilliseconds(100);
  Insert(101, kDeltaFrame, kNotFirst, kNotLast, {},
         1000);
  EXPECT_EQ(packet_buffer_.LastReceivedKeyframePacketMs(),
            clock_.TimeInMilliseconds());

  clock_.AdvanceTimeMilliseconds(100);
  Insert(103, kDeltaFrame, kFirst, kNotLast, {},
         2000);
  EXPECT_EQ(packet_buffer_.LastReceivedKeyframePacketMs(),
