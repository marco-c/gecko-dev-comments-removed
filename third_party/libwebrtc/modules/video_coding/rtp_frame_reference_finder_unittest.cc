...
TEST_F(TestRtpFrameReferenceFinder, ClearTo) {
  ...
  InsertGeneric(sn + 4, sn + 5, false);  
  EXPECT_EQ(1UL, frames_from_callback_.size());

  InsertGeneric(sn + 6, sn + 7, true);  
  EXPECT_EQ(2UL, frames_from_callback_.size());
  reference_finder_->ClearTo(sn + 7);

  InsertGeneric(sn + 8, sn + 9, false);  
...
TEST_F(TestRtpFrameReferenceFinder, Vp8InsertManyFrames_0212) {
  ...
  const int frames_per_keyframe = 120;  
  uint8_t tl0 = 128;
...
TEST_F(TestRtpFrameReferenceFinder, Vp8Tl1SyncFrameAfterTl1Frame) {
  ...
  InsertVp8(1002, 1002, false, 4, 1, 248, false);  
  InsertVp8(1003, 1003, false, 5, 1, 248, true);   
...
TEST_F(TestRtpFrameReferenceFinder, Vp9GofTemporalLayersUpSwitch_02120212) {
  ...
  uint16_t sn = Rand();
  GofInfoVP9 ss;
  ss.SetGofInfoVP9(kTemporalStructureMode4);  
