...
TEST(ScalabilityStructureL3T3KeyTest,
  ...
  structure.OnRatesUpdated(EnableTemporalLayers(3, 3));
  wrapper.GenerateFrames(2, frames);
  EXPECT_THAT(frames, SizeIs(4));
  structure.OnRatesUpdated(EnableTemporalLayers(3, 1));
...
TEST(ScalabilityStructureL3T3KeyTest,
  ...
  auto config = structure.NextFrameConfig(false);
  ASSERT_THAT(config, Not(IsEmpty()));
  ASSERT_EQ(config.front().TemporalId(), 2);

  
  structure.OnRatesUpdated(EnableTemporalLayers(2, 2, 2));
  
  config = structure.NextFrameConfig(false);
  ASSERT_THAT(config, Not(IsEmpty()));
...
TEST(ScalabilityStructureL3T3KeyTest, EnableT2LayerWhileProducingT1Frame) {
  ...
  structure.OnRatesUpdated(EnableTemporalLayers(3, 3, 3));
  ...
  wrapper.GenerateFrames(4, frames);
...
TEST(ScalabilityStructureL3T3KeyTest,
  ...
  structure.OnRatesUpdated(EnableTemporalLayers(2, 2));
  wrapper.GenerateFrames(1, frames);
  EXPECT_THAT(frames, SizeIs(2));
  
  structure.OnRatesUpdated(EnableTemporalLayers(2, 0));
  wrapper.GenerateFrames(1, frames);
...
TEST(ScalabilityStructureL3T3KeyTest, ReenablingSpatialLayerTriggersKeyFrame) {
  ...
  structure.OnRatesUpdated(EnableTemporalLayers(2, 0, 2));
  wrapper.GenerateFrames(2, frames);
