...
class PacingControllerFieldTrialTest
 ...
 protected:
  ...
  MediaStream audio{ RtpPacketMediaType::kAudio,
                     3333,  100,  1000};
  MediaStream video{ RtpPacketMediaType::kVideo,
                     4444,  1000,  1000};
  SimulatedClock clock_;
...
TEST_P(PacingControllerTest, HandlesSubMicrosecondSendIntervals) {
  ...
  pacer_->SetPacingRates(2 * kPacketSize / kPacketSendTime,
                         DataRate::Zero());
  ...
  EXPECT_CALL(callback_, SendPacket).Times(2);
  for (int i = 0; i < 3; ++i) {
    Send(RtpPacketMediaType::kVideo, 12345, i,
         clock_.TimeInMilliseconds(), kPacketSize.bytes());
  }
  pacer_->ProcessPackets();
...
TEST_P(PacingControllerTest, HandlesSubMicrosecondPaddingInterval) {
  ...
  EXPECT_CALL(callback_, SendPacket);
  Send(RtpPacketMediaType::kVideo, 12345, 1234,
       clock_.TimeInMilliseconds(), kPacketSize.bytes());
  pacer_->ProcessPackets();
