...
int main(int argc, char* argv[]) {
  ...
  RTC_CHECK(absl::GetFlag(FLAGS_red) >= 0 &&
            absl::GetFlag(FLAGS_red) <= 127);          
  RTC_CHECK(absl::GetFlag(FLAGS_audio_level) == -1 ||  
            (absl::GetFlag(FLAGS_audio_level) > 0 &&
             absl::GetFlag(FLAGS_audio_level) <= 255));  
  RTC_CHECK(absl::GetFlag(FLAGS_abs_send_time) == -1 ||  
            (absl::GetFlag(FLAGS_abs_send_time) > 0 &&
             absl::GetFlag(FLAGS_abs_send_time) <= 255));  
