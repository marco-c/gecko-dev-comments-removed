...
constexpr size_t kMaxLoopLengthSamples = kSampleRateHz * 50;  
constexpr size_t kInputBlockSizeSamples = 10 * kSampleRateHz / 1000;   
constexpr size_t kOutputBlockSizeSamples = 20 * kSampleRateHz / 1000;  
...
class PowerRatioEstimator : public LappedTransform::Callback {
 ...
 protected:
  void ProcessAudioBlock(const std::complex<float>* const* input,
                         ...
                         size_t ,
                         size_t ,
                         std::complex<float>* const* ) override {
    float low_pow = 0.f;
    float high_pow = 0.f;
