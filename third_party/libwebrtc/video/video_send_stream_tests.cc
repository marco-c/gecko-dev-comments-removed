...
class Vp9HeaderObserver : public test::SendTest {
 ...
 protected:
  ...
  void VerifyTemporalLayerStructure0(const RTPVideoHeaderVP9& vp9) const {
    ...
    EXPECT_EQ(kNoTemporalIdx, vp9.temporal_idx);  
  ...
  void VerifyCommonHeader(const RTPVideoHeaderVP9& vp9) const {
    EXPECT_EQ(kMaxTwoBytePictureId, vp9.max_picture_id);       
    EXPECT_NE(kNoPictureId, vp9.picture_id);                   
    EXPECT_EQ(vp9_settings_.flexibleMode, vp9.flexible_mode);  
    ...
    if (frames_sent_ == 0)
      EXPECT_FALSE(vp9.inter_pic_predicted);  
  ...
  void VerifySsData(const RTPVideoHeaderVP9& vp9) const {
    ...
    EXPECT_EQ(vp9_settings_.numberOfSpatialLayers,  
    ...
    for (int i = static_cast<int>(vp9.num_spatial_layers) - 1; i >= 0; --i) {
      EXPECT_EQ(expected_width, vp9.width[i]);    
      EXPECT_EQ(expected_height, vp9.height[i]);  
      expected_width /= 2;
      expected_height /= 2;
    }
  }
 ...
 public:
  ...
  void ModifyVideoConfigs(
    ...
    if (configure_send_side_ && !has_send_side) {
      ...
      unique_id_generator.AddKnownId(0);  
