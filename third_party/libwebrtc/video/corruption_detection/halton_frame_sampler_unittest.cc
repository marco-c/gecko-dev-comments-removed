    ...
    for (int num_sampled_frames = 0; num_sampled_frames < 8;
      ...
      EXPECT_THAT(halton_frame_sampler
                      .GetSampleCoordinatesForFrameIfFrameShouldBeSampled(
                          false, rtp_timestamp, kNumSamples),
                  Not(IsEmpty()));
      ++rtp_timestamp;
...
TEST(HaltonFrameSamplerTest,
  ...
  halton_frame_sampler.GetSampleCoordinatesForFrameIfFrameShouldBeSampled(
      false, kRtpTimestamp, kNumSamples);
  EXPECT_THAT(
      halton_frame_sampler.GetSampleCoordinatesForFrameIfFrameShouldBeSampled(
          false, kRtpTimestamp + kSufficientDuration,
          kNumSamples),
      Not(IsEmpty()));
  EXPECT_THAT(
      halton_frame_sampler.GetSampleCoordinatesForFrameIfFrameShouldBeSampled(
  ...
  EXPECT_THAT(
      halton_frame_sampler.GetSampleCoordinatesForFrameIfFrameShouldBeSampled(
          false, kRtpTimestamp + 2 * kSufficientDuration,
          kNumSamples),
      Not(IsEmpty()));
}
...
TEST(HaltonFrameSamplerTest,
  ...
  halton_frame_sampler.GetSampleCoordinatesForFrameIfFrameShouldBeSampled(
      false, kRtpTimestamp, 1);
  EXPECT_THAT(
      halton_frame_sampler.GetSampleCoordinatesForFrameIfFrameShouldBeSampled(
          false, kRtpTimestamp + kTooShortDuration,
          1),
      IsEmpty());
}
