...
TEST(UtilsTest, FindCodecFromString) {
  EXPECT_EQ(GetVideoCodecType("VP8"), kVideoCodecVP8);
  EXPECT_EQ(GetVideoCodecType("libvpx-vp9"), kVideoCodecVP9);
  EXPECT_EQ(GetVideoCodecType("ImprovedAV1"), kVideoCodecAV1);
  EXPECT_EQ(GetVideoCodecType("lets_use_h264"), kVideoCodecH264);
}

#if GTEST_HAS_DEATH_TEST
TEST(UtilsTest, IfCodecDoesNotExistRaiseError) {
  EXPECT_DEATH(GetVideoCodecType("Not_a_codec"), _);
}
#endif  
