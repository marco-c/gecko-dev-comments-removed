...
bool DeleteFile(const FilePath& path) {
  return DoDeleteFile(path, false);
}

#if !defined(MOZ_ZUCCHINI)
bool DeletePathRecursively(const FilePath& path) {
  return DoDeleteFile(path, true);
}
...
bool PreReadFile(const FilePath& file_path,
  ...
  return posix_fadvise(fd, 0, len, POSIX_FADV_WILLNEED) == 0;
#elif BUILDFLAG(IS_APPLE)
  File file(file_path, File::FLAG_OPEN | File::FLAG_READ);
  if (!file.IsValid())
    return false;
...
bool CopyFileContentsWithSendfile(File& infile,
  ...
  do {
    ...
    res = HANDLE_EINTR(sendfile(
        outfile.GetPlatformFile(), infile.GetPlatformFile(), nullptr,
        static_cast<size_t>(file_size) - copied));
    if (res <= 0) {
      break;
    }
