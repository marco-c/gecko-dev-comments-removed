...
OnceClosure GetDeleteFileCallbackInternal(
  ...
  return BindOnce(&DeleteFileWithRetry, path, recursive, 0,
...
OnceClosure GetDeleteFileCallback(const FilePath& path,
                                  OnceCallback<void(bool)> reply_callback) {
  return GetDeleteFileCallbackInternal(path, false,
                                       std::move(reply_callback));
}
...
OnceClosure GetDeletePathRecursivelyCallback(
    const FilePath& path,
    OnceCallback<void(bool)> reply_callback) {
  return GetDeleteFileCallbackInternal(path, true,
                                       std::move(reply_callback));
}
#endif  
...
bool DeleteFile(const FilePath& path) {
  return DeleteFileOrSetLastError(path, false);
}

bool DeletePathRecursively(const FilePath& path) {
  return DeleteFileOrSetLastError(path, true);
}
...
File FILEToFile(FILE* file_stream) {
  ...
  if (!::DuplicateHandle(
          ...
          GetCurrentProcess(), &other_handle,
          0,
          FALSE,
...
bool PreventExecuteMapping(const FilePath& path) {
  ...
  return win::DenyAccessToPath(path, *sids, FILE_EXECUTE, 0,
                               false);
}
