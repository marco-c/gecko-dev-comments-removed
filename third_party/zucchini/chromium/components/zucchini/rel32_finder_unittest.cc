...
TEST(Rel32FinderX86Test, Accept) {
  constexpr uint8_t data[] = {
      0xB9, 0x00, 0x00, 0x00, 0xE9,  
      0xE8, 0x00, 0x00, 0x00, 0xE9,  
      0xE8, 0x00, 0x00, 0x00, 0xE9,  
  };
  ...
  EXPECT_EQ(0x05U, next_location(rel_finder));  
  rel_finder.Accept();
  
  
  EXPECT_EQ(0x0AU, next_location(rel_finder));  
  EXPECT_EQ(0x0BU, next_location(rel_finder));  
}
...
constexpr uint8_t kDataX64[] = {
    ...
    +0x4C,  +0x8B,  +0x3D, +0,  +0,  +0, +0,   
    0x4C,   0x8D,   0x05,  0,   0,   0,  0,    
    0x4C,   0x8D,   0x3D,  0,   0,   0,  0,    
    0x66,   0x8B,   0x05,  (0), +0,  +0, +0,   
    +0x66,  +0x8B,  +0x3D, +0,  0,   0,  0,    
    0x66,   0x8D,   0x05,  0,   0,   0,  0,    
    0x66,   0x8D,   0x3D,  0,   0,   0,  0,    
    0x5D,                                      
    0xC3,                                      
};
...
TEST(Rel32FinderX64Test, FindNext) {
  ...
  for (auto location : expected_locations_rip) {
    ...
    EXPECT_EQ(location.second, rel32.location);
    EXPECT_EQ(image.begin() + (rel32.location + 4), rel_finder.accept_it());
    EXPECT_TRUE(rel32.can_point_outside_section);  
    rel_finder.Accept();
  }
  EXPECT_FALSE(rel_finder.FindNext());
}
...
TEST(Rel32FinderAArch32Test, IntegratedArmModeWithoutAbs32) {
  ...
  AddressTranslator translator(GetTrivialTranslator(image.size()));
  Rel32FinderAArch32 rel32_finder(image, translator,  false);

  std::vector<Result> results = ArmExtractRel32(
      image,  {}, DisassemblerElfAArch32::Traits::kVAWidth,
...
TEST(Rel32FinderAArch32Test, IntegratedThumb2ModeWithoutAbs32) {
  ...
  AddressTranslator translator(GetTrivialTranslator(image.size()));
  Rel32FinderAArch32 rel32_finder(image, translator,  true);
...
TEST(Rel32FinderAArch64Test, IntegratedWithoutAbs32) {
  ...
  std::vector<Result> results = ArmExtractRel32(
      image,  {}, DisassemblerElfAArch64::Traits::kVAWidth,
      std::move(rel32_finder));
