...
TEST(Abs32UtilsTest, AbsoluteAddress32) {
  ...
  EXPECT_EQ(kInvalidRva, addr32.ToRva());  
...
TEST(Abs32UtilsTest, AbsoluteAddress64) {
  ...
  EXPECT_TRUE(addr64.Read(0x10U, image64));  
  EXPECT_EQ(kInvalidRva, addr64.ToRva());    
  ...
  EXPECT_FALSE(addr64.Read(0x29U, image64));  
  EXPECT_FALSE(addr64.Read(0x30U, image64));  
...
TEST(Abs32UtilsTest, Win32ReadFail) {
  ...
  EXPECT_DEATH(try_make({4U, 24U}, 32U, 0U), "");  
#endif
...
#if GTEST_HAS_DEATH_TEST
  EXPECT_DEATH(try_make({8U, 24U}, 0U, 11U), "");   
  EXPECT_DEATH(try_make({8U, 24U}, 26U, 32U), "");  
#endif
...
#if GTEST_HAS_DEATH_TEST
  EXPECT_DEATH(try_make({6U, 22U}, 0U, 29U), "");  
  EXPECT_DEATH(try_make({6U, 22U}, 7U, 32U), "");  
#endif
  try_make({6U, 22U}, 14U, 20U);
  try_make({16U}, 16U, 24U);
#if GTEST_HAS_DEATH_TEST
  EXPECT_DEATH(try_make({16U}, 18U, 18U), "");  
#endif
}
...
TEST(Abs32UtilsTest, RemoveUntranslatableAbs32) {
  ...
  const offset_t kAbs1 = 0x04;  
  const offset_t kAbs2 = 0x0B;  
  const offset_t kAbs3 = 0x10;  
  const offset_t kAbs4 = 0x14;  
  const offset_t kAbs5 = 0x26;  
  const offset_t kAbs6 = 0x30;  
  const offset_t kAbs7 = 0x44;  
  const offset_t kAbs8 = 0x48;  
  const offset_t kAbs9 = 0x4C;  
  const offset_t kAbsA = 0x50;  
  const offset_t kAbsB = 0x54;  
  const offset_t kAbsC = 0x58;  
