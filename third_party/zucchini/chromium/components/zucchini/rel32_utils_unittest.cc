...
void CheckReader(const std::vector<Reference>& expected_refs,
  ...
  EXPECT_EQ(absl::nullopt, reader->GetNext());  
}
...
TEST(Rel32UtilsTest, Rel32WriterX86) {
  ...
  EXPECT_EQ(0x00000004U, buffer.read<uint32_t>(0x08));  

  writer.PutNext({0x0010U, 0x0014U});
  EXPECT_EQ(0x00000000U, buffer.read<uint32_t>(0x10));  

  writer.PutNext({0x0018U, 0x0010U});
  EXPECT_EQ(0xFFFFFFF4U, buffer.read<uint32_t>(0x18));  

  writer.PutNext({0x001CU, 0x0004U});
  EXPECT_EQ(0xFFFFFFE4U, buffer.read<uint32_t>(0x1C));  
...
TEST(Rel32UtilsTest, ArmCopyDisp_AArch32) {
  ...
  CheckCopy({0x41, 0xF2, 0xA5, 0x88},  
            {0x04, 0xF3, 0x3C, 0xA2},  
            {0x01, 0xF3, 0xA5, 0x88},  
            {0x44, 0xF2, 0x3C, 0xA2},  
            copier_T20);
  CheckCopy({0x7F, 0xF6, 0xFF, 0xAF},  
            {0x00, 0xF3, 0x00, 0x80},  
            {0x3F, 0xF7, 0xFF, 0xAF},  
            {0x40, 0xF2, 0x00, 0x80},  
            copier_T20);
  ...
  CheckCopy(expect_fail, {0x00, 0xF0, 0x00, 0x90},  
            {0xFF, 0xF7, 0xFF, 0xBF},  
            {0x00, 0xF0, 0x00, 0xC0},  
            copier_T24);
  
  CheckCopy({0xFF, 0xF7, 0xFE, 0xEF},  
            {0x00, 0xF0, 0x00, 0x90},  
            {0xFF, 0xF7, 0xFE, 0xBF},  
            {0x00, 0xF0, 0x00, 0xC0},  
            copier_T24);
