...
class TestDisassembler : public Disassembler {
 public:
  ...
  std::unique_ptr<ReferenceReader> MakeReadRefs1(offset_t ,
                                                 offset_t ) {
    return MakeReadRefs(0);
  }
  std::unique_ptr<ReferenceReader> MakeReadRefs2(offset_t ,
                                                 offset_t ) {
    return MakeReadRefs(1);
  }
  std::unique_ptr<ReferenceReader> MakeReadRefs3(offset_t ,
                                                 offset_t ) {
