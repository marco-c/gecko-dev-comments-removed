...
class Line(object):
    ...
    def pyproject_requires(self):
        ...
        if self._pyproject_requires is None and self.pyproject_toml is not None:
            if self.path is not None:
                ...
                pyproject_results = get_pyproject(self.path)  
    ...
    def _get_vcsrepo(self):
        ...
        checkout_directory = self.wheel_kwargs["src_dir"]  
    ...
    def parse_requirement(self):
        ...
        if name:
            self._requirement = init_requirement(name)  
            if extras:
...
class NamedRequirement(object):
    ...
    _parsed_line = attr.ib(default=None)  
...
class FileRequirement(object):
    ...
    setup_path = attr.ib(default=None, cmp=True)  
    
    path = attr.ib(default=None, cmp=True)  
    
    editable = attr.ib(default=False, cmp=True)  
    
    extras = attr.ib(
        default=attr.Factory(tuple), cmp=True
    )  
    _uri_scheme = attr.ib(default=None, cmp=True)  
    
    uri = attr.ib(cmp=True)  
    
    link = attr.ib(cmp=True)  
    
    pyproject_requires = attr.ib(
        factory=tuple, cmp=True
    )  
    
    pyproject_backend = attr.ib(default=None, cmp=True)  
    
    pyproject_path = attr.ib(default=None, cmp=True)  
    subdirectory = attr.ib(default=None)  
    
    _setup_info = attr.ib(default=None, cmp=True)  
    _has_hashed_name = attr.ib(default=False, cmp=True)  
    _parsed_line = attr.ib(default=None, cmp=False, hash=True)  
    
    name = attr.ib(cmp=True)  
    
    req = attr.ib(cmp=True)  
            ...
            if self.extras and self.setup_info.extras:
                for dep in self.extras:
                    if dep not in self.setup_info.extras:
                        continue
                    extras_list = self.setup_info.extras.get(dep, [])  
                    for req_instance in extras_list:  
...
class Requirement(object):
    ...
    def from_line(cls, line):
        ...
        if parsed_line.hashes:
            args["hashes"] = tuple(parsed_line.hashes)  
        cls_inst = cls(**args)  
        return cls_inst
...
def file_req_from_parsed_line(parsed_line):
    ...
    return FileRequirement(**req_dict)  
...
def vcs_req_from_parsed_line(parsed_line):
    ...
    return VCSRequirement(**vcs_dict)  
