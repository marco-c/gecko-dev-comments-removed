...
def output(
    ...
    if platform not in accepted_platforms:
        raise ValueError(
            f"Unknown platform: {platform}. Accepted platforms are: {accepted_platforms}."  
        )
    ...
    for category_key, category_val in objs.items():
        ...
        for key, metric in category_val.items():
            if isinstance(metric, metrics.Rate):
                print(
                    f"{fail_rate_level}: Rate metric not supported. Metric: {category_key}.{metric.name}",  
                    file=sys.stderr,
                )
                todelete.append(key)
            if isinstance(metric, metrics.Denominator):
                print(
                    f"{fail_rate_level}: Rate metric not supported. Dropping numerators. Metric: {category_key}.{metric.name}",  
                    file=sys.stderr,
                )
