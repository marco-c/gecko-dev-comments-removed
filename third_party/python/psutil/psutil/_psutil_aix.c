...
psutil_proc_basic_info(PyObject *self, PyObject *args) {
    ...
    return Py_BuildValue("KKKdiiiK",
        (unsigned long long) info.pr_ppid,      
        (unsigned long long) info.pr_rssize,    
        (unsigned long long) info.pr_size,      
        TV2DOUBLE(info.pr_start),               
        (int) info.pr_lwp.pr_nice,              
        (int) info.pr_nlwp,                     
        (int) status.pr_stat,                   
        (unsigned long long)info.pr_ttydev      
        );
}
...
psutil_disk_io_counters(PyObject *self, PyObject *args) {
    ...
    for (i = 0; i < disk_count; i++) {
        py_disk_info = Py_BuildValue(
            ...
            diskt[i].rserv / 1000 / 1000,  
            diskt[i].wserv / 1000 / 1000   
        );
