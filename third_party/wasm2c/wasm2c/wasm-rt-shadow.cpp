...
typedef enum class USED_STATE : uint8_t { UNUSED = 0, FREED } USED_STATE;

static const char* used_state_strings[] = {"UNUSED", "FREED"};
...
static const char* own_state_strings[] = {"UNKNOWN", "PROGRAM", "MALLOC"};
...
static cell_data_t unpack(wasm2c_shadow_memory_cell_t byte) {
  ...
  cell_data_t ret{(ALLOC_STATE)alloc_bits, (USED_STATE)used_bits,
                  (OWN_STATE)own_bits};
  return ret;
}
...
void wasm2c_shadow_memory_create(wasm_rt_memory_t* mem) {
  uint64_t new_size =
      ((uint64_t)mem->size) * sizeof(wasm2c_shadow_memory_cell_t);
...
#define allocation_sizes_map(mem) \
  ((std::map<uint32_t, uint32_t>*)mem->shadow_memory.allocation_sizes_map)
...
static inline void memory_state_iterate(wasm_rt_memory_t* mem,
                                        uint32_t ptr,
                                        uint32_t ptr_size,
                                        std::function<iterate_t> callback) {
...
void wasm2c_shadow_memory_reserve(wasm_rt_memory_t* mem,
                                  uint32_t ptr,
                                  uint32_t ptr_size) {
  memory_state_iterate(mem, ptr, ptr_size,
                       [&](uint32_t index, cell_data_t* data) {
...
static void report_error(wasm_rt_memory_t* mem,
  ...
  printf("WASM Shadow memory ASAN failed! %s (Func: %s, Index: %" PRIu32
         ") (Cell state: %s, %s, %s) (Allocated mem: %" PRIu64 ")!\n",
         error_message, func_name, index, alloc_state_string, used_state_string,
         own_state_string, used_mem);
  fflush(stdout);
...
void wasm2c_shadow_memory_dlmalloc(wasm_rt_memory_t* mem,
                                   uint32_t ptr,
                                   uint32_t ptr_size) {
  if (ptr == 0) {
  ...
  if (ptr < mem->shadow_memory.heap_base) {
    report_error(mem, func_name, "malloc returning a pointer outside the heap",
                 ptr, 0);
  }

  memory_state_iterate(
      mem, ptr, ptr_size, [&](uint32_t index, cell_data_t* data) {
        if (data->alloc_state != ALLOC_STATE::UNINIT) {
          report_error(mem, func_name,
                       "Malloc returned a pointer in already occupied memory!",
                       index, data);
        } else {
          data->alloc_state = ALLOC_STATE::ALLOCED;
        }
...
void wasm2c_shadow_memory_dlfree(wasm_rt_memory_t* mem, uint32_t ptr) {
  ...
  if (it == alloc_map->end()) {
    report_error(mem, func_name, "Freeing a pointer that was not allocated!",
                 ptr, 0);
  } else {
    ...
    memory_state_iterate(
        mem, ptr, ptr_size, [&](uint32_t index, cell_data_t* data) {
          if (data->alloc_state == ALLOC_STATE::UNINIT) {
            report_error(mem, func_name, "Freeing uninitialized memory", index,
                         data);
          } else {
...
void wasm2c_shadow_memory_mark_globals_heap_boundary(wasm_rt_memory_t* mem,
                                                     uint32_t ptr) {
  mem->shadow_memory.heap_base = ptr;
  wasm2c_shadow_memory_reserve(mem, 0, ptr);
}

static void check_heap_base_straddle(wasm_rt_memory_t* mem,
                                     const char* func_name,
                                     uint32_t ptr,
                                     uint32_t ptr_size) {
  ...
  if (ptr < heap_base) {
    if (ptr + ptr_size > heap_base) {
      report_error(mem, func_name, "Memory operation straddling heap base!",
                   ptr, 0 );
    }
  }
}

static bool is_malloc_family_function(const char* func_name,
                                      bool include_calloc,
                                      bool include_realloc) {
  if (strcmp(func_name, "w2c_dlmalloc") == 0 ||
      strcmp(func_name, "w2c_dlfree") == 0 ||
      strcmp(func_name, "w2c_sbrk") == 0) {
    return true;
  }
  if (include_calloc) {
...
WASM2C_FUNC_EXPORT void wasm2c_shadow_memory_load(wasm_rt_memory_t* mem,
                                                  const char* func_name,
                                                  uint32_t ptr,
                                                  uint32_t ptr_size) {
      ...
      mem, ptr, ptr_size, [&](uint32_t index, cell_data_t* data) {
        ...
        if (!exempt) {
          ...
          if (data->alloc_state != ALLOC_STATE::INITIALIZED) {
            report_error(mem, func_name,
                         "Reading uninitialized or unallocated memory", index,
                         data);
          }
#else
      if (data->alloc_state == ALLOC_STATE::UNINIT) {
        ...
        if (index < mem->shadow_memory.heap_base) {
          if (malloc_core_family) {
            ...
            if (data->own_state == OWN_STATE::PROGRAM) {
              report_error(mem, func_name,
                           "Core malloc functions reading non malloc globals",
                           index, data);
            }
            ...
            if (data->own_state == OWN_STATE::MALLOC) {
              report_error(mem, func_name, "Program reading malloc globals",
                           index, data);
            }
...
WASM2C_FUNC_EXPORT void wasm2c_shadow_memory_store(wasm_rt_memory_t* mem,
                                                   const char* func_name,
                                                   uint32_t ptr,
                                                   uint32_t ptr_size) {
  check_heap_base_straddle(mem, func_name, ptr, ptr_size);

  bool malloc_write_family = is_malloc_family_function(
  ...
  memory_state_iterate(
      mem, ptr, ptr_size, [&](uint32_t index, cell_data_t* data) {
        ...
        if (!exempt) {
          if (data->alloc_state == ALLOC_STATE::UNINIT) {
            report_error(mem, func_name, "Writing uninitialized memory", index,
                         data);
        ...
        if (index < mem->shadow_memory.heap_base) {
          if (malloc_core_family) {
            ...
            if (data->own_state == OWN_STATE::PROGRAM) {
              report_error(mem, func_name,
                           "Core malloc functions writing non malloc globals",
                           index, data);
            }
            data->own_state = OWN_STATE::MALLOC;
          } else if (malloc_any_family) {
            ...
            if (data->own_state == OWN_STATE::MALLOC) {
              report_error(mem, func_name, "Program writing malloc globals",
                           index, data);
            }
            data->own_state = OWN_STATE::PROGRAM;
          }
...
WASM2C_FUNC_EXPORT void wasm2c_shadow_memory_print_allocations(
