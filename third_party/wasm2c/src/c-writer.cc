...
void CWriter::WriteFuncDeclarations(bool for_header) {
  ...
  for (const Func* func : module_->funcs) {
    ...
    if (!is_import && !static_exclude) {
      WriteFuncDeclaration(func->decl, global_func_name, true );
      Write(";", Newline());
    }
    ++func_index;
  }
...
void CWriter::WriteEntryFuncs() {
  ...
  for (const Func* func : module_->funcs) {
    bool is_import = func_index < module_->num_func_imports;
    if (!is_import) {
      WriteEntryFunc(func->decl, GetGlobalName(func->name), true );
      Write(Newline());
    }
    ++func_index;
  }
  ...
  for (const Expr& expr : exprs) {
    switch (expr.type()) {
      ...
      case ExprType::CallIndirect: {
        ...
        WriteFuncDeclaration(decl, "(*)", false );
