...
#define SWITCH_RANGE_END       } }
        ...
        do {
            ...
            case MSGPACK_CS_HEADER:
                SWITCH_RANGE_BEGIN
                SWITCH_RANGE(0x00, 0x7f)  
                    push_fixed_value(_uint8, *(uint8_t*)p);
                SWITCH_RANGE(0xe0, 0xff)  
                    push_fixed_value(_int8, *(int8_t*)p);
                SWITCH_RANGE(0xc0, 0xdf)  
                    switch(*p) {
                    case 0xc0:  
...
#elif defined(__arm__) && !(__ARM_EABI__) 
