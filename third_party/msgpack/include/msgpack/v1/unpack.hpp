...
private:
    ...
    static void check_ext_size(std::size_t ) {
    }
...
inline void unpacker::expand_buffer(std::size_t size)
{
    ...
    if(m_off == COUNTER_SIZE) {
        std::size_t next_size = (m_used + m_free) * 2;    
        ...
        std::size_t next_size = m_initial_buffer_size;  
...
inline bool unpacker::default_reference_func(msgpack::type::object_type , std::size_t , void*)
{
    return true;
}
