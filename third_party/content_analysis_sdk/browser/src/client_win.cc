...
NTSTATUS WaitForPipeAvailability(const UNICODE_STRING& path) {
  ...
  NTSTATUS sts = fnNtCreateFile(&h, GENERIC_READ | GENERIC_WRITE | SYNCHRONIZE,
      &attr, &io, nullptr, FILE_ATTRIBUTE_NORMAL,
  ...
  IO_STATUS_BLOCK io2;
  sts = fnNtFsControlFile(h, nullptr, nullptr,
      nullptr, &io2, FSCTL_PIPE_WAIT, buffer.data(),
  ...
  while (true) {
    ...
    if (ReadFile(pipe, p, kBufferSize, &read, overlapped)) {
      ...
      if (err == ERROR_IO_PENDING) {
        if (GetOverlappedResult(pipe, overlapped, &read, TRUE)) {
          final_size += read;
          break;
        } else {
          err = GetLastError();
...
DWORD ClientWin::ConnectToPipe(const std::string& pipename, HANDLE* handle) {
  ...
  name.Length = wlen * sizeof(wchar_t);  
