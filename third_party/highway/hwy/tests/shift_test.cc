...
struct TestVariableLeftShifts {
  ...
  HWY_NOINLINE void operator()(T t, D d) {
    if (kSigned) {
      ...
      TestVariableLeftShifts<false>()(t, d);
    }
...
HWY_NOINLINE void TestAllShifts() {
  ForUnsignedTypes(ForPartialVectors<TestLeftShifts<false>>());
  ForSignedTypes(ForPartialVectors<TestLeftShifts<true>>());
...
HWY_NOINLINE void TestAllVariableShifts() {
  ForUnsignedTypes(
      ForPartialVectors<TestVariableLeftShifts<false>>());
  ForSignedTypes(ForPartialVectors<TestVariableLeftShifts<true>>());
