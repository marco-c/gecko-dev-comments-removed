...
struct TestLowerHalf {
  ...
  HWY_NOINLINE void operator()(T , D d) {
    ...
    Store(LowerHalf(v), d2, lanes2.get());  
    size_t i = 0;
    for (; i < Lanes(d2); ++i) {
...
struct TestLowerQuarter {
  ...
  HWY_NOINLINE void operator()(T , D d) {
    ...
    const auto lo2 = LowerHalf(LowerHalf(v));  
...
struct TestConcatOddEven {
  ...
  HWY_NOINLINE void operator()(T , D d) {
    ...
    const Vec<D> v_12 = InterleaveLower(v_1, v_2); 
    const Vec<D> v_34 = InterleaveLower(v_3, v_4); 
    const Vec<D> v_13 =
        ConcatLowerLower(d, v_3, v_1); 
    const Vec<D> v_24 =
        ConcatLowerLower(d, v_4, v_2); 
