...
struct TestPromoteOddEvenTo {
  static HWY_INLINE ToT CastValueToWide(hwy::FloatTag ,
                                        hwy::FloatTag ,
                                        hwy::float16_t val) {
  ...
  template <class T>
  static HWY_INLINE ToT CastValueToWide(hwy::SignedTag ,
                                        hwy::FloatTag ,
                                        T val) {
  ...
  template <class T>
  static HWY_INLINE ToT CastValueToWide(hwy::UnsignedTag ,
                                        hwy::FloatTag ,
                                        T val) {
  ...
  static HWY_INLINE ToT CastValueToWide(ToTypeTag ,
                                        FromTypeTag ,
                                        T val) {
    return static_cast<ToT>(val);
  }
...
struct TestF16 {
  ...
  HWY_NOINLINE void operator()(TF32 , DF32 df32) {
    size_t padded;
...
struct TestF16FromF64 {
  ...
  HWY_NOINLINE void operator()(TF64 , DF64 df64) {
#if HWY_HAVE_FLOAT64
    size_t padded;
    const size_t N = Lanes(df64);  
    HWY_ASSERT(N != 0);
...
class TestIntFromFloat {
  template <typename TF, class DF>
  static HWY_NOINLINE void TestPowers(TF , const DF df) {
    using TI = MakeSigned<TF>;
    const Rebind<TI, DF> di;
  ...
  template <typename TF, class DF>
  static HWY_NOINLINE void TestRandom(TF , const DF df) {
    using TI = MakeSigned<TF>;
    const Rebind<TI, DF> di;
    const size_t N = Lanes(df);
...
struct TestFloatFromUint {
  ...
  HWY_NOINLINE void operator()(TF , const DF df) {
    ...
    HWY_ASSERT_VEC_EQ(df, Set(df, ConvertScalarTo<TF>(32767)),
                      ConvertTo(df, Set(du, 32767)));  
    if (sizeof(TF) > 4) {
      HWY_ASSERT_VEC_EQ(df, Iota(df, 4294967295.0),
                        ConvertTo(df, Iota(du, 4294967295ULL)));  
    }
    ...
    for (size_t rep = 0; rep < AdjustedReps(1000); ++rep) {
      for (size_t i = 0; i < N; ++i) {
        do {
          ...
          CopyBytes<sizeof(TF)>(&bits, &in_pos[i]);  
