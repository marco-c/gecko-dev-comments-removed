...
class TestPer4LaneBlockShuffle {
 private:
  ...
  static HWY_INLINE VFromD<D> InterleaveMaskVectors(D , VFromD<D> a,
                                                    VFromD<D> ) {
    return a;
  }
  ...
  template <class D>
  static HWY_INLINE Vec<D> GenerateTestVect(hwy::NonFloatTag , D d) {
  ...
  template <class D>
  static HWY_INLINE Vec<D> GenerateTestVect(hwy::FloatTag , D d) {
    const RebindToUnsigned<decltype(d)> du;
