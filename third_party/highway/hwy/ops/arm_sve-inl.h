...
template <size_t N, int kPow2>
HWY_INLINE svuint16_t BitCastFromByte(Simd<bfloat16_t, N, kPow2> ,
                                      svuint8_t v) {
...
HWY_API VFromD<D> Reverse2(D , const VFromD<D> v) {  
  const auto even_in_odd = detail::Insert(v, 0);              
  return detail::InterleaveOdd(v, even_in_odd);               
}
...
HWY_API size_t StoreMaskBits(D d, svbool_t m, uint8_t* bits) {
  ...
  const size_t num_bytes = (num_bits + 8 - 1) / 8;  
...
HWY_INLINE svbool_t Lt128(D , const svuint64_t a, const svuint64_t b) {
  ...
  const svbool_t outHx = svsel_b(eqHL, cmpLL, ltHL);   
  return svtrn2_b64(outHx, outHx);                     
}
