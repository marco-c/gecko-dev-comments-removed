...
HWY_API void StoreInterleaved3(VFromD<D> v0, VFromD<D> v1, VFromD<D> v2, D d,
  ...
  const VU vA0 = TableLookupBytesOr0(v0, shuf_A0);  
  const VU vA1 = TableLookupBytesOr0(v1, shuf_A1);  
  const VU vA2 = TableLookupBytesOr0(v2, shuf_A2);  
  const VFromD<D> A = BitCast(d, vA0 | vA1 | vA2);

  
  const VU shuf_B0 = shuf_A2 + k6;  
  const VU shuf_B1 = shuf_A0 + k5;  
  const VU shuf_B2 = shuf_A1 + k5;  
  ...
  const VU shuf_C0 = shuf_B2 + k6;  
  const VU shuf_C1 = shuf_B0 + k5;  
  const VU shuf_C2 = shuf_B1 + k5;  
...
HWY_API void StoreInterleaved3(VFromD<D> v0, VFromD<D> v1, VFromD<D> v2, D d,
  ...
  const VU8 shuf_B0 = shuf_A1 + k3;  
  const VU8 shuf_B1 = shuf_A2 + k3;  
  const VU8 shuf_B2 = shuf_A0 + k2;  
  ...
  const VU8 shuf_C0 = shuf_B1 + k3;  
  const VU8 shuf_C1 = shuf_B2 + k3;  
  const VU8 shuf_C2 = shuf_B0 + k2;  
...
HWY_API void StoreInterleaved3(VFromD<D> part0, VFromD<D> part1,
  ...
  const VU shuf_A1 = Load(du, tbl_v1);  
  const VU shuf_A2 = CombineShiftRightBytes<15>(du, shuf_A1, shuf_A1);
  const VU A0 = TableLookupBytesOr0(v0, shuf_A0);  
  const VU A1 = TableLookupBytesOr0(v1, shuf_A1);  
  const VU A2 = TableLookupBytesOr0(v2, shuf_A2);  
...
HWY_API void StoreInterleaved3(VFromD<D> part0, VFromD<D> part1,
  ...
  const VU8 shuf_A1 = Load(du8, tbl_v1);  
                                          
  const VU8 shuf_A0 = CombineShiftRightBytes<2>(du8, shuf_A1, shuf_A1);
  const VU8 shuf_A2 = Load(du8, tbl_v2);  
...
HWY_API void TruncateStore(VFromD<D> v, const D , T* HWY_RESTRICT p) {
...
template <class D, HWY_IF_T_SIZE_D(D, 4), HWY_IF_V_SIZE_GT_D(D, 8)>
static HWY_INLINE HWY_MAYBE_UNUSED Vec<D> BlockwiseConcatOddEven(D ,
                                                                 Vec<D> v) {
