...
HWY_INLINE VFromD<RebindToUnsigned<D>> BitCastFromByte(
    D , VFromD<Repartition<uint8_t, D>> v) {
...
template <class V, HWY_IF_U16_D(DFromV<V>)>  
HWY_API V Neg(V v) {
...
namespace detail {
HWY_RVV_FOREACH_B(HWY_RVV_RETM, MClear, clr)  
}  
...
template <class DH>
HWY_API VFromD<DH> UpperHalf(const DH , const VFromD<Twice<DH>> v) {
  const Twice<DH> d;
...
HWY_API VI TableLookupBytes(const VT vt, const VI vi) {
  ...
  const Simd<uint8_t, MaxLanes(di8), HWY_MAX(kPow2T, kPow2I)> dm8;  
...
HWY_API VFromD<D> TwoTablesLookupLanes(D d, VFromD<D> a, VFromD<D> b,
  ...
  const auto sel_a_mask = Ne(idx, idx_mod);  
...
HWY_INLINE MFromD<D> U8MaskBitsVecToMask(D , vuint8m1_t v) {
...
template <class VW, HWY_IF_UNSIGNED_V(VW)>  
HWY_API VW RearrangeToOddPlusEven(const VW sum0, const VW sum1) {
...
template <class D>
HWY_INLINE VFromD<D> Min128(D , const VFromD<D> a, const VFromD<D> b) {
...
template <class D>
HWY_INLINE VFromD<D> Max128(D , const VFromD<D> a, const VFromD<D> b) {
  const VFromD<D> aXH = detail::Slide1Down(a);
  const VFromD<D> bXH = detail::Slide1Down(b);
