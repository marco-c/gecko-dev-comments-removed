...
HWY_INLINE constexpr size_t MLenFromD(Simd<T, N> ) {
...
template <typename T, size_t N,
          hwy::EnableIf<(N < HWY_LANES(T) / 8)>* = nullptr>
HWY_API decltype(Set(Full<T>(), T{0})) Set(Simd<T, N> , T arg) {
...
HWY_INLINE VFromD<Simd<uint16_t, N>> BitCastFromByte(
    Simd<bfloat16_t, N> , VFromD<Simd<uint8_t, N * 2>> v) {
...
template <typename T, size_t N, typename TU = MakeUnsigned<T>,
          hwy::EnableIf<(N < HWY_LANES(T) / 8)>* = nullptr>
HWY_INLINE VFromD<Full<TU>> Iota0(Simd<T, N> ) {
...
template <typename T, size_t N,
          hwy::EnableIf<(N < HWY_LANES(T) / 8)>* = nullptr>
HWY_API void Store(VFromD<Simd<T, N>> v, Simd<T, N> ,
                   T* HWY_RESTRICT p) {
...
template <typename T, size_t N,
          hwy::EnableIf<(N < HWY_LANES(T) / 8)>* = nullptr>
HWY_API void ScatterOffset(VFromD<Simd<T, N>> v, Simd<T, N> ,
                           T* HWY_RESTRICT base,
...
template <typename T, size_t N,
          hwy::EnableIf<(N < HWY_LANES(T) / 8)>* = nullptr>
HWY_API VFromD<Simd<T, N>> GatherOffset(Simd<T, N> ,
...
HWY_API VFromD<D> SumOfLanes(D , const VFromD<D> v) {
  const auto v0 = Zero(Full<TFromD<D>>());  
