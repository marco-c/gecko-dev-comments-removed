...
HWY_API VFromD<Full<T>> Zero(Simd<T, N> ) {
  return Zero(Full<T>());
}
...
template <typename T, size_t N, HWY_IF_LE128(T, N)>
HWY_API VFromD<Simd<T, N>> Iota0(Simd<T, N> ) {
  return Iota0(Full<T>());
}
...
HWY_RVV_STORE3(uint, u, 8, m1, 0, 8, StoreInterleaved3, sseg3)
HWY_RVV_STORE3(uint, u, 8, m2, 1, 4, StoreInterleaved3, sseg3)
...
HWY_RVV_STORE4(uint, u, 8, m1, 0, 8, StoreInterleaved4, sseg4)
HWY_RVV_STORE4(uint, u, 8, m2, 1, 4, StoreInterleaved4, sseg4)
...
#define HWY_RVV_CONVERT(BASE, CHAR, SEW, LMUL, SHIFT, MLEN, NAME, OP)          \
  HWY_API HWY_RVV_V(BASE, SEW, LMUL) ConvertTo(                                \
      HWY_RVV_D(CHAR, SEW, LMUL) , HWY_RVV_V(int, SEW, LMUL) v) {       \
    return vfcvt_f_x_v_f##SEW##LMUL(v);                                        \
  }                                                                            \
                                          \
  HWY_API HWY_RVV_V(int, SEW, LMUL) ConvertTo(HWY_RVV_D(i, SEW, LMUL) , \
