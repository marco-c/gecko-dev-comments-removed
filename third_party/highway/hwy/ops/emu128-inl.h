...
template <typename T>
HWY_INLINE constexpr T IncrementWithWraparound(hwy::FloatTag , T t) {
  return t + T{1};
}

template <typename T>
HWY_INLINE constexpr T IncrementWithWraparound(hwy::NonFloatTag , T t) {
...
template <typename T, size_t N>
HWY_API Mask128<T, N> FirstN(Simd<T, N, 0> , size_t n) {
  Mask128<T, N> m;
  for (size_t i = 0; i < N; ++i) {
...
template <typename T, size_t N>
HWY_INLINE Vec128<T, N> Mul(SignedTag , Vec128<T, N> a,
                            const Vec128<T, N> b) {
  for (size_t i = 0; i < N; ++i) {
...
HWY_API Mask128<uint64_t> Lt128(Simd<uint64_t, 2, 0> ,
...
HWY_API Mask128<uint64_t> Lt128Upper(Simd<uint64_t, 2, 0> ,
                                     Vec128<uint64_t> a,
...
HWY_API Mask128<uint64_t> Eq128(Simd<uint64_t, 2, 0> ,
                                Vec128<uint64_t> a, const Vec128<uint64_t> b) {
...
HWY_API Mask128<uint64_t> Ne128(Simd<uint64_t, 2, 0> ,
                                Vec128<uint64_t> a, const Vec128<uint64_t> b) {
...
HWY_API Mask128<uint64_t> Eq128Upper(Simd<uint64_t, 2, 0> ,
                                     Vec128<uint64_t> a,
                                     const Vec128<uint64_t> b) {
...
HWY_API Mask128<uint64_t> Ne128Upper(Simd<uint64_t, 2, 0> ,
                                     Vec128<uint64_t> a,
                                     const Vec128<uint64_t> b) {
...
template <typename T, size_t N>
HWY_API Vec128<T, N> Load(Simd<T, N, 0> ,
...
template <typename T, size_t N>
HWY_API void Store(const Vec128<T, N> v, Simd<T, N, 0> ,
...
HWY_API void ScatterOffset(Vec128<T, N> v, Simd<T, N, 0> , T* base,
...
HWY_API void ScatterIndex(Vec128<T, N> v, Simd<T, N, 0> ,
...
template <typename T, size_t N, typename Offset>
HWY_API Vec128<T, N> GatherOffset(Simd<T, N, 0> , const T* base,
                                  const Vec128<Offset, N> offset) {
  static_assert(sizeof(T) == sizeof(Offset), "Must match for portability");
  Vec128<T, N> v;
  for (size_t i = 0; i < N; ++i) {
...
template <typename T, size_t N, typename Index>
HWY_API Vec128<T, N> GatherIndex(Simd<T, N, 0> ,
...
HWY_API Vec128<ToT, N> PromoteTo(Simd<ToT, N, 0> ,
                                 Vec128<FromT, N> from) {
...
template <typename FromT, typename ToT, size_t N>
HWY_API Vec128<ToT, N> DemoteTo(Simd<ToT, N, 0> ,
                                Vec128<FromT, N> from) {
...
template <size_t N>
HWY_API Vec128<int16_t, 2 * N> ReorderDemote2To(Simd<int16_t, 2 * N, 0> ,
                                                Vec128<int32_t, N> a,
                                                Vec128<int32_t, N> b) {
...
template <size_t N>
HWY_API Vec128<bfloat16_t, N> DemoteTo(Simd<bfloat16_t, N, 0> ,
                                       const Vec128<float, N> v) {
...
template <typename FromT, typename ToT, size_t N>
HWY_API Vec128<ToT, N> ConvertTo(hwy::FloatTag ,
                                 Simd<ToT, N, 0> ,
                                 Vec128<FromT, N> from) {
...
template <typename FromT, typename ToT, size_t N>
HWY_API Vec128<ToT, N> ConvertTo(hwy::NonFloatTag ,
                                 Simd<ToT, N, 0> ,
                                 Vec128<FromT, N> from) {
  static_assert(sizeof(ToT) == sizeof(FromT), "Should have same size");
  Vec128<ToT, N> ret;
...
template <size_t N>
HWY_API Vec128<uint32_t, N> TruncateTo(Simd<uint32_t, N, 0> ,
                                       const Vec128<uint64_t, N> v) {
...
template <typename T, size_t N>
HWY_API Vec128<T, N / 2> UpperHalf(Simd<T, N / 2, 0> ,
                                   Vec128<T, N> v) {
  Vec128<T, N / 2> ret;
...
template <typename T, size_t N>
HWY_API Vec128<T, N> ZeroExtendVector(Simd<T, N, 0> ,
                                      Vec128<T, N / 2> v) {
  Vec128<T, N> ret;
  CopyBytes<N / 2 * sizeof(T)>(v.raw, ret.raw);
  return ret;
...
template <typename T, size_t N>
HWY_API Vec128<T, N> Combine(Simd<T, N, 0> , Vec128<T, N / 2> hi_half,
...
template <typename T, size_t N>
HWY_API Vec128<T, N> ConcatLowerLower(Simd<T, N, 0> , Vec128<T, N> hi,
                                      Vec128<T, N> lo) {
  Vec128<T, N> ret;
...
template <typename T, size_t N>
HWY_API Vec128<T, N> ConcatUpperUpper(Simd<T, N, 0> , Vec128<T, N> hi,
                                      Vec128<T, N> lo) {
  Vec128<T, N> ret;
...
template <typename T, size_t N>
HWY_API Vec128<T, N> ConcatLowerUpper(Simd<T, N, 0> ,
                                      const Vec128<T, N> hi,
                                      const Vec128<T, N> lo) {
  Vec128<T, N> ret;
  CopyBytes<N / 2 * sizeof(T)>(&lo.raw[N / 2], &ret.raw[0]);
...
template <typename T, size_t N>
HWY_API Vec128<T, N> ConcatUpperLower(Simd<T, N, 0> , Vec128<T, N> hi,
                                      Vec128<T, N> lo) {
  Vec128<T, N> ret;
  CopyBytes<N / 2 * sizeof(T)>(lo.raw, &ret.raw[0]);
  CopyBytes<N / 2 * sizeof(T)>(&hi.raw[N / 2], &ret.raw[N / 2]);
...
template <typename T, size_t N>
HWY_API Vec128<T, N> ReverseBlocks(Simd<T, N, 0> ,
                                   const Vec128<T, N> v) {
  return v;
}
...
template <typename T, size_t N>
HWY_API Vec128<T, N> InterleaveUpper(Simd<T, N, 0> ,
                                     const Vec128<T, N> a,
                                     const Vec128<T, N> b) {
  Vec128<T, N> ret;
  for (size_t i = 0; i < N / 2; ++i) {
...
template <typename T, size_t N>
HWY_API bool AllFalse(Simd<T, N, 0> , const Mask128<T, N> mask) {
...
template <typename T, size_t N>
HWY_API size_t FindKnownFirstTrue(Simd<T, N, 0> ,
...
template <typename T, size_t N>
HWY_API intptr_t FindFirstTrue(Simd<T, N, 0> ,
                               const Mask128<T, N> mask) {
  for (size_t i = 0; i < N; ++i) {
