...
HWY_INLINE VFromD<DTo> ZeroExtendResizeBitCast(
    FromSizeTag , ToSizeTag , DTo d_to, DFrom ,
    VFromD<DFrom> v) {
...
HWY_API Vec1<int32_t> DemoteTo(D , Vec1<double> from) {
...
template <class D, typename T = TFromD<D>>
HWY_API Vec1<T> SumOfLanes(D , const Vec1<T> v) {
  return v;
}
template <class D, typename T = TFromD<D>>
HWY_API T ReduceSum(D , const Vec1<T> v) {
  return GetLane(v);
}
template <class D, typename T = TFromD<D>>
HWY_API Vec1<T> MinOfLanes(D , const Vec1<T> v) {
  return v;
}
template <class D, typename T = TFromD<D>>
HWY_API Vec1<T> MaxOfLanes(D , const Vec1<T> v) {
  return v;
}
