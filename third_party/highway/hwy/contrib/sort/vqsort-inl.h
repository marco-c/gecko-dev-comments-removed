...
HWY_NOINLINE void BaseCase(D d, TraitsKV, T* HWY_RESTRICT keys,
  ...
  const FuncPtr funcs[9] = {
     ...
     &Sort2To2<Traits, T>,
     &Sort3To4<Traits, T>,
     &Sort8Rows<1, Traits, T>,  
     kMaxKeysPerVector >= 2 ? &Sort8Rows<2, Traits, T> : nullptr,
     kMaxKeysPerVector >= 4 ? &Sort8Rows<4, Traits, T> : nullptr,
     kMaxKeysPerVector >= 4 ? &Sort16Rows<4, Traits, T> : nullptr,
     kMaxKeysPerVector >= 8 ? &Sort16Rows<8, Traits, T> : nullptr,
#if !HWY_COMPILER_MSVC && !HWY_IS_DEBUG_BUILD
     kMaxKeysPerVector >= 16 ? &Sort16Rows<16, Traits, T> : nullptr,
#endif
  };
...
HWY_INLINE size_t PartitionRightmost(D d, Traits st, T* const keys,
  ...
  (void)numWrittenR;                     
  HWY_DASSERT(numWrittenR <= num_here);  
...
HWY_INLINE size_t Partition(D d, Traits st, T* const keys, const size_t num,
  ...
  if (HWY_LIKELY(num_main != 0)) {
    ...
    while (readL != readR) {
      ...
      HWY_DASSERT(capacityL <= num_main);  
...
HWY_INLINE Vec<D> ChoosePivotForEqualSamples(D d, Traits st,
                                             ...
                                             PivotResult& result) {
  const Vec<D> pivot = st.SetKey(d, samples);  
