...
TEST(PerfCountersTest, RunBranches) {
  ...
  HWY_ASSERT(values[PerfCounters::kRefCycles] == 0.0 ||
             values[PerfCounters::kRefCycles] > 1E3);  
  HWY_ASSERT(values[PerfCounters::kInstructions] == 0.0 ||
             values[PerfCounters::kInstructions] > 100.0);  
  HWY_ASSERT(values[PerfCounters::kBranches] == 0.0 ||
             values[PerfCounters::kBranches] > 100.0);  
  HWY_ASSERT(values[PerfCounters::kBranchMispredicts] == 0 ||
             values[PerfCounters::kBranchMispredicts] > 10.0);  

  HWY_ASSERT(values[PerfCounters::kL3Loads] < 1E8);       
  HWY_ASSERT(values[PerfCounters::kL3Stores] < 1E7);      
  HWY_ASSERT(values[PerfCounters::kCacheRefs] < 1E9);     
  HWY_ASSERT(values[PerfCounters::kCacheMisses] < 1E8);   
  HWY_ASSERT(values[PerfCounters::kBusCycles] < 1E11);    
  HWY_ASSERT(values[PerfCounters::kPageFaults] < 1E4);    
}
