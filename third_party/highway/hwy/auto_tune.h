...
class CostDistribution {
 public:
  static constexpr size_t kMaxValues = 14;  
  ...
  double& M1() { return values_[0]; }  
  double& M2() { return values_[1]; }
  double& Mean() { return values_[2]; }  
 ...
 private:
  ...
  void RemoveOutliers() {
    if (num_values_ < 3) return;  
    HWY_DASSERT(num_values_ <= kMaxValues);

    
    
    
    const double median = Median(values_, num_values_);  
  ...
  double SampleVariance(double sample_mean) const {
    HWY_DASSERT(sample_mean >= 0.0);  
    
    HWY_DASSERT(!IsOnline() && 0 != num_values_ && num_values_ <= kMaxValues);
    if (HWY_UNLIKELY(num_values_ == 1)) return 0.0;  
    double sum2 = 0.0;
    for (size_t i = 0; i < num_values_; ++i) {
...
class AutoTune {
 ...
 private:
  ...
  std::vector<CostDistribution> costs_;  
