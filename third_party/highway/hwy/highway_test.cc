...
HWY_NOINLINE void TestCappedLimit(T ) {
...
class TestFracNLanes {
 private:
  ...
  static HWY_INLINE void DoTestFracNLanes(Simd<T1, N1, 0> ,
                                          Simd<T2, N2, kPow2> d2) {
  ...
  static HWY_INLINE void DoTest4LanesWithPow3(T ) {
 ...
 public:
  template <class T>
  HWY_NOINLINE void operator()(T ) const {
    const Simd<T, 1, 0> d1;
