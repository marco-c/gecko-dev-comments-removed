...
HWY_DLLEXPORT void ToString(const TypeInfo& info, const void* ptr,
  ...
  if (info.sizeof_t == 1) {
    ...
    if (info.is_bf16) {
      ...
      snprintf(string100, 100, fmt, value);  
    ...
    if (info.is_float) {
      ...
      snprintf(string100, 100, fmt, static_cast<double>(value));  
    ...
    if (info.is_float) {
      ...
      snprintf(string100, 100, "0x%08x%08x", hi, lo);  
    }
