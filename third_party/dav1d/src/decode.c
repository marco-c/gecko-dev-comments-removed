...
{
     refmvs_block *const *r = &t->rt.r[(t->by & 31) + 5];
    int count = 0;
...
#define add_sample(dx, dy, sx, sy, rp) do { \
    ...
    } else for (unsigned off = 0, xmask = (uint32_t) masks[0]; np < 8 && xmask;) { 
    ...
    } else for (unsigned off = 0, ymask = (uint32_t) masks[1]; np < 8 && ymask;) { 
    ...
    if (f->frame_hdr->segmentation.enabled) {
        if (!f->frame_hdr->segmentation.update_map) {
                                ...
                                t->l.seg_pred[by4]])))
            {
                ...
                if (b->seg_id > last_active_seg_id) b->seg_id = 0; 
                if (b->seg_id >= DAV1D_MAX_SEGMENTS) b->seg_id = 0; 
            }
