...
namespace xsimd
{
    namespace kernel
    {
        ...
        XSIMD_INLINE batch<float, A> haddp(batch<float, A> const* r, requires_arch<vxe>) noexcept
        {
            ...
            lo01 = vec_mergel(r[0].data, r[1].data); 
            hi01 = vec_mergeh(r[0].data, r[1].data); 
            lo23 = vec_mergel(r[2].data, r[3].data); 
            hi23 = vec_mergeh(r[2].data, r[3].data); 
            sum01 = lo01 + hi01; 
            sum23 = lo23 + hi23; 
            sumeven = (v4sf)vec_mergeh((v2di)sum01, (v2di)sum23); 
            sumodd = (v4sf)vec_mergel((v2di)sum01, (v2di)sum23); 
