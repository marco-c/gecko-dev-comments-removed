...
namespace xsimd
{
    ...
    namespace kernel
    {
        ...
        inline batch<T, A> shuffle(batch<T, A> const& x, batch<T, A> const& y, batch_constant<batch<ITy, A>, Indices...>, requires_arch<generic>) noexcept
        {
            ...
            {
                return swizzle(x, batch_constant<batch<ITy, A>, ((Indices >= bsize) ? 0  : Indices)...>());
            }

            XSIMD_IF_CONSTEXPR(detail::is_swizzle_snd(bsize, Indices...))
            {
                return swizzle(y, batch_constant<batch<ITy, A>, ((Indices >= bsize) ? (Indices - bsize) : 0 )...>());
            }
...
#define builtin_shuffle __builtin_shuffle_vector
        ...
        {
            template <class A, class T>
            inline batch<std::complex<T>, A> load_complex(batch<T, A> const& , batch<T, A> const& , requires_arch<generic>) noexcept
            {
                static_assert(std::is_same<T, void>::value, "load_complex not implemented for the required architecture");
            }

            template <class A, class T>
            inline batch<T, A> complex_high(batch<std::complex<T>, A> const& , requires_arch<generic>) noexcept
            {
                static_assert(std::is_same<T, void>::value, "complex_high not implemented for the required architecture");
            }

            template <class A, class T>
            inline batch<T, A> complex_low(batch<std::complex<T>, A> const& , requires_arch<generic>) noexcept
            {
