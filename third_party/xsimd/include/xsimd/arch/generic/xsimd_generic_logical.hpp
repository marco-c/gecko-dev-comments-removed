...
#define builtin_popcount(v) __builtin_popcountg(v)
                ...
                XSIMD_IF_CONSTEXPR(batch_bool<T, A>::size <= 32)
                {
                    ...
                    m32 = m32 - ((m32 >> 1) & 0x55555555); 
                    m32 = (m32 & 0x33333333) + ((m32 >> 2) & 0x33333333); 
                    return (((m32 + (m32 >> 4)) & 0xF0F0F0F) * 0x1010101) >> 24; 
                }
                else
                {
                    
                    m = m - ((m >> 1) & (uint64_t) ~(uint64_t)0 / 3); 
                    m = (m & (uint64_t) ~(uint64_t)0 / 15 * 3) + ((m >> 2) & (uint64_t) ~(uint64_t)0 / 15 * 3); 
                    m = (m + (m >> 4)) & (uint64_t) ~(uint64_t)0 / 255 * 15; 
                    return (m * ((uint64_t) ~(uint64_t)0 / 255)) >> (sizeof(uint64_t) - 1) * CHAR_BIT; 
                }
#endif
            }
        }
