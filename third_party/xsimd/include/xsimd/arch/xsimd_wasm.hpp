    ...
    namespace kernel
    {
        ...
        namespace detail
        {
            ...
            XSIMD_INLINE batch<double, A> fast_cast(batch<uint64_t, A> const& x, batch<double, A> const&, requires_arch<wasm>) noexcept
            {
                ...
                xH = wasm_v128_or(xH, wasm_f64x2_splat(19342813113834066795298816.)); 
                v128_t mask = wasm_i16x8_make(0xFFFF, 0xFFFF, 0x0000, 0x0000, 0xFFFF, 0xFFFF, 0x0000, 0x0000);
                v128_t xL = wasm_v128_or(wasm_v128_and(mask, x), wasm_v128_andnot(wasm_f64x2_splat(0x0010000000000000), mask)); 
                v128_t f = wasm_f64x2_sub(xH, wasm_f64x2_splat(19342813118337666422669312.)); 
            ...
            XSIMD_INLINE batch<double, A> fast_cast(batch<int64_t, A> const& x, batch<double, A> const&, requires_arch<wasm>) noexcept
            {
                ...
                xH = wasm_i64x2_add(xH, wasm_f64x2_splat(442721857769029238784.)); 
                v128_t mask = wasm_i16x8_make(0xFFFF, 0xFFFF, 0xFFFF, 0x0000, 0xFFFF, 0xFFFF, 0xFFFF, 0x0000);
                v128_t xL = wasm_v128_or(wasm_v128_and(mask, x), wasm_v128_andnot(wasm_f64x2_splat(0x0010000000000000), mask)); 
                v128_t f = wasm_f64x2_sub(xH, wasm_f64x2_splat(442726361368656609280.)); 
                return wasm_f64x2_add(f, xL);
            }
        ...
        XSIMD_INLINE void transpose(batch<T, A>* matrix_begin, batch<T, A>* matrix_end, requires_arch<wasm>) noexcept
        {
            ...
            XSIMD_IF_CONSTEXPR(sizeof(T) == 4)
            {
                ...
                matrix_begin[0] = wasm_i32x4_shuffle(t0, t2, 0, 1, 4, 5); 
                matrix_begin[1] = wasm_i32x4_shuffle(t0, t2, 2, 3, 6, 7); 
                matrix_begin[2] = wasm_i32x4_shuffle(t1, t3, 0, 1, 4, 5); 
                matrix_begin[3] = wasm_i32x4_shuffle(t1, t3, 2, 3, 6, 7); 
            }
            else XSIMD_IF_CONSTEXPR(sizeof(T) == 8)
            {
