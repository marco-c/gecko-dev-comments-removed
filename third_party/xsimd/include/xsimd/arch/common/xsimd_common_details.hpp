    ...
    namespace kernel
    {
        ...
        namespace detail
        {
            ...
            XSIMD_INLINE batch<float, A> fast_cast(batch<uint32_t, A> const& v, batch<float, A> const&, requires_arch<common>) noexcept
            {
                ...
                auto v_lo = batch_cast<int32_t>(v & msk_lo); 
                auto v_hi = batch_cast<int32_t>(v >> 16); 
                auto v_lo_flt = batch_cast<float>(v_lo); 
                auto v_hi_flt = batch_cast<float>(v_hi); 
                v_hi_flt = cnst65536f * v_hi_flt; 
                return v_hi_flt + v_lo_flt; 
            }
