...
namespace xsimd
{
    ...
    namespace kernel
    {
        ...
        XSIMD_INLINE batch<T, A> insert(batch<T, A> const& self, T val, index<I>, requires_arch<common>) noexcept
        {
            struct index_mask
            {
                static constexpr bool get(size_t index, size_t )
                {
                    return index != I;
                }
            };
        ...
        namespace detail
        {
            ...
            load_masked_common(T_in const* mem, batch_bool_constant<T_out, A, Values...>, convert<T_out>, alignment, std::false_type ) noexcept
            {
            ...
            XSIMD_INLINE batch<T, A>
            load_masked_common(T const* mem, batch_bool_constant<T, A, Values...>, convert<T>, Mode, std::true_type ) noexcept
            {
            ...
            template <class A, class T_in, class T_out, bool... Values, class alignment>
            XSIMD_INLINE void
            store_masked_common(T_out* mem, batch<T_in, A> const& src, batch_bool_constant<T_in, A, Values...>, alignment, std::false_type ) noexcept
            {
            ...
            template <class A, class T, bool... Values, class Mode>
            XSIMD_INLINE void
            store_masked_common(T* mem, batch<T, A> const& src, batch_bool_constant<T, A, Values...>, Mode, std::true_type ) noexcept
            {
                using fp_t = sized_fp_t<sizeof(T)>;
        ...
        XSIMD_INLINE batch<T, A> shuffle(batch<T, A> const& x, batch<T, A> const& y, batch_constant<ITy, A, Indices...>, requires_arch<common>) noexcept
        {
            ...
            {
                return swizzle(x, batch_constant<ITy, A, ((Indices >= bsize) ? 0  : Indices)...>());
            }

            XSIMD_IF_CONSTEXPR(detail::is_swizzle_snd(bsize, Indices...))
            {
                return swizzle(y, batch_constant<ITy, A, ((Indices >= bsize) ? (Indices - bsize) : 0 )...>());
            }
...
#define builtin_shuffle __builtin_shufflevector
        ...
        {
            template <class A, class T>
            XSIMD_INLINE batch<std::complex<T>, A> load_complex(batch<T, A> const& , batch<T, A> const& , requires_arch<common>) noexcept
            {
                static_assert(std::is_same<T, void>::value, "load_complex not implemented for the required architecture");
            }

            template <class A, class T>
            XSIMD_INLINE batch<T, A> complex_high(batch<std::complex<T>, A> const& , requires_arch<common>) noexcept
            {
                static_assert(std::is_same<T, void>::value, "complex_high not implemented for the required architecture");
            }

            template <class A, class T>
            XSIMD_INLINE batch<T, A> complex_low(batch<std::complex<T>, A> const& , requires_arch<common>) noexcept
            {
