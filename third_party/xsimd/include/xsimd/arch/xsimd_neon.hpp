...
#undef xsimd_aligned_load
        ...
        namespace detail
        {
            ...
            struct load_masked<>
            {
                ...
                static XSIMD_INLINE batch<T, A> apply(T const* , batch<T, A> acc, std::integral_constant<bool, Use>) noexcept
                {
                    return acc;
                }
            };
        ...
        XSIMD_INLINE batch<T, A> load_masked(T const* mem, batch_bool_constant<T, A, Value, Values...> , Mode, requires_arch<neon>) noexcept
        {
                                                        ...
                                                        requires_arch<neon>) noexcept
        {
            ...
            const auto r_normal = vcvtq_s32_f32(vaddq_f32(
                self, half)); 
            const auto r_trunc = vcvtq_s32_f32(self); 
            ...
            const auto is_delta_half = vceqq_f32(delta, half); 
        ...
        {
            template <class A, class T>
            XSIMD_INLINE batch<T, A> bitwise_lshift(batch<T, A> const& , int , ::xsimd::detail::int_sequence<>) noexcept
            {
        ...
        {
            template <class A, class T>
            XSIMD_INLINE batch<T, A> bitwise_rshift(batch<T, A> const& , int , ::xsimd::detail::int_sequence<>) noexcept
            {
