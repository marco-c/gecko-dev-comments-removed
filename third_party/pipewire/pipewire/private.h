...
struct settings {
	uint32_t log_level;
	uint32_t clock_rate;			
	uint32_t clock_rates[MAX_RATES];	
	uint32_t n_clock_rates;			
	uint32_t clock_quantum;			
	uint32_t clock_min_quantum;		
	uint32_t clock_max_quantum;		
	uint32_t clock_quantum_limit;		
	uint32_t clock_quantum_floor;		
...
struct pw_protocol {
	...
	const struct pw_protocol_implementation *implementation; 
...
struct pw_impl_client {
	...
	struct pw_resource *client_resource;	
...
struct pw_context {
	...
	struct spa_list core_impl_list;		
	struct spa_list protocol_list;		
	struct spa_list core_list;		
	...
	struct spa_list factory_list;		
	struct spa_list metadata_list;		
	struct spa_list link_list;		
	struct spa_list control_list[2];	
	struct spa_list export_list;		
	...
	uint32_t n_support;		
	struct pw_array factory_lib;	

	struct pw_array objects;	

	struct pw_impl_client *current_client;	
...
#define PW_NODE_ACTIVATION_INACTIVE		4
	...
	struct spa_io_segment reposition;		

	struct spa_io_segment segment;			
...
struct pw_impl_node {
	...
	uint32_t supports_lazy;		
	uint32_t supports_request;	

	uint32_t priority_driver;	
	char **groups;			
	char **link_groups;		
	char **sync_groups;		
	uint64_t spa_flags;
	...
	unsigned int want_driver:1;	
	unsigned int in_passive:1;	
	unsigned int out_passive:1;	
	unsigned int runnable:1;	
	unsigned int freewheel:1;	
	unsigned int loopchecked:1;	
	unsigned int always_process:1;	
	unsigned int lock_quantum:1;	
	unsigned int lock_rate:1;	
	unsigned int transport_sync:1;	
	unsigned int target_pending:1;	
	...
	struct spa_list sort_link;	

	struct spa_list peer_list;	
	...
	struct spa_fraction max_latency;	
	...
	struct {
		...
		struct spa_list input_mix;		
		struct spa_list output_mix;		
	...
	struct pw_node_peer *to_driver_peer;		
	struct pw_node_peer *from_driver_peer;		
...
#define PW_IMPL_PORT_MIX_FLAG_NEGOTIATE	(1<<2)	/**< negotiate buffers  */
	uint32_t mix_flags;		
	struct spa_handle *mix_handle;	
	struct pw_buffers mix_buffers;	

	struct spa_list mix_list;	
	struct pw_map mix_port_map;	
	uint32_t n_mix;
...
struct pw_impl_link {
	...
	struct {
		struct pw_impl_port_mix out_mix;	
		struct pw_impl_port_mix in_mix;		
	} rt;
