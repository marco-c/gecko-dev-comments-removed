...
simde_float16_from_float32 (simde_float32 value) {
  simde_float16 res;
    ...
    if (f32u > f16u_max) { 
      f16u = (f32u > f32u_infty) ?  UINT32_C(0x7e00) : UINT32_C(0x7c00); 
    } else { 
      if (f32u < (UINT32_C(113) << 23)) { 
...
simde_float16_to_float32 (simde_float16 value) {
  simde_float32 res;
    ...
    const uint32_t shifted_exp = UINT32_C(0x7c00) << 13; 
    uint32_t f32u;

    f32u = (half & UINT32_C(0x7fff)) << 13; 
    uint32_t exp = shifted_exp & f32u; 
    f32u += (UINT32_C(127) - UINT32_C(15)) << 23; 

    
    if (exp == shifted_exp) 
      f32u += (UINT32_C(128) - UINT32_C(16)) << 23; 
    else if (exp == 0) { 
      f32u += (1) << 23; 
      f32u = simde_float32_as_uint32(simde_uint32_as_float32(f32u) - denorm_magic); 
    }

    f32u |= (half & UINT32_C(0x8000)) << 16; 
