...
function LegacyPresentationControlService() {
  DEBUG && log("LegacyPresentationControlService - ctor"); 
  this._id = null;
}

LegacyPresentationControlService.prototype = {
  startServer: function() {
    DEBUG && log("LegacyPresentationControlService - doesn't support receiver mode"); 
  ...
  connect: function(aDeviceInfo) {
    if (!this.id) {
      DEBUG && log("LegacyPresentationControlService - Id has not initialized; requestSession fails"); 
      return null;
    }
    DEBUG && log("LegacyPresentationControlService - requestSession to " + aDeviceInfo.id); 
    ...
    try {
      ...
      DEBUG && log("LegacyPresentationControlService - createTransport throws: " + e); 
  ...
  close: function() {
    DEBUG && log("LegacyPresentationControlService - close"); 
  },
...
function LegacyTCPControlChannel(id,
                                 transport,
                                 deviceInfo) {
  DEBUG && log("create LegacyTCPControlChannel"); 
...
LegacyTCPControlChannel.prototype = {
  ...
  _sendMessage: function(aJSONData, aOnThrow) {
    ...
    if (!this._connected) {
      DEBUG && log("LegacyTCPControlChannel - send" + aJSONData.type + " fails"); 
      throw Cr.NS_ERROR_FAILURE;
    }
  ...
  _send: function(aMsg) {
    DEBUG && log("LegacyTCPControlChannel - Send: " + JSON.stringify(aMsg, null, 2)); 
    ...
    try {
      ...
      DEBUG && log("LegacyTCPControlChannel - Failed to send message: " + e.name); 
      throw e;
    }
  },
  ...
  onInputStreamReady: function(aStream) {
    try {
      ...
      DEBUG && log("LegacyTCPControlChannel - onInputStreamReady error: " + e.name); 
  ...
  onStartRequest: function() {
    DEBUG && log("LegacyTCPControlChannel - onStartRequest"); 
  },

  
  onStopRequest: function(aRequest, aContext, aStatus) {
    DEBUG && log("LegacyTCPControlChannel - onStopRequest: " + aStatus); 
  ...
  onDataAvailable: function(aRequest, aContext, aInputStream, aOffset, aCount) { 
    ...
    DEBUG && log("LegacyTCPControlChannel - onDataAvailable: " + data); 
    ...
    for (let json of jsonArray) {
      ...
      try {
        ...
        DEBUG && log("LegacyTCPSignalingChannel - error in parsing json: " + e); 
      }
  ...
  _createInputStreamPump: function() {
    DEBUG && log("LegacyTCPControlChannel - create pump"); 
  ...
  set listener(aListener) {
    DEBUG && log("LegacyTCPControlChannel - set listener: " + aListener); 
    if (!aListener) {
      this._listener = null;
      return;
    }
    ...
    if (this._pendingOpen) {
      ...
      DEBUG && log("LegacyTCPControlChannel - notify pending opened"); 
    ...
    if (this._pendingClose) {
      DEBUG && log("LegacyTCPControlChannel - notify pending closed"); 
  ...
  _onAnswer: function(aAnswer) {
    ...
    DEBUG && log("LegacyTCPControlChannel - notify answer: " + JSON.stringify(aAnswer)); 
  ...
  _notifyConnected: function() {
    ...
    DEBUG && log("LegacyTCPControlChannel - notify opened"); 
    this._listener.notifyConnected();
  },
  ...
  _notifyDisconnected: function(aReason) {
    ...
    DEBUG && log("LegacyTCPControlChannel - notify closed"); 
    this._listener.notifyDisconnected(aReason);
  },

  disconnect: function(aReason) {
    DEBUG && log("LegacyTCPControlChannel - close with reason: " + aReason); 
...
this.NSGetFactory = XPCOMUtils.generateNSGetFactory([LegacyPresentationControlService]); 
