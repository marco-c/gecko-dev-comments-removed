...
using data::FileSystemDataManagerVersion001;
...
void MakeDataManagerVersion001(FileSystemDataManagerVersion001*& aResult) {
  nsCOMPtr<nsIFile> testPath;

  auto rv = NS_GetSpecialDirectory(NS_OS_TEMP_DIR, getter_AddRefs(testPath));
  ASSERT_NSEQ(NS_OK, rv);

  const Origin& testOrigin = getTestOrigin();

  rv = testPath->AppendRelativeNativePath(testOrigin);
  ASSERT_NSEQ(NS_OK, rv);

  rv = testPath->AppendRelativePath(u"filesystem"_ns);
  ASSERT_NSEQ(NS_OK, rv);
  ...
  if (exists) {
    rv = testPath->Remove( true);
    ASSERT_NSEQ(NS_OK, rv);
  }

  rv = testPath->Create(nsIFile::DIRECTORY_TYPE, (uint32_t)0644,
                         false);
  ASSERT_NSEQ(NS_OK, rv);

  nsCOMPtr<nsIFile> databaseFile;
  rv = testPath->Clone(getter_AddRefs(databaseFile));
  ASSERT_NSEQ(NS_OK, rv);

  rv = databaseFile->AppendRelativePath(u"metadata.sqlite"_ns);
  ASSERT_NSEQ(NS_OK, rv);

  rv = databaseFile->Create(nsIFile::NORMAL_FILE_TYPE, (uint32_t)0644);
  ASSERT_NSEQ(NS_OK, rv);
  ...
  rv = storageService->OpenDatabase(databaseFile, flags,
...
{
  nsresult rv = NS_OK;
  FileSystemDataManagerVersion001* rdm = nullptr;
  ASSERT_NO_FATAL_FAILURE(MakeDataManagerVersion001(rdm));
  UniquePtr<FileSystemDataManagerVersion001> dm(rdm);

  TEST_TRY_UNWRAP(EntryId rootId, data::GetRootHandle(getTestOrigin()));

  FileSystemChildMetadata firstChildMeta(rootId, u"First"_ns);
  TEST_TRY_UNWRAP_ERR(
      rv, dm->GetOrCreateDirectory(firstChildMeta,  false));
  ASSERT_NSEQ(NS_ERROR_DOM_NOT_FOUND_ERR, rv);

  TEST_TRY_UNWRAP(EntryId firstChild,
                  dm->GetOrCreateDirectory(firstChildMeta,  true));
  ...
  TEST_TRY_UNWRAP(EntryId secondChild,
                  dm->GetOrCreateDirectory(secondChildMeta,  true));
...
TEST(TestDataManagerVersion001, smokeTestCreateRemoveFiles)
{
  ...
  TEST_TRY_UNWRAP_ERR(rv,
                      dm->GetOrCreateFile(firstChildMeta,  false));
  ASSERT_NSEQ(NS_ERROR_DOM_NOT_FOUND_ERR, rv);
  ...
  TEST_TRY_UNWRAP_ERR(
      rv, dm->RemoveDirectory(secondChildMeta,  false));
  ASSERT_NSEQ(NS_ERROR_DOM_FILEHANDLE_NOT_ALLOWED_ERR,
              rv);  

  
  TEST_TRY_UNWRAP(bool isDeleted,
                  dm->RemoveDirectory(secondChildMeta,  true));
  ASSERT_TRUE(isDeleted);
