...
Result<Usage, QMResult> FileSystemDataManagerVersion001::GetUsage() const {
...
nsresult FileSystemDataManagerVersion001::UpdateUsage(int64_t aDelta) {
...
Result<EntryId, QMResult> FileSystemDataManagerVersion001::GetOrCreateDirectory(
  ...
  if (exists) {
    return Err(QMResult(NS_ERROR_STORAGE_CONSTRAINT));
  }
...
Result<EntryId, QMResult> FileSystemDataManagerVersion001::GetOrCreateFile(
    const FileSystemChildMetadata& aHandle, bool aCreate) {
  MOZ_ASSERT(!aHandle.parentId().IsEmpty());
...
FileSystemDataManagerVersion001::GetDirectoryEntries(const EntryId& aParent,
                                                     PageNumber aPage) const {
...
nsresult FileSystemDataManagerVersion001::GetFile(
...
Result<bool, QMResult> FileSystemDataManagerVersion001::RemoveDirectory(
...
Result<bool, QMResult> FileSystemDataManagerVersion001::RemoveFile(
...
Result<Path, QMResult> FileSystemDataManagerVersion001::Resolve(
...
FileSystemDataManagerVersion001::~FileSystemDataManagerVersion001() {
  mConnection->Close();
}
