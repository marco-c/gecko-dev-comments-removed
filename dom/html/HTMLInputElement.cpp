...
static StyleAbsoluteColor ComputeColor(Document* aDocument,
...
nsTArray<nsString> HTMLInputElement::GetColorsFromList() {
  ...
  for (uint32_t i = 0; i < length; ++i) {
    ...
    StyleAbsoluteColor result = ComputeColor(OwnerDoc(), value);
...
void HTMLInputElement::SanitizeValue(nsAString& aValue,
  ...
  switch (mType) {
    ...
    case FormControlType::InputColor: {
      ...
      StyleAbsoluteColor color = ComputeColor(OwnerDoc(), aValue);
