...
add_task(async function() {
  ...
  await testContentVisibilityState(false , browserTest);

  info("test window should report 'hidden' if it is fully covered by another " +
       "window");
  await new Promise(resolve => waitForFocus(resolve, window));
  await waitContentVisibilityChange(true , browserTest);
  ...
  await testContentVisibilityState(true , browserTest);
  ...
  await waitContentVisibilityChange(false , browserTest);
