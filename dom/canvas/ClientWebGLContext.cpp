...
ClientWebGLContext::MakeCrossProcessWebGLContext(WebGLVersion aVersion) {
  ...
  static const size_t CommandQueueSize = 256 * 1024;  
  static const size_t ResponseQueueSize = 8 * 1024;   
  ...
  static const size_t ErrorQueueSize = 4 * 1024;  
 ...
 RefPtr<ClientWebGLContext> ClientWebGLContext::Create(
...
void ClientWebGLContext::DrainErrorQueue(bool toReissue) {
  ...
  if ((!success) ||
      NS_FAILED(NS_DelayedDispatchToCurrentThread(
          drainErrorRunnable.downcast<nsIRunnable>(), 10 ))) {
