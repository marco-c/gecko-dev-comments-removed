...
static std::string ChooseDeviceReplacement(const std::string& str) {
  ...
  if (str.find("FirePro") != -1 || str.find("Radeon") != -1) {
    ...
    const auto RADEON_GCN_GEN2 = RADEON_R9_290;  
  ...
  if (str.find("GeForce") != -1 || str.find("Quadro") != -1) {
    ...
    if (std::regex_search(str, m, kQuadro)) {
      if (str.find("RTX") != -1) return GEFORCE_980;
  ...
  if (str.find("Vivante") != -1) return "Vivante GC1000";
  if (str.find("VideoCore") != -1) return "VideoCore IV HW";
  if (str.find("Tegra") != -1) return "NVIDIA Tegra";

  

  static const std::string D3D_WARP = "Microsoft Basic Render Driver";
  if (str.find(D3D_WARP) != -1) return str;
