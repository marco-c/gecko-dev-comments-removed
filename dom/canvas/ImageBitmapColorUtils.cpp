               ...
               int aWidth, int aHeight)
{
  for (int i = 0; i < aHeight; ++i) {
    ...
    for (int j = 0; j < aWidth; ++j) {
      ...
      const float diff = max - min + EPSILON; 
...
static const float _magic = std::pow((6.0 / 29.0), 3.0); 
static const float _1_3 = 1.0f / 3.0f;
static const float _a = std::pow((29.0 / 6.0), 2.0) / 3.0; 
static const float _b = 16.0f / 116.0f; 
