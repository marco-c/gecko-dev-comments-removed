...
goog.scope(function() {
    ...
    var deString = framework.delibs.debase.deString;
     ...
     es3fSyncTests.NUM_CASE_ITERATIONS = 5;
     es3fSyncTests.MAX_VERIFY_WAIT = 5;
    ...
    es3fSyncTests.FenceSyncCase = function(name, description, numPrimitives, waitCommand, waitFlags, timeout, options) {
        tcuTestCase.DeqpTest.call(this, name, description);
         ...
         this.m_waitFlags = waitFlags;
         this.m_timeout = timeout;
         this.m_caseOptions = options;

         this.m_program = null;
         this.m_syncObject = null;
         this.m_iterNdx = 0;
         this.m_rnd = new deRandom.Random(deString.deStringHash(this.name));
         this.m_state = es3fSyncTests.State.DRAW;
    };
    ...
    es3fSyncTests.generateVertices = function(numPrimitives, rnd) {
         var dst = [];
         var numVertices = 3 * numPrimitives;

        for (var i = 0; i < numVertices; i++) {
            dst.push(rnd.getFloat(-1.0, 1.0)); 
            dst.push(rnd.getFloat(-1.0, 1.0)); 
            dst.push(rnd.getFloat(0.0, 1.0)); 
            dst.push(1.0); 
        }
        return dst;
    };
    ...
    es3fSyncTests.FenceSyncCase.prototype.draw = function() {
         ...
         var header = 'Case iteration ' + (this.m_iterNdx + 1) + ' / ' + es3fSyncTests.NUM_CASE_ITERATIONS;
        ...
        vertices = es3fSyncTests.generateVertices(this.m_numPrimitives, this.m_rnd);
         ...
         var vertexGLBuffer = gl.createBuffer();
