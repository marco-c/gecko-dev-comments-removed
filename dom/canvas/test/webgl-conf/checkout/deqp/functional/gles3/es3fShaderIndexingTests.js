...
goog.scope(function() {
    ...
    es3fShaderIndexingTests.ShaderIndexingCase = function(name, description, isVertexCase, varType, evalFunc, vertShaderSource, fragShaderSource) {
        glsShaderRenderCase.ShaderRenderCase.call(this, name, description, isVertexCase, evalFunc);
         ...
         this.m_vertShaderSource    = vertShaderSource;
         this.m_fragShaderSource    = fragShaderSource;
    };
    ...
    es3fShaderIndexingTests.ShaderIndexingCase.prototype.setupUniforms = function(programID, constCoords) {
         var arr = [];
         var array1d = [];
         var arrLoc = gl.getUniformLocation(programID, 'u_arr');
        if (arrLoc != null) {
    ...
    es3fShaderIndexingTests.createVaryingArrayCase = function(caseName, description, varType, vertAccess, fragAccess) {
        ...
        if (varType === gluShaderUtil.DataType.FLOAT)
         ...
         var vertexShaderSource = tcuStringTemplate.specialize(vtx, params);
    	 var fragmentShaderSource = tcuStringTemplate.specialize(frag, params);
    ...
    es3fShaderIndexingTests.createVectorSubscriptCase = function(caseName, description, isVertexCase, varType, writeAccess, readAccess) {
         ...
         var vecLen = gluShaderUtil.getDataTypeScalarSize(varType);
         var vecLenName = glsShaderRenderCase.getIntUniformName(vecLen);
        ...
        frag += '}\n';
         ...
         var s_swizzles = ['', 'x', 'xy', 'xyz', 'xyzw'];
         var s_rotSwizzles = ['', 'x', 'yx', 'yzx', 'yzwx'];
    ...
    es3fShaderIndexingTests.createMatrixSubscriptCase = function(caseName, description, isVertexCase, varType, writeAccess, readAccess) {
         ...
         var numCols = gluShaderUtil.getDataTypeMatrixNumColumns(varType);
         var numRows = gluShaderUtil.getDataTypeMatrixNumRows(varType);
         var matSizeName = glsShaderRenderCase.getIntUniformName(numCols);
         var vecType = gluShaderUtil.getDataTypeFloatVec(numRows);
    ...
    es3fShaderIndexingTests.ShaderIndexingTests.prototype.init = function() {
        var testGroup = tcuTestCase.runner.testCases;
         ...
         var varyingGroup = tcuTestCase.newTest('varying_array', 'Varying array access tests.');
        ...
        for (var typeNdx = 0; typeNdx < s_floatAndVecTypes.length; typeNdx++) {
            ...
            for (var vertAccessStr in es3fShaderIndexingTests.IndexAccessType) {
                for (var fragAccessStr in es3fShaderIndexingTests.IndexAccessType) {
                    ...
                    var fragAccess = es3fShaderIndexingTests.IndexAccessType[fragAccessStr];
                     var vertAccessName = es3fShaderIndexingTests.getIndexAccessTypeName(vertAccess);
                     var fragAccessName = es3fShaderIndexingTests.getIndexAccessTypeName(fragAccess);
         ...
         var uniformGroup = tcuTestCase.newTest("uniform_array", "Uniform array access tests.");
        ...
        for (var typeNdx = 0; typeNdx < s_floatAndVecTypes.length; typeNdx++) {
         ...
         var tmpGroup = tcuTestCase.newTest("tmp_array", "Temporary array access tests.");
        ...
        for (var typeNdx = 0; typeNdx < s_vectorTypes.length; typeNdx++) {
             var vecGroup = tcuTestCase.newTest("vector_subscript", "Vector subscript indexing.");
