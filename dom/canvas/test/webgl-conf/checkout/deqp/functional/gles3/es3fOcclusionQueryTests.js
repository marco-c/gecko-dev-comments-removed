...
var setParentClass = function(child, parent) {
 ...
 var DEPTH_WRITE_COLOR = [0, 0, 1, 1];
 var DEPTH_CLEAR_COLOR = [0, 0.5, 0.8, 1];
 var STENCIL_WRITE_COLOR = [0, 1, 0, 1];
 var STENCIL_CLEAR_COLOR = [0, 0.8, 0.5, 1];
 var TARGET_COLOR = [1, 0, 0, 1];
 var ELEMENTS_PER_VERTEX = 4;
 var NUM_CASE_ITERATIONS = 10;



 var DEPTH_CLEAR_OFFSET = 100;
 var STENCIL_CLEAR_OFFSET = 100;
 var SCISSOR_OFFSET = 100;
 var SCISSOR_MINSIZE = 250;

 var OCCLUDER_SCISSOR = (1 << 0);
 var OCCLUDER_DEPTH_WRITE = (1 << 1);
 var OCCLUDER_DEPTH_CLEAR = (1 << 2);
 var OCCLUDER_STENCIL_WRITE = (1 << 3);
 var OCCLUDER_STENCIL_CLEAR = (1 << 4);
...
es3fOcclusionQueryTests.OcclusionQueryCase.prototype.generateVertices = function(width, height, primitiveCount, verticesPerPrimitive, rnd, primitiveSize, minZ, maxZ) {
    ...
    for (var i = 0; i < vertexCount; i += 3) {
        ...
        for (var j = 0; j < verticesPerPrimitive; j++) {
            ...
            dst[offset] = rndX + rnd.getFloat(-s, s); 
            dst[offset + 1] = rndY + rnd.getFloat(-s, s); 
            dst[offset + 2] = rnd.getFloat(minZ, maxZ); 
            dst[offset + 3] = 1; 
        }
    }
    return dst;
...
es3fOcclusionQueryTests.OcclusionQueryCase.prototype.draw = function() {
    ...
    for (var i = 0; i < this.m_numOccluderDraws; i++) {
        ...
        var type = occOptions[this.m_rnd.getInt(0, occOptions.length - 1)]; 

        switch (type) {
            case OCCLUDER_DEPTH_WRITE:
                bufferedLogToConsole('Occluder draw ' + (i + 1) + ' / ' + this.m_numOccluderDraws + ' : ' + 'Depth write');

                var occluderVertices = this.generateVertices(2, 2, this.m_numOccludersPerDraw, 3, this.m_rnd, this.m_occluderSize, 0, 0.6); 
    ...
    for (var i = 0; i < this.m_numTargetDraws; i++) {
        var targetVertices = this.generateVertices(2, 2, this.m_numTargetsPerDraw, 3, this.m_rnd, this.m_targetSize, 0.4, 1); 
...
es3fOcclusionQueryTests.OcclusionQueryCase.prototype.verify = function() {
    ...
    var resultAvailable =  (gl.getQueryParameter(this.m_query, gl.QUERY_RESULT_AVAILABLE));
    if (!resultAvailable) {
    ...
    var result =  (gl.getQueryParameter(this.m_query, gl.QUERY_RESULT));
