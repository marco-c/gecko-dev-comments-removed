...
goog.scope(function() {
    ...
    es3fReadPixelTests.ReadPixelsTest = function(name, description, chooseFormat, alignment, rowLength, skipRows, skipPixels, format, type) {
        tcuTestCase.DeqpTest.call(this, name, description);
         ...
         this.m_seed = deString.deStringHash(name);
         this.m_chooseFormat = chooseFormat;
         this.m_alignment = alignment;
         this.m_rowLength = rowLength;
         this.m_skipRows = skipRows;
         this.m_skipPixels = skipPixels;
         this.m_format = format !== undefined ? format : gl.RGBA;
         this.m_type = type !== undefined ? type : gl.UNSIGNED_BYTE;
    ...
    es3fReadPixelTests.ReadPixelsTest.prototype.render = function(reference) {
        var refType =  (reference.getFormat().type);
         var width = reference.getWidth();
         var height = reference.getHeight();
         var level0 = reference.getLevel(0);
        ...
        fragmentSource += '}\n';
         ...
         var program = new gluShaderProgram.ShaderProgram(gl, gluShaderProgram.makeVtxFragSources(vertexSource, fragmentSource));
        ...
        gl.enableVertexAttribArray(coordLoc);
         ...
         var coordsGLBuffer = gl.createBuffer();
        ...
        gl.disableVertexAttribArray(coordLoc);
         ...
         var coordX1 = Math.floor((-0.5 * width / 2.0) + width / 2.0);
         var coordY1 = Math.floor((-0.5 * height / 2.0) + height / 2.0);
         var coordX2 = Math.floor((0.5 * width / 2.0) + width / 2.0);
         var coordY2 = Math.floor((0.5 * height / 2.0) + height / 2.0);
    ...
    es3fReadPixelTests.ReadPixelsTest.prototype.getFormatInfo = function() {
        if (this.m_chooseFormat) {
            this.m_format =  (gl.getParameter(gl.IMPLEMENTATION_COLOR_READ_FORMAT));
            this.m_type =  (gl.getParameter(gl.IMPLEMENTATION_COLOR_READ_TYPE));
        }

         var fmt = gluTextureUtil.mapGLTransferFormat(this.m_format, this.m_type);
         var align_;
        switch (this.m_type) {
            case gl.BYTE:
    ...
    es3fReadPixelTests.ReadPixelsTest.prototype.clearColor = function(reference, align, pixelSize) {
         ...
         var rnd = new deRandom.Random(this.m_seed);
         var framebuffer;
         var renderbuffer;
         var red;
         var green;
        ...
        } else if (this.m_format === gl.RGBA ||  this.m_format === gl.RGB) {
        ...
        this.render(reference);
         ...
         var rowWidth = (this.m_rowLength === 0 ? this.m_width : this.m_rowLength) + this.m_skipPixels;
         var rowPitch = (align ? this.m_alignment * Math.ceil(pixelSize * rowWidth / this.m_alignment) : rowWidth * pixelSize);

        var arrayType = tcuTexture.getTypedArray(reference.getFormat().type);
         var pixelData = new arrayType(rowPitch * (this.m_height + this.m_skipRows));
    ...
    es3fReadPixelTests.ReadPixelsTest.prototype.iterate = function() {
         var format = new tcuTexture.TextureFormat(tcuTexture.ChannelOrder.RGBA, tcuTexture.ChannelType.UNORM_INT8);
         var pixelSize;
         var align;
        ...
        bufferedLogToConsole('Format: ' + this.m_format + ', Type: ' + this.m_type);
         ...
         var reference = new tcuTexture.Texture2D(format, this.m_width, this.m_height);
        reference.allocLevel(0);
         var level0 = reference.getLevel(0);

        this.m_alignment =  (gl.getParameter(gl.PACK_ALIGNMENT));
        bufferedLogToConsole('gl.PACK_ALIGNMENT: ' + this.m_alignment);

        this.m_rowLength =  (gl.getParameter(gl.PACK_ROW_LENGTH));
        bufferedLogToConsole('gl.PACK_ROW_LENGTH: ' + this.m_rowLength);

        this.m_skipRows =  (gl.getParameter(gl.PACK_SKIP_ROWS));
        bufferedLogToConsole('gl.PACK_SKIP_ROWS: ' + this.m_skipRows);

        this.m_skipPixels =  (gl.getParameter(gl.PACK_SKIP_PIXELS));
        bufferedLogToConsole('gl.PACK_SKIP_PIXELS: ' + this.m_skipPixels);

        gl.viewport(0, 0, this.m_width, this.m_height);

         var pixelData = this.clearColor(reference, align, pixelSize);

         var rowWidth = (this.m_rowLength === 0 ? this.m_width : this.m_rowLength);
         var rowPitch = (align ? this.m_alignment * Math.ceil(pixelSize * rowWidth / this.m_alignment) : rowWidth * pixelSize);
         var formatBitDepths = [];
         var redThreshold;
         var greenThreshold;
        ...
        if (this.m_format !== gl.RGBA_INTEGER &&  (gl.getParameter(gl.SAMPLES)) > 1) {
    ...
    es3fReadPixelTests.ReadPixelTests.prototype.init = function() {
         var groupAlignment = tcuTestCase.newTest('alignment', 'Read pixels pack alignment parameter tests');
        ...
        this.addChild(groupAlignment);
         ...
         var groupRowLength = tcuTestCase.newTest('rowlength', 'Read pixels rowlength test');
        ...
        this.addChild(groupRowLength);
         ...
         var groupSkip = tcuTestCase.newTest('skip', 'Read pixels skip pixels and rows test');
