...
var deMath = framework.delibs.debase.deMath;
     ...
     es3fInstancedRenderingTests.MAX_RENDER_WIDTH = 128;
     es3fInstancedRenderingTests.MAX_RENDER_HEIGHT = 128;

     es3fInstancedRenderingTests.QUAD_GRID_SIZE = 127;

    
    es3fInstancedRenderingTests.ATTRIB_DIVISOR_R = 3;
    es3fInstancedRenderingTests.ATTRIB_DIVISOR_G = 2;
    es3fInstancedRenderingTests.ATTRIB_DIVISOR_B = 1;

    es3fInstancedRenderingTests.OFFSET_COMPONENTS = 3; 

    
    es3fInstancedRenderingTests.FLOAT_INT_SCALE = 100.0;
    es3fInstancedRenderingTests.FLOAT_INT_BIAS = -50.0;
    es3fInstancedRenderingTests.FLOAT_UINT_SCALE = 100.0;
    es3fInstancedRenderingTests.FLOAT_UINT_BIAS = 0.0;
    ...
    es3fInstancedRenderingTests.InstancedRenderingCase = function(name, description, drawFunction, instancingType, rgbAttrType, numInstances) {
        tcuTestCase.DeqpTest.call(this, name, description);
         this.m_function = drawFunction;
         this.m_instancingType = instancingType;
         this.m_rgbAttrType = rgbAttrType;
         this.m_numInstances = numInstances;
         this.m_program = null;
         this.m_gridVertexPositions = [];
         this.m_gridIndices = [];
         this.m_instanceOffsets = [];
         this.m_instanceColorR = [];
         this.m_instanceColorG = [];
         this.m_instanceColorB = [];
    };
    ...
    es3fInstancedRenderingTests.InstancedRenderingCase.prototype.init = function() {
        ...
        gl.getError();
         ...
         var isUintCase = gluShaderUtil.isDataTypeUintOrUVec(this.m_rgbAttrType);
         var isMatCase = gluShaderUtil.isDataTypeMatrix(this.m_rgbAttrType);
         var typeSize = gluShaderUtil.getDataTypeScalarSize(this.m_rgbAttrType);
         var isScalarCase = typeSize == 1;
         var swizzleFirst = isScalarCase ? '' : '.x';
         var typeName = gluShaderUtil.getDataTypeName(this.m_rgbAttrType);

         var floatIntScaleStr = '(' + es3fInstancedRenderingTests.FLOAT_INT_SCALE.toFixed(3) + ')';
         var floatIntBiasStr = '(' + es3fInstancedRenderingTests.FLOAT_INT_BIAS.toFixed(3) + ')';
         var floatUintScaleStr = '(' + es3fInstancedRenderingTests.FLOAT_UINT_SCALE.toFixed(3) + ')';
         var floatUintBiasStr = '(' + es3fInstancedRenderingTests.FLOAT_UINT_BIAS.toFixed(3) + ')';
        ...
        DE_ASSERT(isFloatCase || isIntCase || isUintCase || isMatCase);
         ...
         var numInstancesStr = this.m_numInstances.toString() + '.0';

         var instanceAttribs = '';
         var posExpression = '';
         var colorRExpression = '';
         var colorGExpression = '';
         var colorBExpression = '';
        ...
        if (this.m_function == es3fInstancedRenderingTests.DrawFunction.FUNCTION_DRAW_ELEMENTS_INSTANCED) {
            ...
            for (var y = 0; y < es3fInstancedRenderingTests.QUAD_GRID_SIZE + 1; y++)
                for (var x = 0; x < es3fInstancedRenderingTests.QUAD_GRID_SIZE + 1; x++) {
                     var fx = -1.0 + x / es3fInstancedRenderingTests.QUAD_GRID_SIZE * 2.0 / this.m_numInstances;
                     var fy = -1.0 + y / es3fInstancedRenderingTests.QUAD_GRID_SIZE * 2.0;
            ...
            for (var y = 0; y < es3fInstancedRenderingTests.QUAD_GRID_SIZE; y++)
                for (var x = 0; x < es3fInstancedRenderingTests.QUAD_GRID_SIZE; x++) {
                     var ndx00 = y * (es3fInstancedRenderingTests.QUAD_GRID_SIZE + 1) + x;
                     var ndx10 = y * (es3fInstancedRenderingTests.QUAD_GRID_SIZE + 1) + x + 1;
                     var ndx01 = (y + 1) * (es3fInstancedRenderingTests.QUAD_GRID_SIZE + 1) + x;
                     var ndx11 = (y + 1) * (es3fInstancedRenderingTests.QUAD_GRID_SIZE + 1) + x + 1;
            ...
            for (var y = 0; y < es3fInstancedRenderingTests.QUAD_GRID_SIZE; y++)
                for (var x = 0; x < es3fInstancedRenderingTests.QUAD_GRID_SIZE; x++) {
                     var fx0 = -1.0 + (x + 0) / es3fInstancedRenderingTests.QUAD_GRID_SIZE * 2.0 / this.m_numInstances;
                     var fx1 = -1.0 + (x + 1) / es3fInstancedRenderingTests.QUAD_GRID_SIZE * 2.0 / this.m_numInstances;
                     var fy0 = -1.0 + (y + 0) / es3fInstancedRenderingTests.QUAD_GRID_SIZE * 2.0;
                     var fy1 = -1.0 + (y + 1) / es3fInstancedRenderingTests.QUAD_GRID_SIZE * 2.0;
        ...
        if (this.m_instancingType == es3fInstancedRenderingTests.InstancingType.TYPE_ATTRIB_DIVISOR || this.m_instancingType == es3fInstancedRenderingTests.InstancingType.TYPE_MIXED) {
            if (this.m_instancingType == es3fInstancedRenderingTests.InstancingType.TYPE_ATTRIB_DIVISOR) {
                ...
                for (var i = 0; i < this.m_numInstances; i++) {
                 ...
                 var rInstances = Math.floor(this.m_numInstances / es3fInstancedRenderingTests.ATTRIB_DIVISOR_R) + (this.m_numInstances % es3fInstancedRenderingTests.ATTRIB_DIVISOR_R == 0 ? 0 : 1);
             ...
             var gInstances = Math.floor(this.m_numInstances / es3fInstancedRenderingTests.ATTRIB_DIVISOR_G) + (this.m_numInstances % es3fInstancedRenderingTests.ATTRIB_DIVISOR_G == 0 ? 0 : 1);
            for (var i = 0; i < gInstances; i++) {
             ...
             var bInstances = Math.floor(this.m_numInstances / es3fInstancedRenderingTests.ATTRIB_DIVISOR_B) + (this.m_numInstances % es3fInstancedRenderingTests.ATTRIB_DIVISOR_B == 0 ? 0 : 1);
    ...
    es3fInstancedRenderingTests.InstancedRenderingCase.prototype.deinit = function() {
        var numVertexAttribArrays =  (gl.getParameter(gl.MAX_VERTEX_ATTRIBS));
    ...
    es3fInstancedRenderingTests.InstancedRenderingCase.prototype.iterate = function() {
         var width = Math.min(gl.drawingBufferWidth, es3fInstancedRenderingTests.MAX_RENDER_WIDTH);
         var height = Math.min(gl.drawingBufferHeight, es3fInstancedRenderingTests.MAX_RENDER_HEIGHT);

         var xOffsetMax = gl.drawingBufferWidth - width;
         var yOffsetMax = gl.drawingBufferHeight - height;

         var rnd = new deRandom.Random(deString.deStringHash(this.name));

         var xOffset = rnd.getInt(0, xOffsetMax);
         var yOffset = rnd.getInt(0, yOffsetMax);

         var referenceImg = new tcuSurface.Surface(width, height);
         var resultImg = new tcuSurface.Surface(width, height);
     ...
     var testOk = tcuImageCompare.fuzzyCompare('ComparisonResult', 'Image comparison result', referenceImg.getAccess(), resultImg.getAccess(), 0.05 );
    ...
    es3fInstancedRenderingTests.InstancedRenderingCase.prototype.setupVarAttribPointer = function(attrPtr, location, divisor) {
         ...
         var isMatCase = gluShaderUtil.isDataTypeMatrix(this.m_rgbAttrType);
         var typeSize = gluShaderUtil.getDataTypeScalarSize(this.m_rgbAttrType);
         var numSlots = isMatCase ? gluShaderUtil.getDataTypeMatrixNumColumns(this.m_rgbAttrType) : 1; 

        for (var slotNdx = 0; slotNdx < numSlots; slotNdx++) {
             var curLoc = location + slotNdx;
            ...
            if (isFloatCase) {
                ...
                gl.vertexAttribIPointer(curLoc, typeSize, gl.UNSIGNED_INT, 0, 0);
                 ...
                 var numRows = gluShaderUtil.getDataTypeMatrixNumRows(this.m_rgbAttrType);
                 var numCols = gluShaderUtil.getDataTypeMatrixNumColumns(this.m_rgbAttrType);
    ...
    es3fInstancedRenderingTests.InstancedRenderingCase.prototype.setupAndRender = function() {
        ...
        if (this.m_instancingType == es3fInstancedRenderingTests.InstancingType.TYPE_ATTRIB_DIVISOR || this.m_instancingType == es3fInstancedRenderingTests.InstancingType.TYPE_MIXED) {
            if (this.m_instancingType == es3fInstancedRenderingTests.InstancingType.TYPE_ATTRIB_DIVISOR) {
                 ...
                 var offsetLoc = gl.getAttribLocation(program, 'a_instanceOffset');
    ...
    es3fInstancedRenderingTests.InstancedRenderingCase.prototype.computeReference = function(dst) {
         var wid = dst.getWidth();
         var hei = dst.getHeight();

        

        for (var instanceNdx = 0; instanceNdx < this.m_numInstances; instanceNdx++) {
             var xStart = Math.floor(instanceNdx * wid / this.m_numInstances);
             var xEnd = Math.floor((instanceNdx + 1) * wid / this.m_numInstances);

            

             var clrNdxR = this.m_instancingType == es3fInstancedRenderingTests.InstancingType.TYPE_ATTRIB_DIVISOR ? Math.floor(instanceNdx / es3fInstancedRenderingTests.ATTRIB_DIVISOR_R) : instanceNdx;
             var clrNdxG = this.m_instancingType == es3fInstancedRenderingTests.InstancingType.TYPE_ATTRIB_DIVISOR || this.m_instancingType == es3fInstancedRenderingTests.InstancingType.TYPE_MIXED ? Math.floor(instanceNdx / es3fInstancedRenderingTests.ATTRIB_DIVISOR_G) : instanceNdx;
             var clrNdxB = this.m_instancingType == es3fInstancedRenderingTests.InstancingType.TYPE_ATTRIB_DIVISOR || this.m_instancingType == es3fInstancedRenderingTests.InstancingType.TYPE_MIXED ? Math.floor(instanceNdx / es3fInstancedRenderingTests.ATTRIB_DIVISOR_B) : instanceNdx;

             var rInstances = this.m_instancingType == es3fInstancedRenderingTests.InstancingType.TYPE_ATTRIB_DIVISOR ? Math.floor(this.m_numInstances / es3fInstancedRenderingTests.ATTRIB_DIVISOR_R) + (this.m_numInstances % es3fInstancedRenderingTests.ATTRIB_DIVISOR_R == 0 ? 0 : 1) : this.m_numInstances;
             var gInstances = this.m_instancingType == es3fInstancedRenderingTests.InstancingType.TYPE_ATTRIB_DIVISOR || this.m_instancingType == es3fInstancedRenderingTests.InstancingType.TYPE_MIXED ? Math.floor(this.m_numInstances / es3fInstancedRenderingTests.ATTRIB_DIVISOR_G) + (this.m_numInstances % es3fInstancedRenderingTests.ATTRIB_DIVISOR_G == 0 ? 0 : 1) : this.m_numInstances;
             var bInstances = this.m_instancingType == es3fInstancedRenderingTests.InstancingType.TYPE_ATTRIB_DIVISOR || this.m_instancingType == es3fInstancedRenderingTests.InstancingType.TYPE_MIXED ? Math.floor(this.m_numInstances / es3fInstancedRenderingTests.ATTRIB_DIVISOR_B) + (this.m_numInstances % es3fInstancedRenderingTests.ATTRIB_DIVISOR_B == 0 ? 0 : 1) : this.m_numInstances;

            

             var r = clrNdxR / rInstances;
             var g = clrNdxG * 2.0 / gInstances;
             var b = 1.0 - clrNdxB / bInstances;

            

            if (gluShaderUtil.isDataTypeIntOrIVec(this.m_rgbAttrType)) {
                var intR = (r * es3fInstancedRenderingTests.FLOAT_INT_SCALE + es3fInstancedRenderingTests.FLOAT_INT_BIAS);
                var intG = (g * es3fInstancedRenderingTests.FLOAT_INT_SCALE + es3fInstancedRenderingTests.FLOAT_INT_BIAS);
                var intB = (b * es3fInstancedRenderingTests.FLOAT_INT_SCALE + es3fInstancedRenderingTests.FLOAT_INT_BIAS);
                ...
                var uintR = (r * es3fInstancedRenderingTests.FLOAT_UINT_SCALE + es3fInstancedRenderingTests.FLOAT_UINT_BIAS);
                var uintG = (g * es3fInstancedRenderingTests.FLOAT_UINT_SCALE + es3fInstancedRenderingTests.FLOAT_UINT_BIAS);
                var uintB = (b * es3fInstancedRenderingTests.FLOAT_UINT_SCALE + es3fInstancedRenderingTests.FLOAT_UINT_BIAS);
    ...
    es3fInstancedRenderingTests.init = function() {
     ...
     var instanceCounts = [1, 2, 4, 20];
        ...
        for (var _function in es3fInstancedRenderingTests.DrawFunction) {
            ...
            for (var instancingType in es3fInstancedRenderingTests.InstancingType) {
                ...
                for (var countNdx in instanceCounts) {
                     var countName = instanceCounts[countNdx].toString() + '_instances';
        ...
        [
         ...
         var typeTestNumInstances = 4;

         var typesGroup = tcuTestCase.newTest('types', 'Tests for instanced attributes of particular data types');

        testGroup.addChild(typesGroup);

        for (var typeNdx in s_testTypes) {
             var type = s_testTypes[typeNdx];
