...
goog.scope(function() {
        ...
        for (var inputTypeNdx = 0; inputTypeNdx < inputTypes.length; inputTypeNdx++) {
            for (var countNdx = 0; countNdx < counts.length; countNdx++) {
                for (var strideNdx = 0; strideNdx < strides.length; strideNdx++) {
                     var stride = (strides[strideNdx] < 0 ? glsVertexArrayTests.deArray.inputTypeSize(inputTypes[inputTypeNdx]) * 2 : strides[strideNdx]);
                     var aligned = (stride % glsVertexArrayTests.deArray.inputTypeSize(inputTypes[inputTypeNdx])) == 0;
                     var name = 'stride' + stride + '_' + glsVertexArrayTests.deArray.inputTypeToString(inputTypes[inputTypeNdx]) + '_quads' + counts[countNdx];
    ...
    es3fVertexArrayTests.SingleVertexArrayStrideGroup.prototype.init = function() {
        ...
        for (var storageNdx = 0; storageNdx < storages.length; storageNdx++) {
            for (var componentCount = 2; componentCount < 5; componentCount++) {
                for (var countNdx = 0; countNdx < counts.length; countNdx++) {
                    for (var strideNdx = 0; strideNdx < strides.length; strideNdx++) {
                         var packed = this.m_type == glsVertexArrayTests.deArray.InputType.UNSIGNED_INT_2_10_10_10 || this.m_type == glsVertexArrayTests.deArray.InputType.INT_2_10_10_10;
                         var stride = (strides[strideNdx] < 0) ? ((packed) ? (16) : (glsVertexArrayTests.deArray.inputTypeSize(this.m_type) * componentCount)) : (strides[strideNdx]);
                         var alignment = (packed) ? (glsVertexArrayTests.deArray.inputTypeSize(this.m_type) * componentCount) : (glsVertexArrayTests.deArray.inputTypeSize(this.m_type));
                         var bufferUnaligned = (storages[storageNdx] == glsVertexArrayTests.deArray.Storage.BUFFER) && (stride % alignment) != 0;

                         var name = glsVertexArrayTests.deArray.storageToString(storages[storageNdx]) + '_stride' + stride + '_components' + componentCount + '_quads' + counts[countNdx];
    ...
    es3fVertexArrayTests.SingleVertexArrayOutputTypeGroup.prototype.init = function() {
        ...
        var storages = [glsVertexArrayTests.deArray.Storage.BUFFER]; 
        var counts = [1, 256];
    ...
    es3fVertexArrayTests.MultiVertexArrayStrideTests.prototype.addStrideCases = function(spec, depth) {
        ...
        for (var strideNdx = 0; strideNdx < strides.length; strideNdx++) {
            ...
            var arraySpec = new glsVertexArrayTests.MultiVertexArrayTest.Spec.ArraySpec(
             ...
             var aligned = (stride % glsVertexArrayTests.deArray.inputTypeSize(inputType)) == 0;
            if (aligned) {
