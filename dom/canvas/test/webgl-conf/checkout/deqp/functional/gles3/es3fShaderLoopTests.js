...
es3fShaderLoopTests.LoopCase = {
    LOOPCASE_EMPTY_BODY: 0,                                
    LOOPCASE_INFINITE_WITH_UNCONDITIONAL_BREAK_FIRST: 1,        
    LOOPCASE_INFINITE_WITH_UNCONDITIONAL_BREAK_LAST: 2,        
    LOOPCASE_INFINITE_WITH_CONDITIONAL_BREAK: 3,                
    LOOPCASE_SINGLE_STATEMENT: 4,                                
    LOOPCASE_COMPOUND_STATEMENT: 5,                            
    LOOPCASE_SEQUENCE_STATEMENT: 6,                            
    LOOPCASE_NO_ITERATIONS: 7,                                    
    LOOPCASE_SINGLE_ITERATION: 8,                                
    LOOPCASE_SELECT_ITERATION_COUNT: 9,                        
    LOOPCASE_CONDITIONAL_CONTINUE: 10,                            
    LOOPCASE_UNCONDITIONAL_CONTINUE: 11,                        
    LOOPCASE_ONLY_CONTINUE: 12,                                    
    LOOPCASE_DOUBLE_CONTINUE: 13,                                
    LOOPCASE_CONDITIONAL_BREAK: 14,                                
    LOOPCASE_UNCONDITIONAL_BREAK: 15,                            
    LOOPCASE_PRE_INCREMENT: 16,                                    
    LOOPCASE_POST_INCREMENT: 17,                                
    LOOPCASE_MIXED_BREAK_CONTINUE: 18,
    LOOPCASE_VECTOR_COUNTER: 19,                                
    LOOPCASE_101_ITERATIONS: 20,                                
    LOOPCASE_SEQUENCE: 21,                                        
    LOOPCASE_NESTED: 22,                                        
    LOOPCASE_NESTED_SEQUENCE: 23,                                
    LOOPCASE_NESTED_TRICKY_DATAFLOW_1: 24,                        
    LOOPCASE_NESTED_TRICKY_DATAFLOW_2: 25                        
};
...
es3fShaderLoopTests.createGenericLoopCase = function(caseName, description, isVertexCase, loopType, loopCountType, loopCountPrecision, loopCountDataType) {
	...
	if (isVertexCase) {
	 ...
	 var numLoopIters = 3;
     var isIntCounter = gluShaderUtil.isDataTypeIntOrIVec(loopCountDataType);
	...
	if (isIntCounter) {
	 ...
	 var initValue = isIntCounter ? '0' : '0.05';
     var loopCountDeclStr = '' + gluShaderUtil.getPrecisionName(loopCountPrecision) + ' ' + gluShaderUtil.getDataTypeName(loopCountDataType) + ' ndx = ' + initValue;
     var loopCmpStr = 'ndx < ' + iterMaxStr;
     var incrementStr;
...
es3fShaderLoopTests.createSpecialLoopCase = function(caseName, description, isVertexCase, loopCase, loopType, loopCountType) {
    ...
    op += '    ${PRECISION} vec4 res = coords;\n';
     ...
     var counterPrecisionStr = 'mediump';
     var forLoopStr = '';
     var whileLoopStr = '';
     var doWhileLoopPreStr = '';
     var doWhileLoopPostStr = '';
...
es3fShaderLoopTests.ShaderLoopTests.prototype.init = function() {
    ...
    for (var loopType in es3fShaderLoopTests.LoopType) {
         var loopTypeName = es3fShaderLoopTests.getLoopTypeName(es3fShaderLoopTests.LoopType[loopType]);
         var loopTypeGroup = tcuTestCase.newTest(loopTypeName, 'Loop tests with ' + loopTypeName + ' loop type');
        testGroup.addChild(loopTypeGroup);

        for (var loopCountType in es3fShaderLoopTests.LoopCountType) {
             var loopCountName = es3fShaderLoopTests.getLoopCountTypeName(es3fShaderLoopTests.LoopCountType[loopCountType]);

             var groupName = loopCountName + '_iterations';
             var groupDesc = 'Loop tests with ' + loopCountName + ' loop counter.';
            ...
            for (var precision in gluShaderUtil.precision) {
                 var precisionName = gluShaderUtil.getPrecisionName(gluShaderUtil.precision[precision]);

                for (var dataTypeNdx = 0; dataTypeNdx < s_countDataType.length; dataTypeNdx++) {
                     var loopDataType = s_countDataType[dataTypeNdx];
                     var dataTypeName = gluShaderUtil.getDataTypeName(loopDataType);
            ...
            for (var loopCase in es3fShaderLoopTests.LoopCase) {
                 var loopCaseName = es3fShaderLoopTests.getLoopCaseName(es3fShaderLoopTests.LoopCase[loopCase]);
