...
goog.scope(function() {
    ...
    es3fTransformFeedbackTests.State = {
     ...
     es3fTransformFeedbackTests.MAX_VERIFY_WAIT = 5;

     es3fTransformFeedbackTests.VIEWPORT_WIDTH = 128;
     es3fTransformFeedbackTests.VIEWPORT_HEIGHT = 128;
     es3fTransformFeedbackTests.BUFFER_GUARD_MULTIPLIER = 2;
    ...
    es3fTransformFeedbackTests.isProgramSupported = function(spec, tfMode) {
        ...
        var maxTfSeparateComponents = Number(gl.getParameter(gl.MAX_TRANSFORM_FEEDBACK_SEPARATE_COMPONENTS));
         ...
         var totalVertexAttribs = (
            1  + (spec.isPointSizeUsed() ? 1 : 0)
        );
    ...
    es3fTransformFeedbackTests.genShaderSources = function(spec, pointSizeRequired) {
        ...
        for (var i = 0; i < spec.getVaryings().length; ++i) {
            ...
            for (var vecIter = new gluVarTypeUtil.VectorTypeIterator(type); !vecIter.end(); vecIter.next()) {
                var subType = gluVarTypeUtil.getVarType(type, vecIter.getPath());
                var attribName = es3fTransformFeedbackTests.getAttributeName(name, vecIter.getPath());
    ...
    es3fTransformFeedbackTests.createVertexCaptureProgram = function(spec, bufferMode, primitiveType) {
     ...
     var source = es3fTransformFeedbackTests.genShaderSources(spec, primitiveType === gluDrawUtil.primitiveType.POINTS );
    ...
    es3fTransformFeedbackTests.computeInputLayout = function(attributes, varyings, usePointSize) {
        ...
        for (var i = 0; i < varyings.length; i++) {
            for (var vecIter = new gluVarTypeUtil.VectorTypeIterator(varyings[i].type); !vecIter.end(); vecIter.next()) {
                ...
                inputStride += gluShaderUtil.getDataTypeScalarSize(type.getBasicType()) * 4; 
            }
        }
    ...
    es3fTransformFeedbackTests.computeTransformFeedbackOutputs = function(transformFeedbackOutputs, attributes, varyings, transformFeedbackVaryings, bufferMode) {
        ...
        for (var varNdx = 0; varNdx < transformFeedbackVaryings.length; varNdx++) {
             var name = transformFeedbackVaryings[varNdx];
             var bufNdx = (bufferMode === gl.SEPARATE_ATTRIBS ? varNdx : 0);
             var offset = (bufferMode === gl.SEPARATE_ATTRIBS ? 0 : accumulatedSize);
             var output = new es3fTransformFeedbackTests.Output();
    ...
    es3fTransformFeedbackTests.genAttributeData = function(attrib, buffer, stride, numElements, rnd) {
         ...
         var elementSize = 4; 
         var isFloat = gluShaderUtil.isDataTypeFloatOrVec(attrib.type.getBasicType());
         var isInt = gluShaderUtil.isDataTypeIntOrIVec(attrib.type.getBasicType());
         var isUint = gluShaderUtil.isDataTypeUintOrUVec(attrib.type.getBasicType());

         var precision = attrib.type.getPrecision();

         var numComps = gluShaderUtil.getDataTypeScalarSize(attrib.type.getBasicType());

        for (var elemNdx = 0; elemNdx < numElements; elemNdx++) {
            for (var compNdx = 0; compNdx < numComps; compNdx++) {
                 var offset = attrib.offset + elemNdx * stride + compNdx * elementSize;
                if (isFloat) {
    ...
    es3fTransformFeedbackTests.compareTransformFeedbackOutput = function(primitiveType, output, numInputs, buffers) {
        ...
        for (var attrNdx = 0; attrNdx < output.inputs.length; attrNdx++) {
         ...
         var numComponents = gluShaderUtil.getDataTypeScalarSize(type);

         var precision = attribute.type.getPrecision();

         var scalarType = gluShaderUtil.getDataTypeScalarType(type);
         var numOutputs = es3fTransformFeedbackTests.getTransformFeedbackOutputCount(primitiveType, numInputs);

            for (var outNdx = 0; outNdx < numOutputs; outNdx++) {
             var inNdx = es3fTransformFeedbackTests.getAttributeIndex(primitiveType, numInputs, outNdx);
                ...
                for (var compNdx = 0; compNdx < numComponents; compNdx++) {
                 var isEqual = false;
                    ...
                    if (scalarType === 'float') {
                        ...
                        isEqual = (inBuffer[0] == outBuffer[0]); 
                    }
    ...
    es3fTransformFeedbackTests.TransformFeedbackCase.prototype.init = function() {
        ...
        if (this.m_bufferMode == gl.SEPARATE_ATTRIBS) {
            for (var i = 0; i < this.m_transformFeedbackOutputs.length; ++i) {
                this.m_bufferStrides.push(this.m_transformFeedbackOutputs[i].type.getScalarSize() * 4 );
            }
        } else {
            var totalSize = 0;
            for (var i = 0; i < this.m_transformFeedbackOutputs.length; ++i) {
                totalSize += this.m_transformFeedbackOutputs[i].type.getScalarSize() * 4 ;
            }
    ...
    es3fTransformFeedbackTests.TransformFeedbackCase.prototype.draw = function(calls, seed) {
        ...
        this.m_frameWithTf = new tcuSurface.Surface(this.m_viewportW, this.m_viewportH); 
        this.m_frameWithoutTf = new tcuSurface.Surface(this.m_viewportW, this.m_viewportH); 
        ...
        for (var bufNdx = 0; bufNdx < this.m_outputBuffers.length; ++bufNdx) {
            var buffer = this.m_outputBuffers[bufNdx]; 
            var stride = this.m_bufferStrides[bufNdx]; 
            var target = bufNdx; 
            var size = stride * numOutputs; 
            var guardSize = stride * es3fTransformFeedbackTests.BUFFER_GUARD_MULTIPLIER; 
            var usage = gl.DYNAMIC_READ; 
        ...
        for (var bufferNdx = 0; bufferNdx < this.m_outputBuffers.length; ++bufferNdx) {
            var stride = this.m_bufferStrides[bufferNdx]; 
            var size = stride * numOutputs; 
            var guardSize = stride * es3fTransformFeedbackTests.BUFFER_GUARD_MULTIPLIER; 
            var buffer = new ArrayBuffer(size + guardSize); 
            ...
            for (var i = 0; i < this.m_transformFeedbackOutputs.length; ++i) {
                ...
                for (var callNdx = 0; callNdx < calls.length; ++callNdx) {
                    ...
                    if (call.transformFeedbackEnabled) {
                        var inputPtr = inputData[0] + inputOffset * this.m_inputStride; 
                        var outputPtr = outputOffset * stride; 
    ...
    es3fTransformFeedbackTests.TransformFeedbackCase.prototype.verify = function(calls) {
        ...
        var mustBeReady = this.m_outputBuffers.length > 0; 
        var expectedCount = es3fTransformFeedbackTests.computeTransformFeedbackPrimitiveCount(this.m_primitiveType, calls); 
        var available =  (gl.getQueryParameter(this.m_primitiveQuery, gl.QUERY_RESULT_AVAILABLE));
        var verify_offset = 0;
        var queryOk = true;
        if (!available) {
        ...
        var numPrimitives =  (gl.getQueryParameter(this.m_primitiveQuery, gl.QUERY_RESULT));
    ...
    es3fTransformFeedbackTests.RandomCase.prototype.init = function() {
        ...
        var interpModes = [{name: 'smooth', interp: es3fTransformFeedbackTests.interpolation.SMOOTH}, {name: 'flat', interp: es3fTransformFeedbackTests.interpolation.FLAT}, {name: 'centroid', interp: es3fTransformFeedbackTests.interpolation.CENTROID}
         ...
         var isSeparateMode = (this.m_bufferMode === gl.SEPARATE_ATTRIBS);
         ...
         var captureFullArrayWeight = 0.5;
                               ...
                               var rnd = new deRandom.Random(seed);
         var usePosition = rnd.getFloat() < positionWeight;
         var usePointSize = rnd.getFloat() < pointSizeWeight;
         var numAttribVectorsToUse = rnd.getInt(
            1, maxAttributeVectors - 1 - (usePointSize ? 1 : 0)
        );
        ...
        while (numAttributeVectors < numAttribVectorsToUse) {
            ...
            var maxVecs = isSeparateMode ? Math.min(2 , numAttribVectorsToUse - numAttributeVectors) : numAttribVectorsToUse - numAttributeVectors;
    ...
    es3fTransformFeedbackTests.init = function() {
        ...
        for (var primitiveType = 0; primitiveType < primitiveTypes.length; primitiveType++) {
         ...
         var pointSizeGroup = tcuTestCase.newTest('point_size', 'gl_PointSize capture using transform feedback');
