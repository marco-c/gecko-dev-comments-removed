...
goog.scope(function() {
	...
	es3fShaderPackingFunctionTests.PackSnorm2x16Case.prototype.iterate = function() {
		 var rnd = new deRandom.Random(deString.deStringHash(this.name) ^ 0x776002);
		 var inputs = [];
		 var outputs; 
		 var shaderExecutorOutput;
		 var maxDiff = this.m_precision == gluShaderUtil.precision.PRECISION_HIGHP ? 1 : 
											this.m_precision == gluShaderUtil.precision.PRECISION_MEDIUMP ? 33 : 
											this.m_precision == gluShaderUtil.precision.PRECISION_LOWP ? 129 : 0; 
		 var x;
		 var y;
	    ...
	    outputs = new Uint32Array(shaderExecutorOutput.buffer);
		 ...
		 var numValues = inputs.length;
		 var maxPrints = 10;
		 var numFailed = 0;

		for (var valNdx = 0; valNdx < numValues; valNdx++) {
			 var ref0 = (deMath.clamp(Math.floor(deMath.clamp(inputs[valNdx][0], -1.0, 1.0) * 32767.0), -(1 << 15), (1 << 15) - 1)) & 0xFFFF;
			 var ref1 = (deMath.clamp(Math.floor(deMath.clamp(inputs[valNdx][1], -1.0, 1.0) * 32767.0), -(1 << 15), (1 << 15) - 1)) & 0xFFFF;
			 var ref = (ref1 << 16) | ref0;
			 var res = outputs[valNdx];
			 var res0 = (res & 0xffff);
			 var res1 = deMath.shiftRight(res, 16);
			 var diff0 = Math.abs(ref0 - res0);
			 var diff1 = Math.abs(ref1 - res1);

			if (diff0 > maxDiff || diff1 > maxDiff) {
				if (numFailed < maxPrints) {
					bufferedLogToConsole(
						'ERROR: Mismatch in value ' + valNdx +
						', expected packSnorm2x16(' + inputs[valNdx] + ') = ' + ref + 
						', got ' + res + 
	...
	es3fShaderPackingFunctionTests.UnpackSnorm2x16Case = function(shaderType) {
		 var name = 'unpacksnorm2x16' + es3fShaderPackingFunctionTests.getShaderTypePostfix(shaderType);
	...
	es3fShaderPackingFunctionTests.UnpackSnorm2x16Case.prototype.iterate = function() {
		...
		for (var valNdx = 0; valNdx < inputs.length; valNdx++) {
			 var in0 = Math.floor(inputs[valNdx] & 0xffff);
			
			var view = new DataView(new ArrayBuffer(4));
			view.setUint16(0, in0, true);
			in0 = view.getInt16(0, true);
			 var in1 = Math.floor(deMath.shiftRight(inputs[valNdx], 16));
			
			var view = new DataView(new ArrayBuffer(4));
			view.setUint16(0, in1, true);
			in1 = view.getInt16(0, true);
			 var ref0 = deMath.clamp(in0 / 32767., -1.0, 1.0);
			 var ref1 = deMath.clamp(in1 / 32767., -1.0, 1.0);
			 var res0 = outputs[2 * valNdx];
			 var res1 = outputs[2 * valNdx + 1];

			 var diff0 = es3fShaderPackingFunctionTests.getUlpDiff(ref0, res0);
			 var diff1 = es3fShaderPackingFunctionTests.getUlpDiff(ref1, res1);
	...
	es3fShaderPackingFunctionTests.PackUnorm2x16Case.prototype.iterate = function() {
		 ...
		 var outputs; 
		 var maxDiff = this.m_precision == gluShaderUtil.precision.PRECISION_HIGHP ? 1 : 
											this.m_precision == gluShaderUtil.precision.PRECISION_MEDIUMP ? 65 : 
											this.m_precision == gluShaderUtil.precision.PRECISION_LOWP ? 257 : 0; 
		 var x;
		 var y;
		...
		for (var valNdx = 0; valNdx < inputs.length; valNdx++) {
			 var ref0 = deMath.clamp(Math.floor(deMath.clamp(inputs[valNdx][0], 0.0, 1.0) * 65535.0), 0, (1 << 16) - 1) & 0xFFFF;
			 var ref1 = deMath.clamp(Math.floor(deMath.clamp(inputs[valNdx][1], 0.0, 1.0) * 65535.0), 0, (1 << 16) - 1) & 0xFFFF;
			 var ref = (ref1 << 16) | ref0;
			 var res = outputs[valNdx];
			 var res0 = (res & 0xffff);
			 var res1 = deMath.shiftRight(res, 16);
			...
			if (diff0 > maxDiff || diff1 > maxDiff) {
				if (numFailed < maxPrints)
					bufferedLogToConsole('ERROR: Mismatch in value ' + valNdx +
										 ', expected packUnorm2x16(' + inputs[valNdx] + ') = ' + ref  +
										 ', got ' + res  +
										 '\n  diffs = (' + diff0 + ', ' + diff1 + '), max diff = ' + maxDiff);
				else if (numFailed === maxPrints)
					bufferedLogToConsole('...');
	...
	es3fShaderPackingFunctionTests.UnpackUnorm2x16Case.prototype.iterate = function() {
		...
		for (var valNdx = 0; valNdx < inputs.length; valNdx++) {
			 var in0 = Math.floor(inputs[valNdx] & 0xffff);
			 var in1 = Math.floor(deMath.shiftRight(inputs[valNdx], 16));
			 var ref0 = in0 / 65535.0;
			 var ref1 = in1 / 65535.0;
			 var res0 = outputs[2 * valNdx];
			 var res1 = outputs[2 * valNdx + 1];
	...
	es3fShaderPackingFunctionTests.PackHalf2x16Case.prototype.iterate = function() {
		...
		for (var ndx = 0; ndx < 95; ndx++) {
			for (var c = 0; c < 2; c++) {
				 var s = rnd.getBool() ? 1 : -1;
				 var exp = rnd.getInt(minExp, maxExp);
				 var mantissa = rnd.getInt(0) & ((1 << 23) - 1);

				v[c] = (new tcuFloat.deFloat()).construct(s, exp ? exp : 1 , (1 << 23) | mantissa).getValue();
			}
			inputs.push(v);
		}
		...
		for (var valNdx = 0; valNdx < inputs.length; valNdx++) {
			 var ref0 = tcuFloat.newFloat16(inputs[valNdx][0]).bits();
			 var ref1 = tcuFloat.newFloat16(inputs[valNdx][1]).bits();
			 var ref = (ref1 << 16) | ref0;
			 var res = outputs[valNdx];
			 var res0 = (res & 0xffff);
			 var res1 = deMath.shiftRight(res, 16);
			...
			if (diff0 > maxDiff || diff1 > maxDiff) {
				if (numFailed < maxPrints)
					bufferedLogToConsole('ERROR: Mismatch in value ' + valNdx +
									     ', expected packHalf2x16(' + inputs[valNdx] + ') = ' + ref  +
									     ', got ' + res  +
									     '\n  diffs = (' + diff0 + ', ' + diff1 + '), max diff = ' + maxDiff);
				else if (numFailed == maxPrints)
					bufferedLogToConsole('...');
	...
	es3fShaderPackingFunctionTests.UnpackHalf2x16Case.prototype.iterate = function() {
		...
		for (var ndx = 0; ndx < 96; ndx++) {
			for (var c = 0; c < 2; c++) {
				 var s = rnd.getBool() ? 1 : -1;
				 var exp = rnd.getInt(minExp, maxExp);
				 var mantissa = rnd.getInt(0) & ((1 << mantBits) - 1);
				 var value = tcuFloat.newFloat16(0).construct(s, exp ? exp : 1 , (1 << 10) | mantissa).bits();
		...
		for (var valNdx = 0; valNdx < inputs.length; valNdx++) {
			 var in0 = (inputs[valNdx] & 0xffff);
			 var in1 = deMath.shiftRight(inputs[valNdx], 16);
			 var ref0 = tcuFloat.halfFloatToNumber(in0);
			 var ref1 = tcuFloat.halfFloatToNumber(in1);
			 var res0 = outputs[2 * valNdx];
			 var res1 = outputs[2 * valNdx + 1];
			 var refBits0 = tcuFloat.newFloat32(ref0).bits();
			 var refBits1 = tcuFloat.newFloat32(ref1).bits();
			 var resBits0 = tcuFloat.newFloat32(res0).bits();
			 var resBits1 = tcuFloat.newFloat32(res1).bits();

			 var diff0 = Math.abs(refBits0 - resBits0);
			 var diff1 = Math.abs(refBits1 - resBits1);
			...
			if (diff0 > maxDiff || diff1 > maxDiff) {
				if (numFailed < maxPrints)
					bufferedLogToConsole('ERROR: Mismatch in value ' + valNdx + ',\n' +
									 '  expected unpackHalf2x16(' + inputs[valNdx]  + ') = ' +
									 'vec2(' + ref0 + ' / ' + refBits0  + ', ' + ref1 + ' / ' + refBits1  + ')' +
									 ', got vec2(' + res0 + ' / ' + resBits0  + ', ' + res1 + ' / ' + resBits1  + ')' +
									 '\n  ULP diffs = (' + diff0 + ', ' + diff1 + '), max diff = ' + maxDiff);
				else if (numFailed == maxPrints)
					bufferedLogToConsole('...');
