...
var DE_ASSERT = function(x) {
    ...
    es3fFboMultisampleTests.BasicFboMultisampleCase.prototype.render = function(dst) {
        var ctx = this.getCurrentContext();
         var colorFmt = gluTextureUtil.mapGLInternalFormat(this.m_colorFormat);
         var depthStencilFmt = this.m_depthStencilFormat != gl.NONE ? gluTextureUtil.mapGLInternalFormat(this.m_depthStencilFormat) : new tcuTexture.TextureFormat(null, null);
         ...
         var flatShader = new es3fFboTestUtil.FlatColorShader(es3fFboTestUtil.getFragmentOutputType(colorFmt));
        ...
        for (var ndx = 0; ndx < numQuads; ndx++) {
             ...
             var b = rnd.getFloat();
             var a = rnd.getFloat();
             var x0 = rnd.getFloat(-1.0, 1.0);
             var y0 = rnd.getFloat(-1.0, 1.0);
             var z0 = rnd.getFloat(-1.0, 1.0);
             var x1 = rnd.getFloat(-1.0, 1.0);
             var y1 = rnd.getFloat(-1.0, 1.0);
             var z1 = rnd.getFloat(-1.0, 1.0);
    ...
    es3fFboMultisampleTests.BasicFboMultisampleCase.prototype.colorCompare = function(reference, result) {
         var threshold = tcuRGBA.max(es3fFboTestUtil.getFormatThreshold(this.m_colorFormat), tcuRGBA.newRGBAComponents(12, 12, 12, 12));
    ...
    es3fFboMultisampleTests.BasicFboMultisampleCase.prototype.compare = function(reference, result) {
        if (this.m_depthStencilFormat != gl.NONE)
            return es3fFboTestCase.FboTestCase.prototype.compare(reference, result); 
        else
    ...
    es3fFboMultisampleTests.FboMultisampleTests.prototype.init = function() {
         ...
         var sampleCounts = [2, 4, 8];

        for (var sampleCntNdx in sampleCounts) {
             var samples = sampleCounts[sampleCntNdx];
