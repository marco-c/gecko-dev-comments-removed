...
goog.scope(function() {
    ...
    es3fDrawTests.addTestIterations = function(test, baseSpec, type) {
        var spec =  (deUtil.clone(baseSpec));
    ...
    es3fDrawTests.AttributeGroup.prototype.init = function() {
         ...
         var instanced = this.m_method == glsDrawTests.DrawTestSpec.DrawMethod.DRAWARRAYS_INSTANCED ||
            this.m_method == glsDrawTests.DrawTestSpec.DrawMethod.DRAWELEMENTS_INSTANCED;
         var ranged = this.m_method == glsDrawTests.DrawTestSpec.DrawMethod.DRAWELEMENTS_RANGED;
         var testType = instanced ? es3fDrawTests.TestIterationType.INSTANCE_COUNT :
            (ranged ? es3fDrawTests.TestIterationType.INDEX_RANGE : es3fDrawTests.TestIterationType.DRAW_COUNT);

        
         var test = new glsDrawTests.DrawTest(null, 'single_attribute', 'Single attribute array.');
         var spec = new glsDrawTests.DrawTestSpec();
        ...
        for (var ioNdx = 0; ioNdx < iopairs.length; ++ioNdx) {
             var desc = glsDrawTests.DrawTestSpec.inputTypeToString(iopairs[ioNdx].input) + iopairs[ioNdx].componentCount + ' to ' + glsDrawTests.DrawTestSpec.outputTypeToString(iopairs[ioNdx].output);
    ...
    es3fDrawTests.IndexGroup.prototype.init = function() {
        ...
        es3fDrawTests.genBasicSpec(spec, this.m_method);
         ...
         var bufferGroup = new tcuTestCase.DeqpTest('buffer', 'buffer');
         var unalignedBufferGroup = new tcuTestCase.DeqpTest('unaligned_buffer', 'unaligned buffer');
        ...
        for (var testNdx = 0; testNdx < tests.length; ++testNdx) {
            ...
            var indexTest = tests[testNdx];
             var group = indexTest.aligned ? bufferGroup : unalignedBufferGroup;

             var name = 'index_' + glsDrawTests.DrawTestSpec.indexTypeToString(indexTest.type);
             var desc = 'index ' + glsDrawTests.DrawTestSpec.indexTypeToString(indexTest.type) + ' in ' + glsDrawTests.DrawTestSpec.storageToString(indexTest.storage);
             var test = new glsDrawTests.DrawTest(null, name, desc);
            ...
            for (var iterationNdx = 0; iterationNdx < indexTest.offsets.length && indexTest.offsets[iterationNdx] != -1; ++iterationNdx) {
                 var iterationDesc = 'offset ' + indexTest.offsets[iterationNdx];
    ...
    es3fDrawTests.InstancedGridRenderTest.prototype.iterate = function() {
        ...
        var renderTargetHeight = Math.min(1024, gl.canvas.height);
         ...
         var ctx = new sglrGLContext.GLContext(gl);
         var surface = new tcuSurface.Surface(renderTargetWidth, renderTargetHeight);
         var program = new es3fDrawTests.GridProgram();
    ...
    es3fDrawTests.InstancedGridRenderTest.prototype.renderTo = function(ctx, program, dst) {
        ...
        var yellow = [1, 1, 0, 1];
         ...
         var positionBuf = null;
         var offsetBuf = null;
         var colorBuf = null;
         var indexBuf = null;
         var programID = ctx.createProgram(program);
         var posLocation = ctx.getAttribLocation( (programID), 'a_position');
         var offsetLocation = ctx.getAttribLocation( (programID), 'a_offset');
         var colorLocation = ctx.getAttribLocation( (programID), 'a_color');
    ...
    es3fDrawTests.InstancedGridRenderTest.prototype.verifyImage = function(image) {
        ...
        var colorThreshold = 20;
         ...
         var error = new tcuSurface.Surface(image.getWidth(), image.getHeight());
        var isOk = true;

        for (var y = 1; y < image.getHeight() - 1; y++)
        for (var x = 1; x < image.getWidth() - 1; x++) {
             var pixel = new tcuRGBA.RGBA(image.getPixel(x, y));
            var pixelOk = true;
    ...
    es3fDrawTests.RandomGroup.prototype.init = function() {
         ...
         var attribCounts = [1, 2, 5];
         var attribWeights = [30, 10, 1];
         var primitiveCounts = [2, 6, 64];
         var primitiveCountWeights = [20, 10, 1];
         var indexOffsets = [0, 7, 13];
         var indexOffsetWeights = [20, 20, 1];
         var firsts = [0, 6, 12];
         var firstWeights = [20, 20, 1];
         var instanceCounts = [1, 2, 16, 17];
         var instanceWeights = [20, 10, 5, 1];
         var indexMins = [0, 1, 3, 9];
         var indexMaxs = [5, 9, 129, 257];
         var indexWeights = [50, 50, 50, 50];
         var offsets = [0, 1, 5, 12];
         var offsetWeights = [50, 10, 10, 10];
         var strides = [0, 7, 16, 17];
         var strideWeights = [50, 10, 10, 10];
         var instanceDivisors = [0, 1, 3, 129];
         var instanceDivisorWeights = [70, 30, 10, 10];
         ...
         var primitiveWeights = new es3fDrawTests.UniformWeightArray(primitives.length);
         ...
         var drawMethodWeights = new es3fDrawTests.UniformWeightArray(drawMethods.length);
         ...
         var indexTypeWeights = new es3fDrawTests.UniformWeightArray(indexTypes.length);

         var storages = [
            
            glsDrawTests.DrawTestSpec.Storage.BUFFER
        ];
         var storageWeights = new es3fDrawTests.UniformWeightArray(storages.length);
         ...
         var inputTypeWeights = new es3fDrawTests.UniformWeightArray(inputTypes.length);
         ...
         var outputTypeWeights = new es3fDrawTests.UniformWeightArray(outputTypes.length);
         ...
         var usageWeights = new es3fDrawTests.UniformWeightArray(usages.length);

         var insertedHashes = []; 
         var insertedCount = 0;

        for (var ndx = 0; ndx < numAttempts; ++ndx) {
             var random = new deRandom.Random(0xc551393 + ndx); 

             var attributeCount = random.chooseWeighted(attribCounts, attribWeights);
             var spec = new glsDrawTests.DrawTestSpec();

            
            spec.primitive =  (random.chooseWeighted(primitives, primitiveWeights.weights));
            spec.primitiveCount = random.chooseWeighted(primitiveCounts, primitiveCountWeights);
            spec.drawMethod =  (random.chooseWeighted(drawMethods, drawMethodWeights.weights));
            spec.indexType =  (random.chooseWeighted(indexTypes, indexTypeWeights.weights));
            spec.indexPointerOffset = random.chooseWeighted(indexOffsets, indexOffsetWeights);
            spec.indexStorage =  (random.chooseWeighted(storages, storageWeights.weights));
            ...
            for (var attrNdx = 0; attrNdx < attributeCount;) {
                 var valid;
                 var attribSpec = new glsDrawTests.DrawTestSpec.AttributeSpec();

                attribSpec.inputType =  (random.chooseWeighted(inputTypes, inputTypeWeights.weights));
                attribSpec.outputType =  (random.chooseWeighted(outputTypes, outputTypeWeights.weights));
                attribSpec.storage =  (random.chooseWeighted(storages, storageWeights.weights));
                attribSpec.usage =  (random.chooseWeighted(usages, usageWeights.weights));
