...
var gluTextureUtil = framework.opengl.gluTextureUtil;
     ...
     var gl;
     es3fPrimitiveRestartTests.MAX_RENDER_WIDTH = 256;
     es3fPrimitiveRestartTests.MAX_RENDER_HEIGHT = 256;

     es3fPrimitiveRestartTests.MAX_UNSIGNED_BYTE = 255;
     es3fPrimitiveRestartTests.MAX_UNSIGNED_SHORT = 65535;
     es3fPrimitiveRestartTests.MAX_UNSIGNED_INT = 4294967295;

     es3fPrimitiveRestartTests.RESTART_INDEX_UNSIGNED_BYTE = es3fPrimitiveRestartTests.MAX_UNSIGNED_BYTE;
     es3fPrimitiveRestartTests.RESTART_INDEX_UNSIGNED_SHORT = es3fPrimitiveRestartTests.MAX_UNSIGNED_SHORT;
     es3fPrimitiveRestartTests.RESTART_INDEX_UNSIGNED_INT = es3fPrimitiveRestartTests.MAX_UNSIGNED_INT;
    ...
    es3fPrimitiveRestartTests.PrimitiveRestartCase = function(name, description, primType, indexType, _function, beginWithRestart, endWithRestart, duplicateRestarts) {
        tcuTestCase.DeqpTest.call(this, name, description);
         this.m_primType = primType;
         this.m_indexType = indexType;
         this.m_function = _function;
         this.m_beginWithRestart = beginWithRestart; 
         this.m_endWithRestart = endWithRestart; 
         this.m_duplicateRestarts = duplicateRestarts; 
         this.m_program = null;
    ...
    es3fPrimitiveRestartTests.PrimitiveRestartCase.prototype.draw = function(startNdx, count) {
        ...
        switch (this.m_indexType) {
         ...
         var restartIndex = this.m_indexType == es3fPrimitiveRestartTests.IndexType.INDEX_UNSIGNED_BYTE ? es3fPrimitiveRestartTests.RESTART_INDEX_UNSIGNED_BYTE :
        ...
        if (this.m_function == es3fPrimitiveRestartTests.DrawFunction.FUNCTION_DRAW_ELEMENTS) {
            ...
            DE_ASSERT(this.m_function == es3fPrimitiveRestartTests.DrawFunction.FUNCTION_DRAW_RANGE_ELEMENTS);
             ...
             var numIndices = this.getNumIndices();
    ...
    es3fPrimitiveRestartTests.PrimitiveRestartCase.prototype.renderWithoutRestart = function() {
        ...
        for (var indexArrayNdx = 0; indexArrayNdx <= numIndices; indexArrayNdx++) { 
            if (indexArrayNdx >= numIndices || this.getIndex(indexArrayNdx) == restartIndex) {
                ...
                indexArrayStartNdx = indexArrayNdx + 1; 
            }
        }
    };
    ...
    es3fPrimitiveRestartTests.PrimitiveRestartCase.prototype.init = function() {
        ...
        DE_ASSERT(numIndices > 0);
        DE_ASSERT(this.m_beginWithRestart || this.getIndex(0) != restartIndex); 
        DE_ASSERT(this.m_endWithRestart || this.getIndex(numIndices - 1) != restartIndex); 

        if (!this.m_duplicateRestarts)
            for (var i = 1; i < numIndices; i++)
                DE_ASSERT(this.getIndex(i) != restartIndex || this.getIndex(i - 1) != restartIndex); 

    };

    es3fPrimitiveRestartTests.PrimitiveRestartCase.prototype.iterate = function() {
         var width = Math.min(gl.drawingBufferWidth, es3fPrimitiveRestartTests.MAX_RENDER_WIDTH);
         var height = Math.min(gl.drawingBufferHeight, es3fPrimitiveRestartTests.MAX_RENDER_HEIGHT);
        ...
        gl.readPixels(xOffset, yOffset, refImg.m_width, refImg.m_height, refImgTransferFormat.format, refImgTransferFormat.dataType, referenceImg.m_pixels);
         ...
         var testOk = tcuImageCompare.pixelThresholdCompare('ComparisonResult', 'Image comparison result', referenceImg, resultImg, [0, 0, 0, 0]);
        ...
        for (var isRestartBeginCaseI = 0; isRestartBeginCaseI <= 1; isRestartBeginCaseI++) {
            for (var isRestartEndCaseI = 0; isRestartEndCaseI <= 1; isRestartEndCaseI++) {
                for (var isDuplicateRestartCaseI = 0; isDuplicateRestartCaseI <= 1; isDuplicateRestartCaseI++) {
                     var isRestartBeginCase = isRestartBeginCaseI != 0;
                     var isRestartEndCase = isRestartEndCaseI != 0;
                     var isDuplicateRestartCase = isDuplicateRestartCaseI != 0;

                     var specialCaseGroupName = '';
                    ...
                    if (deString.deIsStringEmpty(specialCaseGroupName))
                     ...
                     var specialCaseGroup = tcuTestCase.newTest(specialCaseGroupName, '');
                    testGroup.addChild(specialCaseGroup);

                    for (var primType in es3fPrimitiveRestartTests.PrimitiveType) {
                         var primTypeName = es3fPrimitiveRestartTests.PrimitiveType[primType] == es3fPrimitiveRestartTests.PrimitiveType.PRIMITIVE_POINTS ? 'points' :
                        ...
                        DE_ASSERT(primTypeName != null);
                         ...
                         var primTypeGroup = tcuTestCase.newTest(primTypeName, '');
                        specialCaseGroup.addChild(primTypeGroup);

                        for (var indexType in es3fPrimitiveRestartTests.IndexType) {
                             var indexTypeName = es3fPrimitiveRestartTests.IndexType[indexType] == es3fPrimitiveRestartTests.IndexType.INDEX_UNSIGNED_BYTE ? 'unsigned_byte' :
                            ...
                            DE_ASSERT(indexTypeName != null);
                             ...
                             var indexTypeGroup = tcuTestCase.newTest(indexTypeName, '');
                            primTypeGroup.addChild(indexTypeGroup);

                            for (var _function in es3fPrimitiveRestartTests.DrawFunction) {
                                 var functionName = es3fPrimitiveRestartTests.DrawFunction[_function] == es3fPrimitiveRestartTests.DrawFunction.FUNCTION_DRAW_ELEMENTS ? 'draw_elements' :
