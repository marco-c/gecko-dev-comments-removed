...
goog.scope(function() {
    ...
    var glsShaderExecUtil = modules.shared.glsShaderExecUtil;
     ...
     es3fShaderCommonFunctionTests.TestClass;
    ...
    es3fShaderCommonFunctionTests.numBitsLostInOp = function(input, output) {
         var inExp = tcuFloat.newFloat32(input).exponent();
         var outExp = tcuFloat.newFloat32(output).exponent();
        return Math.max(0, inExp - outExp); 
    };
    ...
    es3fShaderCommonFunctionTests.getUlpDiff = function(a, b) {
         var aBits = tcuFloat.newFloat32(a).bits();
         var bBits = tcuFloat.newFloat32(b).bits();
    ...
    es3fShaderCommonFunctionTests.CommonFunctionCase = function(name, description, shaderType) {
        tcuTestCase.DeqpTest.call(this, name, description);
         ...
         this.m_spec = new glsShaderExecUtil.ShaderSpec();
    ...
    es3fShaderCommonFunctionTests.VarValue = function(varType, values) {
         var basicType = varType.getBasicType();
         var scalarType = gluShaderUtil.getDataTypeScalarTypeAsDataType(basicType);
         var numComponents = gluShaderUtil.getDataTypeScalarSize(basicType);
         var outputStr = "";
    ...
    es3fShaderCommonFunctionTests.CommonFunctionCase.prototype.iterate = function() {
         var numInputScalars = es3fShaderCommonFunctionTests.computeTotalScalarSize(this.m_spec.inputs);
         var numOutputScalars = es3fShaderCommonFunctionTests.computeTotalScalarSize(this.m_spec.outputs);
         var inputData = [];
         var outputData = [];
         var inputType = this.m_spec.inputs[0].varType.getBasicType();
         var outputType = this.m_spec.outputs[0].varType.getBasicType();
         var inputValues;
         var outputValues;
        ...
        for (var outNdx = 0; outNdx < outputValues.length; outNdx++) {
         ...
         var inScalarSizes = es3fShaderCommonFunctionTests.getScalarSizes(this.m_spec.inputs);
         var outScalarSizes = es3fShaderCommonFunctionTests.getScalarSizes(this.m_spec.outputs);
         var curInputPtr = [];
         var curOutputPtr = [];
         var numFailed = 0;
    ...
    es3fShaderCommonFunctionTests.AbsCase.prototype.getInputValues = function(numValues) {
         ...
         var rnd = new deRandom.Random(deString.deStringHash(this.name) ^ 0x235fac);
    ...
    es3fShaderCommonFunctionTests.AbsCase.prototype.compare = function(inputs, outputs) {
        ...
        if (gluShaderUtil.isDataTypeFloatOrVec(type)) {
             var mantissaBits = es3fShaderCommonFunctionTests.getMinMantissaBits(precision);
             var maxUlpDiff = (1 << (23 - mantissaBits)) - 1;

            for (var compNdx = 0; compNdx < scalarSize; compNdx++) {
                in0 = inputs[0][compNdx];
                out0 = outputs[0][compNdx];
                ref0 = Math.abs(in0);
                 var ulpDiff0 = es3fShaderCommonFunctionTests.getUlpDiff(out0, ref0);

                if (ulpDiff0 > maxUlpDiff) {
                    this.m_failMsg += 'Expected [' + compNdx + '] = ' + ref0  + ' with ULP threshold ' + maxUlpDiff + ', got ULP diff ' + ulpDiff0;
                    return false;
                }
            }
        } else
    ...
    es3fShaderCommonFunctionTests.SignCase.prototype.getInputValues = function(numValues) {
         ...
         var rnd = new deRandom.Random(deString.deStringHash(this.name) ^ 0x324);
    ...
    es3fShaderCommonFunctionTests.SignCase.prototype.compare = function(inputs, outputs) {
        ...
        if (gluShaderUtil.isDataTypeFloatOrVec(type)) {
             ...
             var maxUlpDiff = precision === gluShaderUtil.precision.PRECISION_LOWP ?
    ...
    es3fShaderCommonFunctionTests.roundEven = function(v) {
         var q = deMath.deFloatFrac(v);
         var truncated = Math.trunc(v - q);
    ...
    es3fShaderCommonFunctionTests.RoundEvenCase.prototype.getInputValues = function(numValues) {
         ...
         var rnd = new deRandom.Random(deString.deStringHash(this.name) ^ 0xac23f);
        ...
        if (precision !== gluShaderUtil.precision.PRECISION_LOWP) {
            ...
            for (var ndx = 0; ndx < 20; ndx++) {
                 var v = deMath.clamp(ndx - 10.5, ranges[precision][0], ranges[precision][1]);
                for (var scalarNdx = 0; scalarNdx < scalarSize; scalarNdx++) {
                    values[0].push(v);
                }
                numSpecialCases += 1;
    ...
    es3fShaderCommonFunctionTests.RoundEvenCase.prototype.compare = function(inputs, outputs) {
        ...
        if (precision == gluShaderUtil.precision.PRECISION_HIGHP || precision == gluShaderUtil.precision.PRECISION_MEDIUMP) {
            ...
            for (var compNdx = 0; compNdx < scalarSize; compNdx++) {
             ...
             var mantissaBits = es3fShaderCommonFunctionTests.getMinMantissaBits(precision);
             var maxUlpDiff = es3fShaderCommonFunctionTests.getMaxUlpDiffFromBits(mantissaBits); 
             var eps = es3fShaderCommonFunctionTests.getEpsFromBits(1.0, mantissaBits); 

            for (var compNdx = 0; compNdx < scalarSize; compNdx++) {
                in0 = inputs[0][compNdx];
                out0 = outputs[0][compNdx];
                 var minRes = Math.floor(es3fShaderCommonFunctionTests.roundEven(in0 - eps));
                 var maxRes = Math.floor(es3fShaderCommonFunctionTests.roundEven(in0 + eps));
                 var anyOk = false;
    ...
    es3fShaderCommonFunctionTests.ModfCase.prototype.compare = function(inputs, outputs) {
         var type = this.m_spec.inputs[0].varType.getBasicType();
         var precision = this.m_spec.inputs[0].varType.getPrecision();
         var hasZeroSign = es3fShaderCommonFunctionTests.supportsSignedZero(precision);
         var scalarSize = gluShaderUtil.getDataTypeScalarSize(type);
         var mantissaBits = es3fShaderCommonFunctionTests.getMinMantissaBits(precision);
         var in0;
         var out0;
        ...
        for (var compNdx = 0; compNdx < scalarSize; compNdx++) {
            ...
            out1 = outputs[1][compNdx];
             ...
             var refOut1 = Math.floor(in0);
             var refOut0 = in0 - refOut1;

             var bitsLost = precision != gluShaderUtil.precision.PRECISION_HIGHP ? es3fShaderCommonFunctionTests.numBitsLostInOp(in0, refOut0) : 0;
             var maxUlpDiff = es3fShaderCommonFunctionTests.getMaxUlpDiffFromBits(Math.max(mantissaBits - bitsLost, 0));

             var resSum = out0 + out1;

             var ulpDiff = hasZeroSign ? es3fShaderCommonFunctionTests.getUlpDiff(resSum, in0) : es3fShaderCommonFunctionTests.getUlpDiffIgnoreZeroSign(resSum, in0);
    ...
    es3fShaderCommonFunctionTests.IsnanCase = function(baseType, precision, shaderType) {
        ...
        assertMsgOptions(gluShaderUtil.isDataTypeFloatOrVec(baseType), 'Assert error.', false, true);
         ...
         var vecSize = gluShaderUtil.getDataTypeScalarSize(baseType);
         var boolType = vecSize > 1 ?
    ...
    es3fShaderCommonFunctionTests.IsnanCase.prototype.getInputValues = function(numValues) {
         var rnd = new deRandom.Random(deString.deStringHash(this.name) ^ 0xc2a39f);

         var type = this.m_spec.inputs[0].varType.getBasicType();
         var precision = this.m_spec.inputs[0].varType.getPrecision();
         var scalarSize = gluShaderUtil.getDataTypeScalarSize(type);
         var mantissaBits = es3fShaderCommonFunctionTests.getMinMantissaBits(precision);
         var mantissaMask = (~es3fShaderCommonFunctionTests.getMaxUlpDiffFromBits(mantissaBits)) & ((1 << 23) - 1);
         var values = [];
        values[0] = [];

        for (var valNdx = 0; valNdx < numValues * scalarSize; valNdx++) {
             var isNan = rnd.getFloat() > 0.3;
             var isInf = !isNan && rnd.getFloat() > 0.4;
             var mantissa = !isInf ? ((1 << 22) | (Math.abs(rnd.getInt()) & mantissaMask)) : 0;
             var exp = !isNan && !isInf ? (Math.abs(rnd.getInt()) & 0x7f) : 0xff;
             var sign = Math.abs(rnd.getInt()) & 0x1;
             var value = (sign << 31) | (exp << 23) | mantissa;
    ...
    es3fShaderCommonFunctionTests.IsinfCase.prototype.getInputValues = function(numValues) {
        ...
        for (var valNdx = 0; valNdx < numValues * scalarSize; valNdx++) {
             var isInf = rnd.getFloat() > 0.3;
             var isNan = !isInf && rnd.getFloat() > 0.4;
             var mantissa = !isInf ? ((1 << 22) | (Math.abs(rnd.getInt()) & mantissaMask)) : 0;
             var exp = !isNan && !isInf ? (Math.abs(rnd.getInt()) & 0x7f) : 0xff;
             var sign = Math.abs(rnd.getInt()) & 0x1;
             var value = (sign << 31) | (exp << 23) | mantissa;
    ...
    es3fShaderCommonFunctionTests.FloatBitsToUintIntCase.prototype.getInputValues = function(numValues) {
         ...
         var rnd = new deRandom.Random(deString.deStringHash(this.name) ^ 0x2790a);
    ...
    es3fShaderCommonFunctionTests.BitsToFloatCase = function(baseType, shaderType) {
        es3fShaderCommonFunctionTests.CommonFunctionCase.call(this,
         ...
         var inIsSigned = gluShaderUtil.isDataTypeIntOrIVec(baseType);
         var vecSize = gluShaderUtil.getDataTypeScalarSize(baseType);
         var floatType = vecSize > 1 ? gluShaderUtil.getDataTypeFloatVec(vecSize) : gluShaderUtil.DataType.FLOAT;
    ...
    es3fShaderCommonFunctionTests.BitsToFloatCase.prototype.getInputValues = function(numValues) {
         var rnd = new deRandom.Random(deString.deStringHash(this.name) ^ 0xbbb225);
    ...
    es3fShaderCommonFunctionTests.FloorCase.prototype.compare = function(inputs, outputs) {
        ...
        if (precision === gluShaderUtil.precision.PRECISION_HIGHP || precision === gluShaderUtil.precision.PRECISION_MEDIUMP) {
            ...
            for (var compNdx = 0; compNdx < scalarSize; compNdx++) {
                in0 = inputs[0][compNdx];
                out0 = outputs[0][compNdx];
                 var minRes = Math.floor(in0 - eps);
                 var maxRes = Math.floor(in0 + eps);
                 var anyOk = false;
    ...
    es3fShaderCommonFunctionTests.TruncCase.prototype.getInputValues = function(numValues) {
         ...
         var specialCases = [0.0, -0.0, -0.9, 0.9, 1.0, -1.0];
         var values = [];
        values[0] = [];
    ...
    es3fShaderCommonFunctionTests.TruncCase.prototype.compare = function(inputs, outputs) {
        ...
        if (precision === gluShaderUtil.precision.PRECISION_HIGHP || precision === gluShaderUtil.precision.PRECISION_MEDIUMP) {
            ...
            for (var compNdx = 0; compNdx < scalarSize; compNdx++) {
                in0 = inputs[0][compNdx];
                out0 = outputs[0][compNdx];
                 var isNeg = tcuFloat.newFloat32(in0).sign() < 0;
            ...
            for (var compNdx = 0; compNdx < scalarSize; compNdx++) {
                in0 = inputs[0][compNdx];
                out0 = outputs[0][compNdx];
                 var minRes = Math.trunc(in0 - eps);
                 var maxRes = Math.trunc(in0 + eps);
                 var anyOk = false;
    ...
    es3fShaderCommonFunctionTests.RoundCase.prototype.getInputValues = function(numValues) {
        ...
        if (precision === gluShaderUtil.precision.PRECISION_LOWP) {
            ...
            for (var ndx = 0; ndx < 10; ndx++) {
                 var v = deMath.clamp(ndx - 5.5, ranges[precision][0], ranges[precision][1]);
        ...
        if (precision === gluShaderUtil.precision.PRECISION_HIGHP || precision === gluShaderUtil.precision.PRECISION_MEDIUMP) {
            ...
            for (var compNdx = 0; compNdx < scalarSize; compNdx++) {
                ...
                if ((in0 - Math.floor(in0)) === 0.5) {
                     var ref0 = Math.floor(in0);
                     var ref1 = Math.ceil(in0);
                     var ulpDiff0 = hasZeroSign ? es3fShaderCommonFunctionTests.getUlpDiff(out0, ref0) : es3fShaderCommonFunctionTests.getUlpDiffIgnoreZeroSign(out0, ref0);
                     var ulpDiff1 = hasZeroSign ? es3fShaderCommonFunctionTests.getUlpDiff(out0, ref1) : es3fShaderCommonFunctionTests.getUlpDiffIgnoreZeroSign(out0, ref1);
    ...
    es3fShaderCommonFunctionTests.CeilCase.prototype.compare = function(inputs, outputs) {
        ...
        if (precision === gluShaderUtil.precision.PRECISION_HIGHP || precision === gluShaderUtil.precision.PRECISION_MEDIUMP) {
            ...
            for (var compNdx = 0; compNdx < scalarSize; compNdx++) {
                in0 = inputs[0][compNdx];
                out0 = outputs[0][compNdx];
                 var minRes = Math.ceil(in0 - eps);
                 var maxRes = Math.ceil(in0 + eps);
                 var anyOk = false;
        ...
        if (precision === gluShaderUtil.precision.PRECISION_HIGHP || precision === gluShaderUtil.precision.PRECISION_MEDIUMP) {
            ...
            for (var compNdx = 0; compNdx < scalarSize; compNdx++) {
                ...
                if (Math.floor(in0 - eps) == Math.floor(in0 + eps)) {
                    ref = in0 - Math.floor(in0);
                     var bitsLost = es3fShaderCommonFunctionTests.numBitsLostInOp(in0, ref);
                     var maxUlpDiff = es3fShaderCommonFunctionTests.getMaxUlpDiffFromBits(Math.max(0, mantissaBits - bitsLost)); 
                    ulpDiff = es3fShaderCommonFunctionTests.getUlpDiffIgnoreZeroSign(out0, ref);
                    if (ulpDiff > maxUlpDiff) {
    ...
    es3fShaderCommonFunctionTests.addFunctionCases = function(parent, testClass, functionName, floatTypes, intTypes, uintTypes) {
         var group = tcuTestCase.newTest(functionName, functionName);
        ...
        for (var scalarTypeNdx = 0; scalarTypeNdx < scalarTypes.length; scalarTypeNdx++) {
             var scalarType = scalarTypes[scalarTypeNdx];
    ...
    es3fShaderCommonFunctionTests.ShaderCommonFunctionTests.prototype.init = function() {
        ...
        es3fShaderCommonFunctionTests.addFunctionCases(testGroup, es3fShaderCommonFunctionTests.FloatBitsToUintCase, 'floatbitstouint', true, false, false);
         ...
         var intGroup = tcuTestCase.newTest('intbitstofloat', 'intBitsToFloat() Tests');
         var uintGroup = tcuTestCase.newTest('uintbitstofloat', 'uintBitsToFloat() Tests');
