...
var DE_ASSERT = function(x) {
    ...
    es3fFboTestCase.FboTestCase = function(name, description, useScreenSizedViewport ) {
        tcuTestCase.DeqpTest.call(this, name, description);
         this.m_viewportWidth = useScreenSizedViewport === undefined ? gl.drawingBufferWidth : 128;
         this.m_viewportHeight = useScreenSizedViewport === undefined ? gl.drawingBufferHeight : 128;
         this.m_curCtx = null;
    };
    ...
    es3fFboTestCase.FboTestCase.prototype.checkFormatSupport = function(sizedFormat) {
         var isCoreFormat = es3fFboTestCase.isRequiredFormat(sizedFormat);
         var requiredExts = (!isCoreFormat) ? es3fFboTestCase.getEnablingExtensions(sizedFormat) : [];
    ...
    es3fFboTestCase.FboTestCase.prototype.checkSampleCount = function(sizedFormat, numSamples) {
         var minSampleCount = es3fFboTestCase.getMinimumSampleCount(sizedFormat);

        if (numSamples > minSampleCount) {
            
             var supportedSampleCounts = es3fFboTestCase.querySampleCounts(sizedFormat);
    ...
    es3fFboTestCase.FboTestCase.prototype.checkFramebufferStatus = function(target) {
         var status = this.getCurrentContext().checkFramebufferStatus(target);
    ...
    es3fFboTestCase.FboTestCase.prototype.checkError = function() {
         var err = this.getCurrentContext().getError();
            if (err != gl.NO_ERROR)
    ...
    es3fFboTestCase.FboTestCase.prototype.iterate = function() {
         ...
         var width = Math.min(gl.drawingBufferWidth, this.m_viewportWidth);
         var height = Math.min(gl.drawingBufferHeight, this.m_viewportHeight);
         var x = rnd.getInt(0, gl.drawingBufferWidth - width);
         var y = rnd.getInt(0, gl.drawingBufferHeight - height);

        
         var reference = new tcuSurface.Surface(width, height);
         var result = new tcuSurface.Surface(width, height);
        ...
        try {
             var context = new sglrGLContext.GLContext(
                                                             gl,
                                                             [x, y, width, height]);
        ...
        this.setContext(null);
         ...
         var isOk = this.compare(reference, result);
    ...
    es3fFboTestCase.isAnyExtensionSupported = function(context, requiredExts) {
        for (var iter in requiredExts) {
             var extension = requiredExts[iter];
