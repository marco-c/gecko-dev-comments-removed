...
var tcuImageCompare = framework.common.tcuImageCompare;
     ...
     var isArray = false;
        ...
        for (var outNdx = 0; outNdx < outputs.length; outNdx++) {
            ...
            isArray = output.arrayLength > 0;
             var typeName = gluShaderUtil.getDataTypeName(output.type);
             var precName = gluShaderUtil.getPrecisionName(output.precision);
             var isFloat = gluShaderUtil.isDataTypeFloatOrVec(output.type);
             var interp = isFloat ? 'smooth' : 'flat';
    ...
    es3fFragmentOutputTests.FragmentOutputCase.prototype.init = function() {
        ...
        for (var bufNdx = 0; bufNdx <  this.m_fboSpec.length; bufNdx++) {
            this.m_renderbuffer = gl.createRenderbuffer();
             var bufSpec = this.m_fboSpec[bufNdx];
             var attachment = gl.COLOR_ATTACHMENT0 + bufNdx;
    ...
    es3fFragmentOutputTests.getMinSize = function(fboSpec) {
         var minSize = [0x7fffffff, 0x7fffffff];
    ...
    es3fFragmentOutputTests.renderFloatReference = function(dst, gridWidth, gridHeight, numComponents, vertices) {
         ...
         var isSRGB = dst.getFormat().order == tcuTexture.ChannelOrder.sRGB || dst.getFormat().order == tcuTexture.ChannelOrder.sRGBA;
         var cellW = dst.getWidth() / (gridWidth - 1);
         var cellH = dst.getHeight() / (gridHeight - 1);

        for (var y = 0; y < dst.getHeight(); y++) {
            for (var x = 0; x < dst.getWidth(); x++) {
                 var cellX = deMath.clamp(Math.floor(x / cellW), 0, gridWidth - 2);
                 var cellY = deMath.clamp(Math.floor(y / cellH), 0, gridHeight - 2);
                 var xf = (x - cellX * cellW + 0.5) / cellW;
                 var yf = (y - cellY * cellH + 0.5) / cellH;

                 var v00 = es3fFragmentOutputTests.readVec4(vertices, ((cellY + 0) * gridWidth + cellX + 0) * numComponents, numComponents); 
                 var v01 = es3fFragmentOutputTests.readVec4(vertices, ((cellY + 1) * gridWidth + cellX + 0) * numComponents, numComponents); 
                 var v10 = es3fFragmentOutputTests.readVec4(vertices, ((cellY + 0) * gridWidth + cellX + 1) * numComponents, numComponents); 
                 var v11 = es3fFragmentOutputTests.readVec4(vertices, ((cellY + 1) * gridWidth + cellX + 1) * numComponents, numComponents); 
                 ...
                 var s = tri ? 1.0 - xf : xf;
                 var t = tri ? 1.0 - yf : yf;
                 var color = deMath.add(v0, deMath.add(deMath.multiply((deMath.subtract(v1, v0)), [s, s, s, s]), deMath.multiply((deMath.subtract(v2, v0)), [t, t, t, t]))); 
    ...
    es3fFragmentOutputTests.renderIntReference = function(dst, gridWidth, gridHeight, numComponents, vertices) {
        ...
        for (var y = 0; y < dst.getHeight(); y++) {
            for (var x = 0; x < dst.getWidth(); x++) {
                 var cellX = deMath.clamp(Math.floor(x / cellW), 0, gridWidth - 2);
                 var cellY = deMath.clamp(Math.floor(y / cellH), 0, gridHeight - 2);
                 var c = es3fFragmentOutputTests.readIVec4(vertices, (cellY * gridWidth + cellX + 1) * numComponents, numComponents); 
    ...
    es3fFragmentOutputTests.swizzleVec = function(vec, swzNdx) {
     var swz = es3fFragmentOutputTests.s_swizzles()[swzNdx % es3fFragmentOutputTests.s_swizzles().length];
    ...
    es3fFragmentOutputTests.AttachmentData = function() {
        return {
         ...
         readFormat: null,
         numWrittenChannels: 0,
         outPrecision: gluShaderUtil.precision.PRECISION_LOWP,
         renderedData: null,
         referenceData: null
        };
    };
    ...
    es3fFragmentOutputTests.FragmentOutputCase.prototype.iterate = function() {
         ...
         var minCellSize = 8;
         var minBufSize = es3fFragmentOutputTests.getMinSize(this.m_fboSpec); 
         var gridWidth = deMath.clamp(Math.floor(minBufSize[0] / minCellSize), 1, 255) + 1;
         var gridHeight = deMath.clamp(Math.floor(minBufSize[1] / minCellSize), 1, 255) + 1;
         var numVertices = gridWidth * gridHeight;
         var numQuads = (gridWidth - 1) * (gridHeight - 1);
         var numIndices = numQuads * 6;

         var numInputVecs = es3fFragmentOutputTests.getNumInputVectors(this.m_outputs);
         var inputs = []; 
        ...
        for (var inputNdx = 0; inputNdx < numInputVecs; inputNdx++)
         ...
         var readAlignment = 4;
         var viewportW = minBufSize[0];
         var viewportH = minBufSize[1];
         var numAttachments = this.m_fboSpec.length;
        ...
        for (var ndx = 0; ndx < numAttachments; ndx++) {
             var texFmt = gluTextureUtil.mapGLInternalFormat(this.m_fboSpec[ndx].format);
             var chnClass = tcuTexture.getTextureChannelClass(texFmt.type);
             var isFixedPoint = (chnClass == tcuTexture.TextureChannelClass.SIGNED_FIXED_POINT ||
                                                              chnClass == tcuTexture.TextureChannelClass.UNSIGNED_FIXED_POINT);

            
             var refFmt = isFixedPoint ? new tcuTexture.TextureFormat(texFmt.order, tcuTexture.ChannelType.FLOAT) : texFmt;
             var readFmt = es3fFboTestUtil.getFramebufferReadFormat(texFmt);
        ...
        for (var quadNdx = 0; quadNdx < numQuads; quadNdx++) {
             var quadY = Math.floor(quadNdx / (gridWidth - 1));
             var quadX = quadNdx - quadY * (gridWidth - 1);
        ...
        for (var outputNdx = 0; outputNdx < this.m_outputs.length; outputNdx++) {
            ...
            for (var vecNdx = 0; vecNdx < numVecs; vecNdx++) {
                ...
                attachments[output.location + vecNdx].outPrecision = output.precision;
                 ...
                 var pos = 0;
               if (isFloat) {
                    range = es3fFragmentOutputTests.getFloatRange(output.precision); 
                    minVal = [range[0], range[0], range[0], range[0]]; 
                    maxVal = [range[1], range[1], range[1], range[1]]; 

                    if (deMath.deInBounds32(output.location + vecNdx, 0, attachments.length)) {
                    
                    
                     var fmtInfo = tcuTextureUtil.getTextureFormatInfo(attachments[output.location + vecNdx].format);
                    ...
                    for (var y = 0; y < gridHeight; y++) {
                        for (var x = 0; x < gridWidth; x++) {
                            ...
                            yf = y / (gridHeight - 1);
                             var f0 = (xf + yf) * 0.5;
                             var f1 = 0.5 + (xf - yf) * 0.5;

                             var f = es3fFragmentOutputTests.swizzleVec([f0, f1, 1.0 - f0, 1.0 - f1], curInVec); 
                            c = deMath.add(minVal, deMath.multiply(deMath.subtract(maxVal, minVal), f)); 
                    ...
                    range = es3fFragmentOutputTests.getIntRange(output.precision); 
                    minVal = [range[0], range[0], range[0], range[0]]; 
                    maxVal = [range[1], range[1], range[1], range[1]]; 

                    if (deMath.deInBounds32(output.location + vecNdx, 0, attachments.length)) {
                        
                        fmtBits = tcuTextureUtil.getTextureFormatBitDepth(attachments[output.location + vecNdx].format); 
                         var isZero = deMath.lessThanEqual(fmtBits, [0, 0, 0, 0]); 
                        ...
                        for (var i = 0; i < 4; i++) {
                            ...
                            fmtMinVal[i] = -1 * Math.pow(2, fmtBits[i] - 1); 
                            
                            fmtMaxVal[i] = Math.pow(2, fmtBits[i] - 1) - 1; 
                        }
                    ...
                    rangeDiv = es3fFragmentOutputTests.swizzleVec([gridWidth - 1, gridHeight - 1, gridWidth - 1, gridHeight - 1], curInVec); 
                    for (var i = 0; i < 4; i++) {
                        
                        step[i] = Math.floor((maxVal[i] - minVal[i]) / rangeDiv[i]); 
                    }
                    ...
                    for (var y = 0; y < gridHeight; y++) {
                        for (var x = 0; x < gridWidth; x++) {
                            ix = gridWidth - x - 1;
                            iy = gridHeight - y - 1;
                            c = deMath.add(minVal, deMath.multiply(step, es3fFragmentOutputTests.swizzleVec([x, y, ix, iy], curInVec))); 
                    ...
                    range = es3fFragmentOutputTests.getUintRange(output.precision); 
                    maxVal = [range[1], range[1], range[1], range[1]]; 
                    ...
                    for (var y = 0; y < gridHeight; y++) {
                        for (var x = 0; x < gridWidth; x++) {
                            ix = gridWidth - x - 1;
                            iy = gridHeight - y - 1;
                            c = deMath.multiply(step, es3fFragmentOutputTests.swizzleVec([x, y, ix, iy], curInVec)); 
                            pos = (y * gridWidth + x) * numScalars;
        ...
        for (var outputNdx = 0; outputNdx < this.m_outputs.length; outputNdx++) {
            ...
            isInt = gluShaderUtil.isDataTypeIntOrIVec(output.type);
            isUint = gluShaderUtil.isDataTypeUintOrUVec(output.type);
             var scalarSize = gluShaderUtil.getDataTypeScalarSize(output.type);
             var glScalarType = isFloat ?  gl.FLOAT :
                                                     isInt ?  gl.INT :
            ...
            for (var vecNdx = 0; vecNdx < numVecs; vecNdx++) {
                name = 'in' + outputNdx + (isArray ? '_' + vecNdx : '');
                 var loc = gl.getAttribLocation(this.m_program.getProgram(), name);
                ...
                if (loc >= 0) {
                    ...
                    if (isFloat) {
                        ...
                        gl.vertexAttribPointer(loc, scalarSize, glScalarType, false, 0, 0); 
                    } else {
                        gl.bufferData(gl.ARRAY_BUFFER, new Int32Array(inputs[curInVec]), gl.STATIC_DRAW);
                        
                        
                        gl.vertexAttribIPointer(loc, scalarSize, glScalarType, 0, 0); 
                    }
                } else
        ...
        for (var outputNdx = 0; outputNdx < this.m_outputs.length; outputNdx++) {
            ...
            for (var vecNdx = 0; vecNdx < numVecs; vecNdx++) {
                 var location = output.location + vecNdx;
                 var inputData = inputs[curInNdx];

                DE_ASSERT(deMath.deInBounds32(location, 0, this.m_fboSpec.length));

                 var bufW = this.m_fboSpec[location].width;
                 var bufH = this.m_fboSpec[location].height;
                 var descriptor = {
                 ...
                 var buf = new tcuTexture.PixelBufferAccess(descriptor);
                 var viewportBuf = tcuTextureUtil.getSubregion(buf, 0, 0, 0, viewportW, viewportH, 1);
        ...
        for (var attachNdx = 0; attachNdx < numAttachments; attachNdx++) {
            ...
            attachmentH = this.m_fboSpec[attachNdx].height;
             var numValidChannels = attachments[attachNdx].numWrittenChannels;
             var cmpMask = [numValidChannels >= 1, numValidChannels >= 2, numValidChannels >= 3, numValidChannels >= 4];
             var outPrecision = attachments[attachNdx].outPrecision;
             var format = attachments[attachNdx].format;
            ...
            var renderedDescriptor = {
             ...
             var rendered = new tcuTexture.PixelBufferAccess(renderedDescriptor);
             var transferFmt = gluTextureUtil.getTransferFormat(attachments[attachNdx].readFormat);
            ...
            var referenceDescriptor = {
             ...
             var reference = new tcuTexture.ConstPixelBufferAccess(referenceDescriptor);
             var texClass = tcuTexture.getTextureChannelClass(format.type);
             var isOk = true;
            name = 'Attachment ' + attachNdx;
             var desc = 'Color attachment ' + attachNdx;
             var threshold;
            ...
            switch (texClass) {
                case tcuTexture.TextureChannelClass.FLOATING_POINT: {
                    ...
                    switch (format.type) {
                        ...
                        case tcuTexture.ChannelType.UNSIGNED_INT_11F_11F_10F_REV:
                            formatThreshold = [(1 << 17) + 4, (1 << 17) + 4, (1 << 18) + 4, 4]; 
                            break;
                        default:
                            DE_ASSERT(false);
                            break;
                    ...
                    isOk = tcuImageCompare.floatUlpThresholdCompare(name, desc, reference, rendered, finalThreshold );
                    break;
                }
                ...
                case tcuTexture.TextureChannelClass.UNSIGNED_FIXED_POINT: {
                     ...
                     var bits = deMath.min([8, 8, 8, 8], tcuTextureUtil.getTextureFormatBitDepth(format)); 
                    ...
                    threshold = tcuTextureUtil.select(baseThreshold, [2.0, 2.0, 2.0, 2.0], cmpMask); 

                    isOk = tcuImageCompare.floatThresholdCompare(name, desc, reference, rendered, threshold);
                    break;
                }
                ...
                case tcuTexture.TextureChannelClass.UNSIGNED_INTEGER: {
                    ...
                    isOk = tcuImageCompare.intThresholdCompare(name, desc, reference, rendered, threshold);
                    break;
                }
    ...
    es3fFragmentOutputTests.createRandomCase = function(minRenderTargets, maxRenderTargets, seed, colorBufferFloatSupported) {
        ...
        var uintFormats = [
         ...
         var numTargets = rnd.getInt(minRenderTargets, maxRenderTargets);
         var width = 128; 
         var height = 64;
         var samples = 0;
        ...
        while (curLoc < numTargets) {
             var useArray = rnd.getFloat() < 0.3;
             var maxArrayLen = numTargets - curLoc;
             var arrayLen = useArray ? rnd.getInt(1, maxArrayLen) : 0;
             var basicTypeArray = rnd.choose(outputTypes, undefined, 1);
             var basicType = basicTypeArray[0];
             var precisionArray = rnd.choose(precisions, undefined, 1);
             var precision = precisionArray[0];
             var numLocations = useArray ? arrayLen : 1;
        ...
        while (targets.length < numTargets) {
             var outType = outTypes[targets.length];
             var isFloat = gluShaderUtil.isDataTypeFloatOrVec(outType);
             var isInt = gluShaderUtil.isDataTypeIntOrIVec(outType);
             var isUint = gluShaderUtil.isDataTypeUintOrUVec(outType);
             var formatArray = [];
             var format = 0;
    ...
    es3fFragmentOutputTests.init = function(gl) {
        ...
        var precisions = [
         ...
         var basicGroup = tcuTestCase.newTest('basic', 'Basic fragment output tests');
        ...
        if (gl.getExtension('EXT_color_buffer_float')) {
             var floatGroup = tcuTestCase.newTest('float', 'Floating-point output tests');
         ...
         var fixedGroup = tcuTestCase.newTest('fixed', 'Fixed-point output tests');
        ...
        for (var fmtNdx = 0; fmtNdx < requiredFixedFormats.length; fmtNdx++) {
         ...
         var intGroup = tcuTestCase.newTest('int', 'Integer output tests');
        ...
        for (var fmtNdx = 0; fmtNdx < requiredIntFormats.length; fmtNdx++) {
         ...
         var uintGroup = tcuTestCase.newTest('uint', 'Usigned integer output tests');
        ...
        for (var fmtNdx = 0; fmtNdx < requiredUintFormats.length; fmtNdx++) {
         ...
         var arrayGroup = tcuTestCase.newTest('array', 'Array outputs');
        ...
        if (gl.getExtension('EXT_color_buffer_float')) {
             var arrayFloatGroup = tcuTestCase.newTest('float', 'Floating-point output tests');
         ...
         var arrayFixedGroup = tcuTestCase.newTest('fixed', 'Fixed-point output tests');
        ...
        for (var fmtNdx = 0; fmtNdx < requiredFixedFormats.length; fmtNdx++) {
         ...
         var arrayIntGroup = tcuTestCase.newTest('int', 'Integer output tests');
        ...
        for (var fmtNdx = 0; fmtNdx < requiredIntFormats.length; fmtNdx++) {
         ...
         var arrayUintGroup = tcuTestCase.newTest('uint', 'Usigned integer output tests');
        ...
        for (var fmtNdx = 0; fmtNdx < requiredUintFormats.length; fmtNdx++) {
         ...
         var randomGroup = tcuTestCase.newTest('random', 'Random fragment output cases');
        testGroup.addChild(randomGroup);

         var colorBufferFloatSupported = (gl.getExtension('EXT_color_buffer_float') != null);
