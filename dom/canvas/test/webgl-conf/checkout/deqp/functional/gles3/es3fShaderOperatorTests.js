...
var tcuMatrix = framework.common.tcuMatrix;
 ...
 es3fShaderOperatorTests.MAX_INPUTS = 3;
...
es3fShaderOperatorTests.Precision = function(low, medium, high) {
 ...
 es3fShaderOperatorTests.Precision.Low = new es3fShaderOperatorTests.Precision(true, false, false);
 es3fShaderOperatorTests.Precision.Medium = new es3fShaderOperatorTests.Precision(false, true, false);
 es3fShaderOperatorTests.Precision.High = new es3fShaderOperatorTests.Precision(false, false, true);
 es3fShaderOperatorTests.Precision.LowMedium = new es3fShaderOperatorTests.Precision(true, true, false);
 es3fShaderOperatorTests.Precision.MediumHigh = new es3fShaderOperatorTests.Precision(false, true, true);
 es3fShaderOperatorTests.Precision.All = new es3fShaderOperatorTests.Precision(true, true, true);
 es3fShaderOperatorTests.Precision.None = new es3fShaderOperatorTests.Precision(false, false, false);
...
es3fShaderOperatorTests.getGLSLUintMaxAsFloat = function(shaderType, uintPrecision) {
    ...
    switch (shaderType) {
     ...
     var sPrecision = gl.getShaderPrecisionFormat(shaderTypeGL, intPrecisionGL);
...
es3fShaderOperatorTests.ShaderDataSpec = function() {
     this.resultScale = new es3fShaderOperatorTests.FloatScalar(1);
     this.resultBias = new es3fShaderOperatorTests.FloatScalar(0);
     this.referenceScale = new es3fShaderOperatorTests.FloatScalar(1);
     this.referenceBias = new es3fShaderOperatorTests.FloatScalar(0);
     this.precision = es3fShaderOperatorTests.Precision.None;
     this.output;
     this.numInputs = 0;
     this.inputs = [];
...
es3fShaderOperatorTests.evalSequenceNoSideEffCase2 = function(ctx) {
     var result = es3fShaderOperatorTests.sequenceNoSideEffCase2(
        ctx.in_[0][2] > 0.0,
        ctx.in_[1][0] > 0.0,
...
es3fShaderOperatorTests.evalSequenceSideEffCase2 = function(ctx) {
     var result = es3fShaderOperatorTests.sequenceSideEffCase2(
        ctx.in_[0][2] > 0.0,
        ctx.in_[1][0] > 0.0,
        deMath.swizzle(ctx.in_[2], [2, 1])
    );
...
es3fShaderOperatorTests.ShaderOperatorTests.prototype.init = function() {
    ...
    for (var outerGroupNdx = 0; outerGroupNdx < funcInfoGroups.length; outerGroupNdx++) {
        ...
        for (var funcInfoNdx = 0; funcInfoNdx < outerGroupInfo.funcInfos.length; funcInfoNdx++) {
            ...
            for (var inScalarSize = 1; inScalarSize <= 4; inScalarSize++) {
                var outScalarSize = ((funcInfo.outValue == es3fShaderOperatorTests.ValueType.FLOAT) || (funcInfo.outValue == es3fShaderOperatorTests.ValueType.BOOL)) ? 1 : inScalarSize; 
                ...
                for (var precId = 0; precId < precisions.length; precId++) {
                    ...
                    if ((funcInfo.precision[precision]) ||
                        (funcInfo.precision == es3fShaderOperatorTests.Precision.None && precision === 'medium')) { 
    ...
    for (var typeNdx = 0; typeNdx < s_selectionInfo.length; typeNdx++) {
        ...
        for (var prec in gluShaderUtil.precision) {
            ...
            if (isBoolCase && precision != gluShaderUtil.precision.PRECISION_MEDIUMP) 
                continue;
     ...
     var sequenceGroup = new tcuTestCase.DeqpTest('sequence', 'sequence');
    this.addChild(sequenceGroup);

     var sequenceNoSideEffGroup = new tcuTestCase.DeqpTest('no_side_effects', 'Sequence tests without side-effects');
     var sequenceSideEffGroup = new tcuTestCase.DeqpTest('side_effects', 'Sequence tests with side-effects');
    ...
    sequenceGroup.addChild(sequenceSideEffGroup);
     ...
     var SequenceCase = function(containsSideEffects, caseName, expressionStr, numInputs, inputTypes, resultType, evalFunc) {
          this.containsSideEffects = containsSideEffects;
          this.caseName = caseName;
    ...
    for (var caseNdx = 0; caseNdx < s_sequenceCases.length; caseNdx++) {
        for (var precision in gluShaderUtil.precision) {
            for (var shaderTypeNdx = 0; shaderTypeNdx < s_shaderTypes.length; shaderTypeNdx++) {
                 ...
                 var name = gluShaderUtil.getPrecisionName(gluShaderUtil.precision[precision]) + '_' + s_sequenceCases[caseNdx].caseName + '_' + shaderTypeName;
                ...
                for (var inputNdx = 0; inputNdx < s_sequenceCases[caseNdx].numInputs; inputNdx++) {
                     var type = s_sequenceCases[caseNdx].inputTypes[inputNdx];
                     var rangeMin = gluShaderUtil.isDataTypeFloatOrVec(type) ?
                        f(-0.5) : gluShaderUtil.isDataTypeIntOrIVec(type) ?
                        f(-2.0) : gluShaderUtil.isDataTypeUintOrUVec(type) ?
                        f(0.0) : f(-1.0);

                     var rangeMax = gluShaderUtil.isDataTypeFloatOrVec(type) ?
                        f(0.5) : gluShaderUtil.isDataTypeIntOrIVec(type) ?
                        f(2.0) : gluShaderUtil.isDataTypeUintOrUVec(type) ?
                        f(2.0) : f(1.0);
                 ...
                 var expression = 'res = (' + s_sequenceCases[caseNdx].expressionStr + ');';
