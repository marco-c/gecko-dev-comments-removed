...
var tcuLogImage = framework.common.tcuLogImage;
 ...
 var NUM_CASE_ITERATIONS = 1;
 var FAIL_COLOR_RED = [1, 0, 0.0, 1];
 var PASS_COLOR_BLUE = [0, 0, 0.5, 1];
 var BLACK_COLOR = [0, 0, 0.0, 1];
 var FAIL_DEPTH = 0;
 var FAIL_STENCIL = 1;
 var UNIT_SQUARE = [
     1, 1, 0.05, 1,
...
es3fRasterizerDiscardTests.generateVertices = function(numPrimitives, rnd, drawMode) {
    ...
    for (var i = 0; i < numVertices; i++) {
        dst[i * 4] = rnd.getFloat(-1.0, 1.0); 
        dst[i * 4 + 1] = rnd.getFloat(-1.0, 1.0); 
        dst[i * 4 + 2] = rnd.getFloat(0.1, 0.9); 
        dst[i * 4 + 3] = 1.0; 
    }
