...
goog.scope(function() {
	...
	es3fIndexedStateQueryTests.TransformFeedbackCase.prototype.test = function() {
		...
		gl.linkProgram(shaderProg);
		 ...
		 var transformFeedbackId = gl.createTransformFeedback();
	...
	es3fIndexedStateQueryTests.TransformFeedbackBufferBindingCase.prototype.testTransformFeedback = function() {
		 var feedbackPositionIndex = 0;
		 var feedbackOutputIndex = 1;
		 var feedbackIndex = [feedbackPositionIndex, feedbackOutputIndex];
		...
		for (var ndx = 0; ndx < feedbackBuffers.length; ndx++) {
			var boundBuffer =  (gl.getIndexedParameter(gl.TRANSFORM_FEEDBACK_BUFFER_BINDING, ndx));
	...
	es3fIndexedStateQueryTests.TransformFeedbackBufferBufferCase.prototype.testTransformFeedback = function() {
		 ...
		 var rangeBufferOffset = 4;
		 var rangeBufferSize = 8;
		...
		for (var ndx = 0; ndx < requirements.length; ndx++) {
			var state =  (gl.getIndexedParameter(requirements[ndx].pname, requirements[ndx].index));
	...
	es3fIndexedStateQueryTests.UniformBufferBindingCase.prototype.testUniformBuffers = function() {
		 var uniformNames = ['input1', 'input2'];

		 var uniformIndices = gl.getUniformIndices(this.m_program, uniformNames);
		...
		for (var ndx = 0; ndx < buffers.length; ++ndx) {
		 ...
		 var boundBuffer = [];
		for (var ndx = 0; ndx < buffers.length; ndx++) {
			boundBuffer[ndx] =  (gl.getIndexedParameter(gl.UNIFORM_BUFFER_BINDING, uniformIndices[ndx]));
	...
	es3fIndexedStateQueryTests.UniformBufferBufferCase.prototype.testUniformBuffers = function() {
		...
		bufferedLogToConsole('Alignment is ' + alignment);
		 ...
		 var rangeBufferOffset = alignment;
		 var rangeBufferSize = alignment * 2;
		 var rangeBufferTotalSize = rangeBufferOffset + rangeBufferSize + 8; 
	...
	es3fIndexedStateQueryTests.UniformBufferBufferCase.prototype.getAlignment = function() {
		var state =  (gl.getParameter(gl.UNIFORM_BUFFER_OFFSET_ALIGNMENT));
