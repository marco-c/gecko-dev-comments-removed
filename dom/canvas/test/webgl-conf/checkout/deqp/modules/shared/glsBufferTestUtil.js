...
goog.scope(function() {
    ...
    glsBufferTestUtil.VERIFY_QUAD_SIZE = 8; 
    glsBufferTestUtil.MAX_LINES_PER_INDEX_ARRAY_DRAW = 128; 
    ...
    glsBufferTestUtil.VertexArrayVerifier = function() {
        ...
        this.m_byteVecLoc = 0;
         ...
         var glslVersion = gluShaderUtil.getGLSLVersion(gl);
    ...
    glsBufferTestUtil.computePositions = function(gridSizeX, gridSizeY) {
        ...
        for (var x = 0; x < gridSizeX; x++) {
             var sx0 = (x + 0) / gridSizeX;
             var sy0 = (y + 0) / gridSizeY;
             var sx1 = (x + 1) / gridSizeX;
             var sy1 = (y + 1) / gridSizeY;
             var fx0 = 2.0 * sx0 - 1.0;
             var fy0 = 2.0 * sy0 - 1.0;
             var fx1 = 2.0 * sx1 - 1.0;
             var fy1 = 2.0 * sy1 - 1.0;
             var baseNdx = (y * gridSizeX + x) * 8;
    ...
    glsBufferTestUtil.renderQuadGridReference = function(dst, numQuads, rowLength, inPtr) {
        dst.setSize(rowLength * glsBufferTestUtil.VERIFY_QUAD_SIZE, (Math.floor(numQuads / rowLength) + (numQuads % rowLength != 0 ? 1 : 0)) * glsBufferTestUtil.VERIFY_QUAD_SIZE);
         ...
         var dstAccess = dst.getAccess();
        dstAccess.clear([0, 0, 0, 1.0]);

        for (var quadNdx = 0; quadNdx < numQuads; quadNdx++) {
             var x0 = (quadNdx % rowLength) * glsBufferTestUtil.VERIFY_QUAD_SIZE;
             var y0 = Math.floor(quadNdx / rowLength) * glsBufferTestUtil.VERIFY_QUAD_SIZE;
             var v00 = glsBufferTestUtil.fetchVtxColor(inPtr, quadNdx * 4 + 0);
             var v10 = glsBufferTestUtil.fetchVtxColor(inPtr, quadNdx * 4 + 1);
             var v01 = glsBufferTestUtil.fetchVtxColor(inPtr, quadNdx * 4 + 2);
             var v11 = glsBufferTestUtil.fetchVtxColor(inPtr, quadNdx * 4 + 3);

            for (var y = 0; y < glsBufferTestUtil.VERIFY_QUAD_SIZE; y++)
            for (var x = 0; x < glsBufferTestUtil.VERIFY_QUAD_SIZE; x++) {
                 var fx = (x + 0.5) / glsBufferTestUtil.VERIFY_QUAD_SIZE;
                 var fy = (y + 0.5) / glsBufferTestUtil.VERIFY_QUAD_SIZE;

                 var tri = fx + fy <= 1.0;
                 var tx = tri ? fx : (1.0 - fx);
                 var ty = tri ? fy : (1.0 - fy);
                 var t0 = tri ? v00 : v11;
                 var t1 = tri ? v01 : v10;
                 var t2 = tri ? v10 : v01;
                 var color = deMath.add(
    ...
    glsBufferTestUtil.VertexArrayVerifier.prototype.verifyNoTarget = function(buffer, refPtr, offset, numBytes) {
        ...
        var program = this.m_program.getProgram();
         var threshold =  new tcuRGBA.RGBA([3, 3, 3, 3]);
        var isOk = true;

         var positions = [];
        var indices;

         var rendered = new tcuSurface.Surface();
        ...
        while (numVerified < numBytes) {
            ...
            var isLeftoverBatch = numRemaining < numBytesInQuad;
             var numBytesToVerify = isLeftoverBatch ? numBytesInQuad : Math.min(maxQuadsPerBatch * numBytesInQuad, numRemaining - numRemaining % numBytesInQuad);
             var curOffset = isLeftoverBatch ? (numBytes - numBytesInQuad) : numVerified;
             var numQuads = Math.floor(numBytesToVerify / numBytesInQuad);
             var numCols = Math.min(maxQuadsX, numQuads);
             var numRows = Math.floor(numQuads / maxQuadsX) + (numQuads % maxQuadsX != 0 ? 1 : 0);
             var imageSetDesc = 'Bytes ' + (offset + curOffset) + ' to ' + (offset + curOffset + numBytesToVerify - 1);
    ...
    glsBufferTestUtil.IndexArrayVerifier = function() {
        ...
        this.m_colorLoc = 0;
         ...
         var glslVersion = gluShaderUtil.GLSLVersion.V300_ES;
    ...
    glsBufferTestUtil.IndexArrayVerifier.prototype.verify = function(buffer, refPtr, offset, numBytes) {
        ...
        var minBytesPerBatch = 2;
         var threshold = new tcuRGBA.RGBA([0, 0, 0, 0]);
        ...
        var fetchedColor = [];
         ...
         var indexBufferImg = new tcuSurface.Surface(viewportW, viewportH);
         var referenceImg = new tcuSurface.Surface(viewportW, viewportH);
