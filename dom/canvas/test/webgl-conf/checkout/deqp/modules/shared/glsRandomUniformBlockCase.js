...
goog.scope(function() {
    ...
    glsRandomUniformBlockCase.RandomUniformBlockCase.prototype.generateType = function(rnd, typeDepth, arrayOk) {
         var structWeight = 0.1;
         var arrayWeight = 0.1;
         var flags;

        if (typeDepth < this.m_maxStructDepth && rnd.getFloat() < structWeight) {
             var unusedVtxWeight = 0.15;
             var unusedFragWeight = 0.15;
             var unusedOk = (this.m_features & glsRandomUniformBlockCase.FeatureBits.FEATURE_UNUSED_MEMBERS) != 0;
             var memberTypes = [];
             var numMembers = rnd.getInt(1, this.m_maxStructMembers);

            
            for (var ndx = 0; ndx < numMembers; ndx++)
                memberTypes.push(this.generateType(rnd, typeDepth + 1, true));

             var structType = this.m_interface.allocStruct('s' + this.genName('A'.charCodeAt(0), 'Z'.charCodeAt(0), this.m_structNdx));
            this.m_structNdx += 1;
            ...
            return glsUniformBlockCase.newVarTypeStruct(structType);
             ...
             var arraysOfArraysOk = (this.m_features & glsRandomUniformBlockCase.FeatureBits.FEATURE_ARRAYS_OF_ARRAYS) != 0;
             var arrayLength = rnd.getInt(1, this.m_maxArrayLength);
             var elementType = this.generateType(rnd, typeDepth, arraysOfArraysOk);
            ...
            if (!gluShaderUtil.isDataTypeBoolOrBVec(type)) {
                 ...
                 var precisionCandidates = [glsUniformBlockCase.UniformFlags.PRECISION_LOW, glsUniformBlockCase.UniformFlags.PRECISION_MEDIUM, glsUniformBlockCase.UniformFlags.PRECISION_HIGH];
    ...
    glsRandomUniformBlockCase.RandomUniformBlockCase.prototype.generateUniform = function(rnd, block) {
         var unusedVtxWeight = 0.15;
         var unusedFragWeight = 0.15;
         var unusedOk = (this.m_features & glsRandomUniformBlockCase.FeatureBits.FEATURE_UNUSED_UNIFORMS) != 0;
         var flags = 0;
         var name = this.genName('a'.charCodeAt(0), 'z'.charCodeAt(0), this.m_uniformNdx);
         var type = this.generateType(rnd, 0, true); 
    ...
    glsRandomUniformBlockCase.RandomUniformBlockCase.prototype.generateBlock = function(rnd, layoutFlags) {
        assertMsgOptions(this.m_blockNdx <= 'z'.charCodeAt(0) - 'a'.charCodeAt(0), 'generateBlock', false, true);
         ...
         var instanceArrayWeight = 0.3;
         var block = this.m_interface.allocBlock('Block' + String.fromCharCode('A'.charCodeAt(0) + this.m_blockNdx));
         var numInstances = (this.m_maxInstances > 0 && rnd.getFloat() < instanceArrayWeight) ? rnd.getInt(0, this.m_maxInstances) : 0;
         var numUniforms = rnd.getInt(1, this.m_maxBlockMembers);
        ...
        layoutFlags |= rnd.choose(layoutFlagCandidates)[0]; 

        if (this.m_features & glsRandomUniformBlockCase.FeatureBits.FEATURE_MATRIX_LAYOUT) {
             var matrixCandidates = [0, glsUniformBlockCase.UniformFlags.LAYOUT_ROW_MAJOR, glsUniformBlockCase.UniformFlags.LAYOUT_COLUMN_MAJOR];
    ...
    glsRandomUniformBlockCase.RandomUniformBlockCase.prototype.init = function() {
         ...
         var numShared = this.m_maxSharedBlocks > 0 ? rnd.getInt(1, this.m_maxSharedBlocks) : 0;
         var numVtxBlocks = this.m_maxVertexBlocks - numShared > 0 ? rnd.getInt(1, this.m_maxVertexBlocks - numShared) : 0;
         var numFragBlocks = this.m_maxFragmentBlocks - numShared > 0 ? rnd.getInt(1, this.m_maxFragmentBlocks - numShared) : 0;
