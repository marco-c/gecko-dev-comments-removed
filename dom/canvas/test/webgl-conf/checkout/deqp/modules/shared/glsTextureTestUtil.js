...
glsTextureTestUtil.sampleTextureProjected2D = function(dst, src, sq, tq, params) {
     var lodBias = params.flags.use_bias ? params.bias : 0.0;
     var dstW = dst.getWidth();
     var dstH = dst.getHeight();

     var uq = deMath.scale(sq, src.getWidth());
     var vq = deMath.scale(tq, src.getHeight());

     var triS = [deMath.swizzle(sq, [0, 1, 2]), deMath.swizzle(sq, [3, 2, 1])];
     var triT = [deMath.swizzle(tq, [0, 1, 2]), deMath.swizzle(tq, [3, 2, 1])];
     var triU = [deMath.swizzle(uq, [0, 1, 2]), deMath.swizzle(uq, [3, 2, 1])];
     var triV = [deMath.swizzle(vq, [0, 1, 2]), deMath.swizzle(vq, [3, 2, 1])];
    ...
    for (var py = 0; py < dst.getHeight(); py++) {
        for (var px = 0; px < dst.getWidth(); px++) {
             ...
             var s = glsTextureTestUtil.projectedTriInterpolate(triS[triNdx], triW[triNdx], triNx, triNy);
             var t = glsTextureTestUtil.projectedTriInterpolate(triT[triNdx], triW[triNdx], triNx, triNy);
             var lod = glsTextureTestUtil.computeProjectedTriLod2D(params.lodMode, triU[triNdx], triV[triNdx], triW[triNdx], triWx, triWy, dst.getWidth(), dst.getHeight()) + lodBias;
...
glsTextureTestUtil.computeProjectedTriLod2D = function(mode, u, v, projection, wx, wy, width, height) {
     ...
     var dudx = glsTextureTestUtil.triDerivateX(u, projection, wx, width, wy / height);
     var dvdx = glsTextureTestUtil.triDerivateX(v, projection, wx, width, wy / height);
     var dudy = glsTextureTestUtil.triDerivateY(u, projection, wy, height, wx / width);
     var dvdy = glsTextureTestUtil.triDerivateY(v, projection, wy, height, wx / width);
...
glsTextureTestUtil.sampleTexture2DArray = function(dst, src, texCoord, params) {
    ...
    DE_ASSERT(!(params.flags.projected)); 
...
glsTextureTestUtil.verifyTexture2DResult = function(result, src, texCoord, sampleParams, lookupPrec, lodPrec, pixelFormat) {
    ...
    numFailedPixels = glsTextureTestUtil.computeTextureLookupDiff2D(result, reference.getAccess(), errorMask.getAccess(), src, texCoord, sampleParams, lookupPrec, lodPrec);
        ...
        for (var px = 0; px < result.getWidth(); px++) {
            ...
            if (!deMath.boolAll(deMath.lessThanEqual(
                ...
                lookupPrec.colorThreshold))
                 ...
                 var tri0 = nx + ny - posEps <= 1.0;
                 var tri1 = nx + ny + posEps >= 1.0;
...
glsTextureTestUtil.verifyTexture2DArrayResult = function(result, src, texCoord, sampleParams, lookupPrec, lodPrec, pixelFormat) {
    ...
    numFailedPixels = glsTextureTestUtil.computeTextureLookupDiff2DArray(result, reference.getAccess(), errorMask.getAccess(), src, texCoord, sampleParams, lookupPrec, lodPrec);
...
glsTextureTestUtil.computeTextureLookupDiff2D = function(result, reference, errorMask, baseView, texCoord, sampleParams, lookupPrec, lodPrec, watchDog) {
    ...
    DE_ASSERT(result.getWidth() == errorMask.getWidth() && result.getHeight() == errorMask.getHeight());
     ...
     var lodBias = sampleParams.flags.use_bias ? [sampleParams.bias, sampleParams.bias] : [0.0, 0.0];
    ...
    for (var py = 0; py < result.getHeight(); py++) {
        ...
        for (var px = 0; px < result.getWidth(); px++) {
            ...
            if (!deMath.boolAll(deMath.lessThanEqual(deMath.absDiff(resPix, refPix), lookupPrec.colorThreshold))) {
                ...
                if (!isOk) {
                     var red = tcuRGBA.newRGBAComponents(255, 0, 0, 255);
