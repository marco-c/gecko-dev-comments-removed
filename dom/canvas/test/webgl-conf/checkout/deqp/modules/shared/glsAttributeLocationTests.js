...
goog.scope(function() {
    ...
    glsAttributeLocationTests.createVertexShaderSource = function(attributes, attributeAliasing) {
        ...
        glslVersion = attributeAliasing ? gluShaderUtil.GLSLVersion.V100_ES : glslVersion;
         var usesInOutQualifiers = gluShaderUtil.glslVersionUsesInOutQualifiers(glslVersion);
         var vertexShaderTemplate = glsAttributeLocationTests.generateVertexShaderTemplate(attributes);
    ...
    glsAttributeLocationTests.logProgram = function(program) {
        var programLinkOk =  (gl.getProgramParameter(program, gl.LINK_STATUS));
         var programInfoLog = gl.getProgramInfoLog(program);
         var log = 'Program Link Info: ' + programInfoLog +
    ...
    glsAttributeLocationTests.checkAttribLocationQuery = function(program, attributes, bindings) {
        ...
        for (var attribNdx = 0; attribNdx < attributes.length; attribNdx++) {
             var attrib = attributes[attribNdx];
             var expectedLocation = (attrib.getLayoutLocation() != glsAttributeLocationTests.LocationEnum.UNDEF ? attrib.getLayoutLocation() : glsAttributeLocationTests.getBoundLocation(bindings, attrib.getName()));
            var location =  (gl.getAttribLocation(program, attrib.getName()));
    ...
    glsAttributeLocationTests.checkQuery = function(program, attributes, bindings) {
         var isOk = glsAttributeLocationTests.checkActiveAttribQuery(program, attributes);
    ...
    glsAttributeLocationTests.createAndAttachShaders = function(program, attributes, attributeAliasing) {
         var vertexShaderSource = glsAttributeLocationTests.createVertexShaderSource(attributes, attributeAliasing);
         var fragmentShaderSource = glsAttributeLocationTests.createFragmentShaderSource(attributeAliasing);
        ...
        var vertexShaderCompileOk =  (gl.getShaderParameter(vertexShader, gl.COMPILE_STATUS));
        var fragmentShaderCompileOk =  (gl.getShaderParameter(fragmentShader, gl.COMPILE_STATUS));
    ...
    glsAttributeLocationTests.AttribType = function(name, locationSize, typeEnum) {
         this.m_name = name;
         this.m_locationSize = locationSize;
    ...
    glsAttributeLocationTests.Cond = function(name, negate) {
         this.m_negate = negate === undefined ? false : negate;
         this.m_name = name;
    };
    ...
    glsAttributeLocationTests.Attribute = function(type, name, layoutLocation, cond, arraySize) {
         this.m_type = type;
         this.m_name = name;
         this.m_layoutLocation = layoutLocation === undefined ? glsAttributeLocationTests.LocationEnum.UNDEF : layoutLocation;
         this.m_cond = cond === undefined ?
                                glsAttributeLocationTests.NewCondWithEnum(glsAttributeLocationTests.ConstCond.ALWAYS) : cond;
         this.m_arraySize = arraySize === undefined ? glsAttributeLocationTests.ArrayEnum.NOT : arraySize;
    };
    ...
    glsAttributeLocationTests.runTest = function(attributes, preAttachBind, preLinkBind, postLinkBind, relink, reattach, reattachAttributes) {
        ...
        try {
            ...
            if (!preAttachBind.length == 0)
             ...
             var shaders = glsAttributeLocationTests.createAndAttachShaders(program, attributes, glsAttributeLocationTests.hasAttributeAliasing(attributes, activeBindings));
    ...
    glsAttributeLocationTests.BindMaxAttributesTest.prototype.iterate = function() {
         var noBindings = [];
         var maxAttributes = glsAttributeLocationTests.getMaxAttributeLocations();
         var arrayElementCount = (this.m_arraySize != glsAttributeLocationTests.ArrayEnum.NOT ? this.m_arraySize : 1);
    ...
    glsAttributeLocationTests.BindHoleAttributeTest.prototype.iterate = function() {
         var noBindings = [];
         var maxAttributes = glsAttributeLocationTests.getMaxAttributeLocations();
         var vec4 = new glsAttributeLocationTests.AttribType('vec4', 1, gl.FLOAT_VEC4);
         var arrayElementCount = (this.m_arraySize != glsAttributeLocationTests.ArrayEnum.NOT ? this.m_arraySize : 1);
    ...
    glsAttributeLocationTests.BindReattachAttributeTest.prototype.iterate = function() {
         var noBindings = [];
         var vec4 = new glsAttributeLocationTests.AttribType('vec4', 1, gl.FLOAT_VEC4);
         var vec2 = new glsAttributeLocationTests.AttribType('vec2', 1, gl.FLOAT_VEC2);

         var bindings = [];
         var attributes = [];
         var reattachAttributes = [];
