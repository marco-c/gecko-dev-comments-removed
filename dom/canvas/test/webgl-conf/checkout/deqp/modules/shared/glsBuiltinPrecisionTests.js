...
var setParentClass = function(child, parent) {
    ...
    glsBuiltinPrecisionTests.ScalarTraits.prototype.doMakeIVal = function(value) {
        ...
        return new tcuInterval.Interval( (value));
    };
    ...
    glsBuiltinPrecisionTests.ScalarTraits.prototype.doRound = function(fmt, value) {
        return fmt.roundOut(new tcuInterval.Interval(value), false);
    };
    ...
    glsBuiltinPrecisionTests.ExprBase.prototype.getUsedFuncs = function( dst) {
        this.doGetUsedFuncs(dst);
    };
    ...
    glsBuiltinPrecisionTests.ExprBase.prototype.doGetUsedFuncs = function( dst) {
        throw new Error('Virtual function. Please override.');
    };
    ...
    glsBuiltinPrecisionTests.Func.prototype.doGetParamNames = function() {
         var names = new glsBuiltinPrecisionTests.Tuple4('a', 'b', 'c', 'd');
        return names;
    };
    ...
    glsBuiltinPrecisionTests.FloatFunc3.prototype.applyPoint = function(ctx, x, y, z) {
         var exact = this.applyExact(x, y, z);
         var prec = this.precision(ctx, exact, x, y, z);
    ...
    glsBuiltinPrecisionTests.DefaultSamplingInt.prototype.genRandom = function(fmt, prec, rnd) {
         var exp = rnd.getInt(0, this.getNumBits(prec) - 2);
         var sign = rnd.getBool() ? -1 : 1;
    ...
    glsBuiltinPrecisionTests.DefaultSamplingFloat.prototype.genRandom = function(format, prec, rnd) {
         var minExp = format.getMinExp();
         var maxExp = format.getMaxExp();
         var haveSubnormal = format.hasSubnormal() != tcuFloatFormat.YesNoMaybe.NO;

        
        
         var minRoot = deMath.deCbrt(minExp - 0.5 - (haveSubnormal ? 1.0 : 0.0));
         var maxRoot = deMath.deCbrt(maxExp + 0.5);
         var fractionBits = format.getFractionBits();
         var exp = deMath.rint(Math.pow(rnd.getFloat(minRoot, maxRoot),
                                                                3.0));
         var base = 0.0; 
         var quantum = 0.0; 
         var significand = 0.0; 
        ...
        switch (rnd.getInt(0, 16)) {
            ...
            default: {
                 var intFraction = rnd.getInt() & ((1 << fractionBits) - 1);
    ...
    glsBuiltinPrecisionTests.DefaultSamplingFloat.prototype.genFixeds = function(format, dst) {
         ...
         var maxExp = format.getMaxExp();
         var fractionBits = format.getFractionBits();
         var minQuantum = deMath.deFloatLdExp(1.0, minExp - fractionBits);
         var minNormalized = deMath.deFloatLdExp(1.0, minExp);
         var maxQuantum = deMath.deFloatLdExp(1.0, maxExp - fractionBits);
    ...
    glsBuiltinPrecisionTests.PrecisionCase = function(context, name, extension) {
         this.m_extension = extension === undefined ? '' : extension;
         this.m_ctx = context;
         this.m_status;
         this.m_rnd = new deRandom.Random(1234); 
    ...
    glsBuiltinPrecisionTests.PrecisionCase.prototype.testStatement = function(variables, inputs, stmt) {
         ...
         var inCount = glsBuiltinPrecisionTests.numInputs(this.In);
         var outCount = glsBuiltinPrecisionTests.numOutputs(this.Out);
         var numValues = (inCount > 0) ? inputs.in0.length : 1;
         var highpFmt = this.m_ctx.highpFormat;
        var outputs = [];
         var maxMsgs = 100;
         var numErrors = 0;
         var spec = new glsShaderExecUtil.ShaderSpec();
         var env = new glsBuiltinPrecisionTests.Environment(); 
    ...
    glsBuiltinPrecisionTests.generateInputs = function(In, samplings, floatFormat, intPrecision, numSamples, rnd) {
         var ret = new glsBuiltinPrecisionTests.Inputs(In);
         var fixedInputs = new glsBuiltinPrecisionTests.Inputs(In);
    ...
    glsBuiltinPrecisionTests.InOutFuncCase.prototype.runTest = function(Sig_) {
         var inputs = (glsBuiltinPrecisionTests.generateInputs(
                                                    this.In,
                                                    this.getSamplings(),
    ...
    glsBuiltinPrecisionTests.Context = function(name_, floatFormat_, highpFormat_, precision_, shaderType_, numRandoms_) {
         this.name = name_;
         this.floatFormat = floatFormat_;
         this.highpFormat = highpFormat_;
    ...
    glsBuiltinPrecisionTests.ExpandContext.prototype.addStatement = function( stmt) {
    ...
    glsBuiltinPrecisionTests.createFuncGroup = function(ctx, factory) {
         var group = tcuTestCase.newTest(factory.getName(), factory.getDesc());

        for (var precNdx in gluShaderUtil.precision) {
             var precision = gluShaderUtil.precision[precNdx];
             var precName = gluShaderUtil.getPrecisionName(precision);
             var fmt = ctx.formats[precision];
             var highpFmt = ctx.formats[gluShaderUtil.precision.PRECISION_HIGHP];

            for (var shaderNdx in ctx.shaderTypes) {
                 var shaderType = ctx.shaderTypes[shaderNdx];
                 var shaderName = gluShaderProgram.getShaderTypeName(shaderType);
                 var name = precName + '_' + shaderName;
                 var caseCtx = new glsBuiltinPrecisionTests.Context(name, fmt, highpFmt,
    ...
    glsBuiltinPrecisionTests.addBuiltinPrecisionTests = function(cases, shaderTypes, dstGroup) {
         var highp = new tcuFloatFormat.FloatFormat(-126, 127, 23, true,
                                                 tcuFloatFormat.YesNoMaybe.MAYBE, 
                                                 tcuFloatFormat.YesNoMaybe.YES, 
                                                 tcuFloatFormat.YesNoMaybe.MAYBE); 
        
         var mediump = new tcuFloatFormat.FloatFormat(-13, 13, 9, false);
        
        
         var lowp = new tcuFloatFormat.FloatFormat(0, 0, 7, false, tcuFloatFormat.YesNoMaybe.YES);
         var ctx = new glsBuiltinPrecisionTests.PrecisionTestContext(highp, mediump, lowp,
                                                 shaderTypes, 16384);
    ...
    glsBuiltinPrecisionTests.createES3BuiltinCases = function(caseId) {
         var funcs = new glsBuiltinPrecisionTests.BuiltinFuncs();
