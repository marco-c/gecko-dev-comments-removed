...
var gluDrawUtil = framework.opengl.gluDrawUtil;
     ...
     glsShaderLibraryCase.VIEWPORT_WIDTH = 128;
     glsShaderLibraryCase.VIEWPORT_HEIGHT = 128;
...
glsShaderLibraryCase.caseType = {
    CASETYPE_COMPLETE: 0, 
    CASETYPE_VERTEX_ONLY: 1, 
    CASETYPE_FRAGMENT_ONLY: 2 
};
...
glsShaderLibraryCase.genVertexShader = function(valueBlock) {
     ...
     var usesInout = glsShaderLibraryCase.usesShaderInoutQualifiers(state.currentTest.spec.targetVersion);
    ...
    for (var ndx = 0; ndx < valueBlock.values.length; ndx++) {
        ...
        if (val.storageType === glsShaderLibraryCase.shaderCase.STORAGE_INPUT) {
             var floatType = gluShaderUtil.getDataTypeFloatScalars(val.dataType);
...
glsShaderLibraryCase.genFragmentShader = function(valueBlock) {
     ...
     var prec = glsShaderLibraryCase.supportsFragmentHighp(state.currentTest.spec.targetVersion) ? 'highp' : 'mediump';
    ...
    for (var ndx = 0; ndx < valueBlock.values.length; ndx++) {
     var val = valueBlock.values[ndx];
     var floatType = gluShaderUtil.getDataTypeFloatScalars(val.dataType);
     var refType = gluShaderUtil.getDataTypeName(val.dataType);
...
glsShaderLibraryCase.injectExtensionRequirements = function(baseCode, shaderType, requirements) {
    ...
    if (extensions.length === 0)
     ...
     var firstNonPreprocessorLine = true;
     var resultBuf = '';

    for (var i = 0; i < splitLines.length; i++) {
         var isPreprocessorDirective = (splitLines[i].match(/^\s*#/) !== null);
...
glsShaderLibraryCase.specializeVertexShader = function(src, valueBlock) {
    ...
    for (var ndx = 0; ndx < valueBlock.values.length; ndx++) {
     ...
     var valueName = val.valueName;
     var floatType = gluShaderUtil.getDataTypeFloatScalars(val.dataType);
     var dataTypeName = gluShaderUtil.getDataTypeName(val.dataType);
...
glsShaderLibraryCase.specializeVertexOnly = function(src, valueBlock) {
    ...
    for (var ndx = 0; ndx < valueBlock.values.length; ndx++) {
     var val = valueBlock.values[ndx];
     var valueName = val.valueName;
     var type = gluShaderUtil.getDataTypeName(val.dataType);
     ...
     var refType = gluShaderUtil.getDataTypeName(val.dataType);
        ...
        if (val.storageType === glsShaderLibraryCase.shaderCase.STORAGE_INPUT) {
            if (gluShaderUtil.getDataTypeScalarType(val.dataType) === 'float')
                ...
                var offset = gluShaderUtil.isDataTypeIntOrIVec(val.dataType) ? ' * 1.0025' : ''; 
...
glsShaderLibraryCase.setUniformValue = function(gl, pipelinePrograms, name, val, arrayNdx) {
    ...
    for (var programNdx = 0; programNdx < pipelinePrograms.length; ++programNdx) {
         var loc = gl.getUniformLocation(pipelinePrograms[programNdx], name);
         var scalarSize = gluShaderUtil.getDataTypeScalarSize(val.dataType);
         var elemNdx = (val.arrayLength === 1) ? (0) : (arrayNdx * scalarSize);
...
glsShaderLibraryCase.checkPixels = function(surface, minX, maxX, minY, maxY) {
    ...
    for (var y = minY; y <= maxY; y++) {
        for (var x = minX; x <= maxX; x++) {
             var pixel = surface.getPixelUintRGB8(x, y);
             var isWhite = (pixel == 0xFFFFFF);
             var isBlack = (pixel == 0x000000);
...
glsShaderLibraryCase.init = function() {
 ...
 var test = state.currentTest;
    ...
    if (test.spec.caseType === glsShaderLibraryCase.caseType.CASETYPE_COMPLETE) {
     var vertex = glsShaderLibraryCase.specializeVertexOnly(test.spec.vertexSource, valueBlock);
     var fragment = glsShaderLibraryCase.specializeFragmentOnly(test.spec.fragmentSource, valueBlock);
...
glsShaderLibraryCase.execute = function() {
    ...
    var wtu = WebGLTestUtils;
     ...
     var width = Math.min(canvas.width, glsShaderLibraryCase.VIEWPORT_WIDTH);
     var height = Math.min(canvas.height, glsShaderLibraryCase.VIEWPORT_HEIGHT);
     var viewportX = 0;
     var viewportY = 0;
     var numVerticesPerDraw = 4;
     var tessellationPresent = glsShaderLibraryCase.isTessellationPresent();

     var allCompilesOk = true;
     var allLinksOk = true;
     var failReason = null;

     var vertexProgramID = -1;
    ...
    assertMsgOptions(gl.getError() === gl.NO_ERROR, 'Start testcase: ' + test.fullName(), false, true);
     ...
     var program = new gluShaderProgram.ShaderProgram(gl, test.programs[0].programSources);
    ...
    for (var blockNdx = 0; blockNdx < spec.valueBlockList.length; blockNdx++) {
     var block = spec.valueBlockList[blockNdx];
         ...
         var numRenderPasses = Math.max(block.arrayLength, 1);

        
        for (var arrayNdx = 0; arrayNdx < numRenderPasses; arrayNdx++) {
             var numValues = block.values.length;
             var vertexArrays = [];
            ...
            if (spec.expectResult === glsShaderLibraryCase.expectResult.EXPECT_PASS) {
                 var surface = new gluDrawUtil.Surface();
                 var w = s_positions[3];
                 var minY = Math.ceil(((-quadSize / w) * 0.5 + 0.5) * height + 1.0);
                 var maxY = Math.floor(((+quadSize / w) * 0.5 + 0.5) * height - 0.5);
                 var minX = Math.ceil(((-quadSize / w) * 0.5 + 0.5) * width + 1.0);
                 var maxX = Math.floor(((+quadSize / w) * 0.5 + 0.5) * width - 0.5);
