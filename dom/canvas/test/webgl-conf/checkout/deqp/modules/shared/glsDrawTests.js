...
goog.scope(function() {
    ...
    var rrVertexPacket = framework.referencerenderer.rrVertexPacket;
     ...
     glsDrawTests.MAX_RENDER_TARGET_SIZE = 512;
    ...
    glsDrawTests.getMethodInfo = function(method) {
         var infos = [{
                indexed: false, instanced: false, ranged: false, first: true 
            },{
                indexed: false, instanced: true, ranged: false, first: true 
            },{
                indexed: true, instanced: false, ranged: false, first: false 
            },{
                indexed: true, instanced: false, ranged: true, first: false 
            },{
                indexed: true, instanced: true, ranged: false, first: false 
            }
        ];
    ...
    glsDrawTests.copyGLValueToArray = function(dst, val) {
         var dst8 = new Uint8Array(dst.buffer).subarray(dst.byteOffset, dst.byteOffset + dst.byteLength);
    ...
    glsDrawTests.AttributeArray = function(storage, context) {
         ...
         this.m_bound = false;
         this.m_target = glsDrawTests.DrawTestSpec.Target.ARRAY;
         this.m_inputType = glsDrawTests.DrawTestSpec.InputType.FLOAT;
         this.m_outputType = glsDrawTests.DrawTestSpec.OutputType.VEC4;
         this.m_normalize = false;
         this.m_stride = 0;
         this.m_offset = 0;
         this.m_defaultAttrib;
         this.m_instanceDivisor = 0;
         this.m_isPositionAttr = false;
     ...
     glsDrawTests.AttributeArray.prototype.getComponentCount = function() {return this.m_componentCount;};

     glsDrawTests.AttributeArray.prototype.getTarget = function() {return this.m_target;};

     glsDrawTests.AttributeArray.prototype.getInputType = function() {return this.m_inputType;};

     glsDrawTests.AttributeArray.prototype.getOutputType = function() {return this.m_outputType;};

     glsDrawTests.AttributeArray.prototype.getStorageType = function() {return this.m_storage;};

     glsDrawTests.AttributeArray.prototype.getNormalized = function() {return this.m_normalize;};

     glsDrawTests.AttributeArray.prototype.getStride = function() {return this.m_stride;};

     glsDrawTests.AttributeArray.prototype.isBound = function() {return this.m_bound;};

     glsDrawTests.AttributeArray.prototype.isPositionAttribute = function() {return this.m_isPositionAttr;};
        ...
        for (var packetNdx = 0; packetNdx < numPackets; ++packetNdx) {
            ...
            for (var attribNdx = 0; attribNdx < this.m_attrType.length; attribNdx++) {
                var numComponents = this.m_componentCount[attribNdx];
                 var isCoord = this.m_isCoord[attribNdx];
    ...
    glsDrawTests.RandomArrayGenerator.createArray = function(seed, elementCount, componentCount, offset, stride, type, first, primitive, indices, indexSize) {
        assertMsgOptions(componentCount >= 1 && componentCount <= 4, 'Unacceptable number of components', false, true);
         ...
         var min = glsDrawTests.GLValue.getMinValue(type);
         var max = glsDrawTests.GLValue.getMaxValue(type);
        ...
        var packed = type == glsDrawTests.DrawTestSpec.InputType.INT_2_10_10_10 ||
         ...
         var componentSize = glsDrawTests.DrawTestSpec.inputTypeSize(type);
         var elementSize = componentSize * componentCount;
         var bufferSize = offset + Math.max(elementCount * stride, elementCount * elementSize);
        ...
        for (var vertexNdx = 0; vertexNdx < elementCount; vertexNdx++) {
            ...
            if (packed) {
                var packedValue = deMath.binaryOp(
                        deMath.shiftLeft( (components)[3], 30), deMath.binaryOp(
                            deMath.shiftLeft( (components)[2], 20), deMath.binaryOp(
                                deMath.shiftLeft( (components)[1], 10),  (components)[0], deMath.BinaryOp.OR
                                ), deMath.BinaryOp.OR
                            ), deMath.BinaryOp.OR
                        );
    ...
    glsDrawTests.RandomArrayGenerator.createIndices = function(seed, elementCount, offset, min, max, type) {
         var elementSize = glsDrawTests.DrawTestSpec.indexTypeSize(type);
         var bufferSize = offset + elementCount * elementSize;
    ...
    glsDrawTests.AttributePack = function(drawContext, screenSize, useVao, logEnabled) {
         ...
         this.m_arrays = [];
         this.m_program;
         this.m_screen = new tcuSurface.Surface(screenSize[0], screenSize[1]);
    ...
    glsDrawTests.DrawTestSpec.prototype.getName = function() {
         ...
         var hasFirst = methodInfo.first;
         var instanced = methodInfo.instanced;
         var ranged = methodInfo.ranged;
         var indexed = methodInfo.indexed;

        var name = '';

        for (var ndx = 0; ndx < this.attribs.length; ++ndx) {
             var attrib = this.attribs[ndx];
            ...
            if (attrib.useDefaultAttribute) {
                name += 'non_array_' +
                    glsDrawTests.DrawTestSpec.inputTypeToString( (attrib.inputType)) + '_' +
                ...
                name += glsDrawTests.DrawTestSpec.storageToString(attrib.storage) + '_' +
                    attrib.offset + '_' +
                    attrib.stride + '_' +
                    glsDrawTests.DrawTestSpec.inputTypeToString( (attrib.inputType));
        ...
        for (var ndx = 0; ndx < this.attribs.length; ++ndx) {
            ...
            if (attrib.useDefaultAttribute) {
                desc += 'Attribute ' + ndx + ': default, ' + ((ndx == 0 || attrib.additionalPositionAttribute) ? ('position ,') : ('color ,')) +
                    'input datatype ' + glsDrawTests.DrawTestSpec.inputTypeToString( (attrib.inputType)) + ', ' +
    ...
    glsDrawTests.DrawTestSpec.prototype.hash = function() {
         ...
         var methodInfo = glsDrawTests.getMethodInfo(this.drawMethod);
         var arrayed = methodInfo.first;
         var instanced = methodInfo.instanced;
         var ranged = methodInfo.ranged;
         var indexed = methodInfo.indexed;

         var indexHash = (!indexed) ? (0) : (this.indexType + 10 * this.indexPointerOffset + 100 * this.indexStorage);
         var arrayHash = (!arrayed) ? (0) : (this.first);
         var indexRangeHash = (!ranged) ? (0) : (this.indexMin + 10 * this.indexMax);
         var instanceHash = (!instanced) ? (0) : (this.instanceCount);
         var basicHash = this.primitive + 10 * this.primitiveCount + 100 * this.drawMethod;
    ...
    glsDrawTests.DrawTestSpec.AttributeSpec = function() {
         ...
         this.storage = glsDrawTests.DrawTestSpec.Storage.BUFFER; 
         this.usage = null;
    ...
    glsDrawTests.DrawTestSpec.AttributeSpec.createDefaultAttribute = function(inputType, outputType, componentCount) {
        ...
        spec.inputType = inputType;
        spec.outputType = outputType;
        spec.storage = glsDrawTests.DrawTestSpec.Storage.BUFFER; 
        spec.usage = null;
        spec.componentCount = componentCount;
        spec.offset = 0;
        spec.stride = 0;
    ...
    glsDrawTests.DrawTestSpec.AttributeSpec.prototype.valid = function() {
         var inputTypeFloat = this.inputType == glsDrawTests.DrawTestSpec.InputType.FLOAT || this.inputType == glsDrawTests.DrawTestSpec.InputType.HALF;
         var inputTypeUnsignedInteger = this.inputType == glsDrawTests.DrawTestSpec.InputType.UNSIGNED_BYTE || this.inputType == glsDrawTests.DrawTestSpec.InputType.UNSIGNED_SHORT || this.inputType == glsDrawTests.DrawTestSpec.InputType.UNSIGNED_INT || this.inputType == glsDrawTests.DrawTestSpec.InputType.UNSIGNED_INT_2_10_10_10;
         var inputTypeSignedInteger = this.inputType == glsDrawTests.DrawTestSpec.InputType.BYTE || this.inputType == glsDrawTests.DrawTestSpec.InputType.SHORT || this.inputType == glsDrawTests.DrawTestSpec.InputType.INT || this.inputType == glsDrawTests.DrawTestSpec.InputType.INT_2_10_10_10;
         ...
         var outputTypeFloat = this.outputType == glsDrawTests.DrawTestSpec.OutputType.FLOAT || this.outputType == glsDrawTests.DrawTestSpec.OutputType.VEC2 || this.outputType == glsDrawTests.DrawTestSpec.OutputType.VEC3 || this.outputType == glsDrawTests.DrawTestSpec.OutputType.VEC4;
         var outputTypeSignedInteger = this.outputType == glsDrawTests.DrawTestSpec.OutputType.INT || this.outputType == glsDrawTests.DrawTestSpec.OutputType.IVEC2 || this.outputType == glsDrawTests.DrawTestSpec.OutputType.IVEC3 || this.outputType == glsDrawTests.DrawTestSpec.OutputType.IVEC4;
         var outputTypeUnsignedInteger = this.outputType == glsDrawTests.DrawTestSpec.OutputType.UINT || this.outputType == glsDrawTests.DrawTestSpec.OutputType.UVEC2 || this.outputType == glsDrawTests.DrawTestSpec.OutputType.UVEC3 || this.outputType == glsDrawTests.DrawTestSpec.OutputType.UVEC4;
    ...
    glsDrawTests.DrawTest = function(spec, name, desc) {
        tcuTestCase.DeqpTest.call(this, name, desc, spec);
         ...
         this.m_renderCtx = gl;
         this.m_pixelformat = new tcuPixelFormat.PixelFormat(
    ...
    glsDrawTests.DrawTest.prototype.iterate = function() {
        ...
        var compareStep = (this.m_iteration % 2) == 1;
         var iterateResult = (this.m_iteration + 1 == this.m_specs.length * 2) ? (tcuTestCase.IterateResult.STOP) : (tcuTestCase.IterateResult.CONTINUE);
         var spec = this.m_specs[specNdx];
        var updateProgram = (this.m_iteration == 0) || (drawStep && !glsDrawTests.checkSpecsShaderCompatible(this.m_specs[specNdx], this.m_specs[specNdx - 1])); 
        ...
        if (drawStep) {
             var methodInfo = glsDrawTests.getMethodInfo(spec.drawMethod);
             var indexed = methodInfo.indexed;
             var instanced = methodInfo.instanced;
             var ranged = methodInfo.ranged;
             var hasFirst = methodInfo.first;

             var primitiveElementCount = glsDrawTests.getElementCount(spec.primitive, spec.primitiveCount); 
             var indexMin = (ranged) ? (spec.indexMin) : (0);
             var firstAddition = (hasFirst) ? (spec.first) : (0);
             var elementCount = primitiveElementCount + indexMin + firstAddition; 
             var maxElementIndex = primitiveElementCount + indexMin + firstAddition - 1;
             var indexMax = Math.max(0, (ranged) ? (deMath.clamp(spec.indexMax, 0, maxElementIndex)) : (maxElementIndex));
             var coordScale = this.getCoordScale(spec);
             var colorScale = this.getColorScale(spec);
            ...
            for (var attribNdx = 0; attribNdx < spec.attribs.length; attribNdx++) {
                 var attribSpec = spec.attribs[attribNdx];
                ...
                if (attribSpec.useDefaultAttribute) {
                    seed = 10 * attribSpec.hash() + 100 * spec.hash() + attribNdx;
                     var attribValue = glsDrawTests.RandomArrayGenerator.generateAttributeValue(seed, attribSpec.inputType);
                    ...
                    seed = attribSpec.hash() + 100 * spec.hash() + attribNdx;
                     var elementSize = attribSpec.componentCount * glsDrawTests.DrawTestSpec.inputTypeSize(attribSpec.inputType);
                     var stride = (attribSpec.stride == 0) ? (elementSize) : (attribSpec.stride);
                     var evaluatedElementCount = (instanced && attribSpec.instanceDivisor > 0) ? (spec.instanceCount / attribSpec.instanceDivisor + 1) : (elementCount);
                     var referencedElementCount = (ranged) ? (Math.max(evaluatedElementCount, spec.indexMax + 1)) : (evaluatedElementCount);
                     var bufferSize = attribSpec.offset + stride * (referencedElementCount - 1) + elementSize;
                     var data = glsDrawTests.RandomArrayGenerator.createArray(
                        seed,
    ...
    glsDrawTests.compareUintRGB8 = function(c1, c2, threshold) {
        return (Math.abs(((c1 >> 16) & 0xff) - ((c2 >> 16) & 0xff)) <= threshold[0] &&  
                Math.abs(((c1 >> 8) & 0xff) - ((c2 >> 8) & 0xff)) <= threshold[1] &&    
                Math.abs((c1 & 0xff) - (c2 & 0xff)) <= threshold[2]);                   
    };
    ...
    glsDrawTests.isEdgeTripletComponent = function(c1, c2, c3, renderTargetDifference) {
         var roundingDifference = 2 * renderTargetDifference; 
         var d1 = c2 - c1;
         var d2 = c3 - c2;
         var rampDiff = Math.abs(d2 - d1);
    ...
    glsDrawTests.pixelNeighborhoodContainsCoverage = function(target, x, y, coverage) {
        ...
        for (var dy = -1; dy < 2; dy++)
        for (var dx = -1; dx < 2; dx++) {
            var targetCmpCoverage = target.getPixelUintRGB8(x + dx, y + dy) != 0x000000; 
    ...
    glsDrawTests.edgeRelaxedImageCompare = function(imageSetName, imageSetDesc, reference, result, compareThreshold, renderTargetThreshold, maxAllowedInvalidPixels) {
        ...
        for (var x = 1; x < width - 1; ++x) {
             var refPixel = reference.getPixelUintRGB8(x, y);
             var screenPixel = result.getPixelUintRGB8(x, y);
             var isOkReferencePixel = glsDrawTests.pixelNeighborhoodContainsColor(result, x, y, refPixel, compareThreshold); 
             var isOkScreenPixel = glsDrawTests.pixelNeighborhoodContainsColor(reference, x, y, screenPixel, compareThreshold); 
            ...
            if (isOkScreenPixel && isOkReferencePixel) {
                ...
                ++numFailingPixels;
                 ...
                 var referenceCoverage = refPixel != 0x000000; 
                 var screenCoverage = screenPixel != 0x000000; 
                 var isOkReferenceCoverage = glsDrawTests.pixelNeighborhoodContainsCoverage(result, x, y, referenceCoverage); 
                 var isOkScreenCoverage = glsDrawTests.pixelNeighborhoodContainsCoverage(reference, x, y, screenCoverage); 
    ...
    glsDrawTests.DrawTest.prototype.compare = function(primitiveType) {
        ...
        if ( (gl.getParameter(gl.SAMPLES)) > 1) {
            ...
            return tcuImageCompare.fuzzyCompare('Compare Results', 'Compare Results', ref.getAccess(), screen.getAccess(), 0.3, tcuImageCompare.CompareLogMode.RESULT);
             ...
             var primitiveClass = glsDrawTests.getDrawPrimitiveClass(primitiveType);

            switch (primitiveClass) {
                case glsDrawTests.PrimitiveClass.POINT: {
                    
                     var maxAllowedInvalidPixelsWithPoints = 0;
                ...
                case glsDrawTests.PrimitiveClass.LINE: {
                     ...
                     var maxAllowedInvalidPixelsWithLines = 15; 
                ...
                case glsDrawTests.PrimitiveClass.TRIANGLE: {
                     ...
                     var maxAllowedInvalidPixelsWithTriangles = 10;

                    



                     var renderTargetThreshold = [3, 3, 3, 3];
    ...
    glsDrawTests.DrawTest.prototype.getCoordScale = function(spec) {
        ...
        for (var arrayNdx = 0; arrayNdx < spec.attribs.length; arrayNdx++) {
             var attribSpec = spec.attribs[arrayNdx];
             var isPositionAttr = (arrayNdx == 0) || (attribSpec.additionalPositionAttribute);
             var attrMaxValue = 0;
