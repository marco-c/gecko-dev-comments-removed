...
goog.scope(function() {
    ...
    glsVertexArrayTests.ContextArray = function(storage, context) {
         ...
         this.m_target = glsVertexArrayTests.deArray.Target.ARRAY;
         this.m_inputType = glsVertexArrayTests.deArray.InputType.FLOAT;
         this.m_outputType = glsVertexArrayTests.deArray.OutputType.FLOAT;
    ...
    glsVertexArrayTests.ContextArrayPack = function(drawContext) {
         ...
         this.m_arrays = [];
         this.m_program;
         this.m_screen = new tcuSurface.Surface(
    ...
    glsVertexArrayTests.ContextShaderProgram.prototype.shadeVertices = function(inputs, packets, numPackets) {
         var u_coordScale = this.getUniformByName('u_coordScale').value[0];
         var u_colorScale = this.getUniformByName('u_colorScale').value[0];

        for (var packetNdx = 0; packetNdx < numPackets; ++packetNdx) {
             var varyingLocColor = 0;

             var packet = packets[packetNdx];
    ...
    glsVertexArrayTests.copyGLValueToArray = function(dst, val) {
         var val8 = new Uint8Array(val.m_value.buffer); 
        dst.set(val8);
    };
    ...
    glsVertexArrayTests.copyArray = function(dst, src) {
         var src8 = new Uint8Array(src.buffer).subarray(src.byteOffset, src.byteOffset + src.byteLength); 
        dst.set(src8);
    };
    ...
    glsVertexArrayTests.RandomArrayGenerator.setData = function(data, type, rnd, min, max) {
         ...
         var value = glsVertexArrayTests.GLValue.getRandom(rnd, min, max);
    ...
    glsVertexArrayTests.RandomArrayGenerator.createQuadsPacked = function(seed, count, componentCount, offset, stride, primitive) {
        ...
        switch (primitive) {
         ...
         var _data = new ArrayBuffer(offset + quadStride * (count - 1) + stride * 5 + componentCount * glsVertexArrayTests.deArray.inputTypeSize(glsVertexArrayTests.deArray.InputType.INT_2_10_10_10)); 
         var resultData = new Uint8Array(_data).subarray(offset);
        ...
        switch (primitive) {
            case glsVertexArrayTests.deArray.Primitive.TRIANGLES: {
                for (var quadNdx = 0; quadNdx < count; quadNdx++) {
                     var x1 = min + rnd.getInt() % (max - min);
                     var x2 = min + rnd.getInt() % (max - x1);

                     var y1 = min + rnd.getInt() % (max - min);
                     var y2 = min + rnd.getInt() % (max - y1);

                     var z = min + rnd.getInt() % (max - min);
                     var w = rnd.getInt() % max2;

                     var val1 = (w << 30) | (z << 20) | (y1 << 10) | x1;
                     var val2 = (w << 30) | (z << 20) | (y1 << 10) | x2;
                     var val3 = (w << 30) | (z << 20) | (y2 << 10) | x1;

                     var val4 = (w << 30) | (z << 20) | (y2 << 10) | x1;
                     var val5 = (w << 30) | (z << 20) | (y1 << 10) | x2;
                     var val6 = (w << 30) | (z << 20) | (y2 << 10) | x2;
    ...
    glsVertexArrayTests.RandomArrayGenerator.createQuads = function(seed, count, componentCount, offset, stride, primitive, min, max, scale) {
        var componentStride = min.m_value.byteLength; 
        var quadStride = 0;
        var type = min.getType(); 
        ...
        switch (primitive) {
         ...
         var _data = new ArrayBuffer(offset + quadStride * count);
         var resultData = new Uint8Array(_data).subarray(offset);
    ...
    glsVertexArrayTests.RandomArrayGenerator.createPerQuads = function(seed, count, componentCount, stride, primitive, min, max) {
        ...
        for (var quadNdx = 0; quadNdx < count; quadNdx++) {
            for (var componentNdx = 0; componentNdx < componentCount; componentNdx++) {
                 var val = glsVertexArrayTests.GLValue.getRandom(rnd, min, max);
    ...
    glsVertexArrayTests.VertexArrayTest = function(name, description) {
        ...
        var g =  (gl.getParameter(gl.GREEN_BITS));
        var b =  (gl.getParameter(gl.BLUE_BITS));
        var a =  (gl.getParameter(gl.ALPHA_BITS));
        this.m_pixelformat = new tcuPixelFormat.PixelFormat(r, g, b, a);

         this.m_refBuffers = null;
         this.m_refContext = null;
         this.m_glesContext = null;
         this.m_glArrayPack = null;
         this.m_rrArrayPack = null;
         this.m_isOk = false;
         this.m_maxDiffRed = Math.ceil(256.0 * (2.0 / (1 << this.m_pixelformat.redBits)));
         this.m_maxDiffGreen = Math.ceil(256.0 * (2.0 / (1 << this.m_pixelformat.greenBits)));
         this.m_maxDiffBlue = Math.ceil(256.0 * (2.0 / (1 << this.m_pixelformat.blueBits)));
    };
    ...
    glsVertexArrayTests.VertexArrayTest.prototype.init = function() {
         var renderTargetWidth = Math.min(512, canvas.width);
         var renderTargetHeight = Math.min(512, canvas.height);
    ...
    glsVertexArrayTests.VertexArrayTest.prototype.compare = function() {
         var ref = this.m_rrArrayPack.getSurface();
         var screen = this.m_glArrayPack.getSurface();
        ...
        if ( (this.m_glesContext.getParameter(gl.SAMPLES)) > 1) {
            ...
            this.m_isOk = tcuImageCompare.fuzzyCompare('Compare Results', 'Compare Results', ref.getAccess(), screen.getAccess(), 1.5);
             ...
             var threshold = tcuRGBA.newRGBAComponents(this.m_maxDiffRed, this.m_maxDiffGreen, this.m_maxDiffBlue, 255);
             var error = new tcuSurface.Surface(ref.getWidth(), ref.getHeight());

            this.m_isOk = true;

            for (var y = 1; y < ref.getHeight() - 1; y++) {
                for (var x = 1; x < ref.getWidth() - 1; x++) {
                     var refPixel = tcuRGBA.newRGBAFromArray(ref.getPixel(x, y));
                     var screenPixel = tcuRGBA.newRGBAFromArray(screen.getPixel(x, y));
                     var isOkPixel = false;
                    ...
                    if (refThin && screenThin)
                        ...
                        for (var dy = -1; dy < 2 && !isOkPixel; dy++) {
                            for (var dx = -1; dx < 2 && !isOkPixel; dx++) {
                                 ...
                                 var screenCmpPixel = tcuRGBA.newRGBAFromArray(screen.getPixel(x + dx, y + dy));
                                 var r = Math.abs(refPixel.getRed() - screenCmpPixel.getRed());
                                 var g = Math.abs(refPixel.getGreen() - screenCmpPixel.getGreen());
                                 var b = Math.abs(refPixel.getBlue() - screenCmpPixel.getBlue());

                                if (r <= this.m_maxDiffRed && g <= this.m_maxDiffGreen && b <= this.m_maxDiffBlue)
                                    isOkPixel = true;

                                
                                 var refCmpPixel = tcuRGBA.newRGBAFromArray(ref.getPixel(x + dx, y + dy));
    ...
    glsVertexArrayTests.MultiVertexArrayTest.prototype.iterate = function() {
        if (this.m_iteration == 0) {
            var primitiveSize = (this.m_spec.primitive == glsVertexArrayTests.deArray.Primitive.TRIANGLES) ? (6) : (1); 
            var coordScale = 1.0;
            var colorScale = 1.0;
            var useVao = true; 
            ...
            bufferedLogToConsole(this.m_spec.getDesc());
             ...
             var arraySpec = this.m_spec.arrays[0];
            ...
            for (var arrayNdx = 0; arrayNdx < this.m_spec.arrays.length; arrayNdx++) {
                arraySpec = this.m_spec.arrays[arrayNdx];
                 var seed = arraySpec.inputType + 10 * arraySpec.outputType + 100 * arraySpec.storage + 1000 * this.m_spec.primitive + 10000 * arraySpec.usage + this.m_spec.drawCount + 12 * arraySpec.componentCount + arraySpec.stride + arraySpec.normalize;
                 var data = null;
                 var stride = arraySpec.stride == 0 ? arraySpec.componentCount * glsVertexArrayTests.deArray.inputTypeSize(arraySpec.inputType) : arraySpec.stride;
                 var bufferSize = arraySpec.offset + stride * (this.m_spec.drawCount * primitiveSize - 1) + arraySpec.componentCount * glsVertexArrayTests.deArray.inputTypeSize(arraySpec.inputType);
            ...
            try {
                ...
                if (this.isUnalignedBufferOffsetTest())
                    testFailedOptions('Failed to draw with unaligned buffers', false); 
                else if (this.isUnalignedBufferStrideTest())
                    testFailedOptions('Failed to draw with unaligned stride', false); 
                else
    ...
    glsVertexArrayTests.MultiVertexArrayTest.prototype.isUnalignedBufferOffsetTest = function() {
        ...
        for (var i = 0; i < this.m_spec.arrays.length; ++i) {
            if (this.m_spec.arrays[i].storage == glsVertexArrayTests.deArray.Storage.BUFFER) {
                 var inputTypePacked = this.m_spec.arrays[i].inputType == glsVertexArrayTests.deArray.InputType.UNSIGNED_INT_2_10_10_10 || this.m_spec.arrays[i].inputType == glsVertexArrayTests.deArray.InputType.INT_2_10_10_10;

                 var dataTypeSize = glsVertexArrayTests.deArray.inputTypeSize(this.m_spec.arrays[i].inputType);
                if (inputTypePacked)
                    dataTypeSize = 4;
