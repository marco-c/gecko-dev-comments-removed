...
goog.scope(function() {
    ...
    var gluShaderProgram = framework.opengl.gluShaderProgram;
     ...
     glsShaderRenderCase.ShaderEvalFunc;

     glsShaderRenderCase.GRID_SIZE = 64;
     glsShaderRenderCase.MAX_RENDER_WIDTH = 128;
     glsShaderRenderCase.MAX_RENDER_HEIGHT = 112;
     glsShaderRenderCase.DEFAULT_CLEAR_COLOR = [0.125, 0.25, 0.5, 1.0];
     glsShaderRenderCase.MAX_USER_ATTRIBS = 4;
     glsShaderRenderCase.MAX_TEXTURES = 4;
    ...
    glsShaderRenderCase.QuadGrid = function(gridSize, width, height, constCoords, userAttribTransforms, textures) {
        ...
        for (var attribNdx = 0; attribNdx < this.getNumUserAttribs(); attribNdx++)
         ...
         var viewportScale = [width, height, 0, 0];
    ...
    glsShaderRenderCase.ShaderEvalContext = function(quadGrid_) {
         ...
         var bindings = this.quadGrid.getTextures();
    ...
    glsShaderRenderCase.ShaderEvalContext.prototype.reset = function(sx, sy) {
        ...
        this.unitCoords = this.quadGrid.getUnitCoords(sx, sy);
         ...
         var numAttribs = this.quadGrid.getNumUserAttribs();
    ...
    glsShaderRenderCase.ShaderRenderCase = function(name, description, isVertexCase, evalFunc) {
        tcuTestCase.DeqpTest.call(this, name, description);
         ...
         this.m_defaultEvaluator = evalFunc || null;
         this.m_evaluator = new glsShaderRenderCase.ShaderEvaluator(this.m_defaultEvaluator);
         this.m_vertShaderSource = '';
         this.m_fragShaderSource = '';
         this.m_clearColor = glsShaderRenderCase.DEFAULT_CLEAR_COLOR;
    ...
    glsShaderRenderCase.ShaderRenderCase.prototype.postinit = function() {
        ...
        try {
            bufferedLogToConsole(this.m_program.program.info.infoLog); 
    ...
    glsShaderRenderCase.ShaderRenderCase.prototype.postiterate = function() {
        assertMsgOptions(this.m_program !== null, 'Program not specified.', false, true);
         var programID = this.m_program.getProgram();
        ...
        var quadGrid = new glsShaderRenderCase.QuadGrid(
         ...
         var resImage = new tcuSurface.Surface(width, height);
        this.render(resImage, programID, quadGrid);

        
         var refImage = new tcuSurface.Surface(width, height);
        if (this.m_isVertexCase)
         ...
         var testOk = this.compareImages(resImage, refImage, 0.05);
    ...
    glsShaderRenderCase.ShaderRenderCase.prototype.setupDefaultInputs = function(programId) {
        ...
        for (var ndx = 0; ndx < this.m_textures.length; ndx++) {
             var tex = this.m_textures[ndx];
             var sampler = tex.getSampler();
             var texTarget = gl.NONE;
             var texObj = 0;
    ...
    glsShaderRenderCase.ShaderRenderCase.prototype.render = function(result, programId, quadGrid) {
         ...
         var hash = deString.deStringHash(this.m_vertShaderSource) + deString.deStringHash(this.m_fragShaderSource);
         var rnd = new deRandom.Random(hash);
        ...
        gl.clear(gl.COLOR_BUFFER_BIT);
         ...
         var vertexArrays = [];
         var numElements = quadGrid.getNumTriangles()*3;
    ...
    glsShaderRenderCase.ShaderRenderCase.prototype.computeVertexReference = function(result, quadGrid) {
         ...
         var width = result.getWidth();
         var height = result.getHeight();
         var gridSize = quadGrid.getGridSize();
         var stride = gridSize + 1;
         var hasAlpha = gl.getContextAttributes().alpha;
        ...
        for (var x = 0; x < gridSize; x++) {
             ...
             var sy0 = y0 * height;
             var sy1 = y1 * height;
             var oosx = 1.0 / (sx1 - sx0);
             var oosy = 1.0 / (sy1 - sy0);

             var ix0 = Math.ceil(sx0 - 0.5);
             var ix1 = Math.ceil(sx1 - 0.5);
             var iy0 = Math.ceil(sy0 - 0.5);
             var iy1 = Math.ceil(sy1 - 0.5);
            ...
            for (var ix = ix0; ix < ix1; ix++) {
                ...
                assertMsgOptions(deMath.deInBounds32(iy, 0, height), 'Out of bounds.', false, true);
                 ...
                 var sfx = ix + 0.5;
                 var sfy = iy + 0.5;
                 var fx1 = deMath.clamp((sfx - sx0) * oosx, 0.0, 1.0);
                 var fy1 = deMath.clamp((sfy - sy0) * oosy, 0.0, 1.0);

                
                 var tri = fx1 + fy1 <= 1.0;
                 var tx = tri ? fx1 : (1.0 - fx1);
                 var ty = tri ? fy1 : (1.0 - fy1);
                 var t0 = tri ? c00 : c11;
                 var t1 = tri ? c01 : c10;
                 var t2 = tri ? c10 : c01;
    ...
    glsShaderRenderCase.ShaderRenderCase.prototype.computeFragmentReference = function(result, quadGrid) {
        ...
        for (var x = 0; x < width; x++) {
            ...
            this.m_evaluator.evaluate(evalCtx);
             ...
             var color = evalCtx.isDiscarded ? this.m_clearColor : evalCtx.color;
    ...
    glsShaderRenderCase.getDefaultVertexArrays = function(quadGrid, program, vertexArrays) {
         var numElements = quadGrid.getNumVertices();
        ...
        for (var matNdx = 0; matNdx < matrices.length; matNdx++) {
             var loc = gl.getAttribLocation(program, matrices[matNdx].name);

            if (loc < 0)
                continue; 

             var numRows = matrices[matNdx].numRows;
             var numCols = matrices[matNdx].numCols;
