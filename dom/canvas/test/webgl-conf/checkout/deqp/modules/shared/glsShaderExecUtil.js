...
goog.scope(function() {
    ...
    glsShaderExecUtil.ShaderSpec = function() {
         this.version = gluShaderUtil.GLSLVersion.V300_ES; 
         this.inputs = [];
         this.outputs = [];
    ...
    glsShaderExecUtil.ShaderExecutor = function(shaderSpec) {
         this.m_inputs = shaderSpec.inputs;
         this.m_outputs = shaderSpec.outputs;
    };
    ...
    glsShaderExecUtil.generateVertexShader = function(shaderSpec) {
         var usesInout = true;
         var in_ = usesInout ? 'in' : 'attribute';
         var out = usesInout ? 'out' : 'varying';
         var src = '';
         var vecSize;
         var intBaseType;
        ...
        for (var i = 0; i < shaderSpec.outputs.length; i++) {
            ...
            if (gluShaderUtil.isDataTypeBoolOrBVec(output.varType.getBasicType())) {
                ...
                intBaseType = vecSize > 1 ? gluShaderUtil.getDataTypeVector(gluShaderUtil.DataType.INT, vecSize) : gluShaderUtil.DataType.INT;
                 var intType = new gluVarType.VarType().VarTypeBasic(intBaseType, gluShaderUtil.precision.PRECISION_HIGHP);
    ...
    glsShaderExecUtil.generateFragmentShader = function(shaderSpec, useIntOutputs, outLocationMap) {
        ...
        for (var outNdx = 0; outNdx < shaderSpec.outputs.length; ++outNdx) {
             var output = shaderSpec.outputs[outNdx];
             var location = outLocationMap[output.name];
             var outVarName = 'o_' + output.name;
             var decl = new gluVarType.VariableDeclaration(output.varType, outVarName, gluVarType.Storage.STORAGE_OUT, undefined, new gluVarType.Layout(location));
    ...
    glsShaderExecUtil.VertexProcessorExecutor.prototype.execute = function(numValues, inputs) {
        ...
        var outputBuffer = gl.createBuffer();
         ...
         var outputBufferStride = glsShaderExecUtil.computeTotalScalarSize(this.m_outputs) * 4;
        ...
        for (var inputNdx = 0; inputNdx < this.m_inputs.length; inputNdx++) {
            ...
            var ptr = inputs[inputNdx];
             ...
             var vecSize = gluShaderUtil.getDataTypeScalarSize(basicType);
        ...
        for (var outputNdx = 0; outputNdx < this.m_outputs.length; outputNdx++) {
            symbol = this.m_outputs[outputNdx];
             var scalarSize = symbol.varType.getScalarSize();
    ...
    glsShaderExecUtil.FragmentShaderExecutor.prototype.execute = function(numValues, inputs) {
         var useIntOutputs = true;
         var symbol;
        var outputs = [];
        var maxRenderbufferSize =  (gl.getParameter(gl.MAX_RENDERBUFFER_SIZE));
         var framebufferW = Math.min(maxRenderbufferSize, numValues);
         var framebufferH = Math.ceil(numValues / framebufferW);
        ...
        for (var valNdx = 0; valNdx < numValues; valNdx++) {
             var ix = valNdx % framebufferW;
             var iy = Math.floor(valNdx / framebufferW);
        ...
        for (var outNdx = 0; outNdx < this.m_outputs.length; ++outNdx) {
            symbol = this.m_outputs[outNdx];
             var outSize = symbol.varType.getScalarSize();
             var outVecSize = gluShaderUtil.getDataTypeNumComponents(symbol.varType.getBasicType());
             var outNumLocs = gluShaderUtil.getDataTypeNumLocations(symbol.varType.getBasicType());
            ...
            var transferFormat = gluTextureUtil.getTransferFormat(readFormat);
             var outLocation = this.m_outLocationMap[symbol.name];
