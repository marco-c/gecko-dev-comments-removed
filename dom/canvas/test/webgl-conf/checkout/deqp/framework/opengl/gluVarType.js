...
goog.scope(function() {
    ...
    gluVarType.TypeArray = function(elementType, arraySize) {
        this.elementType = gluVarType.newClone(elementType);
        this.size = arraySize;
    };
    ...
    gluVarType.VarType.prototype.getScalarSize = function() {
        switch (this.m_type) {
            case gluVarType.Type.TYPE_BASIC: {
                return gluShaderUtil.getDataTypeScalarSize((this.getBasicType()));
            }
    ...
    gluVarType.declareVariable = function(varType, name, level) {
        ...
        if (curType.isBasicType()) {
            ...
            str += gluShaderUtil.getDataTypeName((curType.getBasicType()));
        } else if (curType.isStructType()) {
             var structPtr = curType.getStruct();

            if (structPtr.hasTypeName())
                str += structPtr.getTypeName();
            else
                str += gluVarType.declareStructType(structPtr, level); 
        } else
        ...
        for (var size = 0; size < arraySizes.length; size++) { 
    ...
    gluVarType.declareStructType = function(structType, level) {
        ...
        for (var memberNdx = 0; memberNdx < structType.getSize(); memberNdx++) { 
             var memberIter = structType.getMember(memberNdx);
