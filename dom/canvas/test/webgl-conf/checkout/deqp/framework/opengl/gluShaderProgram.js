...
gluShaderProgram.genVertexSource = function(source) {
 var shader = new gluShaderProgram.ShaderInfo(gluShaderProgram.shaderType.VERTEX, source);
    return shader;
};
...
gluShaderProgram.genFragmentSource = function(source) {
 var shader = new gluShaderProgram.ShaderInfo(gluShaderProgram.shaderType.FRAGMENT, source);
    return shader;
};
...
gluShaderProgram.Shader = function(gl, type) {
    this.gl = gl;
    this.info = new gluShaderProgram.ShaderInfo(type); 
...
gluShaderProgram.ShaderProgram = function(gl, programSources) {
    ...
    for (var i = 0; i < programSources.sources.length; i++) {
     var shader = new gluShaderProgram.Shader(gl, programSources.sources[i].type);
