...
goog.scope(function() {
    ...
    gluVarTypeUtil.SubTypeIterator.prototype.removeTraversed = function() {
        ...
        while (this.m_path.length) {
            var curComp = this.m_path[this.m_path.length - 1]; 
            var parentType = gluVarTypeUtil.getVarType(this.m_type, this.m_path, 0, this.m_path.length - 1); 
    ...
    gluVarTypeUtil.SubTypeIterator.prototype.findNext = function() {
        ...
        for (;;) {
            ...
            var curType = gluVarTypeUtil.getVarType(this.m_type, this.m_path); 
            ...
            if (curType.isBasicType()) {
                var basicType = curType.getBasicType(); 
    ...
    gluVarTypeUtil.getVarType = function(type, array, start, end) {
        ...
        if (pathIter != end) {
            ...
            var basicType = curType.getBasicType(); 
            var precision = curType.getPrecision(); 
