...
gluDrawUtil.draw = function(gl, program, vertexArrays, primitives, callback) {
    ...
    for (var i = 0; i < vertexArrays.length; i++) {
         var buffer = gluDrawUtil.vertexBuffer(gl, vertexArrays[i]);
        objects.push(buffer);
    }

    if (primitives.indices) {
         var elemBuffer = gluDrawUtil.indexBuffer(gl, primitives);
...
gluDrawUtil.drawIndexed = function(gl, primitives, offset) {
 var mode = gluDrawUtil.getPrimitiveGLType(gl, primitives.type);
...
gluDrawUtil.newPrimitiveListFromIndices = function(type, indices) {
     var primitiveList = new gluDrawUtil.PrimitiveList(type, 0);
...
gluDrawUtil.VertexComponentConversion = {
    VTX_COMP_CONVERT_NONE: 0, 
    VTX_COMP_CONVERT_NORMALIZE_TO_FLOAT: 1, 
    VTX_COMP_CONVERT_CAST_TO_FLOAT: 2 
};
