...
gluTexture.texture2DFromCompressedTexture = function(gl, numLevels, levels) {
    ...
    var refTex = new tcuTexture.Texture2D(level.getUncompressedFormat(), level.getWidth(), level.getHeight());
     var tex2d = new gluTexture.Texture2D(gl, format, true, refTex);
...
gluTexture.Texture2D.prototype.loadCompressed = function(numLevels, levels) {
     var compressedFormat = gluTextureUtil.getGLFormat(levels[0].getFormat());
    ...
    for (var levelNdx = 0; levelNdx < numLevels; levelNdx++) {
         var level = levels[levelNdx];

        
        this.m_refTexture.allocLevel(levelNdx);
         var refLevelAccess = this.m_refTexture.getLevel(levelNdx);
...
gluTexture.computePixelStore = function( format) {
...
gluTexture.cubeFaceToGLFace = function( face) {
    switch (face) {
...
gluTexture.Texture2D.prototype.upload = function() {
    ...
    for (var levelNdx = 0; levelNdx < this.m_refTexture.getNumLevels(); levelNdx++) {
        ...
        gl.texImage2D(gl.TEXTURE_2D, levelNdx, this.m_format, access.getWidth(), access.getHeight(), 0 , transferFormat.format, transferFormat.dataType, access.getDataPtr());
    }
    ...
    for (var face in tcuTexture.CubeFace) {
        for (var levelNdx = 0; levelNdx < this.m_refTexture.getNumLevels(); levelNdx++) {
            if (this.m_refTexture.isLevelEmpty(tcuTexture.CubeFace[face], levelNdx))
             ...
             var access = this.m_refTexture.getLevelFace(levelNdx, tcuTexture.CubeFace[face]);
            DE_ASSERT(access.getRowPitch() == access.getFormat().getPixelSize() * access.getWidth());
            gl.texImage2D(gluTexture.cubeFaceToGLFace(tcuTexture.CubeFace[face]), levelNdx, this.m_format, access.getWidth(), access.getHeight(), 0 , transferFormat.format, transferFormat.dataType, access.getDataPtr());
        }
    }
...
gluTexture.Texture2DArray.prototype.upload = function() {
    ...
    for (var levelNdx = 0; levelNdx < this.m_refTexture.getNumLevels(); levelNdx++) {
        ...
        gl.texImage3D(gl.TEXTURE_2D_ARRAY, levelNdx, this.m_format, access.getWidth(), access.getHeight(), access.getDepth(), 0 , transferFormat.format, transferFormat.dataType, access.getDataPtr());
    }
...
gluTexture.Texture3D.prototype.upload = function() {
    ...
    for (var levelNdx = 0; levelNdx < this.m_refTexture.getNumLevels(); levelNdx++) {
        ...
        DE_ASSERT(access.getRowPitch() == access.getFormat().getPixelSize() * access.getWidth());
        DE_ASSERT(access.getSlicePitch() == access.getFormat().getPixelSize() * access.getWidth() * access.getHeight());
        gl.texImage3D(gl.TEXTURE_3D, levelNdx, this.m_format, access.getWidth(), access.getHeight(), access.getDepth(), 0 , transferFormat.format, transferFormat.dataType, access.getDataPtr());
    }
...
gluTexture.Compressed2D.prototype.uploadLevel = function(level, source) {
    ...
    gl.compressedTexImage2D(gl.TEXTURE_2D, level, this.m_format, source.m_width, source.m_height, 0 , source.m_data);
    assertMsgOptions(gl.getError() === gl.NO_ERROR, 'Texture upload failed', false, true);
};
...
gluTexture.CompressedCube.prototype.uploadLevel = function(level, source) {
    ...
    for (var face in tcuTexture.CubeFace) {
        ...
        gl.compressedTexImage2D(gluTexture.cubeFaceToGLFace(tcuTexture.CubeFace[face]), 0, this.m_format,
                                source.m_width, source.m_height, 0 , source.m_data);
        assertMsgOptions(gl.getError() === gl.NO_ERROR, 'Texture upload failed', false, true);
    }
...
gluTexture.compressedCubeFromInternalFormat = function(gl, format, size, compressed) {
    ...
    for (var face in tcuTexture.CubeFace) {
        tex.m_refTexture.allocLevel(tcuTexture.CubeFace[face], 0);
         ...
         var access = tex.m_refTexture.getLevelFace(0, tcuTexture.CubeFace[face]);
        DE_ASSERT(access.getRowPitch() == access.getFormat().getPixelSize() * access.getWidth());
