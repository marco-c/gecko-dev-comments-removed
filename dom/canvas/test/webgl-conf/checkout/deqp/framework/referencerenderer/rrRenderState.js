...
rrRenderState.RasterizationState = function() {
     this.winding = rrRenderState.Winding.CCW;
     this.horizontalFill = rrRenderState.HorizontalFill.LEFT;
     this.verticalFill = rrRenderState.VerticalFill.BOTTOM;
};
...
rrRenderState.StencilState = function() {
     this.func = rrRenderState.TestFunc.ALWAYS;
     this.ref = 0;
     this.compMask = ~0;
     this.sFail = rrRenderState.StencilOp.KEEP;
     this.dpFail = rrRenderState.StencilOp.KEEP;
     this.dpPass = rrRenderState.StencilOp.KEEP;
     this.writeMask = ~0;
};
...
rrRenderState.BlendState = function() {
     this.equation = rrRenderState.BlendEquation.ADD;
     this.srcFunc = rrRenderState.BlendFunc.ONE;
     this.dstFunc = rrRenderState.BlendFunc.ZERO;
};
...
rrRenderState.FragmentOperationState = function() {
     ...
     this.scissorRectangle = new rrRenderState.WindowRectangle(0, 0, 1, 1);
    ...
    for (var type in rrDefs.FaceType)
     ...
     this.depthFunc = rrRenderState.TestFunc.LESS;
     this.depthMask = true;

     this.blendMode = rrRenderState.BlendMode.NONE;
     this.blendRGBState = new rrRenderState.BlendState();
     this.blendAState = new rrRenderState.BlendState();
...
rrRenderState.RenderState = function(viewport_) {
     this.cullMode = rrRenderState.CullMode.NONE;
     this.provokingVertexConvention;
     this.viewport = viewport_;

     this.rasterization = new rrRenderState.RasterizationState();
     this.fragOps = new rrRenderState.FragmentOperationState();
     this.point = new rrRenderState.PointState();
     this.line = new rrRenderState.LineState();
     this.restart = new rrRenderState.RestartState();
};
