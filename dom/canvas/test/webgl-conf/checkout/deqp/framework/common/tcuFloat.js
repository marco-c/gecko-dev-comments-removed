...
tcuFloat.FloatDescription.prototype.infNumber = function(sign) {
    return ((sign > 0 ? 0 : 1) << (this.ExponentBits + this.MantissaBits)) |
        deMath.shiftLeft(((1 << this.ExponentBits) - 1), this.MantissaBits); 
};
...
tcuFloat.FloatDescription.prototype.construct = function(sign, exponent, mantissa) {
     ...
     var isShorthandZero = exponent == 0 && mantissa == 0;

    
    

     var isDenormOrZero = (exponent == 1 - this.ExponentBias) && (deMath.shiftRight(mantissa, this.MantissaBits) == 0);
     var s = deMath.shiftLeft((sign < 0 ? 1 : 0), (this.ExponentBits + this.MantissaBits));
     var exp = (isShorthandZero || isDenormOrZero) ? 0 : exponent + this.ExponentBias;

    DE_ASSERT(sign == +1 || sign == -1);
    DE_ASSERT(isShorthandZero || isDenormOrZero || deMath.shiftRight(mantissa, this.MantissaBits) == 1);
    DE_ASSERT((exp >> this.ExponentBits) == 0); 
...
tcuFloat.FloatDescription.prototype.constructBits = function(sign, exponent, mantissaBits) {
     var signBit = sign < 0 ? 1 : 0;
     var exponentBits = exponent + this.ExponentBias;
...
tcuFloat.FloatDescription.prototype.convert = function(other) {
    ...
    if (!(this.Flags & tcuFloat.FloatFlags.FLOAT_HAS_SIGN) && other.sign() < 0) {
        ...
        return this.zero(other.sign());
         ...
         var eMin = 1 - this.ExponentBias;
         var eMax = ((1 << this.ExponentBits) - 2) - this.ExponentBias;

         var s = deMath.shiftLeft(other.signBit(), (this.ExponentBits + this.MantissaBits)); 
         var e = other.exponent();
...
tcuFloat.convertFloat32Inline = (function() {
    ...
    return function(fval, description) {
        ...
        if (!(description.Flags & tcuFloat.FloatFlags.FLOAT_HAS_SIGN) && sign < 0) {
        ...
        } else if (exponentBits == ((1 << tcuFloat.description32.ExponentBits) - 1) && mantissaBits == 0) { 
            ...
            var e = isDenorm ? 1 - tcuFloat.description32.ExponentBias : exponentBits - tcuFloat.description32.ExponentBias;
            var m = isZero || isDenorm ? mantissaBits : mantissaBits | (1 << tcuFloat.description32.MantissaBits); 
