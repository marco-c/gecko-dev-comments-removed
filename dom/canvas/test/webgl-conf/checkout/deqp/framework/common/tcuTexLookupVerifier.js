...
goog.scope(function() {
    ...
    tcuTexLookupVerifier.LookupPrecision = function(coordBits, uvwBits, colorThreshold, colorMask) {
         ...
         this.uvwBits = uvwBits || [16, 16, 16];
         this.colorThreshold = colorThreshold || [0, 0, 0, 0];
    ...
    tcuTexLookupVerifier.LodPrecision = function(derivateBits, lodBits) {
         this.derivateBits = derivateBits === undefined ? 22 : derivateBits;
         this.lodBits = lodBits === undefined ? 16 : lodBits;
    };
    ...
    tcuTexLookupVerifier.computeBilinearSearchStepFromFloatLine = function(prec, line) {
        assertMsgOptions(deMath.boolAll(deMath.greaterThan(prec.colorThreshold, [0, 0, 0, 0])), 'Threshold not greater than 0.', false, true);
         ...
         var maxSteps = 1 << 16;
         var d = deMath.absDiff(line.p1, line.p0);
         var stepCount = deMath.divide([d, d, d, d], prec.colorThreshold);
        
        var minStep = deMath.divide([1, 1, 1, 1], deMath.add(stepCount, [1, 1, 1, 1]));
         var step = Math.max(tcuTexLookupVerifier.minComp(minStep), 1 / maxSteps);
    ...
    tcuTexLookupVerifier.computeBilinearSearchStepFromFloatQuad = function(prec, quad) {
        assertMsgOptions(deMath.boolAll(deMath.greaterThan(prec.colorThreshold, [0, 0, 0, 0])), 'Threshold not greater than 0.', false, true);
         ...
         var maxSteps = 1 << 16;
         var d0 = deMath.absDiff(quad.p10, quad.p00);
         var d1 = deMath.absDiff(quad.p01, quad.p00);
         var d2 = deMath.absDiff(quad.p11, quad.p10);
         var d3 = deMath.absDiff(quad.p11, quad.p01);
         var maxD = deMath.max(d0, deMath.max(d1, deMath.max(d2, d3)));
         var stepCount = deMath.divide(maxD, prec.colorThreshold);
         var minStep = deMath.divide([1, 1, 1, 1], deMath.add(stepCount, [1, 1, 1, 1]));
         var step = Math.max(tcuTexLookupVerifier.minComp(minStep), 1 / maxSteps);
    ...
    tcuTexLookupVerifier.computeBilinearSearchStepForUnorm = function(prec) {
        assertMsgOptions(deMath.boolAll(deMath.greaterThan(prec.colorThreshold, [0, 0, 0, 0])), 'Threshold not greater than 0.', false, true);
         ...
         var stepCount = deMath.divide([1, 1, 1, 1], prec.colorThreshold);
         var minStep = deMath.divide([1, 1, 1, 1], (deMath.add(stepCount, [1, 1, 1, 1])));
         var step = tcuTexLookupVerifier.minComp(minStep);
    ...
    tcuTexLookupVerifier.computeBilinearSearchStepForSnorm = function(prec) {
        assertMsgOptions(deMath.boolAll(deMath.greaterThan(prec.colorThreshold, [0, 0, 0, 0])), 'Threshold not greater than 0.', false, true);
         ...
         var stepCount = deMath.divide([2.0, 2.0, 2.0, 2.0], prec.colorThreshold);
         var minStep = deMath.divide([1, 1, 1, 1], deMath.add(stepCount, [1, 1, 1, 1]));
         var step = tcuTexLookupVerifier.minComp(minStep);
    ...
    tcuTexLookupVerifier.isInColorBounds_1Quad = function(prec, quad, result) {
        ...
        var quadMax = tcuTexLookupVerifier.maxQuad;
         var minVal = deMath.subtract(quadMin(quad), prec.colorThreshold);
         var maxVal = deMath.add(quadMax(quad), prec.colorThreshold);
        return deMath.boolAll(
            deMath.logicalOrBool(
    ...
    tcuTexLookupVerifier.isInColorBounds_2Quad = function(prec, quad0, quad1, result) {
        ...
        var quadMin = tcuTexLookupVerifier.minQuad;
        var quadMax = tcuTexLookupVerifier.maxQuad;
         var minVal = deMath.subtract(min(quadMin(quad0), quadMin(quad1)), prec.colorThreshold);
         var maxVal = deMath.add(max(quadMax(quad0), quadMax(quad1)), prec.colorThreshold);
        return deMath.boolAll(
            deMath.logicalOrBool(
                deMath.logicalAndBool(
                    deMath.greaterThanEqual(result, minVal),
    ...
    tcuTexLookupVerifier.isInColorBounds_2Line = function(prec, line0, line1, result) {
        ...
        var lineMax = tcuTexLookupVerifier.maxLine;
         var minVal = deMath.subtract(min(lineMin(line0), lineMin(line1)), prec.colorThreshold);
         var maxVal = deMath.add(max(lineMax(line0), lineMax(line1)), prec.colorThreshold);
        return deMath.boolAll(
            deMath.logicalOrBool(
                deMath.logicalAndBool(
                    deMath.greaterThanEqual(result, minVal),
    ...
    tcuTexLookupVerifier.isInColorBounds_4Quad = function(prec, quad00, quad01, quad10, quad11, result) {
        ...
        var quadMin = tcuTexLookupVerifier.minQuad;
        var quadMax = tcuTexLookupVerifier.maxQuad;
         var minVal = deMath.subtract(min(quadMin(quad00), min(quadMin(quad01), min(quadMin(quad10), quadMin(quad11)))), prec.colorThreshold);
         var maxVal = deMath.add(max(quadMax(quad00), max(quadMax(quad01), max(quadMax(quad10), quadMax(quad11)))), prec.colorThreshold);
        return deMath.boolAll(
            deMath.logicalOrBool(
                deMath.logicalAndBool(
                    deMath.greaterThanEqual(result, minVal),
    ...
    tcuTexLookupVerifier.isLinearRangeValid = function(prec, c0, c1, fBounds, result) {
         ...
         var rMin = deMath.subtract(result, prec.colorThreshold);
         var rMax = deMath.add(result, prec.colorThreshold);
        ...
        for (var compNdx = 0; compNdx < 4; compNdx++) {
            if (!prec.colorMask[compNdx])
             ...
             var i0 = c0[compNdx] * (1 - fBounds[0]) + c1[compNdx] * fBounds[0];
             var i1 = c0[compNdx] * (1 - fBounds[1]) + c1[compNdx] * fBounds[1];
    ...
    tcuTexLookupVerifier.isBilinearRangeValid = function(prec, quad, xBounds, yBounds, searchStep, result) {
        ...
        for (var x = xBounds[0]; x < xBounds[1] + searchStep; x += searchStep) {
             var a = Math.min(x, xBounds[1]);
             var c0 = deMath.add(deMath.scale(quad.p00, (1 - a)), deMath.scale(quad.p10, a));
             var c1 = deMath.add(deMath.scale(quad.p01, (1 - a)), deMath.scale(quad.p11, a));
    ...
    tcuTexLookupVerifier.is2DTrilinearFilterResultValid = function(prec, quad0, quad1, xBounds0, yBounds0, xBounds1, yBounds1, zBounds, searchStep, result) {
        ...
        for (var x0 = xBounds0[0]; x0 < xBounds0[1] + searchStep; x0 += searchStep) {
            for (var y0 = yBounds0[0]; y0 < yBounds0[1] + searchStep; y0 += searchStep) {
                 var a0 = Math.min(x0, xBounds0[1]);
                 var b0 = Math.min(y0, yBounds0[1]);
                ...
                for (var x1 = xBounds1[0]; x1 <= xBounds1[1]; x1 += searchStep) {
                    for (var y1 = yBounds1[0]; y1 <= yBounds1[1]; y1 += searchStep) {
                         var a1 = Math.min(x1, xBounds1[1]);
                         var b1 = Math.min(y1, yBounds1[1]);
    ...
    tcuTexLookupVerifier.isNearestSampleResultValid_CoordAsVec3 = function(level, sampler, prec, coord, result) {
        ...
        var wBounds = tcuTexVerifierUtil.computeNonNormalizedCoordBounds(
         ...
         var minJ = Math.floor(vBounds[0]);
         var maxJ = Math.floor(vBounds[1]);
         var minK = Math.floor(wBounds[0]);
         var maxK = Math.floor(wBounds[1]);
        ...
        for (var k = minK; k <= maxK; k++) {
            for (var j = minJ; j <= maxJ; j++) {
                for (var i = minI; i <= maxI; i++) {
                     var x = tcuTexVerifierUtil.wrap(sampler.wrapS, i, level.getWidth());
                     var y = tcuTexVerifierUtil.wrap(sampler.wrapT, j, level.getHeight());
                     var z = tcuTexVerifierUtil.wrap(sampler.wrapR, k, level.getDepth());
                     var color;
                    if (tcuTexLookupVerifier.isSRGB(level.getFormat())) {
    ...
    tcuTexLookupVerifier.isLinearSampleResultValid_CoordXYAsNumber = function(level, sampler, prec, coordX, coordY, result) {
         var uBounds = tcuTexVerifierUtil.computeNonNormalizedCoordBounds(sampler.normalizedCoords, level.getWidth(), coordX, prec.coordBits[0], prec.uvwBits[0]);
        ...
        for (var i = minI; i <= maxI; i++) {
             ...
             var colorA = tcuTexLookupVerifier.lookupFloat(level, sampler, x0, coordY, 0);
             var colorB = tcuTexLookupVerifier.lookupFloat(level, sampler, x1, coordY, 0);
    ...
    tcuTexLookupVerifier.isLinearSampleResultValid_CoordAsVec3 = function(level, sampler, prec, coord, result) {
        ...
        var wBounds = tcuTexVerifierUtil.computeNonNormalizedCoordBounds(
         ...
         var minJ = Math.floor(vBounds[0] - 0.5);
         var maxJ = Math.floor(vBounds[1] - 0.5);
         var minK = Math.floor(wBounds[0] - 0.5);
         var maxK = Math.floor(wBounds[1] - 0.5);

         var w = level.getWidth();
         var h = level.getHeight();
         var d = level.getDepth();
        ...
        for (var k = minK; k <= maxK; k++) {
            for (var j = minJ; j <= maxJ; j++) {
                for (var i = minI; i <= maxI; i++) {
                     ...
                     var y0 = tcuTexVerifierUtil.wrap(sampler.wrapT, j, h);
                     var y1 = tcuTexVerifierUtil.wrap(sampler.wrapT, j + 1, h);
                     var z0 = tcuTexVerifierUtil.wrap(sampler.wrapR, k, d);
                     var z1 = tcuTexVerifierUtil.wrap(sampler.wrapR, k + 1, d);
                     ...
                     var minB = deMath.clamp((vBounds[0] - 0.5) - j, 0, 1);
                     var maxB = deMath.clamp((vBounds[1] - 0.5) - j, 0, 1);
                     var minC = deMath.clamp((wBounds[0] - 0.5) - k, 0, 1);
                     var maxC = deMath.clamp((wBounds[1] - 0.5) - k, 0, 1);
        ...
        for (var j0 = minJ0; j0 <= maxJ0; j0++) {
            for (var i0 = minI0; i0 <= maxI0; i0++) {
                for (var j1 = minJ1; j1 <= maxJ1; j1++) {
                    for (var i1 = minI1; i1 <= maxI1; i1++) {
                         var c0 = tcuTexLookupVerifier.lookupFloat(level0, sampler, tcuTexVerifierUtil.wrap(sampler.wrapS, i0, w0), tcuTexVerifierUtil.wrap(sampler.wrapT, j0, h0), coordZ);
                         var c1 = tcuTexLookupVerifier.lookupFloat(level1, sampler, tcuTexVerifierUtil.wrap(sampler.wrapS, i1, w1), tcuTexVerifierUtil.wrap(sampler.wrapT, j1, h1), coordZ);
    ...
    tcuTexLookupVerifier.isNearestMipmapLinearSampleResultValid_CoordAsVec3 = function(level0, level1, sampler, prec, coord, fBounds, result) {
         ...
         var h0 = level0.getHeight();
         var h1 = level1.getHeight();
         var d0 = level0.getDepth();
         var d1 = level1.getDepth();
        ...
        var wBounds1 = tcuTexVerifierUtil.computeNonNormalizedCoordBounds(
         ...
         var minJ1 = Math.floor(vBounds1[0]);
         var maxJ1 = Math.floor(vBounds1[1]);
         var minK0 = Math.floor(wBounds0[0]);
         var maxK0 = Math.floor(wBounds0[1]);
         var minK1 = Math.floor(wBounds1[0]);
         var maxK1 = Math.floor(wBounds1[1]);
            ...
            for (var j0 = minJ0; j0 <= maxJ0; j0++) {
                for (var i0 = minI0; i0 <= maxI0; i0++) {
                    for (var k1 = minK1; k1 <= maxK1; k1++) {
                        for (var j1 = minJ1; j1 <= maxJ1; j1++) {
                            for (var i1 = minI1; i1 <= maxI1; i1++) {
                                 var c0 = tcuTexLookupVerifier.lookupFloat(level0, sampler, tcuTexVerifierUtil.wrap(sampler.wrapS, i0, w0), tcuTexVerifierUtil.wrap(sampler.wrapT, j0, h0), tcuTexVerifierUtil.wrap(sampler.wrapR, k0, d0));
                                 var c1 = tcuTexLookupVerifier.lookupFloat(level1, sampler, tcuTexVerifierUtil.wrap(sampler.wrapS, i1, w1), tcuTexVerifierUtil.wrap(sampler.wrapT, j1, h1), tcuTexVerifierUtil.wrap(sampler.wrapR, k1, d1));
    ...
    tcuTexLookupVerifier.isLinearMipmapLinearSampleResultValid_CoordAsVec2AndInt = function(level0, level1, sampler, prec, coord, coordZ, fBounds, result) {
        ...
        var texClass = tcuTexture.getTextureChannelClass(level0.getFormat().type);
         var cSearchStep = (texClass == tcuTexture.TextureChannelClass.UNSIGNED_FIXED_POINT) ? tcuTexLookupVerifier.computeBilinearSearchStepForUnorm(prec) :
                                                (texClass == tcuTexture.TextureChannelClass.SIGNED_FIXED_POINT) ? tcuTexLookupVerifier.computeBilinearSearchStepForSnorm(prec) :
                                                0; 
    ...
    tcuTexLookupVerifier.isLinearMipmapLinearSampleResultValid_CoordAsVec3 = function(level0, level1, sampler, prec, coord, fBounds, result) {
        ...
        var wBounds1 = tcuTexVerifierUtil.computeNonNormalizedCoordBounds(
         ...
         var minJ1 = Math.floor(vBounds1[0] - 0.5);
         var maxJ1 = Math.floor(vBounds1[1] - 0.5);
         var minK0 = Math.floor(wBounds0[0] - 0.5);
         var maxK0 = Math.floor(wBounds0[1] - 0.5);
         var minK1 = Math.floor(wBounds1[0] - 0.5);
         var maxK1 = Math.floor(wBounds1[1] - 0.5);
            ...
            for (var j0 = minJ0; j0 <= maxJ0; j0++) {
                for (var i0 = minI0; i0 <= maxI0; i0++) {
                    ...
                    if (texClass == tcuTexture.TextureChannelClass.FLOATING_POINT)
                     ...
                     var minB0 = deMath.clamp((vBounds0[0] - 0.5) - j0, 0, 1);
                     var maxB0 = deMath.clamp((vBounds0[1] - 0.5) - j0, 0, 1);
                     var minC0 = deMath.clamp((wBounds0[0] - 0.5) - k0, 0, 1);
                     var maxC0 = deMath.clamp((wBounds0[1] - 0.5) - k0, 0, 1);
                        ...
                        for (var j1 = minJ1; j1 <= maxJ1; j1++) {
                            for (var i1 = minI1; i1 <= maxI1; i1++) {
                                ...
                                if (texClass == tcuTexture.TextureChannelClass.FLOATING_POINT)
                                 ...
                                 var minB1 = deMath.clamp((vBounds1[0] - 0.5) - j1, 0, 1);
                                 var maxB1 = deMath.clamp((vBounds1[1] - 0.5) - j1, 0, 1);
                                 var minC1 = deMath.clamp((wBounds1[0] - 0.5) - k1, 0, 1);
                                 var maxC1 = deMath.clamp((wBounds1[1] - 0.5) - k1, 0, 1);
    ...
    tcuTexLookupVerifier.isLookupResultValid_TextureCubeView = function(texture, sampler, prec, coord, lodBounds, result) {
         var numPossibleFaces = 0;

        assertMsgOptions(tcuTexLookupVerifier.isSamplerSupported(sampler), 'Sampler not supported.', false, true);

         var possibleFaces = tcuTexVerifierUtil.getPossibleCubeFaces(coord, prec.coordBits);
            ...
            if (canBeMinified) {
                ...
                if (isLinearMipmap && minTexLevel < maxTexLevel) {
                    ...
                    for (var levelNdx = minLevel; levelNdx <= maxLevel; levelNdx++) {
                         var minF = deMath.clamp(minLod - levelNdx, 0, 1);
                         var maxF = deMath.clamp(maxLod - levelNdx, 0, 1);
    ...
    tcuTexLookupVerifier.isLookupResultValid_Texture2DArrayView = function(texture, sampler, prec, coord, lodBounds, result) {
         var layerRange = tcuTexLookupVerifier.computeLayerRange(texture.getNumLayers(), prec.coordBits[2], coord[2]);
         var coordXY = deMath.swizzle(coord, [0, 1]);
         var minLod = lodBounds[0];
         var maxLod = lodBounds[1];
         var canBeMagnified = minLod <= sampler.lodThreshold;
         var canBeMinified = maxLod > sampler.lodThreshold;
    ...
    tcuTexLookupVerifier.isSeamlessLinearSampleResultValid = function(faces, sampler, prec, coords, result) {
         var size = faces[coords.face].getWidth();
         ...
         var texClass = tcuTexture.getTextureChannelClass(faces[coords.face].getFormat().type);
         var searchStep = (texClass == tcuTexture.TextureChannelClass.UNSIGNED_FIXED_POINT) ? tcuTexLookupVerifier.computeBilinearSearchStepForUnorm(prec) :
                                               (texClass == tcuTexture.TextureChannelClass.SIGNED_FIXED_POINT) ? tcuTexLookupVerifier.computeBilinearSearchStepForSnorm(prec) :
                                               0; 
    ...
    tcuTexLookupVerifier.isSeamplessLinearMipmapLinearSampleResultValid = function(faces0, faces1, sampler, prec, coords, fBounds, result) {
         ...
         var size0 = faces0[coords.face].getWidth();
         var size1 = faces1[coords.face].getWidth();
         ...
         var texClass = tcuTexture.getTextureChannelClass(faces0[coords.face].getFormat().type);
         var cSearchStep = (texClass == tcuTexture.TextureChannelClass.UNSIGNED_FIXED_POINT) ? tcuTexLookupVerifier.computeBilinearSearchStepForUnorm(prec) :
                                                (texClass == tcuTexture.TextureChannelClass.SIGNED_FIXED_POINT) ? tcuTexLookupVerifier.computeBilinearSearchStepForSnorm(prec) :
                                                0; 
    ...
    tcuTexLookupVerifier.getCubeLevelFaces = function(texture, levelNdx, out) {
        for (var faceNdx = 0; faceNdx < 6; faceNdx++)
            out[faceNdx] = texture.getLevelFace(levelNdx,  (faceNdx));
    };
    ...
    tcuTexLookupVerifier.computeLayerRange = function(numLayers, numCoordBits, layerCoord) {
         var err = tcuTexVerifierUtil.computeFloatingPointError(layerCoord, numCoordBits);
         var minL = Math.floor(layerCoord - err + 0.5); 
         var maxL = Math.ceil(layerCoord + err + 0.5) - 1; 
    ...
    tcuTexLookupVerifier.computeLodBoundsFromDerivates = function(dudx, dvdx, dwdx, dudy, dvdy, dwdy, prec) {
         ...
         var minDBound = Math.max(Math.max(mu, mv), mw);
         var maxDBound = mu + mv + mw;
         var minDErr = tcuTexVerifierUtil.computeFloatingPointError(minDBound, prec.derivateBits);
         var maxDErr = tcuTexVerifierUtil.computeFloatingPointError(maxDBound, prec.derivateBits);
         var minLod = Math.log2(minDBound - minDErr);
         var maxLod = Math.log2(maxDBound + maxDErr);
         var lodErr = tcuTexVerifierUtil.computeFixedPointError(prec.lodBits);
    ...
    tcuTexLookupVerifier.computeCubeLodBoundsFromDerivates = function(coord, coordDx, coordDy, faceSize, prec) {
         var allowBrokenEdgeDerivate = false;
        ...
        switch (face) {
         ...
         var bounds = tcuTexLookupVerifier.computeLodBoundsFromDerivatesUV(dudx, dvdx, dudy, dvdy, prec);

        
        if (allowBrokenEdgeDerivate) {
             var dxErr = tcuTexVerifierUtil.computeFloatingPointError_Vector(coordDx, [prec.derivateBits, prec.derivateBits, prec.derivateBits]);
             var dyErr = tcuTexVerifierUtil.computeFloatingPointError_Vector(coordDy, [prec.derivateBits, prec.derivateBits, prec.derivateBits]);
             var xoffs = deMath.add(deMath.abs(coordDx), dxErr);
             var yoffs = deMath.add(deMath.abs(coordDy), dyErr);
