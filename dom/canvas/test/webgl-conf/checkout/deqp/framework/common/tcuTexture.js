...
tcuTexture.getTextureChannelClass = function(channelType) {
    ...
    switch (channelType) {
        ...
        default: return  (Object.keys(tcuTexture.ChannelType).length);
    }
};
...
tcuTexture.getTypedArray = function(type) {
    switch (type) {
        ...
        case tcuTexture.ChannelType.FLOAT_UNSIGNED_INT_24_8_REV: return Float32Array; 
    }
...
tcuTexture.WrapMode = {
    ...
    REPEAT_GL: 2, 
    REPEAT_CL: 3, 
    MIRRORED_REPEAT_GL: 4, 
    MIRRORED_REPEAT_CL: 5 
};
...
tcuTexture.packRGB999E5 = function(color) {
     ...
     var eBias = 15;
     var eMax = (1 << eBits) - 1;
     var maxVal = (((1 << mBits) - 1) * (1 << (eMax - eBias))) / (1 << mBits);
        ...
        switch (this.m_format.type) {
            ...
            case tcuTexture.ChannelType.UNSIGNED_INT_24_8:
                switch (this.m_format.order) {
                    ...
                    case tcuTexture.ChannelOrder.DS: return [nb(pixel, 8, 24), 0, 0, 1 ];
                    default:
                        DE_ASSERT(false);
                }
...
tcuTexture.Texture2DView.prototype.sample = function(sampler, texCoord, lod) {
    return tcuTexture.sampleLevelArray2D(this.m_levels, this.m_numLevels, sampler, texCoord[0], texCoord[1], 0 , lod);
};
    ...
    switch (filterMode) {
        ...
        case tcuTexture.FilterMode.LINEAR_MIPMAP_LINEAR:
            ...
            levelFilter = (filterMode === tcuTexture.FilterMode.LINEAR_MIPMAP_LINEAR) ? tcuTexture.FilterMode.LINEAR : tcuTexture.FilterMode.NEAREST;
             var f = deMath.deFloatFrac(lod);
             var t0 = levels[level0].sample2DOffset(sampler, levelFilter, texCoord[0], texCoord[1], offset);
             var t1 = levels[level1].sample2DOffset(sampler, levelFilter, texCoord[0], texCoord[1], offset);
...
tcuTexture.sampleLevelArray3DOffset = function(levels, numLevels, sampler, s, t, r, lod, offset) {
    ...
    switch (filterMode) {
        ...
        case tcuTexture.FilterMode.LINEAR_MIPMAP_LINEAR:
            ...
            levelFilter = (filterMode === tcuTexture.FilterMode.LINEAR_MIPMAP_LINEAR) ? tcuTexture.FilterMode.LINEAR : tcuTexture.FilterMode.NEAREST;
             var f = deMath.deFloatFrac(lod);
             var t0 = levels[level0].sample3DOffset(sampler, levelFilter, s, t, r, offset);
             var t1 = levels[level1].sample3DOffset(sampler, levelFilter, s, t, r, offset);
...
tcuTexture.getCubeLinearSamples = function(faceAccesses, baseFace, u, v, depth) {
    ...
    [
     ...
     var sampleColors = [];
     var hasBothCoordsOutOfBounds = []; 

    

    for (var i = 0; i < 4; i++) {
         var coords = tcuTexture.remapCubeEdgeCoords(new tcuTexture.CubeFaceCoords(baseFace, baseSampleCoords[i]), size);
    ...
    for (var i = 0; i < 4; i++) {
        if (hasBothCoordsOutOfBounds[i]) {
            DE_ASSERT(bothOutOfBoundsNdx < 0); 
...
tcuTexture.sampleCubeSeamlessLinear = function(faceAccesses, baseFace, sampler, s, t, depth) {
    ...
    if (sampler.normalizedCoords) {
     ...
     var sampleColors = tcuTexture.getCubeLinearSamples(faceAccesses, baseFace, u, v, depth);
...
tcuTexture.sampleCubeSeamlessNearestCompare = function(faceAccess, sampler, ref, s, t, depth) {
    depth = depth ? depth : 0;
     var clampingSampler = deUtil.clone(sampler);
...
tcuTexture.TextureCubeView.prototype.sample = function(sampler, texCoord, lod) {
    ...
    if (sampler.seamlessCubeMap)
        return tcuTexture.sampleLevelArrayCubeSeamless(this.m_levels, this.m_numLevels, coords.face, sampler, coords.s, coords.t, 0 , lod);
    else
        return tcuTexture.sampleLevelArray2D(this.m_levels[coords.face], this.m_numLevels, sampler, coords.s, coords.t, 0 , lod);
};
...
tcuTexture.TextureCube.prototype.allocLevel = function(face, levelNdx) {
     var size = tcuTexture.getMipPyramidLevelSize(this.m_size, levelNdx);
     var dataSize = this.m_format.getPixelSize() * size * size;
