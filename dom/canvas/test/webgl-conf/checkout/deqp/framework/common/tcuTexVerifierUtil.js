...
goog.scope(function() {
    ...
    tcuTexVerifierUtil.computeFloatingPointError = function(value, numAccurateBits) {
         ...
         var v1 = new tcuFloat.deFloat();
         var v2 = new tcuFloat.deFloat();
    ...
    tcuTexVerifierUtil.computeNonNormalizedCoordBounds = function(normalizedCoords, dim, coord, coordBits, uvBits) {
         var coordErr = tcuTexVerifierUtil.computeFloatingPointError(coord, coordBits);
         var minN = coord - coordErr;
         var maxN = coord + coordErr;
         var minA = normalizedCoords ? minN * dim : minN;
         var maxA = normalizedCoords ? maxN * dim : maxN;
         var minC = minA - tcuTexVerifierUtil.computeFixedPointError(uvBits);
         var maxC = maxA + tcuTexVerifierUtil.computeFixedPointError(uvBits);
     ...
     tcuTexVerifierUtil.getPossibleCubeFaces = function(coord, bits) {
         ...
         var ay = Math.abs(y);
         var az = Math.abs(z);
         var ex = tcuTexVerifierUtil.computeFloatingPointError(x, bits[0]);
         var ey = tcuTexVerifierUtil.computeFloatingPointError(y, bits[1]);
         var ez = tcuTexVerifierUtil.computeFloatingPointError(z, bits[2]);
         var numFaces = 0;
