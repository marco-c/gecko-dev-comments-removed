...
var decompressEAC_R11 = function( dst, width, height, src, signedMode) {
...
var decompressEAC_RG11 = function( dst, width, height, src, signedMode) {
     var numBlocksX = tcuCompressedTexture.divRoundUp(width, 4);
     var numBlocksY = tcuCompressedTexture.divRoundUp(height, 4);
    var dstPtr;
    var dstRowPitch = dst.getRowPitch();
    ...
    for (var blockY = 0; blockY < numBlocksY; blockY++) {
        for (var blockX = 0; blockX < numBlocksX; blockX++) {
             var compressedBlockR = get128BitBlockStart(src, blockY * numBlocksX + blockX);
             var compressedBlockG = get128BitBlockEnd(src, blockY * numBlocksX + blockX);
...
var decompressETC2Block = function(dst, src, alphaDst, alphaMode) {
    ...
    var selBR = getBits64(src, 59, 63); 
    var selBG = getBits64(src, 51, 55);
    var selBB = getBits64(src, 43, 47);
    var selDR = extendSigned3To8(getBits64(src, 56, 58)); 
    ...
    if (mode == Etc2Mode.MODE_INDIVIDUAL || mode == Etc2Mode.MODE_DIFFERENTIAL) {
        ...
        for (var x = 0; x < ETC2_BLOCK_WIDTH; x++) {
            for (var y = 0; y < ETC2_BLOCK_HEIGHT; y++, pixelNdx++) {
                 ...
                 var alphaDstOffset = (y * ETC2_BLOCK_WIDTH + x) * ETC2_UNCOMPRESSED_PIXEL_SIZE_A8; 
...
var decompressETC2 = function( dst, width, height, src) {
    var numBlocksX = tcuCompressedTexture.divRoundUp(width, 4);
    var numBlocksY = tcuCompressedTexture.divRoundUp(height, 4);
...
var decompressETC2_EAC_RGBA8 = function( dst, width, height, src) {
    var numBlocksX = tcuCompressedTexture.divRoundUp(width, 4);
    var numBlocksY = tcuCompressedTexture.divRoundUp(height, 4);
    var dstPtr = new Uint8Array(dst.m_data);
    var dstRowPitch = dst.getRowPitch();
...
var decompressETC2_RGB8_PUNCHTHROUGH_ALPHA1 = function( dst, width, height, src) {
    var numBlocksX = tcuCompressedTexture.divRoundUp(width, 4);
    var numBlocksY = tcuCompressedTexture.divRoundUp(height, 4);
    var dstPtr = new Uint8Array(dst.m_data);
    var dstRowPitch = dst.getRowPitch();
