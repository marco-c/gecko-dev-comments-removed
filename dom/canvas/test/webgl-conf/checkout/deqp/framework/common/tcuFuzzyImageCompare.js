...
var tcuTextureUtil = framework.common.tcuTextureUtil;
    ...
    tcuFuzzyImageCompare.compareColors = function(pa, pb, minErrThreshold) {
         var r = Math.max(Math.abs(pa[0] - pb[0]) - minErrThreshold, 0);
         var g = Math.max(Math.abs(pa[1] - pb[1]) - minErrThreshold, 0);
         var b = Math.max(Math.abs(pa[2] - pb[2]) - minErrThreshold, 0);
         var a = Math.max(Math.abs(pa[3] - pb[3]) - minErrThreshold, 0);
    ...
    tcuFuzzyImageCompare.bilinearSample = function(src, u, v, NumChannels) {
         ...
         var i0 = deMath.clamp(x0, 0, w - 1);
         var i1 = deMath.clamp(x1, 0, w - 1);
         var j0 = deMath.clamp(y0, 0, h - 1);
         var j1 = deMath.clamp(y1, 0, h - 1);

         var a = (u - 0.5) - Math.floor(u - 0.5);
         var b = (u - 0.5) - Math.floor(u - 0.5);

         var p00 = src.read(i0, j0, NumChannels);
         var p10 = src.read(i1, j0, NumChannels);
         var p01 = src.read(i0, j1, NumChannels);
         var p11 = src.read(i1, j1, NumChannels);
         var dst = 0;
    ...
    tcuFuzzyImageCompare.separableConvolve = function(dst, src, shiftX, shiftY, kernelX, kernelY, DstChannels, SrcChannels) {
        DE_ASSERT(dst.width == src.width && dst.height == src.height);
         ...
         var tmp = new tcuTexture.TextureLevel(dst.getFormat(), dst.height, dst.width);
        var tmpView = new tcuTexture.RGBA8View(tmp.getAccess());

         var kw = kernelX.length;
         var kh = kernelY.length;
    ...
    tcuFuzzyImageCompare.compareToNeighbor = function(params, rnd, pixel, surface, x, y, NumChannels) {
        ...
        for (var s = 0; s < 32; s++) {
            ...
            dy = y + rnd.getFloat() * 2.0 - 0.5;
             ...
             var sample = tcuFuzzyImageCompare.bilinearSample(surface, dx, dy, NumChannels);
    ...
    tcuFuzzyImageCompare.fuzzyCompare = function(params, ref, cmp, errorMask) {
        ...
        if (!tcuFuzzyImageCompare.isFormatSupported(ref.getFormat()) || !tcuFuzzyImageCompare.isFormatSupported(cmp.getFormat()))
         ...
         var width = ref.getWidth();
         var height = ref.getHeight();
         var rnd = new deRandom.Random(667);

        
         var refFiltered = new tcuTexture.TextureLevel(new tcuTexture.TextureFormat(tcuTexture.ChannelOrder.RGBA, tcuTexture.ChannelType.UNORM_INT8), width, height);
         var cmpFiltered = new tcuTexture.TextureLevel(new tcuTexture.TextureFormat(tcuTexture.ChannelOrder.RGBA, tcuTexture.ChannelType.UNORM_INT8), width, height);
        ...
        var cmpFilteredView = new tcuTexture.RGBA8View(tcuTexture.PixelBufferAccess.newFromTextureLevel(cmpFiltered));
         ...
         var kernel = [0.1, 0.8, 0.1];
         var shift = Math.floor((kernel.length - 1) / 2);
        ...
        for (var y = 1; y < height - 1; y++) {
            for (var x = 1; x < width - 1; x += params.maxSampleSkip > 0 ? rnd.getInt(0, params.maxSampleSkip) : 1) {
                 var err = Math.min(tcuFuzzyImageCompare.compareToNeighbor(params, rnd, refFilteredView.read(x, y, 4), cmpFilteredView, x, y, 4),
                ...
                numSamples += 1;
                 ...
                 var red = err * 500.0;
                 var luma = tcuFuzzyImageCompare.toGrayscale(cmp.getPixel(x, y));
                 var rF = 0.7 + 0.3 * luma;
