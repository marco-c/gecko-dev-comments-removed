    ...
    internalFormat, pixelFormat, pixelType, gl, tiu, wtu)
{
    ...
    for (var tt = 0; tt < targets.length; ++tt) {
        if (optionsVal.is3D) {
            gl.texImage3D(targets[tt], 0, gl[internalFormat], bitmap.width, bitmap.height, 1 , 0,
    ...
    internalFormat, pixelFormat, pixelType, gl, tiu, wtu)
{
    ...
    if (optionsVal.is3D) {
        if (useTexSubImage) {
            ...
            gl.texImage3D(bindingTarget, 0, gl[internalFormat], bitmap.width, bitmap.height, 1 , 0,
                          gl[pixelFormat], gl[pixelType], null);
            ...
            gl.pixelStorei(gl.UNPACK_SKIP_IMAGES, 0);
            gl.texImage3D(bindingTarget, 0, gl[internalFormat], bitmap.width, bitmap.height / 2, 1 , 0,
                          gl[pixelFormat], gl[pixelType], bitmap);
            if (!flipY) {
                tl = srcBL;
                tr = srcBR;
