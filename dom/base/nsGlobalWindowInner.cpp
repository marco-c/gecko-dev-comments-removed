...
nsGlobalWindowInner::PostHandleEvent(EventChainPostVisitor& aVisitor)
{
  ...
  mozilla::Unused << kungFuDeathGrip1; 
  nsCOMPtr<nsIScriptContext> kungFuDeathGrip2(GetContextInternal());
  mozilla::Unused << kungFuDeathGrip2; 
...
nsGlobalWindowInner::GetScriptableTop()
{
  ...
  GetTopImpl(AssertOuter(), getter_AddRefs(window),  true);
  return window.get();
}
...
nsGlobalWindowInner::GetTop()
{
  MOZ_ASSERT(IsOuterWindow());
  nsCOMPtr<nsPIDOMWindowOuter> window;
  GetTopImpl(AssertOuter(), getter_AddRefs(window),  false);
                          ...
                          ErrorResult& aError)
{
  ...
  if (framesFinder) {
    framesFinder->SetRootSearchFrame(AsOuter());   
...
void
nsGlobalWindowInner::SetHasGamepadEventListener(bool aHasGamepad)
{
  MOZ_ASSERT(IsInnerWindow());
...
nsGlobalWindowInner::GetIntlUtils(ErrorResult& aError)
{
 ...
 already_AddRefed<nsGlobalWindowInner>
