  ...
  if (channels > 2) {
    if (mIn.Format() == AudioConfig::FORMAT_FLT) {
      ...
      static const float dmatrix[6][8][2]= {
          {{0.5858f,0},{0,0.5858f},{0.4142f,0.4142f}},
          {{0.4226f,0},{0,0.4226f},{0.366f, 0.2114f},{0.2114f,0.366f}},
          {{0.6510f,0},{0,0.6510f},{0.4600f,0.4600f},{0.5636f,0.3254f},{0.3254f,0.5636f}},
          {{0.5290f,0},{0,0.5290f},{0.3741f,0.3741f},{0.3741f,0.3741f},{0.4582f,0.2645f},{0.2645f,0.4582f}},
          {{0.4553f,0},{0,0.4553f},{0.3220f,0.3220f},{0.3220f,0.3220f},{0.2788f,0.2788f},{0.3943f,0.2277f},{0.2277f,0.3943f}},
          {{0.3886f,0},{0,0.3886f},{0.2748f,0.2748f},{0.2748f,0.2748f},{0.3366f,0.1943f},{0.1943f,0.3366f},{0.3366f,0.1943f},{0.1943f,0.3366f}},
      };
      ...
      static const int16_t dmatrix[6][8][2]= {
          {{9598, 0},{0,   9598},{6786,6786}},
          {{6925, 0},{0,   6925},{5997,3462},{3462,5997}},
          {{10663,0},{0,  10663},{7540,7540},{9234,5331},{5331,9234}},
          {{8668, 0},{0,   8668},{6129,6129},{6129,6129},{7507,4335},{4335,7507}},
          {{7459, 0},{0,   7459},{5275,5275},{5275,5275},{4568,4568},{6460,3731},{3731,6460}},
          {{6368, 0},{0,   6368},{4502,4502},{4502,4502},{5514,3184},{3184,5514},{5514,3184},{3184,5514}}
      };
...
AudioConverter::UpmixAudio(void* aOut, const void* aIn, size_t aFrames) const
{
  ...
  if (mIn.Format() == AudioConfig::FORMAT_FLT) {
    ...
    int16_t* out = static_cast<int16_t*>(aOut);
    for (size_t fIdx = 0; fIdx < aFrames; ++fIdx) {
      int16_t sample = ((int32_t)in[fIdx] * 11585) >> 14; 
