...
def make_googlesource_url(target, commit):
    if target == 'libwebrtc':
        ...
        return ('https://chromium.googlesource.com/chromium/src/build/+archive/' + commit +
                '.tar.gz')
...
def fetch(target, url):
    ...
    if req.status_code == 200:
        ...
        print('Hit status code {} fetching commit. Aborting.'.format(req.status_code),
              file=sys.stderr)
        sys.exit(1)
    with open(os.path.join(LIBWEBRTC_DIR, 'README.mozilla'), 'w') as f:
        f.write('libwebrtc updated from commit {} on {}.'.format(
...
def fetch_local(target, path, commit):
    ...
    if cp.returncode != 0:
        print('Hit return code {} fetching commit. Aborting.'.format(cp.returncode),
              file=sys.stderr)
        sys.exit(1)
    with open(os.path.join(LIBWEBRTC_DIR, 'README.mozilla'), 'w') as f:
        f.write('libwebrtc updated from {} commit {} on {}.'.format(
...
def unpack(target):
    ...
    if target == 'libwebrtc':
        ...
        if os.path.exists(os.path.join(target_path, LIBWEBRTC_USED_IN_FIREFOX[0])):
            for path in LIBWEBRTC_USED_IN_FIREFOX:
                shutil.move(os.path.join(target_path, path),
