                                                       ...
                                                       uint32_t aChannels)
{
  ...
  while (mAudioOutputObserver->Size() > 0) {
    ...
    nsAutoPtr<FarEndAudioChunk> buffer(mAudioOutputObserver->Pop()); 
    if (!buffer) {
      continue;
    }
                                                 ...
                                                 uint32_t aChannels)
{
  ...
  if (MOZ_LOG_TEST(AudioLogModule(), LogLevel::Debug)) {
    ...
    if (mTotalFrames > mLastLogFrames + mSampleFrequency) { 
