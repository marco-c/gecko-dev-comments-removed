...
MediaDevice::GetMediaSource(nsAString& aMediaSource)
{
  ...
  } else if (mMediaSource == MediaSourceEnum::Window) { 
  ...
  Run() override
  {
    ...
    if (mAudioDevice &&
      ...
      stream = msg->CreateSourceStream(
        globalWindow->AbstractMainThreadFor(dom::TaskCategory::Other)); 
                                  ...
                                  bool aFake)
{
  ...
  MediaManager::PostTask(NewTaskFrom([id, aWindowId, audioLoopDev,
    ...
    NS_DispatchToMainThread(NewRunnableFrom([id, handoff]() mutable {
      UniquePtr<SourceSet> result(handoff); 
      RefPtr<MediaManager> mgr = MediaManager_GetInstance();
      if (!mgr) {
        return NS_OK;
      }
    ...
    p2->Then([this, onSuccess, onFailure, windowID, c,
      ...
      if (!askPermission) {
        for (auto& device : **devices) {
          nsresult rv = devicesCopy->AppendElement(device,  false);
          if (NS_WARN_IF(NS_FAILED(rv))) {
            return;
          }
        }
...
MediaManager::GetActiveMediaCaptureWindows(nsIArray** aArray)
{
  ...
  for (auto iter = mActiveWindows.Iter(); !iter.Done(); iter.Next()) {
    ...
    if (capturing) {
      array->AppendElement(window,  false);
    }
  }
