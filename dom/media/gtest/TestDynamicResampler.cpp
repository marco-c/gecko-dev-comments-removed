...
TEST(TestAudioChunkList, UpdateBetweenMonoAndStereo)
{
  ...
  EXPECT_EQ(
      (const int)static_cast<SharedChannelArrayBuffer<float>*>(c3.mBuffer.get())
          ->mBuffers[0]
          .Length(),
      list.ChunkCapacity());
  ...
  EXPECT_EQ(
      (const int)static_cast<SharedChannelArrayBuffer<float>*>(c5.mBuffer.get())
          ->mBuffers[0]
          .Length(),
      list.ChunkCapacity());
  EXPECT_EQ(
  ...
  EXPECT_EQ(
      (const int)static_cast<SharedChannelArrayBuffer<float>*>(c7.mBuffer.get())
          ->mBuffers[0]
          .Length(),
      list.ChunkCapacity());
}
