...
#define TRY(x)                                             \
  ...
  const uint32_t kESDescriptorHeaderSize = 3;        
  const uint32_t kDecoderConfigDescrTagSize = 5;     
  const uint32_t kDecoderConfigDescrFixedSize = 13;  
  const uint32_t kDecoderSpecificInfoTagSize = 5;    
  ...
  TRY(writer.WriteU32(0x00000000));  
  TRY(writer.WriteU32(0x00000000));  
...
static OSStatus _PassthroughInputDataCallback(
    AudioConverterRef aAudioConverter, UInt32* aNumDataPackets ,
...
MediaResult AppleATDecoder::DecodeSample(MediaRawData* aSample) {
  ...
  do {
    ...
    OSStatus rv = AudioConverterFillComplexBuffer(
        ...
        &numFrames , &decBuffer, packets.get());
...
nsresult AppleATDecoder::GetImplicitAACMagicCookie(MediaRawData* aSample) {
  ...
  OSStatus status = AudioFileStreamParseBytes(
      mStream, adtssample->Size(), adtssample->Data(), 0 );
  if (status) {
