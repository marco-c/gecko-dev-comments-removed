...
class DrawCall {
  ...
  public vertexCountInIndexBuffer: number; 
  ...
  .params(
    u =>
      u
        ...
        .unless(p => p.drawCallTestParameter === 'instanceCount' && p.errorScale > 10 ** 4) 
  )
  .fn(t => {
    const p = t.params;
    ...
    const bufferCount = p.additionalBuffers + 2; 
    ...
    const draw = new DrawCall({
      ...
      keepInstanceStepModeBufferInRange: p.indexed && !p.indirect, 
    });
