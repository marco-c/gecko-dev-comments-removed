...
const kValidPixelColor = new Uint8Array([0x00, 0xff, 0x00, 0xff]); 
const kInvalidPixelColor = new Uint8Array([0x00, 0x00, 0x00, 0x00]); 
  ...
  run({
    ...
    if (primitiveRestart) {
      ...
      if (indirect) {
        ...
        renderPass.drawIndexed(drawCount + 1); 
      }
    } else {
      if (indirect) {
