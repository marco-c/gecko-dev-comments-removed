...
struct DeviceLostRequest {
 ...
 void WebGPUParent::DeviceLostCallback(uint8_t* aUserData,
                                                   uint8_t aReason,
                                                   const char* aMessage) {
...
static const char* MapStatusString(ffi::WGPUBufferMapAsyncStatus status) {
  switch (status) {
    ...
    case ffi::WGPUBufferMapAsyncStatus_Error:
      return "Map failed";
    case ffi::WGPUBufferMapAsyncStatus_Sentinel:  
      break;
  }
...
ipc::IPCResult WebGPUParent::RecvDeviceCreateSwapChain(
  ...
  mRemoteTextureOwner->RegisterTextureOwner(aOwnerId,  true);
