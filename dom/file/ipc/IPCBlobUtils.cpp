...
template<typename M>
nsresult
SerializeInputStreamParent(nsIInputStream* aInputStream, IPCBlob& aIPCBlob,
                           M* aManager)
{
  ...
  IPCBlobInputStreamParent* parentActor =
    IPCBlobInputStreamParent::Create(aInputStream, &rv, aManager);
  if (!parentActor) {
    return rv;
  }
...
nsresult
SerializeInputStream(nsIInputStream* aInputStream, IPCBlob& aIPCBlob,
                     nsIContentParent* aManager)
{
  return SerializeInputStreamParent(aInputStream, aIPCBlob, aManager);
}
...
SerializeInternal(BlobImpl* aBlobImpl, M* aManager, IPCBlob& aIPCBlob)
{
  ...
  rv = SerializeInputStream(inputStream, aIPCBlob, aManager);
  if (NS_WARN_IF(rv.Failed())) {
    return rv.StealNSResult();
  }
