...
InternalResponse::InternalResponse(uint16_t aStatus,
 ...
 SafeRefPtr<InternalResponse> InternalResponse::FromIPC(
...
template <typename T>
 SafeRefPtr<InternalResponse> InternalResponse::FromIPCTemplate(
  ...
  *aIPCResponse = ChildToParentInternalResponse(GetMetadata(), Nothing(),
  ...
  if (body) {
    ...
    DebugOnly<bool> ok = mozilla::ipc::SerializeIPCStream(
        body.forget(), aIPCResponse->body()->stream(),  false);
    MOZ_ASSERT(ok);
  }
