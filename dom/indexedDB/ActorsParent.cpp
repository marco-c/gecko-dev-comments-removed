...
public:
  DeleteDatabaseOp(Factory* aFactory,
                   already_AddRefed<ContentParent> aContentParent,
                   const CommonFactoryRequestParams& aParams)
    : FactoryOp(aFactory, std::move(aContentParent), aParams,  true)
    , mPreviousVersion(0)
  { }
...
DatabaseConnection::EnableQuotaChecks()
{
  ...
  DebugOnly<bool> result =
    journalQuotaObject->MaybeUpdateSize(journalFileSize,  true);
  MOZ_ASSERT(result);
...
TransactionBase::AllocRequest(const RequestParams& aParams, bool aTrustParams)
{
  ...
  switch (aParams.type()) {
    ...
    case RequestParams::TObjectStoreGetAllKeysParams:
      actor =
        new ObjectStoreGetKeyRequestOp(this, aParams,  true);
      break;
...
OpenDatabaseOp::OpenDatabaseOp(Factory* aFactory,
                               already_AddRefed<ContentParent> aContentParent,
                               const CommonFactoryRequestParams& aParams)
  : FactoryOp(aFactory, std::move(aContentParent), aParams,  false)
