...
TEST_P(OcclusionQueriesTest, Errors)
{
    ...
    glBeginQueryEXT(GL_ANY_SAMPLES_PASSED_EXT, 0); 
    EXPECT_GL_ERROR(GL_INVALID_OPERATION);

    glBeginQueryEXT(GL_ANY_SAMPLES_PASSED_EXT, query);
    glBeginQueryEXT(GL_ANY_SAMPLES_PASSED_CONSERVATIVE_EXT, query2); 
    EXPECT_GL_ERROR(GL_INVALID_OPERATION);
    ...
    drawQuad(mProgram, "position", 0.8f); 
    glEndQueryEXT(GL_ANY_SAMPLES_PASSED_CONSERVATIVE_EXT); 
    EXPECT_GL_ERROR(GL_INVALID_OPERATION);
    glEndQueryEXT(GL_ANY_SAMPLES_PASSED_EXT);

    glBeginQueryEXT(GL_ANY_SAMPLES_PASSED_CONSERVATIVE_EXT, query); 
    EXPECT_GL_ERROR(GL_INVALID_OPERATION);
