    ...
    for (size_t i = 0; i < ArraySize(mPositionColorBuffer); i++)
    {
        ...
        for (unsigned int j = 0; j < params.numSprites; j++)
        {
            ...
            *reinterpret_cast<float *>(
            ...
            positionColorData[j * mBytesPerSprite + 2 * sizeof(float) + 0] = pointSpriteRed;    
            positionColorData[j * mBytesPerSprite + 2 * sizeof(float) + 1] = pointSpriteGreen;  
            positionColorData[j * mBytesPerSprite + 2 * sizeof(float) + 2] = pointSpriteBlue;   
        }
