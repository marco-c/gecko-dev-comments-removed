...
void ShaderMatrixCase::init (void)
{
	...
	DE_ASSERT(!isInDynMat0 || !isInDynMat1); 
	...
	for (int attribNdx = 0; attribNdx < 4; attribNdx++)
	{
		...
		m_userAttribTransforms[attribNdx](                  0, 3) = 0.2f;								
...
void ShaderMatrixTests::init (void)
{
	static const struct
	{
		...
		const char*		desc;
		MatrixOp		op;
		bool			extendedInputTypeCases; 
		bool			createInputTypeGroup;	
	} ops[] =
	{
			...
			for (int matTypeNdx = 0; matTypeNdx < DE_LENGTH_OF_ARRAY(matrixTypes); matTypeNdx++)
			{
				...
				for (int precNdx = 0; precNdx < DE_LENGTH_OF_ARRAY(precisions); precNdx++)
				{
					...
					if (isOperationArithmeticMatrixMatrix(op))
					{
						...
						{
							ShaderInput otherMatIn(inputType == INPUTTYPE_DYNAMIC ? INPUTTYPE_UNIFORM : inputType, getDataTypeMatrix(otherCols, numCols ), precision);
