...
mozilla::ipc::IPCResult GPUParent::RecvInitCompositorManager(
  ...
  CompositorManagerParent::Create(std::move(aEndpoint),  true);
  return IPC_OK();
}
...
mozilla::ipc::IPCResult GPUParent::RecvNewContentCompositorManager(
    Endpoint<PCompositorManagerParent>&& aEndpoint) {
  CompositorManagerParent::Create(std::move(aEndpoint),  false);
  return IPC_OK();
}
...
mozilla::ipc::IPCResult GPUParent::RecvNewContentRemoteDecoderManager(
  ...
  if (!RemoteDecoderManagerParent::CreateForContent(std::move(aEndpoint),  true)) {
    return IPC_FAIL_NO_REASON(this);
  }
  return IPC_OK();
}
