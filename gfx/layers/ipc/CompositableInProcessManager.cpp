...
Atomic<uint64_t> CompositableInProcessManager::sNextHandle(1);
 ...
 void CompositableInProcessManager::Initialize(
    uint32_t aNamespace) {
  MOZ_ASSERT(NS_IsMainThread());
  sNamespace = aNamespace;
}

 void CompositableInProcessManager::Shutdown() {
  MOZ_ASSERT(NS_IsMainThread());
  StaticMutexAutoLock lock(sMutex);
  sCompositables.clear();
}

 RefPtr<WebRenderImageHost> CompositableInProcessManager::Add(
 ...
 RefPtr<WebRenderImageHost> CompositableInProcessManager::Find(
    const CompositableHandle& aHandle, base::ProcessId aForPid) {
  StaticMutexAutoLock lock(sMutex);
 ...
 void CompositableInProcessManager::Release(
    const CompositableHandle& aHandle, base::ProcessId aForPid) {
  StaticMutexAutoLock lock(sMutex);
