





#ifndef include_gfx_ipc_UiCompositorControllerMessageTypes_h
#define include_gfx_ipc_UiCompositorControllerMessageTypes_h

namespace mozilla {
namespace layers {









enum UiCompositorControllerMessageTypes {
  STATIC_TOOLBAR_NEEDS_UPDATE      = 0,  
  STATIC_TOOLBAR_READY             = 1,  
  TOOLBAR_HIDDEN                   = 2,  
  TOOLBAR_VISIBLE                  = 3,  
  TOOLBAR_SHOW                     = 4,  
  FIRST_PAINT                      = 5,  
  REQUEST_SHOW_TOOLBAR_IMMEDIATELY = 6,  
  REQUEST_SHOW_TOOLBAR_ANIMATED    = 7,  
  REQUEST_HIDE_TOOLBAR_IMMEDIATELY = 8,  
  REQUEST_HIDE_TOOLBAR_ANIMATED    = 9,  
  LAYERS_UPDATED                   = 10, 
  TOOLBAR_SNAPSHOT_FAILED          = 11, 
  COMPOSITOR_CONTROLLER_OPEN       = 20, 
  IS_COMPOSITOR_CONTROLLER_OPEN    = 21  
};


}  
}  

#endif  
