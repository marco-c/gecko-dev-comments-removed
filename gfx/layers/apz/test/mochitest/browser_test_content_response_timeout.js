...
add_task(async () => {
  ...
  await TestUtils.waitForCondition(async () => {
    const backgroundColor = await SpecialPowers.spawn(
      tab.linkedBrowser,
      [],
      () => {
        const target = content.document.querySelector("#target");
        return content.window.getComputedStyle(target).backgroundColor;
      }
    );
    return backgroundColor == "rgb(0, 255, 0)";
  }, "The background color of the element has been changed");
  ok(true, "We got at leasts one scroll event");
