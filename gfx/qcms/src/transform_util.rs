...
pub unsafe extern "C" fn lut_interp_linear16(
    ...
    let mut value: u32 = (input_value as i32 * (length - 1)) as u32; 
    let mut upper: u32 = (value + 65534) / 65535; 
    let mut lower: u32 = value / 65535;
    
    let mut interp: u32 = value % 65535; 
...
unsafe extern "C" fn lut_interp_linear_precache_output(
    ...
    value = *table.offset(upper as isize) as libc::c_uint * interp
        + *table.offset(lower as isize) as libc::c_uint * ((8192 - 1) as libc::c_uint - interp); 
                                                                                                 
    value = value + ((8192 - 1) * 65535 / 255 / 2) as libc::c_uint; 
