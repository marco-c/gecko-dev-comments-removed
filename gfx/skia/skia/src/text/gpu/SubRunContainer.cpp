...
public:
    ...
    void fillVertexData(DrawWriter* dw,
        ...
        for (auto [glyph, leftTop]: quadData()) {
            ...
            verts.append(6) << SkPoint{devOut[0].x, devOut[0].y} << depth << AtlasPt{al, at}  
                            << ssboIndex
                            << SkPoint{devOut[3].x, devOut[3].y} << depth << AtlasPt{al, ab}  
                            << ssboIndex
                            << SkPoint{devOut[1].x, devOut[1].y} << depth << AtlasPt{ar, at}  
                            << ssboIndex
                            << SkPoint{devOut[3].x, devOut[3].y} << depth << AtlasPt{al, ab}  
                            << ssboIndex
                            << SkPoint{devOut[2].x, devOut[2].y} << depth << AtlasPt{ar, ab}  
                            << ssboIndex
                            << SkPoint{devOut[1].x, devOut[1].y} << depth << AtlasPt{ar, at}  
                            << ssboIndex;
        }
...
private:
    ...
    void fillDirectNoClipping(SkZip<Mask2DVertex[4], const Glyph*, const SkPoint> quadData,
        ...
        for (auto[quad, glyph, leftTop] : quadData) {
            ...
            quad[0] = {{dl, dt}, color, {al, at}};  
            quad[1] = {{dl, db}, color, {al, ab}};  
            quad[2] = {{dr, dt}, color, {ar, at}};  
            quad[3] = {{dr, db}, color, {ar, ab}};  
        }
    }
    ...
    void fill2D(SkZip<Quad, const Glyph*, const VertexData> quadData,
        ...
        for (auto [quad, glyph, leftTop] : quadData) {
            ...
            auto [al, at, ar, ab] = glyph->fAtlasLocator.getUVs();
            quad[0] = {lt, color, {al, at}};  
            quad[1] = {lb, color, {al, ab}};  
            quad[2] = {rt, color, {ar, at}};  
            quad[3] = {rb, color, {ar, ab}};  
        }
    }
