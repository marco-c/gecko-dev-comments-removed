...
static inline void add_coverage_delta_segment(int y, SkFixed rowHeight, const SkAnalyticEdge* edge,
    ...
    SkFixed fixedR  = SkIntToFixed(R);
    SkFixed first   = SK_Fixed1 + fixedL - l; 
    SkFixed last    = r - (fixedR - SK_Fixed1); 
    SkFixed firstH  = SkFixedMul_lowprec(first, dY); 

    SkFixed alpha0  = SkFixedMul_lowprec(first, firstH) >> 1;   
    SkFixed alpha1  = firstH + (dY >> 1);                       
    deltas->addDelta(L, y, alpha0 * sign);
    deltas->addDelta(L + 1, y, (alpha1 - alpha0) * sign);
    for(int i = 2; i < len - 1; ++i) {
        deltas->addDelta(L + i, y, dY * sign); 
    }

    SkFixed alphaR2     = alpha1 + dY * (len - 3);                      
    SkFixed lastAlpha   = rowHeight - partialTriangleToAlpha(last, dY); 
...
void gen_alpha_deltas(const SkPath& path, const SkIRect& clippedIR, const SkIRect& clipBounds,
    ...
    for(int index = 0; index < count; ++index) {
        ...
        do {
            ...
            if (lowerCeil <= upperFloor + SK_Fixed1) { 
...
void SkScan::DAAFillPath(const SkPath& path, SkBlitter* blitter, const SkIRect& ir,
    ...
    constexpr int STACK_SIZE = 12 << 10; 
#else
    constexpr int STACK_SIZE = 64 << 10; 
#endif
    SkSTArenaAlloc<STACK_SIZE> stackAlloc; 
