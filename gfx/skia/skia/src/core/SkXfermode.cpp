...
static Sk4f softlight_4f(const Sk4f& s, const Sk4f& d) {
    ...
    Sk4f darkSrc = d * (sa + (s2 - sa) * (Sk4f(1) - m));            
    Sk4f darkDst = (m4 * m4 + m4) * (m - Sk4f(1)) + Sk4f(7) * m;    
    Sk4f liteDst = m.sqrt() - m;                                    
    Sk4f liteSrc = d * sa + da * (s2 - sa) * (Sk4f(4) * d <= da).thenElse(darkDst,
                                                                          liteDst); 

    return color_alpha(s * ida + d * isa + (s2 <= sa).thenElse(darkSrc, liteSrc), 
                       s + d * isa);
}
...
bool SkBlendMode_AppendStages(SkBlendMode mode, SkRasterPipeline* p) {
    ...
    switch (mode) {
        ...
        case SkBlendMode::kSrc:      return true;  
