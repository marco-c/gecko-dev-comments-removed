...
namespace skvm {
    ...
    struct Program::Impl {
        ...
        std::atomic<void*> jit_entry{nullptr};   
        size_t jit_size = 0;
        void*  dylib    = nullptr;
    };
    ...
    Program Builder::done(const char* debug_name,
        ...
        return this->done(debug_name, allow_jit, nullptr);
    }
    ...
    F32 Builder::add(F32 x, F32 y) {
        ...
        if (this->isImm(y.id, 0.0f)) { return x; }   
    ...
    F32 Builder::mul(F32 x, F32 y) {
        if (float X,Y; this->allImm(x.id,&X, y.id,&Y)) { return splat(X*Y); }
        this->canonicalizeIdOrder(x, y);
        if (this->isImm(y.id, 1.0f)) { return x; }  
    ...
    F32 Builder::approx_pow2(F32 x) {
        ...
        F32 approx = add(x,         121.274057500f);
            ...
            approx = clamp(approx, 0, kInfinityBits);  
    ...
    F32 Builder::approx_powf(F32 x, F32 y) {
        ...
        if (this->isImm(x.id, 1.0f)) { return x; }                    
        if (this->isImm(x.id, 2.0f)) { return this->approx_pow2(y); } 
        if (this->isImm(y.id, 0.5f)) { return this->sqrt(x); }        
        if (this->isImm(y.id, 1.0f)) { return x; }                    
        if (this->isImm(y.id, 2.0f)) { return x * x; }                
    ...
    I32 Builder::bit_and(I32 x, I32 y) {
        ...
        if (this->isImm(y.id, 0)) { return splat(0); }         
        if (this->isImm(y.id,~0)) { return x; }                
        if (Val notX = this->holdsBitNot(x.id); notX != NA) {  
            return bit_clear(y, {this, notX});
        }
        if (Val notY = this->holdsBitNot(y.id); notY != NA) {  
    ...
    I32 Builder::select(I32 x, I32 y, I32 z) {
        ...
        if (this->isImm(x.id, 0)) { return z; }                
        if (this->isImm(y.id, 0)) { return bit_clear(z,x); }   
        if (this->isImm(z.id, 0)) { return bit_and  (y,x); }   
        if (Val notX = this->holdsBitNot(x.id); notX != NA) {  
            x.id = notX;
            std::swap(y, z);
        }
    ...
    PixelFormat SkColorType_to_PixelFormat(SkColorType ct) {
        ...
        switch (ct) {
            ...
            case kGray_8_SkColorType:   return {UNORM, 8,8,8,0, 0,0,0,0};  
            case kR8_unorm_SkColorType: return {UNORM, 8,0,0,0, 0,0,0,0};

            case kRGB_565_SkColorType:   return {UNORM, 5,6,5,0, 11,5,0,0};  
            case kARGB_4444_SkColorType: return {UNORM, 4,4,4,4, 12,8,4,0};  

            case kRGBA_8888_SkColorType:  return {UNORM, 8,8,8,8,  0,8,16,24};
            case kRGB_888x_SkColorType:   return {UNORM, 8,8,8,0,  0,8,16,32};  
    ...
    bool Program::jit(const std::vector<OptimizedInstruction>& instructions,
        ...
        SkTHashMap<int, A::Label> constants;    
        A::Label                  iota;         
        A::Label                  load64_index; 
        ...
        *registers_used = 0;  
        ...
        auto emit = [&](Val id, bool scalar) {
            ...
            auto alloc_tmp = [&](int N=1) -> Reg {
                auto needs_spill = [&](Val v) -> bool {
                    SkASSERT(v >= 0);   
                    return stack_slot[v] == NA               
                        && instructions[v].op != Op::splat;  
                };
            ...
            switch (op) {
                ...
                case Op::min_f32: a->vminps(dst(y), r(y), any(x)); break;  
                case Op::max_f32: a->vmaxps(dst(y), r(y), any(x)); break;  
                ...
                case Op::bit_clear: a->vpandn(dst(y), r(y), any(x)); break; 
                ...
                case Op::to_fp16:
                    a->vcvtps2ph(dst(x), r(x), A::CURRENT);  
                    a->vpmovzxwd(dst(), dst());              
                    break;
        ...
        for (Val id = 0; id < (Val)instructions.size(); id++) {
            ...
            if (instructions[id].can_hoist && !emit(id, false)) {
                return false;
            }
        }
        ...
        a->label(&tail);
        {
            ...
            for (Val id = 0; id < (Val)instructions.size(); id++) {
                ...
                if (!instructions[id].can_hoist && !emit(id, true)) {
                    return false;
                }
            }
            restore_incoming_regs();
    ...
    void Program::setupJIT(const std::vector<OptimizedInstruction>& instructions,
        ...
        Assembler a{nullptr};
        int stack_hint = -1;
        uint32_t registers_used = 0xffff'ffff;  
