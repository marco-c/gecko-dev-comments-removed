...
namespace skvm {
    ...
    void Assembler::op(int prefix, int map, int opcode, Ymm dst, Ymm x, Ymm y, bool W) {
    ...
    bool Program::jit(const std::vector<Builder::Instruction>& instructions,
        ...
        auto emit = [&](Val id, bool scalar) {
            ...
            switch (op) {
                ...
                case Op::bit_clear: a->vpandn(dst(), r[y], r[x]); break;  
    ...
    void Program::dumpJIT(const char* debug_name, size_t size) const {
        ...
        struct CodeLoad {
            ...
            0, (uint32_t)(sizeof(CodeLoad) + strlen(name) + 1 + size),
            timestamp_ns(),
