...
sk_sp<SkPicture> SkPicture::Forwardport(const SkPictInfo& info, const SkPictureData* data) {
    ...
    SkPictureRecorder r;
    playback.draw(r.beginRecording(info.fCullRect), nullptr);
...
SkPictureData* SkPicture::backport() const {
    ...
    SkPictureRecord rec(SkISize::Make(info.fCullRect.width(), info.fCullRect.height()), 0);
    rec.beginRecording();
        this->playback(&rec);
    rec.endRecording();
    return new SkPictureData(rec, info, false );
}
