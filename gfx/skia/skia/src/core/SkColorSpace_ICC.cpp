...
static bool load_matrix(SkMatrix44* matrix, const uint8_t* src, size_t len, bool translate,
    ...
    if (translate) {
        ...
        array[ 7] = encodingFactor * SkFixedToFloat(read_big_endian_i32(src + 40)); 
        array[11] = encodingFactor * SkFixedToFloat(read_big_endian_i32(src + 44)); 
    } else {
        array[ 3] = 0.0f;
        array[ 7] = 0.0f;
        array[11] = 0.0f;
...
static bool load_a2b0(std::vector<SkColorSpace_A2B::Element>* elements, const uint8_t* src,
    ...
    static constexpr int kPCSChannels = 3; 
    if (elements->empty()) {
