...
public:
    ...
    AI static void Load4(const void* ptr, SkNx* r, SkNx* g, SkNx* b, SkNx* a) {
        ...
        __m128i even = _mm_unpacklo_epi16(lo, hi),   
                 odd = _mm_unpackhi_epi16(lo, hi);   
        __m128i rg = _mm_unpacklo_epi16(even, odd),  
                ba = _mm_unpackhi_epi16(even, odd);  
        *r = rg;
...
public:
    ...
    AI static void Load4(const void* ptr, SkNx* r, SkNx* g, SkNx* b, SkNx* a) {
        ...
        __m128i _02 = _mm_unpacklo_epi16(_01, _23),  
                _13 = _mm_unpackhi_epi16(_01, _23),  
                _46 = _mm_unpacklo_epi16(_45, _67),
                _57 = _mm_unpackhi_epi16(_45, _67);

        __m128i rg0123 = _mm_unpacklo_epi16(_02, _13),  
    ...
    AI static void Store4(void* ptr, const SkNx& r, const SkNx& g, const SkNx& b, const SkNx& a) {
        __m128i rg0123 = _mm_unpacklo_epi16(r.fVec, g.fVec),  
                rg4567 = _mm_unpackhi_epi16(r.fVec, g.fVec),  
...
template<> AI  Sk8h SkNx_cast<uint16_t, int32_t>(const Sk8i& src) {
#if SK_CPU_SSE_LEVEL >= SK_CPU_SSE_LEVEL_SSE41
...
template<> AI  Sk8h SkNx_cast<uint16_t, float>(const Sk8f& src) {
    return SkNx_cast<uint16_t>(SkNx_cast<int32_t>(src));
}
...
template<> AI  Sk4u SkNx_cast<uint32_t, uint16_t>(const Sk4h& src) {
    return _mm_unpacklo_epi16(src.fVec, _mm_setzero_si128());
}
...
template<> AI  Sk4u SkNx_cast<uint32_t, int32_t>(const Sk4i& src) {
    return src.fVec;
}

template<> AI  Sk4h SkNx_cast<uint16_t, uint32_t>(const Sk4u& src) {
    return SkNx_cast<uint16_t>(SkNx_cast<int32_t>(src));
}

template<> AI  Sk8h SkNx_cast<uint16_t, uint32_t>(const Sk8u& src) {
    return SkNx_cast<uint16_t>(SkNx_cast<int32_t>(src));
}

template<> AI  Sk8u SkNx_cast<uint32_t, uint16_t>(const Sk8h& src) {
