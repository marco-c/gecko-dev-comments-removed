...
struct SkPx {
    ...
    static SkPx Load(const uint32_t* px, int n) {
        SkASSERT(0 < n && n < 8);
        uint8x8x4_t v = vld4_dup_u8((const uint8_t*)px);  
        switch (n) {
            case 7: v = vld4_lane_u8((const uint8_t*)(px+6), v, 6);  
            case 6: v = vld4_lane_u8((const uint8_t*)(px+5), v, 5);  
            case 5: v = vld4_lane_u8((const uint8_t*)(px+4), v, 4);  
            case 4: v = vld4_lane_u8((const uint8_t*)(px+3), v, 3);  
            case 3: v = vld4_lane_u8((const uint8_t*)(px+2), v, 2);  
    ...
    struct Alpha {
        ...
        static Alpha Load(const uint8_t* a, int n) {
            SkASSERT(0 < n && n < 8);
            uint8x8_t v = vld1_dup_u8(a);  
            switch (n) {
                case 7: v = vld1_lane_u8(a+6, v, 6);  
                case 6: v = vld1_lane_u8(a+5, v, 5);  
                case 5: v = vld1_lane_u8(a+4, v, 4);  
                case 4: v = vld1_lane_u8(a+3, v, 3);  
                case 3: v = vld1_lane_u8(a+2, v, 2);  
