...
namespace SK_OPTS_NS {
    ...
    SI void store4(float* ptr, size_t tail, F r, F g, F b, F a) {
        ...
        F _04 = _mm256_unpacklo_pd(rg0145, ba0145),  
          _15 = _mm256_unpackhi_pd(rg0145, ba0145),  
          _26 = _mm256_unpacklo_pd(rg2367, ba2367),  
          _37 = _mm256_unpackhi_pd(rg2367, ba2367);  
...
SI F clamp(F v, F limit) {
    F inclusive = bit_cast<F>( bit_cast<U32>(limit) - 1 );  
...
STAGE(xy_to_unit_angle, Ctx::None) {
    ...
    phi = if_then_else(isnan_(phi), 0              , phi);  
    r = phi;
}
