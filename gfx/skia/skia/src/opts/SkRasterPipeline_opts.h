...
namespace SK_OPTS_NS {
    ...
    SI void load3(const uint16_t* ptr, size_t tail, U16* r, U16* g, U16* b) {
        ...
        auto _02 = _mm_unpacklo_epi16(_0, _2),  
        ...
        auto rg0123 = _mm_unpacklo_epi16(_02, _13),  
             bx0123 = _mm_unpackhi_epi16(_02, _13),  
    ...
    SI void store4(uint16_t* ptr, size_t tail, U16 r, U16 g, U16 b, U16 a) {
        auto rg0123 = _mm_unpacklo_epi16(r, g),  
             rg4567 = _mm_unpackhi_epi16(r, g),  
    ...
    SI void store4(float* ptr, size_t tail, F r, F g, F b, F a) {
        ...
        F _04 = _mm256_castpd_ps(_mm256_unpacklo_pd(_mm256_castps_pd(rg0145), _mm256_castps_pd(ba0145))),  
          _15 = _mm256_castpd_ps(_mm256_unpackhi_pd(_mm256_castps_pd(rg0145), _mm256_castps_pd(ba0145))),  
          _26 = _mm256_castpd_ps(_mm256_unpacklo_pd(_mm256_castps_pd(rg2367), _mm256_castps_pd(ba2367))),  
          _37 = _mm256_castpd_ps(_mm256_unpackhi_pd(_mm256_castps_pd(rg2367), _mm256_castps_pd(ba2367)));  
...
template <typename T> using V = SK_VECTORTYPE(T, 4);
    ...
    SI void load2(const uint16_t* ptr, size_t tail, U16* r, U16* g) {
        ...
        if (SK_EXPECT(tail,0)) {
            ...
            if (tail > 1) {
                _01 = _mm_castpd_si128(_mm_loadl_pd(_mm_castsi128_pd(_01), (const double*)ptr)); 
                if (tail > 2) {
                  _01 = _mm_insert_epi16(_01, *(ptr+4), 4);             
                  _01 = _mm_insert_epi16(_01, *(ptr+5), 5);             
                }
            } else {
                _01 = _mm_cvtsi32_si128(*(const uint32_t*)ptr);         
            }
        } else {
            _01 = _mm_loadu_si128(((__m128i*)ptr) + 0);  
        }
    ...
    SI void load3(const uint16_t* ptr, size_t tail, U16* r, U16* g, U16* b) {
        ...
        auto R = _mm_unpacklo_epi16(_02, _13),  
             G = _mm_srli_si128(R, 8),
             B = _mm_unpackhi_epi16(_02, _13);  
    ...
    SI void load4(const uint16_t* ptr, size_t tail, U16* r, U16* g, U16* b, U16* a) {
        ...
        if (SK_EXPECT(tail,0)) {
            ...
            auto src = (const double*)ptr;
            if (  true  ) { _01 = _mm_castpd_si128(_mm_loadl_pd(_mm_castsi128_pd(_01), src + 0)); } 
            if (tail > 1) { _01 = _mm_castpd_si128(_mm_loadh_pd(_mm_castsi128_pd(_01), src + 1)); } 
            if (tail > 2) { _23 = _mm_castpd_si128(_mm_loadl_pd(_mm_castsi128_pd(_23), src + 2)); } 
        } else {
            _01 = _mm_loadu_si128(((__m128i*)ptr) + 0); 
...
SI F from_half(U16 h) {
    ...
    auto denorm = sk_bit_cast<I32>(em) < 0x0400; 
...
SI U16 to_half(F f) {
    ...
    auto denorm = sk_bit_cast<I32>(em) < 0x38800000; 
...
STAGE(xy_to_unit_angle, NoCtx) {
    ...
    phi = if_then_else(phi != phi , F(0)           , phi);  
    r = phi;
}
