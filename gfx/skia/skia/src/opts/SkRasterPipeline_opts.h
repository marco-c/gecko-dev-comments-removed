...
STAGE(from_2dot2) {
    auto from_2dot2 = [](const SkNf& x) {
        ...
        auto x16 = x.rsqrt().rsqrt().rsqrt().rsqrt();   
        auto x64 = x16.rsqrt().rsqrt();                 
...
STAGE_CTX(store_8888, uint32_t**) {
    auto byte = [](const SkNf& x, int ix) {
        ...
        switch (ix) {
            case 0: return SkNi::Load(&v) & 0xff;  
            case 3: return SkNi::Load(&v) << 24;   
        }
        return (SkNi::Load(&v) & 0xff) << (8*ix);  
    };
...
RGB_XFERMODE(softlight) {
    ...
    SkNf darkSrc = d*(sa + (s2 - sa)*(1.0f - m)),     
         darkDst = (m4*m4 + m4)*(m - 1.0f) + 7.0f*m,  
         liteDst = m.rsqrt().invert() - m,            
         liteSrc = d*sa + da*(s2 - sa) * (4.0f*d <= da).thenElse(darkDst, liteDst);  
    return s*inv(da) + d*inv(sa) + (s2 <= sa).thenElse(darkSrc, liteSrc);  
}
...
SI SkNf mirror(const SkNf& v, float l) {
