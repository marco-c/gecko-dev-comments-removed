...
sk_sp<GrRenderTarget> GrDawnGpu::onWrapBackendRenderTarget(const GrBackendRenderTarget& rt) {
    ...
    return GrDawnRenderTarget::MakeWrapped(
            this, dimensions, sampleCnt, info, "DawnGpu_WrapBackendRenderTarget");
}

sk_sp<GrAttachment> GrDawnGpu::makeStencilAttachment(const GrBackendFormat& ,
...
std::unique_ptr<GrSemaphore> GrDawnGpu::wrapBackendSemaphore(const GrBackendSemaphore& ,
                                                             GrSemaphoreWrapType ,
                                                             GrWrapOwnership ) {
    SkASSERT(!"unimplemented");
    return nullptr;
