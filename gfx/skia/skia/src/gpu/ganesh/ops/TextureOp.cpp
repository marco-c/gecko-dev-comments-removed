...
private:
    ...
    struct Metadata {
        ...
        uint16_t  fFilter     : 2; 
        uint16_t  fMipmapMode : 2; 
        uint16_t  fAAType     : 2; 
        uint16_t  fColorType  : 2; 
        uint16_t  fSubset     : 1; 
        uint16_t  fSaturate   : 1; 
        uint16_t  fUnused     : 6; 
    ...
    TextureOpImpl(GrTextureSetEntry set[],
        ...
        GrAAType netAAType = GrAAType::kNone; 
            ...
            if (netFilter != filter || netMM != mm) {
                ...
                if (filter != GrSamplerState::Filter::kNearest) {
                    if (mustFilter) {
                        ...
                        filterForQuad = GrSamplerState::Filter::kNearest; 
                    }
                }
    ...
    void characterize(Desc* desc) const {
        ...
        desc->fVertexSpec = VertexSpec(quadType, colorType, srcQuadType,  true,
                                       subset, overallAAType,  true,
                                       indexBufferOption);
    ...
    if (aaType == GrAAType::kNone || aaType == GrAAType::kMSAA) {
        ...
        while (state.numLeft() > 0) {
            ...
            if (!clumped) {
                ...
                state.createOp(set, state.numLeft(), runningAA); 
            }
        }
    }
}
