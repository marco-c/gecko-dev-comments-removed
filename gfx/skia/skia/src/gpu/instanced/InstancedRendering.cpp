...
InstancedRendering::Batch* InstancedRendering::recordShape(ShapeType type, const SkRect& bounds,
    ...
    if (!viewMatrix.hasPerspective()) {
        ...
        batch->fInfo.fNonSquare =
            fabsf(devBoundsHalfHeight - devBoundsHalfWidth) > 0.5f || 
            fabs(m[0] * m[3] + m[1] * m[4]) > 1e-3f || 
            fabs(m[0] * m[0] + m[1] * m[1] - m[3] * m[3] - m[4] * m[4]) > 1e-2f; 
    } else {
...
void InstancedRendering::Batch::initBatchTracker(const GrXPOverridesForBatch& overrides) {
    ...
    if (!fParams.empty()) {
        SkASSERT(fInstancedRendering->fParams.count() < (int)kParamsIdx_InfoMask); 
