...
void GrCCStrokeGeometry::quadraticTo(Verb leftJoinVerb, const SkPoint P[3], float maxCurvatureT) {
    ...
    if ((tan0.abs() < SK_ScalarNearlyZero).allTrue() ||  
        (tan1.abs() < SK_ScalarNearlyZero).allTrue()) {  
    ...
    if (isCurvatureTooStrong) {
        ...
        if (leftT > 0) {
            ...
            this->quadraticTo(leftJoinVerb, ptsBuffer, 1);
            if (rightT < 1) {
        ...
        if (rightT < 1) {
            ...
            this->quadraticTo(Verb::kInternalRoundJoin, ptsBuffer + 2, 0);
        } else {
...
void GrCCStrokeGeometry::cubicTo(Verb leftJoinVerb, const SkPoint P[4], float maxCurvatureT,
    ...
    if ((tan1.abs() < SK_ScalarNearlyZero).allTrue()) {  
        p2 = p3;
        tan1 = p3 - p1;
        if ((tan1.abs() < SK_ScalarNearlyZero).allTrue() ||  
            (p0 == p1).allTrue()) {  
    ...
    if (isCurvatureTooStrong) {
        ...
        if (leftT > 0) {
            ...
            this->cubicTo(leftJoinVerb, ptsBuffer, 1,
        ...
        if (rightT < 1) {
            ...
            currCubic = ptsBuffer + 3;
            this->cubicTo(Verb::kInternalRoundJoin, currCubic, 0,
