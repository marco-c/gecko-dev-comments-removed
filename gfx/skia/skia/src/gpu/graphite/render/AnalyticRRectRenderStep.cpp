...
static float local_aa_radius(const Transform& t, const SkV2& p) {
    ...
    float dfdu = (devP.w*dxdu - devP.x*dwdu) * invW2; 
    float dfdv = (devP.w*dxdv - devP.x*dwdv) * invW2; 
    float dgdu = (devP.w*dydu - devP.y*dwdu) * invW2; 
    float dgdv = (devP.w*dydv - devP.y*dwdv) * invW2; 
...
static void write_vertex_buffer(VertexWriter writer) {
    ...
    static constexpr float kCenter = 1.f; 

    
    static constexpr float _______ = 0.f;

    static constexpr float kHR2 = 0.5f * SK_FloatSqrt2; 
...
AnalyticRRectRenderStep::AnalyticRRectRenderStep(StaticBufferManager* bufferManager)
        : RenderStep("AnalyticRRectRenderStep",
                             ...
                             {"mat2", VertexAttribType::kFloat3, SkSLType::kFloat3}},
                     {
                             ...
                             {"jacobian", SkSLType::kFloat4}, 
                             
                             
                             {"edgeDistances", SkSLType::kFloat4}, 
    ...
    if (params.isStroke()) {
        ...
        if (params.geometry().isEdgeAAQuad()) {
            ...
            if (is_clockwise(quad)) {
                ...
                vw << skvx::shuffle<2,1,0,3>(edgeSigns)  
                   << skvx::shuffle<1,0,3,2>(quad.xs())  
            ...
            if (shape.isRect() || (shape.isRRect() && shape.rrect().isRect())) {
                ...
                vw <<  skvx::float4(-1.f)
                   <<  skvx::shuffle<0,2,2,0>(ltrb)
                   <<  skvx::shuffle<1,1,3,3>(ltrb);
            } else {
