...
static constexpr size_t kVertexBufferSize = 16 << 10; 
static constexpr size_t kIndexBufferSize =   2 << 10; 
static constexpr size_t kUniformBufferSize = 2 << 10; 
static constexpr size_t kStorageBufferSize = 2 << 10; 




static constexpr size_t kStaticTransferBufferSize = 2 << 10; 
...
std::pair<void*, BindBufferInfo> DrawBufferManager::prepareMappedBindBuffer(BufferInfo* info,
                                                                            size_t requiredBytes) {
    auto bindInfo = this->prepareBindBuffer(info, requiredBytes, true);
    if (!bindInfo) {
        return {nullptr, {}};
    }
...
StaticBufferManager::BufferInfo::BufferInfo(BufferType type, const Caps* caps)
        : fBufferType(type)
        , fAlignment(starting_alignment(type, true, caps))
