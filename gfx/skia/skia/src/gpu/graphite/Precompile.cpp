...
void PaintOptions::createKey(const KeyContext& keyContext,
    ...
    SolidColorShaderBlock::BeginBlock(keyContext, keyBuilder,  nullptr,
                                      {1, 0, 0, 1});
    keyBuilder->endBlock();
    ...
    if (addPrimitiveBlender) {
        PrimitiveBlendModeBlock::BeginBlock(keyContext, keyBuilder,  nullptr,
    ...
    if (fBlenderOptions.empty()) {
        BlendModeBlock::BeginBlock(keyContext, keyBuilder,  nullptr,
                                   SkBlendMode::kSrcOver);
        keyBuilder->endBlock();
    } else {
