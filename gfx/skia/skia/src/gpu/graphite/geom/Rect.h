...
public:
    ...
    AI const float4& vals() const { return fVals; }  
    AI float4& vals() { return fVals; }  
    ...
    AI bool isEmptyNegativeOrNaN() const {
        return !all(fVals.xy() + fVals.zw() < 0);  
                                                   
    }

    AI float2 size() const { return -(fVals.xy() + fVals.zw()); }  

    AI float2 center() const {
        float4 p = fVals * float4(.5f, .5f, -.5f, -.5f);  
        return p.xy() + p.zw();  
    }

    AI float area() const {
        float2 negativeSize = fVals.xy() + fVals.zw();  
...
private:
    ...
    AI Rect(float4 vals) : fVals(vals) {}  
