...
private:
    ...
    using ColorDepthOrderField = Bitfield<16, 48>; 
    using StencilIndexField    = Bitfield<16, 32>; 
    using RenderStepField      = Bitfield<2,  30>; 
    using PipelineField        = Bitfield<30, 0>;  
    uint64_t fPipelineKey;

    
    
    
    using GeometryUniformField = Bitfield<22, 42>; 
...
std::unique_ptr<DrawPass> DrawPass::Make(Recorder* recorder,
    ...
    UniformSsboTracker geometrySsboTracker(false);
