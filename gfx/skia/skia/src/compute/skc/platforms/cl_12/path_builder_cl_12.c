...
union skc_ringdex_expand
{
  ...
  struct {
#ifndef SKC_DIV_REM_BEFORE_QUOT 
    skc_uint subbuf;
    skc_uint block;
#else
    skc_uint block;
...
struct skc_path_builder_impl
{
  ...
  struct {
    cl_mem                        buffer;   
    struct skc_subbuffer_blocks * subbufs;  
  } blocks;

  struct {
    cl_mem                        buffer;   
    struct skc_subbuffer_cmds   * subbufs;  
  } cmds;

  struct {
    struct skc_release_record   * records;  
    skc_path_t                  * paths;    
  } release;
  ...
  struct {
    struct skc_path_head        * head;     
    struct skc_path_node        * node;     

    struct {
      skc_uint                    rolling;  
      union skc_tagged_block_id * next;     
      skc_uint                    rem;      
    } ids;
...
skc_path_builder_pfn_begin(struct skc_path_builder_impl * const impl)
{
  ...
  impl->wip.ids.next      = impl->wip.head->tag_ids; 
  impl->wip.ids.rem       = impl->runtime->config->block.words - SKC_PATH_HEAD_WORDS; 
...
{
  impl->wip.subblocks.rem     = impl->runtime->config->block.subblocks; 
...
skc_path_builder_pfn_end(struct skc_path_builder_impl * const impl, skc_path_t * const path)
{
  ...
  *path = skc_runtime_handle_device_acquire(impl->runtime); 
                              ...
                              struct skc_path_builder * * const path_builder)
{
  ...
  impl->blocks.buffer   = clCreateBuffer(runtime->cl.context,
                                         ...
                                         config->paths_copy.block.buffer, 
