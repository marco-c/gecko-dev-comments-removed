...
typedef skc_uchar            skc_grid_id_t;  
...
struct skc_grid
{
  ...
  struct skc_grid_pfn_name  waiting; 
  struct skc_grid_pfn_name  execute; 
  struct skc_grid_pfn_name  dispose; 

  struct {
    skc_uint                words[SKC_GRID_SIZE_WORDS]; 
    skc_uint                count;
  } before;
...
struct skc_grid_deps
{
  ...
  struct skc_grid        grids [SKC_GRID_SIZE_IDS];   
  skc_uint               active[SKC_GRID_SIZE_WORDS]; 
...
skc_grid_deps_attach(skc_grid_deps_t const deps,
                     skc_grid_t    * const addr,
                     void          * const data,
                     skc_grid_pfn          waiting_pfn,  
                     skc_grid_pfn          execute_pfn,  
                     skc_grid_pfn          dispose_pfn,  
...
skc_grid_words_set(skc_uint ids[SKC_GRID_SIZE_WORDS], skc_uint const id)
{
  ...
  skc_uint   const post = pre | (0x80000000 >> (id & 0x1F)); 
...
skc_grid_detach(skc_grid_t const grid)
{
  ...
  if (skc_grid_words_set(grid->deps->active,grid->id)) 
    grid->deps->count -= 1;
}
