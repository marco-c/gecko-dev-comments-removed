...
{
  {
    ...
    uint32_t const bs_rows_even = bs_kpt_mod & ~1; 
...
hsg_merge_levels_init_1(struct hsg_merge * const merge, uint32_t const warps, uint32_t const level, uint32_t const offset)
{
  ...
  uint32_t const l = (warps+1)/2; 
  uint32_t const r = (warps+0)/2; 
  ...
  while (level-- > 0)
    {
      ...
      uint32_t const s_pairs_max = merge->rows_bs/2; 
