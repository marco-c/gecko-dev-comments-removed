  ...
  for (uint32_t count=count_lo; count<=count_hi; count+=count_step)
    {
      ...
      fprintf(stdout,"%s, %u.%u.%u.%u, %s, %s, %s, %8u, %8u, %8u, CPU, %s, %9.2f, %6.2f, GPU, %9u, %7.3f, %7.3f, %7.3f, %6.2f, %6.2f\n",
              ...
              loops,
              timestamp_period * elapsed_ns_sum / 1e6 / loops,               
              timestamp_period * elapsed_ns_min / 1e6,                       
              timestamp_period * elapsed_ns_max / 1e6,                       
              1000.0 * count * loops / (timestamp_period * elapsed_ns_sum),  
              1000.0 * count         / (timestamp_period * elapsed_ns_min)); 
    }
