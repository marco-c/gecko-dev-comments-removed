...
#elif defined(__AVX2__)
    return map(x, +[](__m256 v){ return _mm256_floor_ps(v); });  
#elif defined(__SSE4_1__)
    return map(x, +[](__m128 v){ return    _mm_floor_ps(v); });  
#else
...
SI U32 ix_and_ptr(T** ptr, const SkJumper_GatherCtx* ctx, F x, F y) {
    auto clamp = [](F v, F limit) {
        limit = bit_cast<F>( bit_cast<U32>(limit) - 1 );  
...
STAGE_GG(xy_to_unit_angle, Ctx::None) {
    ...
    phi = if_then_else(phi != phi , 0              , phi);  
    x = phi;
}
