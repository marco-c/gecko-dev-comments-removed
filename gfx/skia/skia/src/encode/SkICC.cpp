...
sk_sp<SkData> write_cicp_tag(const skcms_CICP& cicp) {
    SkDynamicMemoryWStream s;
    s.write32(SkEndian_SwapBE32(kTAG_cicp));  
    s.write32(0);                             
...
sk_sp<SkData> write_trc_tag(const skcms_Curve& trc) {
    ...
    if (trc.table_entries) {
        s.write32(SkEndian_SwapBE32(kTAG_CurveType));     
        s.write32(0);                                     
        s.write32(SkEndian_SwapBE32(trc.table_entries));  
...
sk_sp<SkData> write_mAB_or_mBA_tag(uint32_t type,
    ...
    SkDynamicMemoryWStream s;
    s.write32(SkEndian_SwapBE32(type));             
    s.write32(0);                                   
    s.write8(kNumChannels);                         
    s.write8(kNumChannels);                         
    s.write16(0);                                   
    s.write32(SkEndian_SwapBE32(b_curves_offset));  
    s.write32(SkEndian_SwapBE32(matrix_offset));    
    s.write32(SkEndian_SwapBE32(m_curves_offset));  
    s.write32(SkEndian_SwapBE32(clut_offset));      
    s.write32(SkEndian_SwapBE32(a_curves_offset));  
