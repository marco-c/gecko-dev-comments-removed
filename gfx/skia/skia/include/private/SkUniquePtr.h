...
private:
    ...
    struct compressed_base : private B {
         ...
         B& get()  { return *this; }
         B const& get() const  { return *this; }
        void swap(compressed_base&)  { }
    };
    ...
    template <typename B> struct compressed_base<B, false> {
        B fb;
         ...
         B& get()  { return fb; }
         B const& get() const  { return fb; }
        void swap(compressed_base& that)  { SkTSwap(fb, that.fB); }
    };
    ...
    struct compressed_data : private compressed_deleter_type {
        pointer fPtr;
         compressed_data() : compressed_deleter_type(), fPtr() {}
         compressed_data(const pointer& ptr, const deleter_type& d)
        ...
        >>  compressed_data(U1&& ptr, U2&& d)
         ...
         pointer& getPointer()  { return fPtr; }
         pointer const& getPointer() const  { return fPtr; }
         deleter_type& getDeleter()  {
            return compressed_deleter_type::get();
        }
         deleter_type const& getDeleter() const  {
            return compressed_deleter_type::get();
        }
        void swap(compressed_data& that)  {
...
public:
     ...
     unique_ptr(skstd::nullptr_t)  : unique_ptr() { }

    explicit unique_ptr(pointer ptr)  : data(ptr, deleter_type()) {
        static_assert(!is_pointer<deleter_type>::value, "Deleter is nullptr function pointer!");
    }
    ...
    unique_ptr(pointer ptr, remove_reference_t<deleter_type>&& d) 
    ...
    unique_ptr(unique_ptr<U, ThatD>&& that) 
    ...
    template <typename U, typename ThatD> enable_if_t<
        is_convertible<typename unique_ptr<U, ThatD>::pointer, pointer>::value &&
        !is_array<U>::value,
    unique_ptr&> operator=(unique_ptr<U, ThatD>&& that)  {
        reset(that.release());
        get_deleter() = forward<ThatD>(that.get_deleter());
        return *this;
    }

    unique_ptr& operator=(skstd::nullptr_t)  {
        reset();
        return *this;
    }
...
inline void swap(unique_ptr<T, D>& a, unique_ptr<T, D>& b)  {
    a.swap(b);
}
...
template <typename T, typename D>
inline bool operator==(const unique_ptr<T, D>& a, skstd::nullptr_t)  {
    
    return !a.is_attached();
}

template <typename T, typename D>
inline bool operator==(skstd::nullptr_t, const unique_ptr<T, D>& b)  {
