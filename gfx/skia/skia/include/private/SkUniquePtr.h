...
private:
    template <typename B, bool = std::is_empty<B>::value >
    struct compressed_base : private B {
         compressed_base() : B() {}
         compressed_base(const B& b) : B(b) {}
         compressed_base(B&& b) : B(std::move(b)) {}
         B& get()  { return *this; }
    ...
    template <typename B> struct compressed_base<B, false> {
        B fb;
         ...
         compressed_base(B&& b) : fb(std::move(b)) {}
         B& get()  { return fb; }
    ...
    struct compressed_data : private compressed_base<deleter_type> {
        pointer fPtr;
         compressed_data() : compressed_base<deleter_type>(), fPtr() {}
