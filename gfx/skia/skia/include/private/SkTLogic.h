...
template <typename T> struct is_class_detector {
    ...
    static const bool value = sizeof(clazz<T>(0)) == sizeof(yes_type) ;
};
...
struct is_convertible_detector {
    static const bool value = is_void<D>::value;
};
template <typename S, typename D> struct is_convertible_detector<S, D, false> {
    ...
    static const bool value = sizeof(convertible<S, D>(0)) == sizeof(yes_type);
};
