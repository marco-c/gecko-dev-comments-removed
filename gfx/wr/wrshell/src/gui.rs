...
fn do_debug_flags_ui(
    ...
    ui.window("Debug Flags").build(|| {
        ...
        push_flags |= ui.checkbox_flags("Profiler", &mut ctx.data_model.debug_flags, DebugFlags::PROFILER_DBG);
        push_flags |= ui.checkbox_flags("Render Targets", &mut ctx.data_model.debug_flags, DebugFlags::RENDER_TARGET_DBG);
        push_flags |= ui.checkbox_flags("Texture Cache", &mut ctx.data_model.debug_flags, DebugFlags::TEXTURE_CACHE_DBG);
