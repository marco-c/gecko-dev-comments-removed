...
struct Texture {
  ...
  bool allocate(bool force = false, int min_width = 0, int min_height = 0) {
    ...
    if ((!buf || force) && should_free()) {
      ...
      size_t max_stride = max(new_stride, compute_stride(new_bpp, min_width));
...
void ClearTexSubImage(GLuint texture, GLint level, GLint xoffset, GLint yoffset,
  ...
  switch (type) {
    ...
    case GL_UNSIGNED_BYTE: {
      ...
      switch (format) {
        case GL_RGBA:
          color = (color & ~0xFF000000) | (uint32_t(b[3]) << 24);  
          FALLTHROUGH;
        case GL_RGB:
          color = (color & ~0x00FF0000) | (uint32_t(b[2]) << 16);  
          FALLTHROUGH;
        case GL_RG:
          color = (color & ~0x0000FF00) | (uint32_t(b[1]) << 8);  
          FALLTHROUGH;
        case GL_RED:
          color = (color & ~0x000000FF) | uint32_t(b[0]);  
          break;
        default:
          assert(false);
          break;
