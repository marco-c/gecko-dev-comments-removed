...
void IsColorEqual(uint8_t* aBGRX, uint8_t* aRGBX, size_t aSize) {
  ...
  for (size_t i = 0; i < aSize; i += 4) {
    ASSERT_EQ(aBGRX[i + 2], aRGBX[i]);      
    ASSERT_EQ(aBGRX[i + 1], aRGBX[i + 1]);  
    ASSERT_EQ(aBGRX[i], aRGBX[i + 2]);      
    ASSERT_EQ(aBGRX[i + 3], aRGBX[i + 3]);  
  }
}
...
std::unordered_map<uint32_t, std::array<Color, 3>> GetExpectedConvertedRGB() {
  ...
  map.emplace(Hash(BLACK), std::array<Color, 3>{
                                                Color(0, 0, 0),
                                                
                                                Color(0, 0, 0),
                                                
                                                Color(0, 0, 0)});
  map.emplace(Hash(BLUE), std::array<Color, 3>{
                                               Color(0, 82, 0),
                                               
                                               Color(0, 54, 0),
                                               
                                               Color(0, 53, 0)});
  map.emplace(Hash(GREEN), std::array<Color, 3>{
                                                Color(0, 255, 0),
                                                
                                                Color(0, 231, 0),
                                                
                                                Color(0, 242, 0)});
  map.emplace(Hash(CYAN), std::array<Color, 3>{
                                               Color(0, 255, 255),
                                               
                                               Color(0, 248, 255),
                                               
                                               Color(0, 255, 255)});
  map.emplace(Hash(RED), std::array<Color, 3>{
                                              Color(0, 191, 0),
                                              
                                              Color(0, 147, 0),
                                              
                                              Color(0, 162, 0)});
  map.emplace(Hash(MAGENTA), std::array<Color, 3>{
                                                  Color(255, 0, 255),
                                                  
                                                  Color(255, 28, 255),
                                                  
                                                  Color(255, 18, 255)});
  map.emplace(Hash(YELLOW), std::array<Color, 3>{
                                                 Color(255, 255, 0),
                                                 
                                                 Color(255, 255, 0),
                                                 
                                                 Color(255, 255, 0)});
  map.emplace(Hash(WHITE), std::array<Color, 3>{
                                                Color(255, 255, 255),
  ...
  map.emplace(Hash(CHOCOLATE),
              std::array<Color, 3>{
                                   ...
                                   Color(229, 102, 20)});
  map.emplace(Hash(PERU), std::array<Color, 3>{
                                               Color(219, 137, 58),
...
void IsColorMatched(const Color& aColor, uint8_t* aRGBX, size_t aSize,
  ...
  for (size_t i = 0; i < aSize; i += 4) {
    ASSERT_EQ(r, aRGBX[i]);      
    ASSERT_EQ(g, aRGBX[i + 1]);  
    ASSERT_EQ(b, aRGBX[i + 2]);  
    if (aAlphaValue) {
      ASSERT_EQ(*aAlphaValue, aRGBX[i + 3]);  
    }
  }
}
...
TEST(YCbCrUtils, ConvertYCbCrToRGB32OddPicOffset)
{
  ...
  RunOddPicTest(colors, gfx::IntRect(1, 1, 3, 3));  
  RunOddPicTest(colors, gfx::IntRect(1, 0, 3, 4));  
  RunOddPicTest(colors, gfx::IntRect(0, 1, 4, 3));  
}
