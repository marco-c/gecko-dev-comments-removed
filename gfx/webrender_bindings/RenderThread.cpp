...
void RenderThread::HandleFrame(wr::WindowId aWindowId, bool aRender) {
  ...
  UpdateAndRender(aWindowId, startTime, aRender,  Nothing(),
                   Nothing(), hadSlowFrame);
...
static already_AddRefed<gl::GLContext> CreateGLContextANGLE() {
  ...
  if (!gl::GLLibraryEGL::EnsureInitialized( true,
                                           &discardFailureId)) {
