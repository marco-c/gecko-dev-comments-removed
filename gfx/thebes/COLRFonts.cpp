...
static int32_t ApplyVariation(const PaintState& aState, int32_t aValue,
  ...
  for (uint16_t i = 0; i < regionIndexCount; ++i, deltaData += deltaSize) {
    ...
    int32_t delta = *reinterpret_cast<const int8_t*>(deltaData);  
...
struct COLRFonts::GlyphLayers : public BaseGlyphRecord {};
...
const COLRFonts::GlyphLayers* COLRFonts::GetGlyphLayers(hb_blob_t* aCOLR,
  ...
  auto compareBaseGlyph = [](const void* key, const void* data) -> int {
    uint32_t glyphId = (uint32_t)(uintptr_t)key;
    const GlyphLayers* baseGlyph = reinterpret_cast<const GlyphLayers*>(data);
...
bool COLRFonts::PaintGlyphLayers(
  ...
  colors.SetLength(hb_ot_color_palette_get_colors(aFace,  0,
...
struct COLRFonts::GlyphPaintGraph : public BaseGlyphPaintRecord {};
...
const COLRFonts::GlyphPaintGraph* COLRFonts::GetGlyphPaintGraph(
  ...
  if (version == 1) {
    ...
    return static_cast<const GlyphPaintGraph*>(
...
bool COLRFonts::PaintGlyphGraph(
  ...
  colors.SetLength(hb_ot_color_palette_get_colors(face,  0, 0,
                                                  &colorCount, nullptr));
  colorCount = colors.Length();
  ...
  return PaintColrGlyph::DoPaint(state, aPaintGraph, aGlyphId);
}
