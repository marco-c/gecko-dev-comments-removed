...
struct YcbcrDesc final {
  ...
  static constexpr auto Narrow8() {  
    return YcbcrDesc();
  }
  static constexpr auto Full8() {  
    return YcbcrDesc{
        .y0 = 0 / 255.0,
        .y1 = 255 / 255.0,
        .u0 = 128 / 255.0,
...
struct mat final {
  ...
  std::array<avec<float, X_Cols>, Y_Rows> rows = {};  
...
struct ColorspaceTransform final {
  ...
  Lut3 ToLut3() const {
    auto defaultSize = ivec3({31, 31, 15});  
    if (srcSpace.yuv) {
      defaultSize = ivec3({31, 15, 31});  
    }
