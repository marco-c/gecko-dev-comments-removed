...
already_AddRefed<GLContext> GLContextEGLFactory::CreateImpl(
  ...
  if (aWebRender && egl->IsANGLE()) {
    ...
    if (aDepth) {
      ...
      if (!CreateConfigScreen(egl, &config,  aWebRender,
                              aUseGles)) {
...
already_AddRefed<GLContext> GLContextEGLFactory::Create(
  ...
  glContext = CreateImpl(aWindow, aWebRender,  false, aDepth);
#endif  

  if (!glContext) {
    glContext = CreateImpl(aWindow, aWebRender,  true, aDepth);
  }
...
already_AddRefed<GLContextEGL> GLContextEGL::CreateGLContext(
  ...
  std::vector<EGLint> rbab_attribs;  
...
EGLSurface GLContextProviderEGL::CreateEGLSurface(void* aWindow,
                                                  EGLConfig aConfig) {
  EGLSurface surface =
      CreateEGLSurfaceImpl(aWindow, aConfig,  false);
  if (surface == EGL_NO_SURFACE) {
    surface = CreateEGLSurfaceImpl(aWindow, aConfig,  true);
    if (surface == EGL_NO_SURFACE) {
...
already_AddRefed<GLContextEGL> GLContextEGL::CreateEGLPBufferOffscreenContext(
  ...
  RefPtr<GLContextEGL> gl = CreateEGLPBufferOffscreenContextImpl(
      desc, size,  false, out_failureId);
  if (!gl) {
    gl = CreateEGLPBufferOffscreenContextImpl(desc, size,  true,
                                              out_failureId);
  }
  return gl.forget();
}
