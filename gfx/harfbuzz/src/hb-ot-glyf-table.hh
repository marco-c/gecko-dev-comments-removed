...
struct glyf
{
  ...
  _populate_subset_glyphs (const hb_subset_plan_t * plan,
			   hb_vector_t<SubsetGlyph> * glyphs ) const
  {
  ...
  _zero_instruction_length (hb_bytes_t glyph)
  {
    ...
    if (!glyph_header.is_simple_glyph ()) return;  
  ...
  {
    const GlyphHeader &glyph_header = *glyph.as<GlyphHeader> ();
    if (!glyph_header.is_composite_glyph ()) return true;  
  ...
  struct CompositeGlyphHeader
  {
    ...
    static bool get_iterator (const char * glyph_data,
			      unsigned int length,
			      CompositeGlyphHeader::Iterator *iterator )
    {
      const GlyphHeader &glyph_header = *hb_bytes_t (glyph_data, length).as<GlyphHeader> ();
      if (!glyph_header.has_data ()) return false; 
  ...
  struct accelerator_t
  {
    ...
    bool get_composite (hb_codepoint_t glyph,
			CompositeGlyphHeader::Iterator *composite ) const
    {
