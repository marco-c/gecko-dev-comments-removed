...
typedef NumType<false, uint16_t> HBUINT16LE;	
...
typedef NumType<false, float>  HBFLOAT32LE;	
typedef NumType<false, double> HBFLOAT64LE;	
...
struct UnsizedListOfOffset16To : UnsizedArray16OfOffsetTo<Type, OffsetType, BaseType, has_null>
{
  const Type& operator [] (int i_) const
  {
    ...
    if (unlikely ((const void *) p < (const void *) this->arrayZ)) return Null (Type); 
    hb_barrier ();
    return this+*p;
  }
  Type& operator [] (int i_)
  {
    unsigned int i = (unsigned int) i_;
    const OffsetTo<Type, OffsetType, BaseType, has_null> *p = &this->arrayZ[i];
    if (unlikely ((const void *) p < (const void *) this->arrayZ)) return Crap (Type); 
    hb_barrier ();
    return this+*p;
  }
...
struct TupleValues
{
  ...
  static unsigned compile_unsafe (hb_array_t<const int> values, 
