...
struct CmapSubtableFormat4
{
                  ...
                  const hb_vector_t<segment_plan> &segments)
  {
    ...
    c->allocate_size<HBUINT16> (HBUINT16::static_size); 
  ...
  HBUINT16	values[VAR];
#if 0
  HBUINT16	endCount[segCount];	

  HBUINT16	reservedPad;		
  HBUINT16	startCount[segCount];	
  HBINT16		idDelta[segCount];	
  HBUINT16	idRangeOffset[segCount];
  HBUINT16	glyphIdArray[VAR];	
#endif
...
struct CmapSubtableLongGroup
{
  ...
  private:
  HBUINT32		startCharCode;	
  HBUINT32		endCharCode;	
  HBUINT32		glyphID;	
...
struct cmap
{
		       ...
		       void *dest) const
  {
    ...
    if (unlikely (!table->encodingRecord.serialize (&c,  3)))
      return false;
    ...
    format4_plat0_rec.platformID.set (0); 
    format4_plat0_rec.encodingID.set (3);

    
    EncodingRecord &format4_plat3_rec = table->encodingRecord[1];
    format4_plat3_rec.platformID.set (3); 
    format4_plat3_rec.encodingID.set (1); 

    
    EncodingRecord &format12_rec = table->encodingRecord[2];
    format12_rec.platformID.set (3); 
    format12_rec.encodingID.set (10); 
