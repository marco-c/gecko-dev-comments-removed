...
struct cmap
{
		...
		void *dest) const
  {
    ...
    if (unlikely (!table->encodingRecord.serialize (&c,  cmap_subset_plan.format12_groups ? 3 : 2))) return false;

    

    
    EncodingRecord &format4_plat0_rec = table->encodingRecord[0];
    format4_plat0_rec.platformID = 0; 
    format4_plat0_rec.encodingID = 3;

    
    EncodingRecord &format4_plat3_rec = table->encodingRecord[1];
    format4_plat3_rec.platformID = 3; 
    format4_plat3_rec.encodingID = 1; 

    
    if (cmap_subset_plan.format12_groups)
    {
      EncodingRecord &format12_rec = table->encodingRecord[2];
      format12_rec.platformID = 3; 
      format12_rec.encodingID = 10; 
    }
