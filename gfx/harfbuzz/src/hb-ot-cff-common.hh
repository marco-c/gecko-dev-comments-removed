...
struct CFFIndex
{
  ...
  {
    TRACE_SANITIZE (this);
    return_trace (likely ((c->check_struct (this) && count == 0) || 
...
struct CFFIndexOf : CFFIndex<COUNT>
{
  ...
  static unsigned int calculate_serialized_size (unsigned int &offSize_ ,
						 const DATA *dataArray,
						 unsigned int dataArrayLen,
						 hb_vector_t<unsigned int> &dataSizeArray, 
						 const PARAM &param)
  {
...
struct Dict : UnsizedByteStr
{
  ...
  static bool serialize_int_op (hb_serialize_context_t *c, op_code_t op, int value, op_code_t intOp)
  {
    ...
    if ( (!serialize_int<INTTYPE, minVal, maxVal> (c, intOp, value)))
      return false;
