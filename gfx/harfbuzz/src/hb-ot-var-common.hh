...
struct TupleVariationHeader
{
  ...
  bool unpack_axis_tuples (unsigned axis_count,
                           ...
                           hb_hashmap_t<hb_tag_t, Triple>& axis_tuples ) const
  {
			   ...
			   hb_scalar_cache_t *shared_tuple_scalar_cache = nullptr) const
  {
    ...
    bool has_interm = tuple_index & TuppleIndex::IntermediateRegion; 
    if (unlikely (has_interm))
      shared_tuple_scalar_cache = nullptr;

    if (unlikely (tuple_index & TuppleIndex::EmbeddedPeakTuple)) 
    {
    ...
    {
      unsigned int index = tuple_index & TuppleIndex::TupleIndexMask; 
...
struct tuple_delta_t
{
				 ...
				 hb_alloc_pool_t *pool = nullptr)
  {
    ...
    if (!shared_tuples_idx_map)
      compile_coords (axes_index_map, axes_old_index_tag_map); 
  ...
  static bool compile_point_set (const hb_vector_t<bool> &point_indices,
                                 hb_vector_t<unsigned char>& compiled_points )
  {
...
struct TupleVariationData
{
  ...
  struct tuple_iterator_t
  {
    ...
    bool get_shared_indices (hb_vector_t<unsigned int> &shared_indices )
    {
  ...
  template <typename T>
  static bool decompile_deltas (const HBUINT8 *&p ,
				hb_vector_t<T> &deltas ,
				const HBUINT8 *end,
  ...
  bool decompile_tuple_variations (unsigned point_count,
                                   ...
                                   const hb_vector_t<unsigned> &shared_indices,
                                   const hb_array_t<const F2DOT14> shared_tuples,
                                   tuple_variations_t& tuple_variations, 
				   hb_alloc_pool_t *pool = nullptr,
