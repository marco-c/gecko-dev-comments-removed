...
struct Ligature
{
  ...
  bool serialize (hb_serialize_context_t *c,
		  GlyphID ligature,
		  hb_array_t<const GlyphID> components )
  {
    TRACE_SERIALIZE (this);
    if (unlikely (!c->extend_min (*this))) return_trace (false);
    ligGlyph = ligature;
...
struct LigatureSet
{
  ...
  bool serialize (hb_serialize_context_t *c,
		  ...
		  hb_array_t<const GlyphID> &component_list )
  {
    TRACE_SERIALIZE (this);
    if (unlikely (!c->extend_min (*this))) return_trace (false);
 ...
 inline typename context_t::return_t SubstLookup::dispatch_recurse_func (context_t *c, unsigned int lookup_index)
{
 ...
 inline bool SubstLookup::apply_recurse_func (hb_ot_apply_context_t *c, unsigned int lookup_index)
{
  const SubstLookup &l = c->face->table.GSUB.get_relaxed ()->table->get_lookup (lookup_index);
