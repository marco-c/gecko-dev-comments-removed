...
nsresult CollectProcessInfo(ProcessInfo& info) {
  ...
  if (cpuSpeed >= 0) info.cpuSpeed = cpuSpeed;
  if (!cpuVendor.IsEmpty()) info.cpuVendor = cpuVendor;
  if (cpuFamily >= 0) info.cpuFamily = cpuFamily;
  if (cpuModel >= 0) info.cpuModel = cpuModel;
  if (cpuStepping >= 0) info.cpuStepping = cpuStepping;

  if (logicalCPUs >= 0) info.cpuCount = logicalCPUs;
  if (physicalCPUs >= 0) info.cpuCores = physicalCPUs;

  if (cacheSizeL2 >= 0) info.l2cacheKB = cacheSizeL2;
  if (cacheSizeL3 >= 0) info.l3cacheKB = cacheSizeL3;
