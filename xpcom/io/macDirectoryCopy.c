...
enum
{
	getNextItemOp			= 1,	
	copyDirCommentOp		= 2,	
	copyDirAccessPrivsOp	= 3,	
	copyDirFMAttributesOp	= 4,	
	dirCreateOp				= 5,	
	fileCopyOp				= 6		
};
	...
	do
	{
		...
		if ( theGlobals->result == noErr )
		{
			if ( (theGlobals->copyFilterProc == NULL) ||
				 CallCopyFilterProc(theGlobals->copyFilterProc, &theGlobals->myCPB) ) 
			{
				...
				if ( (theGlobals->myCPB.dirInfo.ioFlAttrib & ioDirMask) != 0 )
				{
					...
					GetLevelSize(theGlobals->myCPB.dirInfo.ioDrDirID, theGlobals); 
						  ...
						  EnumerateGlobals *theGlobals)
{
	...
	{	
		...
		++index; 
	} while ( (theGlobals->error == noErr) && (!theGlobals->bailout) ); 
}
									  ...
									  CopyFilterProcPtr copyFilterProc, ConstStr255Param newName)
{
	...
	theGlobals.destinationVRefNum = dstVRefNum; 
