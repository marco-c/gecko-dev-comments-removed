...
kern_return_t MachSendPortSendRight(mach_port_t endpoint, mach_port_t attachment,
  ...
  send_msg.header.msgh_bits = MACH_MSGH_BITS(endpoint_disposition, 0) | MACH_MSGH_BITS_COMPLEX;
  ...
  return mach_msg(&send_msg.header, opts, send_msg.header.msgh_size, 0, MACH_PORT_NULL, timeout,
                  MACH_PORT_NULL);
}


kern_return_t MachReceivePortSendRight(const mozilla::UniqueMachReceiveRight& endpoint,
                                       mozilla::Maybe<mach_msg_timeout_t> opt_timeout,
                                       mozilla::UniqueMachSendRight* attachment,
                                       audit_token_t* audit_token) {
  mach_msg_option_t opts = MACH_RCV_MSG | MACH_RCV_TRAILER_TYPE(MACH_MSG_TRAILER_FORMAT_0) |
  ...
  kern_return_t kr = mach_msg(&recv_msg.header, opts, 0, recv_msg.header.msgh_size, endpoint.get(),
