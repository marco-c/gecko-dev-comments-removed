...
Message::Message(Message&& other) : Pickle(std::move(other)) {
 ...
 Message* Message::IPDLMessage(int32_t routing_id, msgid_t type,
                                         HeaderFlags flags) {
  return new Message(routing_id, type, 0, flags, true);
}

 Message* Message::ForSyncDispatchError(NestedLevel level) {
 ...
 Message* Message::ForInterruptDispatchError() {
  auto* m = new Message();
  auto& flags = m->header()->flags;
  flags.SetInterrupt();
  flags.SetReply();
