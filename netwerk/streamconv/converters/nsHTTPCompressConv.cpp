...
class ZstdWrapper {
 public:
  ZstdWrapper(nsIRequest* aRequest, nsHTTPCompressConv::CompressMode aMode) {
...
#ifdef ZSTD_INFALLIBLE
    mDStream = ZSTD_createDStream_advanced(zstd_allocators);  
#else
    ...
    ZSTD_DCtx_setParameter(mDStream, ZSTD_d_windowLogMax, 23 );
  }
  ~ZstdWrapper() {
    if (mDStream) {
...
nsHTTPCompressConv::OnStopRequest(nsIRequest* request, nsresult aStatus) {
  ...
  if (NS_SUCCEEDED(status) && (mMode == HTTP_COMPRESS_BROTLI ||
    ...
    if (mBrotli && NS_FAILED(mBrotli->mStatus)) {
      DICTIONARY_LOG(("!! Brotli failed in OnStop: %d", mBrotli->mStatus));
...
nsresult nsHTTPCompressConv::BrotliHandler(nsIInputStream* stream,
  ...
  *countRead = 0;
  ...
  const size_t kOutSize = 128 * 1024;  
  uint8_t* outPtr;
  size_t outSize;
  size_t avail = aAvail;
...
nsresult nsHTTPCompressConv::ZstdHandler(nsIInputStream* stream, void* closure,
  ...
  *countRead = 0;
  ...
  const size_t kOutSize = ZSTD_DStreamOutSize();  
  uint8_t* outPtr;
  size_t avail = aAvail;
