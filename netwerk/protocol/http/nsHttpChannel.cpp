...
static void RecordOnStartTelemetry(nsresult aStatus,
  ...
  if (StaticPrefs::network_trr_odoh_enabled()) {
    ...
    uint32_t confirmationState = 0;
    Unused << dns->GetCurrentTrrConfirmationState(&confirmationState);
    const uint32_t kCONFIRM_TRYING_OK = 1;
    const uint32_t kCONFIRM_OK = 2;
    if (confirmationState == kCONFIRM_TRYING_OK ||
        confirmationState == kCONFIRM_OK) {
