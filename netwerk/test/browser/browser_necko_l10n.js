...
const READ_MOCHI_TEST_MESSAGE = "Read mochi.test";
...
function observeStatusLabel(statusLabel, expectedValues) {
  ...
  return new Promise(resolve => {
    ...
    if (prevValue && prevValue !== READ_MOCHI_TEST_MESSAGE) {
      ok(
        expectedValues.has(prevValue),
        `Getting an unexpected value for statusLabel - ${prevValue}`
      );
    ...
    const observer = new MutationObserver(mutations => {
      for (let mutation of mutations) {
        if (mutation.attributeName === "value") {
          if (
            statusLabel.value &&
            statusLabel.value !== prevValue &&
            statusLabel.value !== READ_MOCHI_TEST_MESSAGE
          ) {
            ok(
