...
function VisitCallbackPromise(num, consumption, entries) {
  return new Promise(resolve => {
    const storage = getCacheStorage("disk");
    storage.asyncVisitStorage(
      new VisitCallback(num, consumption, entries, resolve),
      true
    );
  });
}
...
async function run_test() {
  ...
  try {
    ...
    info(`Ensure that only the ${URL2}/a entry remains.`);
    await VisitCallbackPromise(1, 1024, [URL2 + "/a"]);
