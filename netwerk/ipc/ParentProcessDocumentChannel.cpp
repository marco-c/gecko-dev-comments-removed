...
NS_IMETHODIMP ParentProcessDocumentChannel::AsyncOpen(
  ...
  if (!mDocumentLoadListener->Open(
          mLoadState, static_cast<class LoadInfo*>(mLoadInfo.get()), mLoadFlags,
          mCacheKey, mChannelId, mAsyncOpenTime, mTiming,
          std::move(initialClientInfo), mLoadInfo->GetOuterWindowID(),
          GetDocShell()
              ->GetBrowsingContext()
...
ParentProcessDocumentChannel::Observe(nsISupports* aSubject, const char* aTopic,
  ...
  MOZ_ASSERT(channel);
  if (mDocumentLoadListener->GetChannel() != channel) {
