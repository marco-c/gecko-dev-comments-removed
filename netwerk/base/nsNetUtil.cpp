...
already_AddRefed<nsIIOService> do_GetIOService(nsresult* error ) {
...
Result<nsCOMPtr<nsIInputStream>, nsresult> NS_NewLocalFileInputStream(
    nsIFile* file, int32_t ioFlags , int32_t perm ,
    int32_t behaviorFlags ) {
...
already_AddRefed<nsINetUtil> do_GetNetUtil(nsresult* error ) {
  nsCOMPtr<nsIIOService> io;
...
NS_NewLocalFileRandomAccessStream(nsIFile* file, int32_t ioFlags ,
                                  int32_t perm ,
                                  int32_t behaviorFlags ) {
