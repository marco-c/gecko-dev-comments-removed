        ...
        for i in range(n):
            if conditions[i]:  
                ...
                if conditions[i][0] == "!":
                    ...
                    cond = conditions[i]
                if (not (self.implicit_vars and cond in IMPLICIT)) or (  
                    (IMPLICIT[cond] and negated)
                    or (not IMPLICIT[cond] and not negated)  
                ):
                    conds[cond] = conditions[i]
        for j in range(m):
            if fuzzy_ifs[j]:  
                ...
                if fuzzy_ifs[j][0] == "!":
                    ...
                    cond = fuzzy_ifs[j]
                if not (self.implicit_vars and cond in IMPLICIT):  
                    pass  
        ...
        while i < j:
            ...
            if len(m) != 1:  
                ...
                if this_os == os and (
                    ...
                    d_min = min(dmin, d_min)  
                    d_max = max(dmax, d_max)
                    p_min = min(pmin, p_min)  
