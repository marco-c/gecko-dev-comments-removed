...
DEBUG_THRESHOLD = 40 * MS_PER_MINUTE  
OPT_THRESHOLD = 20 * MS_PER_MINUTE  
...
FAILURE_RATIO = 0.4  
INTERMITTENT_RATIO_REFTEST = 0.4  
FAILURE_RATIO_REFTEST = 0.8  
GROUP = "group"
KIND = "kind"
LINENO = "lineno"
LL = "label"
...
class Classification(object):
    ...
    DISABLE_TOO_LONG = "disable_too_long"  
...
class Skipfails(object):
        ...
        for task in tasks:  
            ...
            try:
                ...
                for result in task.results:
                    ...
                    for path in manifest_paths[mm][config]:  
    ...
    def skip_failure(
        ...
        if classification == Classification.DISABLE_MANIFEST:
            ...
            if kind == Kind.TOML:
                for x in path:
                    path_to_filename[x] = self.get_filename_in_manifest(
                        manifest.split(":")[-1], x
                    )
        ...
        if kind != Kind.LIST:
            self.vinfo(f"filename: {path_to_filename}")
        ...
        if self.bugzilla is None:
            ...
            if len(bugs) == 0:
                ...
                product, component = self.get_file_info([p for p in path][0])
                if self.dry_run:
                    self.warning(
        ...
        if task_id in self.extras:  
            ...
            if "runtime" in test_setting:
                for k in test_setting["runtime"]:
                    ...
                    elif k in self.variants:  
                        runtimes.append(self.variants[k])  
    ...
    def wpt_add_skip_if(self, manifest_str, anyjs, skip_if, bug_reference):
        ...
        while i < n:
            line = lines[i]
            if line.startswith("["):
                if section is not None and not anyjs[section]:  
                    if not disabled_key:
    ...
    def get_allpaths(self, task_id, manifest, path):
        ...
        for line in r.text.encode("utf-8").splitlines():
            ...
            if not group or not os.path.exists(group):  
                continue
