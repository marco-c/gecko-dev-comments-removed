...
DictJSON = Dict[str, JSONType]  
ListBug = List[Bug]  
ListInt = List[int]  
ListStr = List[str]  
ManifestPaths = Dict[str, Dict[str, List[str]]]  
OptBug = Optional[Bug]
BugsBySummary = Dict[str, OptBug]  
...
Tasks = List[TestTask]  
TupleOptIntStrOptInt = Tuple[OptInt, str, OptInt]  
...
class Skipfails:
    ...
    def get_kind_manifest(self, manifest: str):
        ...
        if manifest.endswith(".ini"):
            ...
            if path is None or wpt_manifest is None:  
                self.warning(
        ...
        for task in tasks:  
            ...
            try:
                if len(task.results) == 0:
                    continue  
                _fail_types = task.failure_types  
        ...
        for task in tasks:  
            ...
            try:
                ...
                for result in task.results:
                    ...
                    for path in manifest_paths[manifest][config]:  
