test(() => {
  const p1 = trustedTypes.createPolicy("one", {}); 
  assert_equals(p1.name, "one");

  const p2 = trustedTypes.createPolicy("two", {}); 
  assert_equals(p2.name, "two");
}, "Creating policy works for policy in the allowlist.");

test(() => {
  const p3 = trustedTypes.createPolicy("three", {}); 
  assert_equals(p3.name, "three");

  const p4 = trustedTypes.createPolicy("four", {}); 
