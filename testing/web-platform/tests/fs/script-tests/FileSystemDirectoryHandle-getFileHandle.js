...
directory_test(async (t, dir) => {
  const existing_handle = await createFileWithContents(
      t, 'existing-file', '1234567890',  dir);
...
directory_test(async (t, dir) => {
  const existing_handle = await createFileWithContents(
      t, 'file-with-contents', '1234567890',  dir);
...
directory_test(async (t, dir) => {
  const subdir = await createDirectory(t, 'subdir-name',  dir);
...
directory_test(async (t, dir) => {
  ...
  await createEmptyFile(t, file_name,  subdir);
