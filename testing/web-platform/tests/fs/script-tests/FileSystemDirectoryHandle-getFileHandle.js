...
directory_test(async (t, dir) => {
  const existing_handle = await createFileWithContents(
      'file-with-contents', '1234567890',  dir);
...
directory_test(async (t, dir) => {
  const subdir = await createDirectory('subdir-name',  dir);
...
directory_test(async (t, dir) => {
  ...
  const subdir = await createDirectory(subdir_name,  dir);

  const file_name = 'file-name';
  await createEmptyFile(file_name,  subdir);
