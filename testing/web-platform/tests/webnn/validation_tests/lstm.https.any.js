...
const tests = [
  ...
  },
  {
    ...
    weight: {
      dataType: 'float32',
      shape: [ 2, 4 * hiddenSize, inputSize]
    },
    recurrentWeight: {
      dataType: 'float32',
      shape: [ 2, 4 * hiddenSize, hiddenSize]
    },
    steps: steps,
    hiddenSize: hiddenSize,
    options: {
      bias:
          {dataType: 'float32', shape: [ 2, 4 * hiddenSize]},
      recurrentBias:
          {dataType: 'float32', shape: [ 2, 4 * hiddenSize]},
      peepholeWeight:
          {dataType: 'float32', shape: [ 2, 3 * hiddenSize]},
      initialHiddenState: {
        dataType: 'float32',
    ...
    outputs: [
      ...
      {
        dataType: 'float32',
        shape: [steps,  2, batchSize, hiddenSize]
      }
    ]
  },
  {
