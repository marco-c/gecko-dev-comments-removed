...
async function sendEscape() {
  await waitForRender();
  await new test_driver.send_keys(document.activeElement || document.documentElement,'\uE00C'); 
  await waitForRender();
}
...
function assertIsFunctionalPopover(popover, checkVisibility) {
  assertPopoverVisibility(popover, true, false, 'A popover should start out hidden');
  popover.showPopover();
  if (checkVisibility) assertPopoverVisibility(popover, true, true, 'After showPopover(), a popover should be visible');
  popover.showPopover(); 
  popover.hidePopover();
  if (checkVisibility) assertPopoverVisibility(popover, true, false, 'After hidePopover(), a popover should be hidden');
  popover.hidePopover(); 
  popover.togglePopover();
  if (checkVisibility) assertPopoverVisibility(popover, true, true, 'After togglePopover() on hidden popover, it should be visible');
  popover.togglePopover();
  if (checkVisibility) assertPopoverVisibility(popover, true, false, 'After togglePopover() on visible popover, it should be hidden');
  popover.togglePopover(true);
  if (checkVisibility) assertPopoverVisibility(popover, true, true, 'After togglePopover(true) on hidden popover, it should be visible');
  popover.togglePopover(true);
  if (checkVisibility) assertPopoverVisibility(popover, true, true, 'After togglePopover(true) on visible popover, it should be visible');
  popover.togglePopover(false);
  if (checkVisibility) assertPopoverVisibility(popover, true, false, 'After togglePopover(false) on visible popover, it should be hidden');
  popover.togglePopover(false);
  if (checkVisibility) assertPopoverVisibility(popover, true, false, 'After togglePopover(false) on hidden popover, it should be hidden');
...
function assertNotAPopover(nonPopover) {
  ...
  assertPopoverVisibility(nonPopover, false, expectVisible, 'A non-popover should start out visible');
  ...
  assertPopoverVisibility(nonPopover, false, expectVisible, 'Calling showPopover on a non-popover should leave it visible');
  ...
  assertPopoverVisibility(nonPopover, false, expectVisible, 'Calling hidePopover on a non-popover should leave it visible');
