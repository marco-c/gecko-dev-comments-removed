...
usb_test(() => {
  return getFakeDevice().then(({ device }) => {
    ...
    return device.open()
      ...
      .then(() => device.claimInterface(0))
      .then(() => Promise.all([
          assertRejectsWithEndpointNotFoundError(device.transferIn(2, 8)), 
          assertRejectsWithEndpointNotFoundError(device.transferIn(3, 8)), 
          assertRejectsWithError(
              device.transferIn(16, 8), 'IndexSizeError', rangeError),
          assertRejectsWithEndpointNotFoundError(device.transferOut(2, data)), 
          assertRejectsWithEndpointNotFoundError(device.transferOut(3, data)), 
          assertRejectsWithError(
