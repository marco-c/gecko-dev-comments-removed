...
promise_test(async t => {
    ...
    const subdir = await createDirectory(t, subdir_name, dir);

    const file_name = 'file-name';
    const file = await createEmptyFile(t, file_name, subdir);

    
    await file.copyTo(dir, "");
...
promise_test(async t => {
    ...
    await promise_rejects(t, 'SecurityError', file.copyTo(dir, kCurrentDirectory));
...
promise_test(async t => {
    ...
    const first_subdir = await createDirectory(t, first_subdir_name, dir);

    const second_subdir_name = 'second-subdir-name';
    const second_subdir = await createDirectory(t, second_subdir_name, first_subdir);

    const file_name = 'file-name';
    const file = await createEmptyFile(t, file_name, second_subdir);

    await promise_rejects(t, 'SecurityError', file.copyTo(first_subdir, kParentDirectory));
...
promise_test(async t => {
    ...
    const file_name = 'file-name';
    const file = await createEmptyFile(t, file_name, dir);
