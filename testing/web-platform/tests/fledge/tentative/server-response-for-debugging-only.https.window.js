...
async function noRequestsObserved(uuid, timeout = 2000 ) {
...
subsetTest(promise_test, async test => {
  ...
  let debugReports = [createDebugReportsPerOrigin(
       null,
      [createDebugReport(bidderDebugReportURL,  true)])];
...
subsetTest(promise_test, async test => {
  ...
  let debugReports = [createDebugReportsPerOrigin(
      ...
      [createDebugReport('http://nothttps.com',  true)])];
  await testInvalidDebugReportsFields(test, uuid, debugReports);
...
subsetTest(promise_test, async test => {
  ...
  let debugReports = [createDebugReportsPerOrigin(
       window.location.origin,
      [createDebugReport('not a url',  true)])];
  await testInvalidDebugReportsFields(test, uuid, debugReports);
...
subsetTest(promise_test, async test => {
  ...
  let bidderDebugReportURL =
      createBidderReportURL(uuid,  'forDebuggingOnly', igOwner);
  let sellerDebugReportURL =
      createSellerReportURL(uuid,  'forDebuggingOnly');
...
subsetTest(promise_test, async test => {
  ...
  let debugReports = [
    createDebugReportsPerOrigin(
        igOwner, [createDebugReport('not a url',  true)]),
    createDebugReportsPerOrigin(
        window.location.origin, [createDebugReport(
                                    sellerDebugReportURL,  true,
                                     true)])
