...
subsetTest(promise_test, async test => {
  ...
  const componentURL2 = createRenderURL(uuid, null, 'num-value');
  const componentURL3 = createRenderURL(uuid, null, 'string-value');
  const componentURL4 = createRenderURL(uuid, null, 'array-value');
  const componentURL5 = createRenderURL(uuid, null, 'object-value');
  const componentURL6 = createRenderURL(uuid, null, 'wrong-url');
...
subsetTest(promise_test, async test => {
  const uuid = generateUuid(test);
  const renderURL1 = createRenderURL(uuid, null, 'url,group1');
  const renderURL2 = createRenderURL(uuid, null, 'url,group2');
  const auctionConfigOverrides = {
      trustedScoringSignalsURL: TRUSTED_SCORING_SIGNALS_URL,
...
subsetTest(promise_test, async test => {
  ...
  const componentURL1 = createRenderURL(
      uuid, null, 'url,component1');
  const componentURL2 = createRenderURL(
      uuid, null, 'url,component2');
