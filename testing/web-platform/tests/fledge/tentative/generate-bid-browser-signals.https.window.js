...
subsetTest(promise_test, async test => {
  ...
  await joinInterestGroup(
      test, uuid,
      { name: uuid,
        biddingLogicURL: createPrevWinsMsBiddingScriptURL([], 0),
        ads: ads });
  await runBasicFledgeAuctionAndNavigate(test, uuid);
  await waitForObservedRequests(uuid, [createSellerReportURL(uuid)]);
...
subsetTest(promise_test, async test => {
  ...
  await testClickiness(
      test, IG_OWNER, VIEW_CLICK_PROVIDER,  2,
       true,
       true, [VIEW_CLICK_PROVIDER]);
...
subsetTest(promise_test, async test => {
  ...
  await testClickiness(
      test, IG_OWNER, VIEW_CLICK_PROVIDER,  4,
       false,
       false);
...
subsetTest(promise_test, async test => {
  ...
  await testClickiness(
      test, IG_OWNER, VIEW_CLICK_PROVIDER,  6,
       true,
       true, []);
...
subsetTest(promise_test, async test => {
  ...
  await testClickiness(
      test, IG_OWNER, VIEW_CLICK_PROVIDER,  0,
       true,
       true, []);
...
subsetTest(promise_test, async test => {
  ...
  await testClickiness(
      test, IG_OWNER, VIEW_CLICK_PROVIDER,  1,
       true,
       false, [VIEW_CLICK_PROVIDER]);
