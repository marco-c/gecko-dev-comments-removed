...
const pollReports = async (origin, wait_for = 1, timeout = 5000 ) => {
...
subsetTest(promise_test, async test => {
  ...
  const sellerReports =
      await pollReports(location.origin,  1,  1000);
...
subsetTest(promise_test, async test => {
  ...
  const buyerReports =
      await pollReports(OTHER_ORIGIN1,  1,  1000);
...
subsetTest(promise_test, async test => {
  ...
  await joinCrossOriginInterestGroup(test, uuid, OTHER_ORIGIN2, {
    biddingLogicURL:
        createBiddingScriptURLForRealTimeReporting(OTHER_ORIGIN2,  100)
  });
  await runBasicFledgeAuctionAndNavigate(test, uuid, {
    decisionLogicURL: createDecisionScriptURLForRealTimeReporting(uuid),
...
subsetTest(promise_test, async test => {
  ...
  const reports2 =
      await pollReports(OTHER_ORIGIN2,  1,  1000);
