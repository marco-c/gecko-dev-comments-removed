...
const M = (a, b = P) => { let r = a % b; return r >= 0n ? r : b + r; }; 
...
const modL_LE = (hash) => M(b2n_LE(hash), N); 
let _shaS;
...
const utils = {
    ...
    precompute: (w = 8, p = G) => { p.multiply(3n); w; return p; }, 
};
const W = 8; 
const precompute = () => {
    const points = []; 
...
const wNAF = (n) => {
    ...
    const neg = (cnd, p) => { let n = p.negate(); return cnd ? n : p; }; 
    let p = I, f = G; 
...
export { getPublicKey, getPublicKeyAsync, sign, verify, 
