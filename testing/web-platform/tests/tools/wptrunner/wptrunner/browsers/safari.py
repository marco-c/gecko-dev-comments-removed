...
def run_info_extras(logger, **kwargs):
    ...
    if safari_info is not None:
        assert safari_bundle is not None  
...
def get_safari_info(wd_path):
    bundle_paths = [
        os.path.join(os.path.dirname(wd_path), "..", ".."),  
        os.path.join(os.path.dirname(wd_path), "Safari.app"),  
...
def get_webkit_info(safari_bundle_path):
    framework_paths = [
        os.path.join(os.path.normpath(safari_bundle_path), "Contents", "Frameworks"),  
        os.path.join(os.path.normpath(safari_bundle_path), ".."),  
