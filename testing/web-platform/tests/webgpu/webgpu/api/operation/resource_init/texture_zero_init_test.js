  ...
  static generateParams(readMethods) {
    return (
      params().combine(poptions('format', kTextureFormats)).combine(poptions('aspect', kTextureAspects)).unless(({
        format,
        aspect
      }) => aspect === 'depth-only' && !kTextureFormatInfo[format].depth || aspect === 'stencil-only' && !kTextureFormatInfo[format].stencil).combine(poptions('mipLevelCount', kMipLevelCounts)).combine(poptions('sampleCount', kSampleCounts)) 
      .unless(({
        sampleCount,
        mipLevelCount
      ...
      (readMethod === ReadMethod.CopyToBuffer || readMethod === ReadMethod.CopyToTexture) && (format === 'depth24plus' || format === 'depth24plus-stencil8')).unless(({
        readMethod,
        format
      }) => readMethod === ReadMethod.DepthTest && !kTextureFormatInfo[format].depth || readMethod === ReadMethod.StencilTest && !kTextureFormatInfo[format].stencil || readMethod === ReadMethod.ColorBlending && !kTextureFormatInfo[format].color || 
      readMethod === ReadMethod.Sample && kTextureFormatInfo[format].depth).unless(({
        readMethod,
        sampleCount
      }) => 
      sampleCount > 1 && (readMethod === ReadMethod.CopyToBuffer || readMethod === ReadMethod.CopyToTexture)).combine(kCreationSizes) 
      .unless(({
        sampleCount,
        sliceCount
