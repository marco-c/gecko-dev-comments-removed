






'use strict';


index_get_all_values_with_options_test(
     'out-of-line',  {query: 'C'}, 'Single item get');

index_get_all_values_with_options_test(
     'empty',  {}, 'Empty object store');

index_get_all_values_with_options_test(
     'out-of-line',  {}, 'Get all');

index_get_all_values_with_options_test(
     'generated',  {}, 'Get all with generated keys');

index_get_all_values_with_options_test(
     'large-values',  {},
    'Get all with large values');

index_get_all_values_with_options_test(
     'out-of-line',  {count: 10}, 'maxCount=10');

index_get_all_values_with_options_test(
     'out-of-line',
     {query: IDBKeyRange.bound('G', 'M')}, 'Get bound range');

index_get_all_values_with_options_test(
     'out-of-line',
     {query: IDBKeyRange.bound('G', 'M'), count: 3},
    'Get bound range with maxCount');

index_get_all_values_with_options_test(
     'out-of-line',  {
      query:
          IDBKeyRange.bound('G', 'K',  false,  true)
    },
    'Get upper excluded');

index_get_all_values_with_options_test(
     'out-of-line',  {
      query:
          IDBKeyRange.bound('G', 'K',  true,  false)
    },
    'Get lower excluded');

index_get_all_values_with_options_test(
     'generated',
     {query: IDBKeyRange.bound(4, 15), count: 3},
    'Get bound range (generated) with maxCount');

index_get_all_values_with_options_test(
     'out-of-line',  {query: 'Doesn\'t exist'},
    'Non existent key');

index_get_all_values_with_options_test(
     'out-of-line',  {count: 0}, 'maxCount=0');

index_get_all_values_with_options_test(
     'out-of-line',  {count: 4294967295},
    'Max value count');

index_get_all_values_with_options_test(
     'out-of-line',
     {query: IDBKeyRange.upperBound('0')},
    'Query with empty range where  first key < upperBound');

index_get_all_values_with_options_test(
     'out-of-line',
     {query: IDBKeyRange.lowerBound('ZZ')},
    'Query with empty range where lowerBound < last key');

index_get_all_values_with_options_test(
     'out-of-line-not-unique',  {query: 'first'},
    'Retrieve multiEntry key');

index_get_all_values_with_options_test(
     'out-of-line-multi',  {query: 'vowel'},
    'Retrieve one key multiple values');

index_get_all_values_with_options_test(
     'out-of-line',  {direction: 'next'},
    'Direction: next');

index_get_all_values_with_options_test(
     'out-of-line',  {direction: 'prev'},
    'Direction: prev');

index_get_all_values_with_options_test(
     'out-of-line',  {direction: 'nextunique'},
    'Direction: nextunique');

index_get_all_values_with_options_test(
     'out-of-line',  {direction: 'prevunique'},
    'Direction: prevunique');

index_get_all_values_with_options_test(
     'out-of-line-not-unique',  {direction: 'nextunique'},
    'Direction: nextunique with duplicate index keys');

index_get_all_values_with_options_test(
     'out-of-line-not-unique',  {direction: 'prevunique'},
    'Direction: prevunique with duplicate index keys');

index_get_all_values_with_options_test(
     'out-of-line-multi',  {direction: 'nextunique'},
    'Direction: nextunique with multi-entry index');

index_get_all_values_with_options_test(
     'out-of-line-multi',  {direction: 'prevunique'},
    'Direction: prevunique with multi-entry index');

index_get_all_values_with_options_test(
     'out-of-line',  {
      direction: 'prev',
      query: IDBKeyRange.bound('b', 'x'),
    },
    'Direction and query');

index_get_all_values_with_options_test(
     'out-of-line',  {
      direction: 'prev',
      query: IDBKeyRange.bound('b', 'x'),
      count: 4
    },
    'Direction, query and count');

get_all_with_options_and_count_test(
    'getAll',  'out-of-line',  'test_idx',
    'Get all values with both options and count');

get_all_with_invalid_keys_test(
    'getAll',  'out-of-line',  'test_idx',
     true, 'Get all values with invalid query keys');
