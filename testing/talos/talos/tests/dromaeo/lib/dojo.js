...
dojo.isObject = function( it){
	...
	return it !== undefined &&
		(it === null || typeof it == "object" || dojo.isArray(it) || dojo.isFunction(it)); 
}
...
dojo.declare = function( className,  superclass,  props){
	...
	if(mixins){
		dojo.forEach(mixins, function(m){
			if(!m){ throw(className + ": mixin #" + i + " is null"); } 
	...
	dojo.extend(ctor, {declaredClass: className, _constructor: props.constructor});
...
(function(){
	...
	dojo.hasClass = function(node, classStr){
		...
		return ((" "+ d.byId(node)[_className] +" ").indexOf(" "+ classStr +" ") >= 0);  
	};
...
(function(){
	...
	dojo.extend(dojo.NodeList, {
		...
		place: function( queryOrNode,  position){
			...
			return this.forEach(function(i){ d.place(i, item, position); }); 
		},
