...
struct Bar {
  MOZ_LIFETIME_BOUND const Foo &AsFoo() const; 
  MOZ_LIFETIME_BOUND operator const Foo &() const; 
};
...
void testNoLifetimeExtension2() {
  const auto &foo = static_cast<const Foo &>(MakeBar()); 
}

void testNoLifetimeExtension3() {
  const Foo &foo = Bar().AsFoo(); 
}

void testNoLifetimeExtension4(bool arg) {
  const Foo foo;
  const Foo &fooRef = arg ? foo : Bar().AsFoo(); 
}
