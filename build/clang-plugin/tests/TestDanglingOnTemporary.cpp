...
class AnnotateConflict {
  ...
  MOZ_NO_DANGLING_ON_TEMPORARIES int test() { return 0; } 
};
...
int *glob; 
void EscapeFunction1(int *test) { glob = test; } 

void EscapeFunction2(int *test, int *&escape) { escape = test; } 


int *EscapeFunction3(int *test) { return test; } 


int main() {
  int *test = TemporaryFunction().get(); 
  int *test2 = NS_ConvertUTF8toUTF16().get(); 

  UndefinedFunction(NS_ConvertUTF8toUTF16().get());

  NoEscapeFunction(TemporaryFunction().get());
  EscapeFunction1(TemporaryFunction().get()); 

  int *escape;
  EscapeFunction2(TemporaryFunction().get(), escape); 
  int *escape2 = EscapeFunction3(TemporaryFunction().get()); 
}
