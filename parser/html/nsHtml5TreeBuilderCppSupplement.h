    ...
    switch (aNamespace) {
      case kNameSpaceID_XHTML:
        if (nsGkAtoms::img == aName) {
          ...
          if (kind == Some(ScriptKind::eClassic)) {
            if (aAttributes->contains(nsHtml5AttributeName::ATTR_NOMODULE)) {
              ...
              nsAutoString forString, eventString;
          ...
          } else if (kind == Some(ScriptKind::eImportMap)) {
          ...
          if (nsHtml5String src;
              ((kind == Some(ScriptKind::eModule) && !mHasSeenImportMap) ||
               kind == Some(ScriptKind::eClassic)) &&
          ...
          mCurrentHtmlScriptCannotDocumentWriteOrBlock =
              kind != Some(ScriptKind::eClassic) || async || defer;
