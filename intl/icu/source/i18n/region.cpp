...
static const char16_t UNKNOWN_REGION_ID [] = { 0x5A, 0x5A, 0 };  
static const char16_t OUTLYING_OCEANIA_REGION_ID [] = { 0x51, 0x4F, 0 };  
static const char16_t WORLD_ID [] = { 0x30, 0x30, 0x31, 0 };  
...
void U_CALLCONV Region::loadRegionData(UErrorCode &status) {
    ...
    uhash_setValueDeleter(newRegionIDMap.getAlias(), uprv_deleteUObject);  
            ...
            for (int32_t i = 0 ; i < aliasTo.length() && U_SUCCESS(status); i++ ) {
                ...
                if ( aliasTo.charAt(i) == 0x0020 || i+1 == aliasTo.length() ) {
                    ...
                    if (target) {
                        ...
                        aliasFromRegion->preferredValues->adoptElement(preferredValue.orphan(),status);  
                    }
