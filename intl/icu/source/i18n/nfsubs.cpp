...
public:
    ...
    virtual double calcUpperBound(double ) const { return static_cast<double>(divisor); }
...
public:
    ...
    virtual void doSubstitution(double number, UnicodeString& toInsertInto, int32_t pos, int32_t recursionCount, UErrorCode& status) const;
    virtual void doSubstitution(int64_t , UnicodeString& , int32_t , int32_t , UErrorCode& ) const {}
                               ...
                               Formattable& result) const
{
    ...
    if (withZeros) {
        ...
        while (workText.length() > 0 && workPos.getIndex() != 0) {
            workPos.setIndex(0);
            getRuleSet()->parse(workText, workPos, 1, nonNumericalExecutedRuleMask, temp); 
            if (workPos.getIndex() == 0) {
