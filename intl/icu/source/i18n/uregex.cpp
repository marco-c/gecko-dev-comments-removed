...
uregex_getText(URegularExpression *regexp2,
    ...
    if (regexp->fText == nullptr) {
        ...
        if (UTEXT_FULL_TEXT_IN_CHUNK(inputText, inputNativeLength)) {
            ...
            UErrorCode lengthStatus = U_ZERO_ERROR;
            regexp->fTextLength = utext_extract(inputText, 0, inputNativeLength, nullptr, 0, &lengthStatus); 
...
#if 0
static void copyString(char16_t     *destBuffer,    
                       int32_t       destCapacity,  
                       int32_t      *destIndex,     
                                                    
                       const char16_t  *srcPtr,        
                       int32_t       srcLen)        
{
    int32_t  si;
