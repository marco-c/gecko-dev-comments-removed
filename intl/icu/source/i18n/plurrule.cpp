...
PluralRules::PluralRules(UErrorCode& )
:   UObject(),
    mRules(nullptr),
...
double
PluralRules::getUniqueKeywordValue(const UnicodeString& ) {
  
  return UPLRULES_NO_UNIQUE_VALUE;
}

int32_t
PluralRules::getAllKeywordValues(const UnicodeString & , double * ,
                                 int32_t , UErrorCode& error) {
...
static const double powers10[7] = {1.0, 10.0, 100.0, 1000.0, 10000.0, 100000.0, 1000000.0}; 
        ...
        case tEqual:
            {
                ...
                curAndConstraint->rangeList->addElement(-1, status);  
                curAndConstraint->rangeList->addElement(-1, status);  
                rangeLowIdx = 0;
                rangeHiIdx  = 1;
...
void
PluralKeywordEnumeration::reset(UErrorCode& ) {
    pos=0;
}

int32_t
PluralKeywordEnumeration::count(UErrorCode& ) const {
