...
static const int32_t MAX_IMPLICIT_STRING_LENGTH = 40;  
...
StringResource::handlePreflightStrings(SRBRoot *bundle, UHashtable *stringSet,
    ...
    if (bundle->fStringsForm != STRINGS_UTF16_V1) {
        ...
        bundle->f16BitStringsLength += fNumCharsForLength + len + 1;  
    }
}
...
void
SResource::handlePreflightStrings(SRBRoot * , UHashtable * ,
                                  UErrorCode & ) {
...
void
StringResource::handleWrite16(SRBRoot * ) {
    SResource *same;
    if ((same = fSame) != NULL) {
...
void
PseudoListResource::handleWrite16(SRBRoot * ) {
    fRes = URES_MAKE_EMPTY_RESOURCE(URES_TABLE);
    fWritten = TRUE;
}
...
void
SResource::handleWrite16(SRBRoot * ) {
...
void
SResource::handlePreWrite(uint32_t * ) {
    assert(FALSE);
}
...
void
SResource::handleWrite(UNewDataMemory * , uint32_t * ) {
    assert(FALSE);
}
...
void SRBRoot::write(const char *outputDir, const char *outputPkg,
    ...
    indexes[URES_INDEX_LENGTH] |= fPoolStringIndexLimit << 8;  
    indexes[URES_INDEX_ATTRIBUTES] |= (fPoolStringIndexLimit >> 12) & 0xf000;  
    ...
    for (int32_t i = 0; i < count;) {
        ...
        for (j = i + 1; j < count; ++j) {
            ...
            if (res->fString.endsWith(suffixRes->fString)) {
                assert(res->length() != suffixRes->length());  
