...
LocaleDisplayNamesImpl::initialize() {
    ...
    if (pattern.indexOf((char16_t)0xFF08) >= 0) {
        formatOpenParen.setTo((char16_t)0xFF08);         
        formatReplaceOpenParen.setTo((char16_t)0xFF3B);  
        formatCloseParen.setTo((char16_t)0xFF09);        
        formatReplaceCloseParen.setTo((char16_t)0xFF3D); 
    } else {
