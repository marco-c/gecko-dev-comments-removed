...
LocaleDisplayNamesImpl::initialize() {
    ...
    if (pattern.indexOf(static_cast<char16_t>(0xFF08)) >= 0) {
        formatOpenParen.setTo(static_cast<char16_t>(0xFF08));         
        formatReplaceOpenParen.setTo(static_cast<char16_t>(0xFF3B));  
        formatCloseParen.setTo(static_cast<char16_t>(0xFF09));        
        formatReplaceCloseParen.setTo(static_cast<char16_t>(0xFF3D)); 
    } else {
...
LocaleDisplayNamesImpl::localeIdName(const char* localeId,
    ...
    if (result.isBogus() && substitute) {
        ...
        result.setTo(UnicodeString(localeId, -1, US_INV)); 
    }
    return result;
}
...
LocaleDisplayNamesImpl::languageDisplayName(const char* lang,
    ...
    if (result.isBogus() && substitute == UDISPCTX_SUBSTITUTE) {
        ...
        result.setTo(UnicodeString(lang, -1, US_INV)); 
    }
    return adjustForUsageAndContext(kCapContextUsageLanguage, result);
}
...
LocaleDisplayNamesImpl::keyValueDisplayName(const char* key,
    ...
    if (uprv_strcmp(key, "currency") == 0) {
        ...
        const char16_t *currencyName = ucurr_getName(ustrValue.getTerminatedBuffer(),
            locale.getBaseName(), UCURR_LONG_NAME, nullptr , &len, &sts);
        if (U_FAILURE(sts)) {
