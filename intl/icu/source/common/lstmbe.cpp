    ...
    Array1D& ifco)
{
    ...
    ifco.slice(0*hunits, hunits).sigmoid();  
    ifco.slice(1*hunits, hunits).sigmoid(); 
    ifco.slice(2*hunits, hunits).tanh(); 
    ifco.slice(3*hunits, hunits).sigmoid(); 
...
LSTMBreakEngine::divideUpDictionaryRange( UText *text,
    ...
    Array1D backwardRow = fbRow.slice(hunits, hunits);  
