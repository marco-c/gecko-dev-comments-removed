...
public:
    ...
    inline CodePointsIterator &operator++() {  
        ++c_;
    ...
    inline CodePointsIterator operator++(int) {  
...
public:
    ...
    U_FORCE_INLINE static void inc(UnitIter &p, const LimitIter &) {
        ++p;
    }

    U_FORCE_INLINE static void dec(UnitIter , UnitIter &p) {
        --p;
    }

    U_FORCE_INLINE static CodeUnits<CP32, UnitIter> readAndInc(
            UnitIter &p0, UnitIter &p, const LimitIter &) {
        constexpr bool isMultiPass = prv::forward_iterator<UnitIter>;
        uint32_t uc = *p;
        CP32 c = uc;
        ++p;
    ...
    U_FORCE_INLINE static CodeUnits<CP32, UnitIter> decAndRead(UnitIter , UnitIter &p) {
        UnitIter p0 = p;
        uint32_t uc = *--p;
        CP32 c = uc;
        if (uc < 0xd800 || (0xe000 <= uc && uc <= 0x10ffff)) {
...
public:
    ...
    U_FORCE_INLINE Proxy operator++(int) {  
        if (ahead_) {
...
public:
    ...
    U_FORCE_INLINE reverse_iterator &operator++() {  
        if (state_ < 0) {
    ...
    U_FORCE_INLINE reverse_iterator operator++(int) {  
        if (state_ < 0) {
