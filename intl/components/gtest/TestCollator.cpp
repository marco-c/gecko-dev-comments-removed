...
TEST(IntlCollator, SetOptionsSensitivity)
{
  ...
  optResult = collator->SetOptions(
      Collator::Options{.sensitivity = Collator::Sensitivity::Accent});
  ASSERT_TRUE(optResult.isOk());
  ASSERT_EQ(collator->CompareStrings(u"a", u"b"), -1);
  ...
  optResult = collator->SetOptions(
      Collator::Options{.sensitivity = Collator::Sensitivity::Case});
  ASSERT_TRUE(optResult.isOk());
  ASSERT_EQ(collator->CompareStrings(u"a", u"b"), -1);
  ASSERT_EQ(collator->CompareStrings(u"a", u"á"), 0);
  ...
  optResult = collator->SetOptions(
      Collator::Options{.sensitivity = Collator::Sensitivity::Variant});
  ASSERT_TRUE(optResult.isOk());
  ASSERT_EQ(collator->CompareStrings(u"a", u"b"), -1);
  ASSERT_EQ(collator->CompareStrings(u"a", u"á"), -1);
  ASSERT_EQ(collator->CompareStrings(u"a", u"A"), -1);
...
TEST(IntlCollator, IgnorePunctuation)
{
  ...
  auto optResult =
      collator->SetOptions(Collator::Options{.ignorePunctuation = true});
  ASSERT_TRUE(optResult.isOk());

  ASSERT_EQ(collator->CompareStrings(u"aa", u".bb"), -1);

  optResult =
      collator->SetOptions(Collator::Options{.ignorePunctuation = false});
  ASSERT_TRUE(optResult.isOk());
