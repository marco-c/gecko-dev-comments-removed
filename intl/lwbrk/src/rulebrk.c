...

{
	...
	if((t(0) & C) && (t(1) & (VR|MT)) && (c(2) != TH_THANTHAKHAT)) { 
		if((t(-1) & (VRS|VRX))  && c(1) == TH_SARA_I) return -1; 
		if(t(-1) & (V|M)) return 0; 
		if(t(-2) & VRS) return 0;	
		if(!(t(0) & C2) && c(1) == TH_SARA_I) {	
			if(t(-2) & VRX) return 0; 
			if(t(-2) & VC) return 0;	
		}
	}
	if((t(-1) & VRX) && (t(0) & CC)) return 0;				
	if((t(-2) & VRS) && (t(-1) & C) && (t(0) & (V|M))) return 0;

	
	if((t(0) & CX) && (t(1) & C2) && (c(2) != TH_THANTHAKHAT)) {
		if((t(-2) & A) && (t(-1) & CX)) return 0; 
		if((t(-2) & CX) && (t(-1) & MT)) return 0; 
	}
