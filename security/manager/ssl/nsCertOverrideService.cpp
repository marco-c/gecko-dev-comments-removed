...
NS_IMETHODIMP
nsCertOverride::GetAsciiHost( nsACString& aAsciiHost) {
  aAsciiHost = mAsciiHost;
  return NS_OK;
}

NS_IMETHODIMP
nsCertOverride::GetDbKey( nsACString& aDBKey) {
  aDBKey = mDBKey;
  return NS_OK;
}

NS_IMETHODIMP
nsCertOverride::GetPort( int32_t* aPort) {
  *aPort = mPort;
  return NS_OK;
}

NS_IMETHODIMP
nsCertOverride::GetIsTemporary( bool* aIsTemporary) {
  *aIsTemporary = mIsTemporary;
  return NS_OK;
}

NS_IMETHODIMP
nsCertOverride::GetHostPort( nsACString& aHostPort) {
...
nsCertOverride::GetOriginAttributes(
    JSContext* aCtx,  JS::MutableHandle<JS::Value> aValue) {
...
void nsCertOverrideService::GetHostWithPort(const nsACString& aHostName,
                                            int32_t aPort,
                                            nsACString& aRetval) {
  nsAutoCString keyString(aHostName);
  if (aPort == -1) {
    aPort = 443;
  }
  if (!keyString.IsEmpty()) {
    keyString.Append(':');
    keyString.AppendInt(aPort);
  }
