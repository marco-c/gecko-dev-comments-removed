...
class SSSTokenizer final : public Tokenizer {
 public:
  ...
  MOZ_MUST_USE bool ReadBool( bool& value) {
    uint8_t rawValue;
  ...
  MOZ_MUST_USE bool ReadState( SecurityPropertyState& state) {
    uint32_t rawValue;
    if (!ReadInteger(&rawValue)) {
      return false;
    }
  ...
  MOZ_MUST_USE bool ReadSource( SecurityPropertySource& source) {
    uint32_t rawValue;
    if (!ReadInteger(&rawValue)) {
      return false;
    }
...
nsresult nsSiteSecurityService::ProcessPKPHeader(
  ...
  if (certVerifier->VerifySSLServerCert(nssCert, now, nullptr,  
                                        host, certList, flags,
                                        Nothing(),  
                                        Nothing(),  
                                        Nothing(),  
...
NS_IMETHODIMP
nsSiteSecurityService::Observe(nsISupports* , const char* topic,
                               const char16_t* ) {
