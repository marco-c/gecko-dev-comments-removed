...
GenEcKeypair(const UniquePK11SlotInfo& aSlot,
      UniqueSECKEYPrivateKey& aPrivKey,
      UniqueSECKEYPublicKey& aPubKey,
             const nsNSSShutDownPreventionLock&)
{
  MOZ_ASSERT(aSlot);
  if (!aSlot) {
...
GetAttestationCertificate(const UniquePK11SlotInfo& aSlot,
                   UniqueSECKEYPrivateKey& aAttestPrivKey,
                   UniqueCERTCertificate& aAttestCert,
                          const nsNSSShutDownPreventionLock& locker)
{
  MOZ_ASSERT(aSlot);
  if (!aSlot) {
                        ...
                        const nsNSSShutDownPreventionLock&)
{
  ...
  UniqueSECItem wrappedKey(SECITEM_AllocItem( nullptr,
                                              nullptr,
                                             kWrappedKeyBufLen));
  if (!wrappedKey) {
      MOZ_LOG(gNSSTokenLog, LogLevel::Warning,
                        ...
                        uint32_t* aRegistrationLen)
{
  ...
  UniqueSECItem signatureItem(::SECITEM_AllocItem( nullptr,
                                                   nullptr, 0));
  if (!signatureItem) {
    return NS_ERROR_OUT_OF_MEMORY;
  }
