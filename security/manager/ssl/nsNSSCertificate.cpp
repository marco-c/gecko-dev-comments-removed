...
NS_IMETHODIMP
nsNSSCertList::AsPKCS7Blob( nsACString& result)
{
  ...
  nsresult rv = ForEachCertificateInChain(
    [&cmsg, &sigd] (nsCOMPtr<nsIX509Cert> aCert, bool ,
             bool& ) {
...
nsresult
nsNSSCertList::SegmentCertificateChain( nsCOMPtr<nsIX509Cert>& aRoot,
...
nsresult
nsNSSCertList::GetRootCertificate( nsCOMPtr<nsIX509Cert>& aRoot)
{
  if (aRoot) {
    return NS_ERROR_UNEXPECTED;
  }
