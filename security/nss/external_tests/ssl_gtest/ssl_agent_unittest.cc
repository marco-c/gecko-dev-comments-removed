...
void MakeTrivialHandshakeMessage(uint8_t hs_type, size_t hs_len,
  ...
  size_t index = 0;
  out->Write(index, kTlsHandshakeType, 1); ++index; 
  out->Write(index, 3, 1); ++index; 
  out->Write(index, 1, 1); ++index; 
  out->Write(index, 4 + hs_len, 2); index += 2; 

  out->Write(index, hs_type, 1); ++index; 
  out->Write(index, hs_len, 3); index += 3; 
