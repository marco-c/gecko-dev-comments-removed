...
static int configureDHE = -1; 
static int configureReuseECDHE = -1; 
static int configureWeakDHE = -1; 
		       ...
		       CERTCertificate *cert, secuPWData *pwdata)
{
    ...
    nextUpdate = now + (PRTime)60*60*24 * PR_USEC_PER_SEC; 
	...
	case osm_revoked:
		...
		&nextUpdate,
		now - (PRTime)60*60*24 * PR_USEC_PER_SEC, 
		NULL);
	    break;
	default:
	    PORT_Assert(0);
