...
verifyFromSideChannel(CERTCertificate *cert, ServerCertAuth *sca)
{
    ...
    revTestsOverallSoftFail.cert_rev_flags_per_method = 0; 
    ...
    revTestsOverallHardFail.cert_rev_flags_per_method = 0; 
...
PRBool clientCertAsyncSelect = PR_TRUE; 
PRBool clientCertIsBlocked = PR_FALSE;  
...
run()
{
    ...
    if (actAsServer) {
        rv = SSL_ResetHandshake(s, PR_TRUE );
        if (rv != SECSuccess) {
            return 1;
        }
    } else {
