...
tls13_HandleClientKeyShare(sslSocket *ss)
{
    ...
    SECStatus rv;
    TLS13KeyShareEntry *peerShare = NULL; 
    sslEphemeralKeyPair *keyPair;         
    PRCList *cur_p;
...
SECStatus
tls13_AddContextToHashes(sslSocket *ss, SSL3Hashes *hashes ,
                        ...
                        TrafficKeyType type)
{
    ...
    size_t ivSize = spec->cipher_def->iv_size +
                    spec->cipher_def->explicit_nonce_size; 
                    ...
                    sslBuffer *wrBuf)
{
    ...
    if (cipher_def->calg == ssl_calg_null) {
        ...
        rv = cwSpec->aead(
            ...
            wrBuf->buf + headerLen, contentLen + 1, 
            aad, sizeof(aad));
        if (rv != SECSuccess) {
