...
TEST_P(GreasePresenceAbsenceTestAllVersions, GreaseClientHelloExtension) {
  ...
  EXPECT_TRUE(extParser.Skip(2 + 32));     
  EXPECT_TRUE(extParser.SkipVariable(1));  
  if (variant_ == ssl_variant_datagram) {
    EXPECT_TRUE(extParser.SkipVariable(1));  
  }
  EXPECT_TRUE(extParser.SkipVariable(2));  
  EXPECT_TRUE(extParser.SkipVariable(1));  
  EXPECT_TRUE(extParser.Skip(2));          
