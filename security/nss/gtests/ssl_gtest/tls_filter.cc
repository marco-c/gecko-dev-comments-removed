...
bool TlsHandshakeFilter::HandshakeHeader::ReadLength(
  ...
  if (!parser->Read(&message_seq_tmp, 2)) {  
    return false;
  }
...
PacketFilter::Action SelectiveRecordDropFilter::FilterRecord(
 ...
 uint32_t SelectiveRecordDropFilter::ToPattern(
...
PacketFilter::Action ClientHelloPreambleCapture::FilterHandshake(
  ...
  DataBuffer temp;
  TlsParser parser(input);
  EXPECT_TRUE(parser.Read(&temp, 2 + 32));     
  EXPECT_TRUE(parser.ReadVariable(&temp, 1));  
  if (is_dtls_agent()) {
    EXPECT_TRUE(parser.ReadVariable(&temp, 1));  
  }
  EXPECT_TRUE(parser.ReadVariable(&temp, 2));  
  EXPECT_TRUE(parser.ReadVariable(&temp, 1));  
...
PacketFilter::Action ClientHelloCiphersuiteCapture::FilterHandshake(
  ...
  EXPECT_TRUE(parser.SkipVariable(1));  
  if (is_dtls_agent()) {
    EXPECT_TRUE(parser.SkipVariable(1));  
  }

  EXPECT_TRUE(parser.ReadVariable(&data_, 2));  
