...
TEST_F(TlsConnectTest, GatherExcessiveV3Record) {
  ...
  idx = buffer.Write(idx, 0x16, 1);                            
  idx = buffer.Write(idx, 0x0301, 2);                          
  (void)buffer.Write(idx, MAX_FRAGMENT_LENGTH + 2048 + 1, 2);  
...
TEST_F(GatherV2ClientHelloTest, GatherV2RecordLongHeader) {
  ...
  size_t idx = 0;
  idx = buffer.Write(idx, 0x0002, 2);  
  idx = buffer.Write(idx, 0U, 1);      
  (void)buffer.Write(idx, 0U, 2);      
...
TEST_F(GatherV2ClientHelloTest, GatherV2RecordLongHeader2) {
  ...
  size_t idx = 0;
  idx = buffer.Write(idx, 0x0001, 2);  
  idx = buffer.Write(idx, 0U, 1);      
  idx = buffer.Write(idx, 0U, 1);      
  (void)buffer.Write(idx, 0U, 1);      
...
TEST_F(GatherV2ClientHelloTest, GatherV2RecordShortHeader) {
  ...
  size_t idx = 0;
  idx = buffer.Write(idx, 0x8003, 2);  
  (void)buffer.Write(idx, 0U, 3);      
