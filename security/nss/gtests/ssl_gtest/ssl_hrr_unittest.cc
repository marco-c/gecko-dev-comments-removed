...
static void DisableSuiteFromHrr(
  ...
  size_t offset = 2 + 32;  
  ASSERT_TRUE(
      capture_hrr->buffer().Read(offset, 1, &tmp));  
  EXPECT_EQ(0U, tmp);
  offset += 1 + tmp;
  ASSERT_TRUE(capture_hrr->buffer().Read(offset, 2, &tmp));  
  EXPECT_EQ(
      SECSuccess,
...
class HelloRetryRequestAgentTest : public TlsAgentTestClient {
 protected:
  ...
  void MakeCannedHrr(const uint8_t* body, size_t len, DataBuffer* hrr_record,
    ...
    i = hrr_data.Write(i, static_cast<uint32_t>(0), 1);  
