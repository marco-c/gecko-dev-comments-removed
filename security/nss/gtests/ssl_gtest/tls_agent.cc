...
void TlsAgent::SetState(State s) {
 ...
 bool TlsAgent::LoadCertificate(const std::string& name,
 ...
 bool TlsAgent::LoadKeyPairFromCert(const std::string& name,
...
void TlsAgentTestBase::MakeHandshakeMessageFragment(
  ...
  index = out->Write(index, hs_type, 1);  
  index = out->Write(index, hs_len, 3);   
  if (variant_ == ssl_variant_datagram) {
...
void TlsAgentTestBase::MakeTrivialHandshakeRecord(uint8_t hs_type,
                                                  ...
                                                  DataBuffer* out) {
  size_t index = 0;
  index = out->Write(index, ssl_ct_handshake, 1);  
  index = out->Write(index, 3, 1);                 
  index = out->Write(index, 1, 1);                 
  index = out->Write(index, 4 + hs_len, 2);        
