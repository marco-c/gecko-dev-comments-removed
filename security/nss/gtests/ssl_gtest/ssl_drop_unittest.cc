...
class TlsFragmentationAndRecoveryTest : public TlsDropDatagram13 {
 ...
 protected:
  void RunTest(size_t dropped_half) {
    ...
    EXPECT_LE(3U, final_server_flight_count);  
 ...
 private:
  void FirstFlightDropCertificate() {
    ...
    server_filters_.drop_->Reset(1 << 2);  
    server_->Handshake();
    EXPECT_EQ(5U, server_filters_.records_->count());  
  ...
  void SecondAttemptDropHalf(size_t dropped_half) {
    ...
    server_filters_.records_->Clear();
    server_filters_.drop_->Reset({dropped_half});  
    SplitServerMtu(2);
    server_->Handshake();
    EXPECT_LE(4U, server_filters_.records_->count());  
  ...
  void ThirdAttemptDropMiddle() {
    ...
    ShiftDtlsTimers();
    server_->Handshake();
    EXPECT_LE(5U, server_filters_.records_->count());  
  }
  ...
  void SplitServerMtu(size_t count) {
    ...
    static const size_t actual_overhead = 12 +  
                                          1 +   
                                          16;   
...
TEST_P(TlsReorderDatagram13, ReorderServerCertificate) {
  ...
  ShiftDtlsTimers();
  CheckedHandshakeSendReceive();
  EXPECT_EQ(2UL, server_filters_.records_->count());  
  CheckAcks(server_filters_, 0, {0x0002000000000000ULL});
}
...
TEST_P(TlsReorderDatagram13, DataAfterEOEDDuringZeroRtt) {
  ...
  EXPECT_EQ(1UL, client_filters_.records_->count());  
  server_->Handshake();
  client_->Handshake();
  ExpectEarlyDataAccepted(true);
  
  EXPECT_EQ(3UL, client_filters_.records_->count());  
  EXPECT_EQ(TlsAgent::STATE_CONNECTED, client_->state());
