...
TEST_F(LockstoreServiceTest, CreateAndDeleteDek) {
  ...
  RunOnBackground([&]() {
    EXPECT_NS_SUCCEEDED(
        mService->DoCreateDek(coll, mLocalKek, false));
...
TEST_F(LockstoreServiceTest, ListDeks_ContainsCreated) {
  ...
  RunOnBackground([&]() {
    EXPECT_NS_SUCCEEDED(
        mService->DoCreateDek(a, mLocalKek, false));
    EXPECT_NS_SUCCEEDED(
        mService->DoCreateDek(b, mLocalKek, false));
    EXPECT_NS_SUCCEEDED(
        mService->DoCreateDek(c, mLocalKek, false));
