...
static void PR_CALLBACK ClientThread(void* _action) {
  ...
  for (; iterations--;) {
    ...
    if (action != CLIENT_TIMEOUT_ACCEPT) {
      if ((rv = PR_Connect(sock, &serverAddr, timeoutTime)) < 0) {
        if (!debug_mode) {
          failed_already = 1;
        } else
          PR_fprintf(
      ...
      if (action != CLIENT_TIMEOUT_SEND) {
        if ((rv = PR_Send(sock, buf, CLIENT_DATA, 0, timeoutTime)) < 0) {
          if (!debug_mode) {
            failed_already = 1;
          } else {
            PR_fprintf(output,
...
static void RunTest(PRInt32 acceptType, PRInt32 clientAction) {
  int i;
  ...
  clientThread =
      PR_CreateThread(PR_USER_THREAD, ClientThread, (void*)&clientCommand,
  ...
  for (; iterations--;) {
    switch (acceptType) {
      case ACCEPT_NORMAL:
        clientSock = PR_Accept(listenSock, &clientAddr, timeoutTime);
        switch (clientAction) {
          ...
          case CLIENT_NORMAL:
            ...
            bytesRead = PR_Recv(clientSock, buf, CLIENT_DATA, 0, timeoutTime);
        ...
        break;
      case ACCEPT_READ:
        status = PR_AcceptRead(listenSock, &clientSock, &raddr, buf,
                               CLIENT_DATA, timeoutTime);
        switch (clientAction) {
          case CLIENT_TIMEOUT_ACCEPT:
...
#ifdef WINNT
      case ACCEPT_FAST:
        clientSock = PR_NTFast_Accept(listenSock, &clientAddr, timeoutTime);
        switch (clientAction) {
          case CLIENT_TIMEOUT_ACCEPT:
            TEST_ASSERT(clientSock == 0);
            if (debug_mode) {
      ...
      case ACCEPT_READ_FAST:
        status = PR_NTFast_AcceptRead(listenSock, &clientSock, &raddr, buf,
                                      4096, timeoutTime);
        switch (clientAction) {
          case CLIENT_TIMEOUT_ACCEPT:
      ...
      case ACCEPT_READ_FAST_CB:
        status = PR_NTFast_AcceptRead_WithTimeoutCallback(
            ...
            timeout_callback, (void*)CALLBACK_MAGIC);
        switch (clientAction) {
          case CLIENT_TIMEOUT_ACCEPT:
...
void AcceptUpdatedTest(void) { RunTest(ACCEPT_NORMAL, CLIENT_NORMAL); }
void AcceptNotUpdatedTest(void) { RunTest(ACCEPT_FAST, CLIENT_NORMAL); }
void AcceptReadTest(void) { RunTest(ACCEPT_READ, CLIENT_NORMAL); }
