...
static PRIntervalTime NonContentiousLock(PRInt32 loops) {
  ...
  while (loops-- > 0) {
    ...
    if (!_lockfile) {
      if (debug_mode)
        printf("could not create lockfile: %d [%d]\n", PR_GetError(),
               PR_GetOSError());
...
static PRIntervalTime Test(const char* msg,
                           PRIntervalTime (*test)(PRInt32 loops), PRInt32 loops,
                           PRIntervalTime overhead) {
...
int main(int argc, char** argv) {
  ...
  for (cpu = 1; cpu <= cpus; ++cpu) {
    ...
    duration = Test("LockFile non-contentious locking/unlocking",
                    NonContentiousLock, loops, 0);
    (void)Test("LockFile contentious locking/unlocking", ContentiousLock, loops,
               duration);
  }
