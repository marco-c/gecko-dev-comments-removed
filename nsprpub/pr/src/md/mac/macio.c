...
static OSErr CreateMacPathFromUnixPath(const char *unixPath, char **macPath)
{
	...
	*macPath = malloc(strlen(unixPath) + 2);	
	require_action (*macPath != NULL, exit, err = memFullErr;);

	src = unixPath;
	tgt = *macPath;
	
	if (PL_strchr(src, PR_DIRECTORY_SEPARATOR) == src)				
		src++;													
	else
		*(tgt++) = PR_PATH_SEPARATOR;	
		
	if (PL_strstr(src, UNIX_THIS_DIRECTORY_STR) == src)			
		src += 2;												
		
	while (*src) 
	{				
		if (PL_strstr(src, UNIX_PARENT_DIRECTORY_STR) == src) {	
			*(tgt++) = PR_PATH_SEPARATOR;						
			src +=3;
		}
		else if (*src == PR_DIRECTORY_SEPARATOR) {					
			*(tgt++) = PR_PATH_SEPARATOR;
			src++;
		}
		else
...
static OSErr FindGutsFolder(FSSpec *foundSpec)
{
	...
	if (gNavigatorProcInfo.processInfoLength == 0) { 
...
{	
	...
	if (*unixPath != PR_DIRECTORY_SEPARATOR) {				
		...
		if (err == noErr)
		{
			...
			err = FSpGetFullPath(&foundSpec, &pathBufferSize, &hPathStr);		
...
ConvertMacPathToUnixPath(const char *macPath, char **unixPath) 
{
	...
	*unixPath = malloc(strlen(macPath) + 2);	
	if (*unixPath == NULL)
		return (memFullErr);
...
{	
	...
	if (*unixPath != PR_DIRECTORY_SEPARATOR) {				
		...
		if (err == noErr)
			...
			err = GetFullPath(foundVRefNum, foundDirID, macPath, &pathBufferSize);		
