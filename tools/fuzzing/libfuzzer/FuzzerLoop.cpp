...
void Fuzzer::MutateAndTestOne() {
  ...
  for (int i = 0; i < Options.MutateDepth; i++) {
    ...
    bool NewCov = RunOne(CurrentUnitData, Size, true, &II,
                          false, &FoundUniqFeatures);
...
void Fuzzer::ReadAndExecuteSeedCorpora(std::vector<SizedFile> &CorporaFiles) {
  ...
  if (CorporaFiles.empty()) {
    ...
    for (auto &SF : CorporaFiles) {
      ...
      RunOne(U.data(), U.size(),  false,  nullptr,
              Options.KeepSeed,
              nullptr);
      CheckExitOnSrcPosOrItem();
  ...
  if (Corpus.empty() && Options.MaxNumberOfRuns) {
    ...
    Corpus.AddToCorpus({'\n'}, 1, true,
                       false, false,
                       duration_cast<microseconds>(0s), {0}, DFT,
                        nullptr);
  }
}
