...
void TracePC::SetFocusFunction(const std::string &FuncName) {
  ...
  for (size_t M = 0; M < NumModules; M++) {
    ...
    for (size_t I = 0; I < N; I++) {
      if (!(PcIsFuncEntry(&PCTE.Start[I]))) continue;  
...
void TracePC::HandleCmp(uintptr_t PC, T Arg1, T Arg2) {
  ...
  uint64_t HammingDistance = Popcountll(ArgXor);  
...
uintptr_t TracePC::GetMaxStackOffset() const {
  return InitialStack - __sancov_lowest_stack;  
}
...
void __sanitizer_weak_hook_memcmp(void *caller_pc, const void *s1,
  ...
  if (result == 0) return;  
  if (n <= 1) return;  
  fuzzer::TPC.AddValueForMemcmp(caller_pc, s1, s2, n, false);
}
...
void __sanitizer_weak_hook_strncmp(void *caller_pc, const char *s1,
  ...
  n = std::min(n, Len2);
  if (n <= 1) return;  
  fuzzer::TPC.AddValueForMemcmp(caller_pc, s1, s2, n, true);
}
