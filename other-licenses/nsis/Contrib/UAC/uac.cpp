...
#define UAC_HACK_Clammerz
#define UAC_HACK_FGWND1
...
UINT_PTR StrToUInt(LPTSTR s,bool ForceHEX=false,BOOL*pFoundBadChar=0) 
{
	...
	for (TCHAR c=*s; c; c=*(s=StrNextChar(s)) ) 
	{
		...
		{
			if (pFoundBadChar )*pFoundBadChar=true;
			break;
		}
		v*=base;v+=c;
	}
...
LRESULT CALLBACK MSRunAsDlgMod_ShellProc(int nCode,WPARAM wp,LPARAM lp) 
{
	...
	if (nCode >= 0 && (pCWPS=(CWPRETSTRUCT*)lp) && WM_INITDIALOG==pCWPS->message)
	{
		...
		{ 
			...
			if (!lstrcmpi(buf,_T("SysCredential"))) 
			{
...
{
	if(GetOSVerMaj()!=5 || GetOSVerMin()<1)return NULL;
...
FORCEINLINE DWORD MaintainDllSelfRef() 
{ 
...
#	endif
	...
	if (!g.hModAdvAPI) 
	{
		...
		dldprocs[]=
		{
			...
			{LoadLibraryA("SECUR32"),(void**)&_GetUserNameEx,"GetUserNameEx" __DLD_FUNCSUFFIX,true},
			#endif
			{0}
		};
...
DWORD _Exec(HWND hwnd,NSISCH*Verb,NSISCH*Exec,NSISCH*Params,NSISCH*WorkDir,WORD ShowWnd,bool Wait,bool UseCreateProcess) 
{
	...
	if (UseCreateProcess) 
	{
		...
		const NSISCH*Q=( (*Exec!='"') && (*Params) && StrContainsWhiteSpace(Exec)) ? _T("\"") : _T("");
	...
	else WaitForInputIdle(sei.hProcess,1500);
...
void HandleExecExport(bool CreateProc,bool Wait,HWND&hwndNSIS,int&StrSize,NSISCH*&Vars,stack_t**&StackTop,NSIS::extra_parameters*pXParams) 
{
	...
	WORD ShowWnd;
	IPC_SHEXEC*pISE=0;
	stack_t* const pSIVerb=CreateProc?0:StackPop();
...
void StopIPCSrv() 
{
	if (g.threadIPC) 
	{
		...
		UnregisterClass(GetIPCWndClass(),g.hInstance);
		g.hSrvIPC=0;
		g.threadIPC=0;
	}
}
...
DWORD ForkSelf(HWND hParent,DWORD&ForkExitCode,NSIS::extra_parameters*pXParams,UINT NSISStrLen) 
{
	...
	wsprintf(pszParamBuf,_T("/UAC:%X /NCRC%s"),g.hSrvIPC,p);
	...
	if ( GetSysVer()>=5 && !sei.hwnd ) 
	{
		...
		if (GetSysVer()<6 || MustUseInternalRunAs())
			...
			SetWindowLongPtr(hHack,GWL_EXSTYLE,GetWindowLongPtr(hHack,GWL_EXSTYLE)&~WS_EX_APPWINDOW);
...
#ifdef BUILD_XPTEST
	if (!UserIsAdmin)goto DbgXPAsUAC;
#endif
...
{
	NSISFUNCSTART(hwndNSIS,StrSize,Vars,StackTop,XParams);
	TOKEN_ELEVATION_TYPE tet=(TOKEN_ELEVATION_TYPE)NULL; 
	if (MaintainDllSelfRef() ||  _GetElevationType(&tet)) SetErrorFlag(XParams);
