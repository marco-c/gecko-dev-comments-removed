...
void MultiByteToUnicodeString2(UString &dest, const AString &src, UINT )
{
  dest.Empty();
  if (src.IsEmpty())
    return;
...
static void UnicodeStringToMultiByte2(AString &dest, const UString &src, UINT , char defaultChar, bool &defaultCharWasUsed)
{
  dest.Empty();
  defaultCharWasUsed = false;
  if (src.IsEmpty())
