...
HRESULT CInArchive::ReadBlockHeader(CHeader &h)
{
  ...
  const unsigned kBufSize = AES_BLOCK_SIZE + AES_BLOCK_SIZE; 
  Byte buf[kBufSize];
  unsigned filled;
...
STDMETHODIMP CHandler::GetArchiveProperty(PROPID propID, PROPVARIANT *value)
{
  ...
  switch (propID)
  {
    ...
    case kpidEncrypted: if (arcInfo) prop = arcInfo->IsEncrypted; break; 
  ...
  for (;;)
  {
    ...
    for (;;)
    {
      ...
      bool isOk_packSize = true;
      {
        ...
        {
          arch.AddToSeekValue(item.PackSize); 
          arcInfo.EndPos = arch.Position;
        }
      }
