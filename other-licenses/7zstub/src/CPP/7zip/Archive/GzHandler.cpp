...
namespace NGz {
  ...
  static const Byte kSignature_2 = 8; 
...
static HRESULT ReadUInt16(NDecoder::CCOMCoder *stream, UInt32 &value )
{
  value = 0;
  for (int i = 0; i < 2; i++)
  {
...
static HRESULT ReadString(NDecoder::CCOMCoder *stream, AString &s, size_t limit )
{
  s.Empty();
...
HRESULT CItem::ReadHeader(NDecoder::CCOMCoder *stream)
{
  ...
  if (NameIsPresent())
    RINOK(ReadString(stream, Name, kNameMaxLen ));
  if (CommentIsPresent())
    RINOK(ReadString(stream, Comment, kCommentMaxLen ));

  if (HeaderCrcIsPresent())
  {
    UInt32 headerCRC;
    
    RINOK(ReadUInt16(stream, headerCRC ));
