...
static const unsigned CMD_REF_OnFunc_NumShifts = 28; 
static const unsigned CMD_REF_Page_NumShifts = 16; 
...
bool IsDrivePath(const wchar_t *s) { return IS_LETTER_CHAR(s[0]) && s[1] == ':'  ; }
bool IsDrivePath(const char *s)    { return IS_LETTER_CHAR(s[0]) && s[1] == ':'  ; }
...
HRESULT CInArchive::ReadEntries(const CBlockHeader &bh)
{
  ...
  UInt32 overwrite_State = 0; 
  Int32 allowSkipFiles_State = -1; 
  ...
  for (kkk = 0; kkk < bh.Num; kkk++, p += kCmdSize)
  {
    ...
    switch (commandId)
    {
      ...
      case EW_ASSIGNVAR:
      {
        if (params[0] == spec_outdir_VarIndex)
        {
          ...
          if (IsVarStr(params[1], kVar_OUTDIR) &&
              ...
              params[3] == 0)
          {
            spec_outdir_U = UPrefixes.Back(); 
            spec_outdir_A = APrefixes.Back(); 
          }
        }
        ...
        if (IsVarStr(par1, 10)) 
        {
          ...
          if (kkk > kBackOffset)
          {
            ...
            if (cmd == EW_ASSIGNVAR)
            {
              ...
              if (pars[0] == 10 + 4 && pars[2] == 0 && pars[3] == 0) 
              {
                item.Prefix = -1;
                item.NameA.Empty();
                item.NameU.Empty();
      ...
      case EW_SETFILEATTRIBUTES:
      {
        ...
        {
          if (params[0] == Get32(p - kCmdSize + 4 + 4 * 1)) 
          {
      ...
      case EW_PUSHPOP:
      {
        ...
        else
        {
          if (NoLabels(labels + kkk + 1, 2)
              && Get32(p + kCmdSize) == EW_PUSHPOP 
              && GET_CMD_PARAM(p + kCmdSize, 2) == 1
              && Get32(p + kCmdSize * 2) == EW_PUSHPOP 
      ...
      case EW_CREATESHORTCUT:
      {
        ...
        AddOptionalParam(params[5]); 
        break;
      }

      case EW_COPYFILES:
      {
        if (params[2] & 0x04) s += " /SILENT"; 
        if (params[2] & 0x80) s += " /FILESONLY"; 
        AddParams(params, 2);
        if (params[3] != 0)
        {
          s += "    ;";
...
HRESULT CInArchive::Parse()
{
  ...
  if (license_bg != (UInt32)(Int32)-1 && license_bg != -15) 
  {
    Script += "LicenseBkColor";
    if ((Int32)license_bg == -5)  
      Script += " /windows";
