...
static const unsigned k_NumMidBits = 9; 
...
bool CHeader::Parse(const Byte *p)
{
  ...
  if (Is_NL() && Get32(p + 0x49) != 0x0A0D200A) 
    return false;
    ...
    unsigned numVols, unsigned volIndex, UInt64 &complexity)
{
  ...
  if (grainSize_Log < 3 || grainSize_Log > 30 - 9) 
    return S_FALSE;
...
STDMETHODIMP CHandler::GetStream(UInt32 , ISequentialInStream **stream)
{
  COM_TRY_BEGIN
  *stream = 0;
