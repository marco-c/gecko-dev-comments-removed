...
{
   CF_OP2 | CF_BYTEMODE,
   CF_OP2 | CF_BYTEMODE | CF_CHFLAGS,
   CF_OP2 | CF_BYTEMODE | CF_CHFLAGS,
   CF_OP2 | CF_BYTEMODE | CF_CHFLAGS,
   CF_OP1 | CF_JUMP | CF_USEFLAGS,
   CF_OP1 | CF_JUMP | CF_USEFLAGS,
   CF_OP1 | CF_BYTEMODE | CF_CHFLAGS,
   CF_OP1 | CF_BYTEMODE | CF_CHFLAGS,
   CF_OP1 | CF_JUMP,
   CF_OP2 | CF_BYTEMODE | CF_CHFLAGS,
   CF_OP2 | CF_BYTEMODE | CF_CHFLAGS,
   ...
   CF_OP2 | CF_BYTEMODE,
   CF_OP2 | CF_BYTEMODE,
   CF_OP2 | CF_BYTEMODE | CF_USEFLAGS | CF_CHFLAGS ,
   CF_OP2 | CF_BYTEMODE | CF_USEFLAGS | CF_CHFLAGS ,
   CF_OP0
};
...
bool CVm::ExecuteStandardFilter(unsigned filterIndex)
{
  ...
  switch (filterType)
  {
    ...
    case SF_RGB:
    {
      if (dataSize >= kGlobalOffset / 2 || dataSize < 3) 
        return false;
      UInt32 width = R[0];
      UInt32 posR = R[1];
      if (width < 3 || width - 3 > dataSize || posR > 2) 
        return false;
      SetBlockPos(dataSize);
