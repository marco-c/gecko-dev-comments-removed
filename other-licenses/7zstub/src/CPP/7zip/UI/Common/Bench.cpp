...
UInt64 GetBenchMemoryUsage(UInt32 numThreads, UInt32 dictionary, bool totalBench)
{
  ...
  const UInt32 kCompressedBufferSize = kBufferSize; 
...
struct CTotalBenchRes
{
  ...
  UInt64 NumIterations2; 

  UInt64 Rating;
  UInt64 Usage;
  UInt64 RPU;
  
  void Init() {  NumIterations2 = 0; Rating = 0; Usage = 0; RPU = 0; }
...
void GetCpuFeatures(AString &s)
{
  ...
  #ifdef _WIN32
  const unsigned kNumFeatures_Extra = 32; 
    ...
    for (UInt32 i = 0; i < numIterations; i++)
    {
      ...
      for (unsigned freqTest = 0; freqTest < kNumCpuTests; freqTest++)
      {
        ...
        RINOK(FreqBench(complexInCommands, numThreads, printCallback,
            (freqTest == kNumCpuTests - 1 || specifiedFreq != 0), 
            specifiedFreq,
            cpuFreq, resVal));
        callback.NewLine();
      ...
      if (!onlyHashBench)
      {
        res = TotalBench(EXTERNAL_CODECS_LOC_VARS
            ...
            dictIsDefined || fileDataBuffer.Buffer, 
