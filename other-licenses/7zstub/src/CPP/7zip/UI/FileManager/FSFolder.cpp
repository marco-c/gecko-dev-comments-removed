...
STDMETHODIMP CFSFolder::GetParent(UInt32 , UInt32 * , UInt32 * )
{
  return E_FAIL;
}
...
STDMETHODIMP_(Int32) CFSFolder::CompareItems(UInt32 index1, UInt32 index2, PROPID propID, Int32 )
{
  ...
  switch (propID)
  {
    ...
    case kpidIsDir:
    {
      bool isDir1 =  fi1.IsDir();
      bool isDir2 =  fi2.IsDir();
      if (isDir1 == isDir2)
        return 0;
      return isDir1 ? -1 : 1;
    }
...
STDMETHODIMP CFSFolder::CreateFolder(const wchar_t *name, IProgress * )
{
  FString absPath;
...
STDMETHODIMP CFSFolder::CreateFile(const wchar_t *name, IProgress * )
{
  FString absPath;
  GetAbsPath(name, absPath);
...
STDMETHODIMP CFSFolder::Rename(UInt32 index, const wchar_t *newName, IProgress * )
{
  if (index >= (UInt32)Files.Size())
    return E_NOTIMPL;
  const CDirItem &fi = Files[index];
...
STDMETHODIMP CFSFolder::SetProperty(UInt32 index, PROPID propID,
    const PROPVARIANT *value, IProgress * )
{
  if (index >= (UInt32)Files.Size())
    return E_INVALIDARG;
  CDirItem &fi = Files[index];
