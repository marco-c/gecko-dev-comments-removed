...
bool CPanel::OnCreate(CREATESTRUCT * )
{
...
bool CPanel::OnSize(WPARAM , int xSize, int ySize)
{
  if ((HWND)*this == 0)
    return true;
  if (_headerReBar)
...
bool CPanel::OnNotifyReBar(LPNMHDR header, LRESULT & )
{
  switch (header->code)
  {
...
bool CPanel::OnNotify(UINT , LPNMHDR header, LRESULT &result)
{
