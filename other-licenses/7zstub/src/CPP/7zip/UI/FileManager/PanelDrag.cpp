...
public:
  ...
  STDMETHODIMP GetCanonicalFormatEtc ( LPFORMATETC , LPFORMATETC pformatetcOut)
  ...
  STDMETHODIMP DAdvise(FORMATETC * , DWORD , LPADVISESINK , DWORD * )
...
STDMETHODIMP CDataObject::SetData(LPFORMATETC etc, STGMEDIUM *medium, BOOL )
{
...
void CPanel::OnDrag(LPNMLISTVIEW )
{
...
DWORD CDropTarget::GetEffect(DWORD keyState, POINTL , DWORD allowedEffect)
{
...
void CPanel::CompressDropFiles(const UStringVector &fileNames, const UString &folderPath)
{
  ...
  if (createNewArchive)
  {
    ...
    CompressFiles(folderPath2, archiveName, L"",
      ...
      false, 
      true, 
      AreThereNamesFromTemp(fileNames) 
      );
  }
  else
