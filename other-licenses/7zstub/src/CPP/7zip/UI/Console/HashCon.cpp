...
HRESULT CHashCallbackConsole::ScanProgress(const CDirItemsStat &st, const FString &path, bool )
{
  if (NeedPercents())
  {
...
HRESULT CHashCallbackConsole::SetNumFiles(UInt64 )
{
  return CheckBreak2();
}
...
HRESULT CHashCallbackConsole::GetStream(const wchar_t *name, bool )
{
  _fileName = name;
