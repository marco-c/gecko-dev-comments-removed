...
HRESULT CUpdateCallbackConsole::ScanProgress(const CDirItemsStat &st, const FString &path, bool )
{
  if (NeedPercents())
  {
...
HRESULT CUpdateCallbackConsole::DeletingAfterArchiving(const FString &path, bool )
{
  if (LogLevel > 0 && _so)
  {
...
HRESULT CUpdateCallbackConsole::GetStream(const wchar_t *name, bool , bool isAnti, UInt32 mode)
{
  if (StdOutMode)
    return S_OK;
...
HRESULT CUpdateCallbackConsole::ReportUpdateOpeartion(UInt32 op, const wchar_t *name, bool )
{
...
HRESULT CUpdateCallbackConsole::ShowDeleteFile(const wchar_t *name, bool )
{
  if (StdOutMode)
    return S_OK;
