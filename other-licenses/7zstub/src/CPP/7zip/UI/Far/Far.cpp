...
STDMETHODIMP COpenArchiveCallback::SetTotal(const UInt64 )
{
  return CheckBreak2();
}

STDMETHODIMP COpenArchiveCallback::SetCompleted(const UInt64 * )
{
  ShowMessage();
  return CheckBreak2();
}
...
EXTERN_C HANDLE WINAPI OpenFilePlugin(char *name, const unsigned char * , int )
{
  MY_TRY_BEGIN;
...
EXTERN_C int WINAPI Configure(int )
{
  MY_TRY_BEGIN;
