...

            {
              ...
              if (parse.state != MTDEC_PARSE_CONTINUE || finish)
              {
                ...
                if (parse.state == MTDEC_PARSE_END)
                {
                  ...
                  if (crossSize != 0)
                    memcpy(data + parse.srcSize, parseData + parse.srcSize, size - parse.srcSize); 
...

  {
    ...
    if (needWrite)
    {
      ...
      res = p->mtCallback->Write(p->mtCallbackObject, t->index,
          res == SZ_OK && needWriteToStream && !wasInterrupted, 
